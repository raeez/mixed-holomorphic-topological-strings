...
topology.  The cochain identity (level~1) holds unconditionally; the
...
(level~5) holds on an acceptance datum.
...
\item Output: if yes, the unique correction \(C^+_{a,b}\) with
\(B_{a,b}C^+_{a,b}=v_{a,b}\); if no, a covector
...
\S\ref{ssec:app-radial-rational-reduction}.
\end{enumerate}
\item[Output.]  Either
