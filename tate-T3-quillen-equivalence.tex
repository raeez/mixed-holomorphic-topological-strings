...
constructed here, rather than the raw category of all Tate spaces.
...
\begin{proof}
  ...
  CE cochain maps by pullback rather than by projection from \(M\) to \(N\).
...
\label{defn:universal-open-side-twisting-cochain}
  ...
  map and supplies the twisting-cochain datum.
...
\begin{proof}
  ...
  \(A_\infty\)-operations.  This equality supplies the morphism tested
  below.
...
\begin{proof}
  ...
  chosen envelope. Arbitrary unbounded Tate complexes outside the
  envelope require a separate construction.
\end{proof}
...
\begin{proof}
  ...
  kernel/\(P_0\)-pairing obstruction.  Their vanishing supplies the
  finite-window compatibility; the admissible envelope supplies the
  cochain inverse-limit datum for \(\Phi=\varprojlim_M\Phi_M\).
...
\begin{proof}
  ...
  open-BV centrality are separate theorems.
\end{proof}
...
\begin{proof}
  ...
  factorization-algebra-with-non-strict-bracket level.  The algebraic
  bar-cobar comparison uses only the conilpotent admissibility
  hypotheses: the cochain-level $\Phi$ promotes, in the admissible model
...
\label{prop:strict-product-direct-sum-kernel-obstruction}
  ...
  finite-window identity morphism would have to produce the obstructed
...
\begin{prop}[Residual Weiss-cosheaf condition]
  ...
  vocabulary and leaves the unreduced descent obligation open.  The
  admissible Tate
...
\begin{proof}
  ...
  This heat-kernel propagator is the analytic input relating the
