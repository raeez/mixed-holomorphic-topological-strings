...
\begin{thm}[Tate bar-cobar Quillen equivalence in the admissible envelope]
  ...
  \begin{enumerate}
    ...
    \item $\mathrm{Bar}$ is right Quillen: it preserves fibrations and
      ...
      Hovey~\cite{hovey}*{Lemma~1.1.10}; verifying either direction
      suffices, and we have chosen the left adjoint as the primary
      construction since cofibrations on the coalgebra side are given
      definitionally.
...
\begin{proof}
  ...
  lemma~\cite{hovey}*{Lemma~1.1.10}.  We do not need an independent
  construction-level verification on the right side: the lifting
  property defining fibrations on the coalgebra side
  (Lemma~\ref{lem:tate-conilp-model}) is precisely tuned so that
...
\label{prop:tate-T3-T1-base-change}
  ...
  \mathfrak h^{\vee,w}_{\mathrm{cont}}[1]$.  Assume that the
  finite-window towers
  \[
    \mathfrak h_{w,\leq M}
      =\prod_{d\leq M}w(d)\,H_d,\qquad
    \mathfrak h^{\vee,w}_{\leq M}
      =\prod_{d\leq M}w(d)^{-1}H_d^\vee
  \]
  satisfy the bracket-tame, windowwise coadjoint-continuity, and
  Mittag-Leffler coevaluation conditions
  (\ref{w:W1}--\ref{w:W3} of
  Definition~\ref{defn:wt-degree-weight}).
  Then:
  \begin{enumerate}[label=(\roman*)]
    \item The weighted strict ML tower
      ...
      strict habitat
      $\Cat{MLTateChain}_{\geq0}^{\mathrm{str}}$ of
      Definition~\ref{defn:strict-ml-tate-habitat}.
    \item The weighted completion
      ...
      kernel-admissible in the sense of
      Definition~\ref{defn:closed-window-obstruction-terms}: the
...
\begin{proof}
  \emph{Item~(i).}  The bracket-tame condition \ref{w:W1} of
  Definition~\ref{defn:wt-degree-weight} states that there is a
  constant $C\geq1$ with
  $\{w(p)e_{p,i},w(q)e_{q,j}\}\subset C\cdot w(p+q-2)e_{p+q-2,*}$ on
  basis elements; this is exactly the bracket-compatible transition
  condition
  $\pi_{M,N}\colon\mathfrak h_{w,\leq M}\to\mathfrak h_{w,\leq N}$
  underlying the window obstruction
  $\mathfrak o^{\mathrm{br}}_{M,N}=0$ of
  Definition~\ref{defn:closed-window-obstruction-terms}.  Windowwise
  coadjoint-continuity \ref{w:W2} gives
  $\mathfrak o^{\mathrm{coad}}_{M,N}=0$.  The transition maps are
  surjective on each finite degree pair by construction.  Hence the
  weighted tower is a strict ML object.

  \emph{Item~(ii).}  The weighted Casimir
  $C_{\mathfrak h,w}=\sum_d\sum_i(w(d)e_{d,i})\otimes(w(d)^{-1}e^i_d)$,
  restricted to the finite window, has $w_0$-projection
  $C^{\mathrm{w}}_M=\sum_{d\leq M}\sum_iw(d)e_{d,i}\otimes
  w(d)^{-1}e^i_d$, an element of
  $\mathfrak h_{w,\leq M}\otimes\mathfrak h^{\vee,w}_{\leq M}$
  which is a finite sum.  By Mittag-Leffler coevaluation \ref{w:W3} of
  Definition~\ref{defn:wt-degree-weight} and
  Proposition~\ref{prop:wt-casimir-convergence}, the family
  $(C^{\mathrm{w}}_M)_M$ is a strict compatible Mittag-Leffler tensor
  in $\mathfrak h_w\widehat\otimes_{\mathrm{ML}}
  \mathfrak h^{\vee,w}_{\mathrm{cont}}$.  The Casimir transition
  obstruction
  $\mathfrak o^{\mathrm{Cas}}_{M,N}=
  (\pi_{M,N}\otimes\pi^\vee_{M,N})C^{\mathrm{w}}_M-C^{\mathrm{w}}_N$
  vanishes because the truncation acts as the identity on the kept
  weighted basis vectors and as zero on those discarded.  Hence
  $C_{\mathfrak h,w}$ is kernel-admissible in the sense of
  Definition~\ref{defn:closed-window-obstruction-terms}.
  ...
  embedding of Statement~\ref{stmt:tate-model-envelope}.  All
  hypotheses of Theorem~\ref{thm:tate-bar-cobar-quillen} hold for the
  weighted pair: the conilpotent coalgebra
