...
\begin{proof}
  ...
  $L^{\mathrm{coalg}}$-filtration exhaustive on $C$, so the spectral
  sequence
  \[
    E_1^{n,w,p}
      =
      H^p\bigl(\operatorname{gr}^{L,F}C_{n,w}\to
      \mathrm{Bar}\,\Omega(\operatorname{gr}^{L,F}C_{n,w})\bigr)
      \Longrightarrow
      H^p\bigl(\eta_C\bigr)
  \]
  has $E_1$-isomorphism on every page, hence an abutment isomorphism
  inside the bounded-below admissible envelope.  Mittag-Leffler
  exactness on the Tate tower
  (Lemma~\ref{lem:continuous-bar-cobar} item~(1)) makes
  $\varprojlim^1$ vanish, so the windowwise qiso assembles to a global
  admissible filtered quasi-isomorphism on $\eta_C$.
  ...
  complete and Hausdorff in the admissible envelope.  The associated
  bigraded spectral sequence collapses on $E_1$ to a windowwise qiso,
  and exactness of the inverse limit on the Tate tower
  (Lemma~\ref{lem:continuous-bar-cobar} item~(1)) lifts the
  finite-window qiso to an admissible filtered qiso on $\varepsilon_A$.
