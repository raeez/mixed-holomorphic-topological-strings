% !TEX root = main.tex
%
% Universal summation residual.  Named residual class:
% \operatorname{Ob}^{\mathrm{HML}}_{w}, the obstruction to commuting
% the Hadamard parametrix / Hörmander wave-front machinery with the
% strict Mittag-Leffler inverse limit on the weighted Tate coefficient
% module.
...
\cite{hormander-vol1}*{Ch.~7} pins the singular support of the
...
degree, so the coefficient direction is not finite rank.  The question
...
Tate envelope, and what class measures the failure.

The Hadamard/Mittag-Leffler (H/ML) commutation residual measuring this
failure is a pair of independent cohomology classes: a primary
...
\(\varprojlim^1\)-cocycle measuring failure of compatible windowwise
primitives at the lowest unsolved order \(n_0\).  The two coordinates
lie in independent cohomology groups and detect independent failure
modes: a vanishing graphwise QME class does not imply
tower-compatibility of windowwise primitives, and a Mittag-Leffler
tower can produce zero \(\lambda^{\mathrm{fw}}_{n_0}\) while the
underlying cohomology obstruction is nonzero.
...
coordinate is the precise mechanism that fails.  The \(\theta_3\) row
...
coefficients across windows.  Failure of any ingredient names the
corresponding nonzero coordinate of
...
Theorem~\ref{thm:bmk-finite-window-no-go} below proves, by the single
Tate-cohomology identity
\(z_1^{N+2}\cdot\rho_{N+1,0}=-(N+2)\,\rho_{0,1}\), that no finite
Matlis target supports a Hamiltonian-equivariant principal-part
transfer: high transverse holomorphic modes leak back into low brane
momentum modes under Hamiltonian flow, and no finite-window cutoff
absorbs the leak.  The unique surviving structure is the one-pair
...
residual \(\operatorname{Ob}^{\mathrm{HML}}_{w}\) and the BMK no-go
live in independent sectors of the local theorem and admit no transfer
of formula between them.
...
\begin{prop}[Finite-window-to-limit obstruction]\label{prop:finite-window-qme-limit-condition}
  ...
  \begin{enumerate}[label=(L\arabic*)]
    ...
    has been supplied.  The finite-window theorem does not supply this
    hypothesis.
    \item The residuals
  ...
  Without (L1)--(L3), the inverse-limit graph/QME theorem stays
  outside scope.  If
...
\begin{prop}[First finite-window array rows]\label{prop:first-finite-window-array-rows}
  ...
  \]
  after the scalar condition has vanished.  Its primitive cannot be decided
...
\begin{prop}[Genuine order-two graph-weight rows]\label{prop:genuine-order-two-graph-weight-rows}
  ...
  \[
    ...
    S_{\mathrm{CE}(\Gamma,\nu),M}=0.
  \]
  This is only a scalar-condition statement.  It does not imply
  ...
  not yet decided.  The exact missing data are:
...
\begin{proof}
  ...
  graph and source rows.  In the minimal system no such genuine seed row
...
\begin{prop}[Order-two finite-window boundary rows]\label{prop:order-two-finite-window-boundary-rows}
  ...
  For any larger finite-window system, the order-\(2\) residual is not
  decided by the present local data.  After scalar-condition vanishing its
  exact row sum is
  \[
  \begin{aligned}
    R^{\mathrm{ns}}_{2,M}
...
\begin{prop}[Projection formulas for the \(u\), \(v\), and \(q\) rows]\label{prop:projection-defined-uvq-truncation-data}
  ...
  \[
    ...
    \sum_{\eta_1,\eta_2}
      ...
      \Theta^N_{\eta_1,\eta_2;\beta'_1,\beta'_2}.
  \]
  If the lower system retains all ordered normalized bracket rows and no
  ...
  this system the non-scalar QME obstruction is not yet a defined class.
\end{prop}
...
\begin{prop}[Finite-row primitive search interface]\label{prop:finite-row-primitive-search-interface}
  ...
  compatibility.  The last condition is not reached when a fixed-window
  \(H^1\)-obstruction is already detected by such a covector \(\ell\).
\end{prop}
...
\begin{prop}[Concrete order-three row in an enlarged system]\label{prop:concrete-order-three-enlarged-system-row}
  ...
  Therefore this enlarged-system row does not vanish as a local-functional
  cochain, but its scalar condition vanishes.  It is an explicit symbolic
  ...
  primitive is supplied by this row; the class is named, the larger
  order-three QME branch is not solved.
\end{prop}
...
\begin{prop}[\(\theta_3\) finite-row obstruction criterion]\label{prop:theta-three-finite-row-obstruction}
  ...
  subcomplex.  It does not exclude a primitive in a larger
  local-functional complex.  To kill the row one must add either a
...
\begin{prop}[\(\theta_3\) primitive-entry criterion]\label{prop:theta-three-primitive-entry-criterion}
  ...
  coefficient other than \(-1\) for the candidate, is not this
  ...
  compatibility class.  No such payload is supplied here: the one-row
  complex \(K^0_{\theta_3,M}=0\), so
  Proposition~\ref{prop:theta-three-finite-row-obstruction} remains the
  active claim.

  A complete companion-face table is a different condition.  It does not supply a
...
\begin{thm}[\(\theta_3\) companion-face construction obstruction]\label{thm:theta-three-companion-face-obstruction}
  ...
  compatible tower unless the lower-window identity
...
\begin{thm}[Finite-window non-scalar curvature criterion]\label{thm:finite-window-nonscalar-curvature-criterion}
  ...
  \]
  This is a scalar-condition vanishing, not a construction of a non-scalar
  primitive.
  ...
  truncation coefficients in \(M\).  Without this array only the formal
  obstruction symbol of
  Theorem~\ref{thm:finite-window-graph-qme-assembly} is available: no
  computation of \(n_0\), no proof that
...
\begin{rmk}[Hadamard parametrix versus the Bochner-Martinelli-Koppelman kernel]\label{rmk:hadamard-vs-bmk}
  ...
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer}.  No formula
  is transferred between them: the Hadamard parametrix uses the bulk
...
\begin{thm}[BMK finite-window no-go for strict all-window
  Hamiltonian transfer]\label{thm:bmk-finite-window-no-go}
  ...
  finite-Matlis truncation.  For every \(N\geq1\), no \(\C\)-linear
  quotient
  \[
  ...
  In particular, no support-local quotient on the
  derivative-delta-current module
  \(\Delta_{a,b}\in\mathcal D'_0{}^{0,2}(B)\) of
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer} carries the
  Hamiltonian action onto a finite Matlis target.  The obstruction is the
  ...
  \]
  fails for every \(N\): no choice of finite Matlis target supports a
  Hamiltonian-equivariant section of the principal-part tower.  The
...
\begin{proof}
  ...
  \]
  contradicting \(N+2\neq 0\) and \(\rho_{0,1}\neq 0\) in
  \(\mc P_{\leq N}\).  This is the no-go.

  The projected
  Jacobiator~\eqref{eq:bmk-bockstein-jacobiator} is the failure of the
  truncated bracket to satisfy Jacobi: the unprojected coadjoint
  bracket on
  \(\mathfrak h\ltimes\mc P[1]\) is a genuine semidirect Lie module
  and obeys Jacobi, but its image after \(\pi_N\) does not.
  ...
  triple; no quotient \(q_N\) absorbing this return as a unary
  boundary preserves \(\rho_{0,1}\) without collapsing
  \(\mc P_{\leq N}\) itself.
  ...
  coordinates of the same finite-window no-go statement and are
  recorded there.
\end{proof}

\begin{cor}[Pro-Matlis retract as the supremum form selected by the
  no-go]\label{cor:promatlis-retract-as-supremum}
  Theorem~\ref{thm:bmk-finite-window-no-go} forecloses every choice of
  ...
  \[
    ...
    \quad(M\geq N).
  \]
  The pro-Matlis retract is therefore not a workaround; it is the
  terminal object of \(\mathsf{Curr}^{BM}\), the supremum form of the
  BMK lane.  The no-go Theorem~\ref{thm:bmk-finite-window-no-go} is
...
\begin{thm}[Pro-Matlis universal property]\label{thm:promatlis-universal-property}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item a Hamiltonian \(\mathfrak h\)-action through full residue-coadjoint
      operators with no truncation (no finite-window restriction of
      \(\mathfrak h\to\End(Q)\));
...
\begin{proof}
  ...
  Theorem~\ref{thm:bmk-finite-window-no-go} excludes every
...
\begin{rmk}[Reading via the obstruction--twist schema]\label{rmk:bm-route-Gschema}
  Under Theorem~\ref{thm:obstruction-twist-dichotomy} of
  Appendix~\ref{app:master-deformation-complex}, the strict all-window
  Bochner--Martinelli current transfer is the canonical instance of
  clause~(4) (target replacement) of the schema: the residue-coadjoint
  computation
  \(z_1^{N+2}\!\cdot\!\rho_{N+1,0}=-(N+2)\rho_{0,1}\) and the projected
  finite-window Jacobiator \((N+1)(N+2)\rho_{0,1}\ne0\) show that the
  curvature \([F_{\mathcal T}(\Theta_{\mathrm{loc}})]\) in any fixed
  finite Matlis target \(P_{\le N}\) is non-absorptive --- neither
  exact nor central in any admissible extension of the target ---
  and no counterterm internal to a finite current window restores
  coherence.  The corollary above identifies the substituted target:
  the pro-Matlis tower
  \(P^{\Pi}=\varprojlim_N P_{\le N}\), with modified coherence law
  \(\pi_N(f\!\cdot\!\xi)=\pi_N\bigl(f\!\cdot\!\pi_{N+r(f)}\xi\bigr)\),
  is the unique target in which the Hamiltonian action becomes
  coherent.  This is target replacement, not counterterm absorption.
\end{rmk}

\begin{proof}
  The first sentence is Theorem~\ref{thm:bmk-finite-window-no-go}: no
  finite-Matlis quotient supports the Hamiltonian action.  The
  ...
  Theorem~\ref{thm:bmk-finite-window-no-go}; (b) selects the
  ...
  current-topology obstructions not removed by the analytic
  pro-Matlis retract alone.
\end{proof}
...
\begin{rmk}[Physical interpretation: high transverse holomorphic modes
  ...
  Theorem~\ref{thm:bmk-finite-window-no-go} carries a direct physical
  ...
  This leak is what every strict finite-Matlis cutoff cannot absorb:
  ...
  \eqref{eq:bmk-bockstein-identity}, so removing the lift does not
  remove the image.  The pro-Matlis retract of
  ...
  finite-window readout of this consistent flow, not an a priori
