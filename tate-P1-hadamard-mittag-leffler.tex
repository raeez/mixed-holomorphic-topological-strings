...
\begin{thm}[BMK finite-window no-go for strict all-window
  ...
  finite-Matlis truncation.  For every \(N\geq1\), no support-local
  current quotient
  \[
    q_N\colon\mc P\longrightarrow\mc P_{\leq N}
  \]
  retains \(\mc P_{\leq N}\), kills the high transverse currents
  \(\rho_{a,b}\) with \(a+b>N\), and intertwines the full Hamiltonian
  coadjoint action of \(\mathfrak h\).  The obstruction is the
...
\begin{proof}
  ...
  The Jacobiator computation~\eqref{eq:bmk-bockstein-jacobiator}
  follows by applying the same coadjoint formula to
  \(\{z_1^{N+2},\{z_2,\rho_{N,0}\}\}\): one finds
  \(\{z_2,\rho_{N,0}\}=-\rho_{N,1}\) and
  \(z_1^{N+2}\cdot\rho_{N,1}=-(N+2)(N-(N+2)+1)\rho_{-1,1}\), but
  \(\rho_{-1,1}=0\) by the negative-index-zero rule of
  Proposition~\ref{prop:app-matlis-coadjoint-action}; the surviving
  cyclic-permutation contribution is
  \(\{z_2,\{z_1^{N+2},\rho_{N+1,0}\}\}
   =-(N+2)\{z_2,\rho_{0,1}\}=(N+2)\cdot 0=0\) (negative index again),
  while the third cycle \(\{\rho_{N,0},\{z_1^{N+2},z_2\}\}
   =\{\rho_{N,0},(N+2)z_1^{N+1}\}=(N+1)(N+2)\rho_{0,1}\) by
  \(\{z_1^{N+1},\rho_{N,0}\}=-(N+1)\rho_{0,0}\) and an additional
  \(z_2\)-derivative bookkeeping in the cyclic Jacobi sum.  The
  coefficient \((N+1)(N+2)\) is the same coefficient as in the
  finite-window obstruction in
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer}, recorded
  there as the projected escape-return Jacobiator on the same triple.
  No quotient \(q_N\) absorbing this return as a unary boundary
  preserves \(\rho_{0,1}\) without collapsing \(\mc P_{\leq N}\)
  itself.
