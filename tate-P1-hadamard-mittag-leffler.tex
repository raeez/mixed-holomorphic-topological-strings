...
\begin{rmk}[Hadamard parametrix versus the Bochner-Martinelli-Koppelman kernel]\label{rmk:hadamard-vs-bmk}
  ...
  parametrix governs the Costello/QME graph expansion above; the BMK
  kernel underwrites the analytic one-pair pro-Matlis principal-part
  current quotient and the strict all-window obstruction
  \(\operatorname{Ob}^{\mathrm{BMK\mbox{-}curr}}_N\) recorded in
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer}.
  No formula is transferred between them: the Hadamard parametrix uses
  the bulk Riemannian/Hermitian heat data on
  \(\R^2\times\C^2\), the BMK kernel uses the holomorphic homotopy data
  on \(\C^2\) alone, and the local theorem combines their separate
  finite-window outputs through the obstruction vectors stated above.
\end{rmk}

\paragraph*{H/ML residual.}  The two-coordinate class
\(\operatorname{Ob}^{\mathrm{HML}}_{w}=
