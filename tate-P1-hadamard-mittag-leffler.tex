...
supplies four ingredients: the finite Hamiltonian window \(M\) and
...
\(\operatorname{Ob}^{\mathrm{HML}}_{w}\).  The downstream finite-row
machinery -- graph arrays, truncation matrices, scalar-condition
projection, primitive search -- yields a finite cochain whose vanishing
is tested at every window \(M\) and every order \(\hbar^n\).

The principal-part current sector carries the parallel scope
constraint.  Theorem~\ref{thm:bmk-finite-window-obstruction} below
...
momentum modes under Hamiltonian flow.  The unique surviving structure is the one-pair
analytic pro-Matlis retract
(Corollary~\ref{cor:promatlis-retract-as-supremum}), terminal among
Bochner--Martinelli admissible current targets
...
\begin{rmk}[Spacetime/coefficient factorization of the Hadamard parametrix]\label{rmk:hadamard-factorization}
  ...
  the hypotheses of that formalism.  The local input here is this
  heat-kernel/RG framework.
...
\begin{defn}[Finite-window graph/QME system]\label{defn:finite-window-graph-qme-system}
  ...
  separate graph-summability hypothesis is supplied.
\end{defn}
...
\begin{prop}[Finite-window-to-limit obstruction]\label{prop:finite-window-qme-limit-condition}
  ...
  \begin{enumerate}[label=(L\arabic*)]
    ...
    finite in each window, or a separate Costello graph-summability hypothesis
    has been supplied.  This graph-summability hypothesis is separate
...
\begin{defn}[Finite-window graph array]\label{defn:finite-window-graph-array}
  ...
  \begin{enumerate}[label=(A\arabic*)]
    ...
    datum.  The equality of output graphs, coefficients, and marker
...
\begin{prop}[First finite-window array rows]\label{prop:first-finite-window-array-rows}
  ...
  after supplying the remaining order-one rows of
...
\begin{prop}[Universal scalar-contact bracket rows]\label{prop:universal-scalar-contact-bracket-rows}
  ...
  graph system and must be supplied row by row.
\end{prop}
...
\begin{prop}[Genuine order-two graph-weight rows]\label{prop:genuine-order-two-graph-weight-rows}
  ...
  Koszul, and symmetry factors supplied by the finite graph system.
...
\begin{prop}[Truncation matrices for the computed finite-window rows]\label{prop:computed-finite-window-truncation-matrices}
  ...
  \begin{enumerate}[label=(T\arabic*)]
    ...
    larger system supplies genuine seeds
    ...
    hypotheses supply exactly the lower-window entries
...
\begin{proof}
  ...
  finite-window marked-complex compatibility preserves the output graph,
...
\begin{prop}[Projection formulas for the \(u\), \(v\), and \(q\) rows]\label{prop:projection-defined-uvq-truncation-data}
  ...
  \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) full-equivariant marked
  branch.  Fix \(M\geq N\).  Suppose the system supplies ordered
  ...
  If the system supplies only a spanning family with relations, the
  coordinates are an additional presentation datum that fixes the
...
\begin{proof}
  ...
  the system supplies a presentation or a coordinate splitting.
...
\begin{prop}[\(\theta_3\) finite-row obstruction criterion]\label{prop:theta-three-finite-row-obstruction}
  ...
  finite row complex supplied there, set
  \[
  ...
  \]
  This is an exact obstruction statement for the supplied finite row
...
\begin{prop}[\(\theta_3\) primitive-entry criterion]\label{prop:theta-three-primitive-entry-criterion}
  ...
  degree-zero primitive for this row is accepted by the finite-row
  ...
  \[
    ...
    -\mathsf E_{\theta_3,M}.
  \]
  An accepted row-data input supplies the displayed differential entry
  with coefficient \(-1\) for the candidate.  Any other input must first
  be replaced by an explicitly supplied candidate whose differential
  ...
  \]
  Thus such an input kills the fixed-window row and has zero Roos
...
\begin{proof}
  ...
  support proof supply the boundary matrix entry used by the finite row
  complex.
  ...
  table supplies its scalar conditions, truncation coordinates, and
...
\begin{proof}
  ...
  Definition~\ref{defn:finite-window-graph-array} supplies a zero
...
\begin{thm}[Finite-window non-scalar curvature criterion]\label{thm:finite-window-nonscalar-curvature-criterion}
  ...
  The abstract existence of \(\mathcal G_M\) supplies the graph list but
  leaves the coefficient array open.  To decide \(n_0\), the class
...
\begin{rmk}[Hadamard parametrix versus the Bochner-Martinelli-Koppelman kernel]\label{rmk:hadamard-vs-bmk}
  ...
  theorem combines their separate finite-window outputs through the
...
\begin{thm}[BMK finite-window obstruction for strict all-window
  ...
  finite-window \(L_\infty\)-escape repair has nonzero unary block
  \[
...
\begin{proof}
  ...
  triple; any quotient \(q_N\) absorbing this return as a unary
...
\begin{cor}[Pro-Matlis retract as the unique compatible target]\label{cor:promatlis-retract-as-supremum}
  ...
  (Theorem~\ref{thm:promatlis-universal-property}), the unique
  surviving target is the one-pair analytic pro-Matlis retract
  \[
    \widehat{\mc P}_{\Pi}
  ...
  \[
    ...
    \quad(M\geq N).
  \]
  The pro-Matlis retract is therefore the terminal object of
  $\mathsf{Curr}^{BM}$.  Theorem~\ref{thm:bmk-finite-window-obstruction}
  establishes the necessity of the pro-Matlis category in place of
  every finite-Matlis target.
\end{cor}

\begin{thm}[Pro-Matlis universal property]\label{thm:promatlis-universal-property}
  Let \(\mathsf{Curr}^{BM}\) be the category whose objects are
  \(\mathfrak h\)-equivariant \(\C\)-linear pairs
  \((Q,Q^{\mathrm{an}})\) with \(Q^{\mathrm{an}}\subset Q\), equipped
  with:
  \begin{enumerate}[label=(\alph*)]
    \item compatible projections \(\pi_N\colon Q\to\mathcal P_{\le N}\)
      satisfying \(\pi_M=\pi_N\circ\pi_{M,N}\) for \(M\ge N\);
  ...
  \end{enumerate}
  Morphisms are \(\mathfrak h\)-equivariant \(\C\)-linear maps
  \((Q,Q^{\mathrm{an}})\to(Q',Q'^{\mathrm{an}})\) commuting with all
  finite projections \(\pi_N,\pi'_N\) and with the analytic-submodule
  retractions.  Then the pro-Matlis pair
  \(\bigl(\widehat{\mathcal P}_\Pi,\mathcal P_{\mathrm{an}}(B_1)\bigr)\)
  with the analytic moment map \(P^{\mathrm{an}}_\Pi\) of
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer} is terminal
  in \(\mathsf{Curr}^{BM}\): for every object
  \((Q,Q^{\mathrm{an}})\) there is a unique morphism
  \[
...
\begin{proof}
  ...
  \(\mathfrak h\)-equivariant map
  \(Q\to\varprojlim_N\mathcal P_{\le N}=\widehat{\mathcal P}_\Pi\) by
  the universal property of the inverse limit.  Condition~(c) sends
  the analytic retraction onto
  \(\mathcal P_{\mathrm{an}}(B_1)=\tau(\mathcal O(B_1)^\vee_{\mathrm{red}})\)
  by Theorem~\ref{thm:app-finite-window-bmk-current-transfer}, and
  condition~(d) ensures the lift is convergent in the
  ...
  hence on \(\varprojlim_N\mathcal P_{\le N}\).  Hamiltonian
  equivariance, the chain-homotopy compatibility, and the moment
  bounds are preserved by the inverse limit.
  ...
  Corollary~\ref{cor:promatlis-retract-as-supremum} verifies the
...
\begin{proof}[Proof of Corollary~\ref{cor:promatlis-retract-as-supremum}]
  ...
  Minimality.  Any other \(\C\)-linear \(\mathfrak h\)-module with a
  Dolbeault chain homotopy compatible with \(p_N\) and \(i_N\) at
  every \(N\) must
  \begin{enumerate}[label=(\alph*)]
  ...
  on which the kernel acts; (c) forces the underlying ambient module
  to contain every \(\rho_{a,b}\) and to project to \(\mc P_{\leq N}\),
  hence to be \(\widehat{\mc P}_{\Pi}=\varprojlim_N\mc P_{\leq N}\).
  The pair \((\widehat{\mc P}_{\Pi},\mc P_{\mathrm{an}}(B_1))\) is
  therefore the unique smallest target.  The downstream theorem surface
  is the strict all-window factorization-current transfer; it
...
\begin{rmk}[Physical interpretation: high transverse holomorphic modes
  ...
  \eqref{eq:bmk-bockstein-identity}, so the image remains.  The pro-Matlis retract of
  Corollary~\ref{cor:promatlis-retract-as-supremum} is the smallest
  module closed under this leak: the full Taylor tower retains every
  high transverse mode and lets the Hamiltonian flow act
  consistently, while the analytic submodule
  \(\mc P_{\mathrm{an}}(B_1)\) supplies the moment-restriction kernel
  required by the BMK contraction.  The arity-two output projection
