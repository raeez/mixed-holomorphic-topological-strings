...
order in \(\hbar\), with the strict Mittag-Leffler inverse limit in the
coefficient direction; equivalently, whether universal summation of the
windowwise counterterms survives passage to the inverse-limit weighted
Tate envelope, and what class measures the obstruction.
...
\begin{proof}
  ...
  wavefront-admissible product condition to define a canonical distribution.
...
\begin{defn}[Finite-window graph array]\label{defn:finite-window-graph-array}
  ...
  \begin{enumerate}[label=(A\arabic*)]
    ...
    \]
    An entrywise non-scalar summand requires an additional chain
...
\begin{thm}[\(\theta_3\) companion-face construction obstruction]\label{thm:theta-three-companion-face-obstruction}
  ...
  \(H_{p,q}=z_1^pz_2^q\).  On the standard symplectic
  \(\C^2\) with \(\omega=dz_1\wedge dz_2\) the Hamiltonian Poisson rule
  is
