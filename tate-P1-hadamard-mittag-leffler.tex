...
\paragraph*{Organizing question.}  Costello's heat-kernel BV calculus
...
degree, so the coefficient direction is not finite rank.  Does that
finite-rank machinery commute, graphwise and at every order in
\(\hbar\), with the strict Mittag-Leffler inverse limit in the
coefficient direction?  Equivalently, does universal summation of the
windowwise counterterms survive passage to the inverse-limit weighted
Tate envelope, and what is the exact class measuring its failure?

\paragraph*{Unique survivor.}  The H/ML commutation residual is the
named two-coordinate class
\[
...
inverse-limit Roos primitive class
\(\lambda^{\mathrm{fw}}_{n_0}\) controlled by
Theorem~\ref{thm:finite-window-graph-qme-assembly}.  Vanishing of
both coordinates is necessary and sufficient for compatible
graphwise quantum extension on \(\mc E_w\); a single nonzero
coordinate is the precise mechanism that fails.

\paragraph*{Habitat.}  The first coordinate
\(\operatorname{Ob}_{w}\) is the QME obstruction class in
local-functional cohomology with differential
\(Q+\{I_0,-\}\); the second coordinate
\(\lambda^{\mathrm{fw}}_{n_0}\) is the Roos
\(\varprojlim^1\)-cocycle measuring failure of compatible
windowwise primitives at the lowest unsolved order \(n_0\).  The
two coordinates lie in independent cohomology groups and detect
independent failure modes: a vanishing graphwise QME class does not
imply tower-compatibility of windowwise primitives, and a
Mittag-Leffler tower can produce zero \(\lambda^{\mathrm{fw}}_{n_0}\)
while the underlying cohomology obstruction is nonzero.  The
\(\theta_3\) row of
Proposition~\ref{prop:concrete-order-three-enlarged-system-row} is
the minimal explicit non-scalar exhibit: a local-functional row that
passes the full-equivariant scalar-condition vanishing and presents
its missing primitive as the named datum (CE-ancestor, scalar-zero
counterterm, or complete companion-face table).

\paragraph*{Closure.}  The construction completing the residual on a
given graph system supplies the four named ingredients: the finite
Hamiltonian window \(M\) and graph list \(\mathcal G_M\); a chain
projection \(\widehat\sigma_{\mathrm{sc},M}\) onto the scalar-symbol
sector with vanishing image of the residual; primitives \(c_{n,M}\)
of the non-scalar residual at every window; and Roos-compatible
truncation coefficients across windows.  Failure of any ingredient
names the corresponding nonzero coordinate of
\(\operatorname{Ob}^{\mathrm{HML}}_{w}\).  The downstream
finite-row machinery -- graph arrays, truncation matrices,
scalar-condition projection, primitive search -- is included so
that the residual is a finite cochain whose vanishing is tested at
every window \(M\) and every order \(\hbar^n\), not a slogan.

\paragraph*{BMK firewall.}  A second, sharper firewall sits on the
parallel principal-part current sector.
...
can absorb the leak.  The unique surviving habitat is the one-pair
analytic pro-Matlis retract
(Corollary~\ref{cor:promatlis-retract-as-supremum}).  This is the
supremum form of the BMK lane: not a workaround, but the structural
target selected by the no-go.  The Hadamard / QME residual
\(\operatorname{Ob}^{\mathrm{HML}}_{w}\) and the BMK no-go live in
independent sectors of the local theorem and admit no transfer of
...
Theorem~\ref{thm:hadamard-mittag-leffler} controls the analytic factor
windowwise; it isolates the QME obstruction class but does not assemble
it.  The class is a sum over Costello graphs at every \(\hbar\)-order,
each graph carrying its renormalized weight, source-face, BV-bracket,
and counterterm contributions.  To extract the obstruction one must
first fix a finite graph list \(\mathcal G_M\) on a finite Hamiltonian
window \(M\), declare the labels and incidence signs, and write the
order-\(n\) residual as a finite cochain.  The next definition packages
this finite combinatorial assembly so that the residual, its scalar
projection, and its truncation behaviour can be tested as a row system.
...
Proposition~\ref{prop:finite-window-qme-limit-condition} reduces the
finite-window obstruction to two cohomology classes, but neither class
is computable from a graph list alone.  The order-\(n\) residual is a
sum of graph-differential, CE-source, BV-bracket, and lower-counterterm
contributions, each carrying its own incidence sign, scalar projection,
and truncation matrix.  To compute, test, and propagate the residual
window by window, one needs a finite array of rows that records every
contribution in normal form.
...
rows close to zero (Propositions~\ref{prop:first-finite-window-array-rows},
\ref{prop:universal-scalar-contact-bracket-rows},
\ref{prop:order-two-finite-window-boundary-rows}).  The first
non-trivial obstruction can therefore appear no earlier than order
three.  The smallest enlargement that exhibits a non-scalar row with
nonzero local-functional value is a single CE-source face attached to
a genuine marked Costello seed at \(\hbar^3\); this is the row
\(\theta_3\) below, named so that the obstruction class
\(\operatorname{Ob}^{\Pi}_{\theta_3}\) and the companion-face
construction can be tracked explicitly.
...
\begin{rmk}[Hadamard parametrix versus the Bochner-Martinelli-Koppelman kernel]\label{rmk:hadamard-vs-bmk}
  ...
  Dolbeault homotopy on the formal holomorphic disk
...
\begin{thm}[BMK finite-window no-go for strict all-window
  ...
  algebra of the formal symplectic disk \((\widehat{\C^2}_0,\omega)\),
  let
  \[
  ...
  derivative-delta-current habitat
...
\begin{proof}
  ...
  \(\{H_{a,b},H_{r,s}\}=(as-br)H_{a+r-1,b+s-1}\) of the disk:
  \[
  \begin{aligned}
...
\begin{cor}[Pro-Matlis retract as the supremum form selected by the
  ...
  transfer.  The unique surviving habitat compatible with both the BMK
...
\begin{proof}
  ...
  is the strict native all-window factorization-current transfer; it
  ...
  native-current-topology obstructions not removed by the analytic
...
\begin{rmk}[Physical interpretation: high transverse holomorphic modes
  ...
  habitat closed under this leak: the full Taylor tower retains every
...
\paragraph*{H/ML residual and BMK firewall.}  The two-coordinate class
\(\operatorname{Ob}^{\mathrm{HML}}_{w}=
(\operatorname{Ob}_{w},\lambda^{\mathrm{fw}}_{n_0})\) of the opening
is the only datum separating the bulk Hadamard / Hörmander control
(Theorem~\ref{thm:hadamard-mittag-leffler}) from the graphwise
all-window QME extension on the weighted Tate envelope.  Vanishing
with chosen primitives (Theorem~\ref{thm:finite-window-graph-qme-assembly},
Proposition~\ref{prop:finite-window-qme-limit-condition}) is exactly
the Costello-locality / QME compatibility input demanded by
Problem~\ref{prob:weighted-rg-locality}; a single nonzero coordinate
is the precise mechanism that fails, with the
\(\theta_3\) row of
Proposition~\ref{prop:concrete-order-three-enlarged-system-row}
exhibiting the minimal nontrivial exhibit.  No claim of universal
summation is mathematics without either a vanishing certificate or
a named nonzero coordinate.

The BMK lane carries a parallel firewall, but its outcome is
sharper.  Theorem~\ref{thm:bmk-finite-window-no-go} proves, by the
single Tate-cohomology identity
\(z_1^{N+2}\cdot\rho_{N+1,0}=-(N+2)\rho_{0,1}\), that no finite
Matlis target supports a Hamiltonian-equivariant principal-part
transfer; the obstruction
\(\operatorname{Ob}^{\mathrm{BMK\mbox{-}curr}}_N\) of
Theorem~\ref{thm:app-finite-window-bmk-current-transfer} cannot be
killed by any finite-window choice.  The pro-Matlis retract of
Corollary~\ref{cor:promatlis-retract-as-supremum} is therefore not
the residual of an unsolved problem; it is the supremum form of the
BMK lane, structurally selected by the no-go.  The Hadamard / QME
lane and the BMK lane are independent: vanishing of
\(\operatorname{Ob}^{\mathrm{HML}}_{w}\) is a graphwise compatibility
statement on the weighted Tate envelope, while the BMK no-go is an
absolute obstruction on the principal-part current sector that
cannot be removed by any choice of weight, regulator, or graph
list.
