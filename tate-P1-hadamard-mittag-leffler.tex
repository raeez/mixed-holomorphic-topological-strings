...
\begin{prop}[\(\theta_3\) primitive-entry criterion]\label{prop:theta-three-primitive-entry-criterion}
  ...
  \[
    ...
    -\mathsf E_{\theta_3,M}.
  \]
  A row datum with no displayed differential entry, or with any
  coefficient other than \(-1\) for the candidate, is not this
  accepted datum; it must first be replaced by an explicitly supplied
  ...
  \]
  Thus such a datum kills the fixed-window row and has zero Roos
  compatibility class.  No such datum is supplied here: the one-row
...
\begin{proof}
  For a primitive row datum, apply
