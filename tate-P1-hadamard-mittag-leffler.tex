...
\begin{prop}[\(\theta_3\) primitive-entry criterion]\label{prop:theta-three-primitive-entry-criterion}
  ...
  \[
    ...
    -\mathsf E_{\theta_3,M}.
  \]
  A row-data input with no displayed differential entry, or with any
  coefficient other than \(-1\) for the candidate, is not this
  accepted input; it must first be replaced by an explicitly supplied
  ...
  compatibility class.  No such input is supplied here: the one-row
