...
\begin{proof}
  ...
  Supremum status: any other admissible target must (a) accept the
  full Hamiltonian coadjoint action, (b) admit the Dolbeault homotopy
  obtained from the BMK kernel after the diagonal orientation and
  collar quotient, and (c) project compatibly to the finite Matlis
  windows used by the arity-two coefficient comparison
  \(\ell^{\mathrm{BM},N}_2(x,y)=p_N[ix,iy]\).  Condition (a) excludes
  every finite-rank truncation by
  Theorem~\ref{thm:bmk-finite-window-no-go}; conditions (b) and (c)
  exclude every category that is not the full Taylor tower.  The
  one-pair analytic pro-Matlis retract is the smallest object
  satisfying all three; it is therefore the unique structural
  upgrade.  The strict native all-window factorization-current
  transfer is the next theorem surface: it requires the additional
  vanishing of \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\) recorded in
  ...
  compact-current section, divergent formal-Hamiltonian action on the
  pro-product) are independent native-current-topology obstructions
  not removed by the analytic pro-Matlis retract alone.
\end{proof}
