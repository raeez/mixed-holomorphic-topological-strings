...
finite-window projections and Bochner--Martinelli moment map
...
\begin{cor}[Pro-Matlis retract as the compatible replacement target]\label{cor:promatlis-retract-compatible-replacement}
  Theorem~\ref{thm:bmk-finite-window-obstruction} forecloses every choice of
  finite-Matlis target for a Hamiltonian-equivariant principal-part
  transfer.  Inside the category \(\mathsf{Curr}^{BM}\) of compatible
  ...
  establishes the necessity of a pro-Matlis replacement in place of
  every finite-Matlis target.
\end{cor}

\begin{thm}[Pro-Matlis inverse-limit receiver property]\label{thm:promatlis-universal-property}
  Let \(\mathsf{Curr}^{BM}\) be the category whose objects are
  \(\C\)-linear pairs
  ...
  residue-coadjoint actions commute with it.  Then the pro-Matlis pair
  \(\bigl(\widehat{\mathcal P}_\Pi,\mathcal P_{\mathrm{an}}(B_1)\bigr)\)
  with the analytic moment map \(P^{\mathrm{an}}_\Pi\) of
  Theorem~\ref{thm:app-finite-window-bmk-current-transfer} receives,
  from every object \((Q,Q^{\mathrm{an}})\), a unique morphism
  compatible with the fixed finite-window projections and analytic
  retractions:
  \[
    (Q,Q^{\mathrm{an}})\longrightarrow
    \bigl(\widehat{\mathcal P}_\Pi,\mathcal P_{\mathrm{an}}(B_1)\bigr).
  \]
  The assertion is an inverse-limit receiver property for the fixed
  BMK system; it is not a terminality statement after the category is
  enlarged by spectator summands or by changing the analytic
  submodule.
\end{thm}
...
\begin{proof}
  ...
  the universal property of the inverse limit.  Condition~(d) sends
  the analytic submodule to
  \(\mathcal P_{\mathrm{an}}(B_1)=\tau(\mathcal O(B_1)^\vee_{\mathrm{red}})\)
  compatibly with the finite-window maps, and condition~(e) ensures the
  lift is convergent in the
  ...
  hence on \(\varprojlim_N\mathcal P_{\le N}\).  Chain-homotopy
  compatibility and the moment bounds are preserved by the inverse
  limit.  Hamiltonian equivariance is a separate condition on the
  untruncated actions, not a consequence of the finite projections.
  This proves the
  projection-compatible receiver property.
  Theorem~\ref{thm:bmk-finite-window-obstruction} excludes every
  finite-rank target (those have a strict finite-window restriction of
  \(\mathfrak h\), violating~(b)), and
...
\begin{proof}[Proof of Corollary~\ref{cor:promatlis-retract-compatible-replacement}]
  ...
  finite-Matlis quotient has the Hamiltonian-action obstruction.  The
  ...
  \end{enumerate}
  Condition (a) excludes every finite-rank quotient by
