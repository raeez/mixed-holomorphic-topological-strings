...
\cite{costello-renormalization}*{Chs.~5--7} requires a free elliptic
complex with finite-rank coefficient bundle; its Hadamard parametrix
expansion together with Hörmander wave-front tracking
\cite{hormander-vol1}*{Ch.~7} locates the singular support of the
propagator on the bulk diagonal.  The weighted Tate field complex
\eqref{eq:wt-field-complex},
\[
  \mc E_w
    =
  ...
  \Bigl(
    \prod_{d\geq1}w(d)H_d[1]
    \oplus\prod_{d\geq1}w(d)^{-1}H_d^\vee[2]
  \Bigr),
\]
carries an infinite tower of coefficient bundles indexed by Hamiltonian
degree, so the coefficient direction has infinite rank.  The question
is whether that finite-rank machinery commutes, graphwise and at every
order in \(\hbar_{\mathrm{QME}}\), with the strict Mittag-Leffler
inverse limit in the coefficient direction; equivalently, whether
universal summation of the windowwise counterterms survives passage to
the inverse-limit weighted Tate envelope, and what class measures the
obstruction.

Throughout this subsection the bare graph-order symbol \(\hbar\), in
expressions such as \([\hbar^n]\), \(|\Gamma|_\hbar\), and
\(\{ -,-\}_{\hbar,M}\), denotes the Costello quantum master-equation
parameter \(\hbar_{\mathrm{QME}}\).  It is not the Weyl/Moyal parameter
\(\hbar_{\mathrm{W}}\) and not the equivariant line parameter
\(\hbar_\omega\).

The Hadamard/Mittag-Leffler (H/ML) commutation residual measuring this
obstruction is a pair of independent cohomology classes: a primary
QME-obstruction class in the local-functional cohomology of the
weighted complex, and a secondary Roos
\(\varprojlim^1\)-cocycle on the inverse system of finite Hamiltonian
windows.  Concretely,
\[
  \operatorname{Ob}^{\mathrm{HML}}_{w}
  =
  \bigl(
    \operatorname{Ob}_{w},\,
  ...
  \bigr)
  \in
  H^1\!\bigl(\mathcal O_{\mathrm{loc}}(\mc E_w),Q+\{I_0,-\}\bigr)
  \,\oplus\,
  \varprojlim\nolimits^1_M
  H^0\!\bigl(\mathcal Q^\bullet_{\mathcal G,M}(I)\bigr).
\]
The first coordinate \(\operatorname{Ob}_{w}\) is the QME obstruction
class in local-functional cohomology with differential \(Q+\{I_0,-\}\);
the second coordinate \(\lambda^{\mathrm{fw}}_{n_0}\) is the Roos
\(\varprojlim^1\)-cocycle measuring the obstruction to compatible
windowwise primitives at the lowest unsolved order \(n_0\).  The two
coordinates lie in independent cohomology groups and detect independent
obstruction modes: graphwise QME vanishing and tower-compatibility of windowwise
primitives are separate conditions, and a Mittag-Leffler tower can
produce zero \(\lambda^{\mathrm{fw}}_{n_0}\) while the underlying
cohomology obstruction is nonzero.
Theorem~\ref{thm:hadamard-mittag-leffler} reduces the analytic content
of Problem~\ref{prob:weighted-rg-locality} -- exact propagator
factorization, empty coefficient-Casimir wave front, strict ML
transition surjectivity -- to the algebraic remainder: graphwise QME
in the local-functional class \(\operatorname{Ob}_{w}\) and an
inverse-limit Roos primitive class \(\lambda^{\mathrm{fw}}_{n_0}\)
controlled by Theorem~\ref{thm:finite-window-graph-qme-assembly}.
For the fixed graphwise extension problem under these hypotheses,
vanishing of both coordinates is necessary and sufficient for
compatible graphwise quantum extension on \(\mc E_w\); a single nonzero
coordinate is the precise nonzero obstruction mechanism.  The \(\theta_3\) row
of Proposition~\ref{prop:concrete-order-three-enlarged-system-row}
exhibits a local-functional row that passes the full-equivariant
scalar-condition vanishing and presents its missing primitive as the
named datum (CE-ancestor, scalar-zero counterterm, or complete
companion-face table).

The construction completing the residual on a given graph system
gives four ingredients: the finite Hamiltonian window \(M\) and
graph list \(\mathcal G_M\); a chain projection
\(\widehat\sigma_{\mathrm{sc},M}\) onto the scalar-symbol sector with
vanishing image of the residual; primitives \(c_{n,M}\) of the
non-scalar residual at every window; and Roos-compatible truncation
coefficients across windows.  A violated ingredient names the corresponding
nonzero coordinate of
\(\operatorname{Ob}^{\mathrm{HML}}_{w}\).  The finite-row complex --
graph arrays, truncation matrices, scalar projection, and primitives --
yields a finite cochain whose vanishing
is tested at every window \(M\) and every order \(\hbar^n\).

The principal-part current sector carries the parallel finiteness
condition.  Theorem~\ref{thm:bmk-finite-window-obstruction} below
proves, by the single
Tate-cohomology identity
\(z_1^{N+2}\cdot\rho_{N+1,0}=-(N+2)\,\rho_{0,1}\), the obstruction to
Hamiltonian-equivariant principal-part transfer into a finite Matlis
target: high transverse holomorphic modes leak back into low brane
momentum modes under Hamiltonian flow.  The replacement structure used
below is the one-pair analytic pro-Matlis retract
  (Corollary~\ref{cor:promatlis-retract-compatible-replacement}), with its fixed
  finite-window projections and Bochner--Martinelli moment map satisfying
  the fixed-projection receiver criterion of
  Theorem~\ref{thm:promatlis-universal-property}.  The Hadamard / QME
residual \(\operatorname{Ob}^{\mathrm{HML}}_{w}\) and the BMK
finite-window obstruction live in independent sectors of the local
theorem, with separate formulas.
...
\begin{rmk}[Spacetime/coefficient factorization of the Hadamard parametrix]\label{rmk:hadamard-factorization}
  ...
  Costello's heat-kernel locality formalism
  ...
  the hypotheses of that formalism.  The local datum here is this
  heat-kernel/RG formalism.
  ...
  $P_{\epsilon,L}^{w}=P_{\epsilon,L}^{\mathrm{base}}\widehat\otimes
   C_{\mathfrak h\oplus\mathfrak h^\vee[1],w}$. The bulk piece
	  ...
	  algebraic, with zero scale parameter and zero Laplacian. The Hadamard
...
\begin{lem}[Windowwise parametrix compatibility]\label{lem:windowwise-parametrix}
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \item The wave-front set
      ...
      with empty spacetime singular support.
  \end{enumerate}
\end{lem}
...
\begin{proof}
  ...
  the strict finite-window category of
  Definition~\ref{defn:strict-ml-tate-category}, and
  Proposition~\ref{prop:strict-ml-category-exactness} gives
  ...
  with constant point dependence, hence smooth, so
  $\mathrm{WF}(T_2)=\emptyset$. Hence
  $\mathrm{WF}(P_{\epsilon,L}^{w,w_0})=
  \mathrm{WF}(P_{\epsilon,L}^{\mathrm{base}})\times\{0\}$, identified
  with $\mathrm{WF}(P_{\epsilon,L}^{\mathrm{base}})$ on the
...
\begin{thm}[Hadamard control on the Mittag-Leffler coefficient module]\label{thm:hadamard-mittag-leffler}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item For every local interaction
      ...
      finite codomain window, by a finite graph-dependent constant times
    ...
    \item The construction has the ordinary strict inverse-limit
      universal property: a compatible graphwise
      counterterm family
...
\begin{proof}
  ...
  \(\Gamma\) and the fixed finite codomain window under consideration.
  ...
  is used only through a finite-window theorem: the windowwise counterterm
  ...
  \varprojlim_{w_0}\mathcal O_{\mathrm{loc}}(\mathcal E_w^{w_0})$
  exists in the weighted Tate category and is the
  ...
  Proposition~\ref{prop:strict-ml-category-exactness}: it is the unique
...
\begin{rmk}\label{rmk:hormander-commutation}
  The Mittag-Leffler limit in the coefficient direction commutes with
  Hörmander's wave-front analysis because the propagator factorization
  \eqref{eq:wt-propagator-factor} is exact (the bulk wave-front set is
  computed once and tensored), the coefficient factor
  $C_{\mathfrak h,w}$ is an algebraic tensor in the weighted Tate
  category with $\mathrm{WF}=\emptyset$, and the Hadamard parametrix
  coefficients $a_k(x,y)$ depend only on the bulk Riemannian/Hermitian
  data of $\R^2\times\C^2$.  The wave-front bound of
  Lemma~\ref{lem:windowwise-parametrix}~(3) is therefore exact and
  $w_0$-uniform; the Mittag-Leffler limit preserves the
  local-functional class graph by graph at every order $\hbar^n$
  whenever the windowwise counterterm system is compatible.
\end{rmk}
...
\begin{cor}[Hadamard coefficient criterion for the weighted QME lift]\label{cor:wt-cotangent-lift-hadamard-criterion}
  ...
  all-order QME-counterterm source-to-boundary CE/PV comparison
  \[
  \begin{aligned}
    \Phi_{\hbar_{\mathrm{QME}}}^{w}\colon
    &C^\bullet_{\mathrm{CE},\mathrm{cont}}\bigl(
      \mathfrak h_{w,\hbar_{\mathrm{QME}}}\ltimes
      D^{\mathrm{BV}}_{w,\hbar_{\mathrm{QME}}}[1]\bigr)[[\hbar_{\mathrm{QME}}]]\\
    &\longrightarrow
      Z^{P_{0,\hbar_{\mathrm{QME}}}}_{\mathrm{fact}}\bigl(
      \mathrm{Obs}^{\hbar_{\mathrm{QME}}}_{\mathrm{open}}\bigr)\big|_{\mathfrak g_w}
  \end{aligned}
  \]
  extends to the weighted cotangent direction, with compatible
  \(\hbar_{\mathrm{QME}}\)-adic counterterms, as a continuous
  \(\C[[\hbar_{\mathrm{QME}}]]\)-linear cochain map precisely when the obstruction class in
  \(H^1(\mathcal O_{\mathrm{loc}}(\mathcal E_w),Q+\{I_0,-\})\)
...
\begin{proof}
  ...
  that identity are controlled.  The proposition is exactly the
  vanishing statement for the mixed brane-defect QME obstruction class.
\end{proof}
...
\begin{proof}
  ...
  Problem~\ref{prob:weighted-rg-locality} remaining after these analytic
  statements is the local-functional QME class displayed above. The
  \(\mathcal D'_c(I)\otimes\mathcal P[1]\) defect-current compatibility
  clause is the additional datum of
  Problem~\ref{prob:analytic-graph-realization}.
\end{proof}

\subsection*{Finite-window graph/QME assembly}

The QME obstruction class isolated by
Theorem~\ref{thm:hadamard-mittag-leffler} is a sum over Costello graphs
at every \(\hbar\)-order, each graph carrying its renormalized weight,
source-face, BV-bracket, and counterterm contributions.  Extracting it
requires a finite graph list \(\mathcal G_M\) on a finite Hamiltonian
window \(M\), with labels and incidence signs declared, and the
order-\(n\) residual written as a finite cochain.

\begin{defn}[Finite-window graph/QME system]\label{defn:finite-window-graph-qme-system}
  Fix an interval \(I\), an admissible weight \(w\), a finite Hamiltonian
  window \(M\), and a finite set \(\mathcal G_M\) of connected Costello
  graphs.  A finite-window graph/QME system over \((I,w,M,\mathcal G_M)\)
  ...
  \begin{enumerate}[label=(F\arabic*)]
    ...
    labelled arguments.  Denote the resulting source CE object by
    \(C^\bullet_{\mathrm{lab}}(\mathcal G_M)\).

    \item The finite-scale regulator data
    \[
    ...
    counterterms, and the \(d_M\)-boundaries and BV brackets appearing in the
    ...
    either the graph data lie in the balanced graph/counterterm
    subcomplex of
    Definition~\ref{def:app-balanced-costello-graph-subcomplex}, or the
    ...
    \]
    restricted to the chosen label set, whose zero-internal-edge part
    is \(\kappa^{\mathrm{coef},M}_{\hbar,w,I}\) of
    Proposition~\ref{prop:app-bulk-defect-kernel-map} and whose positive
    graph components are the renormalized weights
    \(W^{R,M}_{\Gamma,L,w}\).
  \end{enumerate}
  Summation over graph topologies is part of the datum exactly when a
  graph-summability hypothesis holds.
\end{defn}

\begin{thm}[Finite-window graph/QME assembly]\label{thm:finite-window-graph-qme-assembly}
  Let
  \((I,w,M,\mathcal G_M)\) carry a finite-window graph/QME system in the
  sense of Definition~\ref{defn:finite-window-graph-qme-system}.  Put
  \[
    S^\bullet=
    C^\bullet_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1][[\hbar_{\mathrm{QME}}]],
    \qquad
  ...
  Definition~\ref{def:app-bulk-defect-convolution-complex}.  Suppose
  ...
  \begin{enumerate}[label=(\arabic*)]
    \item The analytic graph weights in the system are finite-window
    local functionals satisfying the stated locality hypotheses.  For
    regular density labels this is
    Proposition~\ref{prop:app-regular-density-costello-graph-kernel}; for
    wavefront-admissible labels this is
    Proposition~\ref{prop:app-wavefront-admissible-costello-graph-kernel}.
    The statement applies to these regular-density and
    wavefront-admissible label classes.
    ...
    \[
      ...
      \right)
    \]
    is nonzero in \(S^1\), the order-\(n\) graph/QME equation has a
    scalar obstruction before passage to the non-scalar kernel.  In the balanced graph/counterterm
    subcomplex this scalar term is zero by
...
\begin{proof}
  The first assertion is exactly the analytic datum given by the
  ...
  Theorem~\ref{thm:hadamard-mittag-leffler} gives the Hadamard
  ...
  Hadamard coefficient with the pushed-forward current label requires the
  wavefront-admissible product condition to define a uniquely determined
  distribution.

  The scalar statement is formal.  A filtered chain projection
  \(\widehat\sigma_{\mathrm{sc},M}\) is part of the system.  If the
  displayed scalar projection of the residual is nonzero, then the
  residual has nonzero scalar component outside
  \(\ker\widehat\sigma_{\mathrm{sc},M}\), so every counterterm inside that
  kernel leaves the scalar component unchanged.  In the balanced graph/counterterm
  subcomplex the projection is the zero chain map, hence the scalar branch
...
\begin{prop}[Finite-window-to-limit obstruction]\label{prop:finite-window-qme-limit-condition}
  Suppose that finite-window graph/QME systems
  \(\{(I,w,M,\mathcal G_M)\}_{M\geq1}\) are given and satisfy the
  following additional hypotheses.
  \begin{enumerate}[label=(L\arabic*)]
    \item The graph system is compatible under truncation:
    ...
    finite in each window, or a Costello graph-summability hypothesis
    holds.  This graph-summability hypothesis is separate
    from the finite-window theorem.
    \item The residuals
  ...
  Hypotheses (L1)--(L3) are the scope of the inverse-limit graph/QME
  theorem.  When (L2) is unmet, the graph sum itself lies outside the
  tower, so the obstruction class is unformed.
\end{prop}

\begin{proof}
  Windowwise, Theorem~\ref{thm:finite-window-graph-qme-assembly} says that
  the only non-scalar obstruction is the class of
  \(\mathfrak o^{\mathrm{fw}}_{n,M}\) in \(H^1\).  If compatible
  primitives \(C_{n,M}\) exist, then
  \(d_MC_{n,M}=-\mathfrak o^{\mathrm{fw}}_{n,M}\) kills every fixed-window
  \(H^1\)-class, and
  \(\pi_{M,N}C_{n,M}-C_{n,N}=0\) kills every primitive-difference class in
  the Roos cokernel.  Thus both components of
  \(\mathfrak O^{\mathrm{fw}}_n\) vanish.

  Conversely, assume the two components vanish.  The vanishing of the
  compatible \(H^1\)-class gives primitives \(c_{n,M}\) window by window.
  Since the residuals commute with truncation,
  \[
    d_N(\pi_{M,N}c_{n,M}-c_{n,N})
      =
    -\pi_{M,N}\mathfrak o^{\mathrm{fw}}_{n,M}
      +\mathfrak o^{\mathrm{fw}}_{n,N}
      =0 .
  \]
  Passing to a cofinal successor tower, put
  \(A_M=H^0(\mathcal Q^\bullet_{\mathcal G,M}(I))\) and
  \[
    \Delta_{\mathcal G}\colon\prod_M A_M\longrightarrow\prod_M A_M,
    \qquad
    \Delta_{\mathcal G}((a_M)_M)
      =\bigl(a_M-\pi_{M+1,M}a_{M+1}\bigr)_M .
  \]
  The closed degree-zero differences
  \[
    \delta_M
      =
    [\pi_{M+1,M}c_{n,M+1}-c_{n,M}]\in A_M
  \]
  form the Roos \(1\)-cocycle whose cokernel class is
  \(\lambda^{\mathrm{fw}}_n\).  If
  \(\lambda^{\mathrm{fw}}_n=0\), choose closed degree-zero representatives
  \(a_M\) with
  \[
    \delta_M=[a_M-\pi_{M+1,M}a_{M+1}].
  \]
  Replacing \(c_{n,M}\) by \(c_{n,M}+a_M\) changes the successor
  primitive defect to a degree-zero boundary.  The finite-window
  truncation maps are coefficient projections; on the boundary part of the
  tower the same successor recursion solves the remaining boundary
  defect.  After this boundary adjustment one obtains primitives
  \(C_{n,M}\) with
  \(\pi_{M+1,M}C_{n,M+1}=C_{n,M}\), hence with
  \(\pi_{M,N}C_{n,M}=C_{n,N}\) for all \(M\geq N\).

  For the Mittag--Leffler assertion, replace \(A_M\) by the stable image
  of \(A_{M'}\to A_M\) for \(M'\gg M\).  The resulting cofinal successor
  system has surjective transition maps.  Given
  \(b=(b_M)_M\in\prod_M A_M\), choose \(a_1\) and lift
  \(a_M-b_M\) to \(a_{M+1}\) under \(A_{M+1}\to A_M\).  Then
  \(\Delta_{\mathcal G}(a)=b\), so
  \(\operatorname{coker}\Delta_{\mathcal G}
    =\varprojlim^1_MA_M=0\).  The weight-transport compatibility follows
  from the same diagonal transport squares used in
  Theorem~\ref{thm:wt-regulator-independence-admissible}.
\end{proof}

The order-\(n\) residual of
Proposition~\ref{prop:finite-window-qme-limit-condition} is a sum of
graph-differential, CE-source, BV-bracket, and lower-counterterm
contributions, each carrying its own incidence sign, scalar projection,
and truncation matrix.  Computing, testing, and propagating the
residual window by window requires a finite array of rows recording
every contribution in normal form.
...
\begin{defn}[Finite-window graph array]\label{defn:finite-window-graph-array}
  ...
  \begin{enumerate}[label=(A\arabic*)]
    ...
    Definition~\ref{defn:finite-window-graph-qme-system}.
    ...
    \]
    or the explicitly chosen smaller marker complex.  If the row comes from
    ...
    \item \emph{Curvature value and scalar condition.}  The row evaluates to a
    local functional
    \[
    ...
    \]
    An entrywise non-scalar summand is governed by a chain
    splitting of \(\widehat\sigma_{\mathrm{sc},M}\); the displayed
    scalar-zero sum is the canonical datum in the present array.
    ...
    \]
    A row whose coefficient labels lie entirely above degree \(N\) has
    all \(t^{M,N}_{\alpha\alpha'}=0\).  A filling of this equation is
    exactly the truncation entry required by the lower-window theorem.
    ...
    \[
      ...
      \gamma_{\mathbf 1}^{(2)}.
    \]
    Here \(\mathfrak m'_L\) is the transported marker on the resulting scalar
    loop \(L\), and \(\gamma_{\mathbf 1}^{(2)}\) denotes the ordered
    two-contact associated-graded scalar-contact symbol.  A surviving
    product \(\gamma_{\mathbf 1}^2\) in the target algebra is a separate
    datum.  The equality of resulting graphs, coefficients, and marker
    transports in~\eqref{eq:app-codim-two-marked-closure} is exactly the
    order-\(2\) composite-face datum.
  \end{enumerate}
...
\begin{prop}[First finite-window array rows]\label{prop:first-finite-window-array-rows}
  The universal computed rows are the reduced current interface row in
  order \(0\) and the two-Hamiltonian scalar-contact row in order \(1\).
  ...
  For the ordered Hamiltonian arguments \((a,z_1)\), \((b,z_2)\), the
  ...
  scalar-contact vertex \(\gamma_{\mathbf 1}\), and
  \[
  ...
  \begin{array}{c|c|c}
    ...
    \lie{gl}_N\text{ ordinary} & N\gamma_{\mathbf 1} & \text{scalar branch}\\
    \text{even block marker }\lambda_0P_{\bar0}+\lambda_1P_{\bar1}
      & N(\lambda_0-\lambda_1)\gamma_{\mathbf 1} & \text{formed when }
        \lambda_0=\lambda_1\\
  ...
  system, the first non-scalar residual is
  \[
    R^{\mathrm{ns}}_{1,M}
      =
    \sum_{\alpha\in
  ...
  \]
  after the scalar condition has vanished.  Its primitive is decided only
  after giving the remaining order-one rows of
...
\begin{proof}
  ...
  \(\operatorname{sdim}\C^{N|N}=0\) calculation.  Thus the row is already
  zero as a local functional in the full-equivariant branch.

  If the array contains only the displayed order-one row, the order-one residual is zero,
  ...
  it is a theorem exactly when the missing row values, signs, scalar conditions,
  and truncation coefficients are given and the resulting \(H^1\)-class
  vanishes.
\end{proof}
...
\begin{prop}[Universal scalar-contact bracket rows]\label{prop:universal-scalar-contact-bracket-rows}
  ...
  \(\pi_{M,N}S_{b,M}=0\).  These rows therefore have zero order-\(2\)
  obstruction and zero counterterm.
  ...
  graph system and must be fixed row by row.
\end{prop}
...
\begin{proof}
  ...
  conditions.  Truncation sends zero to zero, so the zero truncation matrix row is
...
\begin{prop}[Genuine order-two graph-weight rows]\label{prop:genuine-order-two-graph-weight-rows}
  ...
  \(\hbar\)-order \(2\): these are genuine seed rows, distinct from the
  codimension-two composites of two first scalar-contact boundary faces
  and from the bracket rows containing \(\alpha_{\mathrm{sc}}\) closed in
  ...
  Koszul, and symmetry factors given by the finite graph system.
  ...
  full-equivariant marked complex of
  ...
  \[
    ...
    S_{\mathrm{CE}(\Gamma,\nu),M}=0.
  \]
  This is a scalar-condition statement.  The local-functional values
  \(K^M_\Gamma\) and \(d_M K^M_\Gamma\) require their own row data.

  After scalar-condition vanishing, the genuine seed-graph contribution to the
  ...
  \end{aligned}
  \]
  The second sum is zero after strict CE-cycle evaluation.  A genuine
  ...
  Truncation is part of the row data.  For \(M\geq N\) one must choose
  ...
  v^{M,N}_{\Gamma,\nu;\Gamma',\nu'}\).  Rows whose coefficient labels lie
  entirely above degree \(N\) have zero truncation coefficients.  The
  displayed truncation identities are exactly the row entries required by
  the lower-window theorem.

  In the minimal full-equivariant local system of
  ...
  \]
  In any larger system, vanishing, counterterms, and obstruction are
  decided by the following data:
  the finite generator set \(\mathcal G^{\mathrm{gen}}_{2,M}\); the
  ...
  lies in the full-equivariant marked complex, or else its scalar
...
\begin{proof}
  ...
  The scalar-condition assertion follows from
  Theorem~\ref{thm:app-full-equivariant-marked-shadow-vanishing}: on the
  full-equivariant marked complex the filtered scalar projection is the
  zero chain map.  Since this projection only kills the scalar component, it
  gives a scalar-condition theorem for the non-scalar local functional
  \(K^M_\Gamma\).

  The counterterm criterion is exactly
  Definition~\ref{def:app-nonscalar-kernel-row-complex}: once the scalar
  condition is zero, the row sum is a degree-one element of the non-scalar
  ...
  graph and source rows.  In the minimal system the genuine seed-row set
  is empty, so the sum is empty.
\end{proof}
...
\begin{prop}[Order-two finite-window boundary rows]\label{prop:order-two-finite-window-boundary-rows}
  ...
  Proposition~\ref{prop:first-finite-window-array-rows}.  Assume the
  finite marked Costello list contains exactly these minimal rows: the
  genuine \(|\Gamma|_\hbar=2\) seed-graph set, the order-\(2\)
  CE-source-face set, and the order-\(2\) graph-counterterm-face set are
  empty.  Use \(C_{1,M}=0\).

  Then the only order-\(2\) boundary data required by codimension-two closure
  ...
  \end{aligned}
  \]
  The codimension-two closure equations identify the two resulting graphs,
  ...
  For any larger finite-window system, the order-\(2\) residual is
  decided by the added row data.  After scalar-condition vanishing its
  exact row sum is
  \[
  \begin{aligned}
    R^{\mathrm{ns}}_{2,M}
  ...
  only after giving the genuine two-cross/contact graph weights
  \(K^M_\Gamma\), the CE-source faces, the BV bracket rows between
  order-\(1\) graph components distinct from those containing
...
\begin{proof}
  ...
  Equation~\eqref{eq:app-codim-two-marked-closure} identifies the resulting graph,
  coefficient, and marker transport in the two composites, so the two
  boundary-composition rows cancel pairwise.

  The scalar-condition formula is the order-\(2\) instance of the last row datum
  ...
  full-equivariant marked complex, marker transport preserves the signed
  ...
  Under the stated minimal hypotheses the genuine order-\(2\) graph-row
  set, CE-source-row set, and order-\(1\) non-scalar primitive set are
  empty, and the full-equivariant trace calculation kills the remaining
  scalar-contact row.  The possible bracket row
  ...
  Theorem~\ref{thm:finite-window-nonscalar-curvature-criterion}.  The
  ...
  argument and
  Proposition~\ref{prop:app-first-scalar-zero-order-one-row}.  The
  remaining graph weights, signs, scalar projections, and truncation
  coefficients are the data defining \(R^{\mathrm{ns}}_{2,M}\), the
  order-\(2\) obstruction class, and its counterterm equation.
\end{proof}
...
\begin{prop}[Truncation matrices for the computed finite-window rows]\label{prop:computed-finite-window-truncation-matrices}
  ...
  Definition~\ref{defn:finite-window-graph-array}.  For \(M\geq N\), use the
	  ...
	  \(d_{\max}>N\), the row is truncated away and all entries
  ...
  \begin{enumerate}[label=(T\arabic*)]
    ...
    \item When genuine order-\(2\) seed rows are zero, as in the minimal
    full-equivariant system, their truncation matrices are empty.  If a
    larger system gives genuine seeds
    ...
    the coordinate entries of the finite-window projection.  These
    hypotheses give exactly the lower-window entries
    \(u^{M,N}_{\Gamma\Gamma'}\) and
    \(v^{M,N}_{\Gamma,\nu;\Gamma',\nu'}\) for those given genuine rows.

    \item Bracket rows \(b(\beta_1,\beta_2)\) with
    \(\beta_i\notin\{\alpha_{\mathrm{sc}},\alpha_{11}\}\) require data
    beyond the present local computation.  After a lower-window bracket row basis, or
    ...
    missing data for the enlarged system.
  \end{enumerate}
...
\begin{proof}
  ...
  labelled row for \(N\geq1\), and kills it only when the lower window
  omits the degree-one labels.  The diagonal matrices in (T1) and
  ...
  finite-window marked-complex compatibility preserves the resulting graph,
  ...
  bracket argument is zero as a local functional in the retained
  ...
  formulas are additional data beyond the cutoff; they are the required
  ...
  Conversely, those equations are exactly the statement that the given
...
\begin{prop}[Projection formulas for the \(u\), \(v\), and \(q\) rows]\label{prop:projection-defined-uvq-truncation-data}
  Work in a larger finite-window graph system in the local
  \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) full-equivariant marked
  branch.  Fix \(M\geq N\).  Suppose the system gives ordered
  ...
  If the system has a spanning family with relations, the
  coordinates are part of the presentation that fixes the
  ...
  \]
  This is the first coordinate equation after applying
  ...
  \[
    ...
    \sum_{\eta_1,\eta_2}
      ...
      \Theta^N_{\eta_1,\eta_2;\beta'_1,\beta'_2}.
  \]
  If the lower system retains all ordered normalized bracket rows with
  trivial row relations, then
  \[
    \Theta^N_{\eta_1,\eta_2;\beta'_1,\beta'_2}
      =
    \frac{
  ...
  Consequently the enlarged-system order-\(2\) non-scalar residual belongs
  to the compatible finite-window tower only after
  the following data are fixed: the genuine seed weights, source-face
  values, nonzero order-\(1\) rows, all orientation/Koszul/symmetry signs,
  CE-source transport, lower-window row bases or presentations, scalar-condition
  vanishing or scalar projections, and the projection-closure identities
  above.  With this system the obstruction is exactly the class of the
  resulting row sum in
  \(H^1(\mathcal K^\bullet_{\mathrm{ns},M}(I),d_{\mathrm{ns},M})\),
  together with the Roos compatibility class in
  \(\varprojlim^1 H^0(\mathcal K^\bullet_{\mathrm{ns},M}(I))\).  This
  system is the data defining the non-scalar QME obstruction class.
\end{prop}

\begin{proof}
  The first three displayed equations are the coordinate expansion of
  ...
  the system gives a presentation or a coordinate splitting.
  ...
  The cocycle identities follow from
  ...
  Definition~\ref{defn:finite-window-graph-array}.  Once they are fixed,
...
\begin{prop}[Finite-row primitive search interface]\label{prop:finite-row-primitive-search-interface}
  ...
  array at order \(n\), and suppose its scalar condition has vanished:
  \[
    \sum_{\alpha\in\mathsf A^{\mathrm{fw}}_{n,M}}S_{\alpha,M}=0.
  \]
  ...
  \]
  Then a counterterm in the given primitive span,
  \(C_{n,M}=\sum_j c_j\eta^M_j\), exists if and only if the finite linear
  system
  \[
    A^M c=-r^M
  \]
  is solvable.  When it is obstructed, any covector
  \(\ell\in{\left(\C^a\right)}^\vee\) satisfying
  \[
    \ell A^M=0,\qquad \ell(r^M)\neq0
  \]
  separates \(r^M\) from \(\operatorname{im}A^M\), hence the class of
  \(R_{n,M}\) is nonzero in the
  ...
  the primitive columns.  The row block \(T^{M,N}\) is the direct sum of
  ...
  \]
  Thus the primitive search has three conditions, in this order: scalar condition,
  fixed-window linear solvability, and Roos \(\varprojlim^1\)
	  compatibility.  The last condition is reached only after fixed-window
	  \(H^1\)-obstructions detected by such covectors \(\ell\) vanish;
	  in that case the fixed-window obstruction is decisive.
\end{prop}

\begin{proof}
  Once the scalar condition vanishes, the residual lies in the non-scalar kernel
  complex.  Restricting the possible counterterm to the given degree-zero
  span turns the equation
  \(d_{\mathrm{ns},M}C_{n,M}=-R_{n,M}\) into the displayed matrix equation.
  Solvability is equivalent to \(-r^M\in\operatorname{im}A^M\).
  In finite dimension this is detected by a linear functional annihilating
  \(\operatorname{im}A^M\) and pairing nontrivially with \(r^M\), exactly
  as written.

  The truncation identities say that the row residual and the boundaries of
  primitive columns commute with finite-window projection.  The computed
  blocks are those of
...
In the minimal full-equivariant array all order-one and order-two
rows close to zero
(Propositions~\ref{prop:first-finite-window-array-rows},
\ref{prop:universal-scalar-contact-bracket-rows},
\ref{prop:order-two-finite-window-boundary-rows}); the first
non-trivial obstruction therefore appears at order three.  The smallest
enlargement exhibiting a non-scalar row with nonzero local-functional
value is a single CE-source face attached to a genuine marked Costello
seed at \(\hbar^3\), the row \(\theta_3\) of
Proposition~\ref{prop:concrete-order-three-enlarged-system-row}, with
obstruction class \(\operatorname{Ob}^{\Pi}_{\theta_3}\) and
companion-face construction tracked through
Theorem~\ref{thm:theta-three-companion-face-obstruction}.

\begin{prop}[Concrete order-three row in an enlarged system]\label{prop:concrete-order-three-enlarged-system-row}
  Work in the full-equivariant marked branch with \(V=\C^{N|N}\), and let
  \(M\geq3\).  Enlarge the minimal finite-window system by one Hom-valued
  CE-source face row
  \[
    \theta_3=\mathrm{CE}(\Theta_3,\nu_3),
    \qquad |\Theta_3|_\hbar=3,
  \]
  where \(\Theta_3\) is a genuine marked Costello seed, distinct from a
  scalar-contact closure composite and from a bracket row.  The row data are
  as follows.
  ...
  \]
  Indeed, the row lies in the full-equivariant marked complex, so
  ...
  \]
  This equation is part of the valid obstruction-row data; with it
  given, the row defines
  \[
  ...
  Therefore this enlarged-system row is a nonzero local-functional
  cochain with vanishing scalar condition.  It is an explicit symbolic
  ...
  \]
  for some \(C_{\theta_3,M}\in\mathcal K^0_{\mathrm{ns},M}(I)\).  This
  row names the class; solving the larger order-three QME branch requires
  an added primitive or companion rows.
\end{prop}

\begin{prop}[\(\theta_3\) finite-row obstruction criterion]\label{prop:theta-three-finite-row-obstruction}
  Keep the one-row order-three system of
  Proposition~\ref{prop:concrete-order-three-enlarged-system-row}.  In the
  finite row complex given there, set
  \[
  ...
  \]
  This is an exact obstruction statement for the given finite row
  subcomplex.  A primitive in a larger local-functional complex requires
  extra data.  To kill the row one must add either a
  degree-zero primitive \(C_{\theta_3,M}\) with
  ...
  \(\varprojlim^1\)-class of this one-row tower is zero.  A nonzero
  truncation defect leaves zero fixed-window obstruction but represents
  the Roos class by
  \[
  ...
  given solution and its Roos class is zero.
\end{prop}

\begin{proof}
  This is Proposition~\ref{prop:finite-row-primitive-search-interface}
  applied to the one-row presentation.  The degree-zero row set is empty,
  so the image of the finite differential is zero.  The residual vector is the
  basis vector \(\mathsf E_{\theta_3,M}\), so the displayed covector is a
  cokernel separating covector.  The Roos statement is the same proposition applied
...
\begin{prop}[\(\theta_3\) primitive-entry criterion]\label{prop:theta-three-primitive-entry-criterion}
  Keep the row \(\theta_3=\mathrm{CE}(\Theta_3,\nu_3)\) and the notation of
  Propositions~\ref{prop:concrete-order-three-enlarged-system-row}
  and~\ref{prop:theta-three-finite-row-obstruction}.  A proposed
  degree-zero primitive for this row is admissible for the finite-row
  theorem only after the row array gives one of the following two
  primitive entries.
  ...
  \]
  the given primitive column \(C_{\theta_3,M}\) must have boundary-matrix
  coefficient
  \[
    A^M_{\theta_3,C}=-1,
    \qquad
    ...
    -\mathsf E_{\theta_3,M}.
  \]
  Admissible row data give the displayed differential entry
  with coefficient \(-1\) for the primitive.  Any other datum must first
  be replaced by an explicitly given primitive whose differential
  column is the one above.
  ...
  \]
  Thus such a primitive kills the fixed-window row and has zero Roos
  compatibility class.  The one-row complex here has
  \(K^0_{\theta_3,M}=0\), so
  Proposition~\ref{prop:theta-three-finite-row-obstruction} remains the
  active obstruction statement.

  A complete companion-face table is a different condition.  It replaces
  the one-face residual by a full
  ...
  admissible only if it gives every face, every CE coefficient, every
  scalar condition, the \(v^{M,N}\)-truncation matrices with their cocycle
  ...
  residual is \(\mathsf E_{\theta_3,M}\).  Hence it leaves the
  \(\theta_3\) row active and leaves the primitive Roos class unformed.
  The first
...
\begin{proof}
  ...
  \(r^M=(1)\).  The equation \(A^M c=-r^M\) is solved by the given
  primitive with \(c=1\) exactly when the primitive column is
  \(A^M_{\theta_3,C}=-1\).  The CE-ancestor and local-counterterm
  alternatives are precisely two ways of producing such a degree-zero
  column.  The displayed target differential, scalar projection, and
  support proof give the boundary matrix entry used by the finite row
  complex.
  ...
  For a companion-face table the residual vector itself is replaced by
  the signed row sum of all CE-source
  faces.  The finite-row theorem can test that new row sum only after the
  table gives its scalar conditions, truncation coordinates, and
  codimension-two closure data.  A nonzero signed sum remains a
  residual; a zero signed sum cancels before the primitive search and
  leaves the primitive Roos class unformed.
\end{proof}
...
\begin{thm}[\(\theta_3\) companion-face construction obstruction]\label{thm:theta-three-companion-face-obstruction}
  ...
  Proposition~\ref{prop:concrete-order-three-enlarged-system-row}, and put
  \(H_{p,q}=z_1^pz_2^q\).  On
  \((\C^2,\omega=dz_1\wedge dz_2)\) the Hamiltonian Poisson rule
  is
  \(\{H_{a,b},H_{r,s}\}=(as-br)H_{a+r-1,b+s-1}\); in particular
  \(\{H_{1,0},H_{2,1}\}=H_{2,0}\).  Consider the source-algebra ancestor
  \[
    \zeta^0_M=u_{a,H_{1,0}}\,u_{b,H_{0,1}}\,c_{B_\theta,\rho_{2,1}}.
  \]
  ...
  the two \(\rho_{1,1}\)-terms, the nonzero face set is the
  ...
  Its vanishing is an additional identity.
  ...
  \[
    ...
    2E^2_{\nu_{02}}-2E^2_{\nu_{20}}.
  \]
  Thus a fixed-window cancellation at \(M\geq3\) gives a compatible tower
  only after the lower-window identity
  \(E^2_{\nu_{02}}=E^2_{\nu_{20}}\) is proved in the chosen row basis, or
  after the lower-window row presentation is changed and its \(v\)-matrix is
  recomputed.
  ...
  \[
    ...
    \right)_{\nu,\nu'} .
  \]
  Here \(\mu_\nu\) is the marked result and marker-transport entry, and
  \(\mathcal C^{(2)}_{\nu,\nu'}\) is the codimension-two source-face
  incidence/weight table: the two orders of each pair of boundary
  operations, the resulting graph, the orientation/Koszul coefficient, the
  transported marker, and the resulting renormalized row coordinate in the
  declared degree-one basis.  Until this table is given and proves both
  \(R^{\mathrm{cand}}_{\Theta_3,M}=0\) and its transported lower-window
  identities, the proved conclusion remains the one-row obstruction
  of Proposition~\ref{prop:theta-three-finite-row-obstruction}.
\end{thm}
...
\begin{proof}
  ...
  The diagonal transport is finite-window projection on the Hamiltonian
  ...
  Source coefficients alone leave the renormalized graph weights,
  marker transports, codimension-two closure signs, or lower-window row
  coordinates undetermined.  Those entries are the displayed missing
  datum.  They are the data by which
  Definition~\ref{defn:finite-window-graph-array} gives a zero
  residual vector and Proposition~\ref{prop:finite-row-primitive-search-interface}
  still sees the current one-row obstruction.
\end{proof}

\begin{thm}[Finite-window non-scalar curvature criterion]\label{thm:finite-window-nonscalar-curvature-criterion}
  Fix a finite-window graph/QME system
  ...
  graph system, then the order-\(n\) graph curvature is
  \[
  \begin{aligned}
  ...
  \end{aligned}
  \]
  The signs are the orientation signs of the given finite graph
  ...
  The first scalar-contact graph whose coefficient is computed
  ...
  \]
  This is a scalar-condition vanishing.  A non-scalar primitive requires
  its graph-level construction.

  After the scalar condition vanishes, the first non-scalar order is the least
  \(n_0\) for which the cochain \(R_{n_0,M}\) is defined for the chosen
  graph list and survives the lower-order choices.  Its
  class is
  \[
  ...
  The abstract existence of \(\mathcal G_M\) gives the graph list but
  does not determine the coefficient array.  The integer \(n_0\), the
  class \([\mathfrak o^{\mathrm{fw}}_{n_0,M}]\), and the Roos class
  \(\lambda^{\mathrm{fw}}_{n_0}\) are determined only after a valid
  finite-window coefficient array has been chosen in the sense of
  Definition~\ref{defn:finite-window-graph-array}:
  \[
    \mathsf A^{\mathrm{fw}}_{n,M}
  \]
  ...
  scalar-contact filtration, scalar-condition term, row curvature value, and
  truncation coefficients in \(M\).  This array turns the formal
  obstruction symbol of
  Theorem~\ref{thm:finite-window-graph-qme-assembly} into a computation of
  \(n_0\), a test of
  \([\mathfrak o^{\mathrm{fw}}_{n_0,M}]=0\), and a defined
  \(\lambda^{\mathrm{fw}}_{n_0}\).
\end{thm}
...
\begin{proof}
  ...
  curvature in
  Definition~\ref{def:app-bulk-defect-convolution-complex}, evaluated on
  \(\zeta_M\) and then projected to the \(\hbar^n\)-coefficient.  Expanding
  \(-\kappa d_{\mathrm{CE}}\) and the CE coproduct bracket gives the
  three graph sums, with exactly the orientation signs given by the
  finite graph system.  Adding the lower counterterm terms gives
  ...
  computes the cyclic supertrace of every such resulting marker as a power
  of \(\operatorname{sdim}\C^{N|N}=0\), and the empty marker word gives the
  same zero coefficient.  Therefore the scalar condition vanishes on the
  full-equivariant marked complex before any non-scalar cohomology class
  is formed.

  Once the scalar condition is zero, \(R_{n,M}\) lies in
  \(\mathcal Q^\bullet_{\mathcal G,M}(I)=
  ...
  truncation differences are closed degree-zero cochains.  On the
  successor tower \(A_M=H^0(\mathcal Q^\bullet_{\mathcal G,M}(I))\), the
  map
  \[
    \Delta_{\mathcal G}((a_M)_M)
      =
    (a_M-\pi_{M+1,M}a_{M+1})_M
  \]
  has cokernel \(\varprojlim^1_MA_M\), and the classes
  \([\pi_{M+1,M}c_{n,M+1}-c_{n,M}]\) define the corresponding Roos
  \(1\)-cocycle.  Vanishing of this cokernel class is exactly the
  condition that the primitives can be adjusted, first by closed
  degree-zero representatives and then by boundary corrections, to a
  compatible family.
  ...
  cochain \(R_{n,M}\), so its first nonzero order and its cohomology class
  both become undefined.
\end{proof}

\begin{rmk}[Scalar-condition boundary]\label{rmk:finite-window-scalar-condition-boundary}
  ...
  \(\hbar^1\)-coefficient.  In the even block-diagonal marked complex a
  single marker
  ...
  full-equivariant hypotheses; they are scalar-contact rows.
\end{rmk}
...
\begin{rmk}[Relation to regulator independence]\label{rmk:hadamard-regulator-independence}
  ...
  endpoint obstruction of Theorem~\ref{thm:strict-unweighted-obstruction}: the
  required coefficient Casimir has an obstructed strict product/direct-sum
  representative.
\end{rmk}
...
\begin{proof}
  ...
  \(q_N(\rho_{N+1,0})=0\) by the killing hypothesis, this would imply
  \[
...
\begin{proof}[Proof of Corollary~\ref{cor:promatlis-retract-compatible-replacement}]
  ...
  on which the kernel acts; (c) identifies the fixed inverse-limit receiver
