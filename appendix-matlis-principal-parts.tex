\section{The pro-Matlis envelope: Matlis principal parts and the continuous cotangent module}\label{sec:pro-matlis-target}\label{sec:app-matlis-principal-parts}
...
The brane reads positions polynomially; its cotangent momenta live as
residues.  The same local-cohomology object also gives the
two-point diagonal coefficients of the native holomorphic singular
product: after replacing
the origin support \(\mathfrak m=(z_1,z_2)\) by the diagonal
\(\Delta\subset\C^2_z\times\C^2_w\), the basis becomes
\(\rho^\Delta_{a,b}=(z_1-w_1)^{-a-1}(z_2-w_2)^{-b-1}d\zeta_1d\zeta_2\),
and Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product} uses
\(\rho^\Delta_{0,0}\) as the two-dimensional Cauchy kernel.
...
constant line; after the chosen residue normalization, the residue
pairing identifies it with the direct sum
\(\mc P=\bigoplus_{a+b>0}\C\,\rho_{a,b}\).  The Hamiltonian
...
locally nilpotently on \(\mc P\), giving the basis-relabeling
obstruction.

In this appendix the compact subscript \(\hbar\) in
\(\mathcal D_\hbar\), \(\mathcal O_\hbar\),
\(\Delta^{\mathsf F}_\hbar\), and the associated \(\hbar\)-adic lattice
phrases denotes the algebraic Fourier--Rees/PBW variable
\(\hbar_{\mathrm{Rees}}\).  Its comparison with the formal Weyl/Moyal
coefficient formulas is the specialization
\(\hbar_{\mathrm{Rees}}\mapsto\hbar_{\mathrm{W}}\).  It is not the
Costello quantum master-equation parameter \(\hbar_{\mathrm{QME}}\) and
not the equivariant line parameter \(\hbar_\omega\).

Fourier--Rees conjugation gives the only universal
...
realization sits inside the \v{C}ech local-cohomology lattice
\(H^2_{\mathfrak m}(\C[[z_1,z_2]])\,dz_1dz_2[[\hbar]]\) as
...
\cite{hartshorne-local-cohomology}*{Ch.~III, \S2}, and the
power-series residue model of Kunz--Cox--Dickenstein
\cite{kunz-residues-duality}*{Ch.~5}.  The exact cotangent statement used
below is the manuscript's finite-window principal-part realization and
coadjoint-continuity theorem, not an unqualified appeal to local duality.
...
Lemma~\ref{lem:formal-stalk-trace}: a polynomial Hamiltonian becomes
the brane configuration observable
\(O_f=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).  A formal
Hamiltonian \(f\in\mathfrak h\) gives a completed Taylor function of
the Chan--Paton pair.  The cotangent
...
constraints together select the residue-valued cotangent polarization.
...
\label{thm:app-defect-polarization}
  ...
  \begin{enumerate}
    \item \emph{Configuration side: polynomial / Taylor.}  Hamiltonian
      configuration observables on the brane are first the polynomial trace
      family
      \[
        O_f=\overline{\operatorname{Tr}f(\phi_1,\phi_2)},\qquad
        f\in\bar A=\C[z_1,z_2]/\C\cdot1 .
      \]
      Formal \(f\in\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\) are read by
      Taylor windows and stable completion.
    ...
    \item \emph{Cotangent side: distributional / residual.}  Cotangent
      ...
      The two-point diagonal version replaces \(\rho_{a,b}\) by
      \(\rho^\Delta_{a,b}\); its \((0,0)\) generator is the kernel
      \(K^{(2)}_\Delta\) in the native singular product.
  \end{enumerate}
  The two sides are distinct \(\mathfrak h\)-modules.  The
  bidegree relabelling
  \(\widetilde\Psi_{a,b}\leftrightarrow\rho_{a,b}\) carries the same
  index set \((a,b)\) and has the local-nilpotence obstruction to
  intertwining the two Hamiltonian actions: linear-Hamiltonian local
  nilpotence holds on the polynomial side, while the residual side has
  non-locally-nilpotent action
  ...
  one-\(\psi\) label module has the same obstruction after inverting
  \(\hbar\), and the Fourier--Rees label bridge of
...
\begin{proof}
  ...
  have infinite orbits on the residual side, by the same theorem, so the
  same-action module isomorphism is obstructed.  The unique universal label bridge
  ...
  Corollary~\ref{cor:app-matlis-same-action-interpolation-obstruction}.
  The residue-coadjoint pairing is unique up to one scalar by
  Theorem~\ref{thm:app-matlis-residue-rigidity}.
\end{proof}

\begin{rmk}[Finite-window verification of the polarization]
\label{rmk:app-defect-polarization-numerics}
  The exact rational calculation of
  \(\operatorname{ad}^\vee_{z_1^pz_2^q}(\delta_{a,b})\)
  on Taylor-monomial test labels gives the residual-side coefficient
  \(-(pb-qa+p-q)\), identifies the
  ...
  The same calculation evaluates the bidegree-only label map
	  ...
	  polynomial one-\(\psi\) action lacks at every order in
  ...
  different powers of \(\hbar\).  The Moyal star-bracket coefficients
  on monomials up to exponents \(\le 10\) and orders \(\le 11\) have
  antisymmetric odd-order tower
  \(1,\ \tfrac1{24},\ \tfrac1{1920},\ \tfrac1{322560},\ \dots\) matching
  the closed-form \(\tfrac{1}{2^{r-1}\,r!}\) for odd \(r\).
\end{rmk}
...
\label{def:app-matlis-topology}
  ...
  \(\{f,\mathfrak m^{N+1}\}\subset \mathfrak m^N\).  The quotient
  \(\mathfrak h\) removes the constant class, which acts by zero.
  The finite Taylor window of total degree at most \(N\) is
  \[
    \mathfrak h^{\mathrm{Taylor}}_{\leq N}
      =\mathfrak h/F^{N+1}\mathfrak h
      \cong
      \bigoplus_{\substack{a,b\geq0\\0<a+b\leq N}}
        \C z_1^az_2^b ,
  \]
  and for \(M\geq N\) the transition
  \(r^{\mathrm{Taylor}}_{M,N}\colon
  \mathfrak h^{\mathrm{Taylor}}_{\leq M}\to
  \mathfrak h^{\mathrm{Taylor}}_{\leq N}\) is
  truncation of Taylor monomials of total degree \(>N\).  Thus
  \(\mathfrak h=\varprojlim_N\mathfrak h^{\mathrm{Taylor}}_{\leq N}\) in the
...
\label{lem:app-matlis-continuous-dual}
  ...
  \[
    \mathfrak h^\vee_{\mathrm{cont}}
      =\varinjlim_{N\geq1}
        \bigl(\mathfrak h^{\mathrm{Taylor}}_{\leq N}\bigr)^\vee
...
\begin{proof}
  ...
  \(\mathfrak h^{\mathrm{Taylor}}_{\leq N-1}\), hence is a finite linear combination
  of Taylor coefficient functionals.  Conversely every finite linear
  combination of the displayed \(\delta_{a,b}\) kills \(F^{K+1}\mathfrak h\)
  for all sufficiently large cutoffs \(K\), hence is continuous.
\end{proof}
...
\label{def:app-matlis-principal-parts}
  ...
  the representative basis determined by the \v{C}ech quotient, in
  which both exponents are
  strictly negative.
  The basis vector \(\rho_{0,0}=z_1^{-1}z_2^{-1}dz_1dz_2\) is the nonzero
  functional in \(H^2_{\mathfrak m}(R)\,dz_1dz_2\) which evaluates
  ...
  locally finite topological vector spaces.  The space \(\mc P\) is a
  Hamiltonian-coadjoint module; the \(R\)-submodule structure of
  \(H^2_{\mathfrak m}(R)\,dz_1dz_2\) is
  obstructed on \(\mc P\), since
...
\begin{lem}[Finite Matlis windows and completions]
\label{lem:app-matlis-finite-windows}
  For each \(N\geq1\) the residue pairing restricts to a perfect finite
  pairing
  \[
    \langle-,-\rangle_N\colon
    \mc P_{\leq N}\otimes
    \mathfrak h^{\mathrm{Taylor}}_{\leq N}\longrightarrow\C,
    \qquad
    \langle\rho_{a,b},z_1^mz_2^n\rangle_N
      =\delta_{a,m}\delta_{b,n}.
  \]
  For \(M\geq N\), the dual of the Taylor truncation
  \(r^{\mathrm{Taylor}}_{M,N}\colon
  \mathfrak h^{\mathrm{Taylor}}_{\leq M}\to
  \mathfrak h^{\mathrm{Taylor}}_{\leq N}\) is the
  inclusion
  \[
    (r^{\mathrm{Taylor}}_{M,N})^{\vee}\colon
    \mc P_{\leq N}\hookrightarrow \mc P_{\leq M}.
  \]
  Hence the strict continuous dual is the direct limit
  \[
    \mathfrak h^\vee_{\mathrm{cont}}
      \cong \varinjlim_N \mc P_{\leq N}=\mc P .
  \]
  ...
  \[
    ...
    \prod_{\substack{a,b\geq0\\a+b>0}}\C\,\rho_{a,b}
      \cong
    \varprojlim_N\mc P_{\leq N},
  \]
  uses the projections
  \(p_{M,N}\colon\mc P_{\leq M}\to\mc P_{\leq N}\) which discard basis
  vectors of total degree \(>N\).  This product pro-Matlis completion is
  a larger kernel completion; it is not the ordinary continuous dual of
  \(\mathfrak h\).
\end{lem}

\begin{proof}
  The displayed monomial bases of \(\mathfrak h^{\mathrm{Taylor}}_{\leq N}\) and
  \(\mc P_{\leq N}\) have the same finite index set, and the residue
  matrix is the identity matrix in those bases.  Truncating a Taylor
  polynomial from degree \(\leq M\) to degree \(\leq N\) has transpose
  inclusion of the coefficient functionals with degree \(\leq N\).  The
  direct-limit assertion is therefore Lemma~\ref{lem:app-matlis-continuous-dual}
  transported through the residue basis.  The inverse-limit assertion
  follows from the compatible finite projections \(p_{M,N}\): a point of
  \(\varprojlim_N\mc P_{\leq N}\) is exactly a choice of one coefficient
  for every basis vector \(\rho_{a,b}\), with no finite-support
  condition.
\end{proof}
...
\label{prop:app-matlis-realization}
  ...
  \(E_R(\C)\) can hit the excluded scalar residue line under
  multiplication.  The Hamiltonian coadjoint action is
...
\begin{proof}
  ...
  the continuous dual of the quotient \(R/\C\cdot1\).  On finite truncations
...
\label{prop:app-matlis-coadjoint-action}
  ...
  is zero; hence the action preserves \(\mc P\) and avoids the scalar
  residue line,
  ...
  polynomial PBW one-\(\psi\) label module the analogous coefficient is the
...
\begin{proof}
  ...
  signed coefficient is verified by direct computation on \(1225\)
  cases with exponents \(\le 5\).
\end{proof}
...
\begin{proof}
  Fix the finite principal-part window \(\mc P_{\leq N}\).  If
  \(a+b\leq N\), a monomial term
  \(z_1^p z_2^q\cdot\rho_{a,b}\) can be nonzero only when
  \(p\leq a+1\) and \(q\leq b+1\), hence \(p+q\leq N+2\).  Therefore the
  restriction
  \[
    \mathfrak h\times\mc P_{\leq N}\longrightarrow \mc P
  \]
  factors through the finite-dimensional quotient
  \[
    \mathfrak h^{\mathrm{Taylor}}_{\leq N+2}\times \mc P_{\leq N}.
  \]
  Equivalently, \(F^{N+3}\mathfrak h\) acts trivially on
  \(\mc P_{\leq N}\).  This proves well-definedness for formal
  Hamiltonians and continuity in the \(\mathfrak h\)-variable on each
  finite principal-part window.

  The target is also finite-window controlled.  If \(a+b\leq N\), every
  nonzero target has total index
  \[
    (a-p+1)+(b-q+1)=a+b-(p+q)+2\leq N+1
  \]
  because \(p+q>0\) for a reduced Hamiltonian.  Hence each fixed
  Hamiltonian sends \(\mc P_{\leq N}\) into \(\mc P_{\leq N+1}\), and the
  bilinear map on the finite stage factors as
  \[
    \mathfrak h^{\mathrm{Taylor}}_{\leq N+2}\times\mc P_{\leq N}
      \longrightarrow \mc P_{\leq N+1}.
  \]
  The locally finite topology on \(\mc P=\varinjlim_N\mc P_{\leq N}\) is
  the final topology for these finite-dimensional inclusions, so these
  finite-stage factorizations prove continuity of the bilinear coadjoint
  action.
\end{proof}
...
\label{thm:app-matlis-polynomial-descendant-obstruction}
  ...
  every \(\mathfrak h\)-module map from \(\mc P\) to \(M\) has zero image.
  In particular the polynomial PBW one-\(\psi\) descendants
  \(\widetilde\Psi_{a,b}\) and the principal parts \(\rho_{a,b}\) have a
  nonzero ordinary \(\mathfrak h\)-module identification obstruction.
\end{thm}
...
\begin{proof}
  ...
  Therefore a polynomial descendant module with locally nilpotent
  linear Hamiltonians has the local-nilpotence obstruction to realizing
  \(\mc P\), or any nonzero submodule of \(\mc P\).  The obstruction is
  independent of \(\hbar\) and survives
  ...
  Hamiltonians on the polynomial one-\(\psi\) label module is
  \[
  ...
  \(\lambda_{a,b}\neq0\) preserves the obstruction: the linear
...
\label{thm:app-matlis-rees-fourier-bridge}
  ...
  \]
  carries the \(\mathcal D_\hbar\)-action
  \(z_i\cdot\eta=z_i\eta\) and
  \(\partial_i\cdot\eta=\hbar\partial_{z_i}\eta\).  Sending
  ...
  \(\C((\hbar))\).  At the \(\C[[\hbar]]\)-lattice level it is the
  displayed Rees sublattice.  The Rees Fourier
  automorphism
  \[
  ...
  Fourier twist of action displayed above.  Over the Rees lattice before
  ...
  associated-graded label match; ordinary local-cohomology lattices
  retain the displayed Rees shift.
  ...
  \]
  have infinite orbits on every nonzero reduced principal part after
...
\begin{proof}
  ...
  \v{C}ech local-cohomology lattice only after \(\hbar\) is inverted.
  Therefore
  ...
  Matlis action is distinct from the Fourier-transformed action.  It keeps the
  ...
  \[
    ...
    \hbar X_{z_2}\rho_{a,b}
      ...
      =(a+1)\hbar\rho_{a+1,b}.
  \]
  After tensoring with \(\C((\hbar))\), every nonzero vector in
  \(\mc P((\hbar))\) has an infinite orbit under either linear
  ...
  Thus a Fourier--Rees comparison preserving the original Hamiltonian
  action has the same-action obstruction; the positive construction
  changes the polarization and is a flat \( \mathcal D_\hbar \)-module
  bridge for the Fourier-conjugated action stated above.
\end{proof}

\begin{cor}[Same-action obstruction criterion]
\label{cor:app-matlis-same-action-interpolation-obstruction}
  ...
  \(M_\hbar((\hbar))\) and \(\mc P((\hbar))\) with the Matlis
  coadjoint action have a same-action module-isomorphism obstruction.
  The same statement holds for the Rees-normalized
...
\begin{proof}
  ...
  \(\hbar\) preserves local nilpotence.
\end{proof}
...
\label{rmk:app-matlis-rees-bridge-scope}
  ...
  sublattice \(\bigoplus\hbar^{a+b}\C[[\hbar]]\rho_{a,b}\).  The full
  \(\bigoplus\C[[\hbar]]\rho_{a,b}\) appears after \(\hbar\) is inverted.
