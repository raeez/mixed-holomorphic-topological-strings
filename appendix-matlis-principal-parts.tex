...
The defect polarization of the brane line is not a free choice.
Configuration observables on the brane are the polynomial trace classes
\(O_f=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\); these are
\emph{Taylor} in the matrix-valued normal coordinates and form the PBW
special-fibre alphabet.  Cotangent momenta on the same brane, by
...
are therefore Taylor functions of the probe pair.  The cotangent
...
action coming from the Poisson bracket on the disk.  Those three
...
\subsection{Citations into the body of the manuscript}
\label{ssec:app-matlis-dependency-map}

\begin{rmk}[Where the appendix is used]
\label{rmk:app-matlis-what-supports-what}
  \begin{enumerate}
    \item The Matlis realization
      (Proposition~\ref{prop:app-matlis-realization}) and the coadjoint
      formula \eqref{eq:app-matlis-coadjoint-monomial}
    ...
    \item Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction}
      and Corollary~\ref{cor:app-matlis-no-flat-same-action-interpolation}
      are the local-nilpotence obstructions cited in
      Proposition~\ref{prop:no-polynomial-principal-part-boundary} and
      Theorem~\ref{thm:pbw-vs-deformation}: the polynomial PBW
      one-\(\psi\) descendant labels do not realise the continuous
      cotangent module, classical or after Moyal deformation.
    \item Theorem~\ref{thm:app-matlis-rees-fourier-bridge} is the
      Fourier-polarised label bridge cited in
      Remark~\ref{rmk:intro-symbol-separation} and in
      Theorem~\ref{thm:pbw-vs-deformation}: the shared bidegree index
      set \((a,b)\) is a Fourier--Rees associated-graded comparison
      rather than an \(\mathfrak h\)-module isomorphism.
