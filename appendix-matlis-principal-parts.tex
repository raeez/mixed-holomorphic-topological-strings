...
residues.  The same local-cohomology object also supplies the
...
obstruction.  Fourier--Rees conjugation supplies the only universal
...
\label{thm:app-defect-polarization}
  ...
  nilpotence holds on the polynomial side and fails on the residual side
...
\label{rmk:app-defect-polarization-numerics}
  ...
  The same script evaluates the candidate label map
  ...
  scripts run from the command line with zero external dependencies.
\end{rmk}
...
\label{def:app-matlis-principal-parts}
  ...
  locally finite topological vector spaces, rather than a bare algebraic
  relabelling.  The space \(\mc P\) is a Hamiltonian-coadjoint module;
