...
\emph{Load-bearing role.}  Principle~4 of the introduction asserts
that defect momentum is residual: the continuous cotangent module of
the reduced Hamiltonian disk algebra is the Grothendieck--Matlis
principal-part module, paired with Taylor functions by residue, and
not a polynomial copy.  This appendix supplies the construction.  The
load-bearing outputs are the Matlis realisation of
\(\mathfrak h^\vee_{\mathrm{cont}}\) as
\(\mc P=\bigoplus_{a+b>0}\C\rho_{a,b}\)
(Proposition~\ref{prop:app-matlis-realization}), the residue-coadjoint
action
(Proposition~\ref{prop:app-matlis-coadjoint-action}), residue-pairing
rigidity
(Theorem~\ref{thm:app-matlis-residue-rigidity}), the polynomial
descendant obstruction
(Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction}),
and the universal Fourier--Rees label bridge
(Theorem~\ref{thm:app-matlis-rees-fourier-bridge}).  These are the
inputs to
...
Proposition~\ref{prop:finite-window-bm-native-e2-transfer} that
record the matrix-probe versus residue-defect separation governing the
unreduced \(P_0\)-factorization-centre lift.

The \(\mathfrak m\)-adic continuous dual of the reduced Hamiltonian Lie
algebra is the scalar-annihilator inside
\(H^2_{\mathfrak m}(\C[[z_1,z_2]])\,dz_1dz_2\).  The residue pairing
gives the coadjoint action.  Polynomial descendants carry a different
\(\mathfrak h\)-module structure.  Fourier--Rees conjugation gives a
\(\mathcal D_\hbar\)-module bridge after changing polarization; over
\(\C[[\hbar]]\) its target is the Fourier delta Rees lattice, and the
full \v{C}ech principal-part lattice appears after tensoring with
\(\C((\hbar))\).  The source conventions are
Matlis duality
...
\label{def:app-matlis-principal-parts}
  ...
  \]
  We give \(\mc P\) the locally finite direct-sum topology
  \[
...
\label{prop:app-matlis-coadjoint-action}
  ...
  displayed target is \(\rho_{0,0}\), then the coefficient is zero; hence
  the scalar residue line is not produced from \(\mc P\).
\end{prop}
...
\begin{proof}
  ...
  \(pb-qa+p-q=p(q-1)-q(p-1)+p-q\) is zero.  Thus the action preserves the
  scalar-annihilator \(\mc P\).
\end{proof}
...
\begin{proof}
  ...
  For the primitive PBW labels \(\widetilde\Psi_{a,b}\) this obstruction
  is already visible at the boundary of the index quadrant.  The special-fibre action
  has
  \[
  ...
  \]
  after the scalar descendant is removed.  Thus \(z_1\) kills
  \(\widetilde\Psi_{a,0}\), \(a>0\), whereas
  \(z_1\cdot\rho_{a,0}=-\rho_{a,1}\neq0\).  No rescaling
  \(\widetilde\Psi_{a,b}\mapsto\lambda_{a,b}\rho_{a,b}\) with all
  \(\lambda_{a,b}\neq0\) can intertwine the two actions.
\end{proof}
...
\label{rmk:app-matlis-what-supports-what}
  ...
  \begin{enumerate}
    \item Proposition~\ref{prop:app-matlis-realization} and
      ...
      monomial coadjoint formula is the formula used in those
      theorems.
    ...
    \item Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction}
      ...
      realise the continuous cotangent module.
    \item Theorem~\ref{thm:app-matlis-rees-fourier-bridge} is the
      Fourier-polarised label bridge cited in the body Lie/PBW
      separation paragraph (around line~454 of \texttt{main.tex}) and
      again in Theorem~\ref{thm:pbw-vs-deformation}: it makes the
      shared bidegree index set \((a,b)\) into a Fourier--Rees
      associated-graded comparison rather than an
  ...
  a §1 or §2 theorem listed above.  The appendix therefore stands
  exactly as the unique-survivor account of the residual, not
  polynomial, character of the cotangent direction at the brane point.
\end{rmk}
