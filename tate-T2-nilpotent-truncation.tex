...
\emph{Closure-layer organizing question.}  Which subalgebra of the
formal Hamiltonian algebra $\mathfrak h$ admits a finite-dimensional
nilpotent truncation satisfying the lower-central Tate-pronilpotence
hypothesis of Lemma~\ref{lem:continuous-bar-cobar}, and which sectors
must be removed a priori for that hypothesis to hold?

\emph{Named obstruction terms.}  For every $N\geq 1$,
\[
...
$\mathfrak o^{\mathrm{lin}}_N(z_1;z_2^{N+1})=(N+1)z_2^N$; it rules out
$\mathfrak m^{N+1}$ as a Lie ideal once $z_1\in H_1$ is admitted.  The
last two are the semisimple obstruction
...
e_1^i+\sum_ie_{2,i}\otimes e_2^i$, these are the obstruction triple
of Definition~\ref{defn:nilpotent-low-degree-obstruction-terms}.

\emph{Unique survivor.}  Theorem~\ref{thm:phi-trunc-classical}:
classically, on the cubic ideal
$\mathfrak h_{\leq N}=\mathfrak m^3/\mathfrak m^{N+1}$, the finite
cotangent CE/PV identity
$\Phi_{\leq N}\colon
C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g_{\leq N,I})
\xrightarrow{\sim}
\mathrm{PV}(\widehat{\mathbf S}(\mathfrak h_{\leq N,I}))$ is
unconditional.
...
recovers $\mathfrak m^3\subsetneq\mathfrak h$, not the whole.

\emph{Residual.}  Problem~\ref{prob:linear-sector-lift}: extend
$\Phi_{\mathfrak m^3}$ to $\mathfrak h=\mathfrak m$ by supplying a
...
that handles the semisimple
$H_2\cong\mathfrak{sl}_2$ direction.  The truncation route does not
solve $(\mathfrak o^{\mathrm{lin}}_N,\mathfrak o^{\mathrm{ss}},
\mathfrak o^{\mathrm{Cas}}_{\leq2})$; it removes the sector on which
those obstructions are felt.
