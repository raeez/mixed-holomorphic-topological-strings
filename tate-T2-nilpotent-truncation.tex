...
unconditional in the classical sector
...
Problem~\ref{prob:linear-sector-lift} extends
$\Phi_{\mathfrak m^3}$ to $\mathfrak h=\mathfrak m$ by supplying a
coefficient category in which $\mathfrak o^{\mathrm{Cas}}_{\leq2}$
converges and a relative or reductive replacement for the
lower-central Tate-pronilpotent route handling the semisimple
$H_2\cong\mathfrak{sl}_2$ direction.
...
\begin{thm}[Classical truncated cotangent CE/PV identification]\label{thm:phi-trunc-classical}
Set
...
CE/PV cotangent identification of the reduced Hamiltonian model; it
is not an unreduced open BV factorization-centre morphism. The
...
principal-part labels in the CE/PV identification; their unreduced
boundary centrality homotopies remain part of
...
Extend the coordinate-window cotangent lift datum $\Phi_{\mathfrak m^3}$
of \ref{prop:colim-recovery} from $\mathfrak m^3$ to all of
$\mathfrak h=\mathfrak m$ by supplying:
...
closes the classical nilpotent cotangent identification on
$\mathfrak m^3$,
and the residual moves from \ref{prob:formal-factorization-center} as
stated to its restricted form on the low-degree sector
