...
Remark~\ref{rmk:colim-does-not-recover-full} shows the inverse limit
...
subalgebra and is not nilpotent, violating the lower-central
...
absent from $\mathfrak m^3$.  The Casimir is the finite sum
...
\begin{proof}
  ...
  evaluated on generators of degrees $3,\ldots,N$.  That operation does not
...
of \(\mathfrak m^3\).  The variance failure appears already at
...
where the truncation is semisimple rather than nilpotent. Recovering the full
...
apply directly to the resulting Lie algebra.  Ordinary finite-dimensional
bar-cobar and PBW formalism do not fail for \(\mathfrak{sl}_2\); the
