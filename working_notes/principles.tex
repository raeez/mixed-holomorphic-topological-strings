...
Moyal quantization as the Capelli shift
\(\hbar_{\mathrm{W}}N[\bar c]\) and in the Costello scalar-contact
complex as the QME scalar \(\hbar_{\mathrm{QME}}N[\bar c]\).}
...
centre-of-mass mode, so the closed-open mismatch equals the
transgressed class of this quotient sequence.  Algebraic deformation quantization of the
polydisk by the Moyal star product converts the same scalar into the
order-\(\hbar_{\mathrm{W}}\) Capelli term, with
\(\hbar_{\mathrm{W}}N\) the Weyl coefficient; the first
scalar-reduced \(\mathfrak{gl}_N\) QME obstruction is therefore
...
(Definition~\ref{def:E1-brane-algebra}).  The finite-\(N\) generator
boundary value \(\Phi_N^{\mathrm{gen}}\) has reduced-coordinate shadow
the CE/PV dictionary.  A Hochschild-centre map
\(\Phi_N^{\mathrm{Hoch}}\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}\to
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) requires the
completed centrality Maurer--Cartan lift
(Theorem~\ref{thm:closed-to-open-derived-centre-map}).  The native
  bulk is a holomorphic factorization algebra on \(\C^2\)
  valued in \(E_2\)-algebras with the diagonal local-cohomology singular
  product of
  Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}.
  A curve vertex algebra enters only after a restriction datum
...
coherence; a non-absorptive class requires an enlargement of the target
...
\(D^\square_{a,b}=C^+_{a,b}\partial_2\).  At order
\(\hbar_{\mathrm{W}}^1\), the scalar shift
\(\hbar_{\mathrm{W}}N[\bar c]\) of Principle~2 is the universal
...
H^1\bigl(\mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I),\,d_{\mathrm{QME}}\bigr)\)
...
anomaly \(\hbar_{\mathrm{W}}N[\bar c]\) is the central-extension datum for the
projective Hamiltonian action on the scalar-reduced trace sector; the
reduced non-scalar QME class
\([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}}}]\) is the
...
others obstruct that target and require a replacement
