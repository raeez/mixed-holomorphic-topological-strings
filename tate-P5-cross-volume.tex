...
% local theorem without a proof of vanishing of this class.
...
\[
  ...
  \chi_{\mathrm{cl}},
\]
and external use is governed by the acceptance residual class
\[
...
\paragraph{Functorial form.}  The cross-volume relation is not raw
equality of objects across distinct categories.  In the three-volume
architecture
\[
  \mathcal C_{\mathrm{CY}}
  \xrightarrow{\Phi^{\mathrm{FA}}_d}
  E_d\text{-}\mathrm{HolFA}(X)
  \xrightarrow{\mathrm{Sp}_{\Sigma_{d-1},C}}
  A
...
\]
the local theorem on
\(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\) computes
the final arrow only on a formal Hamiltonian Darboux normal slice at
a brane stack: it identifies
\(\operatorname{res}_{L,N}Z^{\mathrm{der}}_{\mathrm{ch}}(A)\simeq
C^\bullet_{\mathrm{CE}}(\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1])\)
as the protected Hamiltonian sector and reads it on the brane through
\(u_f\mapsto J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).  No
identification of objects is asserted across an unproved functor: any
target use must factor through the matched-conventions datum
\(\mathfrak C_{\mathrm{tar}}\) of
Definition~\ref{defn:matched-conventions-theorem-datum} and the
universal Koszul-descent acceptance class
\(\operatorname{Ob}_{\mathrm{UKD}}\) below.

\paragraph{No raw scalar equalities.}  In particular, equalities such
as \(K(A)=\kappa_{\mathrm{BKM}}\), an alleged identification of the
local Hamiltonian scalar cocycle \(\hbar N[\bar c]\) with a
Borcherds--Kac--Moody \(\Delta_5\) scalar, or any naive numerical
match between Vol~I conductor invariants, Vol~II conformal anomalies,
and Vol~III Borcherds weights, are not asserted by the local theorem.
The Vol~III material itself records the operator-level distinction in
canonical cases: at the \(K3\times E\) Calabi--Yau witness the
Heisenberg chiral scalar, the Borcherds weight, and the
Mukai/conductor invariant are numerical outputs of distinct trace
theorems, and any universal trace identity must be operator-level
through the stage-1 universal centre rather than scalar-level after
specialisation.  The cross-volume statement of the local theorem is
therefore a statement about pulled-back curvature data, not a scalar
equality:
\[
  \operatorname{res}_{L,N}\bigl(F_{\mathrm{glob}}(\Theta_{\mathrm{glob}})\bigr)
...
\]
the joint curvature vector in the master deformation complex of
Appendix~\ref{app:master-deformation-complex}, restricted along the
Dirac brane probe.  Cross-volume agreement is the assertion that this
restriction admits a compatible null-homotopy in
\(\mathfrak K_{\mathrm{KD}}\); a nonzero coordinate is a named
obstruction, not a numerical mismatch.

\paragraph{Positive operator-level statement.}  The architecture
admits a positive content as well as a negative one.  The Vol~III
two-stage CY-to-chiral functor
\(\Phi_d=\mathrm{Sp}_{\Sigma_{d-1},C}\circ\Phi_d^{\mathrm{FA}}\)
constructs the chiral algebra \(A=\Phi_d(\mathcal C)\) from a CY-\(d\)
categorical input \(\mathcal C\) in two stages: stage~1 produces a
canonical \(E_d\)-holomorphic factorization algebra on a higher
target \(X\); stage~2 specialises along
\((\Sigma_{d-1},C)\) to the curve-side \(E_1\)-chiral shadow.  The
universal centre lives at stage~1, before specialisation.  The
positive cross-volume statement is therefore operator-level rather
than scalar-level:
\[
  \boxed{\;
  \begin{aligned}
    &Z^{\mathrm{der}}_{\mathrm{ch},\,\mathrm{stage 1}}(A)
    \;\;\text{is the universal trace identity},\\
    &\mathrm{Sp}_{\Sigma_{d-1},C}\colon
    Z^{\mathrm{der}}_{\mathrm{ch},\,\mathrm{stage 1}}\twoheadrightarrow
    Z^{\mathrm{der}}_{\mathrm{ch},\,\mathrm{curve}}
    \;\;\text{is the stage-2 specialisation},\\
    &K(A)\big|_{\mathrm{curve}},\;\;
    \kappa_{\mathrm{BKM}}\big|_{\mathrm{curve}}
    \;\;\text{are distinct stage-2 numerical shadows of the same
    operator-level identity.}
  \end{aligned}
  \;}
\]
The Vol~III material itself records this directional asymmetry: at
the \(K3\!\times\!E\) Calabi--Yau witness, the Heisenberg chiral
scalar, the Borcherds weight \(\kappa_{\mathrm{BKM}}(\Phi_1)=5\),
and the Mukai/conductor invariant are numerical outputs of distinct
trace theorems; equality at the scalar level fails after
specialisation, equality at the operator level before specialisation
is the load-bearing claim.  The local theorem on
\(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\)
contributes one functorial chart of the stage-1 centre: the formal
Hamiltonian Darboux brane-probe restriction
\(\operatorname{res}_{L,N}\colon
Z^{\mathrm{der}}_{\mathrm{ch},\mathrm{stage 1}}(A)
\twoheadrightarrow C^\bullet_{\mathrm{CE}}(\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1])\),
read on the brane as \(u_f\mapsto J(f)\).  Stage-2 numerical shadows
on a curve are external to the local theorem; their agreement or
disagreement is governed by the matched-conventions acceptance
class \(\operatorname{Ob}_{\mathrm{UKD}}\) and the Vol~III
specialisation functor.

\medskip\noindent
The slogan is therefore:
\[
  \text{cross-volume agreement is operator-level at stage 1, not scalar-level at stage 2.}
\]
Any apparent identification of numerical scalars between local
Hamiltonian, on-curve conductor, and Borcherds weight branches must
be lifted to an operator-level identity at the stage-1 universal
centre; a scalar mismatch at stage~2 is an obstruction recorded
by the firewall, not a contradiction of the local theorem.
...
by the corresponding coordinate, and a matched-conventions claim
conditional on a chosen null-homotopy.  These coordinates live
...
cohomology-level coordinate-independence theorem belongs to the
target programme.
...
\begin{defn}[Functor of points of mixed six-relation factorization]\label{defn:mixed-dunn-fact6r-datum}
  ...
  on the ordinary \(\operatorname{Ran}(M\times Y)\), belong to a
  separate target.  A target using arbitrary Weiss/Ran covers must supply
  \(\operatorname{ob}_{\mathrm{WR}}\),
...
\begin{defn}[Target convention data]\label{defn:matched-conventions-templates}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{Compact BCOV datum.}
      ...
      is a target equation in this column~\cite{bcov}; it lies
      outside the local \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_0\)
      theorem surface, which carries neither a compact Calabi-Yau
      threefold nor a genus expansion.
...
\begin{thm}[Igusa / BKM divergence and matched-conventions]\label{thm:divergence-igusa-bkm}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Divergence.}  The Igusa/BKM convention layer consists
      ...
      \(\tau_{\mathrm{DT/PT/OP}}\); these data lie outside the local
      theorem surface.  In the \(\Delta_5\) row, the constants
      \[
        D_5=64^{-1}\Delta_5,\qquad
        \operatorname{den}(\mathfrak g_{\Delta_5})
          =64^{-1}\Delta_5(2Z),
...
\begin{thm}[GW / DT / PT / MNOP divergence and matched-conventions]\label{thm:divergence-gwdt}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Divergence.}  The GW/DT/PT/MNOP convention layer
      ...
      \(\Phi_{\mathrm{MNOP}}\); these data lie outside the local
      theorem surface, whose holomorphic factor is the formal polydisk
...
\begin{thm}[OSV attractor divergence and matched-conventions]\label{thm:divergence-osv}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Divergence.}  The OSV convention layer consists of the
      ...
      conjecture on a compact CY\(_3\); it lies outside the local
      theorem surface on \(\widehat{\C^2}_{0,\mathrm{hol}}\).
    \item \emph{Obstruction class.}  The acceptance obstruction is
      \[
        \operatorname{ob}_{\mathrm{OSV}}
          \in
...
\begin{thm}[Mixed-Dunn divergence and matched-conventions]\label{thm:divergence-mixed-dunn}
  ...
  Definition~\ref{defn:matched-conventions-templates}~(vii).  The
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Divergence.}  The local product-disk site
      ...
      disks \(D\times B\); product-disk covers are not cofinal in
      arbitrary Weiss covers of \(\R\times\C^2\), and the local
      theorem leaves descent on the unrestricted Weiss site
      \(\operatorname{Ran}(\R\times\C^2)\) to a separate target.  A target
...
\S\ref{ssec:twistedM-universality} proves that an ambient
twisted-M sector restricts canonically to the same formal local
model; the matched-conventions class
\(\operatorname{Ob}_{\mathrm{UKD}}(\mathfrak C_{\mathrm{tar}})\)
proves acceptance of that formal local model by an external compact,
modular, automorphic, virtual-cycle, or operadic target.  Vanishing in sequence -- H/ML at the analytic
