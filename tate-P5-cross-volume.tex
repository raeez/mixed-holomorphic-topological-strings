...
\]
with brane line \(\R_{\mathrm{brane}}\times\{0\}\times\{0\}\), closed
Hamiltonian BF sector on the formal holomorphic-symplectic disk
\(\widehat{\C^2}_{0,\mathrm{hol}}\) together with the topological
directions, and open Dirac brane probe on the brane line.  Its
holomorphic dimension is \(d=\dim_\C\widehat{\C^2}_0=2\), and the brane
line is its one-dimensional factorization direction.

The compact CY\(_3\) BCOV programme of
Bershadsky-Cecotti-Ooguri-Vafa~\cite{bcov}, the Vol III
CY-to-chiral frontier, and the Borcherds/BKM bridge to the Igusa cusp
form \(\Delta_5\) each carry their own conventions, obstruction
complexes, and global data.  None of those theorems is a consequence of
the local \(d=2\) Hamiltonian theorem alone: the local theorem
contributes the formal-disk stalk; everything compact, global, modular,
or operadic is target datum.  Convention divergence is load-bearing and
reported, not silently reconciled.  The named obstruction classes below
isolate the exact transfer problem.
