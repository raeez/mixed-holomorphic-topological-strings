...
CY\(_3\) BCOV / Kodaira--Spencer programme of
...
Borcherds / Borcherds--Kac--Moody (BKM) bridge to the Igusa cusp
form \(\Delta_5\); the Gromov--Witten / Donaldson--Thomas /
Pandharipande--Thomas / Maulik--Nekrasov--Okounkov--Pandharipande
...
\begin{thm}[Cross-volume firewall theorem]\label{thm:cross-volume-firewall}
  Let \(\mathfrak T\) be any of the seven quarantined targets of the
  organizing question -- compact CY\(_3\) BCOV / Kodaira--Spencer, Vol
  III CY-to-chiral specialization, Igusa / BKM bridge to \(\Delta_5\),
  ...
  null-homotoped.  Equivalently, the only sentence the local theorem
  provides to a cross-volume target is the formal-stalk component;
  every other sentence (compactness, modularity, virtual cycle,
  attractor, specialization, operadic interchange, global descent,
  hyperdescent, framing) is target datum.  A nonzero coordinate of
  \(\operatorname{Ob}_{\mathrm{UKD}}\) is the exact obstruction to
  acceptance in \(\mathfrak T\); divergence is reported coordinate by
  coordinate and is not silently reconciled.
\end{thm}
...
\begin{proof}
  ...
  divergence row of each quarantined target, given respectively by
  ...
  \ref{thm:divergence-global-ham}: each names the local convention
  layer, the target convention layer, the named obstruction class,
  and the matched-conventions claim.  Independence of any one
  divergence row from another is not asserted in this theorem; what
  is asserted is the absence of implication from local to target on
  every row simultaneously, which follows from the
  Lemma~\ref{lem:no-formal-disk-transfer} restriction of the local
  contribution to the formal stalk.
\end{proof}
...
\paragraph*{Named acceptance vector.}  The acceptance class
...
\(\varprojlim^1\) defects.  The target-specific coordinates name the
seven quarantined programmes one by one: compact CY\(_3\) BCOV
...
\(\mathfrak C_{\mathrm{tar}}\), and is the divergence to be reported
coordinate by coordinate, not silently reconciled.

\paragraph*{Decomposition table (target $\leftrightarrow$ named coordinate).}
The seven quarantined targets are bound to the seven target-specific
coordinates of \(\operatorname{Ob}_{\mathfrak C_{\mathrm{tar}}}^{\mathrm{tar}}\)
by the following table; the universal coordinates apply uniformly across
all rows.
\begin{center}
...
\end{tabular}
\end{center}
Each row inscribes a divergence theorem (this manuscript's local
convention vs.\ the target convention), an obstruction class
(the named coordinate), and a matched-conventions claim
(conditional on a chosen null-homotopy).  The named coordinates
live in syntactically distinct cohomology groups
(\(H^1(\mc K_{\mathrm{obs}})\),
\(H^1(\mathfrak K_{\mathrm{KD}})\), and the appropriate
target-specific complexes); a strict cohomology-level coordinate
independence theorem is target programme business and is not
asserted here.  The firewall claim is the absence of
implication from local to target row by row, in the sense of
...
\subsection*{Named divergence theorems per quarantined target}

The seven matched-conventions templates of
Definition~\ref{defn:matched-conventions-templates} carry seven
independent divergence theorems.  Each theorem records: the
convention used in the local theorem, the convention used in the
target programme, the obstruction class to identification, and the
matched-conventions claim that survives once that obstruction is
killed.  Russian discipline requires the divergence to be named, the
convention layer to be flagged, and the matched-conventions claim to
be conditional on a chosen null-homotopy.  Compactness, modularity,
virtual cycles, attractor identifications, and operadic
interchanges are target data, not consequences of the local
\(d=2\) Hamiltonian theorem.
...
\begin{thm}[Vol III CY-to-chiral divergence and matched-conventions]\label{thm:divergence-volIII}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{Obstruction class.}  The acceptance obstruction is
      ...
      datum; \(\kappa_{\mathrm{cat}}(K3\times E)=0\) is recorded
      only as a convention check, distinct from any Heisenberg, BKM,
      or fibre normalization in Vol III.
    \item \emph{Matched-conventions claim.}  If
...
\begin{thm}[Igusa / BKM divergence and matched-conventions]\label{thm:divergence-igusa-bkm}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Divergence.}  The Igusa/BKM convention layer consists
      ...
      construction.  They are recorded here only as a heuristic /
      convention-checking declaration of the layer at which a target
      acceptance theorem must match; the local Hamiltonian theorem on
      \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\) does
    ...
    \item \emph{Matched-conventions claim.}  If
      ...
      weights, and modular invariance remain target data.  Divergence
      between this manuscript's local convention layer and the BKM
      lane is load-bearing and is reported coordinate by coordinate;
...
\begin{thm}[Mixed-Dunn divergence and matched-conventions]\label{thm:divergence-mixed-dunn}
  ...
  algebra on \(\C^2\) (the formal symplectic surface, with native
  bidegree two in the holomorphic direction; cf.\ CLAUDE.md), the
  topological factorization algebra on \(\R_{\mathrm{brane}}\) (one
  topological direction along the brane line), and a chosen
  brane-line interval factorization map
  \(\Phi^{\mathrm{Ham}}_{\mathrm{fact}}\) on
  product disks \(D\times B\subset\R\times\C^2\).
  \begin{enumerate}[label=(\roman*)]
...
automorphic lane through the Igusa cusp form \(\Delta_5\) -- with
Borcherds lattice \(\Lambda_{\mathrm{Borch}}\), Weyl chamber \(W\),
Jacobi input \(\varphi_{\mathrm{Jac}}\), and primitive recognition
map \(\pi_{\mathrm{prim}}\) -- is firewalled from the local theorem
by the \(\operatorname{ob}_{\mathrm{Igusa/BKM}}\) coordinate of
...
genus expansion~\cite{bcov}, MNOP / GV / OSV / DT / PT virtual-cycle
counts, the Vol III specialization functor, and the mixed-Dunn
rectification problem: each is firewalled by its own coordinate of
