...
The manuscript proves the **formal-Darboux stalk theorem of the Mixed Holomorphic-Topological Deligne conjecture** at $N$ Dirac branes:
\[
...
\]
The trace map $J$ is the formal Darboux coordinate expression of the chiral Hochschild cohomology $C^\bullet_{\mathrm{ch}}(A_b, A_b)$. The Capelli scalar $\hbar N[\bar c]$ is the projective curvature $\Omega_{\mathrm{central}}$ at the trace generator.
...
1. The setup. ($\mathbb{R}^2_{\mathrm{top}} \times \mathbb{C}^2_{\mathrm{hol}}$; Mixed HT Deligne conjecture; this manuscript proves the formal-Darboux stalk.)
...
> You are reconstituting one chapter (or one cross-cutting axis) of the mixed-holomorphic-topological-strings manuscript. The manuscript is the formal-Darboux stalk theorem of the Mixed HT Deligne conjecture: closed sector $= Z^{\mathrm{der}}_{E_d^{\mathrm{ch}}}(\mathcal{C}^{\mathrm{op}}_\partial)$. This manuscript computes the local coordinate expression at a Dirac brane: $A^{\mathrm{cl}}_{\partial, N} = C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N, \mathrm{Kosz}([\phi_1, \phi_2]))$, $J(f) = \mathrm{Tr}\,f(\phi_1, \phi_2)$, $c_f \mapsto \theta_f$, $u_f \mapsto J(f)$.
>
...
**C1 — Chapter 1 setup.** Read `main.tex` opening sections + `abstract.tex`. Reconstitute Chapter 1 to state the Mixed HT Deligne conjecture as the global object and this manuscript as its formal-Darboux stalk. Replace any "main theorem of the whole subject" framing with "formal-Darboux stalk theorem." Opening sentence must be the platonic opener in CLAUDE.md. No globalization claim survives without "+ descent + QME + anomaly + locality $\Rightarrow$ candidate."
...
**C5 — Chapter 5 CE/PV dictionary as Koszul resolution.** Read `tate-P3-universality.tex`, `tate-T2-nilpotent-truncation.tex`, `tate-T5-chain-level-primitive.tex`. The dictionary $c_f \mapsto \theta_f$, $u_f \mapsto J(f)$ is the Koszul resolution of $C^\bullet_{\mathrm{ch}}(A_b, A_b)$. State it as such; verify quasi-isomorphism on cohomology; the universality theorem (P3) is then a corollary.

**C6 — Chapter 6 Capelli scalar.** Read `tate-T4-bv-vanishing.tex`, `appendix-unreduced-bv-qme.tex`. The Capelli scalar identification is the projective curvature evaluation: $\hbar N[\bar c] = \Omega_{\mathrm{central}}|_{J(f)}$. State the determinant-line interpretation. Scheme dependence per `appendix-factorization-current-conventions.tex`.

**C7 — Chapter 7 obstruction calculus.** Read `appendix-master-deformation-complex.tex` + main.tex sections on $\mathfrak{K}_T, \Theta_T, F_T$. State the four-curvature taxonomy as one classifying equation. Six named obstructions (radial Stokes, pro-Matlis, Capelli, modular clutching, BCOV/GV/DT compact comparison, Hall–Drinfeld–Borcherds) are coordinates of one $F_T$ at six gauge slices.

**C8 — Chapter 8 pro-Matlis.** Read `appendix-matlis-principal-parts.tex`. Motivate pro-Matlis as the categorical home of bulk-boundary duality along the $N$-tower. The four-curvature taxonomy lives natively in pro-Matlis; "leakage out of finite window" is the failure of finite-rank Matlis duality.
...
- Identifications surfaced (Capelli $= \Omega_{\mathrm{central}}|_{J(f)}$; modular line $= \Omega_{\mathrm{central}}$; pro-Matlis $= N$-tower categorical home; CE/PV $=$ Koszul resolution; chiral specialization $=$ factorization homology; Dirac brane stack $=$ derived commuting variety).
