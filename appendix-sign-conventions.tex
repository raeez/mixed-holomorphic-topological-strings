...
\label{def:app-sign-hamiltonian-cyclic}
  ...
  same sign.  The only odd letter is \(\psi\).  For a homogeneous cyclic
  word \(F=[a_1\cdots a_n]\), the left cyclic derivative is
  \[
    \partial_qF
      =\sum_{a_i=q}
       (-1)^{|a_1\cdots a_{i-1}|
  ...
  \]
  with even graded-skew symplectic pairing
  \[
    \omega(x,y)=1,\quad \omega(y,x)=-1,
    \qquad \omega(\gamma,\psi)=\omega(\psi,\gamma)=1.
  \]
  The cyclic Hamiltonian convention is
  \[
    \iota_{X_F}\omega=-dF,
    \qquad \{F,G\}_{\mathrm{neck}}=[X_F(G)]_{\mathrm{cyc}}.
  \]
  Solving the contraction equation gives
  \[
    X_F(x)=-\partial_yF,\quad X_F(y)=\partial_xF,
    \qquad
    X_F(\gamma)=(-1)^{|F|}\partial_\psi F,\quad
  ...
  \]
  In particular,
  \(\{[x],[y]\}_{\mathrm{neck}}=[1]\) and
  \(\{[\gamma],[\psi]\}_{\mathrm{neck}}=-[1]\).
\end{defn}
