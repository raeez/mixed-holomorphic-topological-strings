...
\emph{Load-bearing role.}  This appendix is the computational backbone of
the special-fibre versus deformation-level distinction
(Theorem~\ref{thm:pbw-vs-deformation}).  Two modules carry the same
bidegree index set \((a,b)\) and a different
\(\mathfrak h\)-action: the polynomial PBW one-\(\psi\) descendant
labels \(\Psi_{a,b}\) of the open BV brane sector and the residual
Matlis principal parts \(\rho_{a,b}\) of
Appendix~\ref{sec:app-matlis-principal-parts}.  The slogan one
expects, ``defect position is polynomial; defect cotangent momentum is
residual'', is upgraded to a theorem only after a single universal
calculation: the full primitive cyclic Koszul homology of
\((A_\psi,\,d\psi=xy-yx)\) graded by \(\psi\)-count and by
\((R,S)=(\#x+\#\psi,\#y+\#\psi)\).  This appendix supplies the
calculation, identifies the surviving \(H_1\)-line as the special-fibre
image of the PBW Rees lattice, and pins the precise associated-graded
morphism
\(\operatorname{gr}\Phi:\bar A_{\mathrm{desc}}[1]\to\mc P\) to its
special-fibre habitat.  The load-bearing outputs are
\begin{enumerate}
  \item the bidegree homology theorem
    (Theorem~\ref{thm:app-full-primitive-psi-homology}): no primitive
    classes beyond \(\psi\)-degree one and the explicit one-\(\psi\)
    cycle \(\Psi_{R-1,S-1}\) generating the unique \(H_1\)-line;
  \item the universal primitive descendant theorem
    (Theorem~\ref{thm:app-full-universal-primitive-descendant}): every
    stable primitive single-trace model with the same generators and
    \(d\psi=xy-yx\) factors through this one;
  \item the Hamiltonian-sector extraction
    (Theorem~\ref{thm:app-full-hamiltonian-sector-extraction}): the
    decomposition into the closed Hamiltonian summand
    \(\bar A\), its one-\(\psi\) descendant copy
    \(\bar A_{\mathrm{desc}}[1]\), and the removed scalar lines
    \(\C\cdot[1]\) and \(\C\cdot[\Psi_{0,0}]\);
  \item the special-fibre PBW label construction
    (Construction~\ref{constr:app-full-pbw-special-fibre-basis}):
    \(\Psi_{a,b}\in\bar A_{\mathrm{desc}}[1]\) is the
    \(\hbar=0\) image of the Rees lattice
    \(\mathbf S_\hbar(\mathfrak h\oplus\mathfrak h^\vee[1])\)
    cut by the \(\psi\)-count grading;
  \item the associated-graded label morphism
    (Proposition~\ref{prop:app-full-gr-Phi}):
    \(\operatorname{gr}\Phi:\bar A_{\mathrm{desc}}[1]\to\mc P\) is the
    bidegree-only special-fibre relabelling, an isomorphism of
    \(\Z_{\geq0}^2\)-graded vector spaces and not of
    \(\mathfrak h\)-modules;
  \item the Defect Polarization slogan
    (Corollary~\ref{cor:app-full-defect-polarization-slogan}): the full
    \(\psi\)-homology computes the special-fibre shadow of the cotangent
    module, while the Matlis principal-parts module is the
    deformation-level cotangent module itself.
\end{enumerate}
These outputs are the computation cited by
Proposition~\ref{prop:one-psi-homology}, by the body corollary that
records the no-higher-\(\psi\)-class statement
(\(p\geq2\) groups vanish), by the named separation
Theorem~\ref{thm:pbw-vs-deformation}, and by
Theorem~\ref{thm:T5-full-primitive-psi-homology} of the Tate residual
fragment.  The companion Matlis appendix
(Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction},
Theorem~\ref{thm:app-matlis-rees-fourier-bridge}) supplies the
\(\hbar\)-deformation side; this appendix supplies the
\(\hbar=0\) source.
...
\label{rmk:app-full-psi-numerical-anchor}
  ...
  separation in
  Theorem~\ref{thm:pbw-vs-deformation} are therefore certified at the
...
\label{prop:app-full-gr-Phi}
  ...
  label match cited by
  Theorem~\ref{thm:app-matlis-rees-fourier-bridge}, transported to the
  \(\Psi\)-presentation by
  Construction~\ref{constr:app-full-pbw-special-fibre-basis}.
...
\begin{rmk}[Instant computation: the \((4,0,1,1)\) fingerprint]
\label{rmk:app-full-instant-computation}
  ...
  exactly and embeds it inside a sweep of \(1200\) such mismatches; this
  appendix records the same datum as the smallest single-monomial
  certificate.
\end{rmk}
...
\begin{cor}[Defect Polarization slogan]
\label{cor:app-full-defect-polarization-slogan}
  ...
  Appendix~\ref{sec:app-matlis-principal-parts} is the deformation-level
  cotangent module itself.  The brane probe measures positions
...
\label{rmk:app-full-psi-what-supports-what}
  This appendix supplies the full primitive single-trace Koszul
  calculation invoked by the body in four independent places.
  \begin{enumerate}
    \item Theorem~\ref{thm:app-full-primitive-psi-homology} is the
      bidegree-resolved form of the calculation cited by
      Proposition~\ref{prop:one-psi-homology}.  The body proposition
      proves the one-\(\psi\) homology line in each bidegree; the
      appendix proves additionally that all higher-\(\psi\) groups
      vanish.  This vanishing is what allows the body to write the
      stable primitive descendant module as \(\bar A\theta\).  The
      finite numerical anchor of
      Remark~\ref{rmk:app-full-psi-numerical-anchor} certifies the
      bidegree-by-bidegree dimension count up to total exponent five.
    \item Theorem~\ref{thm:app-full-universal-primitive-descendant}
      is the universal property used in the body corollary near
      line~6047 of \texttt{main.tex} (``the result does not identify
      the polynomial one-\(\psi\) span with Matlis principal parts'')
      and quoted again in
      Theorem~\ref{thm:T5-full-primitive-psi-homology} of
      \texttt{tate-T5-chain-level-primitive.tex}: every stable
      primitive single-trace model satisfying the hypotheses factors
      on homology through the displayed two-line module, hence cannot
      generate hidden higher-\(\psi\) classes that would then have to
      be matched against a cotangent target.
    \item Theorem~\ref{thm:app-full-hamiltonian-sector-extraction}
      identifies the surviving sectors after removing the constants
      \([1]\) and the odd scalar \([\Psi_{0,0}]\).
      Construction~\ref{constr:app-full-pbw-special-fibre-basis} is
      the Rees PBW decoration of those sectors: \(\bar A\) is the
      special-fibre image of \(\mathbf S(\mathfrak h)\), and
      \(\bar A_{\mathrm{desc}}[1]\) is the special-fibre image of
      \(\mathbf S^1(\mathfrak h^\vee[1])\otimes\mathbf S(\mathfrak h)\),
      both at \(\hbar=0\).  Together with
      Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction}
      and
      Corollary~\ref{cor:app-matlis-no-flat-same-action-interpolation}
      they form the technical core of the
      \(\Psi\)-versus-\(\rho\) separation in
      Theorem~\ref{thm:pbw-vs-deformation}: the polynomial label
      module is the bidegree-graded one-\(\psi\) sector here, and the
      cotangent module is the residual one in the Matlis appendix;
      they share an index set and not a module structure.
    \item Proposition~\ref{prop:app-full-gr-Phi} and
      Corollary~\ref{cor:app-full-defect-polarization-slogan}
      record the precise associated-graded label morphism
      \(\operatorname{gr}\Phi:\bar A_{\mathrm{desc}}[1]\to\mc P\) and
      the Defect Polarization slogan it certifies: the polynomial PBW
      side is the special-fibre shadow of the cotangent module, and
      the Matlis principal-parts side is the deformation-level cotangent
      module itself.  This statement is the open/closed companion of
      the Fourier--Rees bridge of
      Theorem~\ref{thm:app-matlis-rees-fourier-bridge}, which records
      the same comparison from the principal-parts side: there it is
      a Fourier-twisted \(\mathcal D_\hbar\)-module bridge, and here
      it is the bidegree-only special-fibre relabelling of the
      \(\Psi\)-presentation, with the same conclusion that the two
      modules are not \(\mathfrak h\)-isomorphic.
  \end{enumerate}
  The chip-moving state-complex construction of
  Construction~\ref{constr:app-chip-state-complex} and the cyclic
  Abel-quotient calculation of
  Lemmas~\ref{lem:app-state-chain-identification}--\ref{lem:app-cyclic-abel-stabilizers}
  are the explicit cellular model that proves the cyclic Koszul
  complex has the homology of a circle in each bidegree; they are
  load-bearing for the bidegree theorem above.  The instant
  computation of Remark~\ref{rmk:app-full-instant-computation}
  is the smallest single-monomial certificate that no Fourier-untwisted
  same-action lift of \(\operatorname{gr}\Phi\) exists.  No section of
  this appendix is auxiliary.
\end{rmk}
