...
\emph{Load-bearing role.}  Principle~1 of the introduction asserts
that the open BV theory of the brane probe stack is the Koszul
zero-fibre of the
\(GL_N\)-conjugation moment map and that its primitive trace content
is the matrix-probe family \(J(f)\).  Principle~4 then asserts that
the polynomial one-\(\psi\) descendant labels \(\Psi_{a,b}\) are the
PBW special-fibre shadow of a different module from the residual
cotangent module \(\mc P\).  Both principles need a single universal
calculation: the full primitive cyclic Koszul homology of
\((A_\psi,d\psi=xy-yx)\) graded by \(\psi\)-count.  This appendix
supplies that calculation in the bidegree-organised form.  The
load-bearing outputs are
\begin{enumerate}
...
Proposition~\ref{prop:one-psi-homology}, by the corollary in §6 of
the body that pins the no-higher-\(\psi\)-class statement
(\(p\geq2\) groups vanish), and by
Theorem~\ref{thm:T5-full-primitive-psi-homology} of the Tate residual
fragment.  The \(\Psi_{a,b}\)-versus-\(\rho_{a,b}\) separation theorem
(Theorem~\ref{thm:pbw-vs-deformation}) cites the same generator
together with the Matlis appendix.
...
\label{rmk:app-full-psi-what-supports-what}
  ...
  calculation invoked by the body in three independent places.
  \begin{enumerate}
    \item Theorem~\ref{thm:app-full-primitive-psi-homology} is the
      ...
      stable primitive descendant module as \(\bar A\theta\).
    \item Theorem~\ref{thm:app-full-universal-primitive-descendant}
      is the universal property used in the corollary near
    ...
    \item Theorem~\ref{thm:app-full-hamiltonian-sector-extraction}
      ...
      \([1]\) and the odd scalar \([\Psi_{0,0}]\).  The reduced
      decomposition \(\bar A\oplus\bar A_{\mathrm{desc}}[1]\) is the
      target of the open trace generators \(O_{a,b}\) and the
      one-\(\psi\) primitive labels \(\Psi_{a,b}\) used by the
      body.  Together with
      Theorem~\ref{thm:app-matlis-polynomial-descendant-obstruction}
      it forms the technical core of the
  ...
  load-bearing for the bidegree theorem above.  No section of this
  appendix is auxiliary.
\end{rmk}
