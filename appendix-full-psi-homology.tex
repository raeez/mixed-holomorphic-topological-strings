...
Two formal deformation variables occur below.  The PBW and
Fourier--Rees lattices use the Rees variable \(\hbar_{\mathrm{Rees}}\).
The Moyal coadjoint coefficient checks use the Weyl/Moyal variable
\(\hbar_{\mathrm{W}}\).  No Costello quantum master-equation parameter
is used in this primitive Koszul homology appendix.

The universal primitive single-trace Koszul complex is attached to the
moment-map equation \([x,y]=0\).  Its universality is independent of any
choice of Matlis target, PBW model, finite \(N\) trace identity, or quantum
deformation: after stable trace invariant theory has identified
primitive traces with graded cyclic words, the answer is the cellular homology
of a cyclic-word complex.  Every finite-bidegree stable primitive
descendant calculation over \(\C\), with \(d\psi=xy-yx\) and graded
cyclicity as the trace relation, is obtained from this one.  Passage to
...
\begin{proof}
  ...
  \(\eta_i\)'s and homogeneous of the prescribed \(x,y\)-degree.
  Procesi's first fundamental theorem \cite{procesi}*{Thm.~1.3},
  with finite-\(N\) trace identities controlled by Procesi
  \cite{procesi}*{Thm.~4.6} and Razmyslov \cite{razmyslov}*{Thm.~2},
  identifies the primitive single-trace invariants of total word length
  \(d\), for \(N\geq d\), with ordinary cyclic words in the letters
  \(x,y,\eta_1,\ldots,\eta_p\), modulo only cyclicity of the trace.  The
  replacement of the odd letter by marked even matrices and the subsequent
  sign-isotypic projection are the internal super-cyclic adaptation.
  ...
  precisely the graded cyclic rule for a single odd letter \(\psi\).  This
  is the complete stable trace relation in word length \(d\).
\end{proof}
...
\begin{proof}
  ...
  the even word \(xy-yx\) contributes sign \(+1\) from the replacement.
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    [(yx)u\psi v]=[\psi vyxu].
  \]
  The moved block is even, so the Koszul sign is \(+1\).  The second
...
\label{constr:app-one-psi-three-term-skeleton}
  ...
  \(2,1,0\) of the state complex constructed below: \(C_0\) records point
  configurations, \(C_1\) records one edge-interior point, and \(C_2\) records two
  independent edge-interior points with the orientation fixed by the ordered
  edges.
\end{constr}
...
\begin{constr}[Labelled cyclic-word cell complex]
\label{constr:app-cyclic-word-cell-complex}
  ...
  \(R-p\) stationary indistinguishable points on vertices and \(p\) points
  in the interiors of \(p\) distinct edges.  Several stationary points
  may occupy the same vertex.  Each edge contains at most one interior point.

  Encode such a state by traversing \(\mathscr C_S\).  At a vertex write one
  \(x\) for each stationary point sitting there.  Then write \(y\) if the
  following edge is empty and \(\psi\) if the following edge contains a
  interior point.  This gives a word with \(R-p\) letters \(x\), \(S-p\) letters
  ...
  carrying interior points.  Its \(j\)-th oriented coordinate has boundary
  \[
...
\begin{proof}
  ...
  cyclic words.  The labelled complex orders the \(S\) edge markers, and
  in the coinvariant complex the only
  ...
  Construction~\ref{constr:app-cyclic-word-cell-complex} is
  \[
    \sum_j(-1)^{j-1}([xy]-[yx])
  \]
  in the \(j\)-th moving edge.  The terminal face is the state in which the
  point has crossed the edge and is read as \(xy\); the initial face is read as
...
\begin{proof}
  ...
  the positions of the \(R\) points: stationary points have integer positions,
  and an interior point on the edge \([m,m+1]\) has coordinate \(m+t\) with
  ...
  Construction~\ref{constr:app-cyclic-word-cell-complex}.  Define the Abel map
  \[
    A:\widetilde Y_{R,S}\longrightarrow \R/S\Z
  \]
  by summing the point positions on the oriented circle, counted with
  multiplicity, and reducing modulo \(S\).  On every cell this map is affine.

  Lift the target to \(\R\) and fix a cyclic order of the \(R\) lifted points.
  ...
  contain an interior point; these faces are precisely the cells of the
  cellular subdivision meeting the fibre.  Hence every lifted fibre is a
  ...
  A cyclic relabelling of the \(S\) edges adds a constant to every point
...
\begin{proof}
  ...
  cyclic relabelling by \(r\in \Gamma_S\) adds \(r\) to each of the \(R\) point
  ...
  it into cells.  It is a convex polytope cut out by weak ordering, the
  fixed-sum equation, and the cellular inequalities, and \(G\) acts affinely
  ...
  cellular subdivision.  Hence the stabilizer quotient of the Abel fibre is
  contractible.
\end{proof}
...
\label{lem:app-full-s1-generator}
  ...
  \]
  represents the fundamental \(H_1\)-line of the cyclic-word
  complex.
\end{lem}
...
\begin{proof}
  ...
  in the positive Abel direction: its terminal face has the moved point one edge
...
\begin{proof}
  ...
  \(p\ge3\) have boundaries in \(C_{p-1}\) and preserve
...
\begin{rmk}[Finite-window homology check]
\label{rmk:app-full-psi-numerical-anchor}
  The finite-window calculation tabulates the same three-term skeleton
  over \(\Q\), confirms
  ...
  calculation verifies the open Hamiltonian descendant action of
  ...
  \(\hbar_{\mathrm{W}}^{\geq2}\) higher-order terms; the \(70\) linear-generator
  leading-order tests with labels at most \(5\) identify the exact obstruction
  to any \(\hbar_{\mathrm{W}}\)-adic deformation of the classical
  \(\Psi\to\rho\) projection.  The displayed homology lines and the
  separation in
  Theorem~\ref{thm:pbw-vs-deformation} are therefore proved at the
  bidegree level by an independent finite computation.
\end{rmk}
...
\label{constr:app-full-pbw-special-fibre-basis}
  ...
  \[
    U_{\hbar_{\mathrm{Rees}}}(\mathfrak g)
      \supset
    \mathbf S_{\hbar_{\mathrm{Rees}}}(\mathfrak h\oplus\mathfrak h^\vee[1])
  \]
  denote its Rees PBW lattice over \(\C[[\hbar_{\mathrm{Rees}}]]\), namely the Rees
  algebra of the canonical PBW filtration.  The special fibre is
  \[
    \mathbf S_{\hbar_{\mathrm{Rees}}}(\mathfrak h\oplus\mathfrak h^\vee[1])
      \big|_{\hbar_{\mathrm{Rees}}=0}
      \cong
  ...
  \[
    ...
    \mathbf S(\mathfrak h)\big|_{\hbar_{\mathrm{Rees}}=0} .
  \]
...
\label{prop:app-full-gr-Phi}
  ...
  This morphism is a bigraded vector-space isomorphism with an
  \(\mathfrak h\)-module obstruction. More precisely, on the linear Hamiltonians,
  \[
  ...
  \(\widetilde\Psi_{a,b}\), whereas every nonzero \(\rho_{a,b}\) has an
  infinite principal-part coadjoint orbit.  The
  obstruction to an \(\hbar_{\mathrm{Rees}}\)-adic same-action lift of
...
\begin{proof}
  ...
  invariant; therefore the displayed \(\mathfrak h\)-actions have a
  module-isomorphism obstruction.

  The obstruction to extending \(\operatorname{gr}\Phi\) to an
  \(\hbar_{\mathrm{Rees}}\)-adic same-action map is exactly this local-nilpotence
  ...
  Corollary~\ref{cor:app-matlis-same-action-interpolation-obstruction}.
  Theorem~\ref{thm:app-matlis-rees-fourier-bridge} gives the
...
\label{rmk:app-full-instant-computation}
  ...
  exponent pair.  Take the quartic
  ...
  \[
    \operatorname{ad}^*_{z_1^4}(\delta_{1,1})
      =-24\,\hbar_{\mathrm{W}}^2\,\delta_{0,4},
  \]
  the only nonzero term of order \(\leq3\) in the formal Moyal expansion.
  Fourier-untwisted relabelling has the bidegree obstruction
  \((1,1)\to(0,4)\) versus \((1,1)\to(4,0)\), so the polynomial PBW action
  and the Moyal coadjoint action remain distinct.  Replacing the Moyal coadjoint by its classical
  truncation \((\hbar_{\mathrm{W}}^0)\) gives
  ...
  term \(4\widetilde\Psi_{4,0}\).  Exact finite-window calculation
  confirms this mismatch and embeds it inside a sweep of \(1200\) such
  mismatches.
\end{rmk}
...
\label{thm:app-full-universal-primitive-descendant}
  ...
  \]
  factors on homology through the displayed two-line module.  Hence the
  universal primitive higher-\(\psi\) classes are exactly the displayed
  degree-zero polynomial line and degree-one \(\Psi\)-line.
\end{thm}
...
\begin{proof}
  ...
  The homology of the source is the two-line module computed above.  Since
...
\label{thm:app-full-hamiltonian-sector-extraction}
  ...
  pair as the special-fibre, \(\hbar_{\mathrm{Rees}}=0\), associated graded of the Rees PBW lattice
  of Construction~\ref{constr:app-full-pbw-special-fibre-basis}: the
  closed Hamiltonian summand is the polynomial part
  \(\mathbf S(\mathfrak h)|_{\hbar_{\mathrm{Rees}}=0}\), and the descendant copy is the
  primitive one-\(\psi\) PBW label sector
  \(\mathbf S^1(\mathfrak h^\vee[1])\otimes\mathbf S(\mathfrak h)|_{\hbar_{\mathrm{Rees}}=0}\).
...
\begin{proof}
  ...
  constant direction; it is a stable primitive trace class in the removed
  scalar sector.
  Theorem~\ref{thm:app-full-primitive-psi-homology} proves that
  primitive homology is concentrated in \(\psi\)-degrees \(0\) and \(1\).
  What remains
  ...
  vector-space homology statement.  The Matlis principal-part module
  comparison is the separate \(\mathfrak h\)-module obstruction.
\end{proof}

\begin{cor}[Special-fibre defect polarization]
\label{cor:app-full-defect-polarization-cocycle}
  ...
  two index sets at the special fibre \(\hbar_{\mathrm{Rees}}=0\) and only there.  In
  particular, full primitive \(\psi\)-homology computes the
  special-fibre associated graded of the cotangent module on the open
...
\begin{proof}
  ...
  \(\bar A_{\mathrm{desc}}[1]\) is the special-fibre associated graded and
