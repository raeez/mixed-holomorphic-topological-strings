...
\(A^{\mathrm{cl}}_{\partial,N}\), and the finite-window trace maps
\(J_{\leq W}\), followed by stable scalar-reduced completion, give the coordinate
generator assignment whose centre lift is the closed-to-open morphism
problem.  The global mixed holomorphic--topological Deligne problem is
the typed open--closed obstruction problem whose solution is a
Morita mixed-HT \(E_2^{\mathrm{HT}}\)-centre quasi-isomorphism.  Its
evaluation at a chosen vacuum contains this Hochschild chart as its
formal fibre only after the typed rows of
\eqref{eq:typed-total-obstruction-ledger} have been supplied: the
Swiss-cheese action tower, Morita centre lift, Hochschild-centre chart
lift, flat Darboux-torsor transport, unreduced cotangent-current
realization, Weiss/Ran descent, quantum master-equation data,
anomaly/locality null-homotopies, primitive-coherence data, and
...
\cite{beilinson-drinfeld-chiral}*{\S 3}.  At \(d=2\), the formal-Darboux
...
The global mixed-HT \(E_2^{\mathrm{HT}}\)-Deligne problem is the
promotion of this formal current operad to a Morita mixed-HT
\(E_2^{\mathrm{HT}}\)-centre Maurer--Cartan lift with Ran/Weiss descent
and QME compatibility.
...
\cite{costello-gwilliam}*{Vol.~I Secs.~6.1, 6.5, 7.2}.  The holomorphic
...
languages
\cite{beilinson-drinfeld-chiral}*{\S 3}\cite{francis-thesis}\cite{lurie-ha}*{\S 5.5}:
\begin{enumerate}
...
the closed field-theory sector classical observables.  The Ran
...
\(E_2^{\mathrm{top}}\)-algebra structure (chiral Deligne theorem at \(d=1\);
...
\cite{lurie-ha}*{\S 5.3.1}).  Adjacent constructions are supplied by
Faonte--Hennion--Kapranov higher Kac--Moody/factorization geometry
\cite{faonte-hennion-kapranov-higher-km} and by Francis's
factorization-algebra framework \cite{francis-thesis}; neither is used
as the mixed-HT \(E_{d+1}^{\mathrm{ch}}\)-action.  The Mixed
Holomorphic--Topological Deligne conjecture at $d=2$ is the
...
\]
When no ambiguity is possible, the manuscript writes this Morita centre
as \(Z^{\mathrm{der}}_{E_d^{\mathrm{HT}}}(\mathcal C)\).
The notation records the centre.  For \(d\geq2\), a mixed-HT
chiral \(E_{d+1}\)-algebra structure on it is an additional
...
the full idempotent-complete, colimit-closed mixed-HT factorization
module subcategory generated by \(b\) gives the Hochschild chart
\[
...
\]
The Morita-density hypothesis means that this generated subcategory is
all of \(\widehat{\mathcal C}_{\mathrm{adm}}\), equivalently that
restriction of complete factorisation bimodules to \(b\) is fully
faithful and essentially surjective after idempotent completion and
admissible colimits.  Only under this hypothesis is
\(\operatorname{ev}_b\) an equivalence.  The obstruction to density is
the Verdier quotient
\[
  \mathcal Q_b^{\mathrm{Mor}}
  =
  \widehat{\mathcal C}_{\mathrm{adm}}
  /\langle b\rangle_{\mathrm{adm}},
\]
where \(\langle b\rangle_{\mathrm{adm}}\) is the full
idempotent-complete admissible-colimit subcategory generated by \(b\).
The obstruction to the chart being an equivalence is
\(\operatorname{Cone}(\operatorname{ev}_b)\) in the admissible target.
At the \(N\)-Dirac-brane formal-Darboux vacuum \(b_p\), the theorem
computes the trace-sector image of \(\operatorname{ev}_{b_p}\), not the
full category-level centre.
\end{defn}

\subsection{The formal-Darboux stalk functor}\label{app:hfc:darboux-stalk-functor}

For \(X=\R^k_{\mathrm{top}}\times Y_{\mathrm{hol}}\) and a closed point
\(p\in Y\), restriction to the formal completion
\(\widehat Y_p=\operatorname{Spf}\widehat{\mathcal O}_{Y,p}\) is a
formal-completion stalk.  It becomes a formal-Darboux stalk only after
\(Y\) is holomorphic symplectic of even complex dimension \(2m\) and a
formal Darboux chart at \(p\) has been chosen.

\begin{defn}[Formal-Darboux stalk functor]\label{app:defn:Darboux-stalk-functor}
Let \((Y,\omega_Y)\) be holomorphic symplectic of complex dimension
\(2m\), and let \(p\in Y\).  The underlying restriction is the symmetric
monoidal \(\infty\)-functor
\[
  (-)|_{p}\colon
  \mathrm{ChirAlg}^{\mathrm{HT}}_{k,2m}(\R^k\times Y)
   \longrightarrow
  \mathrm{ChirAlg}^{\mathrm{HT}}_{k,2m}(\R^k\times\widehat Y_p),
\]
sending \(\mathcal F\mapsto
\mathcal F|_{\R^k\times\widehat Y_p}\).  A coordinate expression of this
stalk is additional data: a formal Darboux chart
\[
  \chi\colon
  \bigl(\widehat{\mathbb A}^{2m}_0,\omega_0\bigr)
  \xrightarrow{\;\sim\;}
  \bigl(\widehat Y_p,\omega_Y|_{\widehat Y_p}\bigr),
  \qquad
  \omega_0=\sum_{i=1}^m dq_i\wedge dp_i .
\]
The set \(\mathscr D_p(Y,\omega_Y)\) of such charts is a torsor for
\(\mathrm{Symp}^{\mathrm{form}}(\widehat{\mathbb A}^{2m}_0,\omega_0)\)
by precomposition.  Thus the quotient of the family
\(\{\chi^*((-)|_p)\}_{\chi\in\mathscr D_p(Y,\omega_Y)}\) by this torsor
is intrinsic; a single displayed coordinate algebra is a representative,
not a canonical object.
\end{defn}

\begin{prop}[Darboux-chart torsor equivariance]\label{app:prop:Darboux-equivariance}
Let \(g\in
\mathrm{Symp}^{\mathrm{form}}(\widehat{\mathbb A}^{2m}_0,\omega_0)\) and
\(\chi'\!=\chi\circ g\).  The two coordinate representatives of the
formal-Darboux stalk satisfy a symmetric monoidal natural identification
\[
  (\chi')^*((-)|_p)\cong g^*\chi^*((-)|_p).
\]
Consequently the formal-Darboux stalk is invariant only as the quotient
by the Darboux-chart torsor; for the local theorem
\((Y,\omega_Y)=(\C^2,dz_1\wedge dz_2)\), the coordinates
\((z_1,z_2)\) are a chosen representative of this quotient.
\end{prop}

\begin{proof}
The formal holomorphic Darboux theorem gives at least one chart
\(\chi\).  If \(\chi'\) is another chart, then
\(\chi^{-1}\circ\chi'\) is an automorphism of
\((\widehat{\mathbb A}^{2m}_0,\omega_0)\), hence an element of
\(\mathrm{Symp}^{\mathrm{form}}(\widehat{\mathbb A}^{2m}_0,\omega_0)\).
Conversely \(\chi\circ g\) is a Darboux chart for every such \(g\).  The
displayed natural identification is the contravariant functoriality of
factorisation algebras under formal coordinate change.  No coordinate
representative is selected by the restriction functor itself.
\end{proof}

\subsection{The bulk closed field-theory sector as an
...
in \S\ref{ssec:hamiltonian-bf-algebra}.  The closed-sector
...
\]
for $U\subset\R^2$ and $B\subset\C^2$ open, with a commutative
dg-algebra structure from the symmetric algebra structure of the
continuous CE cochains and an \(E_2^{\mathrm{top}}\)-factorization
structure inherited from extension-by-zero on disjoint opens.  The three
...
of Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}.  Thus the
...
\(\mathcal{C}^{\mathrm{op}}_\partial\) by bulk--boundary
factorization operations.  A
...
\begin{prop}[Brane $E_1$-Hochschild as \(E_2^{\mathrm{top}}\)-algebra]\label{app:prop:brane-HH-E2}
The derived $E_1$-centre
$Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})$ is naturally an
\(E_2^{\mathrm{top}}\)-algebra, equivalent to the Hochschild cochain complex
...
with cup product and the unshifted Gerstenhaber bracket of degree
\(-1\), equivalently a degree-zero Lie bracket on
$HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})[1]$.
This is Deligne's
...
Definition~\ref{app:defn:brane-cat}, the conjectural datum is an
open--closed morphism of mixed-HT chiral algebras
  \[
	  \mathrm{OCA}\colon
	...
	\]
at $d=2$, where the right-hand side is the Morita mixed-HT chiral
derived centre of Definition~\ref{app:defn:Z-der-HT}, together with an
operadic homotopy inverse or, equivalently, a contraction of
\(\operatorname{Cone}(\mathrm{OCA})\) in the admissible coefficient
category.  The datum is defined only after the following construction
packages have been fixed: the mixed-HT Swiss-cheese action
Maurer--Cartan lift, the all-arity mixed-HT \(E_2^{\mathrm{HT}}\)
Morita-centre operations, the admissible Hochschild-centre chart lift,
flat transport over the formal Darboux coordinate torsor, the unreduced
cotangent-current realization,
the Weiss/Ran descent cochains, the Costello quantum master-equation
counterterm tower, the anomaly and locality null-homotopies, and the
comparison-cone contraction.  These are the typed rows of
\eqref{eq:typed-total-obstruction-ledger}.  Relative to this datum the
morphism is required to be a quasi-isomorphism; before these choices are
made the conjecture is an obstruction problem, not a distinguished map.
After choosing a dualisable
...
definition of the category-level centre and is an equivalence only under
the Morita-density condition of Definition~\ref{app:defn:Z-der-HT}.

\emph{Known cases.}
\begin{itemize}
  \item At $d=1$ (replacing $\C^2$ by a smooth curve $C$): proved
    (chiral Deligne theorem at \(d=1\),
    Vol I~\cite{lorgat-vol1};
    Beilinson--Drinfeld~\cite{beilinson-drinfeld-chiral}*{\S 4};
    Frenkel--Ben-Zvi~\cite{frenkel-ben-zvi}).
  \item At $d\geq 2$ Lurie's theorem
    \cite{lurie-ha}*{\S 5.3.1} supplies the topological
    \(E_d\)-Hochschild action after forgetting holomorphic,
    boundary-defect, and mixed-HT analytic structure.  The chiral
    mixed-HT lift is the additional obstruction problem recorded in
    \eqref{eq:typed-total-obstruction-ledger}.  Partial adjacent
    structures are recorded by Faonte--Hennion--Kapranov
    \cite{faonte-hennion-kapranov-higher-km} and by Francis's
    factorization-algebra framework \cite{francis-thesis}; they are not
    the mixed-HT \(E_2^{\mathrm{HT}}\)-Deligne datum.
  \item \emph{Formal-Darboux stalk:} the trace-sector generator boundary
    value
    \[
      \begin{gathered}
        G_{\mathrm{CE}}=
        ...
        \\
        \Phi_N^{\mathrm{gen}}\in
        \operatorname{Hom}^{\mathrm{gr}}_\C
        \bigl(G_{\mathrm{CE}},H_N^{\mathrm{Hoch}}\bigr),
        \qquad
    ...
    On the stable scalar-reduced trace algebra its coordinate projection
    \(\Phi_{\mathrm{coord}}^{\mathrm{st}}\) is the
    Chevalley--Eilenberg/polyvector coordinate isomorphism after the
    admissible Tate/pro-Matlis completion.  The full finite-\(N\)
    derived-centre comparison is governed by the typed Hochschild-centre,
    finite-trace, Darboux-torsor, cotangent-current, descent,
    QME-counterterm, anomaly/locality, and cone rows recorded below.  The
...
\[
  G_{\mathrm{CE},p}=
  \operatorname{Gen}_{\mathrm{CE}}\!
  \bigl(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_p\bigr)
   \simeq
	   \operatorname{Gen}_{\mathrm{CE}}\!
	   \bigl(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\bigr),
  \qquad
...
\(\Phi_N^{\mathrm{gen}}(c_f)=\theta_f\in C^1\),
\(\Phi_N^{\mathrm{gen}}(u_f)=J(f)\cdot(-)\in C^0\) on
Chevalley--Eilenberg generators of
$\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]$.
This generator boundary value is not a cochain-algebra map from the
full bulk CE complex.  After scalar reduction and passage to the
stable trace algebra the coordinate projection
\(\Phi_{\mathrm{coord}}^{\mathrm{st}}\) is the admissible CE/PV
coordinate isomorphism.  For fixed finite \(N\) it is not an equivalence
onto the full Hochschild complex.
The arity-two formal diagonal operation at the stalk is the singular
product of
Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}.
The chain-level lift \(\Phi_N^{\mathrm{Hoch}}\) to a strict
\(P_0\)-factorization-centre morphism is the supplied-data object in the
Hochschild-centre row of the typed ledger: it consists of compatible
primitives for the four-component centre/QME/descent lift criterion
\(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\), together with the
cotangent-current row of \eqref{eq:typed-total-obstruction-ledger}
...
with any curve chiral algebra is governed by a curve-restriction theorem
...
(Conjecture~\ref{app:conj:mixed-HT-Deligne}) is the assertion of an
operadic action: the chiral derived centre
...
\begin{enumerate}[label=\textup{(\roman*)}]
  \item antisymmetry under interchange of the two incoming branches at
        a binary vertex;
  \item the scalar Arnold--Jacobi coefficient identity on every
        three-leaf subtree,
        \[
...
\begin{proof}
  ...
  local-cohomology quotient by the scalar Arnold coefficient identity of
...
\begin{lem}[Arity-four residue pentagon]
\label{app:lem:arity-four-residue-pentagon}
  Let \(z^{(1)},z^{(2)},z^{(3)},z^{(4)}\) be four formal points of
  \(\widehat{\C^2}_0\).  The full diagonal
  \(\Delta_{\{1,2,3,4\}}\) has codimension six.  Let \(P\) run over
  the five parenthesizations
  \[
    (((12)3)4),\quad ((1(23))4),\quad
    (1((23)4)),\quad (1(2(34))),\quad ((12)(34)),
  \]
  and let \(T_P\) be the corresponding rooted binary tree.  For every
  \(P\), the edge-normal coordinates
  \(\{\zeta^P_{e,1},\zeta^P_{e,2}\}_{e\in E_{\mathrm{int}}(T_P)}\)
  form a regular system of parameters for the normal formal scheme of
  \(\Delta_{\{1,2,3,4\}}\).  Hence, for every formal Laurent principal
  part \(F\),
  \[
    \operatorname{Res}_{T_P}
    \left(
      F\,
      \bigwedge_{e\in E_{\mathrm{int}}(T_P)}
      \frac{d\zeta^P_{e,1}\,d\zeta^P_{e,2}}
           {\zeta^P_{e,1}\zeta^P_{e,2}}
    \right)
    =
    \operatorname{Res}_{\Delta_{\{1,2,3,4\}}}(F),
  \]
  so the residue functional is independent of the four-leaf
  parenthesization.  The image of the oriented arity-four Stasheff
  pentagon under the tree-current operation is zero in
  \(\mathsf{PP}^{(2)}_{\mathrm{tree}}\): each pentagon edge is the
  outside one-edge kernel tensored with the arity-three
  Arnold--Jacobi class of
  Lemma~\ref{app:lem:arity-three-residue-jacobi}.  Thus the
  arity-four tree-current pentagon contributes no curvature.
\end{lem}
...
\begin{proof}
  ...
  \[
    ...
    c=z^{(3)}-z^{(4)}.
  \]
  The completed local ring at the full diagonal is
  \[
    \C[[a_1,a_2,b_1,b_2,c_1,c_2,z^{(4)}_1,z^{(4)}_2]].
  \]
  The six functions \(a_1,a_2,b_1,b_2,c_1,c_2\) cut out the full
  collision.  The edge-normal coordinates attached to any one of the
  five four-leaf rooted trees are obtained from these six coordinates
  by an invertible formal change of parameters: each internal edge
  records the relative position of two child clusters, and the three
  internal edge differences recover \(a,b,c\) by a nonsingular
  \(\C\)-linear transformation after the root-cluster coordinate is
  fixed.  The logarithmic Grothendieck residue is invariant under such
  a change of regular parameters.  Equivalently, each displayed
  iterated residue extracts the same coefficient functional on the
  local cohomology module
  \(H^6_{\Delta_{\{1,2,3,4\}}}((\widehat{\C^2}_0)^4,
  \omega_{(\C^2)^4/\C^2})\).  This proves parenthesization
  independence of the residue.

  It remains to identify the four-leaf relation in the coefficient
  algebra.  The five edges of the associahedral pentagon are elementary
  rotations on three adjacent inputs or clusters:
  \[
    (((12)3)4)\leftrightarrow ((1(23))4),\quad
    ((1(23))4)\leftrightarrow (1((23)4)),
  \]
  \[
    (1((23)4))\leftrightarrow (1(2(34))),\quad
    (1(2(34)))\leftrightarrow ((12)(34)),\quad
    ((12)(34))\leftrightarrow (((12)3)4).
  \]
  On each edge, the common outside collision contributes one logarithmic
  two-form, and the changing three-leaf subtree contributes exactly the
  scalar Arnold--Jacobi kernel relation together with the signed graded
  Jacobi identity in \(\mathfrak g\).  This is
  Lemma~\ref{app:lem:arity-three-residue-jacobi} tensored with the
  outside edge and outside iterated bracket.  Therefore every oriented
  edge defect vanishes in the quotient defining
  \(\mathsf{PP}^{(2)}_{\mathrm{tree}}\), and the pentagon boundary
  has zero image under the tree-current action.
\end{proof}

\begin{lem}[Tree-kernel composition and rotation coherence]
\label{app:lem:tree-kernel-rotation-coherence}
Let \(I=\bigsqcup_{a\in A}I_a\).  Let \(T_A\) be a rooted binary tree
with leaves \(A\), and let \(T_a\) be rooted binary trees with leaves
\(I_a\).  In local cohomology along the iterated diagonal,
\[
  \gamma(K_{T_A};\{K_{T_a}\}_{a\in A})
  =
  K_{T_A\circ\{T_a\}_{a\in A}},
\]
where \(\gamma\) is pullback to the iterated collision stratum followed
by wedge product of the relative principal-part forms.  Equivalently,
for every formal Laurent principal part \(F\),
\[
  \operatorname{Res}_{T_A}
  \operatorname{Res}_{\{T_a\}}
  \left(F\,K_{T_A}\wedge\bigwedge_{a\in A}K_{T_a}\right)
  =
  \operatorname{Res}_{T_A\circ\{T_a\}}
  \left(F\,K_{T_A\circ\{T_a\}}\right).
\]
If two rooted binary \(I\)-trees differ by a rotation on a three-leaf
subtree, then the signed rotation defect of the corresponding operation
is the product of the outside tree kernel with the arity-three
Arnold--Jacobi relation of
Lemma~\ref{app:lem:arity-three-residue-jacobi}.  Hence the assignment
\[
  K_T\otimes(x_i)_{i\in I}
  \longmapsto
  K_T\,\mathsf J_{[-,\ldots,-]_T(x_i)}
\]
descends from the free antisymmetric tree operad to
\(\mathsf{PP}^{(2)}_{\mathrm{tree}}\), and its partial compositions are
associative in local cohomology.
\end{lem}

\begin{proof}
Choose cluster coordinates for \(T_A\) and for every \(T_a\).  The
cluster coordinates for the grafted tree
\(T_A\circ\{T_a\}_{a\in A}\) are their concatenation.  The completed
local ring at the full collision diagonal is therefore a formal power
series ring in the normal coordinates attached to the internal edges of
all the trees, together with the root-cluster coordinates.  Each
Grothendieck residue is coefficient extraction in the two normal
coordinates of one internal edge.  Coefficient extractions in disjoint
or nested edge-coordinate blocks commute.  This proves the displayed
residue identity and the compatibility of grafting with wedge product.

A rotation is local on a three-leaf subtree.  After the common outside
kernel and the common outside iterated bracket are factored out, its
kernel component is the scalar Arnold local-cohomology relation
\[
  \eta_{ij}\wedge\eta_{jk}
  +\eta_{jk}\wedge\eta_{ki}
  +\eta_{ki}\wedge\eta_{ij}=0,
\]
and its coefficient component is the signed graded Jacobi identity in
\(\mathfrak g\).  This is
Lemma~\ref{app:lem:arity-three-residue-jacobi} inside the selected
subtree.  The graph of rooted binary trees with fixed leaf set is
generated by such rotations; its elementary two-cells are pentagons for
nested rotations and squares for disjoint rotations.  The pentagon
reduces to two applications of the same three-leaf Arnold--Jacobi
relation in adjacent subtrees.  The square reduces to commutation of
the corresponding residue coefficient extractions.  Thus the rotation
relations are killed in the quotient by antisymmetry, Arnold--Jacobi,
and Cech coboundaries, and the operation is independent of the chosen
...
\]
of Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}.  The
...
Jacobi identity in \(\mathfrak g\).
Lemma~\ref{app:lem:arity-four-residue-pentagon} is the four-leaf
Stasheff check: the five four-leaf residues are the same
full-diagonal Grothendieck residue, and the pentagon boundary is the
sum of five arity-three Arnold--Jacobi faces.  For a larger tree, every
associativity comparison is governed by
Lemma~\ref{app:lem:tree-kernel-rotation-coherence}: grafting of trees
is the same operation as iterated residue along the grafted collision
diagonal, and the signed rotation defects are exactly the
Arnold--Jacobi relations tensored with the graded Jacobi identity.
Antisymmetry of the kernel and antisymmetry of the Lie bracket agree at
each binary vertex.  The Chevalley--Eilenberg
...
\begin{rmk}[Remaining obstruction rows]
\label{app:rmk:tree-kernel-not-global-deligne}
Theorem~\ref{app:thm:formal-polydisk-all-arity-current-operad}
constructs the all-arity formal current-kernel layer.  The Morita
mixed-HT derived centre of the open boundary category, a Ran
\(\mathcal D\)-module presentation, Costello--Gwilliam descent on the
brane-link boundary, and the Costello QME counterterm tower are the
...
equivalence at \(d\geq2\) is the separate operation-map theorem: the
operation maps must lift through the relative
Maurer--Cartan fibre
\[
  \mathfrak{Def}^{\mathrm{HT}}_{\mathrm{Mor}\mbox{-}E_2}
    (\mathcal C^{\mathrm{op}}_\partial,b_p)
\]
and satisfy Ran/Weiss descent.  Construction~\ref{app:prop:hEd-algebras}
is the operadic source model.  The proved \(d=2\) formal-current input is
Theorem~\ref{app:thm:formal-polydisk-all-arity-current-operad}; the
missing lift is exactly the vanishing of the obstruction class in
Definition~\ref{app:defn:morita-chiral-E2-extension-complex}, together
with chosen Maurer--Cartan primitives and descent homotopies.
\end{rmk}
...
\begin{defn}[Morita mixed-HT \(E_2^{\mathrm{HT}}\)-extension complex]
...
\[
  ...
  \mathrm{Conv}\bigl(\mathrm B\mathsf Q,
      ...
      \bigr).
\]
The \emph{Morita mixed-HT \(E_2^{\mathrm{HT}}\)-extension complex} is the shifted
homotopy fibre
\[
  \mathfrak{Def}^{\mathrm{HT}}_{\mathrm{Mor}\mbox{-}E_2}
    (\mathcal C^{\mathrm{op}}_\partial,b_p)
...
\(\mathsf Q\)-action to a Morita mixed-HT \(E_2^{\mathrm{HT}}\)-centre action on
\(Z_\partial\).
\end{defn}
...
A Morita mixed-HT \(E_2^{\mathrm{HT}}\)-centre action on
...
\(\operatorname{res}_{b_p}\).  Put
\[
  \mathfrak D=
  \mathfrak{Def}^{\mathrm{HT}}_{\mathrm{Mor}\mbox{-}E_2}
    (\mathcal C^{\mathrm{op}}_\partial,b_p)
\]
with its twisted differential \(d_\Theta\).  A lift modulo \(F^{r+1}\)
is a degree-one element
\(\alpha_{\le r}\in(\mathfrak D/F^{r+1}\mathfrak D)^1\) satisfying
\[
  d_\Theta\alpha_{\le r}
  +\frac12[\alpha_{\le r},\alpha_{\le r}]
  \equiv 0 \pmod {F^{r+1}} .
\]
Choose any degree-one representative in
\(\mathfrak D/F^{r+2}\mathfrak D\).  Its curvature component
\[
  \omega_{r+1}:=
  \operatorname{gr}^{r+1}\!\left(
    d_\Theta\alpha_{\le r}
    +\frac12[\alpha_{\le r},\alpha_{\le r}]
  \right)
  \in
  \bigl(\operatorname{gr}^{r+1}\mathfrak D\bigr)^2
\]
is \(d_\Theta\)-closed by the Bianchi identity
\(d_\Theta\Omega+[\alpha_{\le r},\Omega]=0\) and the fact that
\(\Omega\in F^{r+1}\).  Replacing the representative by
\(\alpha_{\le r}+\gamma_{r+1}\), with
\(\gamma_{r+1}\in(\operatorname{gr}^{r+1}\mathfrak D)^1\), changes
\(\omega_{r+1}\) by \(d_\Theta\gamma_{r+1}\).  Hence
\([\omega_{r+1}]\in H^2(\operatorname{gr}^{r+1}\mathfrak D)\) is the
obstruction class, and a choice of \(\gamma_{r+1}\) with
\(d_\Theta\gamma_{r+1}=-\omega_{r+1}\) is exactly the next correction.
Completeness identifies a compatible sequence of such corrections with
a Maurer--Cartan lift in the inverse limit.
\end{proof}

\subsection{Three supplied data for chiral
...
\begin{defn}[Formality datum for $\mathsf{hE}_d^{\mathrm{ch}}$]\label{app:hyp:formality}
...
equipped with an all-arity \(\infty\)-operad model and a formality
quasi-isomorphism as part of the supplied datum:
\[
  \mathsf{hE}_d^{\mathrm{ch}}\;\simeq\;H_*(\mathsf{hE}_d^{\mathrm{ch}})
\]
to its homology operad in chain complexes of
\(\mathcal D_{\mathrm{Ran}(\C^d)}\)-modules.  The obstruction to
supplying this datum is the deformation class
\(\mathrm{Ob}_F=[\mathfrak m_d]\) in the deformation complex of the
chosen operad model; equivalently, one needs \(\mathrm{Ob}_F=0\) with a
chosen degree-one primitive.  At \(d=1\) the topological input is the
ordinary Deligne--Tamarkin Hochschild formality theorem
\cite{tamarkin-deligne}\cite{kontsevich-soibelman-deformation};
the curve chiral passage uses Beilinson--Drinfeld
\cite{beilinson-drinfeld-chiral}*{\S 4}.  For \(d\geq2\) no such
chiral formality theorem is used here without the specified vanishing
and primitive.
\end{defn}

\begin{defn}[Chiral centrality datum]\label{app:hyp:centrality}
For \(\mathcal A\in\mathrm{HolFA}_d(\C^d)\), a chiral centrality datum
consists of a chiral bimodule \(\infty\)-category
...
\]
It includes a homotopy-central chiral action of this object on
\(\mathcal A\), compatible with factorization products.  Let
\(\mathfrak{Def}^{\mathrm{ch}}_{\mathrm{cent}}(\mathcal A)\) be the
homotopy fibre of the restriction map from the convolution dg Lie
algebra controlling chiral actions on the pair
\((Z^{\mathrm{der}}_{\mathsf{hE}_d^{\mathrm{ch}}}(\mathcal A),
\mathcal A)\) to the convolution dg Lie algebra controlling the
underlying action before the homotopy-centrality equations are imposed.
The obstruction to supplying the centrality datum is
\[
  \mathrm{Ob}_{C}\in
  H^2\!\left(\mathfrak{Def}^{\mathrm{ch}}_{\mathrm{cent}}(\mathcal A)\right),
\]
and the datum used below is \(\mathrm{Ob}_{C}=0\) together with a
chosen degree-one primitive at each filtration step.
...
\begin{defn}[Ran \( ! \)-descent datum]\label{app:hyp:ran-descent}
A Ran \( ! \)-descent datum consists of a chosen Ran
\(\mathcal D\)-module presentation
\(\mathcal A_{\mathrm{Ran}}\) of the Costello--Gwilliam
factorization algebra \(\mathcal A\), and \(\mathcal A_{\mathrm{Ran}}\)
satisfying \( ! \)-descent on the Ran site.  Equivalently, if
\[
  \chi_{\mathrm{Ran}}\colon
  \mathcal A_{\mathrm{CG}}\longrightarrow
  \mathcal A_{\mathrm{Ran}}^{!}
\]
is the comparison from the Costello--Gwilliam Weiss descent model to the
Ran \( ! \)-descent model in the selected coefficient category, then the
datum is a contracting homotopy of
\(\operatorname{Cone}(\chi_{\mathrm{Ran}})\).  The obstruction is the
cohomology object
\[
  \mathrm{Ob}_{R}:=
  H^\bullet\!\left(\operatorname{Cone}(\chi_{\mathrm{Ran}})\right).
\]
At \(d=1\) this is the Beilinson--Drinfeld factorization theorem for
chiral algebras on curves \cite{beilinson-drinfeld-chiral}*{\S 3}.  At
\(d\geq2\) the present datum is only the displayed Ran comparison, or
equivalently the vanishing of \(\mathrm{Ob}_{R}\) with a chosen
contraction.
\end{defn}

\begin{thm}[Chiral \(E_d\) Deligne supplied-data criterion]\label{app:thm:relative-Ed-deligne}
Let \(\mathcal A\in\mathrm{HolFA}_d(\C^d)\).  Suppose the formality
datum of Definition~\ref{app:hyp:formality}, the chiral centrality datum
of Definition~\ref{app:hyp:centrality}, and the Ran \( ! \)-descent
datum of Definition~\ref{app:hyp:ran-descent} are supplied.  Equivalently,
\(\mathrm{Ob}_{F}=0\), \(\mathrm{Ob}_{C}=0\), and
\(\mathrm{Ob}_{R}=0\), with the primitives and the Ran comparison-cone
contraction specified.  Then the chiral derived centre satisfies
\[
  Z^{\mathrm{der}}_{\mathsf{hE}_d^{\mathrm{ch}}}(\mathcal A)
   \;\simeq\;
   C^\bullet_{\mathrm{ch}}(\mathcal A,\mathcal A)
\]
as \(E_{d+1}^{\mathrm{ch}}\)-algebras.  A nonzero
\(\mathrm{Ob}_{F}\), \(\mathrm{Ob}_{C}\), or \(\mathrm{Ob}_{R}\), or a
missing primitive or comparison-cone contraction, is the exact
obstruction to this criterion in the selected coefficient category.
\end{thm}

\begin{proof}
By Definition~\ref{app:hyp:centrality} the chiral bimodule
\(\infty\)-category
\(\mathcal A\otimes\mathcal A^{\mathrm{op}}\text{-Mod}^{\mathrm{ch}}\)
...
complex \cite{beilinson-drinfeld-chiral}*{\S 3.4}.
This is the two-sided bar resolution in the chiral bimodule category.  It
is not the twisting bar coalgebra \(\mathrm{Bar}(A)\) that appears in a
bar--cobar comparison.

\emph{Model independence.}  By Definition~\ref{app:hyp:ran-descent} the
...
carries the supplied chiral brace operations: a tuple of cochains is
composed by inserting outputs into inputs along the chiral product.
These operations assemble
\(C^\bullet_{\mathrm{ch}}\) into an
\(\mathsf{hE}_d^{\mathrm{ch}}\)-algebra.  By
Definition~\ref{app:hyp:formality} the operad quasi-isomorphism
...
gives the formal current-kernel input.  A proof of formality for the
full chiral \(\mathsf hE_2^{\mathrm{ch,hol}}\)-operad, a Ran descent
theorem, and a Morita centre lift require the additional data recorded
in the next theorem.  The criterion identifies the exact obstruction
datum whose vanishing and chosen primitives promote this formal current
layer to the stalkwise mixed-HT \(E_2^{\mathrm{HT}}\)-Deligne statement.
...
a Dirac brane stack.  The tag \(\mathrm{cl}/\hbar\) means either the
classical current factorization algebra or that algebra equipped with
the supplied Costello QME branch; it does not identify the QME, Weyl,
equivariant, or Fourier--Rees parameters.  Equip this source with the
brane-preserving \(\Omega\)-background
...
The assertion that this action extends to a stalkwise mixed-HT
\(E_2^{\mathrm{HT}}\)-Deligne quasi-isomorphism is equivalent to the
...
\[
  Z^{\mathrm{der}}_{\mathsf{hE}_2^{\mathrm{ch},\mathrm{HT}}}(\mathcal A)
   \;\simeq\;
   C^\bullet_{\mathrm{ch}}(\mathcal A,\mathcal A)
\]
as algebras over the induced \(E_3^{\mathrm{ch}}\)-structure of this
mixed-HT model, projectively with central scalar Capelli anomaly
...
(Corollary~\ref{cor:binary-generation-higher-two-dimensional-singular-product}), so
...
the residual primitives commute with the finite-window restriction and
weight-projection maps up to the specified chain homotopies.

If the four displayed obstruction components vanish, the data of
Definitions~\ref{app:hyp:formality}, \ref{app:hyp:centrality}, and
\ref{app:hyp:ran-descent} hold for this stalk with specified primitives
and comparison-cone contraction.
Theorem~\ref{app:thm:relative-Ed-deligne} then gives the induced
\(E_3^{\mathrm{ch}}\)-equivalence of the mixed-HT model, and its scalar
weight-zero curvature is the Capelli class by
Theorem~\ref{thm:omega-localization-collapse}.
Conversely, such an induced \(E_3^{\mathrm{ch}}\)-equivalence restricts to the
...
A Stein target removes the sheaf-cohomological part of the specified
\v Cech--Weiss descent problem.  Globalisation is governed by the
mixed-HT Swiss-cheese action, the Morita centre lift, the quantum master
equation counterterms, the anomaly and locality null-homotopies, and the
comparison-cone contraction.
...
\item On every \(U_a\) and every finite intersection
  \(U_{a_0\cdots a_p}\), a mixed-HT Swiss-cheese action
  Maurer--Cartan lift is supplied in
  \[
    \mathfrak{Def}^{\mathrm{HT}}_{\mathrm{SC}}\!
    \left(
	    \mathcal A^{\mathrm{cl}/\hbar}_{\mathrm{bulk}}|_{U_{a_0\cdots a_p}},
	    \mathcal C^{\mathrm{op}}_\partial|_{U_{a_0\cdots a_p}}
	    \right),
	  \]
	  with curvature representatives
	  \(\mathcal R^{\mathrm{SC}}_{n+1}\), classes
	  \(\mathfrak o^{\mathrm{SC}}_{n+1}
	  =[\mathcal R^{\mathrm{SC}}_{n+1}]\), and primitives
	  \(H^{\mathrm{SC}}_{n+1}\) satisfying
	  \(d_{\Theta_{\le n}}H^{\mathrm{SC}}_{n+1}
	  =-\mathcal R^{\mathrm{SC}}_{n+1}\) for all \(n\ge2\), compatible
	  with restriction to smaller intersections.
\item On every \(U_a\), the formal current-kernel criterion of
  ...
  vanish, with residual primitives compatible under finite-window
  restriction and weight projection.
...
\item The all-scale quantum master equation counterterm datum, anomaly
  and locality null-homotopies, and comparison-cone contraction of
  Definition~\ref{defn:oca-comparison-cone} are supplied.
\end{enumerate}
Then the supplied Swiss-cheese action data, local centre-lifted maps,
descent primitives, QME counterterms, anomaly/locality null-homotopies,
and cone contraction assemble to a projective open--closed
quasi-isomorphism
\[
  \mathrm{OCA}_N\colon
...
trace-sector scalar anomaly is the projective Lie anomaly
\(\hbar_{\mathrm{W}} N[\bar c]\).  The Stein hypotheses verify only the
Cartan--H\"ormander acyclicity row for \(\C^2\), for the
analytifications of smooth affine holomorphic-symplectic surfaces, and
for cotangent bundles \(T^*\Sigma\) of affine curves; the
quasi-isomorphism conclusion is the datum \textup{(S1)--(S5)}.  The
punctured plane
\(\C^2\setminus\{0\}\) is not covered by the Stein conclusion.
\end{thm}

\begin{proof}
\emph{Local data.}  Condition \textup{(S1)} supplies the source and
target \(\mathrm{SC}^{\mathrm{HT}}_{(1,2)}\)-algebra structures on the
Weiss cover and on all finite intersections.  Condition \textup{(S2)}
is the obstruction criterion of Theorem~\ref{app:thm:FCR-at-stalk}.
On each member of the Weiss cover it gives a projective local
pre-comparison datum \(F_a\) whose central scalar anomaly is
\(\hbar_{\mathrm{W}} N[\bar c]\);
it also records the chosen primitives needed for the Morita centre lift
and residual weight-zero terms.  This is local formal-Darboux data, not
a global morphism.

\emph{\v Cech--Weiss descent.}  Conditions \textup{(S3)} and
\textup{(S4)} place the local \(F_a\)'s in the total complex
\(\operatorname{Tot}\check C^\bullet_{\mathrm{Weiss}}
...
its higher coherences; by \textup{(S4)} those classes have specified
null-homotopies.  Therefore the \(F_a\)'s assemble to a descended
pre-comparison datum.  The Morita centre, QME-counterterm, anomaly, and locality
rows are not consequences of Stein acyclicity.

\emph{Quantum and cone data.}  Condition \textup{(S5)} supplies the QME
counterterms, the anomaly/locality null-homotopies, and the contraction
of the global comparison cone.  Together with the local Morita
centre-lift data of \textup{(S2)}, this lifts the descended
pre-comparison datum to an open--closed morphism and supplies the
projective quasi-isomorphism datum.  The scalar projective anomaly is
unchanged by the descent homotopies, because on every chart it is the
central Capelli class \(\hbar_{\mathrm{W}} N[\bar c]\) of
...
descent obstruction.  If \(\operatorname{per}_X(v)=0\), the compact
variant of Theorem~\ref{app:thm:stein-global-deligne} is governed by the
same mixed-HT Swiss-cheese action, local centre-lift, QME,
anomaly/locality, and comparison-cone data.  A
...
\begin{rmk}[Obstruction class for Definition~\ref{app:hyp:formality}]\label{app:rmk:formality-obstruction}
Definition~\ref{app:hyp:formality} supplies the assertion that the
...
zero.  At \(d=1\) the ordinary \(E_1\)-Hochschild formality theorem of
Tamarkin~\cite{tamarkin-deligne} gives the corresponding topological
input by associator methods.  At \(d\geq2\)
...
The global chiral \(E_d\)-Deligne datum at \(d\geq2\) consists of the
operadic action of Definition~\ref{app:defn:chiral-Ed-operad} together
with Definitions~\ref{app:hyp:formality}, \ref{app:hyp:centrality}, and
...
Definition~\ref{app:defn:morita-chiral-E2-extension-complex}; in this
stalk \(\mathrm{Ob}_C\) is represented by
\(\operatorname{Ob}^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}\).  The full
generic obstruction is the class
\[
  \mathrm{Ob}_{\mathrm{ch}E_d}
   =
	   ...
	   \bigr),
\]
where \(\mathrm{Ob}_{F}\), \(\mathrm{Ob}_{C}\), and \(\mathrm{Ob}_{R}\)
are the formality, chiral centrality, and Ran-descent obstructions
beyond the formal Hamiltonian-current suboperad.  The formal-Darboux
stalk theorem and Theorem~\ref{app:thm:formal-polydisk-all-arity-current-operad}
compute the \(d=2\) local Hamiltonian-current operations to which any
global open--closed comparison must restrict; vanishing of
...
(Definition~\ref{app:defn:brane-cat}) is supplied as global data on the
Costello--Gwilliam brane-link boundary $\partial X$ only with a Weiss
descent datum for local boundary categories.  The datum is the
brane-link descent object below.  A
global mixed-HT Deligne quasi-isomorphism requires this descent datum
together with the chiral \(E_d\), quantum-master-equation counterterm, and
...
written in the factorization vocabulary of
\cite{costello-gwilliam}*{Vol.~I Secs.~6.1, 7.2},
\[
  \partial X
   \;=\;\partial(X\setminus\nu(L))
   \;\cong\;
...
first the formal-completion fibre of this boundary diagram.  In a
collared formal-Darboux Weiss chart whose holomorphic normal factor is
\((\C^2,dz_1\wedge dz_2)\), that holomorphic factor is identified with
the formal-Darboux stalk of
Definition~\ref{app:defn:Darboux-stalk-functor} only after choosing a
point of the Darboux-chart torsor; transition maps transport the
coordinate representative by formal symplectomorphisms in the
\((z_1,z_2)\)-directions.
\end{defn}
...
and the local Morita \(E_2\)-centre lift, relative to the supplied
local mixed-HT Swiss-cheese action data.  A first overlap gluing datum
is a Čech \(1\)-cochain
\[
...
classes in
\(\operatorname{Tot}\check C^\bullet_{\mathrm{Weiss}}
(\mathfrak U;\mathcal E^\bullet_{\mathrm{desc}})\) have representatives
\(\mathcal R^{\mathrm{desc}}_j\) and compatible cochains \(h_j\)
satisfying \(d_{\mathrm{desc}}h_j=-\mathcal R^{\mathrm{desc}}_j\), then
the local trace-sector
formal-Darboux maps glue to a descended pre-open--closed comparison
datum
\[
  \mathrm{preOCA}_N\colon
   \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}
   \longrightarrow
   \underline{\mathrm{End}}_{\mathrm{SC}^{\mathrm{HT}}_{(1,2)}}\!
   \bigl(\mathcal{C}^{\mathrm{op}}_\partial\bigr)
\]
on the Costello--Gwilliam brane-link boundary, relative to the supplied
mixed-HT Swiss-cheese action datum.  It lifts to a global
open--closed morphism into
\(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
(\mathcal C^{\mathrm{op}}_\partial)\) only after the Morita mixed-HT
\(E_2^{\mathrm{HT}}\)-centre lift, the quantum master equation counterterm tower, and
the anomaly and locality rows of
Definition~\ref{defn:global-mixed-ht-deligne-obstruction-datum} have
been killed.  It is a global open--closed quasi-isomorphism only after
the comparison-cone obstruction of
Definition~\ref{defn:oca-comparison-cone} vanishes.
\end{thm}
...
local maps \(F_a\), hence produces the descended pre-comparison datum.
The Morita centre lift, QME-counterterm, anomaly, locality, and cone equations are
independent rows of
...
Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}.  Its quantum
lift is a recursive quantum master equation in the Costello
local-functional complex.  At order \(\hbar_{\mathrm{QME}}^n\) the obstruction lies in
...
it is not killed by the classical Jacobi identity.  The data below name
the exact additional structures and obstruction classes needed to
transport this recursion to the trace-sector centre.
...
brane-side Hochschild target containing the trace-sector generator
action \(\Phi_N^{\mathrm{gen}}\) of Theorem~\ref{thm:main-local}.  A
full identification with the bulk stalk is the admissible centre-lift
problem.
...
homotopy-inverse data have been supplied.  The data below are formulated
on that target.

\begin{defn}[Filtered formal SDR datum]\label{app:hyp:KZ-SDR}
A filtered formal strong-deformation-retract datum is a pronilpotent
filtered strong deformation retract
\[
...
\]
over \(\C[[\hbar_{\mathrm{QME}}]]\), compatible with the scalar-contact filtration,
...
the formal-Darboux stalk theorem.  Its obstruction is the class
\[
  [\mathfrak{ob}_K]\in
  H^\bullet(\mathfrak{Def}_{\mathrm{SDR}})
\]
of the deformation complex whose Maurer--Cartan elements are triples
\((i,p,h)\) satisfying \(pi=\operatorname{id}\) and
\(ip-\operatorname{id}=Qh+hQ\) with the stated filtrations and
centrality compatibilities.
\end{defn}

\begin{rmk}[Formal and analytic transfer]\label{app:rmk:why-analytic}
The classical PVA Jacobi holds in the $\hbar_{\mathrm{QME}}$-adic topology of the
...
content of Definition~\ref{app:hyp:stokes} together with explicit graph
estimates.
\end{rmk}

\begin{defn}[Sectorial Stokes datum]\label{app:hyp:stokes}
If an analytic refinement is sought, a sectorial Stokes datum is the
assertion that the renormalized graph sums and the
transferred homological perturbation data form a sheaf over sectors
\(0<|\hbar_{\mathrm{QME}}|<r\).  On the sectors meeting the real-positive
\(\hbar_{\mathrm{QME}}\)-axis the Stokes automorphisms act trivially on the chosen
trace-sector image, and the sectorial lifts have the same asymptotic
\(\hbar_{\mathrm{QME}}\)-adic series.  Its obstruction class
\([\mathfrak{ob}_S]\) is the Stokes mismatch of this sheaf on overlaps
of sectors.  No KZ connection is assumed unless it is constructed as
part of this analytic sheaf.
\end{defn}

\begin{defn}[Finite-window weight-symmetry datum]\label{app:hyp:reflected-weights}
Under crossing \(z\leftrightarrow w\) on the formal \(E_2\) singular product of
Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product}, the PVA
...
weight window.  The obstruction \([\mathfrak{ob}_W]\) is the cohomology
class of the commutator between \(R\) and the binary kernel in the
finite-window endomorphism complex.  This is \(S_2\)-equivariance of the
binary kernel.  Reflection positivity and the all-arity mixed-HT
\(E_2^{\mathrm{HT}}\)-centre action are separate structures.
\end{defn}

\begin{defn}[Stress-tensor primitive datum]\label{app:hyp:T-Q-G}
...
complex.  The obstruction \([\mathfrak{ob}_G]\) is the cohomology class
of \(T\) in the scalar-contact quotient of the scale-\(L\)
local-functional complex.  Transport to Hochschild cochains is allowed
only after the admissible quantum closed-to-open map has been
constructed.
\end{defn}

\begin{defn}[Finite-scale brane-defect QME datum]
\label{app:defn:finite-scale-brane-defect-qme-datum}
Fix a brane interval \(I\subset \R_{\mathrm{brane}}\), an admissible
Hamiltonian weight \(w\), and a scale \(0<\epsilon<L<\infty\).  A
finite-scale brane-defect quantum master-equation datum consists of the
following objects.
\begin{enumerate}
  \item A filtered Costello local-functional complex
  \[
    \mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I)
	      =
	      \mathcal O_{\mathrm{loc}}^{\mathrm{bd}}
	      \bigl(\mathcal E_w|_I;\,
	      A_{\partial,\hbar_{\mathrm{W}}}^{\mathrm{pp},w}(I)
	      [[\hbar_{\mathrm{QME}}]]\bigr)
	  \]
  with finite Hamiltonian windows
  \(\mathfrak Q^\bullet_{w,M,\partial,\hbar_{\mathrm{QME}}}(I)\),
  transition maps, and the scalar-contact filtration.
  \item An elliptic gauge fixing \(Q^{\mathrm{GF}}\), heat kernel
  \(K_t^w\), and finite-scale propagator
  \[
    P_{\epsilon,L}^w
      =
      \int_\epsilon^L Q^{\mathrm{GF}}K_t^w\,dt
  \]
  satisfying \([Q,P_{\epsilon,L}^w]=K_\epsilon^w-K_L^w\) in the
  weighted coefficient category.  The coefficient coevaluation is the
  weighted BV receiver of Definition~\ref{defn:wt-coefficient-pair};
  in particular this datum includes the convergence of the Casimir
  kernel required by Theorem~\ref{thm:hadamard-mittag-leffler}.
  \item Finite-scale BV operations
  \[
    \{-,-\}_L,\qquad \Delta_L
  \]
  on the same local-functional complex, compatible with finite-window
  transition maps and satisfying the Costello BV identities used in
  \eqref{app:eq:qme-order-recursion} below.
  \item Effective interactions \(I^w[L]\) obtained from a bare
  interaction and graphwise local counterterms by the Costello RG
  operator
  \[
    I^w[L]
      =
      \lim_{\epsilon\to0}
      W\bigl(P_{\epsilon,L}^w,\,
      I^w[\epsilon]+I^{\mathrm{CT}}_w(\epsilon)\bigr),
  \]
  with local counterterms
  \(C_{\Gamma,w}^{M}(\epsilon)\in
  \mathcal O_{\mathrm{loc}}(\mathcal E_w^M)\) supported on the relevant
  bulk diagonals or brane-collision strata.  These counterterms are
  compatible with Hamiltonian-window restriction, admissible weight
  transport, interval extension by zero, and the RG composition law
  \(I^w[L_2]=W(P^w_{L_1,L_2},I^w[L_1])\).
  \item The scale-\(L\) linearized BV differential
  \[
    d_{\mathrm{cl},w}^{L}
      =
      Q+\{S^{(0)}[L],-\}_L
  \]
  on the obstruction complex, and the completed quantum BV differential
  after a solution has been constructed,
  \[
    d_{\mathrm{QME},w}^{L}
      =
      Q+\{I^w[L],-\}_L+\hbar_{\mathrm{QME}}\Delta_L
  \]
  together with a scalar-contact projection stable under
  \(d_{\mathrm{cl},w}^{L}\) and under the finite-stage quantum
  differentials produced by the recursion,
  \[
    ...
    C^\bullet_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1]
      [[\hbar_{\mathrm{QME}}]] .
  \]
  \item A curved bulk-to-defect kernel for the source current complex of
  ...
  \[
    ...
    \mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I),
  \]
  with finite-window residual complexes
  \[
    \mathcal Q^\bullet_{w,M}(I)
      =
      \ker\widehat\sigma_{\mathrm{sc},M},
  \]
  and Roos-compatible transition maps.
\end{enumerate}
For such a datum, the order-\(n\) residual of an action
\(S^{(0)}+\cdots+\hbar_{\mathrm{QME}}^{n-1}S^{(n-1)}\) is
\begin{equation}
\label{app:eq:qme-order-recursion}
  \mathcal R_{n,L}
    =
    -\frac12
    \sum_{\substack{i+j=n\\ i,j<n}}
      \{S^{(i)},S^{(j)}\}_L
    -\Delta_L S^{(n-1)} .
\end{equation}
  Its image in each finite window defines
  \[
  [\mathcal R_{n,M,L}]
    \in H^1(\mathcal Q^\bullet_{w,M}(I),d_{\mathrm{cl},w}^{L}).
  \]
The all-order brane-defect QME tower is solved precisely when these
classes admit primitives \(S^{(n)}_{M,L}\) compatible with scale
transition, finite-window restriction, admissible weight transport, and
the Roos \(\varprojlim^1H^0\) primitive-compatibility condition.
\end{defn}

\begin{thm}[All-order QME obstruction criterion for the trace-sector assignment]\label{app:thm:relative-qme}
Assume the filtered formal SDR datum of
Definition~\ref{app:hyp:KZ-SDR}, the finite-window weight-symmetry datum
of Definition~\ref{app:hyp:reflected-weights}, and the stress-tensor
primitive datum of Definition~\ref{app:hyp:T-Q-G}.  Assume also the
finite-scale brane-defect QME datum of
Definition~\ref{app:defn:finite-scale-brane-defect-qme-datum} and the
quantum centrality homotopies for the centre lift.  For every
\(n,M,L\), let
\([\mathcal R_{n,M,L}]\) be the residual class of
\eqref{app:eq:qme-order-recursion}.  If all classes
\([\mathcal R_{n,M,L}]\) vanish and the chosen primitives are compatible
under finite-window restriction, scale transition, admissible weight
transport, and the Roos \(\varprojlim^1H^0\) condition, then the
formal-Darboux stalk of Theorem~\ref{app:thm:stalk-of-conj} admits,
inside that finite-scale datum, a formal \(\hbar_{\mathrm{QME}}\)-adic
local quantization: there exists a quantum BV master action
$S^{\mathrm{quant}}=S^{\mathrm{cl}}+\sum_{n\geq 1}\hbar_{\mathrm{QME}}^n S^{(n)}$
satisfying
\[
  QS^{\mathrm{quant}}
  +
  \frac12\{S^{\mathrm{quant}},S^{\mathrm{quant}}\}_L
  +\hbar_{\mathrm{QME}}\Delta_L S^{\mathrm{quant}}=0
\]
to all orders as a formal power series in \(\hbar_{\mathrm{QME}}\), and a
quantum trace-sector closed-to-open factorization-centre assignment
\[
  \Phi_N^{\hbar_{\mathrm{QME}}}\colon
  \mathcal A^{\mathrm{q,tr}}_{\mathrm{bulk}}|_p
  \longrightarrow
  Z^{P_{0,\hbar_{\mathrm{QME}}}}_{\mathrm{fact}}
  (\mathrm{Obs}^\hbar_{\mathrm{open}})|_b
\]
whose classical limit is the chosen lift \(\Phi_N^{\mathrm{Hoch}}\).
Conversely, within the same finite-scale datum, a nonzero
\([\mathcal R_{n,M,L}]\) or a nonzero Roos primitive-compatibility class
at any order blocks the order-\(n\) extension of the brane-defect QME
tower.
No analytic convergence in \(\hbar_{\mathrm{QME}}\), global descent, or
full finite-\(N\) Hochschild-centre equivalence is asserted by this
criterion.  After a quantum boundary \(A_\infty\)-algebra
...
\(J^{\mathrm{quant}}(f)=J(f)+\sum_n\hbar_{\mathrm{QME}}^nJ^{(n)}(f)\).
The assignment is a morphism of Hochschild complexes only in the chosen
Hochschild model, with the quantum equivariance homotopies included as
part of the datum.  Descent to Hochschild cohomology and an all-order
derived-centre quasi-isomorphism require the Hochschild equivariance,
centre-lift, cotangent-current, descent, anomaly/locality, and cone rows
of \eqref{eq:typed-total-obstruction-ledger}, together with the data of
...
\]
The right-hand side is \(d_{\mathrm{cl}}^L\)-closed by the order
\(<n\) quantum master equation and the Costello BV identities
\[
  (d_{\mathrm{cl}}^L)^2=0,\qquad
  [d_{\mathrm{cl}}^L,\Delta_L]=0,\qquad
  \Delta_L\{a,b\}_L
   =\{\Delta_La,b\}_L
    +(-1)^{|a|+1}\{a,\Delta_Lb\}_L .
\]
The first identity is the classical master equation for \(S^{(0)}[L]\);
the other two are part of the supplied scale-\(L\) BV package.  Its
cohomology class is the Costello obstruction at loop order \(n\).  By
the supplied datum this class vanishes and a compatible primitive is
chosen; induction constructs \(S^{(n)}\) for every \(n\).
If \([\mathcal R_{n,M,L}]\neq0\) in any finite window, no
\(S^{(n)}_{M,L}\) solves the displayed equation in that window.  If the
finite-window classes vanish but the Roos \(\varprojlim^1H^0\) primitive
class is nonzero, the primitives do not assemble to a compatible
completed tower.  These are exactly the two obstruction alternatives in
the theorem.

The scalar-contact projection, finite-window residual complexes, curved
bulk-to-defect kernel, compatible counterterms, and Roos-compatible
primitive data supply the brane-defect tower.
Definition~\ref{app:hyp:KZ-SDR} transfers the formal tower to the
trace-sector centre.  Definition~\ref{app:hyp:reflected-weights} is the
finite-window \(S_2\)-compatibility of the binary kernel.
Definition~\ref{app:hyp:T-Q-G} supplies the local-functional homotopy for
the stress tensor.  Definition~\ref{app:hyp:stokes}, when imposed, gives
...
\[
  \mathrm{Ob}_{\mathrm{QME}}
   =
   \bigl(
   ([\mathcal R_{n,M,L}])_{n,M,L},
   [\mathfrak{ob}_K],[\mathfrak{ob}_S],
   [\mathfrak{ob}_W],[\mathfrak{ob}_G]\bigr),
\]
with entries the finite-scale Costello residual classes of
Definition~\ref{app:defn:finite-scale-brane-defect-qme-datum}, the
formal-SDR obstruction \( [\mathfrak{ob}_K]\), the sectorial Stokes class
\( [\mathfrak{ob}_S]\), the binary weight-symmetry class
\( [\mathfrak{ob}_W]\), and the stress-tensor-exactness class
\( [\mathfrak{ob}_G]\).  It becomes a single class in
\(H^*(\mathrm{Loc}_{\R^2\times\C^2})\) only after these entries have been
represented in the same Costello local-functional deformation complex.
At fixed loop order, only the finite-window entries explicitly computed
in \(\mathcal Q^\bullet_{w,M}(I)\) are checked.  Analytic convergence
requires the sectorial estimates of Definition~\ref{app:hyp:stokes}.
\end{rmk}

\subsection{Operator-level lift of the Igusa section and its square-root branch}\label{app:hfc:operator-lift-modular}

In the compact \(K3\times E\) comparison, choose a modular line
\(\Omega_{\mathrm{central}}\)
(Remark~\ref{rmk:omega-central-definition}) on the Siegel period
domain \(\mathfrak H_2\)
and a matched-conventions map from the local Capelli scalar class to it.
The Igusa cusp form \(\Phi_{10}\) is then the degree-\(2\) scalar section
of that chosen modular line
\cite{igusa-cusp-form}\cite{gritsenko-nikulin-paramodular}.  The square
root \(\Delta_5=\Phi_{10}^{1/2}\) is defined only after a square-root
cover or branch datum has been fixed.  The formal-Darboux theorem
constructs the local Capelli scalar.  The modular curvature
\(\Omega_{\mathrm{central}}\) is part of the compact matched-conventions
datum.  An operator-level lift is an
operator-valued cocycle whose protected trace is \(\Phi_{10}\), or
\(\Delta_5\) after the square-root datum has been chosen, in the
compact-target theory.

\begin{conj}[Operator lift of the chosen Igusa modular section]\label{app:conj:operator-lift-Delta5}
After the compact comparison has supplied
\(\Omega_{\mathrm{central}}\) and, for \(\Delta_5\), a square-root
branch, there exists a graded state space \(V\), an involution or
real form \(\sigma\), and a protected trace, with \(V\) obtained either
from a specified curve-restricted vertex algebra or from a
higher-dimensional factorization algebra, such that
\[
...
\]
with \(S=\Delta_5\) only on the chosen square-root cover of the Siegel
period domain \((\tau,\rho,\nu)\in\mathfrak H_2\), with the trace
compatible with the chosen modular line \(\Omega_{\mathrm{central}}\).
The required datum is the operator-valued cocycle lifting this scalar
modular section.
\end{conj}

Scalar automorphy does not determine an operator-valued cocycle.  The
missing datum is a curve vertex algebra obtained from a specified curve
restriction, or a higher-dimensional factorization algebra, with a
protected trace.
...
curve-restricted vertex algebra, or to a higher-dimensional
factorization algebra, whose graded trace is \(\Delta_5\) requires an
...
The obstruction is the failure to construct a free-field or
...
that trace to the degree-\(2\) modular line.
\end{constr}
...
\begin{thm}[Global Mixed-HT Deligne supplied-data criterion]\label{app:thm:global-deligne-criterion}
Let \(\mathrm{preOCA}_N\) be a pre-open--closed comparison datum whose
formal-Darboux fibre contains the trace-sector generator boundary value of
Theorem~\ref{app:thm:stalk-of-conj}.  Assume the non-optional typed
obstruction ledger of Theorem~\ref{thm:typed-global-mixed-ht-deligne}
has representatives and primitives:
\[
  \operatorname{Ob}^{\mathrm{typed}}_{\mathrm{nt}}
  :=
  \left(
  ...
  \right)
\]
obtained from \eqref{eq:typed-total-obstruction-ledger} by omitting only
the optional topologisation row.  The five-component vector
\(\operatorname{Ob}^{\mathrm{global},5}\) is the morphism-stage subtuple
\eqref{eq:obs-five-component}; it does not contain the
Hochschild-centre, Darboux-transport, cotangent-current,
common-deformation-complex, primitive-coherence, or cone rows.

Concretely, choose curvature representatives
\(\mathcal R^{\mathrm{SC}}_{n+1}(\Theta_{\le n})\) and
\(\mathcal R^{\mathrm{global},5}\) as in
\eqref{eq:global-five-representatives}; choose a Hochschild-centre
Maurer--Cartan lift
\(\mathcal C_{\Phi_N^{\mathrm{Hoch}}}\) killing
\(\operatorname{Ob}^{\mathrm{cent}}_{\mathrm{Hoch}}\); choose
Darboux-transport primitives and cotangent-current primitives
\[
  h_{\nabla},\quad h_{\nabla,\Theta},\quad h_{\mathrm{cot},I}
\]
for every brane interval \(I\); choose the comparison-unifying datum
\((\mathfrak K_{\mathrm{HT}},\chi_{\mathrm{nat}})\) of
Definition~\ref{defn:common-ht-deformation-complex} and row
\textup{(D9)} of
Definition~\ref{defn:typed-mixed-ht-deligne-equivalence-datum}, with
\(\operatorname{Ob}^{\mathrm{unif}}_{\mathrm{HT}}=0\).  This transports
the six native curvature representatives to the single curvature
\(F_{\mathrm{HT}}(\Theta_{\mathrm{OCA}})\) of
Proposition~\ref{prop:six-projections}.  Choose also the primitive
compatibility datum \(\mathfrak C_{\mathrm{prim}}\) of
\eqref{eq:global-primitive-compatibility-datum} and
\eqref{eq:global-tertiary-primitive-compatibility-datum}, extended to
the all-simplex coherence datum
\eqref{eq:global-all-simplex-primitive-compatibility-datum}.
Applying \(d_{\Theta_{\le n}}\) to \(H^{\mathrm{SC}}_{n+1}\) gives
\(-\mathcal R^{\mathrm{SC}}_{n+1}(\Theta_{\le n})\).  The Hochschild-centre
lift has zero curvature.  Applying \(d_{\mathrm{Darb}}\) to \(h_{\nabla}\) and
\(h_{\nabla,\Theta}\) gives \(-\Omega_{\nabla}\) and
\(-\mathcal R_{\nabla,\Theta}\).  Applying \(d_{\mathrm{cot}}\) to
\(h_{\mathrm{cot},I}\) gives \(-\mathcal R^{\mathrm{cot}}_{\partial,I}\).
The Morita, descent, quantum-master-equation, anomaly, and locality primitives
are sent respectively, using \(d^{L}_{\mathrm{QME},n,M}\) for the
order-\(n\) finite-window QME coefficient differential of
Definition~\ref{defn:typed-brane-defect-qme-tower}, to
	\(-\mathcal R^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}\),
	\(-\mathcal R^{\mathrm{desc}}_{\partial}\),
	\(-\mathcal R^{\mathrm{QME}}_{n,w,r,M,L}\),
	\(-(\mathcal R^{\mathrm{anom}})_{\mathrm{red}}\), and
	\(-\mathcal R^{\mathrm{loc}}\).  This includes Roos primitives
	\(K^{\mathrm{RG}}_{n,w,r,M;L_1,L_2}\) and
	\(K^{\mathrm{win}}_{n,w,r;M,M',L}\).  Applying
	\(d^{L_2}_{\mathrm{QME},n,M}\)
	to the first primitive gives the sum of
	\(-W(P_{L_1,L_2},C_{n,w,r,M,L_1})\) and \(C_{n,w,r,M,L_2}\).  Applying
	\(d^{L}_{\mathrm{QME},n,M}\) to the second primitive gives the sum of
	\(-\pi_{M,M'}C_{n,w,r,M,L}\) and
	\(C_{n,w,r,M',L}\pi^{\mathrm{CE}}_{M,M'}\), together with the analogous
admissible-weight transport primitive.  Then
		\(\mathrm{preOCA}_N\) lifts to the morphism-stage global
open--closed map.
If, in addition,
\(C_{\mathrm{OCA}}=\operatorname{Cone}(\mathrm{OCA}_N)\) is equipped with
a contraction \(\kappa_C\), with \(d_C=d_{\operatorname{Cone}}\), satisfying
\(d_C\kappa_C+\kappa_Cd_C=\operatorname{id}_{C_{\mathrm{OCA}}}\)
in the sense of Definition~\ref{defn:oca-comparison-cone}, or with the
filtered cone datum
\((F^\bullet C_{\mathrm{OCA}},\kappa_0,\widetilde\kappa_0,E)\) of
Theorem~\ref{thm:filtered-oca-cone-criterion}, with \(E\) raising
filtration and \(\sum_{n\ge0}E^n\) convergent, then \(\mathrm{OCA}_N\)
is a global open--closed quasi-isomorphism.  Under the non-optional
typed rows assumed above, this is the \(d=2\) target statement of the
Mixed-HT Deligne criterion on
	\(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) at \(N\) Dirac branes.
	If a topologised target is part of the comparison datum, the optional
	topologisation row \(\operatorname{Ob}^{\mathrm{top}}_\rho\) must also
	vanish.
	The datum consists of the mixed-HT Swiss-cheese action,
	Hochschild-centre lift, all-arity chiral \(E_2\) Morita-centre lift,
	Darboux-torsor transport, unreduced cotangent-current realization,
	brane-link descent, quantum master-equation counterterm tower,
	anomaly/locality primitives, and cone data named above.  A nonzero
	component, a missing primitive, a missing null-homotopy, or a
	non-acyclic comparison cone is the corresponding obstruction.
	The operator-level lift of \(\Phi_{10}\) or
	\(\Phi_{10}^{1/2}\) belongs to the \(K3\times E\) comparison problem.
\end{thm}

\begin{proof}
Theorem~\ref{thm:sc-ht-deligne-swiss-cheese} supplies the
\(\mathrm{SC}^{\mathrm{HT}}_{(1,2)}\)-algebra structures from the
mixed-HT Swiss-cheese Maurer--Cartan lift.  The Hochschild-centre
Maurer--Cartan equation is precisely the centrality condition of
Definition~\ref{defn:typed-hochschild-centre-lift-datum}.  The
Darboux-transport and cotangent-current equations kill the obstruction
classes of Definitions~\ref{defn:hol-bd-datum-HT} and
\ref{defn:typed-cotangent-current-lift-datum}.

Proposition~\ref{app:prop:morita-centre-lift-MC-control} identifies the
Morita mixed-HT \(E_2^{\mathrm{HT}}\)-centre lift with a relative Maurer--Cartan
obstruction.  Theorem~\ref{app:thm:relative-Ed-deligne} gives the
chiral \(E_d\) Deligne operation from the operadic action together with
the supplied data of
Definitions~\ref{app:hyp:formality}--\ref{app:hyp:ran-descent}.
Theorem~\ref{app:thm:global-stalk-from-collapse} gives
Costello--Gwilliam brane-link descent from
Definition~\ref{app:defn:descent-obstruction-complex}.
Theorem~\ref{app:thm:relative-qme} gives the all-order quantum
master-equation obstruction criterion from
...
analytic refinement.  The anomaly and locality equations are the two
remaining rows of Definition~\ref{defn:typed-anomaly-locality-datum}.

The displayed primitives and the comparison-unifying datum install every
non-optional row of \(\operatorname{Ob}^{\mathrm{typed}}\) except the cone
cohomology row, so the pre-comparison datum becomes a global open--closed
morphism.  The displayed contraction, or the filtered criterion, is
precisely the acyclicity condition for the comparison cone, hence the
quasi-isomorphism condition.  No cited criterion constructs the other
criteria's primitives from the formal-Darboux fibre.  The native tuple
becomes a single curvature class precisely through
\((\mathfrak K_{\mathrm{HT}},\chi_{\mathrm{nat}})\); without that row it
remains a tuple of native cohomology classes.
\end{proof}
...
$\hbar_{\mathrm{QME}}$-orders.  The Morita \(E_2\)-Deligne obstruction
...
obstruction class of Conjecture~\ref{conj:chiral-Ed-deligne};
  Faonte--Hennion--Kapranov
  \cite{faonte-hennion-kapranov-higher-km} and Francis
  \cite{francis-thesis} give adjacent factorization structures; the
...
Theorem~\ref{thm:formal-two-dimensional-holomorphic-singular-product} gives its
...
\begin{itemize}
  \item
    ...
    the Morita mixed-HT \(E_2^{\mathrm{HT}}\)-centre action
