...
the existence of a global Morita chiral \(E_2\)-centre equivalence whose
formal fibre at a chosen vacuum is this Hochschild chart, after the
all-arity operations, centre-lift homotopies, descent data, quantum
master-equation data, anomaly/locality null-homotopies, and
comparison-cone contraction data have been constructed.
...
packages have been fixed: the all-arity chiral \(E_2\) operations, the
Morita centre-lift Maurer--Cartan homotopies, the Weiss/Ran descent
cochains, the Costello quantum master-equation counterterm tower, the
anomaly and locality null-homotopies, and the comparison-cone
contraction.  Relative to this datum the morphism is required to be a
quasi-isomorphism; before these choices are made the conjecture is an
obstruction problem, not a distinguished map.  After choosing a dualisable
...
\end{enumerate}
Then the supplied local maps, descent primitives, QME counterterms,
anomaly/locality null-homotopies, and cone contraction assemble to a
projective open--closed quasi-isomorphism
\[
  \mathrm{OCA}_N\colon
...
\(\hbar N[\bar c]\).  The Stein hypotheses verify only the
...
quasi-isomorphism conclusion still requires \textup{(S1)--(S4)}.  The
punctured plane
...
\[
  \mathrm{Ob}_{\mathrm{ch}E_d}
   ...
   \bigr),
\]
where \(\mathrm{Ob}_{F}\) and \(\mathrm{Ob}_{R}\) are the obstructions
...
formal \(\hbar\)-adic local quantization: there exists a quantum BV
master action
...
\]
to all orders as a formal power series in \(\hbar\), and a quantum
trace-sector closed-to-open factorization-centre assignment
			\[
			  \Phi_N^\hbar\colon
			  \mathcal A^{\mathrm{q,tr}}_{\mathrm{bulk}}|_p
			   ...
			   (\mathrm{Obs}^\hbar_{\mathrm{open}})|_b
				\]
					...
					\(\Phi_N^{\mathrm{Hoch}}\).  No analytic convergence in
\(\hbar\), global descent, or whole finite-\(N\) Hochschild-centre
equivalence is asserted by this criterion.  After a
...
hypothesis this class vanishes and a compatible primitive is chosen;
induction constructs \(S^{(n)}\) for every \(n\).
