...
\[
  \widetilde J_N^s(f)=\Tr s(f)(\phi_1,\phi_2),
  \qquad
...
first Taylor datum extends through the displayed Lie, trace-action,
multiplication, and Jacobiator faces of the bracket-admissible
centrality filtration precisely when there is a continuous assignment
of the Hochschild cochains
\(\Theta^{(1)}_{f,N},M_{\widetilde J_N^s(f)}\) and homotopies
...
\[
  \begin{aligned}
    ...
    [\Theta^{(1)}_{f,N},M_{\widetilde J_N^s(g)}]_G
      &=M_{\widetilde J_N^s(\overline{\{s(f),s(g)\}})
              +N\omega(f,g)\mathbf1}
        -b_NH_{f|g},\\
    [M_{\widetilde J_N^s(f)},M_{\widetilde J_N^s(g)}]_G
...
For such a partial extension the obstruction component
...
This statement does not construct a full Hochschild-centre
Maurer--Cartan element.  The formality extension classes
\(\operatorname{Ob}_{\mathcal U,m+1}\), the PBW comparison cone,
and the Roos transition defects govern the construction of
\(\mathcal D_{\mathrm{form}}\).  The \(P_0\)-centrality, all higher
centrality components, brane-defect quantum master equation, and
centre-map Roos descent entries govern the subsequent finite-rank
lift.
\end{thm}
...
\item Algebraic counterterm realization: \(\tau=\sum\tau_i\) with each
  ...
  counterterm, by
  Theorem~\ref{thm:completion-milnor-obstruction-vector}),
  \(\tau_{\mathrm{cent}}\) (when the Hochschild equivariance
  primitives of
  Theorem~\ref{thm:completion-centrality-homotopies} exist).
...
\(\widetilde J_N^s(f)=\Tr s(f)(\phi_1,\phi_2)\) and
\(J_N^s(f)=[\widetilde J_N^s(f)]\) on \(\bar A\),
...
the derived-centre lift requires the supplied formality/PBW/Roos
datum, all higher centrality components, the displayed
Hochschild-equivariance primitives
...
\(M_{\widetilde J_N^s(f)}\), \(\Theta^{(1)}_{f,N}\), and \(T_\rho\),
governed
...
\(J_N^s(f)=[\widetilde J_N^s(f)]\), where
\(\widetilde J_N^s(f)=\Tr s(f)(\phi_1,\phi_2)\), and the
...
Hamiltonian derivations \(X_{f,N}\).  Their first Lie, trace-action,
multiplication, and Jacobiator extension faces obey the criterion of
Theorem~\ref{thm:completion-centrality-homotopies}; a derived-centre
map additionally requires the full Maurer--Cartan hierarchy,
\(P[1]\)-current component, and descent compatibilities.  Effective scalar
...
\item[Theorem~\ref{thm:completion-centrality-homotopies} (Hochschild
  ...
  bracket-admissible sector \(B_q\), the first Taylor datum extends
  through the displayed centrality faces exactly when their defects
  admit continuous compatible primitives.  The ordered pair
  \(z_1^2z_2,z_1z_2^2\) supplies one explicit
  \(H^{\mathrm{Lie}}\) and \(H^{\mathrm{act}}\); it does not supply a
  family for all Hamiltonians.  The unrestricted \(P[1]\)-current and
  higher faces require the separate data of
  Remark~\ref{rmk:completion-cotangent-centrality} and the subsequent
  obstruction rows.
