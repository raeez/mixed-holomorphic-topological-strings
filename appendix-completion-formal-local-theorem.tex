...
They measure separate lifts: radial Weyl compatibility,
...
satisfy the Hochschild action equations
\[
...
\begin{thm}[Graph weights and QME after counterterms]
...
Definition~\ref{def:completion-brane-defect-propagator}, assume a
brane-defect Costello locality theorem for this support stratum, a fixed
chain scalar projection, finite row arrays, graph
summability/wavefront admissibility, and
\(\mathfrak D^{\mathrm{ns}}_n=0\) for every order \(n\), with
compatible primitives.  Then the given
...
Theorem~\ref{thm:completion-wavefront-control}; and the
	...
	\]
	in the corrected residual complex.  Before the compatible primitives
	are inserted, the curvature is the residual non-scalar QME class
...
\begin{proof}
Local asymptotic expansion as \(\varepsilon\to 0\) is the given
brane-defect version of Costello's renormalization theorem
\cite{costello-renormalization}*{Thm.~6.1.1}; the expansion has only
finitely many non-renormalizable divergences in each loop order,
removable by local counterterms in
\(\mathcal O^{\mathrm{bd}}_{\mathrm{loc}}\) by the locality
hypothesis.  The
...
kernel decay (Theorem~\ref{thm:wt-canonical-envelope}, applied
to the brane-defect restriction).  Wavefront control is
Theorem~\ref{thm:completion-wavefront-control}.  The QME after
compatible counterterms is the standard Costello effective-action calculus
...
the curvature before the compatible primitive system is inserted.  The
kernel construction
...
quantization this remains governed by the renormalized
brane-defect propagator and compatible descent homotopies.
\end{thm}
...
propagator (Theorem~\ref{thm:completion-graph-qme}).  These data give
...
quantization the same statement requires the renormalized propagator
...
\begin{thm}[Large-\(N\) trace state]
\label{thm:completion-large-N}
The assembled datum \(\mathfrak U_{\mathcal T}\) does not by itself
construct a state \(\omega_{N,\Omega}\) or the limits
\(\kappa^\infty_k\).  A large-\(N\) sector consists of the additional
state of Definition~\ref{def:completion-large-N-state}, Ward and
cyclicity identities, a factorization condition, scaling exponents
\(\alpha,\beta(k)\), and convergence of the rescaled connected
cumulants.
\end{thm}
...
all-order QME compatibility, and the large-\(N\) state.
\end{thm}
...
system; and (vi) the large-\(N\) state and cumulant convergence data.
...
\item[Theorem~\ref{thm:completion-large-N} (large-\(N\) trace
  state).]  This theorem assumes a state, Ward identity, cyclicity,
  factorization, scaling exponents, and convergence of connected
  cumulants.  Köthe continuity and QME counterterms
...
\item[Theorem~\ref{thm:completion-universal-formal-brane} (closed-open
  ...
  windows, the all-order QME compatibility, and the large-\(N\) state.
