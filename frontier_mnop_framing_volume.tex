...
distinct generating functions.  At chain level, the DT, PT, and GW
...
Orientation and cohomological-Hall-algebra (CoHA) prerequisites enter
through Joyce--Upmeier and Kinjo--Park--Safronov.
...
converges.  The chiral-volume conjecture on the compact CY-to-chiral
target conjectures that the limit equals the Hodge-normalised
Abel--Jacobi period of a representative curve:
\[
...
chiral-volume limit lies above \(\{a_1,a_2,a_3\}\); those
...
the \(2\pi i\) monodromy factor must be supplied.  Bridging the two requires
...
\(L\approx|t(z_c)|\) is a normalisation check until \(\NAJ\) is supplied.
...
equivalence at \(K3\times E\).  On the smooth quintic the crepant
resolution input is vacuous, and the NCCR question degenerates to
...
Kuznetsov Fano HPD lacks the required non-trivial Lefschetz input.  The derived-equivalence statement
...
\begin{proof}
  ...
  hyperk\"ahler and the product with \(E\) leaves the required CY$_3\)
  form.
...
requires a non-trivial Lefschetz decomposition as input; here
...
\begin{rmk}[Distinct $\kappa$ values on $K3\times E$ and its anchors]
  ...
  \begin{itemize}
    ...
    \item \(\kappa_{\mathrm{ch}}^{\mathrm{Heis}}=3\) and
      ...
      \(\Delta_5\) row.
  \end{itemize}
  These values are pairwise distinct under
...
(Shenker; Mari\~no et al.).  Borel summation on the physical ray has a
Stokes obstruction; a non-perturbative completion is required.  Two candidates compete:
...
scope.  Write \(\{F_g^{\mathrm{BCOV}}(X,t)\}_{g\geq 0}\)
...
determined by the holomorphic anomaly recursion and boundary data; the
