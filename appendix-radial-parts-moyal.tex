...
vanishing has two proven forms.  The structural all-bidegree result is
Theorem~\ref{thm:app-radial-bicomplex-acyclic}, under the radial-parts
Statement~\ref{stmt:app-radial-parts-hypotheses}.  Independently of
that statement, Proposition~\ref{prop:app-radial-pr-unconditional-window}
gives exact rational vanishing on the verified finite window listed in
Remark~\ref{rmk:app-radial-finite-verification}.  Outside these two
...
and~\ref{lem:app-radial-dunkl-calogero-cancellation}) against the
	Levasseur--Stafford radial-kernel hypothesis.  A signed-row obstruction
...
Statement~\ref{stmt:app-radial-parts-hypotheses}; the structural
...
Remark~\ref{rmk:app-radial-finite-verification}.  An unlisted
	bidegree remains governed by the obstruction class
\(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\), or else
it depends on Statement~\ref{stmt:app-radial-parts-hypotheses}.  Reflection
...
\label{thm:app-radial-moyal-coefficients}
  ...
  inside the formal Weyl algebra of the constant symplectic plane,
	independent of the finite-\(N\) radial-parts hypothesis and Costello graph
  specialization.
\end{thm}
...
\label{stmt:app-radial-parts-hypotheses}
  ...
  \cite{harish-chandra-invariant-differential-operators}\cite{wallach-reductive-invariant-differential}\cite{levasseur-stafford-harish-chandra}\cite{levasseur-stafford-kernel-harish-chandra}.  After identifying
	\(\mathcal W_N\simeq\mathcal D_\hbar(\mathfrak{gl}_N)\), this theorem
...
\begin{lem}[Nonlinear shear obstruction to weaker radial-parts hypotheses]
...
\begin{lem}[Moment-ideal primitive form of the quantum shear hypothesis]
...
\label{thm:app-radial-bicomplex-acyclic}
  Assume Statement~\ref{stmt:app-radial-parts-hypotheses} in full.
...
\begin{proof}
  ...
  radial-parts hypothesis; Step~3 uses clause~(2) (Levasseur--Stafford
...
\label{thm:app-radial-finite-N}
  Under the radial-parts hypothesis of
...
\begin{proof}
  ...
  same radial-parts hypothesis makes the conjugated radial map injective
  ...
  statement hypotheses and by exact computation on the bidegrees of
...
\label{thm:app-radial-stable-connected}
  Under the same radial-parts hypothesis as
...
\label{rmk:app-radial-finite-verification}
  Exact rational linear algebra over the indexed-diagram basis proves
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) in the following windows:
  \[
  \begin{array}{c|l|l}
  \text{window} & \text{status} & \text{input} \\
  \hline
  a=2\ \text{or}\ b=2 & \text{proved} &
    \text{edge telescoping} \\
  a,b\ge3,\ a+b\le13 & \text{proved} &
    \text{exact rational reduction} \\
  a+b=14,\ \min(a,b)\le6 & \text{proved} &
    \text{exact rational reduction} \\
  a+b=15\ \text{listed subwindow} & \text{proved} &
    \text{exact rational reduction} \\
  (7,7) & \text{open in the finite computation} &
    \Omega^{\mathrm{rad}}_{7,7}\in\operatorname{coker}B_{7,7}
  \end{array}
  \]
  The reduction is the Procesi--Razmyslov kernel-correction route of
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
