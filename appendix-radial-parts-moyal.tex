...
\begin{rmk}[Reading via the obstruction--twist schema]\label{rmk:radial-route-Gschema}
  The per-bidegree class
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\) is
  the curvature of the transport problem from the strict Weyl normal
  form to the decorated PBW/Stokes normal form: under
...
\label{rmk:app-radial-finite-verification}
  ...
  exact rational-arithmetic verification of the algebraic coefficients
  used above.  It checks:
  \[
  \begin{gathered}
  ...
  The script
  \(\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}\)
  is separate from the finite-rank verification.  It enumerates the
  free
  ...
  \[
    ...
    (3,10),(10,3),(4,10),(10,4),(5,9),(9,5),(6,8),(8,6),
  \]
  \[
    (3,11),(11,3),(6,9),(9,6),(7,8),(8,7).
  \]
  The balanced total-degree-\(14\) case \((7,7)\) is the first
  undecided bidegree.
  ...
  extends the closure to every bidegree under the
...
at each bidegree \((a,b)\), to the question whether
\(v_{a,b}\) lies in \(\operatorname{im}B_{a,b}\) inside a
...
Every arithmetic step is exact rational.  Concrete data at each
bidegree---basis ordering \(\beta_{a,b}\), matrix entries of
\(B_{a,b}\), comparison vector \(v_{a,b}\), the integers
\(\operatorname{rank}B_{a,b}\), \(\dim\ker B_{a,b}^*\),
\(\dim\operatorname{coker}B_{a,b}\), and the correction
\(C^+_{a,b}\) when one exists---are read off the elimination
implemented in
\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}.

\begin{rmk}[Range of bidegrees treated]\label{rmk:app-radial-quantum-shear-script-verified-range}
  The exact rational reduction on the radial Stokes target
  \(B_{a,b}\) returns \(v_{a,b}\in\operatorname{im}B_{a,b}\) on the
  balanced diagonal through \((6,6)\), on every non-edge bidegree of
  total degree at most \(13\), and on the
  one-moving-letter edge strips
  \(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) of
  Proposition~\ref{prop:app-edge-pbw-telescoping}.  On this range
  the correction \(C^+_{a,b}\) is read off the elimination, and
  \(\dim\operatorname{coker}B_{a,b}\) is the codimension of the
  decorated PBW Stokes target inside its enveloping
  \((\mathsf W_{a,b}\oplus\mathsf S_{a,b})\)-side.  The
  Procesi--Razmyslov kernel-correction mode of
  Proposition~\ref{prop:app-radial-pr-unconditional-window}
  unconditionally extends \(\Omega^{\mathrm{rad}}_{a,b}=0\) to the
  larger list of Remark~\ref{rmk:app-radial-finite-verification},
  including the total-degree-\(14\) bidegrees
  \((6,8),(8,6)\); the balanced \((7,7)\) is the first undecided
  case.  The all-bidegree statement
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) is the obstruction problem of
  Proposition~\ref{prop:app-radial-dual-potential-obstruction}.
\end{rmk}
...
\begin{cor}[Vanishing on the listed range]\label{cor:app-radial-vanishing-on-listed-range}
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) on the listed bidegrees.  Outside
  the listed range, the per-bidegree class
...
\label{rmk:app-radial-parts-moyal-support-map}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Costello-QME instance.}
