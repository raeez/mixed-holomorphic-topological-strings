...
\begin{proof}
  ...
  \[
    E_{a,b,N}\in\mathcal W_N\widehat\mu(\lie{sl}_N).
  \]
  Lemma~\ref{lem:app-quantum-shear-primitive-obstruction} translates
  this membership to the existence of a moment-ideal primitive
  \(\sum A^j{}_i(a,b,N)\widehat\mu^i{}_j\), which equates with
  \(E_{a,b,N}\) inside \(\mathcal W_N\) for every \(N\).  Stable
  Procesi--Razmyslov injectivity for free indexed normal diagrams
  (Lemma~\ref{lem:app-free-trace-diagram-stable-injectivity}) lifts
  this all-\(N\) finite-rank identity to a free trace-diagram identity
  in bidegree \((a,b)\).  The free identity is exactly the
  decorated PBW Stokes equation
  \(D^\square_{a,b}H_{a,b}=R^{\mathrm{free}}_{a,b}\) with
  \(H_{a,b}\) reconstructed from the moment-ideal coefficients via
  the necklace square-cell decomposition of any cycle in
  \(\ker\partial\) (Lemma~\ref{lem:app-ordinary-necklace-stokes}).
  Hence \(\Omega^{\mathrm{rad}}_{a,b}=0\) and no signed dual row of
  the failure type can exist.
\end{proof}
