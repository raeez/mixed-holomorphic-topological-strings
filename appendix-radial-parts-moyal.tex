...
Closure status across the bidegree lattice is uniform on two
independent fronts.  The edge strips
\(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) close in closed form
...
interior bidegree under the named Statement-input.  Independently,
the Procesi--Razmyslov reduction
(Proposition~\ref{prop:app-radial-pr-unconditional-window}) closes
each bidegree unconditionally by finite rational linear algebra;
this verification has been carried out for every bidegree of total
degree \(\leq 15\), recorded in
...
\label{rmk:app-radial-finite-verification}
  ...
  named lower-degree interior cases, and total degrees \(13\)--\(15\):
  \[
...
\label{rmk:app-radial-parts-moyal-support-map}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Principle 6 (quantization is a measurement test).}
      ...
      Corollary~\ref{cor:app-radial-bicomplex-clause3-equivalence}
      promotes the conditionality to a logical equivalence: under
      clauses~(1) and~(2),
      \(\Omega^{\mathrm{rad}}_{a,b}=0\) is the same statement as
      clause~(3) modulo \(I_N\) for the Moyal-bracket polynomial of
      bidegree \((a,b)\).  This is the obstruction-theoretic content
