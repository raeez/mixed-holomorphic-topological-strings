...
\label{rmk:app-radial-open-bidegree-frontier}
  ...
  \(\{z_1^{a+1}/(a+1),z_2^{b+1}\}_\hbar\) at the chosen bidegree.
  Corollary~\ref{cor:app-radial-bicomplex-clause3-equivalence}
  sharpens this to a logical equivalence: under clauses~(1) and~(2),
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) holds if and only if
  clause~(3) holds modulo the moment ideal for that Moyal-bracket
  polynomial.  No signed dual row
  \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\) can exist under that input.
  The bidegree-local obstruction class is therefore concentrated, in
  a single named place, in clause~(3) for the Moyal-bracket
  polynomial of the chosen bidegree.
