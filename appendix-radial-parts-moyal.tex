...
class, in every bidegree.  Vanishing holds conditionally on the
named radial-parts Statement~\ref{stmt:app-radial-external-input}
by the radial-bicomplex acyclicity theorem
(Theorem~\ref{thm:app-radial-bicomplex-acyclic}), and
unconditionally on the explicit total-degree-\(15\)-and-below
window by stable Procesi--Razmyslov reduction
...
The structural mechanism is direct.  The cyclic-word incidence
complex \(\partial:\mathsf W_{a,b}\to\mathsf V_{a,b}\) is the
\(\hbar^0\) skeleton of the radial bicomplex; the classical lift
\(\partial c=T_{a,b}\) is the ordinary necklace Stokes
Lemma~\ref{lem:app-ordinary-necklace-stokes}.  The free indexed
...
\(R^{\mathrm{free}}_{a,b}\) lies entirely in this positive part.
Its vanishing as a single class \(\Omega^{\mathrm{rad}}_{a,b}\) is
the structural content of
Theorem~\ref{thm:app-radial-bicomplex-acyclic}, proved by testing
on the Cartan via
...
the explicit list of bidegrees verified in this mode reaches
through total degree \(15\) and is recorded in
...
\label{rmk:app-radial-open-bidegree-frontier}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{Procesi--Razmyslov unconditional verification.}
      ...
      The bidegrees enumerated in
      Remark~\ref{rmk:app-radial-finite-verification} (edges and
      interior cases reaching through total degree \(15\)) are
      individually closed by this route.
  \end{enumerate}
