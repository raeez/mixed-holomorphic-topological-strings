...
Levasseur--Stafford radial-kernel input.  A signed-row obstruction
...
radial-parts datum
\cite{harish-chandra-invariant-differential-operators}\cite{wallach-reductive-invariant-differential}\cite{levasseur-stafford-harish-chandra}\cite{levasseur-stafford-kernel-harish-chandra} is named in
...
\label{thm:app-radial-moyal-coefficients}
  ...
  independent of finite-\(N\) radial-parts input and Costello graph
  specialization.
\end{thm}
...
\label{cor:app-radial-all-odd-moyal-coefficients}
  The Weyl/Moyal commutator has only odd raw powers of \(\hbar\).  Its
...
\label{def:app-radial-matrix-weyl}
  ...
  removed by quotienting the empty trace before connected
  large-\(N\) extraction.
\end{defn}
...
\label{stmt:app-radial-external-input}
  ...
  Levasseur--Stafford
  \cite{harish-chandra-invariant-differential-operators}\cite{wallach-reductive-invariant-differential}\cite{levasseur-stafford-harish-chandra}\cite{levasseur-stafford-kernel-harish-chandra}.  After identifying
  ...
  used in the formal-local comparison.  The cited radial-parts sources
  supply the Harish-Chandra kernel theorem; the exact Weyl/Capelli
  convention enters here as an additional normalization.  The finite
  exact checks below test it in ranks \(2\) and \(3\); the all-\(N\)
  radial-parts input remains the structural hypothesis.
...
\begin{proof}
  ...
  Vandermonde-conjugated form absorbs the Calogero potential.
\end{proof}
...
\label{lem:app-radial-nonlinear-shear-obstruction}
  ...
  \]
  The difference \(2\hbar^2y\) is non-scalar.  Thus
  Statement~\ref{stmt:app-radial-external-input}(3) requires the quantum
...
\label{rmk:app-decorated-hodge-potential-form}
  ...
  Capelli term are visible.  A canonical inverse from an arbitrary
  positive diagram residual to a necklace edge cochain is extra data.
...
\begin{proof}
  ...
  \(-\hbar\Phi_r^{(b)}\) and \(+\hbar\Phi_{b-1-r}^{(b)}\).  These are
...
\begin{proof}
  ...
  rational enumerations, and separately re-checks the edge
...
\begin{proof}
  ...
  exact theorem target behind the full displayed formula: mixed Weyl
...
\begin{rmk}[Verified bidegree window]
\label{rmk:app-radial-finite-verification}
  Exact rational linear algebra over the indexed-diagram basis
  unconditionally proves \(\Omega^{\mathrm{rad}}_{a,b}=0\) on every
  edge strip, every non-edge interior bidegree of total degree
  \(\leq 13\), every total-degree-\(14\) bidegree with one component
  at most \(6\), and a partial total-degree-\(15\) sublist; the
  balanced \((7,7)\) is the first undecided case.  The reduction is
  the Procesi--Razmyslov kernel-correction route of
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
  all-bidegree closure is
...
\begin{cor}[Vanishing on the listed range]\label{cor:app-radial-vanishing-on-listed-range}
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) on the listed bidegrees.  For an
  unlisted bidegree, the per-bidegree class
...
\label{prop:app-moyal-capelli-realization-boundary}
  ...
  \begin{enumerate}
    ...
    \item The reduced open-line Wick graph computation realizes the same
      ...
      Costello graph/QME theorem additionally requires the brane-defect
...
\begin{proof}
  ...
  edge strips are
  Proposition~\ref{prop:app-edge-pbw-telescoping}, the unconditional
  finite window
...
\label{rmk:app-radial-open-bidegree-frontier}
  ...
  closes \(\Omega^{\mathrm{rad}}_{a,b}=0\) by exact rational
  linear algebra against the moment ideal, independently of
  Statement-input.  The bidegrees enumerated in
  ...
  mixed monomial of degree \(\leq a+b-2\) in the Capelli/Weyl
  convention is determined deterministically by the
  Levasseur--Stafford spherical-Hecke methods.
