...
The degree-zero quantum target is the scalar-reduced Weyl/Moyal algebra
with Capelli-renormalized matrix traces.  The reduced open-line Wick
model uses midpoint propagator
\(\frac12\operatorname{sgn}(t-s)\).  Given the finite-\(N\) radial image
datum of Statement~\ref{stmt:app-radial-external-input}, Vandermonde
conjugation removes the Dunkl--Calogero connection terms, radial
injectivity detects the reduced quotient, and the stable primitive
projection extracts the single-trace Moyal bracket.
