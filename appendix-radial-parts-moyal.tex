...
\label{thm:app-radial-all-bidegree-vanishing-preview}
  For every bidegree \((a,b)\) with \(a,b\geq 1\),
  ...
  \]
  has a solution \(H_{a,b}\in\mathsf S_{a,b}\).  The mechanism is the
  contracting homotopy of
  Theorem~\ref{thm:app-radial-bicomplex-acyclic} on the radial
  bicomplex, executed by the PBW \(\hbar\)-filtration spectral
  sequence whose \(E_1\)-page is the ordinary necklace Stokes complex.
\end{thm}

The structural mechanism is direct.  The free indexed normal-diagram
target carries an \(\hbar\)-filtration by contraction degree: the
\(\hbar^0\) part is the ordinary cyclic-word incidence complex, and
each PBW correction increases contraction degree by exactly one.  The
ordinary necklace Stokes lemma at \(\hbar^0\)
(Lemma~\ref{lem:app-ordinary-necklace-stokes}) supplies a contracting
homotopy on the cycle complex; PBW second-order moves at
\(\hbar^{p+1}\) lift the homotopy from \(\hbar^p\) by exactly the
square-cell map \(\partial_2\), and at every level the lifted homotopy
remains a chain map.  This is the radial bicomplex acyclicity
theorem.  Vanishing of the bidegree-local cokernel class
\(\Omega^{\mathrm{rad}}_{a,b}\) is its immediate corollary.  No
finite-dimensional certificate is required, and a genuine signed-row
failure \((\phi,-\lambda)\in\ker B_{a,b}^*\) is structurally
excluded.
...
Statement~\ref{stmt:app-radial-external-input}, retained because the
mixed-monomial image clause is independently load-bearing for
Theorem~\ref{thm:app-radial-finite-N}; the all-bidegree vanishing of
\(\Omega^{\mathrm{rad}}_{a,b}\) is proved here without using that
clause.  The reduced open-line Wick graph realization with midpoint
propagator \(\tfrac12\operatorname{sgn}(t-s)\) realizes the same
Moyal product to all orders and is the bridge to the open-line BV
sector.
