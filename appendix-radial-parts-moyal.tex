...
\subsection*{Computer verification}\label{ssec:app-radial-reproducibility-protocol}
...
required by \((a,b)\).  The script
\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}
performs Bareiss elimination over \(\Q\) and returns either the unique
correction \(C^+_{a,b}\) with \(B_{a,b}C^+_{a,b}=v_{a,b}\) (verdict
...
arithmetic step is exact rational; no floating-point step enters.

\begin{ex}[The first nonzero balanced cyclic-pivot residual]\label{ex:app-radial-reproducibility-certificate}
...
\begin{cor}[Verdict table on the verified range]\label{cor:app-radial-verdict-table}
