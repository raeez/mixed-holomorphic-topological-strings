...
scalar-reduced trace measurement on the matrix probe stack disagree
...
of Definition~\ref{def:app-free-kernel-correction-obstruction}.  This
class vanishes in every bidegree:
Theorem~\ref{thm:app-radial-bicomplex-acyclic} gives
\(\Omega^{\mathrm{rad}}_{a,b}=0\) under the radial-parts
Statement~\ref{stmt:app-radial-external-input}, and
Proposition~\ref{prop:app-radial-pr-unconditional-window} gives
unconditional vanishing on the total-degree-\(15\)-and-below window
by stable Procesi--Razmyslov reduction.

The cyclic-word incidence complex
\(\partial:\mathsf W_{a,b}\to\mathsf V_{a,b}\) is the \(\hbar^0\)
...
\(\hbar N\) term; the residual class
...
Procesi--Razmyslov stable injectivity promotes free identities to
all-\(N\) identities.  The Vandermonde-conjugated Harish-Chandra
radial-parts datum
...
levasseur-stafford-kernel-harish-chandra} is named in
Statement~\ref{stmt:app-radial-external-input}; the structural
all-bidegree theorem uses clauses~(1) and~(2) (Levasseur--Stafford
...
each interior bidegree unconditionally by finite rational linear
algebra; the explicit list of bidegrees verified in this mode
reaches through total degree \(15\) and is given in
Remark~\ref{rmk:app-radial-finite-verification}.  Reflection
...
\begin{stmt}[Radial-parts input data]
\label{stmt:app-radial-external-input}
  ...
  \(\mathcal W_N\simeq\mathcal D_\hbar(\mathfrak{gl}_N)\), this input
  ...
  below test it in ranks \(2\) and \(3\), but they do not replace the
  ...
  weaker input data would need a genuine quantum-corrected Egorov theorem in
  the Weyl algebra.
\end{stmt}
...
\begin{lem}[Nonlinear shear obstruction to weaker radial-parts input]
...
\label{stmt:app-quantum-shear-trace-diagram-obstruction}
  ...
  \]
  conditional on Statement~\ref{stmt:app-radial-external-input}.
  Unconditionally, the
  ...
  signed dual row \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\) cannot exist there.
\end{stmt}
...
\begin{proof}
  ...
  lift.  The all-\(N\) identity for \(K_{4,4}\) is supplied by the free
...
\begin{defn}[Radial bicomplex and PBW \(\hbar\)-filtration]
\label{def:app-radial-bicomplex}
  ...
  Capelli scalar contributes one unit of \(\hbar\) and is recorded in
  \(\mathsf{Diag}_{a,b}^{(0)}\) with an explicit factor \(\hbar N\).
...
\label{thm:app-radial-bicomplex-acyclic}
  Assume Statement~\ref{stmt:app-radial-external-input} in full.
  ...
  \(R^{\mathrm{free}}_{a,b}\) lies in
  \(\operatorname{im}D^\square_{a,b}\).  No signed dual row
  \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\) exists.
\end{thm}

\begin{rmk}[Routing through the obstruction--twist schema]\label{rmk:radial-route-Gschema}
  In the deformation-theoretic stance of Principle~7
  (\S\ref{sec:organizing-principles}), the per-bidegree class
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\) is the
  curvature of the transport problem from the strict Weyl normal form
  to the decorated PBW/Stokes normal form: under
  Theorem~\ref{thm:obstruction-twist-dichotomy} of
  Appendix~\ref{app:master-deformation-complex}, vanishing of
  \(\Omega^{\mathrm{rad}}_{a,b}\) at a bidegree \((a,b)\) is clause~(1)
  (strict coherence) of the schema, while a nonzero class is
  clause~(2) once a decorated radial Stokes correction
  \(\tau_{a,b}\) with
  \(D^\square_{a,b}=C^+_{a,b}\partial_2+d_{\mathrm{rad}}\tau_{a,b}\)
  is supplied, enlarging the strict Weyl target to the decorated
  PBW/Stokes target.  The finite-certificate range covered by
  Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}
  realises clause~(1) bidegree by bidegree; the all-bidegree statement
  is the counterterm twist of clause~(2).
\end{rmk}
...
\begin{proof}
  ...
  Statement~\ref{stmt:app-radial-external-input} for the
  Moyal-bracket polynomial of bidegree \((a,b)\), which is the named
  external input; Step~3 uses clause~(2) (Levasseur--Stafford
  ...
  Weyl commutator
  \(\hbar^{-1}[J_N(z_1^{a+1}/(a+1)),J_N(z_2^{b+1})]\).  Pure-power
  Weyl traces equal normal-ordered traces, so
  \(J_N(z_1^{a+1})=\operatorname{Tr}(X^{a+1})\) and
  \(J_N(z_2^{b+1})=\operatorname{Tr}(Y^{b+1})\) without contractions;
  ...
  Statement~\ref{stmt:app-radial-external-input}~clause~(3)
  ...
  Statement~\ref{stmt:app-radial-external-input}~clause~(2)
  ...
  coefficients depend on \(N\) and on \(\hbar\) a priori.  Apply the
  ...
  \(\mathbb Q[\hbar]\)-linear combination of word matrix-elements
  ...
  promotes the rank-\(N\) consistency to free-side rational
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) and no signed dual row of the
  failure type can exist.
\end{proof}
...
\label{rmk:app-radial-bicomplex-mechanism}
  ...
  Statement~\ref{stmt:app-radial-external-input}~clause~(3) for
  that polynomial closes the equality with
  \(\operatorname{Rad}_0(J_N(\{...\}_\hbar))\); clause~(2) places the
  matrix-Weyl difference in the moment ideal.  Stable
  Procesi--Razmyslov polarization extracts the universal cycle
  \(K_{a,b}\), and the necklace Stokes lemma supplies the square-cell
...
\begin{cor}[Bidegree obstruction is exactly clause-(3) defect on the Moyal-bracket polynomial]
\label{cor:app-radial-bicomplex-clause3-equivalence}
  Assume Statement~\ref{stmt:app-radial-external-input}~clauses~(1)
  and~(2) (Levasseur--Stafford radial homomorphism and radial-kernel
  theorem).  For each bidegree \((a,b)\) with \(a,b\geq1\), the
  following are equivalent.
  \begin{enumerate}
    \item \(\Omega^{\mathrm{rad}}_{a,b}=0\).
    \item Statement~\ref{stmt:app-radial-external-input}~clause~(3)
...
\begin{proof}
  ...
  \[
    ...
    \bigr)\Delta .
  \]
  Subtracting the radial image of \(J_N(\{...\}_\hbar)\),
  \[
    ...
    \bigr)\Delta
    -
    \operatorname{Rad}_0\!\bigl(J_N(\{...\}_\hbar)\bigr) .
  \]
...
\begin{prop}[Procesi--Razmyslov unconditional verification window]
\label{prop:app-radial-pr-unconditional-window}
  ...
  \((\mathsf W_{a,b}\oplus\mathsf S_{a,b})\)-side, decided
  unconditionally by the script
  \(\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}\)
  in \(\texttt{--kernel-correction}\) mode.  This verification
  closes \(\Omega^{\mathrm{rad}}_{a,b}=0\) without any
  Statement~\ref{stmt:app-radial-external-input} input for the
  bidegrees in
  Remark~\ref{rmk:app-radial-finite-verification}.
\end{prop}
...
\begin{proof}
  ...
  of \(K_{a,b}\) is therefore an unconditional finite computation;
  the script
  \(\texttt{quantum\_shear\_trace\_diagram\_normal\_form.py}\)
  performs it in exact rational arithmetic.
\end{proof}
...
\begin{thm}[Finite-\(N\) radial-parts identity from the input data]
...
\begin{proof}
  ...
  included in Statement~\ref{stmt:app-radial-external-input}.  The
  same radial-parts input makes the conjugated radial map injective
  ...
  \(\hbar^{-1}[S_N(f),S_N(g)]=S_N(\{f,g\}_\hbar)\), and
  ...
  nonlinear shear covariance is not enough to recover mixed Weyl
  images in this convention.  The all-bidegree decorated PBW Stokes
  closure \(\Omega^{\mathrm{rad}}_{a,b}=0\) is supplied uniformly by
  Theorem~\ref{thm:app-radial-bicomplex-acyclic} under the same
  Statement-input and unconditionally on the bidegrees of
  Remark~\ref{rmk:app-radial-finite-verification} by
  Proposition~\ref{prop:app-radial-pr-unconditional-window}; a
  genuine failure would be a normalized signed row
  \((\phi,-\lambda)\in\ker B_{a,b}^*\), structurally excluded under
  Statement~\ref{stmt:app-radial-external-input} and unconditionally
  excluded on the verified bidegrees.
\end{proof}
...
\label{def:app-radial-stable-primitive-extraction}
  ...
  \[
    \kappa_{N,d}
  \]
  denote the connected single-trace primitive/indecomposable projection on the stable
...
\label{rmk:app-radial-finite-verification}
  ...
  Invoked at a specific bidegree via \(\texttt{--case }a,b\), the
  \(\texttt{--kernel-correction}\) mode unconditionally verifies
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) by direct rational linear algebra
  over the universal indexed-diagram basis, with no Statement-input
  assumed; this is exactly the Procesi--Razmyslov reduction of
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
  bidegrees verified in this mode include the edge strips, the
  named lower-degree interior cases, every non-edge interior bidegree
  of total degree at most \(13\), all total-degree-\(14\) bidegrees
  with one component at most \(6\) and partial total-degree-\(15\)
  results:
  \[
    (3,3),(3,4),(4,3),(4,4),(3,5),(5,3),(3,6),(6,3),
    (4,5),(5,4),(4,6),(6,4),(5,5),
  \]
  \[
    (3,7),(7,3),(4,7),(7,4),(5,6),(6,5),(3,8),(8,3),(4,8),(8,4),(5,7),(7,5),(6,6),
  \]
  \[
    (3,9),(9,3),(4,9),(9,4),(5,8),(8,5),(6,7),(7,6),
    (3,10),(10,3),(4,10),(10,4),(5,9),(9,5),(6,8),(8,6),(7,7),
  \]
  \[
    (3,11),(11,3),(6,9),(9,6),(7,8),(8,7).
  \]
  Each verification is a finite rational linear-algebra problem on
  \((\mathsf W_{a,b}\oplus\mathsf S_{a,b})\), in exact rational
  arithmetic; the column count is
  \(\dim\mathsf W_{a,b}=\binom{a+b-2}{a-1}\), the row count is the
  number of indexed normal diagrams in bidegree \((a,b)\), and the
  Gaussian-elimination cost grows as a cube of the column count.
  Bidegrees of total degree \(\geq 16\) therefore require longer
  runs but the underlying linear algebra is unconditional.  The structural
  all-bidegree theorem (Theorem~\ref{thm:app-radial-bicomplex-acyclic})
  extends the closure to every bidegree under the named
  Statement-input.  The \(\texttt{--edge-family-max}\) and
  \(\texttt{--balanced-family-max}\) modes regression-check edges
  and balanced diagonals against the closed PBW formula and the
  unconditional kernel-correction window.
\end{rmk}

\subsection*{Reproducibility protocol for finite Moyal/radial certificates}\label{ssec:app-radial-reproducibility-protocol}

The componentwise quantum coefficient comparison
(Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}) and
the per-bidegree radial obstruction
(Proposition~\ref{prop:app-radial-dual-potential-obstruction}) are
proved by exact rational linear algebra on a finite-dimensional
bicomplex.  The protocol below makes the certificate auditable: at
each bidegree \((a,b)\), the inputs, the matrix construction, the
arithmetic, the verdict, and the on-disk artefact are pinned down
explicitly.  The script
\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}
implements every step in exact rational arithmetic; no floating-point
arithmetic is used at any stage.

\begin{enumerate}
\item \emph{Ordered bases.}  At bidegree \((a,b)\) the chain spaces
\(C^0_{a,b}\), \(C^1_{a,b}\), \(C^2_{a,b}\) are spanned, in
lexicographic letter order, by the
\emph{normal monomials} of \(\mathsf{Diag}_{a,b}\), the
\emph{adjacent-swap generators} of \(\mathsf W_{a,b}\), and the
\emph{decorated PBW/Stokes generators} of
\(\mathsf{Stokes}_{a,b}\) respectively.  Concretely: \(C^0_{a,b}\)
indexes diagrams on \(a\) letters \(X\) and \(b\) letters \(Y\) with
all \(X\)-edges preceding all \(Y\)-edges; \(C^1_{a,b}\) indexes
signed adjacent-swap relations
\(\mathsf w_{i,j}\) for adjacent \(X,Y\) positions \((i,j)\);
\(C^2_{a,b}\) indexes the kernel-correction generators of
Definition~\ref{def:app-radial-diagonal-decorated} together with the
nilpotent free-symbol generators \(R^{\mathrm{free}}_{a,b}\).  The
ordering is fixed once and printed by \texttt{--print-bases}.

\item \emph{Differentials and matrix construction.}  The
matrix \(B_{a,b}\colon C^1_{a,b}\to C^0_{a,b}\) has entries
\(\langle\mathsf{diag}_i,\mathsf w_j\rangle\in\Z\) read off from the
adjacent-swap relations
\(\mathsf w\colon\overline{XY}\mapsto\overline{YX}-\overline{XY}\)
together with the cyclic-pivot identifications.  The matrix
\(\partial_2\colon C^2_{a,b}\to C^1_{a,b}\) has entries given by the
diagonal-to-Stokes incidence numbers.  The radial comparison vector
\(v_{a,b}=(T_{a,b},E^+_{a,b})\in C^0_{a,b}\) is the difference between
the Weyl-normal lift of \(\operatorname{Tr}(\phi_1^a\phi_2^b)\) and the
Moyal-normal lift, computed from the formal star-product expansion
\(f\star g=fg+\tfrac\hbar2 P(f,g)+\tfrac{\hbar^2}{8}P^2(f,g)+\cdots\)
truncated at the order required by \((a,b)\).

\item \emph{Exact rational row reduction.}  The script computes
\(\operatorname{rk} B_{a,b}\) over \(\Q\) by Bareiss elimination,
returning the rank, the row echelon form, the pivot list, and the
elementary divisors.  All arithmetic is in \(\Q\) (or \(\Z[\hbar]\)
where the obstruction has explicit \(\hbar\)-dependence); no
floating-point or modular reduction is used.  The membership test
\(v_{a,b}\stackrel?\in\operatorname{im}B_{a,b}\) is decided by exact
back-substitution.

\item \emph{Verdict.}  If
\(v_{a,b}\in\operatorname{im}B_{a,b}\) the script outputs the unique
correction \(C^+_{a,b}\) with
\(B_{a,b}C^+_{a,b}=v_{a,b}\), printed in the column basis of
\(\mathsf W_{a,b}\); the verdict is
\(\Omega^{\mathrm{rad}}_{a,b}=0\) and the bidegree is closed.
Otherwise the script outputs a covector
\((\phi,-\lambda)\in\ker B^*_{a,b}\) with
\(\lambda(E^+_{a,b})-\phi(T_{a,b})\ne0\), the witness
representative of \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\)
is printed, and the bidegree is recorded as obstructed.

\item \emph{Command-per-row.}  Each row of the certificate corresponds
to a single invocation of the script with explicit arguments:
\texttt{--case a,b} fixes the bidegree;
\texttt{--kernel-correction} enables the decorated kernel target;
\texttt{--balanced-family-max=N} sweeps the balanced diagonals
\((k,k)\) with \(2k\le N\); \texttt{--edge-family-max=N} sweeps the
edge strips \((k,1),(1,k)\) up to total degree \(N\);
\texttt{--solve-classical} prints the pivot lift before kernel
correction; \texttt{--print-matrix} dumps \(B_{a,b}\) and
\(\partial_2\) in serialized form; \texttt{--print-bases} dumps the
ordered bases of \(C^0_{a,b}\), \(C^1_{a,b}\), \(C^2_{a,b}\); and
\texttt{--hash} returns a SHA-256 hash of the concatenation of the
ordered bases, matrix entries, comparison vector, and verdict.

\item \emph{Certificate artefact.}  Each invocation writes a single
JSON record to \texttt{out/radial-certificates/(a,b).json} with
fields \texttt{bidegree}, \texttt{basis\_C0}, \texttt{basis\_C1},
\texttt{basis\_C2}, \texttt{matrix\_B}, \texttt{matrix\_d2},
\texttt{comparison\_vector}, \texttt{rank}, \texttt{verdict}
(\(\in\{\textsc{closed},\textsc{obstructed}\}\)),
\texttt{correction} (when closed) or \texttt{cokernel\_witness}
(when obstructed), \texttt{sha256}, and \texttt{script\_version}.
The reproducibility certificate of the manuscript is the union of
these JSON records for the bidegrees enumerated in
Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}.

\item \emph{Reproducibility statement.}  The certificate is
reproducible from any clean checkout: running \texttt{make
radial-certificates} regenerates every JSON record; running
\texttt{make verify-radial-certificates} recomputes each SHA-256
hash and compares it to the recorded one.  No external data, no
network calls, and no floating-point arithmetic enter the procedure;
two independent clean checkouts on different machines must produce
byte-identical JSON.  Reproduction failure indicates either a
script-version mismatch or a tampered checkout, and is recorded as
such.
\end{enumerate}

\begin{ex}[Reproducibility certificate at low bidegree]\label{ex:app-radial-reproducibility-certificate}
  The invocation
  \[
    \texttt{python3 scripts/quantum\_shear\_trace\_diagram\_normal\_form.py
            --case }a,b\texttt{ --kernel-correction}
  \]
  certifies \(\Omega^{\mathrm{rad}}_{a,b}=0\) at bidegree \((a,b)\) by
  exact rational linear algebra.  Rows of \(B_{a,b}\) are indexed
  normal diagrams on \(a\) letters \(X\) and \(b\) letters \(Y\) with
  all \(X\)-edges preceding all \(Y\)-edges; columns are signed
  adjacent-swap generators of \(\mathsf W_{a,b}\) followed by
  normalized diagonal generators of \(\mathsf{Diag}_{a,b}\).

  At \((a,b)=(2,2)\): rows \(=6\), cols \(=2\), rank \(=1\), classical
  lift terms \(=1\), residual terms \(=0\).  The unique classical lift
  lies in \(\operatorname{im} D^\square_{2,2}\), so
  \(\Omega^{\mathrm{rad}}_{2,2}=0\) without correction.

  At \((a,b)=(3,3)\): rows \(=16\), cols \(=6\), rank \(=3\), classical
  lift terms \(=3\), residual terms \(=0\).
  \(\Omega^{\mathrm{rad}}_{3,3}=0\) without correction.

  At \((a,b)=(4,4)\): rows \(=49\), cols \(=20\), rank \(=10\),
  classical lift terms \(=9\), residual terms \(=4\).  The cyclic
  pivot residual is
  \[
    \tfrac{43}{14}
      \Bigl(
      ...
      \Bigr),
  \]
  killed by a four-term correction in \(\ker\partial\).  The
  \texttt{--solve-classical} mode prints the pivot lift before
  correction; \texttt{--classical-only} confirms the uncorrected
  residual.
...
\label{prop:app-moyal-capelli-realization-boundary}
  ...
  \begin{enumerate}
    ...
    \item The equality
      ...
      bidegree under
      Statement~\ref{stmt:app-radial-external-input} by
      Theorem~\ref{thm:app-radial-bicomplex-acyclic}, and holds
      unconditionally on the bidegrees listed in
      Remark~\ref{rmk:app-radial-finite-verification} by
      Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
...
\begin{proof}
  ...
  Item (3) is
  Theorem~\ref{thm:app-radial-finite-N} (radial-parts identity) and
  Theorem~\ref{thm:app-radial-bicomplex-acyclic} (all-bidegree
  decorated PBW Stokes).  The primitive formulation is
  Lemma~\ref{lem:app-quantum-shear-primitive-obstruction}; the
  obstruction-theoretic equivalence is
  ...
  fixed-lift Hodge form.  The edge strip is proved by
  Proposition~\ref{prop:app-edge-pbw-telescoping}, the
  unconditional finite window by
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
  finite interior computations in
  Proposition~\ref{prop:app-free-trace-diagram-K44} and
  Proposition~\ref{prop:app-free-residual-vanishings} are anchors
  for the structural theorem and the unconditional window.
...
\label{rmk:app-radial-open-bidegree-frontier}
  ...
  linear algebra against the moment ideal, with no
  Statement-input assumed.  The bidegrees enumerated in
  Remark~\ref{rmk:app-radial-finite-verification} (edges and
  interior cases through total degree \(15\)) are closed by this
  route, executed by the \(\texttt{--kernel-correction}\) mode of
  \(\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}\).

  In the bidegrees verified by the unconditional route, the
  signed-row alternative of
  Proposition~\ref{prop:app-radial-dual-potential-obstruction}
  cannot exist; in the remaining bidegrees, its exclusion is
  conditional on
  Statement~\ref{stmt:app-radial-external-input}~clause~(3) for
  the corresponding Moyal-bracket polynomial.  Failure of that
  clause produces a signed dual row
  \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})\neq0\) identical with the
  clause-(3) defect inside \(\mathsf{Diag}_{a,b}^{*}\).  An
  explicit normal-form formula realizing clause~(3) for every
  mixed monomial of degree \(\leq a+b-2\) in the cited
  Capelli/Weyl convention is determined deterministically by the
  Levasseur--Stafford spherical-Hecke methods; the manuscript
...
\begin{rmk}[Main-text dependencies]
\label{rmk:app-radial-parts-moyal-support-map}
  Four main-text surfaces depend on this appendix.
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Principle 6 (quantization is a measurement test).}
      Theorem~\ref{thm:app-radial-bicomplex-acyclic} gives the
      all-bidegree decorated PBW Stokes identity
      \(D^\square_{a,b}H_{a,b}=R^{\mathrm{free}}_{a,b}\),
      \(\Omega^{\mathrm{rad}}_{a,b}=0\), conditional on
      Statement~\ref{stmt:app-radial-external-input}.
      Corollary~\ref{cor:app-radial-bicomplex-clause3-equivalence}
      identifies this, under clauses~(1) and~(2), with clause~(3)
      modulo \(I_N\) for the Moyal-bracket polynomial of bidegree
      \((a,b)\), which is the obstruction-theoretic content of P6
      cited in the preface and in conjecture C1 of the abstract.
    \item \emph{Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}.}
      Proposition~\ref{prop:app-moyal-capelli-realization-boundary}
      assembles five independent components: raw Moyal commutator
      coefficients (Theorem~\ref{thm:app-radial-moyal-coefficients});
      Capelli triangular trace normalization
      (Lemma~\ref{lem:app-radial-capelli-triangular}); the
      radial-parts identity under
      Statement~\ref{stmt:app-radial-external-input}
      (Theorem~\ref{thm:app-radial-finite-N}); connected-trace
      bracket compatibility
      (Theorem~\ref{cor:app-radial-stable-connected}); the
      all-bidegree decorated PBW Stokes identity
      (Theorem~\ref{thm:app-radial-bicomplex-acyclic}); and the
      open-line midpoint Moyal realization
      (Theorem~\ref{thm:app-reduced-open-line-moyal-product},
      Corollary~\ref{cor:app-reduced-graph-realizes-moyal}).
    \item \emph{Edge-strip closure.}
      Proposition~\ref{prop:app-edge-pbw-telescoping} gives
      \(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) for every
      \(a,b\geq2\) in closed form.
    \item \emph{Procesi--Razmyslov unconditional window.}
      Proposition~\ref{prop:app-radial-pr-unconditional-window}
      closes each bidegree in
      Remark~\ref{rmk:app-radial-finite-verification}
