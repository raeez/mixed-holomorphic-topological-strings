...
\begin{proof}
  ...
  \emph{Step 3 (Moment-ideal lift).}  By
  ...
  \[
    ...
    (\text{all }N\geq1).
  \]
  By Lemma~\ref{lem:app-quantum-shear-primitive-obstruction} this
  membership is equivalent to the existence of coefficients
  \(A^j{}_i(a,b,N)\in\mathcal W_N\) with
  \(E_{a,b,N}=\sum A^j{}_i\widehat\mu^i{}_j\).  After choosing a
  free-word ansatz
  \(A^j{}_i(a,b,N)=\sum_{W} c_W(a,b)\,W^j{}_i\) where \(W\) ranges
  over words of bidegree \((a-1,b-1)\), the membership becomes
  \[
    E_{a,b,N}=\sum_{W\in\mathsf W_{a,b}}c_W\operatorname{Tr}(WM)
    \qquad(\text{all }N\geq1),
  \]
  with rational coefficients \(c_W=c_W(a,b)\) independent of \(N\).
  Existence of such \(N\)-independent coefficients is supplied by the
  free indexed normal-diagram polarization
  argument: the matrix-trace identity in
  \(\mathcal W_N\) holds for every \(N\), and stable
  Procesi--Razmyslov injectivity
  (Lemma~\ref{lem:app-free-trace-diagram-stable-injectivity}) extracts
  rational coefficients on the universal free indexed normal-diagram
  side.
  ...
  \(c=\sum_W c_W W\in\mathsf W_{a,b}\) is a classical lift,
