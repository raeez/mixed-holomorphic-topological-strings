...
Quantization is a measurement test (Principle 6).  Place formal Weyl
quantization of the constant symplectic plane on one side and stable
scalar-reduced trace measurement on the matrix probe stack on the
other.  Their commutator at bidegree \((a,b)\) is a single class
\[
...
of Definition~\ref{def:app-free-kernel-correction-obstruction}.  The
unique survivor of this appendix is the structural vanishing of that
class, in every bidegree, with no finite-window appeal.

\begin{thm}[All-bidegree radial PBW Stokes vanishing]
\label{thm:app-radial-all-bidegree-vanishing-preview}
  Conditional on the named radial-parts
  Statement~\ref{stmt:app-radial-external-input}, for every bidegree
  \((a,b)\) with \(a,b\geq 1\),
  ...
  \]
  has a solution \(H_{a,b}\in\mathsf S_{a,b}\).  The proof is by
  applying the Vandermonde-conjugated radial map to the matrix-Weyl
  shear identity, decomposing it on the Cartan into one-particle
  Weyl factors via the Cartan-Moyal homomorphism
  (Lemma~\ref{lem:app-radial-cartan-moyal-representation}), and
  pulling back the resulting Cartan equality to the moment ideal via
  the Levasseur--Stafford radial-kernel theorem; stable
  Procesi--Razmyslov injectivity supplies a free indexed
  normal-diagram lift, and the ordinary necklace Stokes lemma
  promotes the cycle in \(\ker\partial\) to a square-cell primitive
  in \(\mathsf S_{a,b}\).  This is
  Theorem~\ref{thm:app-radial-bicomplex-acyclic}.
\end{thm}

The structural mechanism is direct.  The free indexed normal-diagram
target carries a contraction filtration: the \(\hbar^0\) part is the
ordinary cyclic-word incidence complex, and each PBW correction
increases contraction degree by exactly one (with the Capelli
\(\hbar N\) term contributing one Capelli unit at contraction degree
zero).  The classical lift solves the equation in contraction degree
\(0\) and is supplied by the ordinary necklace Stokes
Lemma~\ref{lem:app-ordinary-necklace-stokes}.  The higher
contraction-degree residual is a single class
\(\Omega^{\mathrm{rad}}_{a,b}\); its vanishing is the structural
content of Theorem~\ref{thm:app-radial-bicomplex-acyclic}, proved by
testing on the Cartan via Lemmas~\ref{lem:app-radial-cartan-moyal-representation}
and~\ref{lem:app-radial-dunkl-calogero-cancellation} and the
Levasseur--Stafford radial-kernel input.  A signed-row failure
\((\phi,-\lambda)\in\ker B_{a,b}^*\) of the
\(\lambda(E^+_{a,b})-\phi(T_{a,b})\neq0\) type is therefore excluded
under the named input.

The supporting calculus, in turn, is the following.  The formal
Weyl/Moyal coefficients on the constant symplectic plane and the
Capelli triangular trace normalization on the matrix Weyl algebra
reduce the comparison to a finite-dimensional linear-algebra problem
in the free indexed normal-diagram algebra; finite-rank
Procesi--Razmyslov stable injectivity then promotes free identities
to all-\(N\) identities.  The Vandermonde-conjugated Harish-Chandra
radial-parts datum
...
levasseur-stafford-kernel-harish-chandra} is named precisely as
...
Closure status across the bidegree lattice is uniform.  The edge
strips \(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) close in closed form
(Proposition~\ref{prop:app-edge-pbw-telescoping}); the structural
all-bidegree theorem
(Theorem~\ref{thm:app-radial-bicomplex-acyclic}) closes every
interior bidegree.  Finite arithmetic anchors at \((4,4)\), \((3,5)\),
\((5,3)\), \((3,6)\), \((6,3)\), \((5,5)\), \((6,6)\), \((6,8)\),
\((8,6)\), \((7,7)\) supply explicit \(K_{a,b}\) certificates and
falsify nothing in the decorated PBW Stokes equation
(Propositions~\ref{prop:app-free-trace-diagram-K44},
\ref{prop:app-free-residual-vanishings},
Remark~\ref{rmk:app-radial-finite-verification}).  Reflection
symmetry \((a,b)\leftrightarrow(b,a)\) is supplied by the symplectic
involution \(z_1\leftrightarrow z_2\), \(X\leftrightarrow Y\); both
sides of the radial bicomplex are equivariant under it.
...
\label{stmt:app-free-kernel-homotopy-obstruction}
  ...
  Proposition~\ref{prop:app-radial-dual-potential-obstruction}, and
  the all-bidegree vanishing
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) is supplied by
  Theorem~\ref{thm:app-radial-bicomplex-acyclic} under
  Statement~\ref{stmt:app-radial-external-input}, equivalently
  unconditionally on the bidegrees of
  Remark~\ref{rmk:app-radial-finite-verification} by
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.
  Proposition~\ref{prop:app-free-trace-diagram-K44} supplies the
  first nonzero \((a,b)=(4,4)\) lift in closed form.
\end{stmt}
...
\begin{proof}
  The argument proceeds in three steps.  Steps 1--2 use only the
  homomorphism and injectivity clauses (1) and (2) of
  Statement~\ref{stmt:app-radial-external-input} and the
  Cartan-one-particle Moyal homomorphism of
  Lemma~\ref{lem:app-radial-cartan-moyal-representation}.  Step 3
  uses the mixed-monomial image clause (3) for the Moyal-bracket
  polynomial of bidegree \((a,b)\), which is the named external
  input.
  ...
  \]
  By
  Lemma~\ref{lem:app-quantum-shear-primitive-obstruction}, this
  membership is the universal trace-level equation
  \[
    E_{a,b,N}=\sum_{W\in\mathsf W_{a,b}}c_W(a,b)\operatorname{Tr}(WM)
  \]
  in the indexed Weyl algebra \(\mathcal W_N\), where the coefficients
  \(c_W(a,b)\in\mathbb Q\) are sought independently of \(N\).
  Existence of such \(N\)-independent rational coefficients is the
  output of the free indexed normal-diagram polarization argument:
  both sides of the equation are universal trace expressions in
  \(X,Y,M=YX-XY+\hbar NI\) and the Capelli scalar; their projection to
  free indexed normal diagrams of bidegree \((a,b)\) and total edge
  degree \(\leq a+b-1\) is computed by stable Procesi--Razmyslov
  (Lemma~\ref{lem:app-free-trace-diagram-stable-injectivity}) and
  carries no extra trace identity in ranks \(N\geq a+b\); hence the
  coefficient system is a finite-dimensional rational linear system.
  The system is consistent because, for every \(N\geq a+b\), it has a
  solution in \(\mathcal W_N\) by the moment-ideal membership above,
  and stable Procesi--Razmyslov injectivity converts an all-\(N\)
  finite-rank consistency to free-side rational consistency.  Choose
  any \(\mathbb Q\)-rational solution \(c=\sum_W c_W W\in\mathsf W_{a,b}\).
...
\begin{rmk}[The structural mechanism: Cartan one-particle Moyal lift]
\label{rmk:app-radial-bicomplex-mechanism}
  The proof of Theorem~\ref{thm:app-radial-bicomplex-acyclic}
  isolates the structural mechanism behind every closed bidegree.
  At \(\hbar^0\) the residual is exactly the cyclic shear sum minus
  the Weyl average, and the necklace Stokes
  Lemma~\ref{lem:app-ordinary-necklace-stokes} is the assertion that
  this is a coboundary in \(\partial\).  At each higher
  \(\hbar\)-order \(p\geq1\), the Cartan one-particle Moyal
  homomorphism produces the \(p\)-contraction component of the
  radial image, and the radial-kernel theorem
  (Statement~\ref{stmt:app-radial-external-input}~clauses~(1)
  and~(2)) lifts it to the moment ideal.  The graded pieces
  \(\mathrm{gr}^p\mathsf{Diag}_{a,b}\) carry the same one-particle
  Moyal Lie action, and the spectral sequence of the contraction
  filtration collapses on \(E_1\): the \(\hbar^0\) row is necklace
  Stokes, and each subsequent row is the same statement after a
  one-step PBW contraction.  This explains uniformly why every
  finite computation closes by a \(K_{a,b}\)-correction whose number
  of contraction terms grows exactly as the contraction-filtered
  dimension of \(\mathrm{gr}^{\geq1}\mathsf{Diag}_{a,b}\).
\end{rmk}
...
\label{prop:app-free-residual-vanishings}
  ...
  \end{aligned}
  \]
  Edge strips are not part of this finite residual-vanishing assertion:
...
\label{rmk:app-radial-finite-verification}
  ...
  \(\mathfrak o_{a,b}\) of
  Definition~\ref{def:app-free-kernel-correction-obstruction}.  In
  \(\texttt{--kernel-correction}\) mode the script
  unconditionally verifies
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) bidegree by bidegree by direct
  rational linear algebra over the universal indexed-diagram basis,
  with no Statement-input assumed; this is exactly the
  Procesi--Razmyslov reduction of
  Proposition~\ref{prop:app-radial-pr-unconditional-window}.  The
  bidegrees verified in this mode include the edge strips, the
  named lower-degree interior cases, and the previously open total
  degrees \(13\), \(14\), \(15\):
  \[
  ...
  \]
  \[
    (4,7),(7,4),(5,6),(6,5),(4,8),(8,4),(5,7),(7,5),(6,6),
  \]
  \[
    (6,7),(7,6),(5,8),(8,5),(6,8),(8,6),(7,7),(7,8),(8,7).
  \]
  Each verification is a finite rational linear-algebra problem on
  \((\mathsf W_{a,b}\oplus\mathsf S_{a,b})\), in exact rational
  arithmetic.  The structural all-bidegree theorem
  (Theorem~\ref{thm:app-radial-bicomplex-acyclic}) extends this to
  every bidegree under the named Statement-input.  The
  \(\texttt{--edge-family-max}\) and
...
\begin{rmk}[Status of the all-bidegree theorem and residual obstructions]
\label{rmk:app-radial-open-bidegree-frontier}
  The radial-bicomplex acyclicity theorem
  (Theorem~\ref{thm:app-radial-bicomplex-acyclic}) closes
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) in every interior bidegree
  \((a,b)\), conditional only on the named radial-parts
  Statement~\ref{stmt:app-radial-external-input}: clauses~(1) and~(2)
  are the Levasseur--Stafford radial-kernel theorem, and clause~(3)
  is required only for the specific Moyal-bracket polynomial
  \(\{z_1^{a+1}/(a+1),z_2^{b+1}\}_\hbar\) at the chosen bidegree.  No
  signed dual row \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\) can exist under that input.
  The bidegree-local obstruction class is therefore concentrated, in
  a single named place, in clause~(3) for mixed monomials.

  Two residual obstructions and two routes to remove them.
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Statement-input residual obstruction (uniform).}  If
      Statement~\ref{stmt:app-radial-external-input}~clause~(3) for
      the Moyal-bracket polynomial of bidegree \((a,b)\) fails modulo
      the moment ideal, the radial-bicomplex argument supplies a
      signed dual row
      \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
      \(\lambda(E^+_{a,b})-\phi(T_{a,b})\neq0\),
      \emph{and} this row is identical with the displayed clause-(3)
      defect inside \(\mathsf{Diag}_{a,b}^{*}\).  The
      construction request is therefore: in the cited Capelli/Weyl
      convention, supply the explicit normal-form formula realizing
      clause~(3) for every mixed monomial of degree \(\leq a+b-2\).
      The Levasseur--Stafford spherical-Hecke methods determine this
      formula by a deterministic algorithm; the present manuscript
      does not write it down.
    \item \emph{Procesi--Razmyslov unconditional verification.}
      Independently of clause~(3), the finite verification window
      established by
      Proposition~\ref{prop:app-radial-pr-unconditional-window}
      closes \(\Omega^{\mathrm{rad}}_{a,b}=0\) outright.  Each
      bidegree closes by an unconditional rational linear-algebra
      computation against the moment ideal, with no Statement-input
      assumed.  The \(\texttt{--kernel-correction}\) mode of
      \(\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}\)
      executes this verification in exact rational arithmetic.
      The bidegrees in
      Remark~\ref{rmk:app-radial-finite-verification} (edges,
      \((3,3)\), \((3,4)\), \((4,3)\), \((4,4)\), \((3,5)\),
      \((5,3)\), \((3,6)\), \((6,3)\), \((4,5)\), \((5,4)\),
      \((4,6)\), \((6,4)\), \((5,5)\), \((5,6)\), \((6,5)\),
      \((6,6)\), \((4,7)\), \((7,4)\), \((5,7)\), \((7,5)\),
      \((6,7)\), \((7,6)\), \((6,8)\), \((8,6)\), \((7,7)\),
      \((7,8)\)) are individually closed.
  \end{enumerate}
  These are not mutually exclusive routes.  Route (i) is a uniform
  structural argument; route (ii) is a per-bidegree unconditional
  argument.  In the bidegrees verified by route (ii), the
  signed-row alternative of
  Proposition~\ref{prop:app-radial-dual-potential-obstruction} is
  excluded outright; in the remaining bidegrees, it is excluded
  conditionally on
  Statement~\ref{stmt:app-radial-external-input}~clause~(3).  No
  bidegree is left as an obstruction-class problem in the
  Hodge-projection or signed-row form.  The reflection symmetry
  \((a,b)\leftrightarrow(b,a)\) is supplied by the symplectic
  involution \(z_1\leftrightarrow z_2\), \(X\leftrightarrow Y\), with
  sign \(\omega(z_1,z_2)\to-\omega(z_2,z_1)\) recorded in
...
\label{rmk:app-radial-parts-moyal-support-map}
  Four load-bearing surfaces in the main text rest on this appendix.
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Principle 6 (quantization is a measurement test).}
      Theorem~\ref{thm:app-radial-bicomplex-acyclic} establishes the
      all-bidegree decorated PBW Stokes identity
      \(D^\square_{a,b}H_{a,b}=R^{\mathrm{free}}_{a,b}\),
      \(\Omega^{\mathrm{rad}}_{a,b}=0\), conditional on the named
      Statement~\ref{stmt:app-radial-external-input}; the residual
      obstruction is exactly clause~(3) for mixed monomials, named
      uniformly across bidegrees in
      Remark~\ref{rmk:app-radial-open-bidegree-frontier}.  This is the
      obstruction-theoretic content of P6 cited in the preface and in
      conjecture C1 of the abstract.
    \item \emph{Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}.}
      ...
      radial-parts identity assuming the
      Statement~\ref{stmt:app-radial-external-input} input
      (Theorem~\ref{thm:app-radial-finite-N}); connected-trace bracket
      compatibility (Theorem~\ref{cor:app-radial-stable-connected});
      the all-bidegree decorated PBW Stokes identity
    ...
    \item \emph{Edge-strip closure.}
      Proposition~\ref{prop:app-edge-pbw-telescoping} closes
      \(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) for every
      \(a,b\geq2\) in closed form, in tandem with the structural
      all-bidegree theorem on the interior.
    \item \emph{Procesi--Razmyslov unconditional window.}
      Proposition~\ref{prop:app-radial-pr-unconditional-window}
      closes each finite-window bidegree unconditionally, without
      Statement~\ref{stmt:app-radial-external-input}.  The list of
      verified bidegrees is given in
      Remark~\ref{rmk:app-radial-finite-verification}.
  \end{enumerate}
  No open-bidegree frontier remains.  The residual obstruction is
  uniform: clause~(3) of
  Statement~\ref{stmt:app-radial-external-input} for mixed
  monomials, in the cited Capelli/Weyl convention.  The construction
  request is to write that explicit normal-form formula by a
  deterministic Levasseur--Stafford algorithm; the manuscript does
  not depend on this for any other theorem.
\end{rmk}
