...
\begin{rmk}[Routing through the obstruction--twist schema]\label{rmk:radial-route-Gschema}
  ...
  PBW/Stokes target.  The verified range covered by
...
\subsection*{Exact rational reduction}\label{ssec:app-radial-reproducibility-protocol}
...
(Proposition~\ref{prop:app-radial-dual-potential-obstruction}) reduce,
at each bidegree \((a,b)\), to a membership test
\(v_{a,b}\stackrel?\in\operatorname{im}B_{a,b}\) in a
finite-dimensional \(\Q\)-module.  The Stokes boundary
\(B_{a,b}\colon C^1_{a,b}\to C^0_{a,b}\) is built from the
adjacent-swap relation
\(\mathsf w\colon\overline{XY}\mapsto\overline{YX}-\overline{XY}\) and
its cyclic pivots; the comparison vector \(v_{a,b}=(T_{a,b},E^+_{a,b})\)
is the difference between the Weyl-normal and Moyal-normal lifts of
\(\operatorname{Tr}(\phi_1^a\phi_2^b)\), truncated at the order
required by \((a,b)\).  Bareiss elimination over \(\Q\) returns either
the unique correction \(C^+_{a,b}\) with
\(B_{a,b}C^+_{a,b}=v_{a,b}\) (giving
\(\Omega^{\mathrm{rad}}_{a,b}=0\)) or a covector
\((\phi,-\lambda)\in\ker B^*_{a,b}\) with
\(\lambda(E^+_{a,b})-\phi(T_{a,b})\ne0\) (representing
\(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\)).  Every
arithmetic step is exact rational.  The script
\texttt{scripts/quantum\_shear\_trace\_diagram\_normal\_form.py}
exposes the per-bidegree data: basis ordering of
\(\beta_{a,b}\) on \(\mathsf W_{a,b}\oplus\mathsf S_{a,b}\), the
matrix entries of \(B_{a,b}\), the comparison vector \(v_{a,b}\), the
row-reduced echelon form, the rank \(\operatorname{rank}B_{a,b}\),
\(\dim\ker B^*_{a,b}\), \(\dim\operatorname{coker}B_{a,b}\), and the
correction \(C^+_{a,b}\) when membership holds.

\begin{rmk}[Verified range]\label{rmk:app-radial-quantum-shear-script-verified-range}
  The exact rational reduction returns
  \(v_{a,b}\in\operatorname{im}B_{a,b}\) for the following bidegrees:
  the balanced diagonal through \((6,6)\); all non-edge bidegrees of
  total degree at most \(13\); the total-degree-\(14\) cases
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\); and the
  one-moving-letter edge strips
  \(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) of
  Proposition~\ref{prop:app-edge-pbw-telescoping}.  The first
  undecided total-degree-\(14\) systems are \((6,8),(8,6),(7,7)\).
  Within the verified range the unique correction \(C^+_{a,b}\) is
  output by the row reduction, and
  \(\dim\operatorname{coker}B_{a,b}\) records the codimension of the
  decorated PBW Stokes target inside its enveloping
  \((\mathsf W_{a,b}\oplus\mathsf S_{a,b})\)-side.  Outside the
  verified range the row reduction is the same algorithm and would
  return either a correction or an obstruction covector; the
  all-bidegree statement \(\Omega^{\mathrm{rad}}_{a,b}=0\) is the
  obstruction problem of
  Proposition~\ref{prop:app-radial-dual-potential-obstruction}.
\end{rmk}
...
\begin{ex}[The first nonzero balanced cyclic-pivot residual]\label{ex:app-radial-first-balanced-residual}
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) without correction.  The first
...
\begin{thm}[Per-bidegree exact rational reduction]\label{thm:app-radial-bidegree-rational-reduction}
  ...
  Bareiss elimination over \(\Q\) decides
  \(v_{a,b}\stackrel?\in\operatorname{im}B_{a,b}\) in exact rational
  arithmetic and outputs the dual data
  \[
    \operatorname{rank}B_{a,b},\quad
    \dim\ker B^*_{a,b},\quad
    \dim\operatorname{coker}B_{a,b}.
  \]
  When \(v_{a,b}\in\operatorname{im}B_{a,b}\), the elimination returns
  the unique correction
  \(C^+_{a,b}\in\Q^{r_1}\) with
  \(B_{a,b}C^+_{a,b}=v_{a,b}\), giving
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) at that bidegree.  When
  \(v_{a,b}\notin\operatorname{im}B_{a,b}\), the elimination returns
  the obstruction covector
  \((\phi,-\lambda)\in\ker B^*_{a,b}\) with the strict pairing
  ...
  same row reduction repeated on the same basis ordering returns the
  same matrix entries to the last digit, and the row-reduction trace
  is reproducible bit-for-bit.
\end{thm}

\begin{cor}[Per-bidegree row-reduction table on the verified range]\label{cor:app-radial-verdict-table}
  For every bidegree \((a,b)\) listed in
  Remark~\ref{rmk:app-radial-quantum-shear-script-verified-range},
  the rational reduction of
  Theorem~\ref{thm:app-radial-bidegree-rational-reduction} returns
  \(v_{a,b}\in\operatorname{im}B_{a,b}\): the comparison vector lies in
  the image of the Stokes boundary, with explicit correction
  \(C^+_{a,b}\) and the dual data
  \(\dim\ker B^*_{a,b},\,\dim\operatorname{coker}B_{a,b}\) read off from
  the row reduction.  In particular
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) on the verified range.  Outside the
  verified range, the per-bidegree class
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\) is the
...
\label{rmk:app-radial-parts-moyal-support-map}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Principle 5, Costello-QME instance (quantization is a measurement test).}
