...
Quantization is a measurement test (Principle 6).  Take the formal
Weyl quantization of the symplectic disk on one side and stable
scalar-reduced trace measurement on the matrix probe stack on the
other.  At each bidegree \((a,b)\), the obstruction to their
commuting is a single class
\[
...
\]
defined on the necklace word / cyclic word / free indexed
normal-diagram triple of
Definition~\ref{def:app-free-kernel-correction-obstruction}.
Vanishing \(\Omega^{\mathrm{rad}}_{a,b}=0\) is equivalent to the
...
\]
and a genuine failure is a normalized signed row
\((\phi,-\lambda)\in\ker B_{a,b}^*\) with
\(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\)
(Proposition~\ref{prop:app-radial-dual-potential-obstruction}).  This
is the unique survivor of the appendix.

The calculus that makes that single statement honest is the
following.  The formal Weyl/Moyal coefficients on the constant
symplectic plane and the Capelli triangular trace normalization on
the matrix Weyl algebra reduce the comparison to a finite-dimensional
linear-algebra problem in the free indexed normal-diagram algebra,
where finite-rank Procesi--Razmyslov stable injectivity makes
all-\(N\) identities visible window by window.  The
Vandermonde-conjugated Harish-Chandra radial-parts datum
...
Statement~\ref{stmt:app-radial-external-input}, with the missing
clause and its nonlinear-shear obstruction marked.  The decorated
square-cell PBW Stokes complex resolves the kernel-correction
problem into the equation \(D^\square_{a,b}H_{a,b}=R^{\mathrm{free}}_{a,b}\).
The reduced open-line Wick graph realization with midpoint propagator
\(\tfrac12\operatorname{sgn}(t-s)\) realizes the same Moyal product
to all orders.

Closure status across the bidegree lattice.  The edge strips
\(\mathfrak o_{2,b}=\mathfrak o_{a,2}=0\) close in closed form
(Proposition~\ref{prop:app-edge-pbw-telescoping}).  The interior
bidegrees \((4,4)\), \((3,5)\), \((5,3)\), \((3,6)\), \((6,3)\)
close, by the displayed kernel correction or by zero residual
(Propositions~\ref{prop:app-free-trace-diagram-K44},
\ref{prop:app-free-residual-vanishings}).  The first undecided
bidegrees are total degree \(14\): \((6,8)\), \((8,6)\), \((7,7)\).
Reflection symmetry \((a,b)\leftrightarrow(b,a)\) reduces the open
frontier to \((6,8)\) and \((7,7)\).  At each bidegree
Proposition~\ref{prop:app-radial-dual-potential-obstruction} demands
exactly one of two named finite-dimensional data: an explicit Stokes
primitive \(H_{a,b}\), or a normalized signed row
\((\phi,\lambda)\in\ker B_{a,b}^*\) with
\(\lambda(E^+_{a,b})-\phi(T_{a,b})=1\); see
Remark~\ref{rmk:app-radial-open-bidegree-frontier}.
