...
\noindent
This paper proves the mixed holomorphic--topological (mixed-HT) Deligne
theorem at \(N\) Dirac branes in
\(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\): the closed bulk
factorisation algebra \(\mathcal A^{\mathrm{cl}/\hbar}_{\mathrm{bulk}}\)
is the Morita mixed-HT chiral \(E_2\)-centre
\(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
(\mathcal C^{\mathrm{op}}_\partial)\) of the boundary open factorisation
category, a projective open--closed quasi-isomorphism with curvature the
Capelli class \(\hbar N[\bar c]\).  An \(N\)-Dirac-brane stack at
...
of the Chan--Paton matrices.  The closed-to-open comparison is the
...
\]
the centre-lift Maurer--Cartan element whose residual obstruction
vanishes by the strict Hochschild identities and whose positive-weight
part is contracted by the brane-preserving \(\Omega\)-background
localization; it globalises unconditionally to every Stein
holomorphic-symplectic surface by Cartan \v{C}ech--Dolbeault descent.
The
