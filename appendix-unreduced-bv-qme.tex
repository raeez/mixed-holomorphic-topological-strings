...
\begin{proof}
  ...
  Sufficiency.  Conversely, if all four components vanish, the
  candidate \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\) extends to a
  strict factorization-center morphism: (ii) supplies the centrality
  homotopy, (iii) supplies the QME counterterms via the supplied
  finite row machinery, (iv) makes the morphism descent-compatible,
  and (i) is the graded commutativity certificate on the unreduced
  target.  The obstructions are necessary and sufficient for the
