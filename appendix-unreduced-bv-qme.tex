...
\label{thm:app-theta-three-finite-window-H1}
  ...
  The script \texttt{scripts/finite\_window\_graph\_array.py} returns
  the same outputs.  Case (a) corresponds to the rows
  \texttt{theta\_3\_current\_finite\_row\_subcomplex},
  the omitted-CE-ancestor payload, and
  the omitted-counterterm payload, each returning
  \(\texttt{primitive\_exists}=\mathrm{false}\) with cokernel covector
  \(\ell(\mathsf E_{\theta_3,M})=1\).  Case (b) corresponds to
  three repair data --- the supplied scalar-zero counterterm
  \texttt{theta3\_counterterm\_supplied\_exact\_payload} returning
  \(\texttt{primitive\_coefficients}=[\![C_{\theta_3,M}^{\mathrm{ct}},1]\!]\),
  the supplied CE ancestor
  \texttt{theta3\_ce\_ancestor\_supplied\_exact\_payload} returning
  \(\texttt{primitive\_coefficients}=[\![C_{\theta_3,M}^{\mathrm{CE}},1]\!]\),
  and the supplied complete companion-face table
  \texttt{theta3\_complete\_companion\_faces\_supplied\_exact\_payload}
  --- each returning
  \(\texttt{primitive\_exists}=\mathrm{true}\) with the corresponding
  unit primitive coefficient.
\end{thm}
