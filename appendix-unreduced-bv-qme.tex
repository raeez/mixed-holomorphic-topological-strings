...
Stronger global claims about the brane-defect quantum master equation
are obstruction problems in finite-row, finite-window form (Principle
7).  Once a filtered chain scalar projection
\(\widehat\sigma_{\mathrm{sc}}\) on the local-functional complex
\(\mathfrak Q^\bullet_{w,\partial,\hbar}(I)\) is supplied, the
non-scalar Costello QME on a brane interval reduces to a single
cohomology class
\[
...
\(\varprojlim^1H^0\)-primitive-compatibility class.  This is the
unique survivor of the appendix.

The scalar anomaly is the visible footprint of one cohomology class
seen four ways.  In the slogan: \emph{constants vanish as Hamiltonian
vector fields but survive as brane center-of-mass charge.}  The
algebraic central extension of \(\bar A=\C[z_1,z_2]/\C\cdot1\), the
Lie central extension \(\widehat{\mathfrak h}_{\mathrm{cent}}\), the
trace-algebraic boundary central extension, and the quantum Weyl
commutator \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\) are
isomorphic as classes in \(H^2(\bar A;\C)[[\hbar]]\); the cocycle
\(\omega(f,g)=[\{f,g\}]_0\) is the same in each
(Theorem~\ref{thm:app-scalar-anomaly-transgression}).  The four-branch
ledger of cancellations --- ordinary \(\lie{gl}_N\) obstruction by
\(\hbar N[\bar c]\), unreduced central character, balanced
\(\C^{N|N}\) supertrace, and full-equivariant marker tensors --- is
the content of the appendix's branch geometry, and is rigid: any
modification of one branch displaces the same class.

At each finite window \(M\), the obstruction is the linear-algebra
problem
\[
  A^M c^M=-r^M,
\]
of Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}.
The rows of \(A^M\) and the residual \(r^M\) are exactly the four
named row types
\[
...
\]
together with the order-\(n\) homogeneous lower-counterterm rows.
Solvability is the existence of a degree-zero counterterm \(c^M\) in
the supplied row span.  Nonsolvability is a separating covector
\(\ell A^M=0\), \(\ell(r^M)\neq0\).  Tower compatibility adds the
truncation matrices \(T^{M,N}\) on rows and \(P^{M,N}\) on primitive
candidates and the Roos class
\([P^{M,N}c^M-c^N]\in H^0(\mathcal K^\bullet_{\mathrm{ns},N}(I))\).
A heuristic timeout on a finite window is not a resolution.

\paragraph{First exposed larger non-scalar source-face row.}
...
\[
  ...
  S_{\theta_3,M}=0,
\]
and in the supplied one-row complex
...
A named coefficient \(C_{\theta_3}\) is evidence only with one of
three named data: a CE ancestor with controlled boundary; a
scalar-zero Costello local counterterm \(C_{\theta_3,M}\) with
...
vector into the image of \(A^M\).  Tower compatibility passes through
...
Without one of these data, the order-three larger system is not a
solved QME row.

The local geometry is fixed once.  Every formula below uses the
formal-local mixed habitat
\(\R^2_{\mathrm{top}}\times\widehat{\C^2}_0\) with
\(\Omega_{\mathrm{loc}}=dz_1\wedge dz_2\).  Five formal-local facts
separate the branches.  Finite polynomial boundary observables
cannot carry the principal-part module
(Theorem~\ref{thm:app-unreduced-polynomial-centrality-no-go}); the
distributional current principal-part target carries a strict
algebraic \(P_0\) presentation with zero homotopies inside the
algebraic current model
(Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies});
the ordinary scalar-reduced \(\lie{gl}_N\) trace model has nonzero
central CE class \(\hbar N[\bar c]\)
(Theorem~\ref{thm:app-scalar-contact-qme-class}); the unreduced
central-character source absorbs that scalar symbol before scalar
reduction
(Theorem~\ref{thm:app-central-character-qme-cancellation}); and the
balanced \(\C^{N|N}\) supertrace model makes the scalar
representative vanish
(Theorem~\ref{thm:app-balanced-supertrace-qme-cancellation}).
The transgression theorem
(Theorem~\ref{thm:app-scalar-anomaly-transgression}) collects these
into one diagram.  The first non-scalar row that survives the scalar
shadow is the order-three source-face row \(\theta_3\); its
finite-window \(H^1\) is computed explicitly with both verdicts
\([\theta_3]\neq0\) and \([\theta_3]=0\) recorded as theorems
(Theorem~\ref{thm:app-theta-three-finite-window-H1}).  The unreduced
factorization-center cotangent lift is governed by a four-component
obstruction vector, and lifts if and only if that vector vanishes
...
\label{prop:app-local-geometry-scalar-shadow}
  ...
  shadow.  The Hamiltonian scalar cocycle entering every calculation
  below is
  \[
    \omega(f,g)
      =
...
\label{thm:app-scalar-anomaly-transgression}
  In the formal-local mixed habitat
  \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_0\) with
  ...
  produces the Capelli shift \(\hbar N[\bar c]\).
\end{thm}
...
\begin{proof}
  ...
  \(\eta\) does not descend to \(\bar A\).  Hence \([\omega]\) is
  ...
  open Chan--Paton trace.  The slogan summarizes the algebraic
  content: \(X_f(1)=0\) for every \(f\) (the constant line is
  annihilated by every Hamiltonian vector field), while the trace
  pairing \(\operatorname{Tr}(\mathbf 1)=N\) records the constant line
  as the open index loop charge \(\hbar N\).  The four sightings are
  the four ways the same class \([\bar c]\) becomes visible after
  pairing with the open trace, the source CE differential, the Lie
  bracket, or the boundary Weyl commutator.
\end{proof}

\begin{rmk}[Cancellation as transgression branches]
\label{rmk:app-scalar-anomaly-transgression-branches}
  The four-branch cancellation ledger of
  Theorems~\ref{thm:app-central-character-qme-cancellation},
  \ref{thm:app-balanced-supertrace-qme-cancellation},
  \ref{thm:app-supertraceless-monodromy-projection},
  and~\ref{thm:app-full-equivariant-marked-shadow-vanishing} is
  exactly the action of the four sightings on the same class
  \(\hbar N[\bar c]\):
  \begin{itemize}
    \item the \emph{unreduced central character} branch absorbs the
      class through (i): the cocycle is the CE coboundary of
      \(\hbar N\eta\) on the unreduced source, and
      \(d_{\mathrm{CE}}(\hbar N\eta)=\hbar N\omega\) leaves no
      surviving class once \(\eta\) is admitted as a degree-zero
      element of the unreduced Hamiltonian algebra
      (Theorem~\ref{thm:app-central-character-qme-cancellation});
    \item the \emph{balanced \(\C^{N|N}\) supertrace} branch absorbs
      the class through (iii)--(iv): the open trace coefficient is
      replaced by \(\operatorname{sdim}\C^{N|N}=0\), and the cocycle
      is rescaled to zero before its CE class is formed
      (Theorem~\ref{thm:app-balanced-supertrace-qme-cancellation});
    \item the \emph{supertraceless-monodromy} branch is the same
      computation with cyclic supertrace zero on every monodromy word,
      a direct extension of (iv) to marked open loops
      (Theorem~\ref{thm:app-supertraceless-monodromy-projection});
    \item the \emph{full \(\lie{gl}(V)\)-equivariant} branch is the
      same calculation on signed Brauer marker tensors, which all have
      cyclic supertrace zero on \(\C^{N|N}\)
      (Lemma~\ref{lem:app-full-equivariant-cyclic-supertrace} and
      Theorem~\ref{thm:app-full-equivariant-marked-shadow-vanishing}).
  \end{itemize}
  Conversely, the obstruction theorem of
  Theorem~\ref{thm:app-ordinary-glN-scalar-obstruction-criterion}
  shows that any internal local counterterm for the ordinary
  scalar-reduced \(\lie{gl}_N\) branch must contribute a leading
  scalar class \(-\hbar N[\bar c]\); since \([\bar c]\neq0\), no
  scalar-reduced internal counterterm exists, and only the four
  external branch replacements above can absorb the class.
\end{rmk}
...
\label{thm:app-supertraceless-monodromy-projection}
  ...
  create a non-supertraceless marked word.  Preservation therefore holds
  on the unmarked Hamiltonian generator set; it is not asserted for a
  larger Costello specialization with additional marked idempotents,
  holonomies, or counterterm tensors.
\end{thm}
...
\begin{proof}
  ...
  unmarked classical interaction; it introduces no external even
  endomorphism marker on an open scalar loop.
...
\label{prop:app-marked-bst0-equivariant-differential}
  ...
  the missing datum blocking a scalar computation is exactly
...
\begin{proof}
  ...
  The final assertion records only the preceding factorization.  The only
  displayed \(d_{\mathrm{QME}}\) in the current
...
\begin{proof}
  ...
  exact missing datum.  The finite-window system requires compatibility of
...
\label{def:app-native-finite-window-realization-habitat}
  ...
  system.  No graph theorem for arbitrary \(B\in\mathcal D'_c(I)\) is
  ...
  \[
    ...
    \ker\widehat\sigma_{\mathrm{sc},M}.
  \]
  The notation \(\mathcal K_{\mathrm{ns},M}\) reserves a separate symbol
  for the non-scalar kernel, distinct from the ambient QME complex
  \(\mathfrak Q_{\mathcal G,M}\).
...
\label{thm:app-constructive-nonscalar-costello-qme-realization}
  ...
  \]
  If \eqref{eq:app-native-scalar-condition} fails, the non-scalar problem has
  ...
  face, scalar condition, primitive candidate, or truncation matrix; none is
  supplied by analogy.
\end{thm}
...
\begin{proof}
  ...
  Applying the chain scalar projection gives the scalar condition.  If the
  condition is nonzero, the residual is not in
  \(\ker\widehat\sigma_{\mathrm{sc},M}\).  If the condition is zero, the
...
\label{prop:app-closed-rows-and-theta-three-source-face}
  ...
  these data the order-three larger system is not a solved QME row.
\end{prop}
...
\begin{proof}
  ...
  scalar condition is zero in the full-equivariant marked habitat.  In the
...
\label{prop:app-theta-three-bianchi-lower-condition}
  ...
  \[
    ...
    H^0(\mathcal K^\bullet_{\mathrm{ns},N}(I))
  \]
  must vanish next.  Without these two conditions, the lower \(\theta_3\)
...
\label{thm:app-theta-three-finite-window-H1}
  Work in the full-equivariant marked finite-window habitat with
  ...
  \[
    ...
    d_{\theta_3,M}=0.
  \]
  Two verdicts are recorded as theorems on this finite-window subcomplex.
  \begin{enumerate}[label=(\alph*)]
    \item \emph{Bare theorem (no counterterm supplied).}  In the
      ...
      No degree-zero row in the supplied finite complex maps to
      \(-\mathsf E_{\theta_3,M}\); the linear system
      \(A^M c=-r^M\) of
      Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
      has empty column matrix and residual
      \(r^M=\mathsf E_{\theta_3,M}\), so it is unsolvable.
    \item \emph{Counterterm theorem (supplied scalar-zero local
      counterterm \(C_{\theta_3,M}\)).}  Suppose a degree-zero local
      functional \(C_{\theta_3,M}\in\mathcal K^0_{\theta_3,M}\) with
  ...
  Both verdicts are computational facts about the supplied finite-row
  complex, not heuristic statements: they are the two outputs of the
  fixed-window matrix solver of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}.

  Cross-check: this theorem matches the explicit verdicts of
  \texttt{scripts/finite\_window\_graph\_array.py}.  Verdict (a)
  corresponds to the script cases
  \texttt{theta\_3\_current\_finite\_row\_subcomplex},
  \texttt{theta3\_ce\_ancestor\_absent\_payload},
  \texttt{theta3\_counterterm\_absent\_payload}, all returning
  \(\texttt{primitive\_exists}=\mathrm{false}\) with cokernel covector
  \(\ell(\mathsf E_{\theta_3,M})=1\).  Verdict (b) corresponds to
  three named alternative repair payloads --- the supplied
  scalar-zero counterterm
...
\begin{proof}
  Verdict (a).  The supplied finite complex has empty
  ...
  Verdict (b).  Adjoining the degree-zero \(C_{\theta_3,M}\) with
  ...
  Both verdicts are stable under finite-window truncation since the
...
\begin{rmk}[Verdict (a) versus verdict (b): which is the theorem]
\label{rmk:app-theta-three-verdict-which-is-theorem}
  Verdict (a) is the theorem of the \emph{minimal seed list} at
  order three: under the strict hypothesis that no order-three
  scalar-zero counterterm \(C_{\theta_3,M}\) is supplied, the
  finite-window H¹ class is nonzero, with explicit covector
  representative.  Verdict (b) is the theorem of the
  \emph{counterterm-extended} package: under the strict hypothesis
  that \(C_{\theta_3,M}\) with \(\widehat\sigma_{\mathrm{sc},M}=0\)
  and \(d_{\mathrm{ns},M}C_{\theta_3,M}=-\mathsf E_{\theta_3,M}\) is
  supplied, the H¹ class vanishes.  In the manuscript's current data
  package only verdict (a) is the certified theorem; verdict (b) is
  the theorem on the supplied
  \texttt{theta3\_counterterm\_supplied\_exact\_payload} fixture.
  Whichever way the supplied data goes, the result is a stated
  theorem.

  This is what the manuscript means by "first certified larger
  non-scalar row": \(\theta_3\) is the smallest order-\(n\) row whose
  finite-window H¹ is computed exactly to two verdicts, and any named
  coefficient \(C_{\theta_3}\) is admissible only when verdict (b)
  holds with one of the three named data --- a CE ancestor with
  controlled boundary, a supplied scalar-zero local counterterm
  \(C_{\theta_3,M}\), or a complete companion-face table whose signed
  sum moves the residual into the image of \(\widetilde A^M\)
  (Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}).
\end{rmk}
...
\label{prop:app-first-missing-nonscalar-row}
  ...
  \end{aligned}
  \]
  Without these row values, signs, scalar conditions, and truncation
  coefficients, no non-scalar obstruction class or
...
\begin{thm}[Minimal full-equivariant all-order vanishing]\label{thm:app-minimal-full-equivariant-all-order-vanishing}
  ...
  or nonzero scalar-zero row is added to the finite-window system.
...
\begin{proof}
  ...
  contribution is zero in the minimal system.  Therefore no nonzero
  order-three cochain is produced unless the system supplies a genuine
...
\label{prop:app-marked-row-centrality-homotopy-test}
  ...
  Costello graph/counterterm target; any larger system must supply the
  displayed row values, scalar condition, primitive \(H^M_{x,y}\), and
...
\label{thm:app-unreduced-cotangent-lift-obstruction}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Product/tensor-completion centrality.}
      ...
      obstruction records the degree to which this fails on the
      enlarged unreduced target.
    \item \emph{\(P_0\) / shifted-cotangent refinement.}
      ...
      construction-level witness; its absence is the obstruction.
    ...
    \item \emph{Č\v{e}ch--Roos / Weiss-versus-Ran descent gap.}
      ...
      boundary kernel.  In the abstract (C3) this is the
      "\v{C}ech--Roos descent" component.
  \end{enumerate}
  ...
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization},
  whose first finite-window failure is
  \(\theta_3\)
  (Theorem~\ref{thm:app-theta-three-finite-window-H1}).
  Vanishing of (iv) is the descent comparison; in the algebraic
  current model this gap is zero by the same finite-list
  axiomatization, but for genuine unreduced
  \(Z^{P_0}_{\mathrm{fact}}\)-morphisms the Weiss/Ran comparison is
  not automatic.
\end{thm}
...
\begin{proof}
  ...
  homotopy.  Vanishing is necessary because, in the absence of the
  homotopy, the source CE differential and the target
  \(P_0\)-bracket fail to commute on the image of
  ...
  flat.  The first finite-window failure occurs at the \(\theta_3\)
  ...
  zero, but the genuine unreduced \(Z^{P_0}_{\mathrm{fact}}\)-target
  carries the comparison nontrivially in general.
  ...
  \begin{itemize}
    ...
    \item vanishing of (i) at every window inductively certifies
  ...
  The first nonzero \(E_1\) class detects the failure of strict
  primitive-shadow centrality on the unreduced source, which is the
  classical primitive obstruction.
\end{proof}

\begin{rmk}[First nontrivial occurrence of each component]
\label{rmk:app-cotangent-lift-first-nontrivial-occurrence}
  By inspection of the manuscript and of the script outputs, the
  first nontrivial occurrence of each component of
  \(\operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}\)
  is as follows.
  \begin{itemize}
    \item \(\operatorname{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) is
      first nontrivial when one leaves the graded-commutative
      algebraic current target of
      Definition~\ref{def:app-central-character-principal-part-current-algebra}
      for a noncommutative \(\hbar\)-deformed target where the
      product centrality defect carries a Capelli-type contribution.
      Within the appendix, this is the polynomial-no-go boundary
      (Theorem~\ref{thm:app-unreduced-polynomial-centrality-no-go}):
      polynomial open BV observables cannot present the
      principal-part module at all, and hence cannot satisfy strict
      product centrality.
    \item \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is first
      nontrivial when one of the centrality homotopy rows
      \(R^{\mathrm{cent}}_{x,y,M}\) is supplied without a
      corresponding \(H^M_{x,y}\); the row machinery of
      Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}
      catches this in the smallest window where the
      brane-defect bracket of two source generators carries a
      nonzero scalar-zero contribution.  The script
      \texttt{scripts/finite\_window\_graph\_array.py} reports this
      as the centrality-homotopy fixture; in the minimal full-equivariant
      branch the row is zero and the homotopy is zero.
    \item \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}\) is first
      nontrivial at \(\theta_3\)
      (Theorem~\ref{thm:app-theta-three-finite-window-H1}), under
      the strict hypothesis that no scalar-zero counterterm
      \(C_{\theta_3,M}\) is supplied.  The supplied numerical
      anchor is verdict (a) of that theorem.
    \item \(\delta^{\mathrm{Weiss}}_{\check C/R}\) is first
      nontrivial when the codimension-one face of the Weiss-versus-Ran
      cover comparison is supplied with a non-vanishing boundary
      kernel class; in the present manuscript this is recorded as
      a separate descent comparison in the factorization current
      conventions appendix and is not addressed here as a row.
  \end{itemize}
  Hence the manuscript's leading-edge obstruction in this appendix
  is component (iii) at the \(\theta_3\) row, and the leading-edge
  cancellation is the four-branch ledger of
  Remark~\ref{rmk:app-scalar-anomaly-transgression-branches}.
\end{rmk}

\begin{rmk}[What supports what]
\label{rmk:app-unreduced-bv-qme-support-map}
  The non-scalar QME criterion of
  Principle~7 is stated in this appendix in finite-row,
  finite-window matrix form, with named first exposed row and named
  missing data.
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Filtered scalar projection and the unique-survivor
      cohomology class.}  The scalar-contact filtration
      \(F^\bullet\), its associated-graded scalar-symbol map, and the
      filtered chain projection
      \(\widehat\sigma_{\mathrm{sc}}\) are isolated by
      Definition~\ref{def:app-scalar-obstruction-complex} and
      Lemma~\ref{lem:filtered-scalar-projection-obstruction}.  The
      single class
      \([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
      \in H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}})\)
      exposed by
      Proposition~\ref{prop:app-first-scalar-lift-obstruction} and
      Theorem~\ref{thm:app-scalar-contact-qme-class} is the unique
      survivor of this appendix and the conjecture C2 in the
      abstract.  The
      transgression Theorem~\ref{thm:app-scalar-anomaly-transgression}
      assembles the four equivalent sightings (algebraic, Lie,
      trace-algebraic, quantum Weyl) of this same class, with the
      slogan \emph{constants vanish as Hamiltonian vector fields but
      survive as brane center-of-mass charge}.
    \item \emph{Branch ledger.}
      Theorem~\ref{thm:app-central-character-qme-cancellation},
      Theorem~\ref{thm:app-balanced-supertrace-qme-cancellation},
      ...
      and
      Theorem~\ref{thm:app-ordinary-glN-scalar-obstruction-criterion}
      together prove the four-branch ledger cited as Principle~2 of
      the preface and as proved data in the abstract: ordinary scalar
      reduction is obstructed by \(\hbar N[\bar c]\); central
      character, balanced \(\C^{N|N}\) supertrace, supertraceless
      monodromy, and full \(\lie{gl}(V)\)-equivariant marker tensors
      each absorb or kill the scalar.  The branch ledger as the action
      of the four sightings on one class is recorded in
      Remark~\ref{rmk:app-scalar-anomaly-transgression-branches}.
    \item \emph{Polynomial no-go and the distributional current target.}
      Theorem~\ref{thm:app-unreduced-polynomial-centrality-no-go} and
      Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies}
      separate the polynomial impossibility from the strict zero
      homotopy on the algebraic current target, supporting
      Principle~3 (ghosts move, cotangent coordinates measure) and
      Principle~4 (defect momentum is residual) in the preface.
    \item \emph{Native finite-window realization.}
      Definition~\ref{def:app-finite-window-marked-costello-list},
      Definition~\ref{def:app-oriented-full-equivariant-marked-boundary-complex},
      Lemma~\ref{lem:app-oriented-marked-incidence-signs},
      Proposition~\ref{prop:app-finite-window-marked-habitat-compatibility},
      Definition~\ref{def:app-nonscalar-kernel-row-complex},
      Definition~\ref{def:app-native-finite-window-realization-habitat},
      and
      Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
      build the explicit finite-row habitat in which
      \(A^M c^M=-r^M\) is a linear-algebra problem.  The minimal
      vanishing
      Theorems~\ref{thm:app-minimal-full-equivariant-kernel-vanishing}
      and~\ref{thm:app-minimal-full-equivariant-all-order-vanishing}
      bound the scope of the conclusion: zero residual is theorem
      data only in the named minimal full-equivariant array; outside
      that array,
      Proposition~\ref{prop:app-first-missing-nonscalar-row},
      Proposition~\ref{prop:app-first-scalar-zero-order-one-row},
      Proposition~\ref{prop:app-first-order-three-enlarged-row} list
      the next missing row data exactly.
    \item \emph{Centrality homotopies and \(Q_\Omega\)-equivariant
      lift.}  Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}
      and
      Proposition~\ref{prop:app-qomega-centrality-homotopy-criterion}
      record the row form of unreduced centrality homotopies and the
      \(Q_\Omega\)-invariant analogue.
      Definition~\ref{def:app-normal-omega-costello-kernel-datum} and
      Theorem~\ref{thm:app-equivariant-brane-defect-qme-criterion}
      isolate the brane-preserving normal scaling
      \(T_\Omega=\C^*_{\varepsilon_s}\times\C^*_{\varepsilon_1}\times\C^*_{\varepsilon_2}\)
      as a habitat, not as a QME proof.  This is the
      \(\Omega\)-background discipline of the repository
      \texttt{CLAUDE.md}.
    \item \emph{First exposed row \(\theta_3\) and the named missing
      data.}
      Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}
      gives \(R^{\mathrm{row}}_{\theta_3,M}=\mathsf E_{\theta_3,M}\),
      \(S_{\theta_3,M}=0\), and the one-row covector
      \(\ell_{\theta_3}\) detecting nonzero finite-row obstruction.
      Theorem~\ref{thm:app-theta-three-finite-window-H1} promotes this
      to two named verdicts on the supplied finite-row complex:
      \(H^1=\C\,\mathsf E_{\theta_3,M}\neq0\) without a counterterm,
      \(H^1=0\) with a supplied scalar-zero counterterm
      \(C_{\theta_3,M}\) satisfying \(d_{\mathrm{ns},M}C_{\theta_3,M}
      =-\mathsf E_{\theta_3,M}\).  Both verdicts cross-check
      \texttt{scripts/finite\_window\_graph\_array.py}.  Repair
      requires CE ancestor, scalar-zero Costello counterterm
      \(C_{\theta_3,M}\), or a complete companion-face table; tower
      compatibility passes through
      Proposition~\ref{prop:app-theta-three-bianchi-lower-condition}
      and the
      \(\Delta^1_{M,N}=-\pi_{M,N}\mathfrak b^M+\mathfrak b^N\) and
      \(\varprojlim^1 H^0\) tests.  Even-block markers
      (Proposition~\ref{prop:app-even-block-marker-obstruction})
      give a nonzero scalar shadow when
      \(\lambda_0\neq\lambda_1\) and collapse the larger habitat back
      to the full-equivariant single-marker case otherwise.
    \item \emph{Unreduced cotangent lift obstruction.}
      Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}
      states that the unreduced factorization-center morphism
      \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\) lifts to a
      \(Z^{P_0}_{\mathrm{fact}}\)-morphism if and only if the
      four-component obstruction vector
      \(\operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}\)
      vanishes.  The components are the product/tensor-completion
      centrality, the \(P_0\)/shifted-cotangent refinement (computed
      by the centrality-homotopy row machinery), the boundary QME
      (whose first finite-window failure is the \(\theta_3\) row),
      and the Weiss-versus-Ran descent gap.  Remark~\ref
      {rmk:app-cotangent-lift-first-nontrivial-occurrence} identifies
      the first nontrivial occurrence of each component.  At
      associated-graded, the primitive
      \(P_0\)-shadow of
      Theorem~\ref{thm:chain-level-primitive-projection}
      computes the \(E_1\) page of the spectral sequence converging
      to \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\).
  \end{enumerate}
  Stronger global claims (C2, C3, C4 of the abstract) are written here
  as the matrix equation \(A^Mc=-r^M\), the Roos compatibility on
  primitives, and the centrality homotopy / \(\varprojlim^1H^0\) tests
  attached to it; they are not vague open ends.
\end{rmk}
