...
Fix the formal-local mixed geometry
\(\R^2_{\mathrm{top}}\times\widehat{\C^2}_0\) with
\(\Omega_{\mathrm{loc}}=dz_1\wedge dz_2\).  On a brane interval
\(I\subset L\) the open BV theory contributes scalar contact
counterterms each order in \(\hbar_{\mathrm{QME}}\); the residual non-scalar Costello
quantum master equation lives in their orthogonal complement.  The
complex \(\mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I)\) is the
local-functional cochain complex on \(I\) at weighted-Tate coefficient
weight \(w\), with the brane-tangential differential
\(d_\partial\) and the \(\hbar_{\mathrm{QME}}\)-formal grading;
\(\widehat\sigma_{\mathrm{sc}}\) is the filtered scalar-symbol chain
projection killing the scalar-contact filtration;
\(d_{\mathrm{cl}}=Q+\{I_0,-\}\) is the associated-graded classical
BV differential; and
\(d_{\mathrm{QME}}^{L}=Q+\{I[L],-\}_{L}+\hbar_{\mathrm{QME}}\Delta_{L}\) is the
scale-\(L\) quantum master differential of Costello's renormalization.
When a fixed Costello scale is understood, \(d_{\mathrm{QME}}\) denotes
this quantum differential \(d_{\mathrm{QME}}^{L}\).  The
associated-graded scalar-symbol complexes below use \(d_{\mathrm{cl}}\).
The separated parameters are
\(\hbar_{\mathrm{QME}}\), \(\hbar_{\mathrm{W}}\), and \(\hbar_\omega\):
Costello graph order, boundary Weyl/Moyal deformation, and equivariant
Hamiltonian scalarization respectively
(Definition~\ref{def:app-normal-omega-costello-kernel-datum}).  Write
\[
  \kappa_{\mathrm{W}}=\hbar_{\mathrm{W}}N[\bar c],
  \qquad
  \kappa_{\mathrm{QME}}=\hbar_{\mathrm{QME}}N[\bar c].
\]
The first is the boundary Weyl/Capelli projective class of
Theorem~\ref{thm:capelli-projective-anomaly}; the second is the scalar
Costello QME obstruction after the brane-defect kernel identifies its
one-loop boundary contraction with the same Lie cocycle \([\bar c]\).
No formula below identifies \(\hbar_{\mathrm{QME}}\) with
\(\hbar_{\mathrm{W}}\) unless that specialization is stated.
Before passage to the
residual non-scalar complex, the unreduced curvature
\(\operatorname{Ob}_{w,\partial,\hbar_{\mathrm{QME}}}\) has scalar-contact symbol
\[
  \widehat\sigma_{\mathrm{sc}}\bigl(
    \operatorname{Ob}_{w,\partial,\hbar_{\mathrm{QME}}}\bigr)
    =\kappa_{\mathrm{QME}}\,\gamma_{\mathbf 1}
    \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1]
       [[\hbar_{\mathrm{QME}}]].
\]
Forgetting the central ghost coefficient gives the scalar class
\(\kappa_{\mathrm{QME}}\in
H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar_{\mathrm{QME}}]]\).
After this scalar component is killed, the residual non-scalar QME
complex contains the cohomology class
\[
  [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}}}]
  \in
  H^1\!\bigl(\ker\widehat\sigma_{\mathrm{sc}},
    d_{\mathrm{QME}}^{L}\bigr),
\]
together with its finite-window
\(\varprojlim^1H^0\)-primitive-compatibility class
(the secondary Roos cohomology obstruction tracking compatibility of
window-by-window primitives across the truncation tower).  The
Capelli class of Chapter~6
(Theorem~\ref{thm:capelli-projective-anomaly}) is the boundary Weyl
class \(\kappa_{\mathrm{W}}\), with sign and convention fixed by
Appendix~\ref{sec:app-factorization-current-conventions} and the canonical scheme
\(z_1z_2\mapsto\tfrac12(\phi_1\phi_2+\phi_2\phi_1)\) of the Moyal
star product.  Exact rational expansion gives the Moyal coefficient
\(\hbar_{\mathrm{W}}N\).  A Costello QME computation lands on the same
underlying cocycle \([\bar c]\) with coefficient \(\hbar_{\mathrm{QME}}N\)
only after the brane-defect kernel supplies the one-loop boundary
contraction.

The scalar anomaly is one cohomology class realized in four
cohomologies.  The algebraic central extension of
\(\bar A=\C[z_1,z_2]/\C\cdot1\), the Lie central extension
\(\widehat{\mathfrak h}_{\mathrm{cent}}\), the trace-algebraic
boundary central extension, and the quantum Weyl commutator
\([\Phi_{\hbar_{\mathrm{W}}}(z_1),
  \Phi_{\hbar_{\mathrm{W}}}(z_2)]_\star=\hbar_{\mathrm{W}}N\)
carry the same source class \([\bar c]\) under the specified scalar
projection of Proposition~\ref{prop:app-local-geometry-scalar-shadow}.
The cocycle \(\omega(f,g)=[\{f,g\}]_0\) is the same in
each; \(\kappa_{\mathrm{W}}\) and \(\kappa_{\mathrm{QME}}\) are its two
parameterized scalarizations
(Theorem~\ref{thm:app-scalar-anomaly-transgression}).  Hamiltonian
vector fields annihilate the constant line, while the open trace pairs
that line with \(\operatorname{Tr}(\mathbf 1)=N\) and produces the
\(\kappa_{\mathrm{W}}\) class.  Each cancellation branch acts on the
same underlying class \([\bar c]\): ordinary \(\lie{gl}_N\) is obstructed by
\(\kappa_{\mathrm{QME}}\) in the Costello scalar-contact complex, the
unreduced central character contributes the
CE coboundary
\(d_{\mathrm{CE}}(\hbar_{\mathrm{QME}}\chi_N)
  =-\hbar_{\mathrm{QME}}N\omega\), the
balanced \(\C^{N|N}\) supertrace rescales the open trace by
\(\operatorname{sdim}\C^{N|N}=0\), and full-equivariant marker
tensors satisfy
\(\operatorname{Str}_{\mathrm{cyc}}(\mathfrak m)=0\).  Modifying one
branch displaces the same class.

At each finite window \(M\) (a Hamiltonian-degree truncation of the
weighted Tate coefficient pair) the obstruction is the linear-algebra
problem
\[
  A^M c^M=-r^M
\]
of Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization},
where \(A^M\) is the finite matrix whose columns enumerate the
counterterm primitives given by the row span, \(c^M\) is the
unknown counterterm vector, and \(r^M\) is the residual vector
collecting the QME source faces.  The row entries are graph-by-graph
Costello data: \(K^M_\Gamma\) the windowed kernel of the Feynman graph
\(\Gamma\), \(\varepsilon_\Gamma\) its sign, \(\zeta_{M,\nu}\) a CE
test pairing labelled by the cycle \(\nu\), and the bracket
\(\{-,-\}_{\hbar,M}\) the windowed Schouten/BV bracket.  In these
terms,
\[
...
\]
and the order-\(n\) homogeneous lower-counterterm rows.  A degree-zero
counterterm \(c^M\) exists in the given row span exactly when the
system is consistent; otherwise a separating covector
\(\ell A^M=0\), \(\ell(r^M)\neq0\) detects the obstruction.  Tower
compatibility passes through truncation matrices \(T^{M,N}\) on rows
(restriction-of-graphs from window \(M\) to a smaller window \(N\)) and
\(P^{M,N}\) on primitive columns (the matched counterterm projector)
and the Roos class \([P^{M,N}c^M-c^N]\in H^0(\mathcal K^\bullet_{\mathrm{ns},N}(I))\)
(the cohomology class measuring compatibility of windowwise primitives
across truncation).

The order-three row \(\theta_3=\mathrm{CE}(\Theta_3,\nu_3)\) is the
first scalar-zero source-face row beyond the order-one one-cross
scalar contact.  It has
\[
  ...
  S_{\theta_3,M}=0,
\]
and in the one-row complex
\(\mathcal K^0_{\theta_3,M}=0\),
\(\mathcal K^1_{\theta_3,M}=\C\mathsf E_{\theta_3,M}\),
\(d_{\theta_3,M}=0\), the covector
\(\ell_{\theta_3}(\mathsf E_{\theta_3,M})=1\) is an exact obstruction
(Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}).
A named coefficient \(C_{\theta_3}\) requires one of three data: a CE
ancestor with controlled boundary; a scalar-zero Costello local
counterterm \(C_{\theta_3,M}\) with
\(d_{\mathrm{ns},M}C_{\theta_3,M}=-\mathsf E_{\theta_3,M}\) and
truncation-compatible primitives; or a complete companion-face table
of Hom-valued companion rows whose signed sum moves the residual
vector into the image of \(A^M\).  Tower compatibility passes through the truncation defect
\(\Delta^1_{M,N}=-\pi_{M,N}\mathfrak b^M+\mathfrak b^N\)
(\(\mathfrak b^M\) the order-one Bianchi residual at window \(M\),
\(\pi_{M,N}\) the truncation projector) and the
secondary \(\varprojlim^1H^0\) primitive class
(Proposition~\ref{prop:app-theta-three-bianchi-lower-condition}).
The unreduced factorization-center cotangent lift has a
four-component obstruction vector.  Its vanishing is necessary; it is
sufficient only after the finite-window companion tables, centrality
homotopies, counterterms, and descent comparison homotopies are
included in the datum
...
\label{def:app-unreduced-bv-degrees}
  ...
  \]
  This is the cotangent-coordinate grading of the CE/PV block.  The
  ordinary graded dual of the BV degree of \(\rho[1]\) is a separate
  convention.  The
...
\begin{prop}[Local geometry of scalar projections]
\label{prop:app-local-geometry-scalar-shadow}
  ...
  \]
  Thus the scalar projection has zero divergence and zero volume-form anomaly.
  The Hamiltonian scalar cocycle entering every calculation
  below is
  \[
    \omega(f,g)
      =
  ...
  \[
    \hbar_{\mathrm{QME}}N\,\omega(f,g)\gamma_{\mathbf 1}
    \quad\text{in the ordinary }\lie{gl}_N\text{ branch},
  \]
  \[
    \hbar_{\mathrm{QME}}\,\operatorname{sdim}(\C^{N|N})\,
  ...
  \]
  A finite window \(M\) restricts only the polynomial Hamiltonian labels,
  graph labels, and counterterm table to a finite local coefficient
  system; it preserves
  \(\Omega_{\mathrm{loc}}\), the Hamiltonian bracket, and the
...
\begin{proof}
  ...
  is preserved and contributes zero additional scalar contact term.
  ...
  of labels and graph tables inside this same formal polydisk, so it preserves
...
\label{def:app-scalar-obstruction-complex}
  ...
  The associated-graded differential controlling a mixed brane-defect
  scalar-symbol lift is
  \[
    d_{\mathrm{cl}}=Q+\{I_0,-\}.
  \]
  ...
  \[
    \mathfrak Q^\bullet_{\mathrm{bd}}
      =(\mathcal O_{\mathrm{loc}}^{\mathrm{bd}},d_{\mathrm{cl}}),
  \]
  ...
  \(d_{\mathrm{cl}}\)-stable scalar filtration and a filtered
  chain projection
  \[
  ...
  The assertion below is an associated-graded scalar obstruction statement
  until this extra projection is fixed.
  Thus a degree-one QME representative with two Hamiltonian arguments maps to
...
\begin{lem}[Filtered scalar projection obstruction]\label{lem:filtered-scalar-projection-obstruction}
  ...
  \(d_{\mathrm{cl}}\), and put the target
  ...
  \[
    \delta_s=d_{\mathrm{CE}}s-s\,d_{\mathrm{cl}}
  \]
...
\begin{proof}
  ...
  commutator \(d_{\mathrm{CE}}s-sd_{\mathrm{cl}}\) strictly lowers the
  filtration.  Its lowest nonzero component is closed for the Hom
  differential because
  \(d_{\mathrm{CE}}^2=d_{\mathrm{cl}}^2=0\).  Replacing \(s\) by
...
\begin{prop}[First scalar-lift obstruction in the ordinary branch]\label{prop:app-first-scalar-lift-obstruction}
  ...
  \(\mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I)\), with \(N>0\).  Let
  \(J_{\mathrm{sc}}\) be the closed ideal of functionals whose
  associated-graded scalar-contact symbol vanishes, and put
  \(F^r=J_{\mathrm{sc}}^r\), completed
  \(\hbar_{\mathrm{QME}}\)-adically and in the finite-window inverse
  limit.  For any filtered linear lift \(s\) of the
  scalar-symbol projection, the first chain-defect class has Hamiltonian
  two-argument evaluation
  \[
    \operatorname{ev}_{f,g}\bigl(o_{\sigma,w}^{(1)}\bigr)
      =
    \hbar_{\mathrm{QME}}N\,\omega(f,g)\gamma_{\mathbf 1}
      \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})
        [[\hbar_{\mathrm{QME}}]] .
  \]
  In particular
  \[
    \operatorname{ev}_{z_1,z_2}\bigl(o_{\sigma,w}^{(1)}\bigr)
      =\hbar_{\mathrm{QME}}N\,\gamma_{\mathbf 1}\neq0 .
  \]
  Hence a filtered chain lift of the scalar-symbol projection in the
  ordinary scalar-reduced branch is governed by a primitive whose leading
  scalar class is \(-\kappa_{\mathrm{QME}}\), or by a replacement of the source
  or open trace model before scalar reduction.
\end{prop}

\begin{proof}
  The filtration \(F^\bullet\) is the scalar-contact filtration:
  ...
  \[
    ...
    I_\partial(ab,\{f,g\}_{\bar A})
      =
    \hbar_{\mathrm{QME}}N\,\omega(f,g)\int_Ia(t)b(t)\,dt
  \]
  after insertion of \(\gamma_{\mathbf 1}\).  Applying the scalar-symbol
  lift gives the displayed representative of
  \(d_{\mathrm{CE}}s-sd_{\mathrm{cl}}\).  For \(f=z_1\) and \(g=z_2\),
  \(\omega(z_1,z_2)=1\).

  If this first defect were a Hom coboundary, evaluation on the
  Hamiltonian two-argument subcomplex would give a linear primitive
  \(\bar\eta:\bar A\to\C\gamma_{\mathbf 1}\) with
  \(d_{\mathrm{CE}}\bar\eta
    =\hbar_{\mathrm{QME}}N\,\omega\gamma_{\mathbf 1}\).  Pulling
  \(\bar\eta\) back to \(A=\C[z_1,z_2]\) and requiring it to vanish on the
  ...
  finite-window class, so changing \(w\) preserves its vanishing criterion.
  ...
  \(\operatorname{sdim}\C^{N|N}=0\).  The scalar-contact subcomplex and
...
\begin{thm}[Polynomial unreduced centrality obstruction]
\label{thm:app-unreduced-polynomial-centrality-obstruction}
  Let \(M_{\mathrm{pol}}\) be any \(Q\)-stable subcomplex of the
  unreduced polynomial open BV observables in which every element has
  finite polynomial degree in \(\phi_1,\phi_2,\psi\).  Any assignment
  \[
    \rho_{a,b}[1]\longmapsto \Theta_{a,b}\in H^\bullet(M_{\mathrm{pol}},Q)
  \]
  together with centrality homotopies in \(M_{\mathrm{pol}}\) has locally
  nilpotent linear Hamiltonian action on \(Q\)-cohomology, whereas the
  principal-part module \(\mc P\) has non-locally-nilpotent linear
  Hamiltonian action.  Equivalently, an unreduced polynomial lift of the
  reduced map \(\rho_{a,b}[1]\mapsto\Theta_{a,b}\) is the datum of
...
\begin{proof}
  ...
  scalar-translation derivations
  \[
  ...
  \]
  and similarly for \(z_2\), for every \(r\geq1\).  Thus every nonzero
  principal part has non-locally-nilpotent linear Hamiltonian orbit.

  Existence of the stated homotopies in \(M_{\mathrm{pol}}\) identifies,
  after passage to \(Q\)-cohomology, the action of \(O_{1,0}\) and \(O_{0,1}\)
  on the classes \([\Theta_{a,b}]\) with the displayed
  principal-part action.  Local nilpotence is invariant under module
  isomorphism and under passage to a submodule. This is incompatible with
  the non-locally-nilpotent principal-part action above. Therefore the
  unreduced lift requires a category beyond polynomial open BV/Koszul
  observables.
\end{proof}
...
\begin{proof}
  ...
  \[
    ...
    \qquad
    \varphi\in C^\infty_c(\R).
  \]
  This is continuous because multiplication by \(a\) is a continuous
  endomorphism of \(C^\infty_c(\R)\) and
  \(a\varphi\) has compact support.  If
...
\label{def:app-central-character-principal-part-current-algebra}
  ...
  intervals.  This is an algebraic current model.  The finite polynomial
  open BV/Koszul complex is governed by
  Theorem~\ref{thm:app-unreduced-polynomial-centrality-obstruction}.
\end{defn}
...
\label{thm:app-algebraic-factorization-centre-zero-homotopies}
  ...
  current model permits.  A morphism into
  \(Z^{P_0}_{\mathrm{fact}}(\mathrm{Obs}_{\mathrm{open}})\) for the
  genuine unreduced open BV factorization algebra is governed by the
  unreduced graph/QME data.  This is compatible with the polynomial obstruction
...
\begin{proof}
  ...
  algebraic current target, including monomials beyond length-one
  generators.  Let
  ...
  algebraic current target.  The genuine unreduced open BV observable
  factorization algebra and the unreduced
  \(Z^{P_0}_{\mathrm{fact}}\)-morphism require the additional graph/QME data.
\end{proof}
...
\label{lem:app-unreduced-scalar-primitive}
  ...
  \(d_{\mathrm{CE}}\eta=\omega\).  The primitive lives on the unreduced
  algebra \(A\); scalar reduction to \(\bar A=A/\C\cdot1\) removes it.
\end{lem}

\begin{proof}
  Applying the Chevalley--Eilenberg differential to the displayed
  linear functional gives
  \[
...
\label{thm:app-scalar-contact-qme-class}
  ...
  \[
    \operatorname{Tr}(A\,XY)-\operatorname{Tr}(A\,YX)
      =
      \hbar_{\mathrm{W}}N\,\operatorname{Tr}(A)
  ...
  \[
    ...
    \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1]
      [[\hbar_{\mathrm{QME}}]].
  \]
  Under the coefficient-forgetting map it becomes the formal scalar
  multiple
  \(\kappa_{\mathrm{QME}}\in
  H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar_{\mathrm{QME}}]]\).  This
  class is a nonzero scalar-reduced Hamiltonian cohomology class.  Hence an
  internal scalar counterterm in the ordinary scalar-reduced
  \(\lie{gl}_N\) brane-defect model has leading-symbol obstruction
  \(\kappa_{\mathrm{QME}}\).
\end{thm}
...
\begin{proof}
  ...
  quantum version of the same central term in the boundary Weyl algebra:
  the Weyl convention gives
  \([X^i{}_j,Y^k{}_l]=\hbar_{\mathrm{W}}\delta^i_l\delta^k_j\), and the normal
  ordered moment relation is
  \(Y_{\hbar_{\mathrm{W}}}X_{\hbar_{\mathrm{W}}}
    -X_{\hbar_{\mathrm{W}}}Y_{\hbar_{\mathrm{W}}}
    +\hbar_{\mathrm{W}}N I\equiv0\). Hence
  \(XY-YX=\hbar_{\mathrm{W}}N I\) in the quantum Hamiltonian reduction, and tracing
  against \(A\) gives the stated sign.  The matrix-commutator form is
  the same identity written as
  \(\operatorname{Tr}(A(XY-YX))\).
  The scalar QME representative uses the same cocycle \(\omega\), but
  its coefficient is the Costello graph-order parameter
  \(\hbar_{\mathrm{QME}}\), supplied by the one-loop brane-defect
  contraction.
  ...
  extension cocycle
  \(\hbar_{\mathrm{QME}}N\omega\,\gamma_{\mathbf 1}\), hence the
  class
  \[
    \kappa_{\mathrm{QME}}\,\gamma_{\mathbf 1}
    \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1]
  ...
  If its coefficient-forgetting image were exact in the scalar-reduced
  Hamiltonian source, the leading scalar term would give a linear primitive
  ...
  polynomial Hamiltonian algebra; scalar reduction to \(\bar A\) removes
  that primitive.  Thus a scalar-reduced counterterm is equivalent to an
  associated-graded primitive for the class.
\end{proof}

\begin{thm}[Scalar anomaly transgression: four equivalent representatives]
\label{thm:app-scalar-anomaly-transgression}
  In the formal-local mixed geometry
  \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_0\) with
  \(\Omega_{\mathrm{loc}}=dz_1\wedge dz_2\) of
  Proposition~\ref{prop:app-local-geometry-scalar-shadow}, the
  scalar Hamiltonian cocycle
  \[
    \omega(f,g)=[\{f,g\}_{\Omega_{\mathrm{loc}}}]_0,
    \qquad \omega(z_1,z_2)=1,
  \]
  is detected in four equivalent forms.  All four witness the same
  nonzero source class \([\bar c]\in H^2(\bar A;\C)\).  The boundary
  Weyl/Capelli scalarization gives
  \(\kappa_{\mathrm{W}}=\hbar_{\mathrm{W}}N[\bar c]\), while the scalar
  Costello QME row gives
  \(\kappa_{\mathrm{QME}}=\hbar_{\mathrm{QME}}N[\bar c]\).  These are
  two scalar multiples of the same Lie cocycle; they are not identified
  unless a specialization \(\hbar_{\mathrm{W}}=\hbar_{\mathrm{QME}}\) is
  imposed.

  \begin{enumerate}[label=\textnormal{(\roman*)}]
    \item \emph{Algebraic central extension of the Hamiltonian
      Lie algebra.} The short exact sequence of Lie algebras
      \[
        0\longrightarrow \C\cdot1
        \longrightarrow A=\C[z_1,z_2]
        \longrightarrow \bar A=A/\C\cdot1
        \longrightarrow 0,
        \qquad A\text{ with bracket }
        \{f,g\}_{\Omega_{\mathrm{loc}}},
      \]
      is the central extension whose Lie cocycle is exactly
      \(\omega\).  Its class is \([\bar c]\in H^2(\bar A;\C)\) with
      Lemma~\ref{lem:app-unreduced-scalar-primitive}.
    \item \emph{Lie-algebraic central extension.} Setting
      \(\widehat{\mathfrak h}_{\mathrm{cent}}
        =\bar A\oplus\C\bar c\) with bracket
      \[
        [\![\bar f,\bar g]\!]_{\mathrm{cent}}
        =\overline{\{f,g\}}+\omega(f,g)\bar c,
        \qquad
        [\![\bar c,-]\!]_{\mathrm{cent}}=0,
      \]
      produces a Lie central extension whose class is the same
      \([\bar c]\).  This is the source-side \(\widehat{\mathfrak
      h}_{\mathrm{cent}}\).
    \item \emph{Trace-algebraic central extension on the boundary.}
      With the unreduced boundary Hamiltonian generator
      \(I_\partial(a,f)\) of
      Theorem~\ref{thm:app-scalar-contact-qme-class}, the open Chan--Paton
      ordinary-trace open coupling produces the central extension
      \[
        0\longrightarrow \C\cdot\operatorname{Tr}(\mathbf 1)
        \longrightarrow \operatorname{Tr}\C[\phi_1,\phi_2]_{\mathrm{red}}
        \longrightarrow A_{\partial,\mathrm{red}}
        \longrightarrow 0
      \]
      of trace-reduced boundary algebras.  After insertion in the
      scalar Costello QME row, its Lie cocycle is
      \(\hbar_{\mathrm{QME}}N\omega\) and its class in
      \(H^2(A_{\partial,\mathrm{red}};\C)[[\hbar_{\mathrm{QME}}]]\) is
      \(\kappa_{\mathrm{QME}}\).
    \item \emph{Quantum Weyl commutator.}  In the Weyl quantization,
      \[
        [\Phi_{\hbar_{\mathrm{W}}}(z_1),
          \Phi_{\hbar_{\mathrm{W}}}(z_2)]_{\star}
          =\hbar_{\mathrm{W}}N
        \mod \mathcal W_N\widehat\mu(\lie{sl}_N),
        \qquad
        W_1^{\mathrm{sc,ord}}=\kappa_{\mathrm{QME}}
        \quad\text{after Costello one-loop normalization},
      \]
      i.e.\ the Capelli shift makes the scalar commutator equal
      \(\hbar_{\mathrm{W}}N\) times the identity, modulo the Hamiltonian
      moment-map ideal of \(\lie{sl}_N\).  The associated Weyl CE class
      is \(\kappa_{\mathrm{W}}\); the scalar QME row records the same
      underlying cocycle as \(\kappa_{\mathrm{QME}}\).
  \end{enumerate}

  These four are equivalent in the strong sense that the same source class
  \([\bar c]\in H^2(\bar A;\C)\) is the image, under the specified
  scalar projections, of all four cocycles:
  \[
  \begin{array}{ccccccc}
    [\omega]_{(\mathrm{i})}
    & = &
    [\omega]_{(\mathrm{ii})}
    & = &
    [\omega]_{(\mathrm{iii})}^{\mathrm{src}}
    & = &
    [\omega]_{(\mathrm{iv})}^{\mathrm{src}}
    \\[0.5ex]
    & & & & & &
    \in H^2(\bar A;\C).
  \end{array}
  \]
  The two scalarized classes are the images
  \[
  \begin{array}{ccccc}
    [\omega]
    &\longmapsto&
    \hbar_{\mathrm{QME}}N[\omega]=\kappa_{\mathrm{QME}}
    & \in &
    H^2(\bar A;\C)[[\hbar_{\mathrm{QME}}]],
    \\[1ex]
    [\omega]
    &\longmapsto&
    \hbar_{\mathrm{W}}N[\omega]=\kappa_{\mathrm{W}}
    & \in &
    H^2(\bar A;\C)[[\hbar_{\mathrm{W}}]].
  \end{array}
  \]
  The first scalarization is the Costello graph-order class in the QME
  obstruction complex.  The second is the boundary Weyl/Capelli class.

  The radial Poisson field
  \(X_f\) annihilates the constant line in \(A\) (Proposition~\ref
  {prop:app-local-geometry-scalar-shadow}), but the open trace pairs
  the same constant line against \(\operatorname{Tr}(\mathbf 1)=N\) and
  produces the Capelli shift \(\kappa_{\mathrm{W}}\).  Equivalently, the
  same Weyl scalarization is the Lie cohomology class
  \(\hbar_{\mathrm{W}}N[\bar c]\).  If a
  determinant line with connection exists and its Atiyah class
  pulls back to \([\bar c]\), its curvature restricts to
  \(\kappa_{\mathrm{W}}\) at the polynomial trace generator
  \(J(f)=\operatorname{Tr}f(\phi_1,\phi_2)\), \(f\in\bar A\), with
  Taylor-window completion for \(f\in\mathfrak h\), by
  Theorem~\ref{thm:capelli-projective-anomaly}: the four
  representatives are the four forms in which the local-stalk Lie
  cohomology class enters the formal-Darboux computation, with the determinant-line
  interpretation of the central extension given by
  Remark~\ref{rmk:determinant-line-scheme}.
\end{thm}

\begin{proof}
  Representative (i) is the content of
  Lemma~\ref{lem:app-unreduced-scalar-primitive}: \(\omega\) is the
  CE coboundary of \(\eta(f)=-[f]_0\) on the unreduced \(A\), and
  scalar reduction to \(\bar A\) removes \(\eta\).  Hence \([\omega]\) is
  nonzero on \(\bar A\); the central extension presented in (i) is
  this nontrivial class.

  Representative (ii) is obtained by the Chevalley--Eilenberg
  construction of a central extension from a \(2\)-cocycle.  Setting
  \([\![\bar f,\bar g]\!]_{\mathrm{cent}}=\overline{\{f,g\}}+\omega(f,g)\bar c\)
  defines a Lie bracket because \(\omega\) is a \(2\)-cocycle, and the
  associated extension class is \([\omega]=[\bar c]\): if the extension
  split as Lie algebras, a linear splitting would give a \(1\)-cochain
  \(\lambda\) with \(d_{\mathrm{CE}}\lambda=\omega\), while the preceding
  paragraph proves that no such cochain descends to the reduced algebra
  \(\bar A\).  Under the map which sends a central-extension bracket
  cocycle to its Chevalley--Eilenberg cohomology class, this is the
  class of representative (i).

  Representative (iii) is the trace-algebraic content of
  Theorem~\ref{thm:app-scalar-contact-qme-class}: the open Chan--Paton
  ordinary trace pairs \(I_\partial(a,f)I_\partial(b,g)\) and produces
  the residual \(N\,\omega(f,g)\int_\R a(t)b(t)\,dt\).  After insertion
  as a one-loop Costello scalar symbol, this becomes
  \(\hbar_{\mathrm{QME}}N\,\omega(f,g)\int_\R a(t)b(t)\,dt\), which is
  the trace-algebraic QME cocycle.  The boundary
  source map \(A_{\partial,\mathrm{red}}\to\bar A\) (induced by
  \(I_\partial(a,f)\mapsto\bar f\) on the Hamiltonian factor and by
  evaluation against the test function) factors the trace cocycle as
  \(\hbar_{\mathrm{QME}}N\) times the source \(\bar A\)-cocycle \(\omega\).
  Hence the class in
  \(H^2(A_{\partial,\mathrm{red}};\C)[[\hbar_{\mathrm{QME}}]]\) is
  \(\kappa_{\mathrm{QME}}\) under this pullback: the trace
  coefficient and the Costello graph-order parameter are the only
  scalar dependences beyond the source cocycle.

  Representative (iv) is the quantum Weyl reformulation in the same theorem:
  the matrix Weyl commutator \([X,Y]=\hbar_{\mathrm{W}}N I\) modulo
  \(\mathcal W_N\widehat\mu(\lie{sl}_N)\) is the Capelli normal-ordered
  scalar shift.  Tracing against \(A\) and applying the
  associated-graded scalar-symbol projection produces the same scalar
  Weyl CE class \(\kappa_{\mathrm{W}}\).  The Costello scalar QME row
  uses the same source cocycle with coefficient
  \(\hbar_{\mathrm{QME}}N\), hence \(\kappa_{\mathrm{QME}}\).

  The source equalities are induced by the scalar-symbol projection
  of Proposition~\ref{prop:app-local-geometry-scalar-shadow}
  and the open Chan--Paton trace.  The algebraic content:
  \(X_f(1)=0\) for
  every \(f\) (the constant line is annihilated by every Hamiltonian
  vector field), while the trace pairing
  \(\operatorname{Tr}(\mathbf 1)=N\) records the constant line as the
  open index-loop charge.  The formal parameter is supplied later by
  the construction: \(\hbar_{\mathrm{QME}}\) for Costello graph order,
  \(\hbar_{\mathrm{W}}\) for the boundary Weyl algebra.
\end{proof}

\begin{rmk}[Action of the four cancellation branches on the scalar
  anomaly class]
\label{rmk:app-scalar-anomaly-transgression-branches}
  Each branch acts on the class
  \(\kappa_{\mathrm{QME}}\in
  H^2(\bar A;\C)[[\hbar_{\mathrm{QME}}]]\) of
  Theorem~\ref{thm:app-scalar-anomaly-transgression}.  The unreduced
  central character produces the CE coboundary
  \(d_{\mathrm{CE}}(\hbar_{\mathrm{QME}}N\eta)
    =\hbar_{\mathrm{QME}}N\omega\) on the unreduced
  source, with \(\eta(f)=-[f]_0\) of
  Lemma~\ref{lem:app-unreduced-scalar-primitive} admitted as a
  degree-zero element
  (Theorem~\ref{thm:app-central-character-qme-cancellation}).  The
  balanced \(\C^{N|N}\) supertrace replaces the open trace coefficient
  by \(\operatorname{sdim}\C^{N|N}=0\), rescaling the cocycle to zero
  before its CE class is formed
  (Theorem~\ref{thm:app-balanced-supertrace-qme-cancellation}).
  Supertraceless monodromy extends the balanced computation to marked
  open loops with cyclic supertrace zero on every monodromy word
  (Theorem~\ref{thm:app-supertraceless-monodromy-projection}).  Full
  \(\lie{gl}(V)\)-equivariant signed Brauer marker tensors satisfy
  \(\operatorname{Str}_{\mathrm{cyc}}(\mathfrak m)=0\) on
  \(\C^{N|N}\)
  (Lemma~\ref{lem:app-full-equivariant-cyclic-supertrace},
  Theorem~\ref{thm:app-full-equivariant-marked-shadow-vanishing}).
  By Theorem~\ref{thm:app-ordinary-glN-scalar-obstruction-criterion},
  any internal local counterterm in the ordinary scalar-reduced
  \(\lie{gl}_N\) branch must contribute leading scalar class
  \(-\kappa_{\mathrm{QME}}\).  The class \([\bar c]\neq0\), so the four branch
  replacements above are exactly the scalar cancellations available here.
\end{rmk}
...
\label{thm:app-central-character-qme-cancellation}
  ...
  represented by the same central character:
  \[
    J^\chi_N(1)=N,\qquad
    \operatorname{Tr}(A[X,Y])
      =J^\chi_N([z_1,z_2]_{*})\,\operatorname{Tr}(A)
      =\hbar_{\mathrm{W}} J^\chi_N(1)\operatorname{Tr}(A).
  \]
  Thus the local scalar QME symbol becomes exact after replacing the
  scalar-reduced source by the unreduced source with central character
  \(\chi_N\).  This is a source replacement before scalar reduction.  The
  original scalar-reduced QME complex has its own null-homotopy class.
  ...
  \[
    ...
    \hbar_{\mathrm{QME}}N\omega
      +d_{\mathrm{CE}}(\hbar_{\mathrm{QME}}\chi_N)=0
  \]
...
\begin{proof}
  ...
  \([z_1,z_2]_{*}=\hbar_{\mathrm{W}}\), \(J^\chi_N(1)=N\), and the matrix moment
  relation gives
  \(\operatorname{Tr}(A[X,Y])
    =\hbar_{\mathrm{W}}N\operatorname{Tr}(A)\).  Hence the
  ...
  \(YX-XY+\hbar_{\mathrm{W}}N I\equiv0\) says that an \(XY\)-to-\(YX\) interchange
  contributes the scalar \(\hbar_{\mathrm{W}}N I\).  In the scalar-reduced source
  ...
  \(J_N^\chi([z_1,z_2]_*)=\hbar_{\mathrm{W}}N\), so it is exact before
  forming the boundary Weyl class.  The corresponding scalar QME class
  is exact by
  \(d_{\mathrm{CE}}(\hbar_{\mathrm{QME}}\chi_N)
    =-\hbar_{\mathrm{QME}}N\omega\).
\end{proof}
...
\label{thm:app-balanced-supertrace-qme-cancellation}
  ...
  vanishes in the balanced \(\lie{gl}(N|N)\) supertrace model by the
  supertrace calculation.
\end{thm}
...
\begin{defn}[Balanced scalar-contact subcomplex]
\label{def:app-balanced-scalar-contact-subcomplex}
  Let \(V=\C^{N|N}\).  A balanced scalar-contact subcomplex over an interval
  ...
  applying \(d_{\mathrm{QME}}\): every scalar-contact symbol in this
  subcomplex is obtained by contracting a single identity endomorphism in
  the supertrace pairing.
\end{defn}

\begin{prop}[Balanced scalar-contact projection]
\label{prop:app-balanced-scalar-contact-projection}
  On every balanced scalar-contact subcomplex of
  Definition~\ref{def:app-balanced-scalar-contact-subcomplex} with
  ...
  \(o^{(r)}_{\sigma,w,\mathrm{bal}}(I)\) vanish on this subcomplex.  In
  particular the first two-Hamiltonian-argument defect is
  \[
...
\begin{proof}
  By definition of the subcomplex, every scalar-contact symbol factors as
  ...
  graded.  Take the zero filtered lift to the stated scalar CE target.  Since both composites
  ...
  \(\hbar_{\mathrm{QME}}N\,\omega(f,g)\gamma_{\mathbf 1}\) is replaced by
  \(\operatorname{sdim}(V)\).  Thus
  \[
    \hbar_{\mathrm{QME}}\,\operatorname{sdim}(V)\,
    \omega(f,g)\gamma_{\mathbf 1}=0,
  \]
  including the test \((f,g)=(z_1,z_2)\).
\end{proof}

\begin{rmk}[Extension obstruction outside the balanced subcomplex]
\label{rmk:app-balanced-projection-extension-obstruction}
  Proposition~\ref{prop:app-balanced-scalar-contact-projection} kills
  the scalar-contact projection tower.  The full non-scalar graph/QME
  residual is governed by the non-scalar graph/QME obstruction complex.  Enlarging
  \(\mathfrak H^\bullet_{\mathrm{bal,sc}}(I)\) to a complete filtered
  subcomplex \(\mathfrak H'\) carrying scalar symbols with unspecified
  \(\operatorname{sdim}(V)\)-factorization, the obstruction to
  extending the scalar projection is the first nonzero homogeneous
  component of
  \[
...
\begin{defn}[Balanced Costello graph/counterterm subcomplex]
\label{def:app-balanced-costello-graph-subcomplex}
  ...
  \[
    ...
    \mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}},\operatorname{str}}(I)
  \]
  is a balanced graph subcomplex if it is a complete
...
\label{prop:app-balanced-costello-graph-preservation}
  Let \(V=\C^{N|N}\).  Every balanced graph subcomplex
  \(\mathfrak G^\bullet_{\mathrm{bal,Cost}}(I)\) of
  Definition~\ref{def:app-balanced-costello-graph-subcomplex} is a
  balanced scalar-contact subcomplex in the sense of
  Definition~\ref{def:app-balanced-scalar-contact-subcomplex}.  Hence the
  zero projection
  \[
...
\begin{proof}
  By Definition~\ref{def:app-balanced-costello-graph-subcomplex}, each
  ...
  differential creates scalar-contact symbols only in the identity-loop
  class.  Therefore
  \[
...
\label{thm:app-costello-graph-extension-obstruction}
  ...
  \(\mathfrak G^\bullet_{\mathrm{Cost}}(I)\) is the
  ...
  \[
    ...
    \right).
  \]
  If \(s'\) is another filtered lift with
  \(s'-s=h\) in filtration degree \(-r\), then the degree-\((-r)\)
  component of the chain-map defect changes by
  \[
    \delta_{s'}^{(-r)}-\delta_s^{(-r)}
    =
    d_{\mathrm{CE}}h-h\,d_{\operatorname{gr}},
  \]
  the differential in
  \(\operatorname{Hom}_{-r}(
  \operatorname{gr}_{F}\overline{\mathfrak G}^\bullet_{\mathrm{Cost}}(I),
  S^\bullet)\).  Hence the displayed cohomology class is independent
  of the lift.  A filtered chain projection extending the balanced
  ...
  \]
  Thus a graph or counterterm term beyond the identity-loop
  superdimension class is admitted by the balanced theorem exactly after
...
\begin{proof}
  ...
  scalar-contact subcomplex.  If
  \(d_{\mathrm{CE}}\bar\sigma_{\mathrm{Cost}}
    -\bar\sigma_{\mathrm{Cost}}d_{\operatorname{gr}}\)
  is nonzero, then the associated-graded scalar extractor has a chain-map
  defect, and every filtered chain lift carries that defect in its
  associated graded.  This is
  the zeroth defect
  \(\delta^{(0)}_{\mathrm{Cost},\sigma}(I)\).
  ...
  homogeneous graph or counterterm generator, with the
  ...
  given Costello graph differential, orientation signs, and scalar
  symbols.  If all added terms satisfy
  Definition~\ref{def:app-balanced-costello-graph-subcomplex}, then
...
\label{prop:app-first-nonidentity-loop-scalar-symbol}
  ...
  containing the polynomial Hamiltonian arguments under discussion, and let
  ...
  Then
  \(X_{\Gamma^{T_\bullet}_{1,M}}(a,f;b,g)\in F^1\), its leading term has
  \(F^1/F^2\) coefficient equal to the displayed scalar, and
  \[
    X_{\Gamma^{T_\bullet}_{1,M}}(a,f;b,g)
      =
  ...
  \]
  Thus this is the first scalar symbol beyond the identity-loop
...
\begin{proof}
  ...
  \(F^1\).  The one-edge graph has exactly one scalar contact, hence its
  associated graded is read in \(F^1/F^2\).  The boundary Wick
  ...
  The open index loop is computed in a homogeneous basis
  ...
  Suppose the stated marked-loop subcomplex has only supertraceless
  monodromy words, before and after applying \(d_{\mathrm{QME}}\).
  The formula above then gives zero associated scalar symbol on every
  graph and counterterm generator and on every associated-graded
  boundary term.  Hence
  \(d_{\mathrm{CE}}0-0\,d_{\mathrm{QME}}=0\), so the zero scalar
  projection is a filtered chain map on that subcomplex.
\end{proof}

\begin{defn}[Supertraceless-monodromy filtered subspace]
\label{def:app-supertraceless-monodromy-precomplex}
  ...
  \]
  over all filtration degrees \(q\).  Write
  \(\mathcal B_{\operatorname{st}=0}=0\) when every class in this family
  vanishes.
  If \(\mathcal B_{\operatorname{st}=0}=0\), this filtered subspace is a complete
  \(d_{\mathrm{QME}}\)-stable filtered subcomplex; in that case it is
  the supertraceless-monodromy graph/counterterm subcomplex and
  is denoted
...
\label{thm:app-supertraceless-monodromy-projection}
  ...
  Definition~\ref{def:app-supertraceless-monodromy-precomplex}.  The
  filtered subspace is preserved by \(d_{\mathrm{QME}}\) if and only if
  \[
  ...
  vanish on the supertraceless-monodromy subcomplex.

  More explicitly, let \(X_{\Gamma,M}\in F^p\) be a homogeneous
  generator of the filtered subspace and suppose that, modulo \(F^{p-r+1}\),
  \[
    d_{\mathrm{QME}}X_{\Gamma,M}
      =
  ...
  the boundary terms.  The first possible preservation obstruction is the
  quotient class
  \[
  ...
  \(d_{\mathrm{QME}}\)-preservation, and its scalar projection is precisely
  ...
  Definition~\ref{def:app-balanced-costello-graph-subcomplex}, the
  obstruction \(\mathcal B_{\operatorname{st}=0}\) vanishes.  These
  generators carry only the empty monodromy word, and the displayed
  differential \(Q+\{I_0,-\}\) contains only the identity-loop marker.
  Therefore applying
  \(d_{\mathrm{cl}}=Q+\{I_0,-\}\) to this displayed Hamiltonian
  generator set preserves supertraceless monodromy.  Preservation by the
  full scale-\(L\) differential \(d_{\mathrm{QME}}^{L}\) further requires
  that \(\hbar_{\mathrm{QME}}\Delta_L\) and \(I[L]-I_0\) create only empty
  identity-loop scalar contacts with zero supertrace; equivalently, the
  Costello specialization has \(\mathcal B_{\operatorname{st}=0}=0\).
  A larger specialization with marked idempotents, holonomies, or
  counterterm tensors is governed by
  \(\mathcal B_{\operatorname{st}=0}\) on those added generators.
\end{thm}
...
\begin{proof}
  ...
  \(d_{\mathrm{QME}}\)-boundary of every generator of the filtered subspace has
  ...
  On the preserved subcomplex every scalar-contact loop word
  \(T_\bullet\) satisfies
  ...
  The formula for
  \(\mathcal B^{(r)}_{\operatorname{st}=0}(X_{\Gamma,M})\) is the
  ...
  carries the empty word \(\varnothing\), so its
  coefficient is
  \(\operatorname{Str}(\operatorname{id}_V)=0\).  The differential
  \(Q+\{I_0,-\}\) acts on BV fields and takes brackets with the
  unmarked classical interaction; it preserves the empty marker word on an
  open scalar loop.
  Thus the image of the unmarked Hamiltonian generator set again has only
  empty loop words.  The identity-loop graph subcomplex has the same
  property by
  Definition~\ref{def:app-balanced-costello-graph-subcomplex}, which
  requires the same factorization after applying \(d_{\mathrm{QME}}^L\).
  This proves the \(d_{\mathrm{cl}}\)-preservation result on the
  displayed generator set.  The full quantum statement follows precisely
  when the \(\Delta_L\) and \(I[L]-I_0\) contributions satisfy the same
  identity-loop condition; arbitrary marked graph/counterterm tensors
  remain governed by \(\mathcal B_{\operatorname{st}=0}\).
\end{proof}
...
\label{prop:app-marked-bst0-equivariant-differential}
  ...
  equivariance on open Chan-Paton labels.  A complete computation
  of \(\mathcal B_{\operatorname{st}=0}\) for a marked
  Costello graph/counterterm enlargement requires, for every homogeneous
  generator \(X_{\Gamma,M}\in F^p\), a marked boundary expansion
  \[
    d_{\mathrm{QME}}X_{\Gamma,M}
      =
    \sum_{\Gamma',\lambda}
  ...
  \[
    ...
    \right]_{\operatorname{st}\neq0,\;F^{p-r}/F^{p-r+1}},
  \]
  and its two-Hamiltonian-argument scalar projection is
  \[
    \hbar
    \sum_{\Gamma',\lambda}
      \varepsilon_{\Gamma,\Gamma',\lambda}\,
  ...
  On the displayed unmarked Hamiltonian generator set the
  differential is \(Q+\{I_0,-\}\), with unmarked classical interaction;
  the balanced graph/counterterm subcomplex only allows identity-loop
  scalar contractions before and after \(d_{\mathrm{QME}}\).  Hence
  \[
    \mathcal B_{\operatorname{st}=0}=0
  \]
  on that generator set.  For any larger marked Costello
  specialization equipped with an oriented finite-window marked
  boundary complex as in
  Definition~\ref{def:app-oriented-full-equivariant-marked-boundary-complex}
  has the scalar computation below.  For a larger marked specialization,
  the missing datum is exactly equation~\eqref{eq:app-marked-differential-datum},
  including its signs and invariant marker tensors.
  ...
  when \(T=\lambda\operatorname{id}_V\).  Its scalar projection is then
  \[
  ...
  enlargement.  In the full
  \(\lie{gl}(N|N)\)-equivariant Costello differential, an allowed
  single-marker term is a scalar multiple of the identity.  If one imposes
  only the even block-diagonal group, or gives external marker
  tensors, the admissible datum is the invariant tensor family above, and
  the first nonzero scalar projection is detected exactly by the displayed
...
\begin{proof}
  ...
  Definition~\ref{def:app-supertraceless-monodromy-precomplex} applied to
  the marked boundary expansion
  equation~\eqref{eq:app-marked-differential-datum}.  The scalar-projection
  ...
  elementary endomorphisms gives \(\lambda_0=\lambda_1\).  Hence
  \(T=\lambda\operatorname{id}_V\).  The cyclic supertrace is then
  \(\lambda\operatorname{sdim}(V)=0\) for \(V=\C^{N|N}\).

  On the unmarked Hamiltonian generator set the associated-graded
  differential \(d_{\mathrm{cl}}=Q+\{I_0,-\}\) carries the empty endomorphism
  label; together with
  Definition~\ref{def:app-balanced-costello-graph-subcomplex}, which
  requires identity-loop factorization after applying
  \(d_{\mathrm{QME}}\), these terms have cyclic supertrace
  ...
  boundary complex; equation~\eqref{eq:app-marked-differential-datum}
  records the residual data.
\end{proof}

\begin{defn}[Finite-window marked Costello list]
\label{def:app-finite-window-marked-costello-list}
  Fix a finite-window graph/QME system for the local mixed HT
  brane-defect theory,
  \((I,w,M,\mathcal G_M)\) in the sense of
  Definition~\ref{defn:finite-window-graph-qme-system}.  A finite-window
  ...
  \begin{enumerate}[label=(L\arabic*)]
    ...
    vertices \(C^M_{\Gamma,w}(\epsilon;B^\Gamma_\bullet)\) given by
    the system;
  ...
  \(\varepsilon_{\beta_i}\in\C[[\hbar]]\), a resulting graph
  ...
  evaluation on fixed CE cycles this extra face evaluates to zero.

  The finite list is required to contain every result of (B1)--(B4) that
  appears in the chosen system, and also the source face when the
  Hom-valued kernel table is used.  If a resulting graph, coefficient,
  marker transport, or counterterm vertex lies outside
  \(\mathcal G^{\mathrm{mk}}_M\), that boundary entry is the omitted datum;
  the larger specialization contains an unconstructed primitive class.
  ...
  finite list and, after the orientation-line isomorphism induced by the
  two ordered boundary bases
  \((\beta_i,\beta_{j|i})\) and \((\beta_j,\beta_{i|j})\),
  \[
  ...
  \(\Gamma\), the finite window consists of the finitely many
  ...
  \]
  Here \(\mathcal S_M\) is the finite seed set of graph and
  counterterm graphs, and the codimension-two closure equations make
  \(\partial_J\) independent of the chosen order of \(J\).  A defect in
  this displayed equality is precisely the finite graph-list obstruction.
\end{defn}
...
\begin{proof}
  ...
  result obtained from \(\beta_i\) and \(\beta_j\), \(i<j\), appears
  ...
  resulting graph, and marker tensor.  Hence the two terms cancel.  Summing
  over all unordered pairs gives \(d_{\mathrm{mk}}^2=0\).
\end{proof}

\begin{prop}[Finite-window marked-complex compatibility]
\label{prop:app-finite-window-marked-subcomplex-compatibility}
  Let \((I,w,M,\mathcal G_M)\) be a finite-window graph/QME system for
  ...
  \(\kappa^M_{\mathcal G,w,I}\) and the local counterterms of the system
  land in this marked complex exactly when their graph components and
  ...
  marked complex precisely when they send the finite boundary table,
...
\begin{proof}
  ...
  Definition~\ref{defn:finite-window-graph-qme-system}: regular-density
  ...
  the same finite label list.  Each omitted graph component or counterterm
  face is an explicit missing datum for the equality with \(d_M\).  The finite-window system requires compatibility of
...
\begin{proof}
  ...
  cyclic multiplication identifies the outgoing leg of the \(i\)-th
  endomorphism with the argument of the \(\tau_m(i)\)-th endomorphism,
  while \(\mathfrak C_\sigma\) identifies it with the argument of the
...
\begin{thm}[Full-equivariant marked scalar projection vanishing]
\label{thm:app-full-equivariant-marked-shadow-vanishing}
  Let \(V=\C^{N|N}\).  Let
  \(\mathfrak G^\bullet_{\mathrm{mk,full}}(I)\) be a complete
  finite-window inverse-limit graph/counterterm complex for the
  ...
  Proposition~\ref{prop:app-finite-window-marked-subcomplex-compatibility}.
  This is a finite-list statement in every window.  Then the marked
  preservation obstruction has zero
  cyclic-supertrace projection:
  \[
    \operatorname{Str}_{\mathrm{cyc}}
  ...
  \(X_{\Gamma,o,\mathfrak m,M}\in F^p\), the first scalar CE projection is
  \[
    \hbar
    \sum_{i:\,r(\beta_i)=r}
      (-1)^{i-1}
...
\begin{proof}
  ...
  gives zero cyclic supertrace for every such resulting tensor when
  \(V=\C^{N|N}\).

  Formula~\eqref{eq:app-oriented-marked-first-lowering} gives the
...
\label{def:app-nonscalar-kernel-row-complex}
  ...
  filtered chain map.  In the full-equivariant marked complex of
  ...
  of Definition~\ref{defn:finite-window-graph-array}.  If the scalar condition
  vanishes,
  \[
...
\begin{defn}[Finite-window realization complex]
\label{def:app-native-finite-window-realization-complex}
  A Costello/QME realization complex for the mixed HT brane-defect
  model on
  \[
    \R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}
  \]
  is the following finite-window datum.  First, one fixes a graph/QME
  system \((I,w,M,\mathcal G_M)\) in the sense of
  Definition~\ref{defn:finite-window-graph-qme-system}, a
  ...
  \]
  Thus the source-face term is part of the Hom differential.  For a degree-zero kernel it
  ...
  \(K^M_\Gamma=W^{R,M}_{\Gamma,L,w}(B^\Gamma_\bullet)\) given by the
  system.  A graph theorem for arbitrary \(B\in\mathcal D'_c(I)\) is
  ...
  \[
    ...
    \ker\widehat\sigma_{\mathrm{sc},M}.
  \]
  The non-scalar kernel complex is formed after this projection is a
  chain map.
\end{defn}
...
\label{thm:app-constructive-nonscalar-costello-qme-realization}
  Work in a finite-window realization complex of
  Definition~\ref{def:app-native-finite-window-realization-complex}.
  Suppose that the finite-window QME residual has been killed through
  order \(\hbar^{n-1}\) by chosen counterterms \(C_{<n,M}\).  The
  order-\(n\) realization is decided by
  the following finite row construction.
  ...
  \[
    \mathfrak s_{n,M}
      =
    \sum_{\alpha\in\mathsf A^{\mathrm{fw}}_{n,M}}S_{\alpha,M}=0.
    \label{eq:app-native-scalar-condition}
  \]
  The non-scalar problem at order \(n\) begins after
  \eqref{eq:app-native-scalar-condition} holds.  Then
  \[
    R^{\mathrm{ns}}_{n,M}
      =
    \sum_{\alpha\in\mathsf A^{\mathrm{fw}}_{n,M}}
  ...
  \(\rho^M_1,\ldots,\rho^M_a\) and degree-zero counterterm primitives
  \(\eta^M_1,\ldots,\eta^M_b\) inside the same finite system, and write
  \[
  ...
  \]
  in the completed brane-defect kernel complex.  A larger
  Costello specialization requires its missing graph weight, source
  face, scalar condition, primitive column, or truncation matrix
  as explicit data for the criterion.
\end{thm}
...
\begin{proof}
  ...
  Definition~\ref{def:app-native-finite-window-realization-complex}.  The
  last displayed family is the order-\(n\) contribution of lower
  counterterms to the same curved Maurer--Cartan equation.

  Applying the chain scalar projection gives the scalar condition.  A
  nonzero scalar condition places the residual outside
  \(\ker\widehat\sigma_{\mathrm{sc},M}\).  A zero scalar condition places the
  residual in the kernel complex.  Codimension-two closure is exactly
  the finite-row form of the Bianchi identity: the two ways of taking two
  elementary faces have opposite incidence signs and the same resulting graph,
  ...
  \(d_{\mathrm{ns},M}C_{n,M}\).  Expanding \(C_{n,M}\) in the given
  degree-zero row basis gives the linear system
  \eqref{eq:app-native-counterterm-linear-system}.  The solvability
  obstruction is the residual vector class in the cokernel of \(A^M\);
  a left-null covector with nonzero value on \(r^M\) is precisely the
  finite-dimensional cokernel separation.  The two truncation identities
  say that both residuals and boundaries commute with finite-window
  projection.  After windowwise primitives are chosen, their compatibility
  with truncation is measured by the displayed Roos \(1\)-cocycle in
...
\begin{prop}[Closed rows and the first given open source-face row]
\label{prop:app-closed-rows-and-theta-three-source-face}
  ...
  \(\alpha_{\mathrm{sc}}\) with arguments \((z_1,z_2)\) has
  \[
  ...
  \]
  The labelled scalar-zero row \(\alpha_{11}\) with arguments
  \((z_1,z_1)\) has
  \[
  ...
  The first nonzero scalar-zero source-face row entering the row
  machinery is the order-three symbolic row
  ...
  \(d_{\mathrm{ns},M}\mathsf E_{\theta_3,M}=0\) holds, it defines
  \[
  ...
  \]
  detects a nonzero finite-row obstruction.  To remove it one must construct
  either a degree-zero local counterterm
  \(C_{\theta_3,M}\) with
  \[
  ...
  companion rows whose sum changes the residual vector.  These data make
  the order-three larger system a solved QME row.
\end{prop}
...
\begin{proof}
  ...
  Proposition~\ref{prop:concrete-order-three-enlarged-system-row}.  Its
  scalar condition is zero in the full-equivariant marked complex.  In the
  one-row finite complex, the degree-zero primitive space is zero,
  so the boundary matrix is the \(1\times0\) zero matrix and the residual
  vector is the basis vector \(\mathsf E_{\theta_3,M}\).  The displayed
  covector annihilates the image of the differential and pairs
  nontrivially with the residual.
  This proves nonvanishing only in the given finite row subcomplex.
  The displayed \(C_{\theta_3,M}\) or companion-row datum is exactly what
  enlarges the complex enough to kill the row.
\end{proof}

\begin{prop}[Bianchi-exposed lower condition for \(\theta_3\)]
\label{prop:app-theta-three-bianchi-lower-condition}
  In the lower window \(N=2\), let
  \(E^2_{\nu_{02}}\) and \(E^2_{\nu_{20}}\) be the two transported
  source-face rows attached to the \(\theta_3\) source differential.  The
  canonical source-coordinate primitive is
  \[
  ...
  \[
    A_D^2c=-r_2
  \]
  has a cokernel obstruction in the given lower local-functional row complex.  The
  normalized cokernel
  \[
  ...
  \]
  One must therefore add a genuine scalar-zero local-functional
  column
  \[
    A_B^2=(0,0,-1)^T,\qquad
  ...
  \]
  possibly after an explicitly given scalar-zero transport correction.
  ...
  \[
    ...
    H^0(\mathcal K^\bullet_{\mathrm{ns},N}(I))
  \]
  must vanish next.  These two conditions promote the lower \(\theta_3\)
  Bianchi primitive from a fixed-window matrix target to a compatible
  finite-window Costello/QME tower datum.
\end{prop}

\begin{proof}
  The source differential gives the two lower source-face rows with
  coefficients \(2\) and \(-2\).  Applying \(K^2_{\Theta_3}\) becomes a
  chain map after its Hom-valued Bianchi defect is killed, and
  \[
  ...
  If \(A_D^2c=-r_2=(-2,2,0)^T\), the first two coordinates give
  \(c=1\), while the Bianchi coordinate gives \(c=0\).  Thus the equation
  is inconsistent.  The same conclusion is the cokernel calculation:
  \(\widetilde\lambda_{02,\mathfrak b}\) annihilates \(A_D^2\) and pairs
  nontrivially with \(r_2\).
  ...
  \(-1\) on it, this boundary is a new scalar-zero local-functional
  column.
  ...
  This is only a formal finite-window primitive until the local functional
  and its admissibility are fixed.
  ...
  family exists after this degree-one class vanishes in
  \(H^1(\mathcal K^\bullet_{\mathrm{ns},N}(I))\).  After choosing
  primitives \(B^N\), set
  \(A_N=H^0(\mathcal K^\bullet_{\mathrm{ns},N}(I))\) and use the
  successor Roos map
  \[
    \Delta((a_N)_N)
      =
      \bigl(a_N-\pi_{N+1,N}a_{N+1}\bigr)_N
      \colon \prod_N A_N\longrightarrow\prod_N A_N .
  \]
  The primitive-difference classes
  \([\pi_{N+1,N}B^{N+1}-B^N]\in A_N\) define the compatibility class in
  \(\operatorname{coker}\Delta=\varprojlim\nolimits^1_NA_N\).
\end{proof}

\begin{thm}[First non-scalar finite-window \(H^1\) at the
  \(\theta_3\) row]
\label{thm:app-theta-three-finite-window-H1}
  Work in the full-equivariant marked finite-window complex with
  \(V=\C^{N|N}\) and the given minimal generator set at order \(\le2\)
  of Theorem~\ref{thm:app-minimal-full-equivariant-all-order-vanishing}.
  Adjoin only the order-three CE-source row \(\theta_3\) of
  Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}.
  The non-scalar kernel subcomplex at \(\theta_3\) is the given
  finite row complex
  \[
    \mathcal K^0_{\theta_3,M}=0,\qquad
    \mathcal K^1_{\theta_3,M}=\C\,\mathsf E_{\theta_3,M},\qquad
  ...
  \begin{enumerate}[label=(\alph*)]
    \item \emph{Counterterm omitted.}  In the
      one-row complex above, the \(d_{\mathrm{ns},M}\)-cohomology in
      degree one is
      \[
        H^1(\mathcal K^\bullet_{\theta_3,M},d_{\theta_3,M})
          =\C\cdot\mathsf E_{\theta_3,M}\neq0,
      \]
      ...
      \[
        [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}}}]
          ...
          =[\mathsf E_{\theta_3,M}]\neq0,
      \]
      detected, after fixing the row basis
      \(\mathsf E_{\theta_3,M}\), by the dual covector
      \(\ell_{\theta_3}(\mathsf E_{\theta_3,M})=1\).
      The given finite complex has zero degree-zero row space; the linear system
      \(A^M c=-r^M\) of
      Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
      has empty column matrix and residual
      \(r^M=\mathsf E_{\theta_3,M}\), so it is unsolvable.
    \item \emph{Scalar-zero local counterterm \(C_{\theta_3,M}\).}
      Suppose a degree-zero local functional
      \(C_{\theta_3,M}\in\mathcal K^0_{\theta_3,M}\) with
      \(\widehat\sigma_{\mathrm{sc},M}(C_{\theta_3,M})=0\) exists
      so that, in the enlarged finite complex
      \[
        \widetilde{\mathcal K}^0_{\theta_3,M}
          =\C\,C_{\theta_3,M},\qquad
        \widetilde{\mathcal K}^1_{\theta_3,M}
          =\C\,\mathsf E_{\theta_3,M},\qquad
        \widetilde d\,C_{\theta_3,M}=-\mathsf E_{\theta_3,M},
      \]
      the differential matrix is the scalar \(-1\).  Then
      \[
        H^1(\widetilde{\mathcal K}^\bullet_{\theta_3,M},\widetilde d)
          =0,\qquad
        [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}}}]
          \big|_{\theta_3,M}=0,
      \]
      and, after fixing \(C_{\theta_3,M}\) as the basis of the added
      degree-zero row, the coefficient \(c=1\)
      solves the row equation \(\widetilde A^M c=-r^M=
      -\mathsf E_{\theta_3,M}\).  Tower compatibility holds with the
      identity primitive transport
      \(P^{M,N}=1\), zero Roos class
      \([P^{M,N}c_M-c_N]=0\), and zero Bianchi tower obstruction
      \(\Delta^1_{M,N}=0\) when the same \(C_{\theta_3,M}\) is
      retained at all windows containing \(\theta_3\).
  \end{enumerate}

  Each case is the result of the fixed-window matrix solver of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
  on the corresponding row complex.
\end{thm}

\begin{proof}
  Case (a).  The given finite complex has empty
  \(\mathcal K^0_{\theta_3,M}=0\), so the differential matrix
  \(\bigl(d_{\mathrm{ns},M}\big|_{\mathcal K^0_{\theta_3,M}}\bigr)\)
  is the empty \(1\times0\) matrix and the residual is
  \(r^M=\mathsf E_{\theta_3,M}\).  The scalar gate
  \(S_{\theta_3,M}=0\) holds by
  Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face},
  so the row lives in the kernel complex.  Since
  \(d_{\theta_3,M}=0\) on the one-element basis, the row is closed.
  Its cohomology class equals the basis vector itself, and the dual
  covector \(\ell_{\theta_3}(\mathsf E_{\theta_3,M})=1\) annihilates
  the (empty) image of \(d_{\theta_3,M}\) but is nonzero on
  \(\mathsf E_{\theta_3,M}\).  This is the cokernel separation of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}.

  Case (b).  Adjoining the degree-zero \(C_{\theta_3,M}\) with
  \(\widehat\sigma_{\mathrm{sc},M}(C_{\theta_3,M})=0\) keeps both rows
  in the non-scalar kernel.  The differential matrix is now the
  scalar \(\widetilde A^M=(-1)\), so
  \(\widetilde A^M c=-1\cdot c\) and \(\widetilde A^M c=-r^M=-1\)
  has unique solution \(c=1\).  The H¹ class is therefore zero.
  Tower compatibility: with identity transport
  \(\pi_{M,N}C_{\theta_3,M}=C_{\theta_3,N}\) on every window
  containing \(\theta_3\), the Roos cocycle
  \(P^{M,N}c_M-c_N=1-1=0\), and the Bianchi tower obstruction
  \(\Delta^1_{M,N}=-\pi_{M,N}\mathfrak b^M+\mathfrak b^N=0\) by the
  same identity transport on \(\mathsf E_{\theta_3}\).

  Both cases are stable under finite-window truncation since the
  scalar gate \(S_{\theta_3,N}=0\) is preserved (the cyclic supertrace
  vanishes at every window), and the residual transports as
  \(\pi_{M,N}\mathsf E_{\theta_3,M}=\mathsf E_{\theta_3,N}\) when both
  windows contain \(\theta_3\) and as \(0\) otherwise.
\end{proof}

\begin{rmk}[Finite-window computation at the \(\theta_3\) row]
\label{rmk:app-theta-three-finite-window-verification}
  The finite-window matrix calculation gives the dichotomy of
  Theorem~\ref{thm:app-theta-three-finite-window-H1}.  If the
  Chevalley--Eilenberg ancestor and the scalar-zero counterterm are
  both omitted, \(\mathsf E_{\theta_3,M}\) has no primitive and is
  separated by the cokernel covector
  \(\ell(\mathsf E_{\theta_3,M})=1\).  If one supplies a scalar-zero
  counterterm, a Chevalley--Eilenberg ancestor, or the complete
  companion-face table, the primitive exists with coefficient
  \([\![C_{\theta_3,M}^{\mathrm{ct}},1]\!]\),
  \([\![C_{\theta_3,M}^{\mathrm{CE}},1]\!]\), or the corresponding
  unit-coefficient signed companion sum.
\end{rmk}

\begin{rmk}[Dichotomy at \(\theta_3\)]
\label{rmk:app-theta-three-dichotomy-sub-theorem}
  The two cases of
  Theorem~\ref{thm:app-theta-three-finite-window-H1} apply under
  disjoint algebraic conditions on the order-three finite-row complex
  at \(\theta_3\).  Case (a) is the minimal-generator case: when the
  order-three scalar-zero counterterm
  \(C_{\theta_3,M}\) is omitted, the finite-window
  \(d_{\mathrm{ns},M}\)-cohomology in degree one is
  \(\C\,\mathsf E_{\theta_3,M}\), with explicit covector
  representative \(\ell_{\theta_3}\).  Case (b) is the
  counterterm-extended complex: when \(C_{\theta_3,M}\) with
  \(\widehat\sigma_{\mathrm{sc},M}(C_{\theta_3,M})=0\) and
  \(d_{\mathrm{ns},M}C_{\theta_3,M}=-\mathsf E_{\theta_3,M}\) is
  given, the finite-window \(H^1\) vanishes.  A named coefficient
  \(C_{\theta_3}\) is admissible exactly in case (b), with one
  of the three primitive data --- CE ancestor with controlled boundary,
  scalar-zero local counterterm \(C_{\theta_3,M}\), or complete
  companion-face table whose signed sum moves the residual into the
  image of \(\widetilde A^M\)
  (Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}).
\end{rmk}
...
\label{def:app-normal-omega-costello-kernel-datum}
  ...
  displayed operator is a curved Cartan operator with square \(L_{V_\Omega}\).
  ...
  chosen \(N\)-brane, \(M\)-window system.  The coefficient ring is
  \[
    R_\Omega^{N,M}
      =
  ...
  \(\hbar_{\mathrm{W}}=\hbar_\omega\).  Identification of
  \(\hbar_{\mathrm{QME}}\) with either parameter is an explicit theorem
  specialization.  The equivariant Hamiltonian bracket is the
  ...
  \((I,w,M,\mathcal G_M)\) is a finite-window marked Costello system as in
  Definitions~\ref{defn:finite-window-graph-qme-system}
  ...
  \begin{enumerate}[label={(\(\Omega\arabic*\))}]
    ...
    \item For every localized normal character
      ...
      \[
        ...
        \operatorname{id}-\operatorname{pr}_{\chi=0}
	      ...
	      \(\chi\).  This is the normal-mode acyclicity datum.  Graph
	      curvature classes and QME counterterms are independent
	      finite-row data.
    ...
    \item An equivariant finite-scale gauge fixing and propagator
      ...
      finite-window weighted bounds required by the nonequivariant system
    ...
    \item A named normal-localization normalization
      ...
      normalizations are always read with the selected branch.  A comparison
      between the residue branch, Euler branch, and Euler-rescaled residue
      branch must state the real orientation sign \(\sigma_s\) and every
      denominator.
    ...
    \item An associated-graded scalar-symbol extractor
      \[
      ...
      \(\widehat\sigma_{\mathrm{sc},\Omega,M}\) of
      \(\sigma_{\mathrm{sc},\Omega,M}\) exists exactly when the
      Hom-obstruction test of
      Theorem~\ref{thm:app-equivariant-brane-defect-qme-criterion}
      vanishes.
  ...
  \end{enumerate}
  This datum gives the equivariant structure: coefficient ring,
  \(Q_\Omega\), normal contractions, and equivariant propagator data.
  Equivariant QME and centrality obstructions are formed inside this
  structure by the criterion of
  Theorem~\ref{thm:app-equivariant-brane-defect-qme-criterion}.
\end{defn}

\begin{thm}[Equivariant brane-defect QME obstruction criterion]
\label{thm:app-equivariant-brane-defect-qme-criterion}
  Fix a normal \(\Omega\)-equivariant finite-window system
  ...
  chain projection exactly when the following relative Hom
  ...
  \]
  Suppose the Hom-valued kernel of item \((\Omega7)\) is fixed, and
  ...
  Therefore the normal \(\Omega\)-background determines the
  equivariant structure.  A QME proof requires, in addition, the chain
  scalar projection, the
...
\begin{proof}
  ...
  cochain \(h\) changes the defect by
  \(D_\Omega(h)\).  Completeness gives the successive criterion and the
  compatibility condition in the inverse finite-window tower.
  ...
  counterterms.  Applying the scalar projection gives the scalar condition.  If
  the condition vanishes and the lower-order equations hold, the equivariant
  ...
  is its \(H^1\)-class.  For compatible primitives set
  \(A_M=H^0(\mathcal Q^\bullet_{w,\Omega,M}(I))\) and
  \[
    \Delta_\Omega((a_M)_M)
      =
      \bigl(a_M-\pi_{M+1,M}a_{M+1}\bigr)_M .
  \]
  The successor differences of any chosen primitive family represent a
  class in
  \(\operatorname{coker}\Delta_\Omega
  =\varprojlim\nolimits^1_M A_M\), and its vanishing is the
  compatibility condition of
  ...
  complex.  The curvature residual boundary, scalar-projection chain data,
  and source-row centrality homotopies are additional QME data.
\end{proof}
...
\label{thm:app-minimal-full-equivariant-kernel-vanishing}
  ...
  Proposition~\ref{prop:first-finite-window-array-rows}.  Assume that the
  list consists exactly of these rows through codimension two: it contains
  the \(|\Gamma|_\hbar=1\) scalar-contact row and its codimension-two
  closure, and it has zero additional \(|\Gamma|_\hbar=1\) graph rows,
  zero genuine \(|\Gamma|_\hbar=2\) seed graphs, zero order-\(2\) CE-source
  faces, and zero order-\(2\) graph counterterm faces.
  ...
  \]
  Thus the minimal full-equivariant finite-window array has zero genuinely
  ...
  a larger finite marked list gives an additional scalar-zero row.
\end{thm}
...
\begin{proof}
  ...
  and their resulting graphs, coefficients, and marker transports are
  ...
  same cyclic supertrace calculation.  The minimal list has zero genuine
  order-\(2\) seed graph, CE-source face, or counterterm face, so the
  kernel complex has zero remaining row.  Thus \(R^{\mathrm{ns}}_{2,M}=0\), and the
...
\label{prop:app-first-missing-nonscalar-row}
  ...
  first genuinely non-scalar obstruction beyond scalar
  ...
  \[
    ...
    S_{\alpha,M}=0,
  \]
  with its row value \(R^{\mathrm{row}}_{\alpha,M}\).  Equivalently,
  ...
  marker tensor, incidence sign, scalar condition, and truncation coefficients
  \(t^{M,N}_{\alpha\alpha'}\).

  If the finite marked list explicitly has zero such order-\(1\) rows, the
  next required datum is the order-\(2\) scalar-zero row sum
  \[
  \begin{aligned}
    R^{\mathrm{ns}}_{2,M}
      ={}&
  ...
  \end{aligned}
  \]
  These row values, signs, scalar conditions, and truncation coefficients
  define the non-scalar obstruction class and counterterm problem beyond
  the minimal vanishing theorem above.
\end{prop}

\begin{proof}
  The scalar projection is a chain map on the full-equivariant marked
  complex by Theorem~\ref{thm:app-full-equivariant-marked-shadow-vanishing},
  so its kernel is a \(d_M\)-stable complex.  After the scalar condition
  ...
  order-\(1\) non-scalar class can only be given by an additional
  ...
  add to \(1\).  Each is a row only after the finite graph list gives
  its labels, signs, marker transport, scalar condition, and truncation
  coefficients.

  With zero order-\(1\) rows, the next possible residual is the
  order-\(2\) curvature expansion, including genuine order-\(2\) graph
  components, CE-source faces, brackets of order-\(1\) components, and
  lower-counterterm terms.  These data are exactly the row entries required by
...
\label{prop:app-first-scalar-zero-order-one-row}
  ...
  \(\alpha_{11}\) with ordered Hamiltonian arguments
  \((a,z_1)\), \((b,z_1)\), the boundary Weyl edge \(E_{\mathrm{Weyl}}\),
  the scalar-contact vertex \(\gamma_{\mathbf 1}\), and the same
  full-equivariant marker complex as before.  Choose the orientation in
  ...
  Thus the finite closed graph-list axioms support explicit scalar-zero
  order-one rows beyond
  ...
  order-one class is encoded by the specified row
...
\begin{proof}
  ...
  \]
  Multiplying by the transported full-equivariant cyclic supertrace
  preserves this zero coefficient.  Hence both the local functional row and
  ...
  this added labelled zero row; a nonzero order-one class is decided by
  the nonzero order-one row values.
\end{proof}

\begin{rmk}[Order two after zero nonzero order-one rows]
\label{rmk:app-order-two-after-zero-nonzero-order-one-rows}
  If zero rows such as \(\alpha_{11}\) are suppressed and the finite marked
  list gives zero nonzero order-one rows outside
  ...
  finite system this displayed row sum is a cochain only after the
  genuine order-two graph weights, CE-source faces, bracket signs, scalar
  conditions, and truncation coefficients are fixed.
\end{rmk}

\begin{thm}[Minimal full-equivariant finite-list vanishing criterion]\label{thm:app-minimal-full-equivariant-all-order-vanishing}
  Work in the local
  \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\)
  ...
  the sense of
  Definition~\ref{def:app-finite-window-marked-costello-list}.  Assume that
  the finite-window system adds zero further positive-order seed graphs,
  CE-source faces, graph counterterm faces, or nonzero scalar-zero rows.

  Then, inside this minimal finite marked list, for every \(n\geq1\)
  \[
...
\begin{proof}
  ...
  \(K^M_{\alpha_{\mathrm{sc}}}=0\) as a local functional before scalar
  projection.  The proof is the cyclic-supertrace calculation of
  ...
  rows in the minimal system have at least one positive-order factor of this
  ...
  are zero.  The minimality hypothesis sets the genuine \(dK\), CE-source,
  and graph-counterterm row spaces to zero.  Therefore the order-\(n\)
  scalar-zero row sum
...
\begin{prop}[First order-three enlarged-system datum]\label{prop:app-first-order-three-enlarged-row}
  Keep the full-equivariant local branch, and suppose the minimal system has
  been closed through order two with
  \(C_{1,M}=C_{2,M}=0\) and zero nonzero order-one or order-two rows outside
  the scalar-contact closure.  Then a nonzero order-three obstruction
  requires row data beyond the finite graph-list axioms.  A nonzero order-three
  cochain exists only after the finite-window system gives at least one
  ...
  \]
  In a nonminimal system with given nonzero order-one and order-two
  components one must also give each bracket row
  \[
    R^{\mathrm{row}}_{b(\Gamma_1,\Gamma_2),M}
      =
    \frac12\varepsilon_{\Gamma_1,\Gamma_2}
  ...
  \]
  Only after this condition is fixed and vanishes is the non-scalar obstruction
  class defined:
  \[
...
\begin{proof}
  ...
  Theorem~\ref{thm:finite-window-nonscalar-curvature-criterion} with
  ...
  contribution is zero in the minimal system.  Every nonzero order-three
  cochain in this system therefore comes from a genuine order-three graph
  row, an order-three CE-source row, or a bracket row between already
  given nonzero lower-order components.  The displayed formulas are
  precisely the row values and scalar conditions required by
  ...
  Theorem~\ref{thm:finite-window-nonscalar-curvature-criterion}.
\end{proof}
...
\label{prop:app-marked-row-centrality-homotopy-test}
  ...
  the CE-source rows, the convolution bracket rows, and the given
  counterterm rows of the marked table.  The scalar condition is
  \[
  ...
  A marked finite-window system gives an unreduced
  centrality homotopy for the pair \((x,y)\) exactly when it also
  gives a degree \(|x|+|y|-1\) row
  ...
  Theorem~\ref{thm:app-curved-kernel-centrality-homotopy-criterion}.  The
  centrality-homotopy row and its compatibility identities are additional
  data beyond scalar-projection vanishing.
  ...
  \(K^M_{\alpha_{\mathrm{sc}}}=0\), and the marked table contains zero
  additional positive-order seed graph, CE-source face, graph counterterm
  face, or nonzero scalar-zero row.  Hence
  \[
    R^{\mathrm{cent}}_{x,y,M}=0,\qquad H^M_{x,y}=0
  \]
  is a valid homotopy inside that minimal finite marked complex.  This
  constructs the homotopy only for the minimal system.  An enlarged
  unreduced Costello graph/counterterm target must contain the displayed
  row values, scalar condition, primitive \(H^M_{x,y}\), and transition
  coefficients.
\end{prop}
...
\begin{proof}
  ...
  scalar condition.  If the condition vanishes, the row lies in the non-scalar
  kernel complex of
  ...
  over finite windows and admissible weights.  Thus a row omitted from
  the marked table is additional graph, source-face, counterterm, or
  homotopy data beyond the scalar projection.

  In the minimal full-equivariant system all positive-order scalar-contact
  rows are zero before cohomology by
  Theorem~\ref{thm:app-minimal-full-equivariant-all-order-vanishing}.
  Since the minimal hypothesis allows zero other positive-order rows, the
  centrality row is zero and the zero primitive is compatible with all
  truncation maps.  The final assertion is the contrapositive of the
  row formula: once an enlarged system adds a nonzero graph, source,
  bracket, or counterterm row, the corresponding scalar condition and primitive
...
\begin{proof}
  ...
  scalar condition is necessary because only its vanishing places the row in
  ...
  \(T_\Omega\)-invariance of the primitive is an independent condition
  beyond scalar-projection vanishing.  The inverse-limit
  assertion is the same primitive-compatibility argument as in
...
\label{prop:app-even-block-marker-obstruction}
  ...
  projections.  Its scalar projection is
  \[
    \operatorname{Str}(T)=N(\lambda_0-\lambda_1).
  \]
  For a two-Hamiltonian scalar contact the obstruction class is
  \[
    \hbar_{\mathrm{QME}}N(\lambda_0-\lambda_1)\,
    \omega(f,g)\gamma_{\mathbf 1},
    \qquad
    \hbar_{\mathrm{QME}}N(\lambda_0-\lambda_1)[\bar c]\,\gamma_{\mathbf 1}
    \text{ in cohomology}.
  \]
  Since \([\bar c]\neq0\) in the scalar-reduced source, this smaller
  subalgebra has zero \([\bar c]\)-projection for single markers if and only if
  \(\lambda_0=\lambda_1\), i.e. the marker is scalar and the subalgebra
  ...
  the supertraceless-monodromy submodule of
...
\label{thm:app-ordinary-glN-scalar-obstruction-criterion}
  ...
  \(W_{\mathrm{add}}\) be any additional formal-local closed-exchange or
  brane-defect graph contribution whose leading scalar associated
  graded is \(w_{\mathrm{add}}\in C^2_{\mathrm{Lie}}(\bar A;\C)\).  A
  cancellation
  \[
    \operatorname{Ob}_{\mathrm{sc}}
      +W_{\mathrm{add}}+d_{\mathrm{QME}}C=0
  \]
  ...
  \[
    [w_{\mathrm{add}}]=-\kappa_{\mathrm{QME}}
      ...
      H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar_{\mathrm{QME}}]].
  \]
  In particular, with \(W_{\mathrm{add}}=0\), or with
  \(w_{\mathrm{add}}\) exact, the ordinary scalar-reduced
  \(\lie{gl}_N\) brane-defect model carries the scalar QME obstruction
  \(\kappa_{\mathrm{QME}}\).
\end{thm}
...
\begin{proof}
  ...
  to
  \[
    \hbar_{\mathrm{QME}}N\omega+w_{\mathrm{add}}+\delta\bar\eta=0
  ...
  necessary condition.  If \(W_{\mathrm{add}}=0\), or if
  \(w_{\mathrm{add}}\) is exact, this condition implies
  \(\kappa_{\mathrm{QME}}=0\) in
  \(H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar_{\mathrm{QME}}]]\).
  For \(N>0\) and formal \(\hbar_{\mathrm{QME}}\),
  Theorem~\ref{thm:app-scalar-contact-qme-class} identifies
  \([\bar c]\neq0\) on \(\bar A\).  Thus the ordinary scalar-reduced
  obstruction survives every internal local counterterm.
\end{proof}

\begin{thm}[Unreduced cotangent lift obstruction criterion]
\label{thm:app-unreduced-cotangent-lift-obstruction}
  Fix a finite-window marked Costello system
  \((I,w,M,\mathcal G_M,\mathcal G^{\mathrm{mk}}_M)\) for the local
  mixed HT brane-defect theory and a filtered chain scalar projection
  \(\widehat\sigma_{\mathrm{sc},M}\)
  (Definition~\ref{def:app-native-finite-window-realization-complex}).
  Let
  \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\colon
  C^\bullet_{\mathrm{CE}}(\widetilde{\mathfrak g}_I)
  \to
  Z^{P_0}_{\mathrm{fact}}(\mathrm{Obs}_{\mathrm{open}})\)
  be a factorization-center morphism extending the
  algebraic current presentation
  \(\Phi_I^\chi\) of
  Construction~\ref{constr:app-algebraic-factorization-centre-lift}
  beyond the algebraic current target.  Define the obstruction
  vector
  \[
    \operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}
      =
  ...
  \begin{enumerate}[label=(\roman*)]
    \item \emph{Product/tensor-completion centrality.}
      \(\operatorname{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) is the
      \(H^{|x|+|y|}\)-class of the product centrality defect
      \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}(x)\Phi_{\mathrm{fact}}^{\mathrm{unred}}(y)
      -(-1)^{|x||y|}\Phi_{\mathrm{fact}}^{\mathrm{unred}}(y)\Phi_{\mathrm{fact}}^{\mathrm{unred}}(x)\)
      in
      \(\mathfrak Q^\bullet_{\mathcal G,M}(I)/\operatorname{im}(d_M)\).
      In the algebraic current model
      (Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies})
      this defect is zero on generators by graded commutativity; the
      obstruction measures the corresponding product-centrality defect
      on the enlarged unreduced target.
    \item \emph{\(P_0\) / shifted-cotangent refinement.}
      \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is the
      \(H^{|x|+|y|}\)-class of the \(P_0\)-bracket defect
      \(\{\Phi_{\mathrm{fact}}^{\mathrm{unred}}(x),\Phi_{\mathrm{fact}}^{\mathrm{unred}}(y)\}-
      \Phi_{\mathrm{fact}}^{\mathrm{unred}}(\delta_x y)\) in the same
      cohomology, evaluated as a finite row sum
      \[
        R^{\mathrm{cent}}_{x,y,M}
          =
        \sum_{\Gamma\in\mathsf A_{x,y,M}^{d}}
          ...
          \varepsilon^{\mathrm{CE}}_\Gamma
          K^M_\Gamma(\zeta_{\Gamma,x,y})
          + \tfrac12\sum_{(\Gamma_1,\Gamma_2)}
          \varepsilon_{\Gamma_1,\Gamma_2}
          \{K^M_{\Gamma_1},K^M_{\Gamma_2}\}_{\hbar,M}
          + R^{\mathrm{ct}}_{x,y,M}
      \]
      of the row machinery
      (Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}).
      A centrality homotopy \(H^M_{x,y}\) with
      \(R^{\mathrm{cent}}_{x,y,M}+d_M H^M_{x,y}=0\) is the
      construction-level witness; the obstruction is the cohomology
      class of the row before such a witness exists.
    \item \emph{Boundary QME obstruction.}
      \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}\) is the
      compatible inverse-system class
      \[
        \bigl(\mathfrak o^{\mathrm{ns}}_{n,M}\bigr)_M
          =
        \bigl([R^{\mathrm{ns}}_{n,M}]\bigr)_M
        \in
        \varprojlim_M
        H^1\bigl(\mathcal K^\bullet_{\mathrm{ns},M}(I),
                  d_{\mathrm{ns},M}\bigr)
      \]
      of Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization},
      together with the secondary
      \(\varprojlim^1H^0\) primitive-compatibility class
      \(\lambda_{n,M}\).  The first nontrivial finite-window value
      occurs at the \(\theta_3\) row
      (Theorem~\ref{thm:app-theta-three-finite-window-H1}).
    \item \emph{Č\v{e}ch--Roos / Weiss-versus-Ran descent gap.}
      \(\delta^{\mathrm{Weiss}}_{\check C/R}\) is the codimension-one
      face class of the descent comparison between the Weiss cover
      \(\mathcal U^{\mathrm{Weiss}}\) and the Ran-space cover
	      \(\mathcal U^R\): the first descent obstruction is the
	      Č\v{e}ch--Roos difference
	      \(\delta^{\mathrm{Weiss}}_{\check C/R}\) in the finite-window
	      \(\varprojlim^1 H^0\) of the corresponding boundary kernel.
	      Sufficiency requires all higher Č\v{e}ch/Ran compatibility
	      classes and transition homotopies.
  \end{enumerate}

  Then vanishing of
  \(\operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}\)
  is necessary for an unreduced
  \(Z^{P_0}_{\mathrm{fact}}\)-morphism.  If the finite-window
  Hom-valued companion tables, centrality homotopies, QME
  counterterms, and Čech--Roos descent homotopies specified below are
  part of the datum, then it is sufficient:
  \[
    \boxed{\;\Phi_{\mathrm{fact}}^{\mathrm{unred}}
      \text{ lifts to an unreduced }
      Z^{P_0}_{\mathrm{fact}}\text{-morphism}
      \quad\Longleftarrow\quad
      \operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}
      =0
      \text{ with these choices.}\;}
  \]
  At the associated-graded level, the primitive
  indecomposable \(P_0\)-projection of
  Theorem~\ref{thm:chain-level-primitive-projection}
  is the first associated-graded target for the centrality filtration.
  A spectral sequence converging to
  \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is obtained only after the
  filtration is complete, exhaustive, and compatible with the
  Hom-valued companion differential.  In that case its first
  differential is the primitive Hamiltonian projection
  acting on the source one-\(\psi\) label, and its first nonzero
  piece is the obstruction class to the strict centrality of the
  primitive-projection lift.

  The four components are independent in the following sense.
  Vanishing of (i) is a graded commutativity test on the unreduced
  target and is automatic in the algebraic current model.  Vanishing
  of (ii) is the centrality homotopy test of
  Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}.
  Vanishing of (iii) is the boundary QME content of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization},
  whose first finite-window obstruction is
  \(\theta_3\)
  (Theorem~\ref{thm:app-theta-three-finite-window-H1}).
  Vanishing of (iv) is the descent comparison; in the algebraic
  current model this gap is zero by the same finite-list
  axiomatization.  For genuine unreduced
  \(Z^{P_0}_{\mathrm{fact}}\)-morphisms the Weiss/Ran comparison
  requires its own descent datum.
\end{thm}


\begin{proof}
  Necessity in each component.

  Component (i).  Any factorization-center morphism into
  \(Z^{P_0}_{\mathrm{fact}}(\mathrm{Obs}_{\mathrm{open}})\) lands in
  the equalizer of the two left/right factorization products.  Thus, for
  a source generator \(x\) and an open observable \(y\), the centrality
  cochain
  \[
    \mu(F(x),y)-(-1)^{|x||y|}\mu(y,F(x))
  \]
  is zero in the strict center and is a \(d_M\)-boundary for a lifted
  homotopy-center representative.  The cohomology class
  \(\operatorname{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) of such a
  lift is the obstruction to this strict graded centrality; its
  vanishing is necessary.  In the algebraic current model with
  graded-commutative target it is automatic
  (Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies}).

  Component (ii).  A factorization-center morphism into
  \(Z^{P_0}_{\mathrm{fact}}\) intertwines the source
  CE/Schouten bracket with the target
  \(P_0\)-bracket.  The deviation from this intertwining is precisely
  the row sum
  \(R^{\mathrm{cent}}_{x,y,M}\) of
  Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}; its
  cohomology class is the obstruction to choosing a centrality
  homotopy.  Vanishing is necessary because the homotopy is exactly the
  datum making the source CE differential commute with the target
  \(P_0\)-bracket on the image of
  \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\).

  Component (iii).  A factorization-center morphism produces a
  curved Maurer--Cartan element of the brane-defect kernel complex.
  Its curvature class is exactly the residual
  \(R^{\mathrm{ns}}_{n,M}\) of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization},
  in the inverse-limit version recorded as
  \(\bigl(\mathfrak o^{\mathrm{ns}}_{n,M}\bigr)_M\).  Vanishing,
	  together with the \(\varprojlim^1H^0\) compatibility class, gives
	  the order-\(n\) obstruction condition after lower orders are solved
	  and the finite row system is complete.  The first finite-window obstruction occurs at the \(\theta_3\)
  row by Theorem~\ref{thm:app-theta-three-finite-window-H1}.

  Component (iv).  The factorization-center construction depends
  globally on the chosen Weiss/Ran cover; a different cover may give
  a different value of the same morphism on overlapping
  factorizations.  The codimension-one Č\v{e}ch difference between
  Weiss and Ran covers is a class in the appropriate boundary-kernel
  \(H^0\); its vanishing is the descent compatibility for the
  factorization-center morphism.  In the algebraic current model
  with the displayed finite-list axiomatization this difference is
  zero, but the genuine unreduced
  \(Z^{P_0}_{\mathrm{fact}}\)-target carries the comparison
  nontrivially in general.

  Sufficiency under given companion data.  Conversely, suppose all
  four components vanish and the companion tables, centrality
  homotopies, QME counterterms, and descent homotopies are part of the
  datum.  Construct the lift by induction on \(\hbar_{\mathrm{QME}}\)-order
  and finite window \(M\).  At order zero the morphism is the
  algebraic current presentation \(\Phi_I^\chi\) of
  Construction~\ref{constr:app-algebraic-factorization-centre-lift},
  which already gives a strict (zero-homotopy)
  factorization-center morphism into the algebraic current target
  (Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies});
  hence (i) vanishes at order zero.  At each order \(n\geq1\) and
  each window \(M\):
  \begin{itemize}
    \item vanishing of (iii) gives a degree-zero counterterm
      \(C_{n,M}\) with
      \(d_{\mathrm{ns},M}C_{n,M}=-R^{\mathrm{ns}}_{n,M}\)
      (Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization});
      together with the \(\varprojlim^1H^0\)-primitive-compatibility
      class, this assembles a compatible finite-window counterterm tower;
    \item vanishing of (ii) gives a centrality homotopy
      \(H^M_{x,y}\) with
      \(R^{\mathrm{cent}}_{x,y,M}+d_M H^M_{x,y}=0\) for every source
      generator pair (Proposition~\ref{prop:app-marked-row-centrality-homotopy-test});
    \item vanishing of (iv) makes the resulting morphism agree on
      Weiss and Ran cover overlaps, by the codimension-one
      Č\v{e}ch--Roos comparison;
    \item vanishing of (i) at every window inductively proves
      strict graded centrality on the constructed lift.
  \end{itemize}
  Under these given finite-window data, the four vanishings are the
  rows of the inductive extension problem for the algebraic current
  presentation to a strict \(Z^{P_0}_{\mathrm{fact}}\)-morphism: each is a separate row in
  the curved Maurer--Cartan equation of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
  and its Hom-valued kernel companion, and any one nonzero
  component blocks the inductive step at the corresponding row.
  Hence the obstructions are necessary, and they are sufficient for the
  \(Z^{P_0}_{\mathrm{fact}}\)-lift only with the stated companion and
  descent data.

  The associated-graded statement.  Take the primitive projection
  filtration of Theorem~\ref{thm:chain-level-primitive-projection}.
  At the associated-graded level, the source differential is the
  CE differential and the target differential is the
	  Hamiltonian/Schouten bracket on block-sum primitive single-trace
	  classes.  If the primitive filtration is complete, exhaustive, and compatible
  with the Hom-valued companion differential, the \(E_1\) page is this
  primitive associated-graded target and its first differential is the
	  Hamiltonian projection on one-\(\psi\) primitive single-trace classes, as
  computed in Theorem~\ref{thm:chain-level-primitive-projection-P0}.
  Without these filtration hypotheses the statement is only the
  associated-graded primitive obstruction.
\end{proof}

\begin{rmk}[Locus of each component of
  \(\operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{descent}}\)]
\label{rmk:app-cotangent-lift-first-nontrivial-occurrence}
  The component
  \(\operatorname{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) is nonzero
  outside the graded-commutative algebraic current target of
  Definition~\ref{def:app-central-character-principal-part-current-algebra},
  on a noncommutative \(\hbar\)-deformed target where the product
  centrality defect carries a Capelli-type contribution.  The
  polynomial-obstruction boundary
  (Theorem~\ref{thm:app-unreduced-polynomial-centrality-obstruction})
  exhibits the same obstruction in the polynomial open BV/Koszul
  category: polynomial open BV observables have locally nilpotent
  translation action, while the principal-part module requires
  non-locally-nilpotent action for strict product centrality.

  The component \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is
  detected at the smallest window where the brane-defect bracket of
  two source generators carries a nonzero scalar-zero contribution whose
  primitive requires a compatible centrality homotopy
  \(H^M_{x,y}\)
  (Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}).
  In the minimal full-equivariant branch the row vanishes and the
  zero homotopy suffices; the finite-window matrix calculation gives
  the same answer in the centrality-homotopy complex.

  The component \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}\) is
  nonzero at \(\theta_3\) in case (a) of
  Theorem~\ref{thm:app-theta-three-finite-window-H1}, where the
  counterterm is omitted; case (b) of the same theorem gives
  \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}=0\) at
  \(\theta_3\) under the given counterterm
  \(C_{\theta_3,M}\).

  The component \(\delta^{\mathrm{Weiss}}_{\check C/R}\) is nonzero
  when the codimension-one face of the Weiss-versus-Ran cover
  comparison carries a non-vanishing boundary-kernel class; this is a
  separate descent comparison in the factorization-current convention
  complex.
\end{rmk}
