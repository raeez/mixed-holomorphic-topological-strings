...
\(\Omega_{\mathrm{loc}}=dz_1\wedge dz_2\).  On the brane-defect
local-functional complex
\(\mathfrak Q^\bullet_{w,\partial,\hbar}(I)\), a filtered chain scalar
projection \(\widehat\sigma_{\mathrm{sc}}\) reduces the non-scalar
Costello QME on a brane interval to a single cohomology class
\[
...
and \(P^{M,N}\) on primitive candidates and the Roos class
