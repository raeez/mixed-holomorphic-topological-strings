...
window-by-window primitives across the truncation tower).
...
(the cohomology class measuring failure of windowwise primitives to
agree across truncation).
...
\begin{rmk}[Routing through the obstruction--twist schema]\label{rmk:scalar-anomaly-route-Gschema}
  Under Theorem~\ref{thm:obstruction-twist-dichotomy} of
  Appendix~\ref{app:master-deformation-complex}, the scalar-anomaly
  cocycle
  \(\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\) is the
  prototype of clause~(3) (projective / gerbe coherence): the class is
  central but not exact in the strict scalar-reduced target, no
  internal counterterm cancels it for \(N>0\), and the only admissible
  repair is the central extension
  \(0\to\C\!\cdot\!\mathbf 1\to\mathfrak h_{\hbar,N}\to\bar A\to 0\)
  with cocycle \(\hbar N\bar c\), under which the brane carries the
  projective Hamiltonian action
  \([\Phi_\hbar(f),\Phi_\hbar(g)]_\star=\hbar\Phi_\hbar(\{f,g\})
  +\hbar N\bar c(f,g)\mathbf 1\).  The four faces -- algebraic central
  extension, Lie cocycle, trace-algebraic boundary, Weyl/Moyal
  commutator -- are four readings of the same clause~(3) curvature.
\end{rmk}
...
\label{rmk:app-scalar-anomaly-transgression-branches}
  ...
  \(-\hbar N[\bar c]\); since \([\bar c]\neq0\), no internal
  counterterm exists, and the four external branch replacements above
  exhaust the cancellations.
\end{rmk}
...
\label{def:app-balanced-scalar-contact-habitat}
  ...
  present in this subcomplex except those obtained by contracting a single
...
\label{rmk:app-balanced-projection-extension-obstruction}
  ...
  the scalar-contact projection tower, not the full non-scalar
  graph/QME residual.  Enlarging
  \(\mathfrak H^\bullet_{\mathrm{bal,sc}}(I)\) to a complete filtered
  subcomplex \(\mathfrak H'\) carrying scalar symbols not known to
  factor through \(\operatorname{sdim}(V)\), the obstruction to
...
\label{thm:app-supertraceless-monodromy-projection}
  ...
  create a non-supertraceless marked word.  Preservation holds on the
...
\begin{proof}
  ...
  \(d_{\mathrm{QME}}=Q+\{I_0,-\}\) carries no external endomorphism
  label; together with
  Definition~\ref{def:app-balanced-costello-graph-subhabitat}, which
...
\label{def:app-finite-window-marked-costello-list}
  ...
  omitted boundary entry; the larger specialization stays a
  construction target.
...
\label{def:app-native-finite-window-realization-habitat}
  ...
  \[
    ...
    \ker\widehat\sigma_{\mathrm{sc},M}.
  \]
  If this projection is not a chain map, the non-scalar kernel complex
  is not formed.
\end{defn}
...
\label{thm:app-constructive-nonscalar-costello-qme-realization}
  ...
  to be supplied; the criterion is not extended by analogy.
\end{thm}
...
\label{prop:app-theta-three-bianchi-lower-condition}
  ...
  \[
    A_D^2c=-r_2
  \]
  has no solution in the supplied lower local-functional row complex.  The
  normalized cokernel
  \[
...
\begin{proof}
  ...
  \(-1\) on it, the current complex does not contain such a boundary.
...
\label{thm:app-theta-three-finite-window-H1}
  ...
  \begin{enumerate}[label=(\alph*)]
    \item \emph{No counterterm supplied.}  In the
  ...
  Each verdict is the output of the fixed-window matrix solver of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
  on the corresponding row complex.

  The script \texttt{scripts/finite\_window\_graph\_array.py} returns
  the same outputs.  Verdict (a) corresponds to the cases
  \texttt{theta\_3\_current\_finite\_row\_subcomplex},
  \texttt{theta3\_ce\_ancestor\_absent\_payload}, and
  \texttt{theta3\_counterterm\_absent\_payload}, each returning
  \(\texttt{primitive\_exists}=\mathrm{false}\) with cokernel covector
  \(\ell(\mathsf E_{\theta_3,M})=1\).  Verdict (b) corresponds to
  three repair payloads --- the supplied scalar-zero counterterm
...
\begin{rmk}[Routing through the obstruction--twist schema]\label{rmk:theta3-route-Gschema}
  The \(\theta_3\) finite-window verdict is the prototype of clause~(2)
  (counterterm coherence) of
  Theorem~\ref{thm:obstruction-twist-dichotomy} of
  Appendix~\ref{app:master-deformation-complex}: bare
  \(H^1_{\mathrm{bare}}=\C\!\cdot\!E_{\theta_3}\ne0\) is the nonzero
  curvature class \([F_{\mathcal T}(\Theta_{\mathrm{loc}})]\) in the
  strict Costello QME target; supplied-counterterm \(H^1=0\) under the
  unique \(dC=-E\), \(c=1\) is the null-homotopy
  \(\tau=C\) realising
  \(F_{\mathcal T}(\Theta_{\mathrm{loc}}+\tau)=0\) inside the same
  target.  The window therefore exhibits the cleanest case of the
  schema: an exact obstruction killed by a unique counterterm, no
  central extension required, no target replacement.  The all-row
  generalisation is governed by the reduced non-scalar QME class
  \([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\).
\end{rmk}
...
\label{rmk:app-theta-three-verdict-which-is-theorem}
  ...
  at \(\theta_3\).  Verdict (a) is the sub-theorem of the minimal
  seed list: when no order-three scalar-zero counterterm
  \(C_{\theta_3,M}\) is supplied, the finite-window
  \(d_{\mathrm{ns},M}\)-cohomology in degree one is
  \(\C\,\mathsf E_{\theta_3,M}\), with explicit covector
  representative \(\ell_{\theta_3}\).  Verdict (b) is the sub-theorem
  ...
  \(C_{\theta_3}\) is admissible exactly under verdict (b), with one
...
\begin{proof}
  ...
  \(T_\Omega\)-invariance of the primitive is required separately, and
  is not derivable from scalar-shadow vanishing.  The inverse-limit
...
\begin{rmk}[Routing through the obstruction--twist schema]\label{rmk:cotangent-lift-route-Gschema}
  Under Theorem~\ref{thm:obstruction-twist-dichotomy} of
  Appendix~\ref{app:master-deformation-complex}, the four-component
  obstruction
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}=
   ...
   \delta^{\mathrm{Weiss}}_{\check C/R})\) is the curvature vector of
  the transport from the reduced cochain CE/PV map to the unreduced
  factorization-centre lift.  Componentwise: vanishing of
  \(\mathrm{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) and
  \(\mathrm{ob}^{P_0}_{\mathrm{cent}}\) with compatible null-homotopies
  is clause~(1) (strict centrality) at the product/\(P_0\) level;
  exactness with chosen homotopies
  \(h^{\mathrm{prod}}_{x,a}\) and \(h^{P_0}_{x,a}\) is clause~(2) and
  promotes the strict centre target
  \(Z^{P_0}_{\mathrm{fact}}\) to the derived centre
  \(Z^{P_0,\mathrm{der}}_{\mathrm{fact}}\); a nonzero
  \(\mathrm{ob}^{\mathrm{bd}}_{\mathrm{QME}}\) requires a Costello
  counterterm in the boundary QME (clause~(2)); a nonzero
  \(\delta^{\mathrm{Weiss}}_{\check C/R}\) requires either Weiss/Ran
  descent homotopies (clause~(2)) or a stratified gerbe-valued descent
  (clause~(3)).  The stronger global, chain-level, all-bidegree, or
  modular completion is the sequence of these clauses.
\end{rmk}
...
\label{rmk:app-cotangent-lift-first-nontrivial-occurrence}
  ...
  polynomial-no-go boundary
  (Theorem~\ref{thm:app-unreduced-polynomial-centrality-no-go})
  exhibits the same obstruction in the polynomial open BV/Koszul
  category: polynomial open BV observables cannot present the
  principal-part module, hence cannot satisfy strict product
  centrality.

  The component \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is
  detected at the smallest window where the brane-defect bracket of
  two source generators carries a nonzero scalar-zero contribution
  without a compatible centrality homotopy
  \(H^M_{x,y}\)
  (Proposition~\ref{prop:app-marked-row-centrality-homotopy-test}).
  ...
  nonzero at \(\theta_3\) under verdict (a) of
  Theorem~\ref{thm:app-theta-three-finite-window-H1} --- the
  counterterm-absent sub-theorem.  Verdict (b) of the same theorem
  gives \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}=0\) at
  ...
  separate descent comparison in the factorization-current
  conventions appendix and is not pursued here as a row.
\end{rmk}
