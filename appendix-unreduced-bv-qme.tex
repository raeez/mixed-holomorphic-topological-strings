...
\label{thm:app-unreduced-cotangent-lift-obstruction}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{Weiss-versus-Ran descent gap.}
      \(\delta^{\mathrm{Weiss}}_{\check C/R}\) is the codimension-one
      face class of the Weiss/Ran descent comparison: working over
      the Weiss cover
      \(\mathcal U^{\mathrm{Weiss}}\), the same factorization-center
      lift descends to the Ran-space cover
      \(\mathcal U^R\) iff the Č\v{e}ch difference
      \(\delta^{\mathrm{Weiss}}_{\check C/R}\) vanishes in the
      finite-window \(H^0\) of the corresponding boundary kernel.
  \end{enumerate}
