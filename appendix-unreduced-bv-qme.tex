...
\label{thm:app-theta-three-finite-window-H1}
  ...
  \texttt{scripts/finite\_window\_graph\_array.py}, which in the
  current data package returns
  \(\texttt{primitive\_exists}=\mathrm{false}\) with covector
  \(\ell(\mathsf E_{\theta_3,M})=1\) for verdict (a), and
  \(\texttt{primitive\_exists}=\mathrm{true}\) with primitive
  coefficient \(c=1\) on \(C_{\theta_3,M}\) for verdict (b).
\end{thm}
