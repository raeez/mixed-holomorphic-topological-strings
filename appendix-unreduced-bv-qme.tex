...
quantum master equation lives in their orthogonal complement.  Naming
the players: \(\mathfrak Q^\bullet_{w,\partial,\hbar}(I)\) is the
...
projection killing the scalar-contact filtration; and
\(d_{\mathrm{QME}}=Q+\{I_0,-\}+\hbar\Delta\) is the quantum master
differential of Costello's renormalization.  Once these are in place,
the residual non-scalar QME obstruction is a single cohomology class
\[
...
scalar projection at the U(1) centre-of-mass slice has explicit value
\[
  \widehat\sigma_{\mathrm{sc}}\bigl(
    [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\bigr)
    =\hbar N[\bar c]
    =\Omega_{\mathrm{central}}\bigm|_{J(f)},
\]
the Capelli class of Chapter~6
(Theorem~\ref{thm:capelli-equals-modular-curvature}); equivalently,
the scalar projection of the non-scalar Costello QME residual is the
projective curvature of the modular line bundle evaluated at the
trace generator \(J(f)=\operatorname{Tr}f(\phi_1,\phi_2)\), with sign
...
star product.  Verification of the Moyal coefficient
\(\hbar N\) is supplied by
\texttt{scripts/check\_moyal\_coefficients.py}.
...
(Theorem~\ref{thm:app-scalar-anomaly-transgression}).  Slogan:
\emph{constants vanish as Hamiltonian vector fields but survive as
brane center-of-mass charge.}  Each cancellation branch acts on this
...
candidate counterterms supplied by the row span, \(c^M\) is the
...
counterterm \(c^M\) exists in the supplied row span exactly when the
...
\(P^{M,N}\) on primitive candidates (the matched counterterm projector)
...
The unreduced factorization-center cotangent lift is governed by a
four-component obstruction vector, and lifts if and only if that
vector vanishes
...
\label{def:app-scalar-obstruction-complex}
  ...
  \(d_{\mathrm{QME}}\)-stable scalar filtration and a filtered
  chain projection
  \[
  ...
  until this extra projection is supplied.
...
\label{thm:app-unreduced-polynomial-centrality-obstruction}
  ...
  reduced map \(\rho_{a,b}[1]\mapsto\Theta_{a,b}\) would require
...
\label{thm:app-scalar-contact-qme-class}
  ...
  \(\lie{gl}_N\) brane-defect model would require a leading symbol
  primitive for \(\hbar N[\bar c]\).
\end{thm}
...
\begin{proof}
  ...
  that primitive.  Thus a scalar-reduced counterterm would require an
  associated-graded primitive for the class.
\end{proof}

\begin{thm}[Scalar anomaly transgression: four equivalent sightings]
\label{thm:app-scalar-anomaly-transgression}
  ...
  quantum scalar \(\hbar N\) supplied by the open Chan--Paton index
  ...
  Slogan: \emph{constants vanish as Hamiltonian vector fields but
  survive as brane center-of-mass charge.}  The radial Poisson field
  ...
  same class is the projective curvature of the modular line bundle
  \(\Omega_{\mathrm{central}}\) evaluated at the trace generator
  \(J(f)=\operatorname{Tr}f(\phi_1,\phi_2)\) by
  Theorem~\ref{thm:capelli-equals-modular-curvature}: the four named
  sightings are the four ways the local-stalk curvature value enters
  the formal-Darboux computation, with the determinant-line
  interpretation of the central extension supplied by
  Remark~\ref{rmk:determinant-line-scheme}.
\end{thm}

\begin{proof}
  Sighting (i) is the content of
  ...
  Sighting (ii) is the standard correspondence between Lie central
  ...
  Sighting (iii) is the trace-algebraic content of
  Theorem~\ref{thm:app-scalar-contact-qme-class}: the open Chan--Paton
  ...
  Sighting (iv) is the quantum Weyl reformulation in the same theorem:
  ...
  open index loop charge \(\hbar N\).  The four sightings are the
  four ways the same class \([\bar c]\) becomes visible after pairing
...
\label{thm:app-central-character-qme-cancellation}
  ...
  original scalar-reduced QME complex would require its own null-homotopy.
...
\label{rmk:app-balanced-projection-extension-obstruction}
  ...
  residual is a separate obstruction problem.  Enlarging
...
\label{thm:app-supertraceless-monodromy-projection}
  ...
  Therefore applying
  \(d_{\mathrm{QME}}\) to this displayed Hamiltonian generator set
  preserves supertraceless monodromy.  Preservation holds on the
  unmarked Hamiltonian generator set.  A larger Costello specialization
  with additional marked idempotents, holonomies, or counterterm
  tensors is governed by \(\mathcal B_{\operatorname{st}=0}\) on those
  added generators.
\end{thm}
...
\begin{proof}
  ...
  \(d_{\mathrm{QME}}=Q+\{I_0,-\}\) carries the empty endomorphism
  label; together with
  Definition~\ref{def:app-balanced-costello-graph-subhabitat}, which
...
\label{def:app-finite-window-marked-costello-list}
  ...
  the larger specialization stays a construction target.
...
\label{thm:app-constructive-nonscalar-costello-qme-realization}
  ...
  face, scalar condition, primitive candidate, or truncation matrix
  as explicit input to the criterion.
\end{thm}
...
\label{prop:app-closed-rows-and-theta-three-source-face}
  ...
  \]
  detects a nonzero finite-row obstruction.  To remove it one must supply
...
\begin{proof}
  ...
  one-row finite complex, the degree-zero candidate primitive space is zero,
...
\label{prop:app-theta-three-bianchi-lower-condition}
  ...
  \[
    A_D^2c=-r_2
  \]
  has a cokernel obstruction in the supplied lower local-functional row complex.  The
  normalized cokernel
  \[
  ...
  Bianchi repair from a fixed-window matrix target to a solved
...
\label{thm:app-theta-three-finite-window-H1}
  ...
  \begin{enumerate}[label=(\alph*)]
    \item \emph{Counterterm omitted.}  In the
      ...
      The supplied finite complex has zero degree-zero row space; the linear system
      \(A^M c=-r^M\) of
      Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
    ...
    \item \emph{Scalar-zero local counterterm \(C_{\theta_3,M}\)
      supplied.}  Suppose a degree-zero local functional
      \(C_{\theta_3,M}\in\mathcal K^0_{\theta_3,M}\) with
      \(\widehat\sigma_{\mathrm{sc},M}(C_{\theta_3,M})=0\) is supplied
  ...
  Each case is the output of the fixed-window matrix solver of
  Theorem~\ref{thm:app-constructive-nonscalar-costello-qme-realization}
  on the corresponding row complex.
\end{thm}

\begin{proof}
  Case (a).  The supplied finite complex has empty
...
\begin{rmk}[Computer verification at the \(\theta_3\) row]
\label{rmk:app-theta-three-script-verification}
  The script \texttt{scripts/finite\_window\_graph\_array.py}
  reproduces the case dichotomy of
  Theorem~\ref{thm:app-theta-three-finite-window-H1}.  Case~(a) is
  returned on the omitted-CE-ancestor and omitted-counterterm rows
  of the row family \texttt{theta\_3\_current\_finite\_row\_subcomplex}:
  each yields \(\texttt{primitive\_exists}=\mathrm{false}\) with
  cokernel covector \(\ell(\mathsf E_{\theta_3,M})=1\).  Case~(b) is
  returned on the three repair rows of the same family --- the
  supplied scalar-zero counterterm
  \texttt{theta3\_counterterm\_supplied\_exact\_payload}, the supplied
  CE ancestor \texttt{theta3\_ce\_ancestor\_supplied\_exact\_payload},
  and the supplied complete companion-face table
  \texttt{theta3\_complete\_companion\_faces\_supplied\_exact\_payload}
  --- each returning \(\texttt{primitive\_exists}=\mathrm{true}\) with
  \(\texttt{primitive\_coefficients}\) equal to
  \([\![C_{\theta_3,M}^{\mathrm{ct}},1]\!]\),
  \([\![C_{\theta_3,M}^{\mathrm{CE}},1]\!]\), or the corresponding
  unit-coefficient signed companion sum.
\end{rmk}

\begin{rmk}[Sub-theorem dichotomy at \(\theta_3\)]
\label{rmk:app-theta-three-dichotomy-sub-theorem}
  The two cases of
  Theorem~\ref{thm:app-theta-three-finite-window-H1} apply under
  disjoint fixture conditions on the order-three finite-row complex
  at \(\theta_3\).  Case (a) is the sub-theorem of the minimal
  seed list: when the order-three scalar-zero counterterm
  ...
  representative \(\ell_{\theta_3}\).  Case (b) is the sub-theorem
  of the counterterm-extended complex: when \(C_{\theta_3,M}\) with
  \(\widehat\sigma_{\mathrm{sc},M}(C_{\theta_3,M})=0\) and
  \(d_{\mathrm{ns},M}C_{\theta_3,M}=-\mathsf E_{\theta_3,M}\) is
  supplied, the finite-window \(H^1\) vanishes.  A named coefficient
  \(C_{\theta_3}\) is admissible exactly in case (b), with one
  of the three repair data --- CE ancestor with controlled boundary,
...
\label{def:app-normal-omega-costello-kernel-datum}
  ...
  \begin{enumerate}[label={(\(\Omega\arabic*\))}]
    ...
    \item For every localized normal character
      ...
      curvature classes and QME counterterms are separate data.
  ...
  \end{enumerate}
  This datum supplies the equivariant structure: coefficient ring,
...
\label{thm:app-equivariant-brane-defect-qme-criterion}
  Fix a candidate normal \(\Omega\)-equivariant finite-window system
  ...
  Therefore the normal \(\Omega\)-background supplies only the
...
\begin{proof}
  ...
  this added labelled zero row; a nonzero order-one class is decided after
  the nonzero order-one row values have been supplied.
\end{proof}
...
\label{rmk:app-order-two-after-zero-nonzero-order-one-rows}
  ...
  list supplies zero nonzero order-one rows outside
  ...
  conditions, and truncation coefficients have been supplied.
\end{rmk}
...
\begin{prop}[First order-three enlarged-system datum]\label{prop:app-first-order-three-enlarged-row}
  ...
  cochain exists only after the finite-window system supplies at least one
  ...
  \]
  In a nonminimal system with supplied nonzero order-one and order-two
  ...
  \]
  Only after this condition is supplied and vanishes is the non-scalar obstruction
  class defined:
  \[
...
\begin{proof}
  ...
  supplied nonzero lower-order components.  The displayed formulas are
...
\label{prop:app-marked-row-centrality-homotopy-test}
  ...
  A marked finite-window system supplies an unreduced
  ...
  unreduced Costello graph/counterterm target must supply the displayed
...
\begin{thm}[Unreduced cotangent lift obstruction: necessary and
  sufficient]
\label{thm:app-unreduced-cotangent-lift-obstruction}
  ...
  be a candidate factorization-center morphism extending the
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{\(P_0\) / shifted-cotangent refinement.}
      ...
      class of the row before such a witness is supplied.
    ...
    \item \emph{Č\v{e}ch--Roos / Weiss-versus-Ran descent gap.}
      ...
      \(\mathcal U^R\): the same factorization-center lift descends
      to \(\mathcal U^R\) if and only if the Č\v{e}ch--Roos
      difference \(\delta^{\mathrm{Weiss}}_{\check C/R}\) vanishes in
      the finite-window \(\varprojlim^1 H^0\) of the corresponding
      boundary kernel.
  \end{enumerate}
  ...
  computes the first page \(E_1\) of the spectral sequence
  converging to \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\):
  the \(E_1\) differential is the primitive Hamiltonian shadow
...
\begin{proof}
  ...
  \(\operatorname{ob}^{\mathrm{prod}}_{\mathrm{cent}}\) of a candidate
  ...
  together with the \(\varprojlim^1H^0\) compatibility class, is the
  unique condition making the curved Maurer--Cartan element actually
  flat.  The first finite-window obstruction occurs at the \(\theta_3\)
  ...
  Construct the lift by induction on \(\hbar_{\mathrm{QME}}\)-order
  and finite window \(M\).  At order zero the candidate is the
  algebraic current presentation \(\Phi_I^\chi\) of
  Construction~\ref{constr:app-algebraic-factorization-centre-lift},
  which already supplies a strict (zero-homotopy)
  factorization-center morphism into the algebraic current target
  (Theorem~\ref{thm:app-algebraic-factorization-centre-zero-homotopies});
  hence (i) vanishes at order zero.  At each order \(n\geq1\) and
  each window \(M\):
  \begin{itemize}
    \item vanishing of (iii) supplies a degree-zero counterterm
      \(C_{n,M}\) with
    ...
    \item vanishing of (ii) supplies a centrality homotopy
      \(H^M_{x,y}\) with
  ...
  \end{itemize}
  The four vanishings are the only obstructions to inductively
  extending the algebraic current presentation to a strict
  \(Z^{P_0}_{\mathrm{fact}}\)-morphism: each is a separate row in
  ...
  Hence the obstructions are necessary and sufficient for the
  \(Z^{P_0}_{\mathrm{fact}}\)-lift to exist.
  ...
  \(\operatorname{ob}^{P_0}_{\mathrm{cent}}\) is exactly this
  primitive-shadow target; its differential is the
  Hamiltonian shadow on one-\(\psi\) primitive trace classes, as
  computed in Theorem~\ref{thm:chain-level-primitive-projection-P0}.
  The first nonzero \(E_1\) class is the obstruction to strict
  primitive-shadow centrality on the unreduced source, which is the
  classical primitive obstruction.
\end{proof}
...
\label{rmk:app-cotangent-lift-first-nontrivial-occurrence}
  ...
  zero homotopy suffices; the script
  \texttt{scripts/finite\_window\_graph\_array.py} reports the same
  in the centrality-homotopy fixture.
  ...
  counterterm-omitted sub-theorem; case (b) of the same theorem gives
  \(\operatorname{ob}^{\mathrm{bd}}_{\mathrm{QME}}=0\) at
  \(\theta_3\) under the supplied counterterm
  \(C_{\theta_3,M}\).
  ...
  separate descent comparison treated in the factorization-current
