...
BV differential; and
\(d_{\mathrm{QME}}^{L}=Q+\{I[L],-\}_{L}+\hbar\Delta_{L}\) is the
...
\(\operatorname{Ob}_{w,\partial,\hbar}\) has scalar-contact symbol
\[
  \widehat\sigma_{\mathrm{sc}}\bigl(
    \operatorname{Ob}_{w,\partial,\hbar}\bigr)
...
(Theorem~\ref{thm:capelli-equals-modular-curvature}) is the scalar
projection of the unreduced obstruction before reduction to
\(\ker\widehat\sigma_{\mathrm{sc}}\), with sign
and convention fixed by
Appendix~\ref{sec:app-factorization-current-conventions} and the
canonical scheme
...
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\) carry the same
class in \(H^2(\bar A;\C)[[\hbar]]\) under the canonical scalar
projection; the cocycle \(\omega(f,g)=[\{f,g\}]_0\) is the same in each
...
class \(\hbar N[\bar c]\).  Each cancellation branch acts on this
single class --- ordinary \(\lie{gl}_N\) is obstructed by
\(\hbar N[\bar c]\), the unreduced central character contributes the
CE coboundary \(d_{\mathrm{CE}}(\hbar\chi_N)=-\hbar N\omega\), the
...
\label{prop:app-local-geometry-scalar-shadow}
  ...
  \]
  Thus the scalar shadow has zero divergence and zero volume-form anomaly.
...
\begin{prop}[First scalar-lift obstruction in the ordinary branch]\label{prop:app-first-scalar-lift-obstruction}
  ...
  ordinary scalar-reduced branch requires additional data whose leading
  scalar class is \(-\hbar N[\bar c]\), or a replacement of the source or
...
\label{thm:app-algebraic-factorization-centre-zero-homotopies}
  ...
  genuine unreduced open BV factorization algebra requires the additional
...
\begin{proof}
  ...
  extension cocycle \(\hbar N\omega\,\gamma_{\mathbf 1}\), hence the
  class
  \[
    \hbar N[\bar c]\,\gamma_{\mathbf 1}
    \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1][[\hbar]].
...
\label{thm:app-scalar-anomaly-transgression}
  ...
  \]
  is detected in four equivalent forms.  All four witness one and the
  same nonzero class in \(H^2(\bar A;\C)[[\hbar]]\).  Up to the
  quantum scalar \(\hbar N\) given by the open Chan--Paton index
  loop, the cocycle is the same in each.
  ...
  \begin{enumerate}[label=\textnormal{(\roman*)}]
    ...
    \item \emph{Trace-algebraic central extension on the boundary.}
      ...
      \[
        ...
        \longrightarrow 0
      \]
      of trace-reduced boundary algebras, whose Lie cocycle is
      \(\hbar N\omega\) and whose class in
      \(H^2(A_{\partial,\mathrm{red}};\C)[[\hbar]]\) is
      \(\hbar N[\bar c]\).
    \item \emph{Quantum Weyl commutator.}  In the Weyl quantization,
      \[
        ...
        \quad\text{in cohomology},
      \]
      i.e.\ the Capelli shift makes the scalar commutator equal
      \(\hbar N\) times the identity, modulo the Hamiltonian moment-map
      ideal of \(\lie{sl}_N\).  The associated CE class is again
      \(\hbar N[\bar c]\).
  \end{enumerate}

  These four are equivalent in the strong sense that the same class
  \([\bar c]\in H^2(\bar A;\C)\) is the image, under the canonical
  ...
  \begin{array}{ccccc}
    ...
    H^2(\bar A;\C),
    \\[1ex]
    \big\downarrow\,\hbar N\cdot
    &  &
    \big\downarrow\,\hbar N\cdot
    &  &
    \\[1ex]
    [\hbar N\omega]_{(\mathrm{iii})}
    & = &
    [\hbar N\omega]_{(\mathrm{iv})}
    & \in &
    H^2(\bar A;\C)[[\hbar]],
  \end{array}
  \]
  with horizontal equalities of cocycles given by the Lie/algebraic
  identification of (i) with (ii) and the trace/Weyl identification of
  (iii) with (iv), and vertical multiplications by \(\hbar N\) given
  by the open Chan--Paton trace pairing.
  ...
  produces the Capelli shift \(\hbar N[\bar c]\).  Equivalently, the
  same class is the Lie cohomology class \(\hbar N[\bar c]\).  If a
  determinant line with connection exists and its Atiyah class
  pulls back to \([\bar c]\), its curvature restricts to
  \(\hbar N[\bar c]\) at the trace generator
  \(J(f)=\operatorname{Tr}f(\phi_1,\phi_2)\) by
  Theorem~\ref{thm:capelli-equals-modular-curvature}: the four
...
\begin{proof}
  ...
  Representative (ii) is the standard correspondence between Lie central
  extensions of \(\bar A\) by \(\C\) and elements of
  \(H^2(\bar A;\C)\).  Setting
  \([\![\bar f,\bar g]\!]_{\mathrm{cent}}=\overline{\{f,g\}}+\omega(f,g)\bar c\)
  defines a Lie bracket because \(\omega\) is a \(2\)-cocycle, and the
  associated extension class is \([\omega]=[\bar c]\) by construction.
  This is the same class as (i) up to canonical identification.

  Representative (iii) is the trace-algebraic content of
  Theorem~\ref{thm:app-scalar-contact-qme-class}: the open Chan--Paton
  ordinary trace pairs \(I_\partial(a,f)I_\partial(b,g)\) and produces
  the residual \(\hbar N\,\omega(f,g)\int_\R a(t)b(t)\,dt\), which is
  the trace-algebraic cocycle.  The boundary
  ...
  \(\hbar N\) times the canonical \(\bar A\)-cocycle \(\omega\).
  Hence the class in
  \(H^2(A_{\partial,\mathrm{red}};\C)[[\hbar]]\) is \(\hbar N\) times
  the same scalar \([\bar c]\) under this canonical pullback: the
  trace coefficient is the only scalar dependence beyond the source
  cocycle.

  Representative (iv) is the quantum Weyl reformulation in the same theorem:
  the matrix Weyl commutator \([X,Y]=\hbar N I\) modulo
  ...
  CE class \(\hbar N[\bar c]\).

  The three vertical/horizontal equalities of the diagram are induced
  by the canonical scalar-symbol projection
  (Proposition~\ref{prop:app-local-geometry-scalar-shadow}) and the
  open Chan--Paton trace.  The algebraic content: \(X_f(1)=0\) for
  ...
  open index loop charge \(\hbar N\).  The four representatives are the
  four forms of the same class \([\bar c]\) after pairing
  with the open trace, the source CE differential, the Lie bracket,
  or the boundary Weyl commutator.
\end{proof}
...
\begin{proof}
  ...
  \(J_N^\chi([z_1,z_2]_*)=\hbar N\), so it is exact before forming
...
\label{thm:app-supertraceless-monodromy-projection}
  ...
  that \(\hbar\Delta_L\) and \(I[L]-I_0\) create only empty
...
\label{def:app-finite-window-marked-costello-list}
  ...
  finite list and, after the canonical identification of resulting orientation
  lines,
  \[
...
\label{thm:app-constructive-nonscalar-costello-qme-realization}
  ...
  counterterms \(C_{<n,M}\).  The order-\(n\) realization is decided by
...
\label{prop:app-theta-three-bianchi-lower-condition}
  ...
  Bianchi primitive from a fixed-window matrix target to a solved
...
\begin{proof}
  ...
  primitives, their compatibility is measured by the usual Roos
    ...
    \item \emph{Counterterm omitted.}  In the
      ...
      \[
        [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
          ...
          =[\mathsf E_{\theta_3,M}]\neq0,
      \]
      detected by the unique covector
      \(\ell_{\theta_3}(\mathsf E_{\theta_3,M})=1\) on the row basis.
    ...
    \item \emph{Scalar-zero local counterterm \(C_{\theta_3,M}\).}
      ...
      \[
        ...
        [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
          \big|_{\theta_3,M}=0,
      \]
      and the unique counterterm \(c=1\) (i.e.\ adopting
      \(C_{\theta_3,M}\) with coefficient one in the QME expansion)
...
\begin{proof}
  ...
  Its cohomology class equals the basis vector itself, and the unique
...
\label{rmk:app-theta-three-dichotomy-sub-theorem}
  ...
  (Proposition~\ref{prop:app-closed-rows-and-theta-three-source-face}).
  For the brane-preserving \(\Omega\)-background of
  Definition~\ref{def:app-normal-omega-costello-kernel-datum} the
  \(\theta_3\) row carries positive \(\hbar\)-order, hence positive
  \(T_\Omega\)-weight \(\epsilon_1+\epsilon_2\); the case-(b)
  counterterm is the localization primitive
  \(C_{\theta_3,M}=h_\Omega\mathsf E_{\theta_3,M}\) of
  Theorem~\ref{thm:omega-localization-collapse}, so the non-scalar
  quantum master equation closes and the residual curvature is the
  central Capelli class \(\hbar N[\bar c]\).
\end{rmk}
...
\label{prop:app-first-scalar-zero-order-one-row}
  ...
  order-one class still requires an additional given row
...
\label{prop:app-marked-row-centrality-homotopy-test}
  ...
  data beyond scalar-shadow vanishing.
...
\begin{proof}
  ...
  beyond scalar-shadow vanishing.  The inverse-limit
...
\label{prop:app-even-block-marker-obstruction}
  ...
  subalgebra has zero \([\bar c]\)-shadow for single markers if and only if
...
\begin{proof}
  ...
  to
  \[
    \hbar N\omega+w_{\mathrm{add}}+\delta\bar\eta=0
  ...
  \(w_{\mathrm{add}}\) is exact, this condition forces
  \(\hbar N[\bar c]=0\) in \(H^2_{\mathrm{Lie}}(\bar A;\C)[[\hbar]]\).
  For \(N>0\) and formal \(\hbar\),
...
\begin{proof}
  ...
  \(Z^{P_0}_{\mathrm{fact}}(\mathrm{Obs}_{\mathrm{open}})\) sends
  every source generator to a graded center element by definition,
  hence the product centrality defect is a \(d_M\)-boundary on every
  generator pair.  The cohomology class
  ...
  \begin{itemize}
    \item vanishing of (iii) gives a degree-zero counterterm
      ...
      class, this assembles a compatible counterterm tower;
  ...
  The associated-graded statement.  Take the primitive shadow
  ...
  Hamiltonian/Schouten bracket on primitive trace classes.  The
  If the primitive filtration is complete, exhaustive, and compatible
  with the Hom-valued companion differential, the \(E_1\) page is this
  primitive-shadow target and its first differential is the
  Hamiltonian shadow on one-\(\psi\) primitive trace classes, as
