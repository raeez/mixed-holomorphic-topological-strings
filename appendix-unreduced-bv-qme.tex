...
\begin{proof}
  ...
  the trace-algebraic cocycle.  Its class in
  \(H^2(A_{\partial,\mathrm{red}};\C)[[\hbar]]\) is \(\hbar N\) times
  the same scalar \([\bar c]\): the trace coefficient is the only
  scalar dependence.
...
\label{rmk:app-unreduced-bv-qme-support-map}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \emph{Branch ledger.}
      ...
      each absorb or kill the scalar.
    ...
    \item \emph{First exposed row \(\theta_3\) and the named missing
      ...
      Repair requires CE ancestor, scalar-zero Costello counterterm
