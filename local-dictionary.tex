...
\begin{defn}[Admissible host category]\label{defn:admissible-host-category}
  ...
  \begin{enumerate}[label=\textup{(A\arabic*)}]
    ...
    \item \emph{Scalar reduction.}  Let
      ...
      cokernels.  The unit of an algebra object is retained unless the
    ...
    \item \emph{Stable trace passage.}  Fix a word degree \(d\).  Let
      \(\mathrm{Tow}^{\mathrm{tr}}_d\) be the category whose objects are
      finite-window rational towers
      \[
        \bigl\{
        \operatorname{Prim}H^0(\mathcal R_N^{GL_N},Q)_d
        /\C\cdot\operatorname{Tr}(1)
        \bigr\}_{N\ge d}
      \]
      with transition maps induced by the Procesi--Razmyslov stable
      trace identifications in degree \(d\); morphisms commute with
      these transition maps.  The degree-\(d\) stable trace functor is
      \[
        \operatorname{StTr}_d\colon
        \mathrm{Tow}^{\mathrm{tr}}_d\longrightarrow \mathrm{Vect}_\C,
        \qquad
        \mathcal R_\bullet\longmapsto
        \varprojlim_{N\geq d}
        \operatorname{Prim}H^0(\mathcal R_N^{GL_N},Q)_d
        \big/\C\cdot\operatorname{Tr}(1).
      \]
      The completed functor
      \(\operatorname{StTr}=\widehat{\bigoplus}_{d\ge1}
      \operatorname{StTr}_d\) lands in the declared Tate completion
      chosen for the theorem using it.  In the formal-Darboux theorem
      this output is identified with the reduced Hamiltonian generator
      space \(\mathfrak h\), and
      \(A_{\partial,H}^{\mathrm{st}}\) is the completed symmetric
      algebra on that output.  Physical large-\(N\) trace-state data
      and cumulants are separate.  Here
      \(H^0(\mathcal R_N^{GL_N},Q)\) means the
      Chevalley--Eilenberg-length-zero invariant Koszul cohomology of
      the reduced polynomial brane complex.  Reductive exactness is the
      finite-window rational-module statement of
      Lemma~\ref{lem:reductive-invariants-koszul-exactness};
      Roos/strict-limit data control the later inverse system.  Positive
    ...
    \item \emph{Operation estimates.}  Every product, bracket,
      ...
      Costello counterterm tower acting on an object of
      ...
      window-loss term is recorded.  A theorem invoking a diagonal
      Casimir or diagonal local-cohomology kernel must state the topology
      in which the displayed infinite sum converges.
  \end{enumerate}
...
\begin{proof}
  ...
  supplies only evaluation, not the inverse kernel.  Therefore a
...
\begin{defn}[Trace-sector reduction]\label{defn:trace-sector-reduction}
  ...
  Chevalley--Eilenberg length, not polynomial word degree.
  ...
  finite-\(N\) Hochschild centre means the derived centre represented by
...
\paragraph{\texorpdfstring{Stable trace range and physical large-\(N\) conventions.}{Stable trace range and physical large-N conventions}}
