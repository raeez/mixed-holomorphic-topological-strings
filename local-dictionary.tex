...
\paragraph{Host category: Tate $\C$-vector spaces.}
...
Beilinson--Drinfeld factorizable-$\mathcal D$-module-on-$\mathrm{Ran}(\C^2)$
description, equivalent to it via the de Rham complex; the BV /
heat-kernel framework selects Costello--Gwilliam as native.
...
\item[(i)]\emph{(Support locality on the brane line.)}  The equal-time
  bracket kernel \(\delta(t-t')\) on two brane-time points (with \(s\)
  reserved for the transverse \(\R_s\) coordinate of \(X\)) implies,
  after smearing against test functions \(a(t),b(t')\),
  \[
...
operators force the \(\C^2\) holomorphic factorization algebra
\(\mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}\) and its mixed
topological-current enhancement
\(\mathcal F_{\mathrm{Ham}}^{\mathrm{mix}}\).
\end{thm}

\paragraph{Holomorphic factorization on \(\C^2\) and its curve
restrictions.}
...
\(\mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}\) on a holomorphic curve,
not the two-complex-dimensional algebra itself.\\
\hline
\end{longtable}
...
action.  Outside the Fourier--Rees theorem the spans of
