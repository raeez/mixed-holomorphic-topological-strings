...
\]
rests on a holomorphic orientation on the formal polydisk
\((\widehat\C^2_0,\omega)\); three locality operators on the brane
line; and the separation between native two-complex-dimensional
structure and one-complex-dimensional reductions.

Each entry names an algebraic side (a CE/PV cochain, a residue module,
a shifted-cotangent generator, a coordinate ring), a physical side (a
Hamiltonian current, a brane measurement, a principal-part observable),
and the comparison morphism between them.  An entry without a
comparison morphism appears as a named obstruction class.
...
\end{enumerate}
The \(Q\)-cohomology of (ii) and (iii) carries the locality
conclusion; (i) is a chain-level identity after distributional
pairing.
...
currents \(\Theta_\rho(B)\) satisfy (i) and (ii) but not (iii):
\(\rho\in\mathcal P\) is a residue current at the brane point, not a
polynomial jet.

Items (ii) and (iii) restrict the dg Lie input of an acceptable
factorization algebra to be locally constant in
\(\R^2_{\mathrm{top}}\) and holomorphic in
...
\]
Item (i) selects the \(P_0\)-bracket on smeared currents.  The three
operators force the native \(\C^2\) holomorphic factorization algebra
...
\(\C^2_{\mathrm{hol}}\).  The taxonomy below records each native object
on \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) and the extra
reduction datum required to extract a curve VOA from it.
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}
  >{\raggedright\arraybackslash}p{0.67\textwidth}@{}}
\hline
...
\]
in the coordinate/admissible sense recorded below.  This is a
two-complex-dimensional holomorphic factorization algebra, not a
...
A restriction of the native \(\C^2\) Hamiltonian holomorphic
factorization algebra to a one-complex-dimensional vertex/OPE algebra
requires the extra datum
\[
  \mathfrak R_L
  =
...
to the restricted curve object, not to the two-complex-dimensional
holomorphic factorization algebra or the real interval current
algebra.\\
\hline
...
interval-current brackets; the data \(\zeta_{L,\hbar}\), the fields
\(J_f(w)\), the central level, and the OPE singular expansion are
separate constructions.\\
\hline
Curve chiral algebra interface tuple &
An externally constructed curve chiral algebra enters as the reduced
...
\(\mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}\), not the native \(\C^2\)
holomorphic factorization algebra.\\
\hline
\end{longtable}

\paragraph{Normal \(\Omega\)-background.}
The brane-preserving equivariant deformation scales the normal bundle
\(N_LX=\R_s\oplus\C_{z_1}\oplus\C_{z_2}\) with \(t\) fixed; a rotation
in the \((t,s)\)-plane does not preserve \(L\) as fixed locus.  The
...
The local obstruction habitat for
Problem~\ref{prob:quantum-p0-operation-realization}.  The scalar
...
not available for the raw full formal polydisk without admissibility data\\
\hline
\(c^I\) &
\(\theta^I\) &
...
finite-certificate-relative datum: formal coefficients, Capelli
...
Algebraic side: the brane probe at the \(\hbar=0\) special fibre.
...
a distribution, no product of two distributions appears.
\begin{center}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}
  >{\raggedright\arraybackslash}p{0.67\textwidth}@{}}
\hline
