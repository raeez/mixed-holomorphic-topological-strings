...
component
\(\mathfrak o_{\mathrm{prim}}\subset\operatorname{Ob}_T\), expanded
into its four constituents (centrality, QME, descent), under the
sublattice constraint just displayed.  In that filtration the
chain-level primitive shadow constructed below is the \emph{associated
graded} of the unreduced lift problem on its zero-\(\psi\) Hamiltonian
and one-\(\psi\) descendant generator subspaces, equivalently the
\(E_1\)-page of the obstruction-filtration spectral sequence whose
\(E_\infty\)-page is \(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\) itself.
The chain-level construction is honest evidence on the \(E_1\)-page;
its strict \(Q\)-, factorization-, and \(P_0\)-bracket compatibilities
are the \(d_1\)-vanishings.  The four enumerated obstruction classes
above are the higher differentials, none of which is supplied by the
...
\begin{proof}
  ...
  Item~(1).  A \(P_0\)-centrality homotopy is a chain
  \(h^{P_0}_{\mathrm{cent}}\) on the full unreduced open observable
  complex with
  \(Q h^{P_0}_{\mathrm{cent}}+h^{P_0}_{\mathrm{cent}}Q
   =\Phi\circ\{-,-\}_{\mathrm{closed}}-\{-,-\}_{\mathrm{open}}\circ
   (\Phi\otimes\Phi)\), and analogously for the factorization product;
  the chain-level primitive shadow is a projection onto the
  generator subspace and contains no information about how to extend
  \(\{-,-\}_{\mathrm{open}}\) past the symmetric-algebra augmentation
  ideal squared.  By
  Theorem~\ref{thm:chain-level-primitive-projection-P0}, the homotopy
  on the indecomposable subspace can be taken to be zero, but this is
  the \(d_1\)-vanishing of the spectral sequence on \(F_1/F_0\), not the
  full \(P_0\)-centrality homotopy on \(F_{\geq2}\).
