...
\emph{Named obstruction class.}  The missing primitive in the
\(\varprojlim^1H^0\) tower of
Theorem~\ref{thm:wt-nonscalar-counterterm-criterion}, namely the
secondary class
\[
  \lambda_n\in
  \varprojlim\nolimits^1_M H^0\bigl(\mathcal Q^\bullet_{w,M}(I)\bigr),
\]
in which \(\mathcal Q^\bullet_{w,M}(I)\) is the finite-window
non-scalar local-functional complex of
Definition~\ref{defn:wt-nonscalar-qme-tower}.  Vanishing of
\(\lambda_n\) is the precise primitive-compatibility datum that
licences a chain-level primitive of the \(P_0\)-shadow recursion
compatibly with truncation; nonvanishing is the Roos primitive
obstruction.
