...
changing polarization rather than module action.
...
\begin{thm}[Primitive one-antifield homology]\label{thm:T5-one-psi-homology}
  ...
  removed constant-Hamiltonian sector rather than in
  \(\bar A_{\mathrm{desc}}\).
\end{thm}
...
\begin{constr}[Reduced Moyal principal-part descendant target]
  ...
  target rather than by correcting the polynomial one-\(\psi\)
  representatives.
...
\begin{rmk}\label{rmk:role-of-deRham-contraction}
  ...
  in degree \(1\) for an open interval \(I\), represented by
	  ...
	  against this density rather than point-evaluation in \(H_c^0(I)\).
\end{rmk}
