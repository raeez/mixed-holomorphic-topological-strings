...
\]
that exists strictly, without heat-kernel propagator or finite-scale
regularization, and realizes the indecomposable \(P_0\)-shadow of the
reduced cotangent comparison.
...
\(\mathfrak o_{\mathrm{prim}}\) on the \(F_1/F_0\) layer of the
trace-count filtration; the residue is the pair
...
Theorem~\ref{thm:no-same-action-rees-fourier-descendants}.  The Roos
secondary class
\(\lambda_n\in
...
counterterms, not for the chain-level construction.
...
copy
\(\bar A_{\mathrm{desc}}=\bigoplus_{a+b>0}\C\cdot[\Psi_{a,b}]\); no
higher-\(\psi\) correction line is available.
...
(Proposition~\ref{prop:stable-trace-invariants}); no analytic
propagator and no Tate-completion convergence input enters.
...
shows that the classical primitive map does not intertwine the Moyal
coadjoint action on the principal-part target.  The exact same-action
Rees obstruction
Theorem~\ref{thm:no-same-action-rees-fourier-descendants} pins this
to a local-nilpotence mismatch, which the Fourier-twisted Rees
construction
...
\begin{proof}
  ...
  Applying $\pi_{\mathrm{prim}}$ produces the single-trace
  $ab\otimes\{f,g\}_{\bar A}$ in
  $\Omega^\bullet_c(I)\widehat\otimes\bar A$.

  On the other route, $\pi_{\mathrm{prim}}B_f(a)=a\otimes f$ and
  $\pi_{\mathrm{prim}}B_g(b)=b\otimes g$, and the bracket on
  $\Omega^\bullet_c(I)\widehat\otimes\bar A$ is the
  $P_0$ bracket of Construction~\ref{constr:interval-fact-algebras}:
  $\{a\otimes f,b\otimes g\}_{\bar A}=ab\otimes\{f,g\}_{\bar A}$.  The
  two routes agree.
  ...
  remaining $(k-1)$-trace tail).  The result lies in $F_k$, and by
  Lemma~\ref{lem:primitive-shadow-kernel} $\pi_{\mathrm{prim}}$ of
  this is zero whenever $k\geq 2$.  Conversely
  $\pi_{\mathrm{prim}}G=0$ because $G\in F_{\geq 2}$, so
  $\{\pi_{\mathrm{prim}}F,\pi_{\mathrm{prim}}G\}_{\bar A}=0$.

  \emph{Stratum $(k_1,k_2)$ with $k_1\geq 2$ or $k_2\geq 2$.}  Both
  sides vanish by the same Leibniz argument.

  \emph{Stratum $(0,k)$ or $(k,0)$.}  One factor lies in $F_0$, a
  scalar de Rham form.  The bracket of a scalar de Rham form with
  anything is zero (the canonical equal-time bracket acts only on
  matrix fields), so
  $\{F,G\}_{\mathrm{open}}=0$ and
  $\{\pi_{\mathrm{prim}}F,\pi_{\mathrm{prim}}G\}_{\bar A}=0$.

  In every stratum the difference is zero; $h_{P_0}\equiv 0$ realizes
  the homotopy formula trivially.
\end{proof}
...
\begin{rmk}[Why the homotopy is zero]\label{rmk:why-homotopy-zero}
  ...
  single-trace component, so the only stratum left for a potential
  defect is the multi-trace stratum, on which both sides of the
  comparison square are identically zero.
\end{rmk}
...
\begin{rmk}[Comparison with the heat-kernel propagator route]
  ...
  The chain-level route is independent of the heat kernel and hence
  independent of the Tate-completion convergence question of
  Problem~\ref{prob:formal-factorization-center}, item~(1).  It
  produces no renormalized BV homotopies; the one-loop and higher-loop
  quantum lift requires the Costello renormalization of the
  brane-defect propagator, recorded separately as
...
\begin{thm}[Scope of the chain-level primitive indecomposable
shadow]\label{thm:T5-primitive-shadow-scope}
  The chain-level primitive indecomposable \(P_0\)-shadow map of
  Theorem~\ref{thm:chain-level-primitive-projection} supplies the
  associated-graded \(E_1\)-page evidence on the zero-\(\psi\) and
  one-\(\psi\) primitive subspaces of the unreduced cotangent
  factorization-centre obstruction filtration: equivalently the
  generator-subspace component of
  Problem~\ref{prob:formal-factorization-center}~item~(3) and the
  indecomposable-shadow component of
  Theorem~\ref{thm:main-local}.  It does not supply, and is not a
  surrogate for, any of the following four data of the unreduced lift:
  \begin{enumerate}
    \item \emph{(Centrality homotopies.)}  The strict cochain-level
      \(P_0\)-centrality homotopy
      \(h^{P_0}_{\mathrm{cent}}\) of unreduced principal-part currents
      against arbitrary unreduced open observables, and the
      factorization-product centrality homotopy
      \(h^{\mathrm{prod}}_{\mathrm{cent}}\), corresponding to the
      components
      \(\operatorname{ob}_{\mathrm{cent}}^{P_0}\) and
      \(\operatorname{ob}_{\mathrm{cent}}^{\mathrm{prod}}\) of
      \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\).  The chain-level
      primitive shadow is strictly compatible with the open \(P_0\)
      bracket and the factorization product on the indecomposable
      quotient (Theorems
      \ref{thm:chain-level-primitive-projection-Q},
      \ref{thm:chain-level-primitive-projection-factorization},
      \ref{thm:chain-level-primitive-projection-P0}); it does not
      address either centrality homotopy on the full unreduced complex,
      because the unreduced complex contains positive-trace-count
      strata which the indecomposable projection kills.
    \item \emph{(Tate/weighted coefficient kernel.)}  The convergent
      Casimir kernel
      \(C_{\mathfrak h}=\sum_{d\geq1}\sum_i e_{d,i}\otimes e^i_d\) inside
      a coefficient category in which the Costello propagator
      \(P_{\epsilon,L}\) extends, the component
      \(\mathfrak o_{\mathrm{Cas}}\) of \(\operatorname{Ob}_T\), and the
      stratified Weiss \v{C}ech--Roos descent class
      \(\delta_{\check C/R}^{\mathrm{Weiss}}\) of
      Remark~\ref{rmk:weiss-ran-descent-obstruction}.  The chain-level
      construction uses no analytic input and therefore neither
      provides nor obstructs the Tate completion.
    \item \emph{(Brane-defect propagator extension.)}  The Costello
      heat-kernel propagator \(P_{\epsilon,L}\) extended to the
      brane-defect coefficient module of
      \ref{defn:endpoint-admissible-coefficient-module}, the component
      \(\mathfrak o_{\mathrm{BV,end}}\) of \(\operatorname{Ob}_T\), and
      its renormalized one-loop and higher-loop QME endpoint
      \(\operatorname{ob}_{\mathrm{QME}}^{\mathrm{bd}}\).  The
      chain-level construction is the analytic-free \(E_1\)-page
      surrogate for this datum on the indecomposable subspace; the
      renormalized BV homotopies of Costello are recorded separately as
      Problem~\ref{prob:formal-factorization-center}, items~(1)--(2).
    \item \emph{(One-\(\psi\) Moyal-coadjoint intertwiner.)}  The
      polynomial PBW descendant action and the principal-part
      coadjoint action are not isomorphic as \(\mathfrak h_\hbar\)-modules
      (Proposition~\ref{prop:quantum-descendant-label-obstruction},
      Theorem~\ref{thm:no-same-action-rees-fourier-descendants}); the
      one-\(\psi\) label shadow records labels only and does not
      intertwine these two actions, equivalently the chain-level
      construction provides no quantum lift of the cotangent label map.
  \end{enumerate}
  In the spectral-sequence presentation, the chain-level primitive
  shadow is the \(E_1\)-page and the higher differentials
  \(d_2,d_3,\ldots\) are the four enumerated data.  The chain-level
  construction is sufficient \emph{exactly and only} for the classical
  associated-graded primitive indecomposable shadow on the displayed
  generator subspaces.
\end{thm}

\begin{proof}
  Theorem~\ref{thm:chain-level-primitive-projection} establishes the
  four positive compatibilities (\(Q\), factorization, \(P_0\)-bracket,
  cosheaf) on the indecomposable quotient.  Each negative item of the
  scope statement is a separate residual:

  Item~(1).  A \(P_0\)-centrality homotopy on the unreduced complex
  is a chain \(h^{P_0}_{\mathrm{cent}}\) trivializing
  \(\{\Phi(\xi),F\}_{\mathrm{open}}\) for every closed input
  \(\xi\) and every open observable \(F\), at the chain level rather
  than only on cohomology; analogously for
  \(h^{\mathrm{prod}}_{\mathrm{cent}}\) and the factorization product.
  The chain-level primitive shadow is a projection onto the generator
  subspace \(F_1/F_0\) and contains no information about the
  bracket of \(\Phi\)-image against an element of \(F_{\geq2}\) at
  the chain level; the indecomposable projection annihilates
  \(F_{\geq2}\), so the strict zero homotopy of
  Theorem~\ref{thm:chain-level-primitive-projection-P0} is the
  \(d_1\)-vanishing of the obstruction-filtration spectral sequence
  on \(F_1/F_0\), not the full \(P_0\)-centrality homotopy on the
  unreduced complex \(F_{\geq2}\).

  Item~(2).  The Tate/weighted Casimir convergence and the stratified
  Weiss \v{C}ech--Roos descent are coefficient-category and
  topology-of-covers data which the chain-level construction never
  uses; their habitats are
  \(\widehat{\mathfrak h}\widehat\otimes
  \widehat{\mathfrak h}^\vee_{\mathrm{cont}}\) and the Cech cone
  \(\Delta^{N,M}_{V,\mathcal U}\) of
  Remark~\ref{rmk:weiss-ran-descent-obstruction}, neither of which
  appears anywhere in the proofs above.

  Item~(3).  The chain-level shadow is independent of the analytic
  input \((P_{\epsilon,L},Q+\hbar\Delta)\) by
  Remark~\ref{rmk:no-heat-kernel-needed}; therefore it can neither
  produce the Costello renormalized BV homotopies nor obstruct them.
  The renormalized brane-defect QME class
  \(\operatorname{ob}_{\mathrm{QME}}^{\mathrm{bd}}\) is the residual
  endpoint of \(\mathfrak o_{\mathrm{BV,end}}\) and lives in the
  cohomology
  \(H^1(\mathcal O_{\mathrm{loc}}(\mathcal E_w),Q+\{I_w,-\})\) of
  \ref{thm:hadamard-mittag-leffler}~(iii), which is constructed from
  the Hadamard parametrix; this complex contains no chain-level
  primitive datum.

  Item~(4).  The negative result is
  Proposition~\ref{prop:quantum-descendant-label-obstruction} together
  with Theorem~\ref{thm:no-same-action-rees-fourier-descendants}: at
  bidegree \((1,1)\) on the test Hamiltonian \(z_1^4\) the polynomial
  PBW descendant action gives \(4\,\widetilde\Psi_{4,0}\) at order
  \(\hbar^0\), while the Moyal coadjoint action gives
  \(-24\,\hbar^2\rho_{0,4}+\cdots\); independently, the local
  nilpotence of \(z_1,z_2\) on the polynomial Rees Weyl module is
  incompatible with the non-local-nilpotence on
  \(\mc P((\hbar))\).  Both obstructions appear after the
  indecomposable projection, and decomposable terms cannot repair
  them.

  Hence the chain-level primitive shadow supplies exactly the
  associated-graded \(E_1\)-page on the displayed generator subspace,
  no more and no less.
\end{proof}
