...
\]
of Problem~\ref{prob:formal-factorization-center} exists if and only
if the joint obstruction
\[
  \operatorname{Ob}_{\mathrm{cent/QME/desc}}
    ...
    \bigr)
\]
vanishes with chosen compatible null-homotopies: the
...
specialization of
Theorem~\ref{thm:tate-cotangent-lift-iff} to the centrality / QME /
descent components of the obstruction vector
\(\operatorname{Ob}_T\).  In that filtration the chain-level
primitive shadow constructed below is the \emph{associated graded} of
the unreduced lift problem on its zero-\(\psi\) Hamiltonian and
one-\(\psi\) descendant generator subspaces, that is, the \(E_1\)-page
of the Beilinson--Roos spectral sequence whose \(E_\infty\)-page is
...
\(d_1\)-vanishings.  The higher differentials are the displayed
obstruction components, none of which is supplied by the present
construction.
