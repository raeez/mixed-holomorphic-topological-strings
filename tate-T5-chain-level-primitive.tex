...
constructs the primitive open-side shadow of the same stalk, not a
replacement OPE.
...
regularization, and realizes the Koszul-resolution cocycle representing
...
Rees obstruction
Theorem~\ref{thm:no-same-action-rees-fourier-descendants} identifies this
...
\begin{proof}
  ...
  preserving the trace count; smearing by a de Rham form on $I$ does not
  change it either, so $Q F_k\subset F_k$.
...
\begin{rmk}\label{rmk:no-heat-kernel-needed}
  The chain-level primitive shadow uses no heat-kernel propagator on
  $\R^2\times\C^2$: the Costello--Gwilliam role of
...
\begin{rmk}\label{thm:T5-primitive-shadow-scope}
  ...
  with Theorem~\ref{thm:no-same-action-rees-fourier-descendants}.  The
  chain-level construction uses no analytic input
  (\ref{rmk:no-heat-kernel-needed}); the renormalized BV homotopies
...
\begin{rmk}\label{rmk:role-of-deRham-contraction}
  ...
  against this density, not point-evaluation in \(H_c^0(I)\).
\end{rmk}
