...
\emph{Closure-layer organizing question.}  Item~(3) of
Problem~\ref{prob:formal-factorization-center}: which chain-level
projection
\[
...
\]
exists strictly, without heat-kernel propagator or finite-scale
regularization, and realizes the indecomposable \(P_0\)-shadow of the
reduced cotangent comparison?

\emph{Named obstruction class.}  The chain-level primitive component
\(\mathfrak o_{\mathrm{prim}}\) of the unreduced cotangent lift
obstruction vector
\(\operatorname{Ob}_T\) of
Definition~\ref{defn:tate-cotangent-lift-obstruction-vector}: the
...
chain-level construction below kills the indecomposable-shadow part
of \(\mathfrak o_{\mathrm{prim}}\) on the \(F_1/F_0\) layer of the
...
intertwiner whose unique survivor is
...
counterterms, not for the chain-level construction below.

\emph{Higher-\(\psi\) ceiling.}  For the cyclic chip-moving Koszul
complex \(K^\bullet_{R,S}\) of
...
\(\C[C_S]\)-semisimplicity.  Hence the chain-level primitive lives
...
higher-\(\psi\) correction line is available, and the missing-datum
search is constrained to that two-summand target.

\emph{Unreduced cotangent lift, scope equivalence.}  The unreduced
...
Remark~\ref{rmk:weiss-ran-descent-obstruction}.  This iff is the
restriction of Theorem~\ref{thm:tate-cotangent-lift-iff} to the
two components
...
Remark~\ref{rmk:weiss-ran-descent-obstruction}, retained here because
its local form is forced by the Weiss-cosheaf condition on the
unreduced factorization centre even when the cross-volume part of
\(\mathfrak o_{\mathrm{cv}}\) vanishes.
In that filtration the chain-level primitive shadow constructed below
is the \emph{associated graded} of the unreduced lift problem on its
...
\(\Phi_{\mathrm{fact}}^{\mathrm{unred}}\) itself.  The chain-level
construction is honest evidence on the \(E_1\)-page; its strict
\(Q\)-, factorization-, and \(P_0\)-bracket compatibilities are the
\(d_1\)-vanishings.  The four enumerated obstruction classes above
are the higher differentials, none of which is supplied by the
present construction.

\emph{Unique survivor.}
Theorem~\ref{thm:chain-level-primitive-projection}: the algebraic
primitive/indecomposable projection on the free symmetric algebra
\(\mathbf S(\mathrm{Cyc}(F)_{\mathrm{nonempty}})\), where
\(F=\C\langle x_1,x_2\rangle\) is the free associative algebra on the
two probe-matrix generators and
\(\mathrm{Cyc}(F)_{\mathrm{nonempty}}=F/[F,F]\setminus\C\cdot 1\) is
its space of nontrivial cyclic-word generators, composed with the
zero-\(\psi\) Hamiltonian abelianization
\(\mathrm{Cyc}(F)_{\mathrm{nonempty}}\twoheadrightarrow\bar A\), is a
strict \(Q\)-chain map, factorization-compatible after unit retention,
and \(P_0\)-bracket-compatible with explicit zero homotopy.

\emph{Construction inputs.}  The chain structure of the reduced BV
algebra (Construction~\ref{constr:reduced-bv-algebra}), the de Rham
...
propagator and no Tate-completion convergence input is used.

\emph{Residual.}  The quantum descendant label obstruction
Proposition~\ref{prop:quantum-descendant-label-obstruction}: the
classical primitive map does not intertwine the Moyal coadjoint
action on the principal-part target.  The exact same-action Rees
obstruction
...
\begin{constr}[Primitive one-antifield chain complex]\label{constr:T5-one-psi-complex}
  ...
  Theorem~\ref{thm:chain-level-primitive-projection-Q} reduces the
  question of strict \(Q\)-compatibility on this layer to a finite
  combinatorial homology calculation: identify every closed
  one-\(\psi\) primitive cycle modulo \(Q\) of a two-\(\psi\)
  primitive.  The chain complex below records exactly that calculation
  in bidegree \((k,l)\), and the next two results compute its
  \(H_1\)-line.
...
shadow]\label{thm:T5-primitive-shadow-scope}
  ...
  \end{enumerate}
  Equivalently, in the spectral-sequence presentation of the masthead,
  the chain-level primitive shadow is the \(E_1\)-page; the higher
  differentials \(d_2,d_3,\ldots\), are the four enumerated data, none
  of which is supplied here.  The chain-level construction is
  sufficient \emph{exactly and only} for the classical
