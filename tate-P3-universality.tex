...
\begin{proof}
  ...
  \(\Phi^{\mathrm{Ham}}_{\mathrm{fact}}\) of
  Theorem~\ref{thm:hamiltonian-current-factorization}, giving the
  bulk-to-boundary evaluation
  \(f\mapsto\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).  Scalar
  anomaly compatibility identifies \([\bar c]_\Theta\) with
  \([\bar c]\) inside \(H^2_{\mathrm{Lie}}(\bar A,\C)\) through the
  comparison map \(\chi_{\mathrm{anom}}\).  Composition of these
  three constructions defines \(\mu_{(X,\omega,p,\Theta)}\).
  ...
  \(GL_N\)-invariant and preserves the connected-trace projection.
