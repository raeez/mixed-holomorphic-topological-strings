...
datum.  Universality is therefore inscribed as a precise four-class
criterion, not a heuristic.  External use of the formal local model
in any compact CY, MNOP, GV, OSV, DT, PT, Vol III, Igusa, or BKM
target is governed by a separate matched-conventions firewall
...
\paragraph*{Universality residual.}  The four-coordinate class
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\) defined in the opening, and
realized by
Definition~\ref{defn:protected-twist-obstruction-datum}, is the only
data separating the ambient datum from the formal local Hamiltonian
sector.  Universality is the obstruction theorem
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}=0\Longleftrightarrow\)
admissible protected restriction
(Lemma~\ref{lem:protected-primary-obstruction-test},
Theorem~\ref{thm:protected-twist-hamiltonian-criterion}); the universal
terminus is the formal-disk model
(Corollary~\ref{cor:protected-formal-uniformity}); and the only
admissible cross-volume use is through the matched-conventions
interface (Corollary~\ref{cor:protected-matched-conventions-interface}),
governed by the firewall theorem of \S\ref{app:convention-contract}.
A claim of universality without a vanishing certificate or a named
nonzero coordinate is not mathematics.
