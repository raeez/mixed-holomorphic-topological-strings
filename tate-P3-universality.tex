...
proposes a brane-point datum that should reduce, on its protected
sector, to the formal-local Hamiltonian model of
\S\ref{ssec:local-theorem}.  The reduction is not automatic: it is
controlled by four obstruction classes -- the period of the symplectic
vector field, the closed-field comparison cone, the scalar-anomaly
mismatch, and the brane-defect quantum master equation.  Their joint
vanishing constructs the admissible protected Hamiltonian sector and
identifies it with the formal-local model.
