...
formal symplectic polydisk \(\widehat{\C^2}_0\).  An ambient theory --
...
restriction of that ambient datum to the formal polydisk
\(\widehat X_p\) the \emph{same} formal Hamiltonian model, for every
\((X,\omega)\), every \(p\), and every admissible ambient source?  And
if so, by which class does the comparison fail when an ambient
datum is rejected?

\paragraph*{Unique survivor.}  The word \emph{universal} earns a
single precise meaning here.  The formal-local Hamiltonian model is
the terminal object in the category whose objects are admissible
protected Hamiltonian sectors at brane points of connected
holomorphic-symplectic surfaces and whose morphisms are formal
Darboux symplectomorphisms intertwining the protected
bulk-to-boundary structure
(Theorem~\ref{thm:formal-local-universal-property} below).
Concretely, every admissible protected sector \emph{factors uniquely
up to canonical isomorphism} through the triple
\[
...
\]
Whether a candidate ambient datum lies in this category at all is
the obstruction theorem: it lies in the category iff the
four-coordinate \emph{universality residual class}
\[
...
universality without a vanishing proof or a named nonzero
coordinate is not a theorem of this paper.

\paragraph*{Habitat.}  The four coordinates of
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\) lie in independent
cohomology groups, so no coordinate is implied by the others.  The
period class \(\operatorname{per}(v)\) sits in the de Rham extension
\(0\to\C_X\to\mc O_X\to\Omega^1_{X,\mathrm{cl}}\to0\); the
closed-field comparison cone \(\mc C_{\mathrm{cl},p}\) measures
field-level mismatch on the protected closed sector; the scalar
anomaly mismatch \(\operatorname{ob}_{\mathrm{scal}}\) lives on
\(\bar A=\C[z_1,z_2]/\C\cdot1\); the brane-defect QME class
\(\operatorname{ob}_{\mathrm{QME}}\) lives on the weighted protected
local-functional complex.  A connected compact surface with
vanishing period need not have an acyclic comparison cone; a
balanced scalar anomaly need not produce a QME-flat brane
interaction; a QME-flat sector need not match the local scalar
class.  The universal-property statement is therefore not the
assertion that all four classes are tied \textemdash{} it is the assertion
that, when all four vanish, the protected sector \emph{is}
\(\mathfrak M^{\mathrm{Ham}}_{\mathrm{loc}}\) up to canonical
isomorphism, and when one is nonzero the precise coordinate records
what the ambient datum carries beyond what the formal local model
has absorbed.

\paragraph*{Closure.}  Three closures separate four claims.  First,
the universal property
(Theorem~\ref{thm:formal-local-universal-property}) is a categorical
statement: terminality under the admissibility-respecting
morphisms.  Second, the obstruction theorem
(Theorem~\ref{thm:protected-twist-hamiltonian-criterion}) is the
iff-vanishing statement converting universality into a four-class
criterion on candidate ambient data.  Third, the cross-volume
firewall
(Corollary~\ref{cor:protected-matched-conventions-interface},
\S\ref{app:convention-contract}) blocks any compact, modular,
automorphic, or virtual-cycle conclusion from following from
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}=0\) alone: external
acceptance into a compact CY, MNOP, GV, OSV, DT, PT, Vol III, Igusa,
or BKM target is a separate matched-conventions descent class with
its own obstruction vector.  Together these three closures
say that universality of the formal-local Hamiltonian sector is a
universal-property theorem with a measurable failure class and a
firewalled cross-volume use; nothing more, and nothing less.

\paragraph*{Cross-volume hook for Measurement Universality.}  The
universal-property formulation here is the formal-local image of a
broader Measurement Universality statement at the principle layer.
The principle-layer envelope characterizes the GL\(_N\)-invariant
trace evaluation
\(f\mapsto\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) by
characterizing axioms (Darboux-natural, polynomial,
GL\(_N\)-invariant, scalar-reduced, with linear part sending
\(z_i\) to \(\operatorname{Tr}\phi_i\) modulo the killed scalar
trace, and Hamiltonian variation matching the moment-map action)
and asserts existence and uniqueness up to canonical
isomorphism on the stable connected trace algebra
\(H^0(\mathcal R_N^{GL_N},Q)_{\mathrm{conn,st}}\) of
\S\ref{ssec:open-brane-operators}; its formal-local image is the
bulk-to-boundary evaluation triple of
Corollary~\ref{cor:protected-formal-uniformity}.  This file does not
assert the principle-layer Measurement Universality Theorem; it
records the formal-local universal-property image and the obstruction
class \(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\) that a
principle-layer formulation must control under formal-disk
restriction.
...
\begin{thm}[Universal property of the formal-local Hamiltonian sector]\label{thm:formal-local-universal-property}
...
depends on the Chan-Paton matrix factor.  The next two theorems
sharpen this coordinate.  The Casimir-balanced equivariant scalar
QME criterion
...
\begin{rmk}[Dirac language inside the protected sector]
  ...
  language \textemdash{} the brane as a constrained
  matrix probe whose stable scalar-reduced measurement is the unique
  \(GL_N\)-invariant Darboux-natural extension of the moment-map
  action on linear generators
  (Theorem~\ref{thm:formal-local-universal-property}) \textemdash{} is a
  shorthand for the already identified formal local model.  The brane stack probes the
  formal symplectic normal polydisk; Dirac reduction gives the open trace
...
\paragraph*{Universality residual.}  Three statements together
\emph{are} universality of the formal-local Hamiltonian sector, and
nothing else in this file is.  First, the universal property
(Theorem~\ref{thm:formal-local-universal-property}) identifies the
formal-local Hamiltonian model
\(\mathfrak M^{\mathrm{Ham}}_{\mathrm{loc}}\) as the terminal object
of \(\mathsf{ProtHam}\), giving existence and uniqueness up to
canonical isomorphism of the morphism
\(\mu_{(X,\omega,p,\Theta)}\) for each admissible object.  Second,
the obstruction iff
(Lemma~\ref{lem:protected-primary-obstruction-test},
Theorem~\ref{thm:protected-twist-hamiltonian-criterion}) is the
membership criterion for \(\mathsf{ProtHam}\): a candidate ambient
datum supplies an object of \(\mathsf{ProtHam}\) iff
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}=0\) with chosen
null-homotopies; a single nonzero coordinate names the exact ambient
mechanism that fails.  Third, the matched-conventions firewall
(Corollary~\ref{cor:protected-matched-conventions-interface},
\S\ref{app:convention-contract}) is the cross-volume gate: the
formal-local image of an admissible ambient datum is accepted by a
compact BCOV, MNOP, GV, OSV, DT, PT, Vol III, Igusa, or BKM target
only after a separate matched-conventions descent class vanishes,
component by component, in the target convention complex.

The universal property is therefore inscribed as a categorical
statement, not a heuristic, and the word \emph{universal} is reserved
in this file for precisely this terminal-object characterization and
its explicit obstruction class
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\).  No compact-target,
modular, automorphic, or virtual-cycle conclusion is licensed by
\(\operatorname{Ob}^{\mathrm{univ}}_{p,X}=0\) alone; that conclusion
is the cross-volume gate of
Corollary~\ref{cor:protected-matched-conventions-interface}, which
this file does not assert.

% \textemdash{} End of fragment.
