...
\begin{equation}\label{eq:typed-centre-first-taylor}
  ...
  C^\bullet_{\mathrm{Hoch},\mathrm{adm}}(A,A)[1].
\end{equation}
The formal-Darboux theorem prescribes the Hamiltonian trace-sector face
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) of this coefficient.  Only
after pairing it
...
is the coordinate shadow of
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\), not a map
...
\[
  ...
  +\cdots=0.
\]
The formula \(1-m\) for \(H^m\) is a degree in the full shifted
centrality convolution complex; it includes the source and open-input
suspensions.  Target Hochschild degrees depend on the operation slot.
For degree-zero Hamiltonians,
\(H^{\mathrm{Lie}}_{f,g}\) has degree \(-1\) in
\(C^\bullet_{\mathrm{Hoch},\mathrm{adm}}(A,A)[1]\), equivalently
unshifted total Hochschild degree \(0\): its arity-one derivation
representative has internal degree \(-1\).  The pure Lie
\(H^{3,\mathrm{Lie}}_{f,g,h}\) has shifted-target degree \(-2\),
equivalently unshifted total degree \(-1\).  By contrast, if an action
primitive is represented by \(M_b\) with \(b\in A^{-1}\), then
\(M_b\in C^0_{\mathrm{Hoch}}(A,A)\) has unshifted total degree \(-1\)
and shifted-target degree \(-2\); its open-input suspension is part of
its degree in \(\mathcal K^{\mathrm{cent}}_{\partial,N}\).  Thus the
uniform primary degree belongs to the full coloured convolution
grading, not to the Hochschild target alone.
...
\(P_0\)-actions prescribed independently of \(\tau_{N,1}\).  On the
trace face this prescription is
\[
  O_g=\widetilde J_N^s(g),\qquad
  \rho_f(O_g)
    =\widetilde J_N^s\!\left(
       \overline{\{s(f),s(g)\}}\right)
      +N\omega(f,g)\mathbf1 .
\]
A strict centre class is the special case
...
\]
The scalar-reduced CE/PV theorem supplies a strict infinitesimal
Hamiltonian action on its abstract trace-coordinate chart;
Proposition~\ref{prop:trace-sector-darboux-flat-transport} packages this
coordinate action as a strict flat transport datum.  A realization by
chosen finite-rank Koszul cocycles instead uses the
\(H^{\mathrm{Lie}},H^{\mathrm{act}},H^3,\ldots\) components of the
Hochschild-centre Maurer--Cartan datum.  The bar-centre family requires
the separate primitives
\((h_{\nabla},h_{\nabla,\Theta})\) satisfying the two displayed
\(d_{\mathrm{Darb}}\)-equations.
\end{defn}

\begin{prop}[Strict Darboux transport on the abstract trace-coordinate chart]
...
\]
and on the abstract completed trace-coordinate target
\[
  H^{\mathrm{tr}}_{N,\mathrm{adm}}
  =
  \langle \theta_g,M_{J(g)}:g\in\mathfrak h\rangle_{\mathrm{coord}},
\]
by
\[
...
product and bracket filtrations.  They define a strict flat
Darboux-transport datum on the scalar-reduced coordinate trace-sector
chart
\[
...
If the symbols are realized by chosen finite-rank Hochschild cocycles,
\(\theta_f=X_{f,N}\), their actual adjoint action obeys
\[
  \begin{aligned}
    [X_{f,N},X_{g,N}]_G
      -X_{\overline{\{s(f),s(g)\}},N}
      &=-d_{\mathrm{cent}}H^{\mathrm{Lie}}_{f,g},\\
    [X_{f,N},M_{\widetilde J_N^s(g)}]_G
      -M_{\widetilde J_N^s(\overline{\{s(f),s(g)\}})}
      -N\omega(f,g)M_{\mathbf1}
      &=-d_{\mathrm{cent}}H^{\mathrm{act}}_{f|g}.
  \end{aligned}
\]
It is therefore homotopy-flat only after these primitives, the
\(d_{\mathrm{cent}}H^3=-\delta^3\) coherence, and all higher
Maurer--Cartan components have been supplied.  Transport between
Darboux charts further requires these cochains to form a cocycle in the
Darboux/Weiss total descent complex.  The strict coordinate calculation
does not supply that finite-rank or descended action.
\end{prop}
...
\[
  ...
  L_{\{f,g\}}(x_h).
\]
The coordinate differentials are equivariant because the CE/PV
trace-sector complex uses the adjoint \(\mathfrak h\)-module structure.
Finally
\[
...
\]
Thus the abstract trace-coordinate preconnection has zero curvature
and preserves the coordinate chart strictly.  The preceding
finite-rank equations show why no strict statement about its chosen
Hochschild representatives, the non-trace Hochschild summands, or the
full Morita centre follows from this calculation.
\end{proof}

\begin{defn}[Relative Darboux extension obstruction]
\label{defn:relative-darboux-extension-obstruction}
Fix the strict abstract trace-coordinate transport of
Proposition~\ref{prop:trace-sector-darboux-flat-transport}.  The
construction below concerns this strict coordinate chart.  A
finite-rank Hochschild realization instead requires a full arity-one
extension \(\widetilde\tau_{N,1}\) of
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\), including the separately
supplied \(P[1]\)-component, and the analogous convolution complex
twisted at the complete Maurer--Cartan base point
\[
  \mathcal C_{\mathrm{tr}}
    =\widetilde\tau_{N,1}
      +H^{\mathrm{Lie}}+H^{\mathrm{act}}
      +H^{\mathrm{prod}}+H^{P_0}+H^3+\cdots,
  \qquad
  d_{\mathcal C_{\mathrm{tr}}}
    =d_{\mathrm{cent}}+
      [\mathcal C_{\mathrm{tr}},-]+\cdots ,
\]
together with a proof that the trace receiver is a cochain map for the
twisted differential.  That homotopy-relative Darboux complex is
additional data; it is not supplied by the strict coordinate
calculation.  The
...
the coordinate Chevalley--Eilenberg/polyvector differential on
\(H^{\mathrm{tr}}_{N,\mathrm{adm}}\), and
...
\]
a preconnection \(\nabla_0\) extending the strict abstract coordinate
transport has
\[
  r_{\mathrm{tr}}(\Omega_\nabla)=0,\qquad
...
\(\nabla_0\) whose restriction is the abstract trace-coordinate
transport of
...
\begin{proof}
The abstract trace-coordinate curvature and its Maurer--Cartan
compatibility curvature vanish by
...
to the zero primitives of the abstract coordinate transport.  A
finite-rank Hochschild realization requires the separately supplied
homotopy-relative complex twisted at \(\mathcal C_{\mathrm{tr}}\).
\end{proof}
...
\begin{equation}\label{eq:bar-resolution-derived-centre}
        ...
        \end{gathered}
\end{equation}
        The first two arrows are the sought full centre lift and its
        arity-one coefficient.  The formal-Darboux theorem prescribes
        only
        \(\widetilde\tau_{N,1}|_{\mathfrak h}
          =\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\);
        its \(P[1]\)-component is part of the cotangent-current datum.
        The last line is the coordinate-dual trace expression, not the
        type of the twisting cochain itself.
...
Hamiltonian face of the first Taylor coefficient
\[
  \tau_{N,1}^{\mathrm{tr}}\big|_{\mathfrak h}\colon
  \mathfrak h\longrightarrow
  C^\bullet_{\mathrm{Hoch},\mathrm{adm}}\!
    (A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})[1],
\]
which is the prescribed restriction of an arity-one coefficient for a
candidate continuous twisting cochain on
\(\overline C^{\mathrm{CE},\mathrm{cont}}_\bullet(\mathfrak g)\).
Its component on \(P[1]\) is a separate cotangent-current datum.
...
Hamiltonian face
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\); its
...
\(E_1^{\mathrm{top}}\)-product on
  ...
  \(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) on the Hamiltonian face,
  while the \(P[1]\)-component is a separate cotangent-current datum.
  Under continuous coordinate duality and
...
\[
  ...
  =
  \bigl(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h},
        K^{(2)}_\Delta\bigr).
\]
...
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\).  Its coordinate-dual values are
...
\begin{enumerate}[label=\textup{(D\arabic*)}]
  ...
  \item \emph{Hochschild-centre chart lift.}
    ...
    \[
      ...
      \end{aligned}
    \]
    Here \(\rho\) and \(\rho^{P_0}\) are prescribed independently of
    \(\tau_{N,1}\), as in
    Definition~\ref{defn:typed-hochschild-centre-lift-datum}; on the
    trace face \(\rho_f(\widetilde J_N^s(g))\) is the ordered
    representative of the source Poisson action plus
    \(N\omega(f,g)\mathbf1\).
    ...
    Equivalently, after fixing a full first Taylor coefficient whose
    Hamiltonian restriction is
    \(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) and whose
    \(P[1]\)-component is separately supplied, the relative class
    \[
    ...
    In the full shifted centrality convolution grading \(H^m\) has
    degree \(1-m\).  Its target Hochschild degree also includes the
    operation-slot and input suspensions and is not uniformly \(2-m\).
    The
    ...
    \(\Phi_N^{\mathrm{gen}}\) of the Hamiltonian trace face.
  ...
  \item \emph{Darboux-torsor transport.}
    ...
    strict abstract trace-coordinate transport of
    ...
    full primitives must restrict to zero coordinate-chart primitives
    for a strict extension, or to coordinate-exact primitives for
    extension up to gauge homotopy.  A finite-rank Hochschild
    realization instead requires a separately supplied relative
    complex and a cochain map to the endomorphism/Hom deformation
    complex whose differential is twisted at
    \(\mathcal C_{\mathrm{tr}}\).  Under this map the full Darboux
    curvature cocycles must admit compatible finite-rank primitives;
    the strict coordinate receiver alone supplies neither the map nor
...
Chevalley--Eilenberg chain coalgebra, followed by desuspension.  Fix a
supplied full first Taylor coefficient
\[
  \widetilde\tau_{N,1}\colon
  \mathfrak g_{\mathrm{src}}\longrightarrow
  C^\bullet_{\mathrm{Hoch},\mathrm{adm}}(A,A)[1].
\]
Its restriction to \(\mathfrak h\) is the formal-Darboux Hamiltonian
face \(\tau^{\mathrm{tr}}_{N,1}|_{\mathfrak h}\); its restriction to
the cotangent-current summand \(P[1]\) is a separately supplied
continuous current cochain.  Only the Hamiltonian trace face has the
coordinate-dual expression
\[
  \Phi_N^{\mathrm{gen}}(c_f)=\theta_f,\qquad
  \Phi_N^{\mathrm{gen}}(u_f)=M_{J(f)} .
\]
...
\]
twisted by the supplied full coefficient \(\widetilde\tau_{N,1}\).
Its elements are centrality
...
\]
with
\(r_{\mathrm{gen}}(\mathcal C_{\le m})=\widetilde\tau_{N,1}\),
...
centrality curvature before imposing the supplied full first Taylor
coefficient; the relative class above is the obstruction to extending
that coefficient to a continuous Hochschild-centre twisting cochain.
\end{defn}
...
\(\widetilde\tau_{N,1}\) extends to order \(m+1\) if and only if
\[
...
\[
  ...
  (\mathcal C_{\le m}).
\]
Strict preservation of the supplied full first Taylor coefficient means
\[
...
\begin{proof}
This is the obstruction theory of the filtered homotopy fibre of
\(r_{\mathrm{gen}}\), based at \(\widetilde\tau_{N,1}\).  The
...
\begin{proof}
The formal-Darboux trace fibre \(T_0\) fixes only the Hamiltonian
restriction \(\tau^{\mathrm{tr}}_{N,1}|_{\mathfrak h}\), its
coordinate-dual trace values, and the binary diagonal kernel.  A full
first Taylor coefficient enters rows \(\mathrm{D3}\) and \(\mathrm{D5}\)
only after a continuous cotangent-current cochain on \(P[1]\) has been
supplied.
...
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) on the Hamiltonian face;
the full coefficient also contains the separately supplied
cotangent-current component.  Its coordinate-dual expression is
...
required to extend the strict abstract trace-coordinate transport, the sharper
...
with primitives whose restrictions are the zero coordinate-chart
primitives or coordinate-exact gauge homotopies.  For the finite-rank
Hochschild realization, a homotopy-relative Darboux complex twisted at
\(\mathcal C_{\mathrm{tr}}\) and compatible with Darboux/Weiss
restrictions is additional data.  This full bar-centre extension is not
...
\(H^3\in(\mathcal K^{\mathrm{cent}}_{\partial,N})^{-2}\), and
  \(H^m\in(\mathcal K^{\mathrm{cent}}_{\partial,N})^{1-m}\) in the
  full shifted coloured-convolution grading; target Hochschild degrees
  are determined separately by the operation and input slots &
...
supplies the Hamiltonian restriction
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) entering
\(\mathrm{O}_{\mathrm{Hoch}}\); the coordinate expression
...
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\colon\mathfrak h\to
...
\(\tau_{N,1}^{\mathrm{tr}}|_{\mathfrak h}\) with coordinate-dual CE/PV expression
