...
trace-sector generator boundary value in Hochschild cochains
...
\(\mathcal B^{\mathrm{adm}}\).  It is a quasi-isomorphism after the
following five pieces of chain-level data have been constructed:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
The first three pieces of data give the three source components of
...
central extension.  The fourth datum is therefore the strict
trivialisation of the anomaly and locality rows.  The fifth datum is the
acyclicity statement for the comparison cone.  Steinness supplies no
all-arity operation, no counterterm tower, and no cone contraction.
\end{proof}
...
Then the supplied OCA map is a quasi-isomorphism in the projective category
...
curvature of the trace-sector action.  The Stein hypothesis contributes
only the acyclicity row of the sheaf-theoretic descent complex; the
operadic, QME, locality, and cone data are independent inputs.
\end{proof}
...
\begin{enumerate}[label=\textup{(\arabic*$'$)},start=7]
  ...
  \item \(Y=\mathrm{Hilb}^N(\C^2)\) the Hilbert scheme of \(N\) points
        ...
        exceptional stratum is controlled by a central obstruction class
...
on the Stein atlas: the all-arity chiral \(E_2\) operation, QME solution,
locality homotopy, and cone contraction.  The remaining obstruction to
...
chartwise projective OCA data only after the chiral \(E_2\), descent,
QME, locality, and cone-contraction data are fixed.  The descent
...
Capelli centre.  The chosen Stein atlas supplies the
\v{C}ech--Dolbeault acyclicity row; the OCA quasi-isomorphism on \(Y\)
exists only after a null-homotopy of
...
neighbourhood of the brane point.  A global comparison requires the
five supplied data of Theorem~\ref{thm:contractible-stein-descent-criterion-HS};
...
\begin{itemize}
  ...
  \item (17)--(21): the character, Coulomb, Higgs, multiplicative
        ...
        own Stein atlas, OCA comparison, all-arity chiral \(E_2\) datum,
        QME solution, locality homotopy, descent null-homotopy, and cone
        contraction.
\end{itemize}
In each case the formal-Darboux theorem is the local trace-sector
closed-to-open map at a smooth brane point.  The full
derived \(E_2^{\mathrm{HT}}\)-centre identification is the global
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item to give the \emph{trace map}
        ...
        Theorem~\ref{thm:capelli-projective-anomaly} as the
