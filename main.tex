...
The formal CE/PV source-to-boundary coordinate comparison is the
formal-local instance of the finite and admissible cotangent CE/PV
construction.  The finite identity is purely algebraic:
CE cochains on \(T^*[1]\mathfrak l\) are Schouten polyvectors on
\(\mathbf S(\mathfrak l)\).  Completion at the formal disk keeps the same
formula by finite-coordinate tests.  Bar-cobar/Koszul duality is the
separate admissible enhancement governed by conilpotence and filtered
convergence.  Thus the boundary Hamiltonian is not a Hamiltonian ghost;
it is the function coordinate dual to the cotangent generator.
This paragraph uses the cotangent-coordinate grading fixed above:
the Schouten/cotangent bracket has degree \(-1\), while the reduced
Hamiltonian boundary bracket introduced later is the degree-zero
Poisson bracket.
...
\begin{proof}
	  The first item is
	  Theorem~\ref{thm:finite-cotangent-ce-pv-identity}; the second is
	  Theorem~\ref{thm:formal-disk-completed-ce-pv} together with
  Proposition~\ref{prop:cotangent-coordinate-boundary-hamiltonian}.  The
...
The local theorem is built from a single geometric setup: a topological
line carrying a stack of probes, crossed with a holomorphic-symplectic
disk to which they couple. Quantizing this setup gives the data on which
	the component theorems are verified.
...
\begin{prop}[Local mixed model]\label{prop:local-model-mixed-definition}
  ...
  theory. The constructed operation is the degree-zero Hamiltonian
...
\label{thm:universal-formal-local-ce-pv-recognition}
\label{prop:cotangent-coordinate-boundary-hamiltonian}
  Let
  \[
    \mathfrak h=\C[[z_1,z_2]]/\C\cdot1
  \]
...
mechanisms and the residual obstruction. The
  Costello--Li 2012 BCOV/HCS theorem is recorded as a negative applicability
  check: it does not transfer to the geometry studied here, and the
...
class.  Hadamard control is not anomaly cancellation: it supplies the
coefficient-kernel and locality regulator, while the QME branch still
requires the scalar and non-scalar obstruction classes to vanish in their
own local-functional complexes.
...
multiplier $A$. We work with degree-zero rational \(GL_N\)-invariant
functions in characteristic zero.  This is the algebraic invariant
calculation used by the trace theorem, not the full Lie-algebra
Chevalley--Eilenberg BRST cohomology of \(\lie{gl}_N\).  The scalar
gauge direction acts trivially and would leave scalar ghost classes in
the unreduced Lie-CE complex; those stack-level stabilizer classes are
outside the Hamiltonian trace calculation.
...
enveloping algebras: admissible Koszul duality.
The admissible calculation has staged input.  The finite, coordinate, and
bracket-admissible CE/PV statements precede any kernel assertion; a
kernel-admissible Maurer--Cartan tensor appears only when the diagonal
converges.  The enveloping/bar-cobar step is an admissible envelope
criterion.  The cochain layer is the
completed cotangent CE/PV identity of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}, valid for the formal
disk by finite coordinate tests.  The enveloping layer is a bar-cobar
criterion: it applies exactly when the chosen Tate or weighted coefficient
category makes the CE coalgebra conilpotent and the filtered inverse
limits exact.  The strict product/direct-sum full-disk coefficient pair is
therefore not discarded; it contributes the obstruction tuple whose
vanishing or replacement is the input required for the enveloping
upgrade.
