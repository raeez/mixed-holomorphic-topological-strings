...
\begin{thm}[Formal-Darboux stalk identification at $N$ Dirac branes]\label{thm:main-local}
  ...
  \bigoplus_{a+b>0}\C\,\rho_{a,b}$,
  $\rho_{a,b}=z_1^{-a-1}z_2^{-b-1}\,dz_1dz_2$ paired by
  $\langle\rho_{a,b},z_1^mz_2^n\rangle=\delta_{a,m}\delta_{b,n}$, and
  shifted-cotangent BF Lie algebra
  $\mathfrak g=\mathfrak h\ltimes P[1]$.  The Capelli scalar
  $\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}\in
  H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]$,
  $\bar A=\C[z_1,z_2]/\C\cdot 1$
  (Theorem~\ref{thm:capelli-equals-modular-curvature}), determines
  the projective central extension
  \[
  ...
  \]
  with $K$ mapping to the trace-$U(1)$ centre.  Write
  \[
    A^{\mathrm{cl}}_{\partial,N}=
    C^\bullet_{\mathrm{CE}}\bigl(\mathfrak{gl}_N,\,
         \mathrm{Sym}(\mathfrak{gl}_N\oplus\mathfrak{gl}_N
  ...
  \]
  for the finite-$N$ brane $E_1$-algebra retaining
  $\operatorname{Tr}\mathbf 1=N$, and $\mathcal B^{\mathrm{adm}}$
  for the admissible filtered nuclear Tate / pro-Matlis / Capelli-projective
  derived-centre category of Appendix~\ref{app:completion-blueprint}:
  the host coefficient category in which continuous duality is exact,
  the Casimir of $\mathfrak h$ converges along the coordinate-window
  filtration $W_n=\mathrm{span}_\C\{z_1^az_2^b:0<a+b\leq n\}$
  (Theorem~\ref{thm:wt-cotangent-lift}), the cotangent side is
  pro-Matlis, the singular kernel $K^{(2)}_\Delta$ is bracket-
  and kernel-admissible, the chiral $E_2$-operadic centrality
  homotopies act, and the trace-$U(1)$ centre is implemented as the
  projective central charge $\hbar N[\bar c]$.  Inside
  $\mathcal B^{\mathrm{adm}}$ the formal-Darboux stalk of the bulk
  holomorphic factorization algebra
  ...
  \begin{equation}\label{eq:thm-A-equivalence}
    ...
    HH^\bullet_{\mathrm{adm}}\!\bigl(
      ...
      A^{\mathrm{cl}}_{\partial,N}\bigr)
  \end{equation}
  as $E_2$-algebras, where $Z^{\mathrm{der,adm}}_{E_1}$ and
  $HH^\bullet_{\mathrm{adm}}$ are the derived $E_1$-centre and
  Hochschild cochain complex computed in $\mathcal B^{\mathrm{adm}}$.
  ...
  \[
    ...
    J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2),
  \]
  with $\theta_f$ the Hamiltonian Schouten derivation and the
  trace-$U(1)$ centre $K$ mapping to $\operatorname{Tr}\mathbf 1=N$.
  The arity-two singular product is
  \[
    \mathsf J_x(z)\mathsf J_y(w)\;\sim\;
    K^{(2)}_\Delta(z,w)\,\mathsf J_{[x,y]_{\widehat{\mathfrak g}_{\mathrm{cent}}}}(w),
    \qquad
    K^{(2)}_\Delta(z,w)=\frac{d\zeta_1\,d\zeta_2}{\zeta_1\zeta_2},
    \quad\zeta_i=z_i-w_i,
  \]
  the diagonal local-cohomology residue class on
  $\Delta\subset\C^2_z\times\C^2_w$.  The scalar-reduced shadow
  $K=0$ recovers the equivalence with $A^{\mathrm{cl,red}}_{\partial,N}
  =A^{\mathrm{cl}}_{\partial,N}/\C\cdot\operatorname{Tr}\mathbf 1$
  and $J(f)\rightsquigarrow\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}$.
\end{thm}

\begin{proof}
  Inside $\mathcal B^{\mathrm{adm}}$, the coordinate-window filtration
  $W_n$ is preserved by Schouten action
  $\theta_{z_1}=\partial_{z_2}$, $\theta_{z_2}=-\partial_{z_1}$,
  $\theta_f(W_n)\subset W_{n+\deg f-1}$, and
  Theorem~\ref{thm:wt-cotangent-lift} identifies
  $\widehat{\mathfrak g}_{\mathrm{cent}}$ as
  $\varprojlim_n(\mathfrak h/W_n^\perp\ltimes(W_n)^\vee[1])
  \oplus\C\cdot K$ with admissible duality and conilpotent CE chains.
  ...
  (Theorem~\ref{thm:universal-filtered-cobar-recognition}) supply
  the continuous Hochschild--Kostant--Rosenberg isomorphism
  \[
    HH^\bullet_{\mathrm{adm}}\!\bigl(A^{\mathrm{cl}}_{\partial,N},
                    A^{\mathrm{cl}}_{\partial,N}\bigr)
    \;\simeq\;
    PV^\bullet_{\mathrm{adm}}\!\bigl(\mathrm{Sym}\,\mathfrak h\bigr)^{GL_N}
  \]
  on the formal symplectic disk, the right-hand side being polyvector
  fields computed in $\mathcal B^{\mathrm{adm}}$.  The matrix Poisson
  identity reads as cohomology class through adjacent-swap exactness
  (Lemma~\ref{lem:adjacent-swaps}) and natural-correction primitivity
  (Lemma~\ref{lem:natural-correction-obstruction}); the dictionary
  $c_f\mapsto\theta_f$, $u_f\mapsto O_f$ is
  Theorem~\ref{thm:reduced-ce-pv-central-operation}, realising the
  Koszul resolution of $C^\bullet_{\mathrm{ch}}(A_b,A_b)$.  The
  two-variable diagonal expansion
  $K^{(2)}_\Delta=d\zeta_1d\zeta_2/(\zeta_1\zeta_2)$ on
  $\Delta\subset\C^2_z\times\C^2_w$ is the chiral $E_2$-operadic
  product supplied by the support-local principal-part current datum
  (Theorem~\ref{thm:reduced-principal-part-boundary-current});
  operadic associativity is local-factorization compatibility on
  disjoint and nested polydisks, the higher operations inherited as
  part of the chiral $E_2$ structure through the
  factorization-algebra bulk-boundary correspondence
  \cite{costello-gwilliam}.  The $E_1$-Hochschild theorem
  \cite{lurie-ha}*{Theorem~5.5.3.11} transports the $E_2$-structure
  across $HH^\bullet_{\mathrm{adm}}$ through the chiral
  Deligne--Tamarkin centrality
  $A\rightsquigarrow Z^{\mathrm{der}}_{\mathrm{ch}}(A)$.  The
  projective Capelli completion absorbs $K\mapsto\operatorname{Tr}
  \mathbf 1$ into the central charge $\hbar N[\bar c]=
  \Omega_{\mathrm{central}}|_{J(f)}$
  (Theorem~\ref{thm:capelli-equals-modular-curvature}); the
  finite-$N$ scalar anomaly is the curvature of the modular line
