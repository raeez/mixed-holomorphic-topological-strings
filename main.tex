...
\end{titlingpage}
  ...
  the formal-Darboux stalk identification of that conjecture at \(N\)
  Dirac branes over the Heisenberg-core vacuum \(b\): the equivalence
  \[
  ...
  condition on the worldline \(\R_t\subset X\).  Its two holomorphic
  transverse coordinates become Chan--Paton matrices
  \[
    \phi_1,\phi_2\in\mathfrak{gl}_N .
  \]
  Equivalently, the stack determines the substitution
  \[
    \rho_\phi\colon A\longrightarrow\End(\C^N),
    \qquad z_i\longmapsto\phi_i,
  \]
  with worldline kinetic term
  \(\int_{\R_t}\operatorname{Tr}(\phi_1\dot\phi_2)\).  The open BV
  complex adds the Koszul antifield
  \(\psi\in\mathfrak{gl}_N[1]\) and differential
  \[
    Q\phi_i=0,\qquad Q\psi=[\phi_1,\phi_2].
  \]
  This realizes the conjugation moment-map ideal as a derived zero
  fibre.  Since the entries of \([\phi_1,\phi_2]\) are not a regular
  sequence for \(N\ge2\), the ghost-zero brane moduli is the derived
  commuting-pair stack
  \[
    [\mu^{-1}_{\mathrm{der}}(0)/GL_N].
  \]
  The Hilbert scheme \(\Hilb^N(\C^2)\) appears only after adding a
  cyclic framing vector \(v\in\C^N\) with
  \(\C[\phi_1,\phi_2]\cdot v=\C^N\).  On that framed branch, the
  Moyal/Weyl deformation gives the Nekrasov--Schwarz
