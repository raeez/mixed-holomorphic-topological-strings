...
\begin{equation}\label{eq:setup-Jf-flagship}
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\end{equation}
the trace of \(f\) evaluated on the Chan--Paton pair, projected to the
stable scalar-reduced quotient. Under the formal-stalk-chart hypotheses
...
The shifted-cotangent partner $\beta$ is read at the brane as the
...
Koszul resolution of \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) at the
formal-Darboux stalk of the Dirac brane vacuum
(Theorem~\ref{thm:main-local}).  Scalar reduction kills the constant
...
\mathrm{Kosz}([\phi_1,\phi_2]))\),
\(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\),
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\) is tested below at the
brane vacua of the central families. Each example records: the brane
vacuum \(b\); the chart algebra \(A_b=\mathrm{End}(b)\); the trace
\(J(f)\); the chiral Hochschild target
\(C^\bullet_{\mathrm{ch}}(A_b,A_b)\); status. The local theorem of
\S\ref{ssec:local-theorem} is the formal-Darboux stalk; identification
beyond the stalk passes through descent, QME, anomaly, and locality
...
\(0\in\C^3\). The mixed-HT formal-Darboux stalk sees only the formal
holomorphic-symplectic stalk
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\) inside the
\(\C^3\)-target, with the third direction supplying the
twisted-M-theory equivariant scaling of
\cite{costello-twistedM}. The chart algebra is the
matrix-Chan--Paton completion at the brane vacuum; the trace
\(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) reads
through the Nekrasov vertex on the cyclic framed stable branch
of the Hilbert scheme \(\Hilb^N(\C^2)\). The
formal-Darboux stalk identification is proved at the
...
\(A_b=\C[[z_1,z_2]]\) and the trace is the formal-Darboux
\(J(f)\) of \S\ref{ssec:local-theorem}. The elliptic genus
\(\mathrm{Ell}(K3;\tau,z)\) of \cite{witten-elliptic-genus}
is the global modular consequence: the closed shadow at level 2
of the modular line on \(\overline{\mathcal A}_g\). 
The formal-Darboux stalk identification is proved pointwise on
\(K3\) (\S\ref{ssec:local-theorem}); the global identification
of the chiral Hochschild target with the elliptic genus is a
