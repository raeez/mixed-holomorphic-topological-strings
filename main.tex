...
\end{titlingpage}
  ...
  \[
    ...
    f\in\C[[z_1,z_2]] .
  \]
  A brane is a finite-dimensional module over the Hamiltonian algebra.
  The generators \(z_1,z_2\) act as the commuting Chan--Paton pair
  \((\phi_1,\phi_2)\in\mathfrak{gl}_N^{\oplus 2}\), exhibiting the brane
  as a noncommutative point of the formal symplectic disk.  A closed
  Hamiltonian \(f\) is evaluated at that point in the only stable
  \(GL_N\)-invariant, Dirac-constrained, Darboux-natural way available ---
  by trace substitution
  \(f\mapsto\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\)
  (Theorem~\ref{thm:formal-local-universal-property}).  The closed
  Hamiltonian algebra acts on itself; the brane is where its values
  are read.  The local geometry is
  \(X=\R_t\times\R_s\times\C^2_{z_1,z_2}\) with brane line
  \(L=\R_t\times\{s=z_1=z_2=0\}\); the measurement is \(J\).

  \medskip
  \noindent
  The closed BV sector is the shifted cotangent extension
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
  with \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\); the CE/PV cochain
  dictionary \(c^I\!\mapsto\!\theta^I,\,u_I\!\mapsto\!O_I\) discriminates
  the two roles a Hamiltonian plays --- \(c_f\) moves the closed sector
  through the Schouten field \(\theta_f\), \(u_f\) is measured on the
  brane as \(O_f=J(f)\) --- and the assignment \(c_f\!\mapsto\!O_f\) is
  a type error.  The cotangent module is the Grothendieck--Matlis
  principal-part bundle \(\mathcal P\), not a polynomial copy; the
  RG-stable analytic completion is the weighted-Tate completion \(B_w\)
  of Theorem~\ref{thm:wt-canonical-habitat}.  The named obstructions
  appear as cohomology classes, kernels, and matrix equations: the
  scalar anomaly \(\hbar N[\bar c]\); the radial cokernel
  \(\Omega^{\mathrm{rad}}_{a,b}\); the four-component unreduced
  cotangent-lift obstruction vector; the \(\theta_3\) finite-window
  \(H^1\); the strict all-window Bochner--Martinelli no-go.
  ...
  centre-of-mass motion, and the unreduced Poisson bracket is
  ...
  proves vanishing of \(\Omega^{\mathrm{rad}}_{a,b}\) on the
  balanced diagonal through \((6,6)\), every non-edge bidegree of
  total degree at most \(13\), and the total-degree-\(14\) bidegrees
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\), reproduced by the
  decorated-PBW Stokes script in
  \S\ref{ssec:tate-residual-resolution}.  Conditional on
  Theorem~\ref{thm:app-radial-bicomplex-acyclic} the radial bicomplex
  is interior-acyclic; unconditionally, every interior bidegree closes
  through total degree~15 by stable Procesi--Razmyslov reduction.

  \medskip\noindent
  The unreduced \(P_0\)-factorization-centre lift exists exactly when
...
A brane is a finite-dimensional module over the Hamiltonian algebra. The
generators \(z_1,z_2\) act as the commuting Chan--Paton pair
\((\phi_1,\phi_2)\in\mathfrak{gl}_N^{\oplus 2}\), exhibiting the brane as
a noncommutative point of the formal symplectic disk. A bulk Hamiltonian
\(f\in\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) is read at that
point by the unique stable \(GL_N\)-invariant Darboux-natural evaluation
map
