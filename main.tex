...
\end{titlingpage}
  ...
  the conjecture's formal-Darboux stalk at \(N\) Dirac branes over
  the Heisenberg-core vacuum \(b\): at the stalk the closed sector
  is the \(E_2\)-Hochschild cohomology of the brane \(E_1\)-algebra
  \(A^{\mathrm{cl}}_{\partial,N}\) on the worldline \(\R_t\), and the
  coordinate expression is the trace character
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  statement requires an additional theorem choosing a complex line or
  holomorphic defect, a transverse boundary condition or restriction
  functor, and the finite Laurent OPE expansion on that restricted
  object.
\end{defn}
