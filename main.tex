...
\begin{thm}[Class~A: universal local mixed-HT Deligne theorem at $N$ Dirac branes]\label{thm:main-local}
...
\begin{thm}[Class~C (conditional): mixed Holomorphic--Topological Deligne identification on the Costello--Gwilliam brane-link boundary]\label{thm:main-global-conditional}
...
of Theorem~\ref{thm:main-global-conditional}.  The detailed
elaboration of the frontier programmes governing the global
identification --- the chiral \(E_d\) Deligne conjecture at
\(d\geq 2\), the Hall--Drinfeld--Borcherds comparison spectral sequence
on compact non-toric CY\(_3\), the operator-level lift of the modular
line on \(K3\times E\), the \(W_\infty[\lambda]/E_\infty\) admissible
endpoint, and the MNOP / framing / gravitational-lift frontier inputs
--- is collected in \texttt{working\_notes/frontier.tex}.  The main
paper proves the formal-Darboux stalk; none of the frontier programmes
is a consequence (Theorem~\ref{thm:cross-target-firewall}, the
cross-target firewall).  We restate below the three load-bearing
conjectures referenced from the main paper's conditional/unconditional
ledger; their elaboration, plus the \(W_\infty[\lambda]/E_\infty\)
conjecture and the seven frontier inputs, live in the working note.
...
\begin{conj}[Class~C: chiral $E_d$ Deligne]\label{conj:chiral-Ed-deligne}
...
\cite{costello-gwilliam}*{Vol.~I}).  The \(d\geq 2\) case requires three
constructions beyond the present literature: a chiral
\(E_d\)-operad on \(\C^d\) extending the Cauchy kernel to all arities;
a chiral \(E_d\)-operadic centre construction; a chiral
\(E_d\)-Hochschild cochain complex with appropriate \(E_{d+1}\)
structure.  The formal-Darboux stalk theorem
(Theorem~\ref{thm:main-local}) supplies the arity-two \(d=2\)
formal-coordinate operation as the diagonal kernel
\(K^{(2)}_\Delta\,\mathfrak g^{\otimes 2}\to\mathfrak g\); the
all-arity extension is the content of the conjecture.  Detailed
discussion in \texttt{working\_notes/frontier.tex}.
...
\begin{conj}[Class~C: Hall--Drinfeld--Borcherds comparison spectral
  ...
  Hall--Drinfeld--Borcherds matched-conventions datum (convention
  contract Appendix~\ref{app:convention-contract},
  Theorem~\ref{thm:matched-conventions-transfer}), the five comparison
  weights organise into the \(E_2\)-page of a spectral sequence
  \[
    E_2^{p,q}(X)
    \;=\;
...
\end{conj}
Compact-target evidence (Fourier--Mukai partners, quintic DT/PT data,
\(S^3\) framing, chiral-volume finite evidence, Gopakumar--Vafa
closure) is in \texttt{frontier\_mnop\_framing\_volume.tex} and the
elaboration in \texttt{working\_notes/frontier.tex}.

\subsection{Open problem: operator-level lift of the modular line on
  \texorpdfstring{$K3\times E$}{K3 x E}}\label{ssec:open-modular-lift}

\begin{conj}[Class~C: operator-level lift of $\Phi_{10}$]\label{conj:operator-modular-lift}
  Let \(b\) be a Dirac brane vacuum on \(\partial(K3\times E)\),
  the Costello--Gwilliam brane-link boundary of a Dirac brane defect
  in \(K3\times E\).  Suppose the compact-target
  holomorphic-topological bulk theory and its boundary trace have been
  constructed.  The operator-lift problem is to lift the level-\(2\)
  projective curvature
  \(\Omega_{\mathrm{central}}\bigl|_{\overline{\mathcal A}_2}\) to an
  operator-valued cocycle
  \(\widehat\Omega_{\mathrm{central}}^{\,(2)}\) on that bulk theory,
  whose protected scalar trace
  \(\operatorname{Tr}_{\mathcal C^{\mathrm{op}}_\partial}\bigl(\widehat\Omega_{\mathrm{central}}^{\,(2)}\bigr)\)
  equals \(\Phi_{10}=\chi_{10}\) at the specified paramodular points.
  The Hall--Borcherds comparison residual of
  Conjecture~\ref{conj:hall-drinfeld-borcherds-ss} restricted to the
  \(K3\times E\) five-layer page is the obstruction class to this
  lift.
\end{conj}
By Lemma~\ref{lem:scalar-non-faithfulness} the scalar identity
\(Z^{K3\times E}_{\mathrm{BPS}}=\Phi_{10}^{-1}\) is the shadow of a
not-yet-constructed chain-level object;
\(\Delta_5=\Phi_{10}^{1/2}\) is the Borcherds square-root branch.
Cross-target reference: the Igusa cusp form volume.  Detailed
discussion in \texttt{working\_notes/frontier.tex}.
