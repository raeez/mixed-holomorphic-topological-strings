...
\end{titlingpage}
  ...
  \]
  The closed-string symplectic form pulls back to the brane phase
  space \(\mathcal P_N=\mathfrak{gl}_N^{\oplus2}\) as
  \[
  ...
  \]
  so Dirac reduction imposes
  \[
    [\phi_1,\phi_2]=0,
  \]
  factoring \(\rho_\phi\) through the commuting variety.  A
  Hamiltonian \(f\) on the formal symplectic normal polydisk
  ...
  \[
    J(f)=\overline{\operatorname{Tr}\,\rho_\phi(f)}
        =\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\in
        A_\partial=\widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h),
        \qquad
        \mathfrak h=A/\C\cdot1.
  \]
  This is the brane microscope.

  \medskip\noindent
  The brane represents the commutative coordinate ring \(A\); the
  Hamiltonian Lie algebra \(\mathfrak h\) does not act on the
  Chan--Paton space \(\C^N\) by commuting operators, but on the
  representation moduli \(\Rep_N(A)/\!/GL_N\) by Hamiltonian vector
  fields.  A Hamiltonian \(f\) carries two avatars: the
  Hamiltonian-symmetry coordinate \(c_f\) acts as the Schouten field
  \(\theta_f\) on functions on the moduli, while the
  shifted-cotangent insertion \(u_f\) is measured on the brane as
  \(O_f=J(f)\); the collapse \(c_f\mapsto O_f\) is a type error.
  The closed BV sector is the shifted-cotangent Lie algebra
  \[
    \mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1],
  \]
  and continuous duality on the formal-disk topology identifies
  \(\mathfrak h^\vee_{\mathrm{cont}}\) with the Grothendieck--Matlis
  principal-part module
  \[
    \mathcal P=\bigoplus_{a+b>0}\C\rho_{a,b},\qquad
    \rho_{a,b}=z_1^{-a-1}z_2^{-b-1}\,dz_1dz_2,
  \]
  paired with Taylor Hamiltonians by residue.  Residues are defect
  momenta conjugate to sampling, not polynomial PBW shadows.

  \medskip\noindent
  Stable polynomial \(GL\)-invariance, Dirac descent, scalar
  reduction, Darboux equivariance under the formal symplectomorphism
  group, primitive block-sum additivity under brane stacking, and
  linear normalization on \(z_1,z_2\) make the trace character
  \(J\) the unique linear measurement
  \(\widetilde J\colon\mathfrak h\to A_\partial\)
  (Theorem~\ref{thm:Jf-coordinate-uniqueness}).  Procesi--Razmyslov
  on cyclic words, adjacent-swap collapse on the moment-map
  zero-fibre, and Loday--Quillen--Tsygan single-trace primitivity
  fix the answer monomial-by-monomial
  ...
  (Theorem~\ref{thm:formal-local-universal-property}).  An odd matrix
  antifield \(\psi\in\mathfrak{gl}_N[1]\) with BV differential
  \(Q\phi_i=0,\ Q\psi=[\phi_1,\phi_2]\) realizes the moment-map ideal
  as the Koszul derived zero fibre, and the matrix entries of
  \([\phi_1,\phi_2]\) are not a regular sequence for \(N\ge2\): the
  open sector is the derived zero fibre, not an ordinary smooth
  quotient.

  \medskip\noindent
  The local-string-theoretic content of the model is exhausted by
  three factorization objects.  The bulk
  \(E_2\)-algebra
  \[
    \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}(B)=
    \mathcal F^{\mathrm{cl}}_{\mathrm{Ham}}(D^2\times B)
  \]
  is the value of the closed mixed-HT factorization algebra
  \(\mathcal F^{\mathrm{cl}}_{\mathrm{Ham}}\in
  \mathrm{Fact}_{\mathrm{HT}}(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}})\)
  on a topological disk times a holomorphic polydisk
  (Theorem~\ref{thm:mixed-HT-factorization-theorem},
  Corollary~\ref{cor:E2-bulk-algebra}).  The brane
  \(E_1\)-algebra
  \[
    A^{\mathrm{cl}}_{\partial,N}=
    C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N,\mathrm{Kosz}_N)
  \]
  is the BRST algebra of the Dirac matrix model on a small interval
  of the worldline (Definition~\ref{def:E1-brane-algebra}).  The
  closed-to-open derived-centre map
  \[
    \Phi_N\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}
    \longrightarrow Z^{\mathrm{der}}_{E_1}\!\bigl(A^{\mathrm{cl}}_{\partial,N}\bigr)
  \]
  has reduced-coordinate shadow
  ...
  \medskip\noindent
  The centre of mass remembers the constant killed by Hamiltonian
  reduction.  The unreduced linear bracket
  \(\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N\) and the
  Weyl/Moyal commutator
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\) detect the
  same Lie 2-cocycle
  \(\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\) on
  four faces -- algebraic central extension, Lie cocycle,
  trace-algebraic boundary extension, and Weyl/Moyal commutator
  (Theorem~\ref{thm:app-scalar-anomaly-transgression}); the strict
  scalar-reduced Hamiltonian action is replaced by a projective
  action on the central extension
  \(0\to\C\to\mathfrak h_{\hbar,N}\to\bar A\to0\).

  \medskip\noindent
  Quantization, globalization, and chain-level upgrades are
  transport problems for the local matrix-microscope Maurer--Cartan
  datum into a filtered \(L_\infty\) target
  \(\mathfrak K_{\mathcal T}\) controlling the proposed upgrade,
  with master equation
  \(F_{\mathcal T}(\Theta_{\mathrm{loc}}+\tau)=0\)
  (Theorem~\ref{thm:obstruction-twist-dichotomy}).  Vanishing
  curvature gives strict coherence; null-homotopy gives counterterm
  coherence; central closed curvature gives projective or
  ...
  governing all-bidegree Weyl/trace compatibility
  (Theorem~\ref{thm:componentwise-quantum-coefficient-comparison};
  Theorem~\ref{thm:app-radial-bidegree-rational-reduction}); the
  unreduced factorization-centre vector
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\) governing the
  chain-level lift of \(\Phi_N\) to a strict \(P_0\)-centre map
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}); the
  reduced Costello QME class
  \([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\) of which
  the cubic row \(\theta_3\) is the prototype counterterm twist
  (\(H^1_{\mathrm{bare}}=\C\!\cdot\!E_{\theta_3}\), killed by the
  unique counterterm \(dC=-E,\,c=1\),
  Theorem~\ref{thm:app-theta-three-finite-window-H1}); and the
  strict Bochner--Martinelli current-transfer class
  \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\), non-absorptive in every
  finite Matlis window and selecting the pro-Matlis tower as the
  coherent target replacement
  (Theorems~\ref{thm:bmk-finite-window-no-go},
  \ref{thm:promatlis-universal-property}).

  \medskip\noindent
  The local theorem is the formal-Darboux brane-probe restriction of
  the derived chiral centre \(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\) of
  the companion programme: Vol~I builds
  \(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\) by ordered bar-cobar duality
  and modular Maurer--Cartan calculus; Vol~II identifies the pair
  \((Z^{\mathrm{der}}_{\mathrm{ch}}(A),A)\) as the bulk-boundary
  datum of a 3d holomorphic--topological boundary-bulk system;
  Vol~III supplies the Calabi--Yau categorical source through the
  two-stage functor
  \(\Phi_d=\mathrm{Sp}_{\Sigma_{d-1},C}\circ\Phi^{\mathrm{FA}}_d\),
  \(A=\Phi_d(\mathcal C)\); the present paper computes the last
  arrow.  Cross-volume use passes through the matched-conventions
  obstruction \(\operatorname{Ob}_{\mathrm{UKD}}\) of
...
\]
(Theorem~\ref{thm:reduced-ce-pv-central-operation}); the assignment
\(c_f\mapsto O_f\) collapses an operation into its measurement and
is a type error.  Continuous duality on the formal-disk topology
identifies the cotangent module with the Grothendieck--Matlis
principal-part module
\[
...
\begin{proof}
  Existence of \(\Phi_N\) at the derived-\(E_1\)-centre level follows
  from \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}\) being an
  \(E_2\)-algebra (Corollary~\ref{cor:E2-bulk-algebra}) acting on the
  \(E_1\)-algebra \(A^{\mathrm{cl}}_{\partial,N}\) through the universal
  bulk-to-boundary map of the Lurie--Francis classification of pairs
  \((E_2,E_1)\) with central action.  The reduced-coordinate shadow is
  the matrix-microscope shadow of the BV pairing on
  \(\mathfrak g\)-cochains read on monomial generators
  \(c^{a,b},u_{a,b}\) of
  Corollary~\ref{cor:ce-pv-formal-disk}; uniqueness of the boundary
  values \(u_f\mapsto J(f)\) is
