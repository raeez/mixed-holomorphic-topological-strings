...
\end{titlingpage}
  ...
  At a holomorphic-symplectic brane point in a Calabi--Yau target,
  given an admissible protected Hamiltonian restriction of the
  ambient closed-open theory, the present paper studies the
  resulting formal local model: the holomorphic-topological BF
  theory on \(X=\R^2_{t,s}\times\C^2_{z_1,z_2}\), topological in
  \((t,s)\) and holomorphic in \((z_1,z_2)\); its open sector is a
  stack of \(N\) Dirac branes on the worldline \(\R_t\); its closed
  sector is the shifted-cotangent local Lie algebra
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
  on \(\widehat{\C^2}_0\), \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1\);
  and the brane probe is the trace functional
  \(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).  That an
  admissible protected restriction exists for any compact
  Calabi--Yau threefold target is not proved here; it is an external
  input identified across volumes by the matched-conventions
  firewall (Theorem~\ref{thm:matched-conventions-transfer}).
