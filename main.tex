...
\end{titlingpage}
  ...
  \medskip\noindent
  \emph{The trace map \(J\).}  A Hamiltonian
...
trace observables generating the open factorization algebra on the
brane line.  The closed Hamiltonian
\(f\in\mathfrak h\) is read on this brane sector as the
\(\mathrm{GL}_N\)-invariant trace functional
...
map \(J\) below.  The closed BV theory has two coordinate types, and
the chart \(A_b\) reads
...
the open factorization algebra on the brane line.  The CE/PV
