...
\end{titlingpage}
  ...
  holomorphic-symplectic brane point, a closed sector conjectured
  to coincide with the chiral Hochschild cohomology of the boundary
  algebra: the Mixed Holomorphic--Topological Deligne conjecture, the
  two-complex-dimensional extension of the chiral Deligne theorem
  proved at \(d=1\) for chiral algebras factorising over curves.
  This manuscript proves the conjecture's formal-Darboux stalk at \(N\)
  Dirac branes; the coordinate expression is the trace character
  ...
  \medskip\noindent
  \emph{Closed-open coupling.}  The closed-string sector classical
  observables factorization algebra on $\C^2$ admits two equivalent
  presentations: a Costello--Gwilliam holomorphic factorization algebra
  on \(\C^2\) valued in \(E_2\)-algebras
  \[
  ...
  of the closed-string sector classical observables by the
  ...
  Appendix~\ref{app:completion-blueprint}.  The native
...
the formal-Darboux stalk
(Definition~\ref{def:formal-darboux-stalk-chart}) at the Dirac brane
stack on
...
its formal-Darboux stalk at the brane vacuum.  Three distinct
...
manuscript proves its formal-Darboux stalk at one Dirac brane
vacuum.  Where \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) appears below,
...
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) at the chosen Dirac brane
...
\begin{proof}
  ...
  \(HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) as
...
cohomology at the brane vacuum.  The data are the formal Darboux
...
\begin{lem}[Dirac reduction of the brane phase space]\label{lem:dirac-probe-reduction}
  Let
...
derived \(E_1\)-centre
\(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\simeq
HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) (Deligne's
\(E_1\)-Hochschild theorem), which the Mixed
...
The trace map
\(J\colon\mathfrak h\to A^{\mathrm{cl}}_{\partial,N}\) is the unique
linear \(\mathrm{GL}_N\)-equivariant Darboux-natural primitive
single-trace map satisfying scalar reduction, Dirac descent, and
\(J(z_i)=\overline{\operatorname{Tr}\,\phi_i}\); its image generates
the open-string sector classical observables factorization algebra
on the brane line.  The CE/PV
dictionary \(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\) on
\(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
identifies the closed CE coordinates with Schouten polyvectors and
boundary observables; this dictionary is the Koszul resolution of the
brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\), proved in the section
below.  The Mixed Holomorphic--Topological Deligne conjecture
identifies this at the formal-Darboux stalk with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b_p\)).
...
The CE/PV dictionary
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\) is the Koszul resolution
of the brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) at the chart
\(A_{b_p}\); the Mixed Holomorphic--Topological Deligne conjecture
identifies this at the formal-Darboux stalk with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b_p\)).  Two
type distinctions on three disjoint alphabets fix the dictionary.
...
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) at the formal-Darboux
...
The local theorem is the formal-Darboux stalk identification of
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\) (the bulk chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b\)) with
\(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\simeq
HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) (Costello--Gwilliam
bulk--boundary correspondence + Deligne's \(E_1\)-Hochschild theorem;
both sides \(E_2\)-algebras), at the chosen Dirac brane vacuum \(b\)
with chart algebra \(A_b=\mathrm{End}(b)\).  A global compact-target
identification requires, in addition, a descent datum across charts, a
quantum master equation (QME) closure against an anomaly polynomial,
and a locality datum on the Kato--Nakayama log boundary; the local
theorem produces a candidate global identification when all four are
supplied.  The Dirac brane stack selects the open BV complex as the derived
zero-fibre of the conjugation moment map, with stable trace observables
\(O_{k,l}\); the trace map
\(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\) is the unique polynomial
\(GL_N\)-equivariant Darboux-natural primitive single-trace map
satisfying scalar reduction, Dirac descent, and \(J(z_i)=\mathrm{Tr}\,\phi_i\)
...
quantization map.  Stronger statements are exact obstruction
...
theorem of \S\ref{ssec:local-theorem}.
...
chiral Hochschild target is the free polynomial reading
\(C^\bullet_{\mathrm{ch}}(\C[[z_1,z_2]],\C[[z_1,z_2]])\simeq
\mathrm{PV}(\C[[z_1,z_2]])\) of
\cite{beilinson-drinfeld-chiral}*{\S\S 3, 19} on the formal
...
\([\hbar N\bar c]_{\widehat{\mathfrak g}_k}=[\hbar k]\).  The
chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(V_k(\mathfrak g_0),V_k(\mathfrak g_0))
\simeq Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\mathfrak g_0))\) along
the chiral Deligne ladder
\cite{beilinson-drinfeld-chiral}.  Identification of the
...
fields.  The chart algebra is the \(\beta\gamma\) vertex algebra
...
\([\hbar N\bar c]_{\beta\gamma}=[\hbar\cdot 2]=[\hbar c]\).  The
chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(V^{\beta\gamma},V^{\beta\gamma})\), the
chiral derived centre of the free \(\beta\gamma\) factorization
algebra of \cite{costello-gwilliam}*{Vol.~II~\S 5.4}.  Proved at
...
The bulk \(C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c)\)
is the chiral derived centre of the Virasoro algebra at level
\(c\); under matched-conventions transfer to
...
\(k\).  The chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(W_N,W_N)\).
...
\begin{thm}[{Conditional $\mathcal W_\infty[\lambda]/E_\infty$ admissible endpoint}]\label{thm:w-infty-e-infty-admissible}
  ...
  evaluated on the admissible locus), with chart the
  CE/PV dictionary
  \[
...
\]
of the bulk chiral $E_d$ factorization algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ on $\C^d$ with the chiral
derived $E_d$-centre of the boundary open category.  At one Dirac
brane vacuum $b$ on the Kato--Nakayama log boundary
$\partial X^{\mathrm{KN}}$, the formal-Darboux stalk
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b$ identifies with
$Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})$ by
Costello--Gwilliam bulk--boundary correspondence and Deligne's
$E_1$-Hochschild theorem, both as $E_2$-algebras.  Three open problems are
...
firewall (\S\ref{ssec:cross-volume-firewall};
Theorem~\ref{thm:cross-volume-firewall};
...
$E_1$-Hochschild $HH^\bullet(A^{\mathrm{cl}}_{\partial,N})$ at one
...
of $b_p$), generalising the Beilinson--Drinfeld chiral Deligne theorem
