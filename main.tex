...
of Appendix~\ref{app:completion-blueprint}, the open--closed comparison
$\mathrm{OCA}_N|_b$ of Definition~\ref{defn:oca-comparison} sends the
$E_1^{\mathrm{top}}$-brane algebra
$A^{\mathrm{cl}}_{\partial,N}$ at $N$ Dirac branes on the worldline
$\R_t$ (with Koszul differential $Q\psi=[\phi_1,\phi_2]$) to the
formal-Darboux stalk
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b$ of the bulk mixed-HT
factorisation algebra at $b\in\widehat{\C^2}$, and the map is an
equivalence; on Chevalley--Eilenberg generators it is given by
\eqref{eq:ce-pv-dictionary-strong}.  There are no residual obstructions
at the formal-Darboux stalk; the global obstructions to extending
$\mathrm{OCA}_N$ across the brane-link boundary are classified by
...
\begin{thm}[Mixed Holomorphic--Topological Deligne identification on the Costello--Gwilliam brane-link boundary, conditional]\label{thm:main-global-conditional}

In the admissible category $\mathcal B^{\mathrm{adm}}$ on the
brane-link boundary $\partial X$, the open--closed comparison
$\mathrm{OCA}_N$ of Definition~\ref{defn:oca-comparison} sends the
bulk mixed-HT factorization algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ of
Definition~\ref{def:closed-mixed-HT-factorization} to the derived
$E_2^{\mathrm{HT}}$-centre
$Z^{\mathrm{der}}_{E_2^{\mathrm{HT}}}(\mathcal C^{\mathrm{op}}_\partial)$
of Definition~\ref{defn:six-carriers}~(C3); the global equivalence
holds conditional on the three-component obstruction
$\operatorname{Ob}^{\mathrm{global}}$ stated below
(typed five-component refinement \eqref{eq:obs-five-component}).  The
formal-Darboux stalk fibre is
Theorem~\ref{thm:oca-formal-darboux-fibre}$=$Theorem~\ref{thm:main-local},
and the typed five-component upgrade is
Theorem~\ref{thm:typed-global-mixed-ht-deligne}.
  ...
  $\operatorname{Ob}^V_{\mathrm{mod}}$ to lifting
  $\Omega_{\mathrm{central}}$ to an operator-valued cocycle is
  required; its scalar shadow is the
