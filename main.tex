...
chiral Hochschild cohomology \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) at
the chart \(A_b=\mathrm{End}(b)\) of the chosen Dirac brane vacuum
\(b\) (Theorem~\ref{thm:formal-local-universal-property}).
...
A \emph{brane vacuum} on the holomorphic--symplectic factor of
\(X=\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) is a closed point
\(b\in\C^2\).  The \emph{formal-Darboux stalk chart at \(b\)} is the
quadruple
\[
  \bigl(\widehat{\C^2}_b,\,\omega_b,\,A_b,\,(z_1,z_2)\bigr),
\]
where: \(\widehat{\C^2}_b=\operatorname{Spf}\widehat{\mathcal O}_{\C^2,b}\) is the
formal completion at \(b\); \(\omega_b=dz_1\wedge dz_2\) is the
holomorphic symplectic form in formal Darboux normal form; the
\emph{chart algebra} is
\(A_b=\widehat{\mathcal O}_{\C^2,b}\cong\C[[z_1,z_2]]=\End(b)\); and
\((z_1,z_2)\) is a formal Darboux coordinate system in which
\(\omega_b\) is standard, supplied by the formal holomorphic Darboux
theorem.  The \emph{Dirac brane formal-Darboux stalk chart at \(N\)
branes over \(b\)} is this datum together with the Chan--Paton
substitution
\[
  \rho_\phi\colon A_b\to M_N(\C),\qquad
  z_i\mapsto\phi_i\in\mathfrak{gl}_N,\qquad
  [\phi_1,\phi_2]=0,
\]
of the calculation below.  For a sheaf, factorization algebra, or
chiral algebra \(\mathcal X\) on
\(\R^2_t\times\C^2_{z_1,z_2}\), the \emph{formal-Darboux stalk of
\(\mathcal X\) at \(b\)} is the restriction
\(\mathcal X|_{\R^2_t\times\widehat{\C^2}_b}\) in these coordinates.
\end{defn}

A Hamiltonian \(f\in A_b\) on the formal symplectic normal polydisk is
read on the brane stack as a single \(GL_N\)-invariant number, the
trace character
...
In the chart \(A_b=\mathrm{End}(b)\) of the chosen Dirac brane vacuum
\(b\) on the brane line \(L=\R_t\times\{z_1=z_2=0\}\) (introduced in
\S\ref{sec:derived-commuting-variety}, with the primitive open object
the boundary category
\(\mathcal{C}^{\mathrm{op}}_\partial=
\Phi^{\mathrm{FA}}_d(\mathcal A)|_{\partial X^{\mathrm{KN}}}\) on the
Kato--Nakayama log boundary \(\partial X^{\mathrm{KN}}\) of
\((X,D,\tau)\)), the formal-Darboux stalk
\(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) is computed through the trace
map \(J\) below; the chiral algebra structure on \(A_b\) is the one
inherited as the formal-Darboux stalk of the closed-string sector
classical observables factorization algebra
(\S\ref{ssec:binding-convention-layer}).  The closed BV theory has
two coordinate types and the chart \(A_b\) carries only one of them.
...
boundary observables on the chart \(A_b\).
Lemma~\ref{lem:formal-stalk-trace} gives the trace map
\[
  J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)
\]
on the second alphabet, equivalently
\(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) after stable
scalar reduction.  \(J\) is the unique \(\C\)-linear, \(GL_N\)-equivariant,
...
the chart \(A_b\).  Closed Hamiltonians act through \(c_f\); \(J\)
...
\(C^\bullet_{\mathrm{ch}}(A_b,A_b)\), proved in the section below.
...
\(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) at the chart \(A_b\); the cotangent
CE/PV reduced central-operation spine of
\S\ref{ssec:coordinate-free-ce-pv-synthesis} is the formal-Darboux stalk
expression of that resolution.
...
\(b=\) the Virasoro vacuum module \(V_{\mathrm{Vir},c}\) at central
charge \(c\). The chart algebra is
\(A_b=\mathrm{Vir}_c\); the trace
\(J(f)=\overline{\operatorname{Tr}f(L_n)}\) reads the polynomial
Hamiltonian on the Virasoro generators. The chiral Hochschild
target is
\(C^\bullet_{\mathrm{ch}}(\mathrm{Vir}_c,\mathrm{Vir}_c)\). The
algebraic holographic reading identifies the local mixed-HT
sector at the Virasoro chart with the Brown--Henneaux boundary
algebra; this identification is of HT sectors only, not the
construction of a dynamical-metric path integral on
\(\mathrm{AdS}_3\). Identification of the formal
Darboux stalk: proved at the Heisenberg core. The full Virasoro
reading is the standard Frenkel--Ben-Zvi datum
\cite{frenkel-ben-zvi}*{\S\S 2.5, 3.4}; the AdS\(_3\)/algebraic-HT
holographic identification is of HT sectors, not a 3d-gravity
construction.
