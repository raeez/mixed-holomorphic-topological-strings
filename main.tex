...
\end{titlingpage}
  ...
  Topological string theory in the B-model is governed at all loops
  by the Kodaira--Spencer master action of
  Bershadsky--Cecotti--Ooguri--Vafa \cite{bcov} and admits a
  renormalized BV foundation due to Costello and Costello--Li
  ...
  target, this string field theory reduces to a single universal
  local model: the holomorphic-topological BF theory on
  \(X=\R^2_{t,s}\times\C^2_{z_1,z_2}\), topological in \((t,s)\) and
  holomorphic in \((z_1,z_2)\); its open sector is a stack of
  \(N\) Dirac branes on the worldline \(\R_t\); its closed sector is
  the shifted-cotangent local Lie algebra
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
  on \(\widehat{\C^2}_0\), \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1\); and the
  universal brane probe is a single trace functional, the matrix
  microscope.

  \medskip\noindent
  \emph{The classical BV theory.}  The BV fields are an
  $\mathfrak h$-connection $\mathcal A$ and a cotangent partner
  $\mathcal B$,
  \[
    \mathcal A\in
  ...
  interaction at scale \(L\) is \(I[L]=W(P_{\varepsilon,L},I)\), with
  $S[L]=S_{\mathrm{free}}+I[L]$ the effective action, satisfying the
  QME
  \[
    ...
    \{S[L],-\}=Q+\{I[L],-\}_L.
  \]
  Costello deformation cohomology of $I[L]$ at scale $L$ is the
  obstruction theory of the matrix microscope.
  ...
  \medskip\noindent
  \emph{The matrix microscope.}  A Hamiltonian \(f\in\mathfrak h\) is
  measured on the brane as the gauge-invariant trace character
  \[
    J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}.
  \]
  \emph{Theorem} (Theorem~\ref{thm:Jf-coordinate-uniqueness}): \(J\)
  is the unique linear \(GL_N\)-equivariant primitive single-trace
  measurement
  ...
  value at a representation lands in
  \(\operatorname{HC}_0(\End V)\cong\C\).  The forcing is
  ...
  point, the matrix microscope is the terminal datum
  ...
  \]
  the precosheaf-with-Weiss-descent native to the BV / heat-kernel
  analytic framework (Costello--Gwilliam Vol.~II,
  ...
  worldline (Definition~\ref{def:E1-brane-algebra}).  The closed-to-open map
  \[
    \Phi_N\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}
    \longrightarrow Z^{\mathrm{der}}_{E_1}\!\bigl(A^{\mathrm{cl}}_{\partial,N}\bigr)
  \]
  is Koszul duality between the bulk holomorphic-$E_2$ object and the
  brane $E_1$-algebra in the Costello sense.  A Hamiltonian
  $f\in\mathfrak h$ has two CE coordinate avatars on
  $\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]$:
  the generator-dual $c_f$ acts on observables as the Hamiltonian
  Schouten field $\theta_f$, the cotangent-dual $u_f$ is measured on
  the brane as $O_f=J(f)$; the reduced-coordinate shadow of $\Phi_N$ is
  ...
  \medskip\noindent
  \emph{Obstruction calculus and formality.}  Globalization,
  quantization, all-bidegree Weyl/Moyal compatibility, and
  chain-level upgrades are filtered \(L_\infty\) transports of the
  local datum \(\Theta_{\mathrm{loc}}\) into a target deformation
  complex \(\mathfrak K_{\mathcal T}\), with master equation
  \(F_{\mathcal T}(\Theta_{\mathrm{loc}}+\tau)=0\)
  (Theorem~\ref{thm:obstruction-twist-dichotomy}).  The four upgrade
  directions investigated here---all-bidegree Weyl/radial normal
  form, chain-level $P_0$-factorization-centre lift, reduced
  non-scalar Costello QME, and strict all-window Bochner--Martinelli
  current transfer---each carry a named curvature component in the
  filtered $L_\infty$ algebra
  $\mathfrak K_{\mathrm{loc}}$ controlling deformations of $\Phi_N$:
  \begin{itemize}
    \item the radial Weyl cokernel
      $\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}$,
      where $B_{a,b}$ is the decorated PBW Stokes operator on the
      bidegree-$(a,b)$ row, controlling all-bidegree Weyl/trace
      compatibility
      (Theorem~\ref{thm:componentwise-quantum-coefficient-comparison};
      Theorem~\ref{thm:app-radial-bidegree-rational-reduction});
    \item the factorization-centre vector
      $\operatorname{Ob}_{\mathrm{cent/QME/desc}}$ controlling the
      chain-level $P_0$-centre lift of $\Phi_N$
      (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction});
    \item the reduced Costello QME class
      ...
      with order-three CE-source row \(\theta_3\) exhibiting
    ...
    \item the Bochner--Martinelli class
      $\operatorname{Ob}^\Pi_{\mathrm{BM}}$, with non-vanishing
      per-window representative in every finite Matlis window
      $\mathcal P_{\le N}$
  ...
  \medskip\noindent
  \emph{Wider context.}  The local matrix microscope is the
  holomorphic-2-dimensional analog of the Heisenberg chiral algebra
  of Beilinson--Drinfeld with $\mathcal P$ in the role of the
  ...
  family \cite{costello-twistedM}; and, on its cyclic framed
  stable branch, the universal local probe of $\Hilb^N(\C^2)$ in its
  noncommutative Moyal deformation.  Global compact-target lifts---BCOV
  holomorphic anomaly, Gopakumar--Vafa, Donaldson--Thomas---proceed by
  transporting the local Maurer--Cartan element
  $\Theta_{\mathrm{loc}}$ into the corresponding compact-target
  deformation complex; the obstruction calculus above is the
  formal-Darboux input.

  \medskip\noindent
  The local theorem is the formal-Darboux brane-probe restriction of
...
The matrix microscope reads a Hamiltonian \(f\in\C[[z_1,z_2]]\) on the
formal symplectic normal polydisk as a single \(GL_N\)-invariant
number on the brane stack: its trace character
...
is a finite Chan--Paton representation
\[
  \rho_\phi\colon A=\C[[z_1,z_2]]\longrightarrow\End(\C^N),
  \qquad z_i\longmapsto\phi_i\in\mathfrak{gl}_N.
\]
The holomorphic symplectic form \(dz_1\wedge dz_2\) pulls back along
this substitution and traces to
\[
...
\(f(\phi_1,\phi_2)\in\End(V)\) is defined on arbitrary commuting
matrix pairs and gives the polynomial measurement
...
records its image at the universal Artin base.  The
finite-matrix-polynomial layer is the special fibre of the formal
layer at \(R=\C\); below we work in whichever layer is in play.
...
\[
  \mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1].
\]
A Hamiltonian \(f\in\mathfrak h\) carries two Chevalley--Eilenberg
coordinates on \(\mathfrak g\): the generator-dual \(c_f\) acting as
the Hamiltonian Schouten vector field \(\theta_f\) on the
...
\begin{ex}[A single Matlis residue]\label{exa:single-matlis-residue}
  ...
  mode here and that polynomial shadow are separated by the
...
infinite-dimensional.  A finite shadow exists exactly when the closed
...
\begin{rmk}[Two avatars of a Hamiltonian: operation and measurement]\label{rmk:intro-symbol-separation}
  ...
  coordinate \(u_f\); the Hamiltonian CE coordinate \(c_f\) acts on
  observables as the Schouten vector field \(\theta^I\).  Their
  bracket compatibility \([\theta^I,O_J]_S=\delta^I_J\) is the
  Schouten--observable pairing, and the
  \(P_0\)-centrality homotopies in
  Problem~\ref{prob:formal-factorization-center} are organized by it.
  Lemma~\ref{lem:type-discipline} records the dictionary as a single
  rule.
...
observable is the image of the second.  This is the type-discipline
lemma; every later use of the symbols \(c_f,u_f,\theta_f,O_f\) refers
back to it.
...
\begin{lem}[Type discipline: a Hamiltonian has two avatars]\label{lem:type-discipline}
  ...
  \begin{itemize}
    ...
    \item As closed cotangent insertion:
      ...
      \(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).
  \end{itemize}
  Closed Hamiltonians act through their \(c\)-avatar; brane
  measurements record their \(u\)-avatar; the CE/PV ladder
...
\subsection{The brane centre of mass remembers what Hamiltonian reduction forgets}\label{subsec:scalar-anomaly}
...
\begin{lem}[Hamiltonian vector field on the representation stack]\label{lem:hamiltonian-vector-field-on-Rep}
  For $f\in\mathfrak h=A/\C\cdot 1$ define a derivation
  $\theta_f\in\mathrm{Der}\bigl(\mathcal O(\Rep_N^\wedge(A))\bigr)$ by
  \[
    \theta_f\phi_1
    ...
    \qquad
    \theta_f\psi
     =-\bigl[\partial_{z_2}f|_{(\phi_1,\phi_2)},\phi_2\bigr]
       +\bigl[\partial_{z_1}f|_{(\phi_1,\phi_2)},\phi_1\bigr] .
  \]
  Then $\theta_f$ commutes with the Koszul differential $Q\psi=[\phi_1,\phi_2]$,
  preserves the moment-map ideal, descends to a derivation on the
  derived commuting-pair stack
...
\begin{proof}
  ...
  $\theta_f=\{J_f,-\}$ on the matrix coordinate algebra and gives the
  displayed formulas on $\phi_1,\phi_2$.  The correction on $\psi$ is
  the unique odd derivation extending $\theta_f$ for which
  $[\theta_f,Q]=0$: applying the Leibniz rule to
  $Q\psi=[\phi_1,\phi_2]$ produces
  $[\theta_f\phi_1,\phi_2]+[\phi_1,\theta_f\phi_2]=Q\theta_f\psi$,
  which is the formula above.  The moment-map ideal is preserved
  because $\{f(\phi_1,\phi_2),\mu_\epsilon\}=0$ for every $GL_N$-invariant
...
\begin{thm}[Closed-to-open derived-centre map and CE/PV reduction]\label{thm:closed-to-open-derived-centre-map}
  ...
  \[
    ...
    Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})
     \simeq HH^\bullet(A^{\mathrm{cl}}_{\partial,N},
                       A^{\mathrm{cl}}_{\partial,N}),
  \]
  whose reduced-coordinate shadow on Hamiltonian generators is the
  CE/PV dictionary
  \[
    c_f\longmapsto\theta_f,\qquad
    u_f\longmapsto O_f=J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}
    \qquad(f\in\mathfrak h),
  \]
  matching the cochain identity of
  Theorem~\ref{thm:reduced-ce-pv-central-operation} on
  \(c^{a,b},u_{a,b}\).  The full chain-level lift to the strict
  ...
  The hierarchy is
  \[
    \underbrace{\bigl(c_f\mapsto\theta_f,\,u_f\mapsto O_f\bigr)}_{\text{reduced CE/PV coordinate map}}
    \;\subset\;
    \underbrace{\Phi_N\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}
     \to Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})}_{\text{derived }E_1\text{-centre map}}
    \;\subset\;
    \underbrace{\Phi^{\mathrm{cl}}\colon\mathrm{Obs}^{\mathrm{cl}}_{\mathrm{closed},H}\to Z^{P_0}_{\mathrm{fact}}(\mathrm{Obs}^{\mathrm{cl}}_{\mathrm{open},N})}_{\text{full chain-level }P_0\text{-centre map}}
  \]
  with the rightmost inclusion conditional on
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}=0\) (with compatible
  null-homotopies).
\end{thm}

\begin{proof}
  The chain map \(\Phi_N\) is constructed explicitly on generators:
  for $f\in\mathfrak h$, the Hamiltonian-dual coordinate $c_f$ is sent
  to the derivation $\theta_f\in\mathrm{Der}(A^{\mathrm{cl}}_{\partial,N})$
  of Lemma~\ref{lem:hamiltonian-vector-field-on-Rep}, and the
  cotangent-dual coordinate $u_f$ is sent to multiplication by the
  trace character $J_f=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}$.
  Compatibility with the CE differential matches the bracket identity
  $\theta_f(J_g)=J_{\{f,g\}_{\bar A}}$ of
  Lemma~\ref{lem:hamiltonian-vector-field-on-Rep}: the CE relations
  $d_{\mathrm{CE}}c^K=-\frac12 C^K_{IJ}c^Ic^J$,
  $d_{\mathrm{CE}}u_K=C^L_{KJ}u_L c^J$ go to
  $\bigl[\theta_f,\theta_g\bigr]=\theta_{\{f,g\}_{\bar A}}$ and
  $\theta_g\cdot J_f=J_{\{g,f\}_{\bar A}}$ on the right by
  Proposition~\ref{prop:first-order-bracket}, completing the chain map
  on generators.  Continuous extension across the
  graded-commutative coordinate algebra is the statement of
  Theorem~\ref{thm:reduced-ce-pv-central-operation}.  At the
  $(E_2,E_1)$-pair level, this construction realises the universal
  bulk-to-boundary map of the Lurie--Francis classification on
  $(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0},A^{\mathrm{cl}}_{\partial,N})$;
  uniqueness of the boundary values \(u_f\mapsto J(f)\) is
  Theorem~\ref{thm:Jf-coordinate-uniqueness}.  The chain-level
  \(P_0\)-factorization-centre upgrade requires the obstruction-vanishing
  data of Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction};
  this is a strictly stronger statement than the derived-\(E_1\)-centre
  map, and the difference is the curvature class
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\) in the master
  deformation complex.
\end{proof}

\begin{rmk}[The native object is not a curve VOA]\label{rmk:not-a-curve-voa}
  The native bulk object is the holomorphic factorization algebra
  \(B\mapsto\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}(B)\) on
  \(\C^2_{\mathrm{hol}}\) valued in \(E_2\)-algebras, not a curve
  vertex/OPE algebra.  A one-dimensional vertex algebra arises only
  after a separate restriction datum: a holomorphic curve
  \(C\hookrightarrow\C^2\) with transverse boundary condition or
  factorization-homology pushforward, which singles out a
  one-complex-dimensional holomorphic factorization shadow on which
  Zhu/finite-Laurent OPE applies.
\end{rmk}
...
\begin{rmk}[Type discipline: \(c_f\) versus \(u_f\)]\label{rmk:type-discipline-cf-uf}
  ...
  probe pair, the brane measurement.  The closed Hamiltonian generator
  enters the boundary algebra through the cotangent direction \(u_f\)
...
the polynomial PBW shadow: the Moyal coadjoint action acts on
the residue module, and its image on the polynomial substitute lies
...
\begin{proof}
  ...
  The reverse flow \(z_1\mapsto z_1-tz_2\) on \(z_1^k\) gives
  \(c_{a,b}=c_{k,0}\); chained against the linear values \(c_{1,0}=
   c_{0,1}=1\), \(c_{a,b}=1\) on every bidegree.  Continuity in the
...
obstruction vector]\label{thm:tate-cotangent-lift-iff}
  ...
  \begin{itemize}
    ...
    \item \emph{Physical meaning.}  The matched-bracket open-closed
      ...
      regulator is installed.  Vanishing of \(\operatorname{Ob}_T\) names exactly
...
\medskip\noindent
\emph{The centre of mass remembers what Hamiltonian reduction
...
\begin{rmk}[Computer verification]\label{rmk:reproducibility-certificate}
  ...
  named cohomology class blocking extension
  (Proposition~\ref{prop:app-radial-dual-potential-obstruction}).
\end{rmk}
...
transports.  Zero curvature gives strict coherence;
\(d_\Theta\)-exact curvature gives counterterm or homotopy coherence;
central, closed, non-exact curvature gives projective or gerbe-valued
coherence; non-absorptive curvature forces an enlargement of the
target along a substitution
\(\mathcal T\rightsquigarrow\mathcal T'\).  Volumes I, II, and III supply the algebraic, physical,
and Calabi--Yau categorical arenas in which these transports live;
the local theorem is the formal Hamiltonian
...
\[
  \boxed{\;
  \begin{aligned}
    &\text{The brane is a finite Chan--Paton representation of the formal}\\
    &\text{coordinate algebra; the Hamiltonian Lie algebra acts on the}\\
    &\text{representation moduli by vector fields, and the closed-to-open}\\
    &\text{measurement is the gauge-invariant trace character.}
  \end{aligned}
  \;}
\]
An obstruction is the memory of structure erased by an over-strict
target; the master deformation complex computes that memory.
