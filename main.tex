...
\end{titlingpage}
  ...
  Bershadsky--Cecotti--Ooguri--Vafa \cite{bcov}, renormalised by
  Costello and Costello--Li
  \cite{costello-renormalization,costello-li-quantum-bcov,costello-li-open-closed-bcov},
  reduces near a holomorphic-symplectic brane point to a
  holomorphic-topological Hamiltonian BF theory on the formal
  neighbourhood
  \(X=\R^2_{t,s}\times\widehat{\C^2}_{0,\mathrm{hol}}\), topological
  in the worldvolume coordinates \((t,s)\) and holomorphic in the
  transverse coordinates \((z_1,z_2)\).  The closed-string fields
  $\mathcal A$ take values in the reduced Hamiltonian Lie algebra of
  the formal symplectic polydisk,
  $\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1$ with Poisson bracket
  $\{f,g\}=\partial_{z_1}f\,\partial_{z_2}g
  -\partial_{z_2}f\,\partial_{z_1}g$; the antifields $\mathcal B$
  take values in the continuous Grothendieck--Matlis residue dual
  $\mathfrak h^\vee_{\mathrm{cont}}=
  H^2_{(z_1,z_2)}(\C[[z_1,z_2]])\,dz_1dz_2$ paired by
  $\langle f,\omega\rangle=\Res_0 f\omega$.  Together they form the
  shifted-cotangent BF Lie algebra
  $\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]$
  on the formal symplectic polydisk
  $\widehat{\C^2}_0$, with classical action
  \[
    S=\int_X\langle\mathcal B,
      (d+\bar\partial)\mathcal A
      +\tfrac12[\mathcal A,\mathcal A]_{\mathfrak h}\rangle .
  \]
  The classical observables of this theory form a holomorphic
  factorization algebra on $\C^2$ valued in $E_2$-algebras,
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ --- a
  two-complex-dimensional chiral algebra of Hamiltonian currents,
  factorising over the holomorphic surface in the sense of
  Costello--Gwilliam \cite{costello-gwilliam}.

  \medskip\noindent
  An open boundary is supplied by a stack of $N$ Dirac branes on
  the worldline $\R_t\subset X$.  Its two holomorphic transverse
  coordinates become Chan--Paton matrices
  $\phi_1,\phi_2\in\mathfrak{gl}_N$ under the substitution
  $z_i\mapsto\phi_i$, subject to the moment-map constraint
  $[\phi_1,\phi_2]=0$ resolved derivatively by a Koszul antifield
  $\psi\in\mathfrak{gl}_N[1]$ with $Q\psi=[\phi_1,\phi_2]$.  The
  classical open-string algebra on the worldline is the
  ...
  \[
    J\colon\mathfrak h\longrightarrow
        \mathcal O(\mu^{-1}_{\mathrm{der}}(0))^{GL_N},\qquad
  ...
  $\mu^{-1}_{\mathrm{der}}(0)$ at the formal-Darboux stalk over the
  Heisenberg-core vacuum $b\in\C^2$.
  ...
  on algebraic curves
  \cite{beilinson-drinfeld-chiral}*{\S 4}.  This manuscript proves
  the formal-Darboux stalk identification of that conjecture: the
  equivalence
  \[
  ...
  map $J$ is the formal Darboux coordinate expression of this
  equivalence; the formal-Darboux stalk theorem
  (Theorem~\ref{thm:main-local}) is the tangent vector at one
  Dirac brane vacuum to a global identification that, away from the
  stalk, is the open chiral $E_d$-Deligne conjecture at $d=2$.
  ...
  \]
  the holomorphic-surface analogue of the Heisenberg current on the
  formal disk.  Inverse limits over Matlis windows
  ...
  de Rham complex.  Restriction along a holomorphic curve
  $C\hookrightarrow\C^2$ produces a chiral algebra on \(C\), equivalently
  a vertex algebra after choosing a formal coordinate, with generators and relations the
  ...
  $C\hookrightarrow\C^2$ recovers a chiral algebra on \(C\), equivalently
  a vertex algebra after choosing a formal coordinate, with generators and relations the one-variable
  ...
  Moyal commutator (with $\Phi_\hbar\colon\C[z_1,z_2]\to M_N(\C)$
  the Weyl-symmetric quantisation map of \S\ref{ssec:two-algebras})
  ...
  \medskip\noindent
  The local model is the holomorphic factorization algebra of
  Hamiltonian currents on \(\C^2\).  Its coefficient dual is the
  Matlis principal-part module \(\mathcal P\).  In the curve analogy,
  \(\mathcal P\) replaces the Kac--Moody central-pairing module
  \(\Omega_K\), and \(\mathfrak h\) replaces the regular functions on
  the curve.  The same formal local sector appears in Costello's
  twisted M-theory family \cite{costello-twistedM}.  On the cyclic
  framed stable branch it is the formal-Darboux chart of
  \(\Hilb^N(\C^2)\) in its Moyal deformation.
...
heat-kernel propagator $(K_L-K_\epsilon)$ are fixed in
Appendix~\ref{app:sign-conventions} and
Appendix~\ref{sec:app-factorization-current-conventions}.  The QFT
loop $\hbar$ controls the local BV/Moyal expansion; the string-genus
$g_s$ is the parameter of the ambient theory, not used by the local
theorem.  The local theorem is compatible with the BCOV holomorphic-anomaly
equation in the matched-conventions sense
(Theorem~\ref{thm:divergence-bcov}) but does not assert it.
...
for chiral algebras on curves (Vol I).
...
The pair \((\phi_1,\phi_2)\) is a Chan--Paton representation of the two
formal holomorphic coordinates.  Its scalar reduction is one
...
The formal-stalk chart uses only the germ of \(Y\) at that point.  It
does not use the global topology of \(Y\).  Holomorphic Darboux
...
topological direction supplies the transverse propagator direction for
...
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}
\]
on this chart.  This is the formal Darboux coordinate of the
brane-side \(E_1\)-Hochschild complex
...
Deligne's \(E_1\)-Hochschild theorem.  The input data are the formal
...
It is independent of \(\phi_1,\phi_2\).  Let \(Q\) be the derivation
...
At \(d=1\), for a chiral algebra on a curve, the chiral derived
centre \(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\simeq
C^\bullet_{\mathrm{ch}}(A,A)\) is the theorem of
...
inherits the two-dimensional diagonal OPE
\[
  \mathsf{J}_x(z)\mathsf{J}_y(w)
  \sim
  K^{(2)}_\Delta(z,w)\mathsf{J}_{[x,y]_{\mathfrak g}}(w)
\]
of Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  After
restriction to a holomorphic curve, or after an affine chart,
Drinfeld--Sokolov reduction, or large-\(N\) spin limit, the
one-variable OPEs in the examples are restrictions or
matched-conventions transfers of this diagonal contact algebra.
...
\(\widehat{\mathfrak g}_k\)-chart, with chart-multiplicity formula
  \([\hbar N\bar c]_{\widehat{\mathfrak g}_k}=[\hbar k]\).  After
restricting to a holomorphic curve
...
the chiral Deligne theorem at \(d=1\)
...
fields after restricting to a holomorphic curve.  In the two-variable
formal neighbourhood, the same Heisenberg pair has the contact term
...
\([\hbar N\bar c]_{\beta\gamma}=[\hbar\cdot 2]=[\hbar c]\).  After
restricting to a holomorphic curve (Remark~\ref{rmk:not-a-curve-voa}), the $d=1$
...
\(k\).  After restricting to a holomorphic curve
...
of $b_p$) with two-dimensional singular product
\[
  \mathsf{J}_x(z)\mathsf{J}_y(w)
  \sim
  K^{(2)}_\Delta(z,w)\mathsf{J}_{[x,y]_{\mathfrak g}}(w),
\]
generalising the chiral Deligne theorem at \(d=1\) for chiral
algebras on algebraic curves.  The
