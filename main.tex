...
\end{titlingpage}
  ...
  \(c_f\mapsto O_f\) is a type error
  ...
  defining cohomology class, kernel, or matrix equation
  (Problem~\ref{prob:formal-factorization-center},
  Problem~\ref{prob:quantum-p0-operation-realization},
  the BMK obstruction \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\), and
  the radial obstruction \(\Omega^{\mathrm{rad}}\)).
...
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}
\]
on that matrix-valued transverse coordinate.  No ambient compact
Calabi--Yau is invoked; only the formal Darboux disk at the brane point
and the Chan--Paton substitution
\[
  z_1\rightsquigarrow\phi_1,\qquad z_2\rightsquigarrow\phi_2,\qquad
  \phi_i\in\mathfrak{gl}_N .
\]
The substitution is the only datum.  Every other piece of open-side
structure---symplectic potential, moment map, Dirac constraint, BV
differential, gauge-invariant operator algebra---is its forced
consequence.
...
The lemma fixes the meaning of the open sector.  It is not a
representation of \(\bar A\) on a chosen module; it is the
substitution-realised Lie algebra image of \(\bar A\) inside the open
trace cohomology, and that image generates everything else.  Every
closed-side construction in the manuscript is tested against the brane
by reading it through \(J\).

The matrix microscope is exact at the level of the scalar trace algebra.
It is not exact at the level of the full \(P_0\)-factorization centre.
The natural strengthening---a chain-level morphism from closed
observables on \(L\) to the open factorization centre---fails for a
...
needs three pieces of data: the Lie algebra of infinitesimal closed
symmetries, its shifted cotangent extension carrying the antifields, and
a continuous duality identifying the cotangent module on which closed
observables are read.  Each is forced.
...
gravity \cite{bcov} fixes this language; the present model is its
two-complex-dimensional formal-disk analogue.  Thus the closed BV Lie
algebra is the shifted cotangent extension
...
Cotangent fields on a formal brane are residues, not polynomials.  Each
...
encoded by a single principal-part coefficient.  This is the
defect-momentum content of the closed BF cotangent partner.  An
...
(Corollary~\ref{cor:descendant-coadjoint-difference}).  This is the
type-distinction enforced in \S\ref{ssec:intro-notation-index}.
...
\begin{rmk}[Type discipline: two forbidden assignments]\label{rmk:intro-symbol-separation}
  Two assignments collapse the alphabet distinction and are excluded
  from the manuscript.

  \emph{(i) The operation/measurement type error.}  The boundary
  Hamiltonian observable on the brane is the image of the
  shifted-cotangent coordinate \(u_f\), not of the Hamiltonian CE
  coordinate \(c_f\).  The CE/PV map of
  Theorem~\ref{thm:reduced-ce-pv-central-operation} sends
  \(u_f\mapsto O_f\); the assignment \(c_f\mapsto O_f\) is a forbidden
  type-collapse, identifying an operation (Hamiltonian vector field
  \(\theta^I\)) with an observable (Schouten function \(O_I\)).  This
  collapse breaks the \(P_0\)-bracket compatibility
  \([\theta^I,O_J]_S=\delta^I_J\) on which the central-operation map
  depends, and obstructs every \(P_0\)-centrality homotopy in
  Problem~\ref{prob:formal-factorization-center}.

  \emph{(ii) The label coincidence \(\Psi_{a,b}\leftrightarrow\rho_{a,b}\).}
  The polynomial PBW descendant \(\Psi_{a,b}\) and the principal part
  \(\rho_{a,b}\) carry the same \((a,b)\)-bidegree.  They are not
  identified as \(\mathfrak h\)-modules: their adjoint actions differ
  by the structural separation of
  Corollary~\ref{cor:descendant-coadjoint-difference}, and the only
  intermodule passage in the manuscript is the Fourier-polarized Rees
  bridge of Theorem~\ref{thm:app-matlis-rees-fourier-bridge}.
\end{rmk}
...
visible, equal to \(N\).  This asymmetry is not bookkeeping.  It is a
single Lie cohomology class \([\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)\),
realised both as an abelian-extension obstruction on \(\bar A\) and as
the U(1) trace centre of \(\mathfrak{gl}_N\), and it survives Moyal
quantisation as the order-\(\hbar\) Capelli shift.  The present
subsection inscribes the mismatch into one cocycle and states its three
faces; every subsequent theorem touching \(\widehat{\mathfrak h}\) or
\(\mathcal A_\partial\) flags ``before/after killing the scalar''
explicitly.
...
The class \([\bar c]\) is the same datum read in two languages:
Chevalley--Eilenberg cohomology on the closed side, the
\(\mathfrak{gl}_N\) trace centre on the open side.  The next two
theorems make this explicit, with proofs phrased on each side.
...
\begin{rmk}[Quotient-discipline flag]\label{rmk:quotient-discipline-flag}
  Every theorem in the manuscript using $\mathfrak h$, $\bar A$, or
  $\mathcal A_{\partial}=H^0_{\mathrm{red}}$ lives \emph{after} killing
  ...
  $[\bar c]$.  Theorem~\ref{thm:main-local},
  Theorem~\ref{thm:bulk-boundary},
  Corollary~\ref{cor:local-bulk-boundary-coupling}, and
  Proposition~\ref{prop:first-order-bracket} are all stated reduced;
  their unreduced refinements are controlled by the central extensions
  of Theorems~\ref{thm:u1-center-anomaly}
  and~\ref{thm:u1-center-anomaly-open}.
\end{rmk}

\subsubsection*{Moyal survival of the U(1) center class}

The mismatch class $[\bar c]$ is classical.  The next theorem is the
Moyal-survival statement: deformation quantisation of the formal disk
returns $[\bar c]$ at order $\hbar$ as the leading Capelli shift, with
no higher-order modification on linear trace generators.  The classical
extension and its quantisation agree on the central direction.
...
Two algebras carry the local model.  Both are forced.

The matrix-microscope reading of Principle 1 produces the open algebra.
The probe stack is the Dirac-reduced first-order action
...
(Remark~\ref{rmk:equal-time-locality}).  Nothing here is data: the
algebra is what the matrix microscope reads.

The cotangent reading of Principle 3 produces the closed algebra.
...
\paragraph{Comparison: a universal CE/PV recognition criterion.}
The comparison between \(\mathcal A_N\) and \(\mathcal B\) is not a
postulated map.  It is the cochain-level CE/PV identity on generators
\[
...
assignment \(c_f\mapsto O_f\) is a type error, collapsing operations
onto observables and breaking the cotangent-side homotopies.

What promotes this dictionary to a recognition theorem is the
universal CE/PV ladder of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.  Its content, with
the present notation: at the coordinate envelope the generator map
extends to the dg graded-commutative isomorphism
\(\Phi_{\mathrm{coord}}\colon
...
The criterion is a recognition test.  A pair
\((\mathcal A_N(I),\mathcal B)\) is a CE/PV pair to a specified depth
exactly when the corresponding admissibility hypothesis is verified.
...
\paragraph{Named obstruction: strict large-\(N\) and unreduced lift.}
Strict large-\(N\) is not part of the local theorem.  Identification
of the raw finite-\(N\) operator algebras with the strict formal
Hamiltonian envelope requires an open-trace \(A_\infty\)-Koszul
acceptance datum
...
(Theorem~\ref{thm:universal-ce-pv-koszul-criterion}, item (3)).  In the
absence of such a datum, the cochain CE/PV identity is the only
admissible matching; bar-cobar/enveloping conclusions and large-\(N\)
operator-algebra identifications are not.

The unreduced cotangent-included lift to a chain morphism of
factorization algebras is the obstruction problem
\[
  \Phi_{\mathrm{cl}}\colon
...
\]
named in Problem~\ref{prob:formal-factorization-center}.  Its
defining datum is the strict Casimir support obstruction
\(C_{\mathfrak h}\), the linear translation term
...
classes are exactly what blocks the unreduced cotangent centrality
homotopies of Theorem~\ref{thm:witten-centrality-local-form}.  The
microscope of \(\mathcal A_N\) reads scalar trace classes; the
unreduced cotangent factorization-centre datum requires the named
companion data, and is absent without it.

\paragraph{Provenance.}  Costello's protected sector of perturbative
twisted M-theory on \(\R\times X\), with \(X\) a holomorphic-symplectic
surface and a stack of \(N\) topological branes on \(\R\times p\),
suggests a Hamiltonian formal sector at the symplectic disk
\(\widehat X_p\).  This manuscript does not prove such a sector for an
arbitrary ambient background; if one is supplied as an admissible
protected Hamiltonian restriction, the local theorem of
...
This paragraph is after the Hamiltonian scalar quotient; before
projection the unreduced bracket has the central term
$N\omega(f,g)\int_I ab\,dt$.
...
transverse holomorphic dependence is distributional.
...
\begin{thm}[BV replacement for unitarity in the protected sector]
  ...
  For the local mixed holomorphic-topological sector studied here, the
...
cyclic open BV theory, and a bulk-to-brane map.  The words
\emph{topological} and \emph{holomorphic} are not stylistic
modifiers.  Each names the unique factorization habitat compatible
with one of the locality differentials of
Proposition~\ref{prop:three-localities}.  The Definitions below fix
each habitat by that compatibility.  This is a local
string-field-theoretic use of the word ``string'', not a definition
of a global theory of worldsheet moduli or amplitudes.

The protected algebraic content is what
Theorem~\ref{thm:bv-replacement-unitarity} preserves: the classical
and quantum master equations, locality, and factorization
associativity.  What \emph{topological} and \emph{holomorphic} add
is the constraint on how local operators vary across spacetime,
forced by the locality differentials.

Gwilliam--Williams \cite{gwilliam-williams-holomorphic-bosonic-string}
are the precedent for formulating a holomorphic string in the
Costello BV/factorization formalism, with Dolbeault fields
\((\gamma,\beta,c,b)\), one-loop quantization in the critical complex
dimension, and a factorization algebra recovering BRST cohomology.
The object here is not that theory and not a replacement definition:
it is a formal local target-space sector with de Rham/Floer
directions, a holomorphic-symplectic formal disk, Hamiltonian
...
\begin{rmk}[Unique habitat; BCOV as source convention]
  ...
  BCOV/Kodaira--Spencer \cite{bcov} supplies the source convention
  only: Dolbeault polyvectors, cotangent BV notation, the divergence
  determined by a holomorphic volume form.  In the present surface
  model the holomorphic-symplectic form \(dz_1\wedge dz_2\) and the
  volume form \(dz_1dz_2\) are local unimodularity data, providing
  the BV pairing, divergence operator, and cyclic trace densities;
  divergence-free vector fields are represented locally by
  Hamiltonians modulo constants (Principle~2).  No compact-CY claim
  is imported.
\end{rmk}
...
\begin{rmk}[Quantum extension]
  ...
  renormalized BV action satisfying the quantum master equation.
  The manuscript proves the componentwise quantum coefficient
  statement: weighted BV kernel convergence, the balanced
  scalar-contact zero projection on its habitat, reduced weighted
  Moyal principal-part current brackets, and a degree-zero
  ...
  total degree at most \(13\), and the total-degree \(14\) cases
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\);
  Proposition~\ref{prop:app-edge-pbw-telescoping} proves the complete
  ...
  \(D^\square_{a,b}=C^+_{a,b}\partial_2\); failure is exactly a
  signed row \((\phi,-\lambda)\in\ker B_{a,b}^*\) with
  \(\lambda(E^+_{a,b})-\phi(T_{a,b})\ne0\).  This sharpens the older
  \(\mathcal C_{a,b}\)-left-cokernel language for the obstruction
  classes \(\mathfrak o_{a,b}\) of
  Definition~\ref{def:app-free-kernel-correction-obstruction}.  The
  finite-window non-scalar closed-open BV criterion is the vanishing
  problem in \(\mathfrak Q^\bullet_{w,\partial,\hbar}(I)\) of
  Problem~\ref{prob:quantum-p0-operation-realization}.
\end{rmk}

\paragraph{Named obstruction.}
Definition~\ref{def:local-th-string} fixes the unique factorization
habitat compatible with all three locality differentials of
Proposition~\ref{prop:three-localities}.  It is not the renormalized
quantum theory.  A full-renormalized quantum mixed h-t string
requires Costello graph data outside the local Hamiltonian theorem:
the analytic graph-realization datum of
...
all-bidegree radial vanishing
\(\Omega^{\mathrm{rad}}_{a,b}=0\) of the previous remark.  The
classical Definition is supplied here; the renormalized quantum
extension is residual and named.

\paragraph{Three levels.}
Definition~\ref{def:local-th-string} separates three levels.  A
pure topological sector is locally constant in all real directions;
examples include the A-model local/Floer direction and the
Chern--Simons-type open field theory on a topological brane.  A
pure holomorphic sector is Dolbeault in its complex directions and
produces a holomorphic factorization algebra;
Gwilliam--Williams' holomorphic bosonic string is a worldsheet
example.  A mixed holomorphic-topological sector lives on
\(M\times Y\) with locally constant factorization in \(M\) and
holomorphic factorization in \(Y\).  The A/B terminology used below
is one source of such examples: the A-side supplies the de Rham
direction and the B-side supplies the Dolbeault or formal
...
records this object as
\(\mathcal F^{\mathrm{hol}}_{\mathrm{Ham}}\) with mixed enhancement
\(\mathcal F^{\mathrm{mix}}_{\mathrm{Ham}}\); every theorem of this
section is a theorem about this completed object.  Compact and global
comparisons require separate hypotheses and are not part of the
formal-local claim.
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  Thus any use of the word ``chiral'' in this paper refers to a
  two-complex-dimensional holomorphic factorization algebra and its mixed
  topological-current extensions.  A one-dimensional VOA/OPE statement
  requires an additional theorem choosing a complex line or holomorphic
  defect, a transverse boundary condition or restriction functor, and the
  finite Laurent OPE expansion on that restricted object.
\end{defn}
...
\begin{prop}[Bochner--Martinelli current data and finite-window obstruction for native \(E_2\) transfer]\label{prop:finite-window-bm-native-e2-transfer}
  ...
  \(z_1\cdot\rho_{a,0}=-\rho_{a,1}\).  Hence this proposition proves the
  BMK current data, the finite-window obstruction theorem, and the
  arity-two native \(\C^2\) coefficient comparison.  It does not prove a
  curve VOA, a Zhu algebra, a strict support-local finite-window quotient,
  or a strict all-arity transfer.
\end{prop}
...
\begin{prop}[Native Darboux disk constructions]\label{prop:native-darboux-disk-constructions}
  ...
  \begin{enumerate}
    ...
    \item The Bochner--Martinelli--Koppelman current limit, finite
      ...
      \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\).
  \end{enumerate}
  These constructions are internal to the formal local
  \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) model.
\end{prop}
...
\begin{rmk}[Formal stalk restriction]
  The Hamiltonian identification used here is formal local. On a holomorphic
  ...
  $\iota_{X_f}\omega=-df$. A global Hamiltonian sector therefore requires
  a sheaf of local Hamiltonians and the vanishing of the corresponding
  holomorphic de Rham obstruction. The formal disk used in this paper has
  no such obstruction by the holomorphic Poincare lemma.
\end{rmk}
...
algebra: a holomorphic volume input on the Darboux disk, not a
compact-CY assumption.

\subsection{Hamiltonian sector and stable large N}

The stable large-\(N\) comparison is an admissibility target, not a
source of the local theorem.  This subsection isolates the Hamiltonian
sector at a single brane point and records the stability discipline that
governs every later large-\(N\) statement.  The coefficient algebras are
\(\bar A=\C[z_1,z_2]/\C\cdot1\) on the polynomial side and
\(\mathfrak h=\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) on the
completed side, with \(\mathfrak h^\vee=\widehat{\mathfrak h}^\vee_{\mathrm{cont}}\)
its continuous dual; \(\widehat{\mathbf S}\) denotes the completed
symmetric algebra and \(C^\bullet_{\mathrm{CE},\mathrm{cont}}\) the
matched continuous CE complex.  Constants are quotiented out for the
reason fixed in \S\ref{ssec:intro-notation-index}: a constant Hamiltonian
has zero Hamiltonian vector field.  On the open side
\(\operatorname{Tr}_N(1)=N\) is the empty trace, a central scalar tracked
through scalar reduction without quotienting the operator unit; if \(T\)
is a positive word-degree trace expression, \(\overline T\) denotes its
connected class.  Shifts follow the BV convention: an element of \(V[1]\)
has parity reversed and cohomological degree one lower than the
corresponding element of \(V\).  The CE/PV cotangent block below uses its
own coordinate grading: the odd BV cotangent generator \(\lambda[1]\)
has BV degree \(-1\), while the dual CE coordinate \(u_\lambda\) is
...
\paragraph{Admissibility hypotheses.}
By \emph{stable large-\(N\)} we mean the degreewise stable value of the
invariant algebras under block restriction, accessed only after the
following four hypotheses are checked.
\begin{enumerate}
  \item[(A1)] Procesi--Razmyslov stable invariant theory
    \cites{procesi,razmyslov}: stable invariants of two matrices are
    freely generated by cyclic trace words.
  \item[(A2)] Adjacent-swap exactness in the connected sector: the
    Koszul differential \(Q\psi=[\phi_1,\phi_2]\) makes adjacent
    transpositions \(Q\)-exact (Lemma~\ref{lem:adjacent-swaps}), so
    ghost-zero cyclic words collapse on \(H^0\) to commutative monomials
  ...
  \item[(A3)] Connected-trace projection: the LQT cyclic-homology
    template \cites{loday-quillen,tsygan,loday-vallette} supplies the
    primitive single-cycle class.  Same-rank deletion is not a chain
    map; the finite-rank bridge is the separated-block zigzag
    \(P^{\mathrm{sep}}_{M,K,L}=\beta_{M,K,L}P_{1\mathrm{cyc}}\alpha_{M,K,L}\)
    of Remark~\ref{rmk:same-rank-vs-LQT} below.
  \item[(A4)] Stable-window finite-\(N\) bound: the LQT projection is a
    chain map only for \(N\ge\max(K,L+2)\)
    (Theorem~\ref{thm:stable-eulerian-lqt-window}).  Outside this window
    finite-\(N\) trace identities are present and break the projection.
\end{enumerate}
Hypotheses (A1)--(A4) are the universally-checked content of
``stable large-\(N\)'' in this manuscript.  They do not by themselves
identify the raw finite-\(N\) operator algebras with the strict formal
Hamiltonian envelope: that identification is the open-trace
\(A_\infty\)-Koszul acceptance datum
\((A_{\mathrm{op}},C_{\mathrm{op}},B_{\mathrm{CE/PV}},\tau_{\mathrm{op}})\)
of Definition~\ref{defn:stable-ainfty-koszul-acceptance-datum}, with
acceptance certified by
Theorem~\ref{thm:stable-ainfty-koszul-under-hypotheses}.  Without that
datum, the comparison stays inside the connected-trace pattern of
Theorem~\ref{thm:main-local}; no global \(HC_{*-1}\) convention is
transferred.

\paragraph{Named obstruction (open-trace acceptance datum).}
The obstruction blocking the strict large-\(N\) identification is the
open-trace \(A_\infty\)-Koszul acceptance complex
\((A_{\mathrm{op}},C_{\mathrm{op}},B_{\mathrm{CE/PV}},\tau_{\mathrm{op}})\).
...
The reduced Hamiltonian sector below uses Procesi--Razmyslov invariant
theory together with the explicit Koszul differential, not the strict
identification; the LQT theorem supplies the template.
...
The dictionary uses three disjoint alphabets carrying two type checks.
Witten's source convention forces the closed-CE-coordinate type check
\(c\mapsto\theta\) versus \(u\mapsto O\): the Hamiltonian-dual coordinate
\(c\) measures vector-field operations, while the cotangent-dual
coordinate \(u\) measures boundary Hamiltonians.  Grothendieck residue
duality forces the cotangent-realization type check \(\Psi\) versus
\(\rho\): the polynomial one-\(\psi\) trace classes \(\Psi_{k,l}\) are
PBW special-fibre labels on the open side, the principal parts
\(\rho_{a,b}\) are the deformation-level cotangent module on the closed
side.  Coincident bidegrees \((k,l)=(a,b)\) are an indexing convention,
not an identification of underlying modules.
...
\paragraph{Label-collision discipline.}
The vector-space pairing
\(e^{\mathrm{PBW}}_{k,l}\mapsto[\Psi_{k,l}]\) on the PBW special fibre is
\emph{not} the principal-part realization and \emph{not} an
...
The available passage from the PBW label carried by \(\Psi_{k,l}\) to the
principal-part label \(\rho_{k,l}\) is the Fourier-polarized Rees
...
not the unreduced quantum boundary observable.  Corollary~\ref{cor:descendant-coadjoint-difference}
records the exact deformation-level mismatch.

\subsection{Cotangent CE/PV reduced central-operation spine}
\label{ssec:coordinate-free-ce-pv-synthesis}

This subsection states the measurement claim that turns a closed
Hamiltonian into a brane observable: closed CE cochains of the shifted
cotangent Hamiltonian Lie algebra are Schouten polyvectors of the
boundary Hamiltonian polynomial algebra, in the coordinate window, with
\(c^I\mapsto\theta^I\) and \(u_I\mapsto O_I\).  The Schouten/cotangent
bracket carrying the comparison has degree \(-1\); the reduced
Hamiltonian boundary bracket on \(O_I\) is the degree-zero Poisson
bracket induced by the matrix Poisson form.  Completion at the formal
disk is coefficientwise -- each requested output of \(d_{\mathrm{CE}}c\),
\(d_{\mathrm{CE}}u\), \(d_\pi^{\mathrm{coord}}\theta\), or
\(d_\pi^{\mathrm{coord}}O\) is a finite monomial test -- so the
finite-dimensional Schouten identity assembles to the formal disk
identity on the coordinate envelope.  Bar-cobar/Koszul duality is the
separate admissible enhancement, governed by conilpotence, exact
continuous duality, filtered cobar convergence, and the open-trace
\(A_\infty\)-Koszul acceptance datum named in \S\ref{ssec:two-algebras}.
...
\subsection{Local theorem: constituent constructions}\label{ssec:quantum-target}

Theorem~\ref{thm:main-local} assembles from nine constituent
constructions.  Each entry records the principle it realizes, the named
obstruction blocking its strengthening, and the pointer to its proof.

\begin{enumerate}
  \item[(C1)] \emph{Affine skyscraper Yoneda algebra.}  Realizes
    Witten's source convention at the brane point: the open field at
    \(\mathcal O_p\in\C^2\) is
    \(\Lambda(\varepsilon_1,\varepsilon_2)\) with cyclic trace fixed by
    the local volume.  Obstruction: a global compact-CY datum is not
    asserted; only the residue/cyclic pairing is used.  Proof:
    Proposition~\ref{prop:skyscraper-ext}.
  \item[(C2)] \emph{Open BV reduction.}  Realizes Dirac: the cyclic
    Chern--Simons action of an open brane reduces, after the linear
    Dirac normalization, to the first-order matrix model
    \eqref{eq:open-action-derived} with
    \(Q\psi=[\phi_1,\phi_2]\).  Obstruction: the moment-map equations
    are not a regular sequence at \(N\ge2\), so the BV truncation is
    only first-Koszul.  Proof: Lemma~\ref{lem:open-action-reduction},
    Proposition~\ref{prop:open-bv-truncation}.
  \item[(C3)] \emph{Dirac probe reduction of the phase space.}
    Realizes Dirac as a constrained-mechanics statement: the open
    algebra is gauge-invariant functions on the derived commuting-pair
    locus, not an imposed trace algebra.  Obstruction: \(N\ge2\)
    non-flatness on the off-diagonal stratum (recorded in the proof).
    Proof: Lemma~\ref{lem:dirac-probe-reduction}.
  \item[(C4)] \emph{Stable invariant-theory of two matrices.}
    Realizes admissibility hypothesis (A1): stable invariants are
    cyclic trace words.  Obstruction: only stable; finite-\(N\) trace
    identities re-enter outside the \(N\ge\max(K,L+2)\) window.  Proof:
    Proposition~\ref{prop:stable-trace-invariants},
    Corollary~\ref{cor:matrix-evaluation-well-defined}.
  \item[(C5)] \emph{Primitive one-\(\psi\) Koszul homology.}  Realizes
    the P3 cotangent measurement on the open side as a PBW
    special-fibre label.  Obstruction: same-action identification with
    the principal-part module fails
    (Theorem~\ref{thm:pbw-vs-deformation},
    Corollary~\ref{cor:descendant-coadjoint-difference}); only the
    Fourier-polarized Rees label bridge
    (Theorem~\ref{thm:app-matlis-rees-fourier-bridge}) connects the
    two.  Proof: Proposition~\ref{prop:one-psi-homology}.
  \item[(C6)] \emph{Hamiltonian polyvector reduction.}  Realizes
    Witten's source convention on the closed side: the Hamiltonian BF
    fields reduce to
    \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\),
    and closed local operators are
    \(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\).
    Obstruction: the strict-product CE/PV comparison fails; the
    coordinate-window theorem of
    \S\ref{ssec:coordinate-free-ce-pv-synthesis} replaces it.  Proof:
    Proposition~\ref{prop:hamiltonian-polyvector-reduction},
    Proposition~\ref{prop:grav-ops}.
  \item[(C7)] \emph{Bulk-to-boundary Hamiltonian evaluation.}  Realizes
    the P3+P4 measurement: a bulk Hamiltonian \(f\) is read off the
    brane as \(O_f=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\), with
    bracket \(\{O_f,O_g\}_\partial=O_{\{f,g\}}\) on the reduced side
    and central \(N\omega(f,g)\) on the unreduced side.  Obstruction:
    the unreduced lift requires the centrality homotopies and current
    kernel of Remark~\ref{rmk:unreduced-lift-obstruction} and
    Problem~\ref{prob:formal-factorization-center}.  Proof:
    Construction~\ref{constr:boundary-evaluation},
    Theorem~\ref{thm:bulk-boundary},
    Corollary~\ref{cor:local-bulk-boundary-coupling}.
  \item[(C8)] \emph{Coordinate CE/PV source-to-boundary synthesis.}
    Realizes the P3 measurement claim: closed CE cochains of
    \(\mathfrak g\) identify with Schouten polyvectors of
    \(\widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h)\) on the
    coordinate window, with \(c\mapsto\theta\) and \(u\mapsto O\).
    Obstruction: the bracket and kernel upgrades require the
    bracket-admissible \(B\) and kernel-admissible
    \((B,\mathcal K_B)\) of clause (2) of
    Theorem~\ref{thm:universal-ce-pv-koszul-criterion}; the
    bar-cobar/Koszul upgrade requires the open-trace
    \(A_\infty\)-Koszul acceptance datum of clause (3).  Proof:
    Theorem~\ref{thm:coordinate-free-closed-open-synthesis},
    Theorem~\ref{thm:reduced-ce-pv-central-operation},
    Theorem~\ref{thm:formal-disk-completed-ce-pv}.
  \item[(C9)] \emph{Componentwise quantum coefficient comparison.}
    Realizes the P6 quantization measurement: weighted BV kernel
    convergence, the balanced scalar-contact zero projection, reduced
    weighted Moyal principal-part current brackets, and the
    degree-zero Moyal trace.  Obstruction: the all-interior radial
    obstruction class
    ...
    equivalently the decorated PBW Stokes problem for
    \(D^\square_{a,b}=C^+_{a,b}\partial_2\); the non-scalar QME class
    \([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
    \in H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}})\)
    of Problem~\ref{prob:quantum-p0-operation-realization}.  Proof:
    Theorem~\ref{thm:componentwise-quantum-coefficient-comparison};
    edge strips by Proposition~\ref{prop:app-edge-pbw-telescoping}.
\end{enumerate}

The first bracket calculation \eqref{eq:bracket-coefficient},
\(\{O_{k,l},O_{m,n}\}_\partial=(kn-lm)O_{k+m-1,l+n-1}\), is the
arithmetic kernel that all nine constructions are required to recover.

\subsection{The local theorem}\label{ssec:local-theorem}

The local theorem is the measurement claim of the seven-principle
mechanism.  Dirac (P1) selects the open BV complex as the derived
zero-fibre of the conjugation moment map and the stable trace
observables \(O_{k,l}\).  The U(1) center class \([\bar c]\) (P2)
records the scalar invisibility/anomaly: invisible after empty-trace
projection, surviving as the Moyal/Capelli shift \(\hbar N[\bar c]\)
under quantization.  Witten's source convention (P3) selects the
closed-side target as a derived \(P_0\)-centre and forces the
...
residue duality (P4) selects the cotangent coefficient module as
Matlis principal parts \(\rho_{a,b}\) and separates them from the
polynomial PBW special-fibre shadow \(\Psi_{k,l}\): the bidegree label
collision is structural, not an identification.  The three locality
differentials of Proposition~\ref{prop:three-localities} (P5) -- support,
topological, formal holomorphic -- are independent and force the mixed
factorization-algebra shape of \(\mathcal F^{\mathrm{mix}}_{\mathrm{Ham}}\).
Componentwise quantum coefficient comparison
(Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}) realises
the quantization measurement test (P6).  Stronger statements are
obstruction problems: factorization-centre lift, non-scalar QME,
all-bidegree PBW Stokes, and native all-window BMK transfer (P7).
...
admissibility criterion of clause (3) below.
...
\begin{rmk}[Dirac probe principle in the reduced Hamiltonian sector]\label{rmk:dirac-thesis}
  Theorem~\ref{thm:main-local} makes precise the Dirac
  thesis sentence for the reduced Hamiltonian sector:
  \begin{quote}
    \itshape
    ...
    sector, the coordinate CE cochains of the shifted-cotangent
    Hamiltonian Lie algebra identify, after a bracket-admissible
    realization, with reduced \(P_0\) Hamiltonian central operations on
    the stable open brane algebra.
  \end{quote}
  This follows from \ref{thm:main-local}: the brane stack is
  ...
  effective traceless moment-map ideal
  $I_N=\mc W_N\widehat\mu(\lie{sl}_N)$ produces
  ...
  degree-zero Hamiltonian sector has the radial-parts quantum comparison
  stated later; the full unreduced factorization centre is the obstruction
  datum of Problem~\ref{prob:formal-factorization-center}.
\end{rmk}

\paragraph{Obstruction vector for stronger statements.}
...
\end{itemize}
Theorem~\ref{thm:main-local} is the measurement statement of the
seven-principle mechanism on the formal local model alone; its portable
content is the cochain CE/PV identification, with the enveloping/Koszul
upgrade governed by the admissible criterion of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.  The protected
twisted-M-theory origin is not used: once an ambient background supplies
an admissible protected Hamiltonian restriction, the local theorem
applies to that restricted sector and no further.
...
Every theorem below reads off this single geometric pairing.
...
worldvolume. Three structural requirements isolate its habitat.

\emph{Symplectic pair.} The pair \((\phi_1,\phi_2)\) realizes a
...
\emph{Formal Darboux locality.} The matrix probe sees only the local
behaviour at the brane, never global topology of \(Y\). Every theorem
in this paper is a theorem about the formal completion of \(Y\) at the
brane point. A holomorphic Darboux theorem trivialises the germ to
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\), so \(Y\) contributes only its
formal symplectic disk.

\emph{Trace channel.} The brane stack carries an open trace observable
...
These three requirements force
...
a single brane point. Its packaging as a sheaf along the brane line
\(L\) and over the formal holomorphic point requires two further data:
the topological factor along \(\R_t\) and the holomorphic factor
transverse to \(L\) inside \(\widehat{\C^2}_0\).
...
is the gauge symmetry generated by that moment map. Three structural
moves derive the open theory.
...
\begin{proof}
  ...
  sector is Lemma~\ref{lem:open-action-reduction} below; the
  identification of the BV differential with the moment-map Koszul
  differential is Lemma~\ref{lem:dirac-probe-reduction} below; the
  ...
  combined with Proposition~\ref{prop:open-zero-fibre-traces}. The
  three derivations together establish the proposition.
\end{proof}

The constructions and lemmas below carry out the three reductions in
detail, then close \S\ref{ssec:open-mixed-bft} with the named
obstruction blocking the unreduced Tate-coefficient cotangent lift.
...
We now compute the three reductions promised in
Proposition~\ref{prop:open-bv-action-derived}. Lemma~\ref{lem:open-action-reduction}
identifies the ghost-zero piece of the Chern--Simons action with the
Dirac action \eqref{eq:open-action-derived-23};
Lemma~\ref{lem:dirac-probe-reduction} identifies the multiplier
coupling with the moment-map zero-fibre; Proposition~\ref{prop:open-bv-truncation}
identifies the BV truncation with the Koszul resolution. These three
reductions construct the matrix-microscope avatar of the open BV
theory.
...
\begin{proof}
  ...
  \(\varepsilon\)-degree.  This is a physical ghost-zero truncation.  The
  Dirac-normalized variables give the ordinary \(GL_N\)-conjugation action
  used in the reduced local-operator calculation.
\end{proof}
...
\subsubsection*{Closing obstruction: unreduced Tate-coefficient
cotangent lift}

The construction above produces the \emph{reduced} open BV theory
and its Procesi--Razmyslov ghost-zero observable algebra. The
\(P_0\)-algebra structure on the unreduced open observables, with
the cotangent direction realised at the chain level, is not produced
by these constructions and is the named obstruction below.
...
\begin{prob}[Unreduced cotangent lift of the open BV theory]\label{prob:open-cotangent-lift}
  ...
  \]
  before primitive projection. The full unreduced cotangent lift is
  recorded as Problem~\ref{prob:formal-factorization-center}; the
  open-side data isolated above is its restriction to the brane line.
\end{prob}

The Procesi--Razmyslov ghost-zero theorem of
Proposition~\ref{prop:open-zero-fibre-traces} closes the reduced
ghost-zero open observable algebra; the centrality homotopies and
Tate-cotangent module of Problem~\ref{prob:open-cotangent-lift} are
the residual data. Their failure mode is the obstruction class of
...
$\bigl(\widehat{\C^2}_0,\,\omega=dz_1\wedge dz_2\bigr)$.  Two
geometric measurements force the closed BV complex.  Divergence-freeness
...
defect current of
Theorem~\ref{thm:reduced-principal-part-boundary-current}: this is
P3 (cotangent fields measure) and P4 (the cotangent module is the
Matlis residue module) read off the closed BV complex.  The BF action
of Section~\ref{ssec:hamiltonian-bf-action} is the unique cyclic local
...
\begin{rmk}[Casimir/Tate obstruction to a strict cotangent kernel]\label{rmk:closed-bv-casimir-obstruction}
  ...
  (Definition~\ref{defn:bracket-kernel-admissible}).  This is the named
  obstruction guarding every theorem in
  Sections~\ref{ssec:hamiltonian-bf-action}--\ref{ssec:reduced-brane-coupling}
  against an unreduced cotangent-kernel claim.
\end{rmk}
...
\end{equation}
The measurement principle of this section is that the bulk Hamiltonian
bracket is the brane Poisson bracket of these observables: this is the
moment-map operation forced by Dirac probe physics, not postulated.
...
with Theorem~\ref{thm:brane-measurement-principle} this assembles the
two measurement maps into a cosheaf source datum on intervals
$I\subset\R$, resolved below.
...
\begin{rmk}[Locality from the canonical equal-time bracket]\label{rmk:equal-time-locality}
  ...
  recorded in \ref{cor:local-bulk-boundary-coupling}.  In the classical
  reduced, de Rham-contracted sector this delta-function locality
  produces a strict support-local \(P_0\) factorization product.  The
  unreduced BV current complex still requires its own homotopies,
...
\begin{proof}
  ...
  $\mathfrak h_I$ as a biderivation.  In this theorem
...
\begin{rmk}[Locally constant equivalence and $E_1$-algebra translation]\label{rmk:E1-translation}
  ...
  invariance by Lurie, \emph{Higher Algebra}, \S 5.5.4, in the
	  ...
	  factorization-center stalk.  The present reduced Hamiltonian theorem
...
\begin{constr}[Compact current PBW/Rees source]\label{constr:compact-current-pbw-rees-source}
  ...
  This is the PBW/Rees source for a Layer 4 comparison involving
  Hamiltonian and principal-part modes.
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  Four mathematical entries separate the residual
  (\S\ref{ssec:tate-residual-resolution}): the Costello--Li 2012
  BCOV/HCS theorem is a negative source-comparison and does not transfer
  to the present mixed defect geometry; weighted
...
defect.  This subsection assembles that vector, names its habitats, and
identifies the construction problem inscribed by each entry.
...
vector]\label{defn:tate-cotangent-lift-obstruction-vector}
  ...
  \begin{equation}\label{eq:tate-obstruction-vector}
    \operatorname{Ob}_T
      ...
      \bigr),
  \end{equation}
  with components attached to inputs T1--T5 and P1, P3, P5: the weighted
  Casimir kernel class \(\mathfrak o_{\mathrm{Cas}}\) (T1), the nilpotent
  truncation class \(\mathfrak o_{\mathrm{nilp}}\) (T2), the
  Quillen/Tate-bar-cobar class \(\mathfrak o_{\mathrm{Quil}}\) (T3), the
  BV endpoint tuple \(\mathfrak o_{\mathrm{BV,end}}\) (T4), the
  chain-level primitive class \(\mathfrak o_{\mathrm{prim}}\) (T5), the
  Hadamard locality class \(\mathfrak o_{\mathrm{Had}}\) (P1), the
  protected universality class \(\mathfrak o_{\mathrm{univ}}\) (P3), and
  the cross-volume matched-conventions class \(\mathfrak o_{\mathrm{cv}}\)
  (P5).  Every component is named with its defining habitat in the
  paragraphs below.  The reference Costello--Li 2012 BCOV/HCS theorem is
  not a transferable source for the present brane-defect geometry; the
  hypothesis-separation lemma at the head of T4 fixes that boundary and
  forces the eight components.
\end{defn}

\paragraph{T1 (weighted completion); component
\(\mathfrak o_{\mathrm{Cas}}\).}
...
\paragraph{T2 (nilpotent truncation); component
\(\mathfrak o_{\mathrm{nilp}}\).}
The truncation
...
\paragraph{T3 (Quillen equivalence); component
\(\mathfrak o_{\mathrm{Quil}}\).}
...
\paragraph{T4 (BV-cohomology vanishing); component
\(\mathfrak o_{\mathrm{BV,end}}\).}
...
\paragraph{T5 (chain-level primitive); component
\(\mathfrak o_{\mathrm{prim}}\).}
...
\paragraph{P1 (Hadamard/Mittag-Leffler); component
\(\mathfrak o_{\mathrm{Had}}\).}
...
\paragraph{P3 (universality); component \(\mathfrak o_{\mathrm{univ}}\).}
...
\paragraph{P5 (cross-volume); component \(\mathfrak o_{\mathrm{cv}}\).}
...
\begin{proof}
  The classical assertion is the assembly of the named criteria.
...
\begin{cor}[Proved sublattice and named residual]\label{cor:tate-residual-sublattice}
  The present manuscript proves the sublattice of
  \(\operatorname{Ob}_T\) consisting of:
  ...
  The residual obstruction is the named subvector
  \[
  ...
  \ref{thm:matched-conventions-transfer}.  Each component is a precise
  construction problem with named missing datum, not a heuristic
  expectation.
\end{cor}

\begin{proof}
  The proved sublattice is the disjoint union of the cited theorems;
  each statement supplies the proved component named.  The residual
  subvector is the complement: the named classes whose vanishing is not
  proved by the displayed sublattice and whose construction problem is
  inscribed by the corresponding paragraph above.  Iff-vanishing is
  Theorem~\ref{thm:tate-cotangent-lift-iff}.
\end{proof}

\subsection{Open-brane operators and the large $N$ limit}\label{ssec:open-brane-operators}

\paragraph{Measurement claim.}
...
connected single-trace projection cannot absorb.  This is the
operator-level realisation of Principle~1 (matrix-probe
measurement) and Principle~2 (scalar invisibility, scalar anomaly).
Two named theorems determine the algebra: Procesi--Razmyslov 1976
(\cites{procesi,razmyslov};
Proposition~\ref{prop:stable-trace-invariants}) fixes the classical
content, and Capelli 1890 (\cites{capelli,howe-capelli};
Remark~\ref{rmk:normal-ordering-obstruction}) forces the quantum
scalar.  The proved local theorem (Theorem~\ref{thm:main-local})
reads its operator side off the present subsection.

\paragraph{Principle 1 in coordinates: matrix probes give cyclic
words.}
...
quotient \(V_H=\bar A\oplus\bar A_{\mathrm{desc}}[1]\) is the
open-side operator avatar of the matrix-probe measurement claim.

\paragraph{Principle 2 in coordinates: the Capelli scalar shift.}
...
term.  Concretely, the Weyl-ordered moment relation forces
\[
...
disconnected products, but it does not remove this lower-degree
trace insertion.  The Capelli shift is the quantum mirror of the
closed-side scalar class \([\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)\)
of Theorem~\ref{thm:u1-center-anomaly-open}: the same scalar appears
...
\paragraph{Stable large-\(N\) is admissibility, not source.}
The manuscript imports no stable large-\(N\) trace theorem.  The
operator-side comparison closes at finite \(N\) through the
...
with \(\lambda(E^+_{a,b})-\phi(T_{a,b})\ne 0\).  This is the named
admissibility block the subsection closes with: the open-trace
...
\begin{defn}[Three large-\(N\) operations]\label{defn:three-large-N-operations}
  Three different operations must not be conflated. They have different
  domains and are applied in the order
  displayed below.
  \begin{enumerate}
...
\begin{rmk}[Named admissibility obstruction: open-trace $A_\infty$-Koszul acceptance datum]\label{rmk:open-trace-koszul-admissibility}
  The operator content of this subsection closes with one named
  admissibility datum.  After the Capelli renormalisation
  ...
  homology lie outside the sector and outside the admissibility datum.
  Stable large-\(N\) brane-trace asymptotics enter the manuscript only
  through this admissibility datum
  (Definition~\ref{defn:stable-ainfty-koszul-acceptance-datum}); no
  large-\(N\) source theorem is imported.
\end{rmk}

\subsection{Operators in the closed Hamiltonian sector}\label{sec:grav-ops}

\paragraph{Measurement claim.}
On the closed side of the local mixed model, ghosts move and
cotangent coordinates measure (Principle~3): Hamiltonian-dual CE
...
Hamiltonian disk algebra of Principle~2 and
\(\mathfrak h^\vee_{\mathrm{cont}}\cong\mathcal P\) its continuous
residue dual of Principle~4
(Theorem~\ref{thm:principal-part-coadjoint-uniqueness}).  This is
Proposition~\ref{prop:grav-ops}: the operator-level realisation of
Principle~3.

\paragraph{Forcing reasons.}
Two structural facts force the identification.  First, the kinematic
BRST operator \(Q_0=d_{\R^2}+\bar\partial_{\C^2}\) contracts every
positive topological and anti-holomorphic jet, by the Euler homotopy
...
field \(\alpha\) and the cotangent field \(\beta\).  Second, the
residual interaction differential reproduces the Lie brackets of
...
inputs contribute zero.  The residual differential is therefore the
Chevalley--Eilenberg differential of \(\mathfrak g\), and the closed
local operator algebra is its continuous cochain algebra.

\paragraph{Open-closed match.}
The open side of \S\ref{ssec:open-brane-operators} has supplied the
...
\(A_\infty\)-Koszul admissibility datum of
Remark~\ref{rmk:open-trace-koszul-admissibility}.  The closed side
must match these as continuous CE cochains.  The match is the
coordinate CE/PV identity of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}: on the
coordinate window, Hamiltonian-dual CE coordinates \(c^I\) map to
Schouten polyvector coordinates \(\theta^I\) on
\(\widehat{\mathbf S}(\bar A)\), and shifted-cotangent-dual CE
coordinates \(u_I\) map to boundary Hamiltonian observables \(O_I\)
on the probe pair.  The wrong assignment \(c_f\mapsto O_f\) is a
type error forbidden by Principle~3
(Theorem~\ref{thm:reduced-ce-pv-central-operation}).  The full
seven-layer assembly is \S\ref{ssec:stratified-koszul} below.
...
\begin{rmk}[Named obstruction: strict-product $P_0$ comparison block]\label{rmk:strict-product-p0-obstruction}
  Principle~3 produces the closed local operator algebra
  \(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\), and the
  cochain-level CE/PV identity of
  Theorem~\ref{thm:universal-ce-pv-koszul-criterion} matches its
  generators to Schouten polyvectors of the boundary polynomial
  algebra \(\widehat{\mathbf S}(\bar A)\).  The match is exact only on
  the coordinate-window CE generators; promoting it to a strict product
  \(P_0\)-comparison on the full Hamiltonian disk fails, and the
  failure is named exactly.  The full disk algebra \(\mathfrak h\)
  carries the product/direct-sum Tate pair
  \(\mathfrak h\oplus\mathfrak h^\vee_{\mathrm{cont}}\) of
  Lemma~\ref{lem:tate-casimir-obstruction}: the displayed coefficient
  coevaluation
  \(C_{\mathfrak h}=\sum_d\sum_i e_{d,i}\otimes e^i_d\) is not a
  convergent element of either completed tensor factor; the Casimir
  obstruction \(\mathfrak o_{\mathrm{Cas}}\) is the named block,
  repaired only by the weighted Tate completion
  (Theorem~\ref{thm:wt-cotangent-lift}) or by an explicit nilpotent
  truncation \(\mathfrak m^3/\mathfrak m^{N+1}\).  Equivalently, the
  ...
  (Theorem~\ref{thm:pbw-vs-deformation},
  Corollary~\ref{cor:descendant-coadjoint-difference}) guarantees that
  the polynomial one-\(\psi\) descendants of
  ...
  Three named obstructions therefore close this subsection:
  ...
  module-structure mismatch.  These are repaired in
...
\paragraph{Comparison claim.}
The closed local operator algebra
\(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\) of
\S\ref{sec:grav-ops} and the open brane Hamiltonian descendant algebra
\(\widehat{\mathbf S}(\bar A\oplus\bar A_{\mathrm{desc}}[1])\) of
\S\ref{ssec:open-brane-operators} compare as \(E_1\)-algebras through
Koszul duality (Priddy 1970, \cite{priddy}; Loday--Vallette 2012,
\cite{loday-vallette}): the operation that exchanges commutative
cochains for associative enveloping algebras.  On the strict
formal-disk product/direct-sum coefficient pair, the comparison
fails; on the weighted Tate replacement of
\S\ref{ssec:tate-residual-resolution}, it holds.  The seven-layer
stratification below names the principle inscribed at each layer
and the precise obstruction class at each layer boundary.  These are
the Koszul-spine assembly of the local theorem
(Theorem~\ref{thm:main-local}).

\begin{enumerate}[wide=0pt,leftmargin=*]
  \item \emph{Operadic input (Principle~3, type discipline).}  The
    closed BV side has dg Lie input
    \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\):
    cotangent ghosts measure.  The open Hamiltonian side has
    coCommutative comodule input on
    \(\bar A\oplus\bar A_{\mathrm{desc}}[1]\): probe traces and
    primitive Koszul descendants.  The Lie/coComm operadic pair is
    the operadic precondition of Koszul duality.
    \emph{Named obstruction:}  the wrong type assignment, sending
    Hamiltonian-dual CE coordinate \(c_f\) to the boundary observable
    \(O_f\), is the type error forbidden by Principle~3
    (Theorem~\ref{thm:reduced-ce-pv-central-operation}: the boundary
    observable is the image of \(u\), not of \(c\)).
  \item \emph{Admissible Tate CE/bar-cobar (Principle~5 locality, Tate
    convergence).}  The cochain layer is the completed cotangent
    CE/PV identity of
    ...
    makes the CE coalgebra conilpotent and the filtered inverse limits
    exact.
    \emph{Named obstruction:} the strict product/direct-sum full-disk
    coefficient pair
    \(\bigl(\prod_dH_d\bigr)\oplus\bigl(\bigoplus_dH_d^\vee\bigr)\)
    fails the bar-cobar criterion
    (Lemma~\ref{lem:tate-casimir-obstruction}); its obstruction tuple
  ...
  \item \emph{Tate Quillen equivalence (Beilinson--Drinfeld 2004,
    ...
    \emph{Named obstruction:} the lower-central Tate-pronilpotence
    hypothesis is not automatic on the strict full disk; it is the
    precise input that the weighted completion or nilpotent
    truncation supplies and that the strict
    Beilinson--Feigin--Mazur \cite{beilinson-feigin-mazur} product
    pair fails.
  \item \emph{Coordinate CE/PV reduced \(P_0\) central operation
    (Principle~3, cochain level).}  On the coordinate window the
    CE/PV identity matches Hamiltonian-dual CE coordinates to
    Schouten polyvector coordinates, and shifted-cotangent-dual CE
    coordinates to boundary Hamiltonian functions on the probe pair
    (Theorem~\ref{thm:reduced-ce-pv-central-operation}).
    \emph{Named obstruction:}  this is a cochain-level reduced
    morphism, not a strict factorisation \(P_0\)-centre morphism on
    the unreduced cotangent side; the unreduced lift obstruction is
    Remark~\ref{rmk:unreduced-lift-obstruction}, pinned to
    Problem~\ref{prob:formal-factorization-center}.
  \item \emph{PBW/Rees special-fibre shadow (Principle~4, first
    half).}  The Rees enveloping algebra \(U_\hbar(\mathfrak g)\) has
    PBW special fibre at \(\hbar=0\) isomorphic to
    \(\widehat{\mathbf S}(\mathfrak g)\), supplying the label-level
    identification of the brane Hamiltonian descendants
    \(\Psi_{a,b}\) with the abstract bidegree
    \(\bar A_{\mathrm{desc}}\)
    (Corollary~\ref{cor:cotangent-boundary-pairing}).
    \emph{Named obstruction:} the symmetric PBW action on
    \(\Psi_{a,b}\) is not the coadjoint action on the cotangent
    module \(\mathcal P\)
    (Theorem~\ref{thm:pbw-vs-deformation},
  ...
  \item \emph{Matlis/Grothendieck cotangent module (Principle~4,
    second half).}  The continuous dual
    \(\mathfrak h^\vee_{\mathrm{cont}}\) is the Grothendieck--Matlis
    principal-part module \(\mathcal P\), the residue annihilator of
    the constants quotient (Matlis 1958, \cite{matlis-injective};
    Hartshorne local cohomology, \cite{hartshorne-local-cohomology};
    Theorem~\ref{thm:principal-part-coadjoint-uniqueness}).
    \emph{Named obstruction:} the cotangent module \(\mathcal P\)
    cannot be realised as ordinary polynomial boundary observables
    in the polynomial open BV/Koszul matrix model
  ...
  \item \emph{Brane-line factorisation-current comparison
    (Principles~5, 6, 7).}  Hamiltonians smeared on a brane interval
    lift to factorisation currents whose binary collisions reproduce
    the Hamiltonian Poisson bracket and the residue-coadjoint action
    on \(\mathcal P\)
    ...
    Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}.
    \emph{Named obstructions} (Principle~7, each a finite-window
    obstruction theorem): the all-window strict native
    Bochner--Martinelli transfer is obstructed by
...
corresponding sign.  Since the scalar comoment is zero, the effective
...
\begin{rmk}[Degree-zero quantum Hamiltonian sector]\label{rmk:quantum-hamiltonian-upgrade}
  ...
  Theorem~\ref{thm:finite-n-reduced-moyal}; the reduced-BRST descent
  prerequisite for arbitrary Weyl-ordered products is
  ...
  Corollary~\ref{cor:degree-zero-quantum-upgrade} identifies the stable
  primitive generators with the Weyl/Capelli-renormalized classes
  $\overline{J_N(f)}$.  The \(J_N\)-basis change absorbs the finite-\(N\)
  Capelli contact and its triangular \(\hbar N\) lower-trace terms by
  Lemma~\ref{lem:capelli-renormalized-stable-trace}(iii); the connected
  projection then removes only the empty trace and decomposable products.
  Interior bidegrees reduce to the exact Weyl-trace image clause of
  Statement~\ref{stmt:app-radial-external-input}, equivalently to quantum
  shear primitives
...
\begin{rmk}
  ...
  case of deformation quantization \cite{kontsevich-dq}. Costello's BV/RG
  formalism supplies the general renormalized BV construction under its
  elliptic, gauge-fixing, renormalization, and obstruction-vanishing
  hypotheses. Costello--Li construct
  ...
  Their hypotheses differ from the mixed ordinary-\(\mathfrak{gl}_N\)
  Hamiltonian BF defect problem.  The simply-laced five-dimensional
  holomorphic Chern--Simons/affine-Yangian quantization theorem concerns
  its own holomorphic gauge theory.
\end{rmk}
    ...
    \item after the first normalization, compute the three-propagator boundary
      ...
      -\partial_{z_2}\otimes\partial_{z_1}$;
	    ...
	    \item extend the bulk-to-defect kernel to the cotangent boundary-current
	      ...
	      allowed substitute.
	  \end{enumerate}
	  The computations here settle the formal polynomial Hamiltonian algebra
	  and the formal Moyal coefficients. The analytic graph-realization
	  problem is the sector-identification, local counterterm, brane-defect
	  kernel, principal-part-current compatibility, and first/third
...
\begin{proof}
  ...
  Problem~\ref{prob:quantum-p0-operation-realization}.  These results prove
  the listed branches and no more.
\end{proof}
...
Capelli-renormalized stable trace have an unconditional formal target
and an exact decorated radial obstruction theorem.  The formal target is the
Weyl/Moyal algebra of the constant symplectic formal disk.  The ordinary
Weyl-trace realization is governed by the quantum shear primitive
\(E_{a,b,N}\), equivalently by the free normal-diagram classes
\(\mathfrak o_{a,b}\), and more sharply by the dual-potential class
\(\Omega^{\mathrm{rad}}_{a,b}\) or decorated PBW Stokes identity
\(D^\square_{a,b}=C^+_{a,b}\partial_2\).  This is the radial-parts
algebraic component of the quantum \(P_0\)-operation comparison. It is
not a construction of the descendant sector and it is not the Costello
graph realization.
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  The finite-window theorem after
  Theorem~\ref{thm:componentwise-quantum-coefficient-comparison} is the non-scalar
  Costello graph/QME obstruction criterion.  It is the vanishing problem in the
  ...
  \]
  The degree-zero radial comparison supplies only the Hamiltonian
  generator restriction and is itself pinned to the moment-ideal
  primitive of
  Lemma~\ref{lem:app-quantum-shear-primitive-obstruction}.  The weighted
  Tate theorem supplies the coefficient kernel and graphwise
  boundedness.  The balanced scalar-contact theorem constructs the zero
  filtered scalar projection only on its balanced habitat.  It does not
  prove that the full Costello graph/counterterm habitat lies there,
  construct the bulk-to-defect kernel, solve the closed-exchange branch,
  or kill the reduced non-scalar class.
\end{prob}
...
\begin{prob}[Tate-coefficient lift of the descendant cochain isomorphism]\label{prob:tate-coefficient-descendant-lift}
  ...
  quantum lift on the descendant sector requires the hypothesis of
...
\begin{rmk}
  ...
  Theorem~\ref{thm:phi-hbar-all-order}, under the radial image hypotheses
  of Statement~\ref{stmt:app-radial-external-input} or the equivalent
  quantum shear primitive construction,
  the formal Moyal coefficient algebra, the Capelli renormalization, the
  finite-\(N\) radial-parts identity, and the reduced open-line boundary
  product form a self-contained algebraic construction. Two independent obstruction
  data lie outside that construction: the Tate completion needed for the
  descendant cochain summand, and the analytic Costello specialization of
...
\begin{prob}[Costello first/third graph-normalization obstruction datum]\label{prob:first-third-graph-normalizations}
  ...
  Theorem~\ref{thm:first-third-costello-normalizations} records the implication
	  from these data once the Costello specialization supplies the checks. Items
	  (i)--(viii) are precisely the analytic specialization and normalization
	  data.
\end{prob}
