...
\end{titlingpage}
  ...
  quasi-isomorphism.  The formal-Darboux trace-sector stalk of that
  problem at the Dirac brane vacuum is the local theorem below.  Before
  ...
  \begin{enumerate}[label=\textup{(\arabic*)}]
    ...
    \item The finite-\(N\) Capelli term \(\hbar N[\bar c]\) is the
      ...
      it becomes determinant-line curvature when a determinant or
      modular line with connection pulls its Atiyah class back to
      \([\bar c]\).
  \end{enumerate}

  \medskip\noindent
  \emph{Scope.}  The local source is the two-complex-dimensional
  ...
  A global mixed-HT Deligne equivalence requires the
  ...
  \begin{enumerate}[label=\textup{(A\arabic*)}]
    ...
    \item The global Deligne target is the Morita mixed-HT chiral \(E_2\)-centre
      \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
      (\mathcal C^{\mathrm{op}}_\partial)\) in the admissible completion
      \(\mathcal B^{\mathrm{adm}}\).  The target of
      Theorem~\ref{thm:intro-main-formal-darboux} is only the
      formal-Darboux scalar-reduced stable trace-sector fibre
      \(A_{\partial,H}^{\mathrm{st}}\) and its admissible
      polyvector/Hochschild coordinate algebra.  It is obtained by
      \(U_{\mathrm{sc}}\), \(\operatorname{StTr}\), and the
      Roos/strict-limit convention; the finite-\(N\) trace-relation
      ideal and Chevalley--Eilenberg stabilizer cohomology are the
      kernel and cokernel of the finite-\(N\) restriction.
  ...
  \begin{thm}[Main theorem, introductory form]\label{thm:intro-main-formal-darboux}
    ...
    \(N\) Dirac branes.  The Morita mixed-HT chiral \(E_2\)-centre
    \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}\!
    ...
    The finite-\(N\) boundary value constructed here is the generator assignment
    \[
      ...
      J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)} ,
    \]
    not a Hochschild cochain map by itself.  A finite-\(N\) Hochschild
    cochain map \(\Phi_N^{\mathrm{Hoch}}\) is a conditional centre-lift
    datum: it requires a Maurer--Cartan element
    \(\Theta_{\mathrm{OCA}}+\tau\) and the residual weight-zero
    primitives of Theorem~\ref{thm:omega-localization-collapse}.  Its
    projective scalar anomaly is then the Capelli class
    \(\hbar N[\bar c]\).
    ...
    Deligne problem at \(d=2\), not the all-arity Morita chiral
    \(E_2\)-Deligne theorem.  The all-arity centre lift is governed by
    ...
    the descent, centre-lift, Costello quantum master equation,
    ...
    vertex-algebra reductions and compact Calabi--Yau comparisons are
    matched-conventions problems with separate obstruction classes.
  \end{thm}
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}
    ...
    CE/PV trace-sector coordinate map & proved &
    ...
    \(\mathcal B^{\mathrm{adm}}\) are named parts of the theorem; the
    statement is not the whole finite-\(N\) Hochschild centre. \\
    Hochschild cochain map
    \(\Phi_N^{\mathrm{Hoch}}\) & conditional &
    ...
    Capelli class \(\hbar N[\bar c]\) & proved &
    ...
    Curve vertex-algebra reduction & conditional &
  ...
  \medskip\noindent
  \emph{Logical core.}  The proof begins with the finite cotangent
  CE/PV calculation, passes to
  \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\) only after the
  Grothendieck--Matlis residue dual and completed tensor products have
  been fixed, and then applies stable trace theory to the derived
  commuting stack.  The six structural equations are
  \[
  ...
  \[
    ...
    K\ \text{acts by}\ N,\qquad
    [\omega]=\hbar N[\bar c].
  \]
  The finite-dimensional model is
  Theorem~\ref{thm:coordinate-free-cotangent-ce-pv}; the formal
  completion is Definition~\ref{defn:admissible-host-category};
  stable trace selection is Definition~\ref{defn:trace-sector-reduction};
  uniqueness of \(J\) is Theorem~\ref{thm:Jf-coordinate-uniqueness};
  the coordinate theorem is Theorem~\ref{thm:main-local}; the Capelli
  anomaly is Theorem~\ref{thm:capelli-projective-anomaly}.  A
  finite-\(N\) example and the finite-\(N\) leftover classes are
  \S\ref{ssec:finite-N-leftovers}.  The sign conventions used in the
  CE/PV and BV computations are Appendix~\ref{app:sign-conventions}.

  \medskip\noindent
  \emph{Obstruction atlas.}  The formal-Darboux theorem gives the
  trace-sector fibre.  A full mixed-HT Deligne equivalence would require
  the following additional constructed data.
  \begin{center}
  \footnotesize
  \renewcommand{\arraystretch}{1.18}
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}
    ...
    \textbf{Datum} & \textbf{Native complex} & \textbf{Required
    vanishing or construction} \\
    Morita chiral \(E_2\)-centre &
    \(\mathfrak{Def}^{\mathrm{HT}}_{\mathrm{Mor}\mbox{-}E_2}
    (\mathcal C^{\mathrm{op}}_\partial,b_p)\) &
    Maurer--Cartan lift extending \(K^{(2)}_\Delta\) to all chiral
    \(E_2\) arities; obstruction
    \(\operatorname{Ob}^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}\). \\
    Open--closed descent &
    Costello--Gwilliam \v{C}ech--Weiss--Ran locality complex on
    \(\partial_{\mathrm{link}}X\) &
    Typed map \(\mathrm{OCA}_N\) with formal fibre
    Theorem~\ref{thm:main-local}; vanishing of
    \(\operatorname{Ob}^{\mathrm{desc}}_{\partial}\). \\
    Quantum master equation &
    Scale-\(L\) local-functional complex with
    \(d_{\mathrm{QME}}^{L}=Q+\{I[L],-\}_{L}+\hbar\Delta_L\) &
    Gauge fixing, heat kernel, finite-scale propagator, BV Laplacian,
    weighted summability, and vanishing of the Milnor/Roos obstruction
    vectors \(\mathfrak D^{\mathrm{ns}}_n\) of
    Theorem~\ref{thm:completion-milnor-obstruction-vector}; only then
    is a compatible formal counterterm tower in
    \(\mathcal B^{\mathrm{adm}}\) obtained. \\
    Anomaly and projectivity &
    Swiss-cheese convolution complex
    \(\mathrm{Conv}_{\mathrm{SC}}(\mathcal A_{\mathrm{bulk}},
    Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
    (\mathcal C^{\mathrm{op}}_\partial))\) &
    Null-homotopy whose scalar trace-sector projection is
    \(\hbar N[\bar c]\); determinant or modular curvature requires a
    line with connection. \\
    Cone equivalence &
    \(\operatorname{Cone}(\mathrm{OCA}_N)\) in
    \(\mathcal B^{\mathrm{adm}}\) &
    Contracting homotopy and, for an equivalence of mixed-HT
    Swiss-cheese algebras, an operadic homotopy inverse. \\
    Curve and compact comparisons &
    Matched-conventions complexes for curve restriction, compact BCOV,
    Hall--Drinfeld--Borcherds, and Igusa normalization &
    Transverse modes, pushed-forward bracket, BV pairing, brane image,
    anomaly matching, compact-target trace, and period conventions.
  \end{tabular}
  \end{center}

  \medskip\noindent
  \emph{Minimal reading path.}  The core verification path is
  \S\ref{sec:setup},
  \S\ref{sec:bf-lie-algebra},
  \S\ref{sec:derived-commuting-variety},
  \S\ref{sec:trace-map},
  \S\ref{sec:ce-pv-koszul},
  Theorem~\ref{thm:main-local},
  \S\ref{sec:capelli-scalar},
  Appendix~\ref{app:sign-conventions}, and
  Appendix~\ref{app:completion-formal-local-theorem}.  The compact
  comparison material in \S\ref{sec:compact-comparison-obstructions}
  is a separate obstruction analysis.
...
A curve vertex operator algebra is a
...
derived-centre comparison is governed by the obstruction object
\(\mathfrak{Ob}_{\mathrm{Del}}^{\mathrm{glob}}\) and by the typed
...
moment-map ideal of conjugation on
	...
	(Theorem~\ref{thm:capelli-projective-anomaly}).  The
...
Lie anomaly class \(\hbar N[\bar c]\) of
Theorem~\ref{thm:capelli-projective-anomaly}.
...
The B-model topological string has a Costello holomorphic--topological
local sector on
\(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\).  At the Dirac brane
vacuum \(b_p\) with Chan--Paton space \(\C^N\), the transverse open
chart is the derived commuting stack
\([\mu^{-1}_{\mathrm{der}}(0)/\mathrm{GL}_N]\), with
\(\mu(\phi_1,\phi_2)=[\phi_1,\phi_2]\), and a reduced Hamiltonian
\(f\in\C[[z_1,z_2]]/\C\cdot1\) has boundary trace
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)} .
\]
The theorem proved below identifies the scalar-reduced stable
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  one-dimensional vertex operator algebra statement requires a
  restriction theorem choosing a complex
  line or holomorphic defect, a transverse boundary condition or
  restriction functor, and the finite Laurent singular expansion on that
  restricted object.
\end{defn}
...
\begin{thm}[Trace-sector generator boundary value and CE/PV reduction]\label{thm:closed-to-open-derived-centre-map}
  There is a finite-\(N\) generator-level trace-sector boundary value,
  landing in Hochschild cochains only on Chevalley--Eilenberg generators,
  \[
    \Phi_N^{\mathrm{gen}}\colon
  ...
  \[
    Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})
     ...
     (A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N}).
  \]
  The generator boundary value is
  \[
  ...
  brane trace algebra.  The displayed target records the Hochschild
...
\begin{proof}
  ...
  The displayed generator formulas define the Hochschild-cochain
  boundary values on generators.
  The
...
finite-window Hamiltonian quotient transfer and forcing the
...
one-dimensional vertex algebra is present only after a
...
stable-range operation, not a physical large-\(N\) state or cumulant
limit.  It is accessed only after four hypotheses.  The
...
non-cyclicity trace relations beyond the degree-\(d\) window when
\(N\geq d\) \cite{procesi}\cite{razmyslov}.  Thus stable invariants of
two matrices are freely generated by cyclic trace words in each fixed
degree.  In the connected sector the Koszul
differential
...
\cite{loday-quillen}\cite{tsygan}\cite{loday-vallette}) identifies the
...
\begin{proof}
  Procesi--Razmyslov invariant theory \cite{procesi}\cite{razmyslov}
  gives two facts.  First, the first fundamental theorem for
  simultaneous conjugation says that
  $\C[\lie{gl}_N^2]^{GL_N}$ is generated by trace functions of words in
  the two generic matrices.  Second, the second fundamental trace
  theorem says that all polynomial relations among these trace
  functions are generated by the fundamental trace identities of matrix
  size \(N\).  After polarization in characteristic zero, no
  non-cyclicity relation occurs in total word degree \(d\leq N\).  Fix
  a total word-degree \(d\). For every \(N\geq d\) the degree-\(d\)
  relations are exactly cyclicity of the trace.
...
\begin{lem}[Koszul homotopy for adjacent transpositions]\label{lem:koszul-adjacent-transpositions}
  ...
  \(\phi_1,\phi_2\), the adjacent-swap difference
  \[
    \mathrm{Tr}(u\phi_1\phi_2v)-\mathrm{Tr}(u\phi_2\phi_1v)
  \]
  is the \(Q\)-boundary of the invariant degree-\(-1\) chain
  \[
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &=Q\,\mathrm{Tr}(\psi vu)\\
    &=\mathrm{Tr}((Q\psi)vu)\\
    &=\mathrm{Tr}((\phi_1\phi_2-\phi_2\phi_1)vu)\\
    &=\mathrm{Tr}(u\phi_1\phi_2v)
      -\mathrm{Tr}(u\phi_2\phi_1v).
  \end{align*}
  The trace is \(GL_N\)-invariant, so \(H_{u,v}\) lies in
  \(\mathcal R_N^{GL_N}\).  Quotienting the empty trace preserves
  exactness because the displayed boundary has positive word degree.
\end{proof}
...
\[
  ...
  -\operatorname{Tr}(u\phi_2\phi_1v),
\]
so \(J(f)\) depends only on the symbol \(f\).
...
\eqref{eq:open-action-derived}; its ghost-zero invariant ring is
  ...
  finite-window rational invariant Koszul cohomology in CE length zero;
  by Lemma~\ref{lem:formal-stalk-trace} the finite-window connected
  \(GL_N\)-invariants stable in \(N\) are exactly the trace family
...
finite-\(N\) data not seen by the trace-sector map.  The
...
\begin{thm}[Formal-Darboux trace-sector coordinate theorem at \(N\) Dirac branes]\label{thm:main-local}
  ...
  computation is governed by the trace-identity ideal, the stack-level
  ...
  \]
  and this map is the CE/PV coordinate isomorphism exactly after
  replacement of the finite-\(N\) algebra by the stable scalar-reduced
  trace algebra above.
...
\begin{proof}
  ...
  descent problem encoded by the obstruction vector named in the
  theorem.
\end{proof}
...
\begin{rmk}[Formal representatives and the global obstruction]
  ...
  class.  They are the formal current and trace-sector boundary
	  values whose extension, gluing, renormalisation, anomaly
		  ...
		  encoded by \(\operatorname{Ob}^{\mathrm{global},5}\) and
...
\begin{thm}[Reduced Moyal commutator: radial hypothesis and obstruction criterion]\label{thm:finite-n-reduced-moyal}
  ...
  Here the limiting object is the renormalized associated-graded
...
\begin{prob}[Hamiltonian specialization datum for Costello perturbative BV]\label{prob:analytic-graph-realization}
  ...
  whose restriction is this Hamiltonian BF sector and whose obstruction
...
\begin{rmk}
  ...
  realization of this coefficient still requires an elliptic parent or dimensional
    ...
    \item identify the connected three-propagator brane-defect ribbon graphs
      whose tensor contraction is $P^3(f,g)$;
	    ...
	    \item check that connected stable-trace extraction preserves the
	      ...
	      physical large-\(N\) cumulant extraction would additionally require
	      the state datum of
...
\begin{thm}[Capelli projective anomaly and determinant-line condition]
...
\begin{proof}
  ...
  by \S\ref{ssec:reduced-ce-pv-trace-map};
...
inverse limit additionally requires the analytic-retraction and
...
\[
  ...
  +\operatorname{Tr}(X)^3=0.
\]
Equivalently, in the finite window \(W=3\) the stable power-sum
generator \(p_{3,0}\) maps to
\(J_2(z_1^3)=\operatorname{Tr}(X^3)\), but its image in the
indecomposable quotient of the rank-two trace algebra is zero because
the right hand side is a polynomial in lower traces.  The finite-rank
class \(J_2(z_1^3)\) exists as a trace function; it is not a free
stable trace generator.  This is the finite-\(N\) leftover excluded
...
\[
  ...
  =-4096\,\Phi_{10}^{-1}.
\]
These are compact-target convention equations.  The formal-Darboux
