...
<<<<<<< HEAD
measurable boundary observables.  Lemma~\ref{lem:formal-stalk-trace}
gives the trace substitution
=======
boundary observables the chart \(A_b\) reads.
Lemma~\ref{lem:probe-trace-microscope} gives the trace map
>>>>>>> eab5f47 (rectify(chapter-4): trace map (not measurement); chart vs primitive; canonical J form; globalization disclaimer)
\[
