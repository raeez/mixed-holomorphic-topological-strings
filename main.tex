...
Fix the chosen Dirac brane vacuum
\(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) at \(p=0\in\C^2\)
(Definition~\ref{def:formal-darboux-stalk-chart}), with chart algebra
\(A_{b_p}=\End_{\mathcal{C}^{\mathrm{op}}_\partial}(b_p)\cong\C[[z_1,z_2]]\)
on the brane line
\(L=\R_t\times\{z_1=z_2=0\}\) of
\S\ref{sec:derived-commuting-variety}.  The closed BV theory carries
two CE coordinate types and the chart carries only one of them:
Hamiltonian ghosts \(c^I\)---dual to generators
...
boundary observables.  The trace map of
Lemma~\ref{lem:formal-stalk-trace} is
\[
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\]
the stable scalar-reduced cyclic-trace class of the unreduced
\(J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\).  This is the formal
Darboux coordinate expression at \(b_p\) of the brane-side derived
\(E_1\)-centre
\(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})
\simeq HH^\bullet(A^{\mathrm{cl}}_{\partial,N})\) (Deligne's
\(E_1\)-Hochschild theorem), which the Mixed Holomorphic--Topological
Deligne conjecture identifies at the formal-Darboux stalk with the
chiral derived centre \(C^\bullet_{\mathrm{ch}}(A_{b_p},A_{b_p})\) at
\(d=2\) (\S\ref{ssec:binding-convention-layer}).  \(J\) is the unique \(\C\)-linear, \(GL_N\)-equivariant,
Darboux-natural primitive single-trace map
\(\mathfrak h\to\mathcal O(\mu^{-1}_{\mathrm{der}}(0))^{GL_N}\)
satisfying scalar reduction, Dirac descent, and \(J(z_i)=\mathrm{Tr}\,\phi_i\)
(Theorem~\ref{thm:formal-local-universal-property}).  Centrality is not
a separate condition: it is the moment-map identity
\(\{J(f),J(g)\}_\partial=J(\{f,g\}_{\mathfrak h})\) on the chart
\(A_{b_p}\).  Closed Hamiltonians act through \(c_f\); \(J\)
records \(u_f\); below, the symbols \(c_f,u_f,\theta_f,O_f\) refer
back to Lemma~\ref{lem:type-discipline}.
