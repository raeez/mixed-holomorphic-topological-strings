...
\subsection{The Dirac brane formal-stalk chart in one calculation}\label{ssec:brane-microscope-one-calculation}\label{ssec:formal-darboux-stalk-one-calculation}
...
The primitive open object is the boundary category
...
\((X,D,\tau)\).  At \(N\) coincident Dirac branes on the brane line
\(L=\R_t\times\{z_1=z_2=0\}\), the chart at the chosen brane vacuum
\(b\) is the matrix interaction algebra
\(A_b=\mathrm{End}(b)\); this manuscript computes the formal-Darboux
stalk of \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) through the chart, not the
primitive object \(\mathcal{C}^{\mathrm{op}}_\partial\) itself.  The
closed BV theory has two coordinate types, and the chart \(A_b\) reads
only one of them.  Hamiltonian ghosts \(c^I\)---dual to generators
...
boundary observables the chart \(A_b\) reads.
...
quantum master equation (QME) certificate against an anomaly polynomial,
...
supplied.  The Dirac probe selects the open BV complex as the derived
...
A stack of \(N\) Dirac branes carries a finite-matrix trace measurement of the
...
disk. \emph{Status.} Proved (the local theorem
...
\cite{beilinson-drinfeld-chiral}. \emph{Status.}
...
algebra of
\cite{costello-gwilliam}*{Vol.~II~\S 5.4}. \emph{Status.}
...
\(\mathrm{AdS}_3\). \emph{Status.} Identification of the formal
...
\(N\to\infty\) limit. \emph{Status.} The \(W_N\)-chart
...
of the Hilbert scheme \(\Hilb^N(\C^2)\). \emph{Status.} The
...
of the modular line on \(\overline{\mathcal A}_g\). \emph{Status.}
...
\(\Phi_{10}\), denominator
\(\Delta_5\)). \emph{Status.} Pointwise formal-Darboux stalk
...
the bulk-to-defect kernel. Under joint vanishing of the
matched-conventions cross-volume obstruction
\(\mathfrak o_{\mathrm{cv}}\) of \S\ref{ssec:tate-residual-resolution}
the local stalk transfers to a candidate global identification.
\emph{Status.} The formal-Darboux stalk is proved
pointwise (\S\ref{ssec:local-theorem}); the global
identification is a candidate, conditional on
descent + QME + anomaly + locality joint vanishing.
