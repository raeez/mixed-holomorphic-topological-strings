...
\end{titlingpage}
  ...
  \[
    ...
    f\in\C[[z_1,z_2]] .
  \]
  The brane does not carry an abstract representation of the
  Hamiltonian algebra: it evaluates Hamiltonians on its matrix-valued
  transverse coordinate.  Among polynomial \(GL_N\)-invariant
  Darboux-natural scalar-reduced maps agreeing with the moment-map
  action on linear generators, this evaluation is the unique
  measurement
  (Theorem~\ref{thm:formal-local-universal-property}).  The hypotheses
  ...
  minimal RG-stable analytic completion in which the heat-kernel
  propagator extends and the Casimir converges is the weighted-Tate
  completion \(B_w\), generated by \(\{O_f,\theta_f\}_{f\in\bar A}\)
  under the Schouten bracket and pointwise product
  ...
  finite-certificate range certified unconditionally by the
  decorated-PBW Stokes script
  (\texttt{quantum\_shear\_trace\_diagram\_normal\_form.py
  --kernel-correction}) covers the balanced bidegree diagonal through
  \((6,6)\), every non-edge bidegree of total degree at most \(13\),
  the total-degree-\(14\) bidegrees
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\), the explicit
  total-degree-\(15\) bidegrees
  \((6,7),(7,6),(5,8),(8,5),(6,8),(8,6),(7,7),(7,8),(8,7)\), and the
  complete one-moving-letter edge strips.  The radial bicomplex is
  ...
  \medskip\noindent
  Each upgrade beyond the proved core is a vanishing of a named
  obstruction class.  The unreduced \(P_0\)-factorization-centre lift
  ...
  (Theorem~\ref{thm:formal-local-universal-property}) &
  proved &
  characterisation by polynomial \(GL_N\)-invariance, Darboux
  naturality, scalar reduction, and agreement with the moment-map
  ...
  (Theorem~\ref{thm:wt-canonical-habitat}) &
  proved &
  \(B_w\) is the minimal RG-stable analytic completion under Schouten
  bracket and pointwise product in which the heat-kernel propagator
  extends and the Casimir
  \(C_{\mathfrak h,w}\) converges\\
  \hline
  ...
  range certified: balanced bidegree diagonal through \((6,6)\); every
  non-edge bidegree of total degree
  \(\le13\); total-degree-\(14\) bidegrees
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\); every one-moving-letter
  edge strip\\
  \hline
...
\[
  ...
  \bar A=\C[z_1,z_2]/\C\cdot1.
\]
The BV antifield \(\psi\in\mathfrak{gl}_N[1]\) with
\(Q\psi=[\phi_1,\phi_2]\) gives the Koszul derived zero-fibre model
for the moment-map equation
(\S\ref{ssec:open-mixed-bft}); the closed shifted-cotangent extension
\(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
with \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\) supplies the canonical
closed sector
(\S\ref{ssec:closed-mixed-hamiltonian}); the CE/PV cochain dictionary
\(c_f\mapsto\theta_f,\;u_f\mapsto O_f\) compares the two
(Theorem~\ref{thm:reduced-ce-pv-central-operation}); and the
continuous dual is the Matlis principal-part module
\(\mathfrak h^\vee_{\mathrm{cont}}\cong\mathcal P\)
(Theorem~\ref{thm:principal-part-coadjoint-uniqueness}).

\begin{exa}[Classical point probe, \(N=1\)]\label{exa:point-probe-N1}
...
\begin{exa}[Linear Hamiltonians and the scalar anomaly]\label{exa:linear-hamiltonians-anomaly}
  For \(f=z_1\), \(g=z_2\), the unreduced brane Poisson bracket is
  \[
    \{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N,
  \]
  the central extension of \(\bar A\) by \(\bar c\) on linear
  generators.  Moyal quantization promotes this to
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\); the first
  scalar-reduced \(\mathfrak{gl}_N\) QME obstruction is
  \(W_1^{\mathrm{sc,ord}}=\hbar N[\bar c]\)
  (Theorem~\ref{thm:app-scalar-anomaly-transgression}).
\end{exa}

\begin{exa}[A single Matlis residue]\label{exa:single-matlis-residue}
  ...
  \(\mathcal P\) without local nilpotency.  The polynomial PBW class
  \(\widetilde\Psi_{a,b}\) of
  Corollary~\ref{cor:descendant-coadjoint-difference} carries the
  same label and a strictly different \(\mathfrak h\)-module structure;
  no relabelling interpolates between the two.
\end{exa}
...
\begin{rmk}[The microscope obstruction]\label{rmk:microscope-obstruction}
  ...
  \(\{J(f),J(g)\}=J(\{f,g\})\) of
  Theorem~\ref{thm:witten-centrality-local-form} below.  The strengthening to
  ...
  cotangent currents \(\Theta_\rho\) (the boundary current attached to a
  residue \(\rho\in\mathfrak h^\vee_{\mathrm{cont}}\) by smearing, defined
  in Theorem~\ref{thm:reduced-principal-part-boundary-current} below).  The
  ...
  Theorem~\ref{thm:bmk-finite-window-no-go} below).
  The matrix microscope is the scalar shadow of
  \(\Phi_{\mathrm{cl}}\); the cotangent shadow is residual and named
  by the obstruction classes above.
\end{rmk}
...
the polynomial PBW special-fibre descendants \(\Psi_{k,l}\) of
...
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.  When prose below
refers to ``the CE/PV comparison'' or ``the CE/PV ladder'', the rung
...
The brane does not represent the Hamiltonian algebra abstractly: it
literally evaluates Hamiltonians on its matrix-valued transverse
coordinate. A bulk Hamiltonian
\(f\in\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) is read by the
brane through the substitution map
...
Corollary~\ref{cor:matrix-evaluation-well-defined}). Third, formal
Darboux naturality together with linear normalization on a single
Hamiltonian generator forces \(J\) on linear inputs and extends it
\(\widehat{\mathfrak h}\)-multiplicatively to all Taylor Hamiltonians.
...
The remainder of \S2 unfolds the algebraic content of \(J\).
...
\begin{proof}
  ...
  together with adjacent-swap exactness (Lemma~\ref{lem:adjacent-swaps})
  collapses every cyclic word to a commutative monomial trace
  \(\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\)
  (Corollary~\ref{cor:matrix-evaluation-well-defined}); the ghost-zero
  invariant ring computed by the Koszul derived zero-fibre model and
  Procesi--Razmyslov is \(\mathbf S(\bar A)\) on the connected
  single-trace generators.

  \emph{Scalar reduction (U3).}  Scalar reduction kills the multi-trace
  decomposable summands at the connected indecomposable quotient
  \(\mathfrak m/\mathfrak m^2\) of
  Proposition~\ref{prop:brane-ops}; \(\widetilde J\) is determined by
  its values on the indecomposable primitives.

  \emph{Darboux naturality (U4).}  The action of formal Darboux
  symplectomorphisms intertwines \(\widetilde J\) with substitution by
  \(\Phi^*\): equality on linear generators propagates to the orbit of
  the linear generators under canonical transformations, exhausting the
  polynomial algebra \(\bar A\) by formal Darboux density.  Continuity
  of \(\widetilde J\) in the \(\widehat{\mathfrak h}\)-topology
  extends from \(\bar A\) to all of \(\widehat{\mathfrak h}\).

  \emph{Linear normalization (U5).}  The values
  \(\widetilde J(z_1)=\overline{\operatorname{Tr}\phi_1}\),
  \(\widetilde J(z_2)=\overline{\operatorname{Tr}\phi_2}\) fix the
  scalar on each degree-one primitive.  Combined with (U1)--(U4),
  \(\widetilde J(z_1^az_2^b)=
   \overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\) at every bidegree;
  hence \(\widetilde J=J\).
\end{proof}
...
\begin{rmk}[Reproducibility certificate for the finite Moyal/radial verification]\label{rmk:reproducibility-certificate}
  The finite-certificate clause of
  Theorem~\ref{thm:componentwise-quantum-coefficient-comparison} is a
  computer-assisted verification at the four enumerated bidegree
  ranges.  The protocol used is:
  \begin{enumerate}[label=(\arabic*)]
    \item \emph{Arithmetic.}  Exact rational arithmetic over
      \(\mathbb Q\); no floating-point step.  Implementation:
      Python~3 with the \texttt{fractions} module and
      \texttt{numpy.array(dtype=object)} of \texttt{Fraction} entries.
    \item \emph{Constructed matrices.}  At each bidegree \((a,b)\) the
      verification builds the radial Stokes boundary
      \(B_{a,b}\colon\mathsf S_{a,b}\!\oplus\!\mathsf W_{a,b}\to
      \mathsf T_{a,b}\) with the decorated weight matrix
      \(C^+_{a,b}\) on the \(\mathsf W_{a,b}\) summand of
      Proposition~\ref{prop:app-radial-dual-potential-obstruction};
      the rationals \(\partial_2\)-Stokes coefficient table; and the
      principal-part edge tower of
      Proposition~\ref{prop:app-edge-pbw-telescoping}.
    \item \emph{Linear-algebra outputs.}  \(\ker B_{a,b}^*\) and
      \(\operatorname{coker}B_{a,b}\) are represented as exact
      Smith-normal-form bases over \(\mathbb Q\); the obstruction
      class
      \(\Omega^{\mathrm{rad}}_{a,b}=
        [(T_{a,b},E^+_{a,b})]\in\operatorname{coker}B_{a,b}\) is
      reported as a specific basis vector or as the zero class.
    \item \emph{Verified bidegree list.}  Balanced diagonal through
      \((6,6)\); every non-edge bidegree of total degree \(\le13\);
      total-degree-\(14\) bidegrees
      \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\); every
      one-moving-letter edge strip.
    \item \emph{Reproduction.}  The harness lives at
      \texttt{scripts/check\_moyal\_coefficients.py} and
      \texttt{scripts/check\_one\_psi\_homology.py}; running each
      from the repository root prints the per-bidegree verdict and
      writes a SHA-256 hash of the rational coefficient table to
      \texttt{out/finite\_certificate.sha256}.  A divergence between
      the stored hash and the recomputed hash indicates either a
      transcription drift or a coefficient regression and is reported
      as a verification failure.
  \end{enumerate}
  Items (1)--(5) constitute the certificate.  No computational claim
  outside the bidegree list of (4) is asserted; the all-bidegree
  obstruction \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\)
  is the named cohomology class blocking extension to the full
  bidegree lattice
  (Proposition~\ref{prop:app-radial-dual-potential-obstruction}).
\end{rmk}
