...
\end{titlingpage}
  ...
  Beilinson--Drinfeld algebraic-geometric framework
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is the factorizable
  $\mathcal D$-module on $\mathrm{Ran}(\C^2)$ valued in $E_2$-algebras
  (the holomorphic-2d chiral algebra), related to the analytic
  presentation above by the de Rham complex (Riemann--Hilbert);
  restriction along a holomorphic $C\hookrightarrow\C^2$ is a curve
  vertex algebra (Remark~\ref{rmk:not-a-curve-voa}).
...
brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\), proved in the section
