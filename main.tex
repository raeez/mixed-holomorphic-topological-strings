...
\end{titlingpage}
  ...
  on \(\widehat{\C^2}_0\), \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1\); and
  the universal brane probe is the trace functional
  ...
  obstruction theory of the closed-to-open coupling \(\Phi_N\) of
  ...
  \(\operatorname{HC}_0(\End V)\cong\C\)
  ...
  the brane as $O_f=J(f)$.  $\Phi_N$ acts on these generators by
  ...
  \medskip\noindent
  \emph{Obstruction calculus.}  Globalization, quantization,
  all-bidegree Weyl/Moyal compatibility, and chain-level upgrades are
  filtered \(L_\infty\) transports of \(\Theta_{\mathrm{loc}}\) into
  a target deformation complex \(\mathfrak K_{\mathcal T}\), with
  master equation
  ...
  \ref{thm:app-radial-bidegree-rational-reduction}); the chain-level
  ...
  classes live in the filtered \(L_\infty\) algebra
  \(\mathfrak K_{\mathrm{loc}}\) controlling deformations of
  \(\Phi_N\).

  \medskip\noindent
  The local model is the holomorphic-2-dimensional analog of the
  Heisenberg chiral algebra of Beilinson--Drinfeld, with
  \(\mathcal P\) in the role of the Kac--Moody central pairing
  module \(\Omega_K\) for a curve \(K\) and \(\mathfrak h\) in the
  role of regular functions; the formal-symplectic local fixture of
  the Costello twisted M-theory family \cite{costello-twistedM}; and,
  on its cyclic framed stable branch, the universal local probe of
  \(\Hilb^N(\C^2)\) in its noncommutative Moyal deformation.  Global
  compact-target lifts---BCOV holomorphic anomaly, Gopakumar--Vafa,
  Donaldson--Thomas---transport \(\Theta_{\mathrm{loc}}\) into the
  corresponding compact-target deformation complex, with the
  obstruction classes above as the formal-Darboux input.
...
\subsection{The brane probe in one calculation}\label{ssec:brane-microscope-one-calculation}
...
polynomial layer is the special fibre of the formal layer at
\(R=\C\).
...
gauge-invariant content of the probe pair is the trace family
\(\{J(f)\}\), and the closed sector
\(\bar A=\C[z_1,z_2]/\C\cdot1\) acts on the brane through nothing
else.

\begin{lem}[Trace realisation of the formal polydisk]\label{lem:probe-trace-microscope}
...
\begin{rmk}[Cotangent obstruction beyond \(J\)]\label{rmk:microscope-obstruction}
  ...
  below).  Substitution into the probe pair encodes the symbol-level
  information of \(f\), not the cotangent-direction antifield content
  of the closed BV algebra.  The obstruction is the unreduced
  ...
  shadow of \(\Phi_{\mathrm{cl}}\); the cotangent shadow is residual,
  classified by these obstruction classes.
\end{rmk}
...
the brane.  Closed Hamiltonians act through their \(c\)-avatar; brane
measurements record their \(u\)-avatar; below, the symbols
\(c_f,u_f,\theta_f,O_f\) refer back to the type-discipline lemma.

The CE/PV comparison is graded into five rungs at separate
...
Formal completion at \(L\) is forced by the trace substitution:
...
from the polynomial PBW special-fibre shadow.  Together with the
...
\begin{thm}[Closed-to-open derived-centre map and CE/PV reduction]\label{thm:closed-to-open-derived-centre-map}
  ...
  the reduced-generator shadow of $\Phi_N$; $\Phi_N$ is in turn the
  reduced-generator shadow of the chain-level $P_0$-centre map
...
\begin{proof}
  ...
  $[\theta_f,\theta_g]=\theta_{\{f,g\}_{\bar A}}$ and
  $\theta_f(J_g)=J_{\{f,g\}_{\bar A}}$ on the right by
...
its scalar shadow is two complex coordinates conjugate under one
...
\begin{prop}[Geometry forced by the brane probe]\label{prop:local-model-mixed-definition}
  ...
  observable family is forced to
...
\subsubsection*{Koszul reduction of the brane probe}

The Dirac brane stack carries the polynomial probe ring
\[
  \mc R^{\mathrm{poly}}_N=
...
\begin{prop}[Derived zero-fibre and Procesi--Razmyslov trace ring]\label{prop:open-zero-fibre-traces}
  The brane-probe ghost-zero open observable algebra is the
...
\begin{rmk}[Koszul duality and derived normal coordinates]\label{rmk:open-koszul-duality}
  ...
  brane-probe ring and the skyscraper Ext algebra are the same
...
ghost number, encoding the BV antifields and ghosts of the brane
probe's gauge symmetry.

\subsection{Open mixed brane field theory}\label{ssec:open-mixed-bft}

Under the brane-probe hypotheses of \S\ref{ssec:local-setup} ---
...
\subsubsection*{Action: brane-probe Liouville plus Dirac multiplier}

The brane-probe symplectic form
...
\begin{prop}[Open BV action from the brane probe]\label{prop:open-bv-action-derived}
...
\begin{rmk}[Skyscraper--matrix duality]
  ...
  manifestation of Koszul duality between the brane-probe polynomial
...
Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction} is
necessary and sufficient: joint vanishing with chosen null-homotopies
constructs the morphism, and a nonzero entry is the precise
construction defect.  The chain-level primitive shadow of
