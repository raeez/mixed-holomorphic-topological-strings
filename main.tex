...
\subsection*{Summary of results: proved, conditional, open}
\label{ssec:proof-boundary-table}
\addcontentsline{toc}{subsection}{Summary of results: proved, conditional, open}
...
of Theorem~\ref{thm:main-global-conditional}.  By the cross-target
firewall (Theorem~\ref{thm:cross-target-firewall}), none of the
following frontier programmes is a consequence: the chiral \(E_d\)
Deligne conjecture at \(d\geq 2\), the Hall--Drinfeld--Borcherds
comparison spectral sequence on compact non-toric CY\(_3\), the
operator-level lift of the modular line on \(K3\times E\), and the
\(W_\infty[\lambda]/E_\infty\) admissible endpoint identification.  We
state the three load-bearing conjectures here in their canonical form;
the chiral $E_2$-Deligne conjecture supplies
\(\operatorname{Ob}^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}\), the
Hall--Drinfeld--Borcherds spectral sequence supplies the
\(d_2\)-differential of
\(\operatorname{Ob}^{\mathrm{desc}}_{\partial}\), and the operator
lift of \(\Phi_{10}\) supplies the additional class
\(\operatorname{Ob}^V_{\mathrm{mod}}\) on the modular line.
...
\cite{beilinson-drinfeld-chiral} in the factorization-algebra
...
not in the literature; nor is the corresponding chiral
\(E_d\)-Hochschild cochain complex with its \(E_{d+1}\) structure.
The formal-Darboux stalk theorem (Theorem~\ref{thm:main-local}),
together with Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope},
supplies the arity-two \(d=2\) formal-coordinate operation as the
diagonal kernel
\(K^{(2)}_\Delta\,\mathfrak g^{\otimes 2}\to\mathfrak g\); the
all-arity extension is the content of Conjecture~\ref{conj:chiral-Ed-deligne}.
...
\end{conj}
On $X=K3\times E$ the $E_2$-page reads $(0,0,3,5,24)$, with the
Borcherds--Kac--Moody weight $\kappa_{\mathrm{BKM}}(\Phi_{10})=5$
matching the level-$2$ Gritsenko--Nikulin paramodular form
\cite{gritsenko-nikulin-paramodular}, and convergence to
$\kappa(K3\times E)$ is the conjectural content.  Compact-target
evidence on the $S^3$ framing, quintic DT/PT data, and Gopakumar--Vafa
closure is the subject of the companion volume
...
governing the paramodular product
\cite{gritsenko-nikulin-paramodular}.  The
formal-Darboux stalk supplies only the local fibre of such a
construction; the cross-target firewall (Theorem~\ref{thm:cross-target-firewall})
records the matched-conventions class
\(\operatorname{ob}_{\mathrm{Igusa/BKM}}\) which must vanish for the
lift to descend.  Cross-target reference: the Igusa cusp form volume.
