...
bulk is the derived chiral centre.  For a chiral algebra $A$ on a smooth curve $C$ the $d=1$ chiral
derived centre is
...
principal-part module rather than another copy of Taylor polynomials.  The natural pairing
...
pattern of Theorem~\ref{thm:main-local}; global \(HC_{*-1}\)
convention transfer requires the acceptance datum.
...
\begin{proof}
  ...
  extension or kernel datum can enter: the failure is
...
projected brackets fail Jacobi after truncation, and the diagonal cotangent Casimir
...
is the Grothendieck--Matlis principal-part \(\mathcal P\), a
principal-part module rather than a polynomial substitute, and the
...
topologised subalgebra in which the Schouten bracket converges; the
  strict ambient product completion fails this and obstructs the
...
operator-algebra identifications require the datum.  The unreduced
...
\ref{thm:bmk-finite-window-obstruction}
...
\begin{rmk}[Worked instances]\label{rmk:Jf-uniqueness-running-examples}
  ...
  obstruction works monomial-by-monomial rather than by an algebra-homomorphism
  extension.
\end{rmk}
...
\begin{ex}[Low-degree monomial verification]\label{ex:low-degree-phi}
  ...
  \begin{itemize}
    \item \emph{Degree-1 generators.} Take $H_{1,0}=z_1$,
      ...
      the generator \(u_{0,0}\) is absent, and the
...
\begin{proof}
  ...
  absent.  Jacobi with two Hamiltonian entries and one
...
\begin{lem}[Finite-window obstruction for the full Hamiltonian sector]\label{lem:finite-window-hamiltonian-obstruction}
  ...
  \begin{enumerate}
    \item finite degree windows fail to be Lie subalgebras;
    \item projected degree-window brackets can violate Jacobi;
    \item $\mathfrak m$-adic jet quotients fail to be Lie quotients in the
...
\begin{proof}
  ...
  which has degree $2N-2>N$ for $N>2$, so degree $\leq N$ windows fail to be Lie
  subalgebras.
  ...
  The $\mathfrak m$-adic quotients fail stability under translations generated
  by linear Hamiltonians:
  $$\{z_1^{N+1},z_2\}=(N+1)z_1^N\notin\mathfrak m^{N+1}.$$
  Hence $\mathfrak h/\mathfrak m^{N+1}$ fails to be a Lie quotient for the full
  Hamiltonian algebra.
...
\begin{proof}
  ...
  coadjoint action rather than by deforming the polynomial one-\(\psi\)
  labels.  On the dense strict Matlis subtarget it is the
  ...
  hypothesis is necessary: the PBW one-\(\psi\) label map fails to
...
\begin{prop}[Finite-window criterion for the first \(\theta_3\) row]\label{prop:theta-three-finite-window-criterion}
  ...
  \(B^2_{02,20}\) column is absent.  The only degree-zero lower column is
...
\begin{proof}
  ...
  A companion-face table is a signed source-face relation rather than a
  primitive.  It changes the residual
...
\begin{thm}[Costello first/third normalization recognition criterion]\label{thm:first-third-costello-normalizations}
  ...
  \[
    ...
    (\mathfrak o_{\mathrm{Cas}},
     ...
     \mathfrak n_3).
  \]
  With this datum absent, Theorem~\ref{thm:open-line-midpoint-graph-weights}
  proves the reduced open-line Wick normalization. Costello heat-kernel graph
...
transport, and descent (Vol.~III, calabi-yau-quantum-groups), supplies
external comparison data for the formal-Darboux stalk theorem of this
manuscript.  The matched-conventions class
$\operatorname{ob}_{\mathrm{VolIII}}$ of
