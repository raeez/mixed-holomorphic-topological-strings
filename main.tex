...
\end{titlingpage}
  ...
  variety on $\mathfrak{gl}_N\oplus\mathfrak{gl}_N$: for
  $N\ge 2$ the entries of $[\phi_1,\phi_2]$ have syzygies and define
  the derived commuting-pair stack.  The Koszul complex
  ...
  Loday--Quillen--Tsygan \cites{loday-quillen,tsygan} identify the
  ...
  holomorphic singular product of
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is a two-variable
  ...
  A one-dimensional chiral algebra is obtained after constructing a
  restriction or pushforward to a curve $C\hookrightarrow\C^2$.  Such
  a construction must specify the transverse modes, the pushed-forward
  bracket, the BV pairing, the image of the brane chart, and the
  anomaly normalisation.  Heisenberg, affine Kac--Moody,
  \(\beta\gamma\), Virasoro, and \(W\)-algebra vertex algebras enter
  as curve-chiral comparison targets with these extra hypotheses.
...
section; \(\Delta_5=\Phi_{10}^{1/2}\) denotes a chosen square-root
branch on the paramodular cover (its operator-level lift on
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  holomorphic singular product is not an open item in this problem:
...
At each closed point $p\in\C^2$ the formal-Darboux stalk
identification of Theorem~\ref{thm:main-local} reads
\[
  \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}
   \;\simeq\;
  Z^{\mathrm{der}}_{E_1}\!\bigl(A^{\mathrm{cl}}_{\partial,N}\bigr)
   \;\simeq\;
  HH^\bullet\bigl(A^{\mathrm{cl}}_{\partial,N},
                  A^{\mathrm{cl}}_{\partial,N}\bigr),\qquad
  A^{\mathrm{cl}}_{\partial,N}
    =C^\bullet_{\mathrm{CE}}\bigl(\mathfrak{gl}_N,\,
       \mathrm{Kosz}([\phi_1,\phi_2])\bigr),
\]
with formal-Darboux coordinate
$J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}$.  The two
examples below compute $J$ at the two simplest brane multiplicities.

\subsubsection{The single brane: \texorpdfstring{$N=1$}{N=1}}\label{sssec:ex-heisenberg}

At $N=1$ the matrices commute, the formal-Darboux chart is
$A_b=\widehat{\mc O}_{\C^2,p}\cong\C[[z_1,z_2]]$ with
$(\phi_1,\phi_2)=(z_1,z_2)$, and the trace is evaluation:
\[
  J(z_1^a z_2^b)=z_1^a z_2^b .
\]
The closed-to-open dictionary is
$c_f\mapsto\theta_f$, $u_f\mapsto J(f)$.  Weyl quantisation gives
$[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar$, the one-pair
Heisenberg relation; the Capelli scalar
$\hbar N[\bar c]\big|_{N=1}=\hbar[\bar c]$ vanishes after scalar
reduction.
...
[\phi_1,\phi_2]=0\}$, with the Procesi--Razmyslov forcing
making $J$ the unique $GL_N$-equivariant primitive single-trace
Darboux-natural map.  The Weyl-quantised commutator is
$[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\cdot I$ in
$M_N(\C)$, and the Capelli scalar
$\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}\in
H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]$
(Theorem~\ref{thm:capelli-equals-modular-curvature}) is the
projective curvature of the modular line bundle at the trace
generator.
...
\begin{conj}[Operator-level lift of $\Phi_{10}$]\label{conj:operator-modular-lift}
  ...
  boundary of $K3\times E$.  Suppose the compact-target
