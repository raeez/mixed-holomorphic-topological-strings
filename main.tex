...
\end{titlingpage}
  ...
  \small
  \textbf{No curve VOA, Zhu algebra, or one-variable OPE is asserted on
  the native habitat.}
  The native holomorphic object is the two-complex-dimensional
  factorization algebra on \(\C^2_{\mathrm{hol}}\) built from
  \(\Omega^{0,\bullet}_c(B)\widehat\otimes
   (\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1])\) with
  \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1\); Beilinson--Drinfeld
  vertex/OPE/Zhu data lives on a curve, the habitat here is a surface
  \cite{beilinson-drinfeld}.  Any reduction to a curve chiral object
  requires a separately named restriction datum: the reduced directions,
  the residue or compact-support pushforward, the surviving
  \(z_2\)-mode or principal-part coefficient system, the brane image,
  the induced BV pairing and Schouten bracket, and the matched anomaly
  data.  Every appearance of the word \emph{chiral} below is in this
  two-complex-dimensional holomorphic factorization sense.
  \end{minipage}}
  \end{center}
  ...
  action on linear generators.  Under these hypotheses the local
  habitat is forced to be \(X=\R_t\times\R_s\times\C^2_{z_1,z_2}\) with
  \(L=\R_t\times\{s=z_1=z_2=0\}\), and the measurement is forced to be
  \(J\).
  ...
  \]
  Each arrow is a theorem inscribed below.  The closed BV theory is the
  ...
  RG-stable analytic completion is the weighted-Tate habitat \(B_w\) of
  ...
  finite-window \(H^1\), and the strict native all-window
  ...
  matrices, computed by the Koszul resolution of the constraint
  \([\phi_1,\phi_2]=0\); on the closed-string subalgebra
  ...
  \paragraph{Principle 2: the center of mass remembers what Hamiltonian
  reduction forgets.}
  ...
  habitat \(B_w\), generated by \(\{O_f,\theta_f\}_{f\in\bar A}\)
  ...
  axes --- the topological time \(t\), the topological propagator
  direction \(s\), and the holomorphic normal coordinates \(\bar z_i\)
  --- and the locality of the factorization theory must record each
  axis with its own homotopy.  Three differentials carry the load:
  \[
    \delta(t-s),\qquad
  ...
  \]
  registering contact along the brane line, cohomological triviality
  of topological translations, and residue support in the holomorphic
  ...
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\) of Principle~2:
  ...
  native all-window Bochner--Martinelli current transfer fails for
  ...
  \begin{center}
  \textbf{What is proved, what is conditional, what is named obstruction.}
  \end{center}

  \noindent
  Every theorem below falls into one of three classes: a proved
  theorem; a theorem conditional on a named hypothesis, with the
  hypothesis stated not implicit; or a proved obstruction theorem
  identifying the exact cohomology class, kernel, cokernel, or
  finite-window matrix equation that prevents a stronger statement.
  ...
  \tau_{\mathrm{op}})\)\\
  \hline
  Canonical weighted-Tate habitat \(B_w\)
  (Theorem~\ref{thm:wt-canonical-habitat}) &
  proved &
  ...
  \hline
  Strict native all-window Bochner--Martinelli current transfer
  ...
  \noindent
  Curve-VOA / Zhu / OPE statements on the native
  \(\C^2_{\mathrm{hol}}\) factorization object are not asserted at any
  status (cf.~the warning at the head of this section).  Compact
  Calabi-Yau, BCOV \(F_g\), CY-to-chiral, CoHA, OSV / GV, Igusa, and
  BKM transfers are external comparison surfaces; the only Calabi-Yau
  datum used in the core local theorems is the trivial holomorphic
  volume \(dz_1\wedge dz_2\) on \(\widehat{\C^2}_0\).
...
\end{equation}
and the constraint is imposed by Koszul resolution.  Introduce a
fresh odd matrix-valued antifield
...
\begin{rmk}[The microscope obstruction]\label{rmk:microscope-obstruction}
  ...
  cohomology classes obstructing strict native all-window support-local
  ...
  \(\Phi_{\mathrm{cl}}\); the cotangent shadow is residual, not papered
  over.
\end{rmk}

\begin{rmk}[The native object is a two-complex-dimensional factorization algebra, not a one-dimensional VOA]\label{rmk:not-a-voa}
  The closed observables produced by the brane-probe analysis live on
  the complex surface \(\C^2\): the holomorphic factorization algebra
  \(\mathcal F^{\mathrm{hol}}_{\mathrm{Ham}}\) of
  \S\ref{ssec:two-algebras},
  Definition~\ref{def:local-hamiltonian-chiral-factorization-algebra}
  takes its value on holomorphic polydisks \(B\subset\C^2\), and its
  dg Lie input
  \(\Omega^{0,\bullet}_c(B)\widehat\otimes
  (\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1])\) --- with
  \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\) the Hamiltonian Lie algebra
  modulo constants of \S\ref{ssec:hamiltonian-bf-algebra} --- carries
  two complex variables \(z_1,z_2\) and the residue duality between them.  No one-dimensional vertex algebra, OPE expansion, or Zhu
  reduction is asserted on this native object.  Reduction to a curve
  chiral algebra requires a separate codimension-one defect,
  transverse boundary condition, and Laurent expansion datum, and is
  not part of the local Hamiltonian theorem
...
extension and Casimir convergence --- is the weighted-Tate habitat
...
\begin{rmk}[Quantum extension]
  ...
  the canonical weighted-Tate habitat \(B_w\)
  ...
  balanced scalar-contact zero projection on its habitat, reduced
...
\paragraph{Why this is the minimal habitat.}
...
\begin{prop}[Native Darboux polydisk constructions]\label{prop:native-darboux-disk-constructions}
  For
  \[
...
\begin{proof}
  The formal Darboux polydisk is the habitat fixed in the opening of this
  section.  Item (1) is
...
\begin{rmk}[Reduced chiral-interface target]\label{rmk:constructed-chiral-interface-target}
  ...
  target, not part of the native Darboux polydisk construction.  It must pass through a
...
all-window strict native side, with \(\mathfrak o_{\mathrm{Cas}}\) the
Casimir-kernel block, BMK no-go
Theorem~\ref{thm:bmk-finite-window-no-go} ruling out one-pair
analytic strict-native transfer and forcing the pro-Matlis retract as
...
on Tate/nilpotent/weighted habitat plus bounded inverse-limit
...
the Koszul resolution of the moment-map ideal
...
\begin{thm}[Coordinate uniqueness of \(J\)]\label{thm:Jf-coordinate-uniqueness}
  ...
  Proposition~\ref{prop:brane-ops}. Among all linear maps
  \[
    \widetilde J\colon\mathfrak h\longrightarrow A_\partial
  \]
  satisfying:
  \begin{enumerate}[label=(U\arabic*)]
    \item (\(GL_N\)-invariance, stable) \(\widetilde J(f)\) is a stable
      \(GL_N\)-invariant trace polynomial in \((\phi_1,\phi_2)\) modulo
      the moment-map ideal \([\phi_1,\phi_2]=0\), in the sense of
      Proposition~\ref{prop:stable-trace-invariants};
    \item (Darboux naturality) \(\widetilde J\) commutes with the formal
      \(GL_N\)-equivariant symplectomorphisms preserving
      \(\omega=dz_1\wedge dz_2\), i.e.\ for every formal canonical
      transformation \(\Phi\colon\widehat{\C^2}_0\to\widehat{\C^2}_0\)
      one has \(\widetilde J(\Phi^*f)=\Phi^*\widetilde J(f)\) under the
      induced action on \(A_\partial\);
    \item (scalar reduction) \(\widetilde J\) descends through the
      stable scalar-reduced quotient
      \(A_\partial=A^{GL_\infty}_\infty/\C\cdot\operatorname{Tr}(1)\);
    \item (linear normalization) on linear generators
      \(\widetilde J(z_1)=\overline{\operatorname{Tr}\phi_1}\) and
      \(\widetilde J(z_2)=\overline{\operatorname{Tr}\phi_2}\),
  \end{enumerate}
  the brane substitution
  \(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\) is the unique
  solution. The Lie-algebra morphism property
  \(\{\widetilde J(f),\widetilde J(g)\}_\partial=
  \widetilde J(\{f,g\}_{\bar A})\) holds for any solution
  (Theorem~\ref{thm:bulk-boundary}); uniqueness is the additional
  content of (U1)--(U4). The category-theoretic terminal-object
  formulation, including the protected admissibility hypotheses, is
...
\begin{proof}
  ...
  \(GL_N\)-invariance (U1) restricts \(\widetilde J(f)\) to a polynomial
  in cyclic words on \((\phi_1,\phi_2)\) modulo finite-rank trace
  identities, with stable representative an element of
  \(\mathbf S(\operatorname{Cyc}(F)_{\mathrm{nonempty}})\). Restriction
  to the moment-map zero-fibre and the adjacent-swap exactness
  Lemma~\ref{lem:adjacent-swaps} collapses every cyclic word to a
  commutative monomial trace
  \(\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\)
  (Corollary~\ref{cor:matrix-evaluation-well-defined}); the
  ghost-zero invariant ring computed by Koszul resolution and
  Procesi--Razmyslov is therefore
  \(\mathbf S(\bar A)\) on the connected single-trace generators. On
  each Taylor monomial \(z_1^az_2^b\) the value
  \(\widetilde J(z_1^az_2^b)\) is a scalar multiple of
  \(\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\) plus multi-trace
  decomposables.
  Scalar reduction (U3) kills the multi-trace decomposable summands
  at the level of the connected indecomposable quotient
  \(\mathfrak m/\mathfrak m^2\) of Proposition~\ref{prop:brane-ops}, so
  \(\widetilde J\) is determined by its values on the indecomposable
  primitives.
  Linear normalization (U4) fixes the scalar on the two degree-1
  primitives. Darboux naturality (U2) under translations
  \(z_i\mapsto z_i+c_i\), dilations
  \(z_1\mapsto\lambda z_1, z_2\mapsto\lambda^{-1}z_2\), and the
  diagonal Hamiltonian flow generated by linear generators propagates
  the normalization through every Taylor degree: the \(GL_N\)-action
  commutes with substitution, so \(\widetilde J(\Phi^*f)=\Phi^*J(f)\)
  forces \(\widetilde J=J\) on the orbit of the linear generators
  under canonical transformations, which exhausts the polynomial
  algebra \(\bar A\) by formal Darboux density. Continuity of
  \(\widetilde J\) in the \(\widehat{\mathfrak h}\)-topology extends
  the equality from \(\bar A\) to all of \(\widehat{\mathfrak h}\).
\end{proof}
...
cotangent extension of this match --- a strict native, all-window,
...
endpoint is therefore foreclosed.  The native all-window analytic
no-go is Theorem~\ref{thm:bmk-finite-window-no-go}: the strict native
...
strict-native refinement is blocked by a named entry of
...
its habitat, weighted Moyal principal-part current brackets, and a
...
\begin{thm}[Componentwise quantum coefficient comparison]\label{thm:componentwise-quantum-coefficient-comparison}
  ...
  obstruction class on its own habitat.
  \begin{itemize}
    \item \emph{Components proved compatible:} weighted BV kernel
      ...
      \(0_{\mathrm{sc}}\) on its scalar-contact habitat; reduced
    ...
    \item \emph{Vanishing criterion for the four named components:}
      ...
      external input on the scalar-contact balanced habitat and on the
    ...
    \item \emph{Physical meaning:} the Moyal star, the matrix Weyl
      ...
      componentwise on the proved habitat; mismatch on either of the
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  ...
  cohomology class on a single habitat.
  \begin{itemize}
    ...
    \item \emph{First nonzero example or known failure:} the
      ...
      inside the current Bianchi-exposed habitat.
