...
\end{titlingpage}
  ...
  \section*{The matrix microscope}
  \addcontentsline{toc}{section}{The matrix microscope}

  A stack of \(N\) Dirac branes on the topological line is a
  finite-matrix microscope for the formal symplectic polydisk
  \((\widehat{\C^2}_0,dz_1\wedge dz_2)\), and a closed Hamiltonian on
  the polydisk is measured on the brane stack by literal substitution
  into a matrix-valued transverse coordinate:
  \[
  ...
  \]
  \[
    J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)},\qquad
    f\in\C[[z_1,z_2]] .
  \]
  The brane is not a representation of the Hamiltonian algebra;
  it is a noncommutative point of the formal symplectic disk.  Its
  transverse position is the commuting Chan--Paton pair
  \((\phi_1,\phi_2)\in\mathfrak{gl}_N^{\oplus 2}\) imposed by the
  conjugation moment map, and a closed Hamiltonian \(f\) is evaluated
  at that point in the only stable \(GL_N\)-invariant, Dirac-constrained,
  Darboux-natural way available --- by trace substitution
  \(f\mapsto\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\)
  (Theorem~\ref{thm:formal-local-universal-property}).  The closed
  Hamiltonian algebra acts on itself; the brane is where its values are
  read.  The hypotheses
  are explicit: one Dirac brane topological line, one topological
  propagator direction, formal holomorphic symplectic normal surface,
  \(GL_N\)-conjugation gauge symmetry, Darboux-natural scalar-reduced
  polynomial \(GL_N\)-invariance, and agreement with the moment-map
  action on linear generators.  Under these hypotheses the local geometry is
  \(X=\R_t\times\R_s\times\C^2_{z_1,z_2}\) with
  \(L=\R_t\times\{s=z_1=z_2=0\}\), and the measurement is
  \(J\).

  \medskip
  \noindent
  Uniqueness propagates.  A single forcing chain links the brane probe
  to the cohomology classes that name its obstructions:
  \[
    \boxed{\;
    \text{brane probe}\,\Longrightarrow\,
    \text{trace measurement}\,\Longrightarrow\,
    \text{shifted-cotangent BV}\,\Longrightarrow\,
    \text{CE/PV operations}\,\Longrightarrow\,
    \text{Matlis principal parts}\,\Longrightarrow\,
    \text{named obstructions} .\;}
  \]
  The closed BV theory is the
  shifted cotangent extension
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
  with \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\); the CE/PV cochain
  dictionary \(c^I\!\mapsto\!\theta^I,\,u_I\!\mapsto\!O_I\) fixes the
  type discipline (a Hamiltonian \emph{moves} the closed sector through
  \(c_f\) and is \emph{measured} on the brane through \(u_f\), not the
  reverse); the cotangent module is the Grothendieck--Matlis
  principal-part bundle \(\mathcal P\), not a polynomial copy; and the
  RG-stable analytic completion is the weighted-Tate completion \(B_w\) of
  Theorem~\ref{thm:wt-canonical-habitat}.  At the end of the chain the
  named obstructions appear as cohomology classes, kernels, or matrix
  equations: the scalar anomaly \(\hbar N[\bar c]\), the radial
  cokernel \(\Omega^{\mathrm{rad}}_{a,b}\), the four-component
  unreduced cotangent-lift obstruction vector, the \(\theta_3\)
  finite-window \(H^1\), and the strict all-window
  Bochner--Martinelli no-go.

  \medskip\noindent
  The open BV theory of the probe stack is the derived zero-fibre of
  the \(GL_N\)-conjugation moment map.  Its ghost-zero invariant ring
  is the commutative Procesi--Razmyslov trace ring of two commuting
  matrices, computed by the Koszul derived zero-fibre model for the
  moment-map equation \([\phi_1,\phi_2]=0\); the matrix entries of
  \([\phi_1,\phi_2]\) are not a regular sequence for \(N\geq2\), so
  this is the derived zero fibre, not a regularity assertion.  On the
  closed-string subalgebra
  \(\bar A=\C[z_1,z_2]/\C\cdot1\) of polynomial Hamiltonians modulo
  constants this ring is the trace family \(J(f)\).  Stable connected
  \(GL_N\)-invariants are traces of cyclic words in
  \(\phi_1,\phi_2\); the moment-map constraint
  \([\phi_1,\phi_2]=0\) collapses cyclic words to commutative
  monomials; Darboux naturality kills any dependence on the chosen
  symplectic coordinate presentation; agreement with the moment-map
  action on \(z_1,z_2\) fixes the scalar-reduced extension; and only
  literal substitution \(f\!\mapsto\!\overline{\operatorname{Tr}
  f(\phi_1,\phi_2)}\) survives
  ...
  symmetry algebra is \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot1\); on the
  unreduced brane phase space the same constants survive as the brane
  center-of-mass motion, and the unreduced Poisson bracket is
  \(\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N\).  Reduction
  on one side and survival on the other is a single distinguished
  class
  \[
  ...
  \]
  Moyal quantization \cite{kontsevich-dq} in the ordinary
  scalar-reduced \(\mathfrak{gl}_N\) branch returns the same scalar as
  the Capelli term \cite{capelli,howe-capelli}, namely
  \(\hbar N[\bar c]\), and the first quantum example is the linear
  brane commutator
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\).  Four detections
  of one cocycle --- algebraic central extension on the closed Taylor
  ring, Lie central extension on \(\bar A\), trace-algebraic boundary
  central extension on the unreduced phase space, and quantum Weyl
  commutator at order \(\hbar\) --- are the four faces of the same
  scalar-reduction step transgressed against the brane probe
  ...
  \medskip\noindent
  A Hamiltonian \(f\in\bar A\) has two avatars on the brane probe.  As
  a generator of canonical transformations it is represented in the
  closed BV algebra by the CE coordinate \(c_f\) and acts as the
  Schouten vector field \(\theta_f\); as a closed cotangent insertion
  it is represented by the shifted-cotangent coordinate \(u_f\) and is
  measured on the brane as the boundary observable
  \(O_f=J(f)\).  The closed BV algebra is the shifted cotangent Lie
  algebra
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
  ...
  \]
  the boundary observable is the image of \(u\), not of \(c\), and the
  wrong assignment \(c_f\!\mapsto\!O_f\) collapses the operation
  \(\theta_f\) into its measurement \(O_f\) and is a type error.  The
  weighted-Tate completion \(B_w\) is minimal in the category of
  \(w\)-weighted bracket-admissible RG-stable Tate completions
  (Theorem~\ref{thm:wt-canonical-habitat}): the smallest filtered
  subspace generated by \(\{O_f,\theta_f\}_{f\in\bar A}\) under Schouten
  bracket and pointwise product in which the heat-kernel propagator
  extends and the Casimir converges
  (Theorem~\ref{thm:reduced-ce-pv-central-operation}). Canonicity is
  relative to \(w\); the same construction at a cofinally equivalent
  admissible weight gives a filtered-quasi-isomorphic target.

  \medskip\noindent
  A Taylor Hamiltonian is what the brane samples; a principal part is
  the dual distributional momentum concentrated at the brane.  It is
  not another polynomial mode, but a residue functional that detects a
  Taylor coefficient.  The continuous dual of Taylor functions at the
  brane point is therefore the Grothendieck--Matlis principal-part
  module \cite{matlis-injective}
  \[
    \mathfrak h^\vee_{\mathrm{cont}}\cong\mathcal P
     =\bigoplus_{a+b>0}\C\,\rho_{a,b},\qquad
    \rho_{a,b}=z_1^{-a-1}z_2^{-b-1}\,dz_1dz_2,
  \]
  paired with Taylor Hamiltonians by residue.  The polynomial PBW
  classes \(\Psi_{a,b}\) carry the same labels \((a,b)\) and a
  different \(\mathfrak h\)-module structure: the residue and PBW
  shadows are not relabellings of each other, and the Moyal coadjoint
  action acts on the residue side and does not lift to the polynomial
  shadow
  ...
  \medskip\noindent
  A measurement at the brane line travels along three independent
  axes --- the topological time \(t\) on the brane line, the
  topological propagator direction \(s\) transverse to the brane, and
  the holomorphic normal coordinates \(\bar z_i\) --- and the locality
  of the factorization theory must record each axis with its own
  homotopy.  Three differentials carry the load (with \((t,t')\) two
  brane-time points and \(s\) reserved for the transverse direction):
  \[
    \delta(t-t'),\qquad
  ...
  \]
  registering contact between two points on the brane line,
  cohomological triviality of topological translations, and residue
  support in the holomorphic normal direction.  None of the three implies the others, and none
  of them by itself supplies the unreduced cotangent centrality
  homotopies; together they force the factorization-current model on
  the brane line, with the trace observables exhibiting all three and
  the shifted-cotangent currents only the first two
  (Proposition~\ref{prop:three-localities},
  Theorem~\ref{thm:dict-kinematic-locality}).

  \medskip\noindent
  Moyal quantization tests whether Weyl ordering on the formal
  symplectic polydisk and the brane trace measurement \(J\) commute.
  The first quantum example is the linear scalar anomaly
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\) of the
  closed--open mismatch above:
  Weyl quantization on the closed side and brane measurement on the
  open side return the same scalar \(\hbar N\), so the comparison
  closes at linear order in the ordinary scalar-reduced
  \(\mathfrak{gl}_N\) branch
  (Theorem~\ref{thm:quantum-classical-anomaly}).  At general bidegree
  \((a,b)\) the obstruction to closing the comparison is a single
  cohomology class
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\)
  (Proposition~\ref{prop:app-radial-dual-potential-obstruction}).  The
  decorated-PBW Stokes script
  \texttt{quantum\_shear\_trace\_diagram\_normal\_form.py
  --kernel-correction} verifies vanishing of
  \(\Omega^{\mathrm{rad}}_{a,b}\) on its default 13-case window plus
  the bidegrees enumerated by \texttt{--edge-family-max} and
  \texttt{--balanced-family-max}; the bidegrees explicitly cited in
  Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}
  --- balanced diagonal through \((6,6)\), every non-edge bidegree of
  total degree at most \(13\), the total-degree-\(14\) bidegrees
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\), and every
  one-moving-letter edge strip --- are reproduced by running the
  script with the flag combinations specified in
  \S\ref{ssec:tate-residual-resolution}.  The radial bicomplex is
  interior-acyclic conditional on the named radial-parts statement
  ...
  \medskip\noindent
  Each strengthening beyond the proved core is the vanishing of a
  named cohomology class.  The unreduced \(P_0\)-factorization-centre lift
  exists exactly when the four-component obstruction vector
  \((\mathrm{ob}^{\mathrm{prod}}_{\mathrm{cent}},
  \mathrm{ob}^{P_0}_{\mathrm{cent}},
  \mathrm{ob}^{\mathrm{bd}}_{\mathrm{QME}},
  \delta^{\mathrm{Weiss}}_{\check C/R})\) vanishes
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction},
  Problem~\ref{prob:formal-factorization-center}).  The smallest
  non-scalar Costello quantum master equation window
  \(\theta_3\) carries the bare verdict
  \(H^1=\C\!\cdot\!E_{\theta_3}\ne0\) and the supplied-counterterm
  verdict \(H^1=0\) under the unique counterterm \(dC=-E\), \(c=1\)
  (Theorem~\ref{thm:app-theta-three-finite-window-H1}).  The strict
  all-window Bochner--Martinelli current transfer fails for
  every \(N\) by the residue-coadjoint computation
  \(z_1^{N+2}\!\cdot\!\rho_{N+1,0}=-(N+2)\rho_{0,1}\) and the
  projected Jacobiator \((N+1)(N+2)\rho_{0,1}\ne0\)
  (Theorem~\ref{thm:bmk-finite-window-no-go}).  The all-bidegree
  decorated PBW Stokes identity is the family of cokernel classes
  \(\Omega^{\mathrm{rad}}_{a,b}\), realised as the radial-parts
  bicomplex
  ...
  constants vanish closed-side as Hamiltonian vector fields, survive
  open-side as the brane center of mass\\
  \hline
  Cotangent defect momentum &
  \(\mathcal P=\bigoplus_{a+b>0}\C\,\rho_{a,b}\) &
  residues are the dual distributional modes concentrated at the
  brane\\
  \hline
  Quantization &
  Moyal star on \(\C[[z_1,z_2]]\); Weyl on \(\mathfrak{gl}_N^2\) &
  brane-measurement test of Weyl ordering against trace evaluation\\
  \hline
  Failure of full quantum comparison &
  \(\Omega^{\mathrm{rad}}_{a,b},\;
  [\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\) &
  radial-parts cokernel and Costello quantum-master-equation
  obstruction\\
  \hline
  \end{tabular}
  \end{center}
  ...
  \begin{tabular}{@{}p{0.34\textwidth}p{0.20\textwidth}p{0.40\textwidth}@{}}
  \hline
  \textbf{Theorem} & \textbf{Status} & \textbf{Content / hypothesis / obstruction class}\\
  \hline
  \textbf{Theorem~A.}
  \(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) is the unique
  algebraic measurement
  (Theorem~\ref{thm:Jf-coordinate-uniqueness}) &
  proved &
  unique among linear maps \(\mathfrak h\to\widehat{\mathbf S}(\bar A)\)
  satisfying stable \(GL_N\)-invariance, the Dirac constraint
  \([\phi_1,\phi_2]=0\), scalar reduction, Darboux equivariance, and
  linear normalization on \(z_1,z_2\); proof monomial-by-monomial via
  invariant theory plus the no-natural-correction lemma\\
  \hline
  \textbf{Theorem~B.}  \(J\) is the terminal protected Hamiltonian
  sector at a holomorphic-symplectic brane point
  (Theorem~\ref{thm:formal-local-universal-property}) &
  proved &
  among admissible protected ambient data, the formal-disk model
  \((\widehat{\mathfrak h}\ltimes\widehat{\mathfrak h}^\vee_{\mathrm{cont}}[1],
  \Omega^*(\R)\otimes\Lambda(\varepsilon_1,\varepsilon_2)
  \otimes\mathfrak{gl}_N[1],[\bar c])\) with bulk-to-brane evaluation
  \(J\) is terminal up to canonical isomorphism; the obstruction to
  membership of an external ambient datum is the four-component
  universal-property class
  \(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\). Dependency: Theorem~B
  is downstream of Theorem~A, not the reverse\\
  \hline
  Universal CE/PV Koszul criterion, five clauses
  (Theorem~\ref{thm:universal-ce-pv-koszul-criterion}) &
  per-clause &
  cochain identity \(c^I\!\mapsto\!\theta^I,\,u_I\!\mapsto\!O_I\)
  proved unconditionally; \(P_0\)-bracket clause conditional on
  bracket-admissibility of the chosen sub-PV model \(B\); MC-kernel
  clause conditional on a kernel-admissible datum
  \((B,\mathcal K_B)\); bar-cobar / enveloping clause conditional on a
  Tate / nilpotent / weighted admissibility datum with conilpotent
  endpoint and \(\varprojlim^1\)-vanishing; open-trace
  \(A_\infty\)-Koszul clause conditional on an acceptance datum
  \((A_{\mathrm{op}},C_{\mathrm{op}},B_{\mathrm{CE/PV}},
  \tau_{\mathrm{op}})\)\\
  \hline
  Canonical weighted-Tate completion \(B_w\)
  (Theorem~\ref{thm:wt-canonical-habitat}) &
  proved &
  \(B_w\) is minimal in the category of \(w\)-weighted bracket-admissible
  RG-stable Tate completions: the smallest complete filtered subspace
  closed under Schouten bracket and pointwise product, containing the
  regulated Casimir contractions, and stable under the heat-kernel/RG
  propagator. Canonicity is relative to the chosen admissible weight
  \(w\); cofinality across admissible weights is not asserted\\
  \hline
  Principal-part coadjoint isomorphism, Matlis form
  (Theorem~\ref{thm:principal-part-coadjoint-uniqueness}) &
  proved &
  \(\mathfrak h^\vee_{\mathrm{cont}}\cong\mathcal P\) by residue, with
  coadjoint formula
  \(z_1^pz_2^q\!\cdot\!\rho_{a,b}=
   -(pb-qa+p-q)\rho_{a-p+1,b-q+1}\) unique up to a scalar\\
  \hline
  Three-locality kinematic theorem
  (Theorem~\ref{thm:dict-kinematic-locality}) &
  proved &
  \(\delta(t-t')\), \(L_v=[Q,\iota_v]\),
  \(L_{\partial/\partial\bar z_i}=[Q,\iota_{\partial/\partial\bar z_i}]\)
  are independent and jointly force the brane-line
  factorization-current model\\
  \hline
  Scalar anomaly four-face transgression
  (Theorems~\ref{thm:u1-center-anomaly},
  \ref{thm:u1-center-anomaly-open},
  \ref{thm:quantum-classical-anomaly},
  \ref{thm:app-scalar-anomaly-transgression}) &
  ...
  detected by algebraic, Lie, trace-algebraic, and Moyal Weyl
  commutator faces; Capelli order-\(\hbar\) shift
  \cite{capelli,howe-capelli}\\
  \hline
  Componentwise quantum coefficient comparison
  (Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}) &
  proved with finite certificate &
  range certified by
  \texttt{quantum\_shear\_trace\_diagram\_normal\_form.py
  --kernel-correction} run with \texttt{--balanced-family-max=6} and
  \texttt{--edge-family-max} sufficient to cover every one-moving-letter
  edge strip; total-degree-\(14\) bidegrees
  \((3,11),(11,3),(4,10),(10,4),(5,9),(9,5)\) covered by explicit
  \texttt{--case} arguments\\
  \hline
  Radial / Weyl normal-form obstruction
  (Proposition~\ref{prop:app-radial-dual-potential-obstruction}) &
  proved &
  per-bidegree obstruction
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\);
  ...
  \hline
  Radial bicomplex acyclicity
  (Theorem~\ref{thm:app-radial-bicomplex-acyclic}) &
  conditional, with finite-degree certificate &
  interior acyclicity conditional on the named radial-parts statement;
  unconditional through total degree~15 by stable Procesi--Razmyslov
  reduction\\
  \hline
  \(\theta_3\) finite-window
  \(H^1\)
  (Theorem~\ref{thm:app-theta-three-finite-window-H1}) &
  proved &
  bare verdict \(H^1=\C\!\cdot\!E_{\theta_3}\ne0\) with covector
  \(\ell=1\); supplied-counterterm verdict \(H^1=0\) under the unique
  \(dC=-E,c=1\) counterterm\\
  \hline
  Strict all-window Bochner--Martinelli current transfer
  (Theorem~\ref{thm:bmk-finite-window-no-go}) &
  proved obstruction theorem &
  fails for every \(N\); residue-coadjoint computation
  \(z_1^{N+2}\!\cdot\!\rho_{N+1,0}=-(N+2)\rho_{0,1}\); projected
  Jacobiator \((N+1)(N+2)\rho_{0,1}\ne0\); obstruction vector
  \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\)\\
  \hline
  Pro-Matlis retract is the supremum form
  (Corollary~\ref{cor:promatlis-retract-as-supremum}) &
  proved &
  \(P_{\Pi}^{\mathrm{an}}=\tau P_{\mathrm{an}}\),
  \(\pi_NP_{\Pi}^{\mathrm{an}}=p_N\); the one-pair analytic retract is
  the strongest current-transfer object selected by the BMK leak\\
  \hline
  Unreduced cotangent-lift four-component obstruction
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction},
  Problem~\ref{prob:formal-factorization-center}) &
  proved obstruction theorem &
  the chain morphism
  \(\Phi_{\mathrm{cl}}\colon
   \mathrm{Obs}^{\mathrm{cl}}_{\mathrm{closed}}\big|_L\to
   \mathrm Z^{P_0}_{\mathrm{fact}}
   (\mathrm{Obs}^{\mathrm{cl}}_{\mathrm{open},N})\)
  exists exactly when
  ...
  \(\varprojlim^1H^0\)-primitive-compatibility class; constructed once
  a complete \(d_{\mathrm{QME}}\)-stable scalar-contact filtration is
  supplied and the successive scalar-symbol extension classes vanish\\
  \hline
  \end{tabular}
  \end{center}
...
\subsection{The brane microscope in one calculation}\label{ssec:brane-microscope-one-calculation}

The local geometry is the product
\[
  X=\R_t\times\R_s\times\widehat{\C^2}_{0,\mathrm{hol}}:
\]
\(\R_t\) is the brane worldline; \(\R_s\) is the transverse topological
propagator direction; \(\widehat{\C^2}_{0,\mathrm{hol}}\) is the formal
holomorphic symplectic normal disk at the brane point.  Omission of
\(\R_t\) voids the open-line observable algebra; omission of \(\R_s\)
voids the bulk-to-boundary propagator; omission of
\(\widehat{\C^2}_{0,\mathrm{hol}}\) voids the Hamiltonian algebra;
relaxation of the formal completion admits global geometry into the
brane stalk.

\(N\) Dirac branes on
\(L=\R_t\times\{s=z_1=z_2=0\}\subset X=\R_t\times\R_s\times\C^2\)
carry a Chan--Paton normal coordinate
\((\phi_1,\phi_2)\in\mathfrak{gl}_N^{\oplus2}\).  The holomorphic
symplectic form \(dz_1\wedge dz_2\) pulls back along
\(z_i\rightsquigarrow\phi_i\) and traces to
\[
...
\(\mu(\epsilon)=-\operatorname{Tr}(\epsilon[\phi_1,\phi_2])\), so the
Dirac first-class constraint is \([\phi_1,\phi_2]=0\): the simultaneous
spectrum of \((\phi_1,\phi_2)\) is the configuration of \(N\) points
in the polydisk.  Substitution into a closed Hamiltonian followed by
trace and stable scalar reduction is the brane measurement
\[
  z_i\rightsquigarrow\phi_i,\qquad
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\in
    \widehat{\mathbf S}(\bar A),\qquad
  \bar A=\C[z_1,z_2]/\C\cdot1,
\]
into the completed symmetric algebra \(\widehat{\mathbf S}(\bar A)\)
on the polynomial Hamiltonian subalgebra \(\bar A\).  An odd matrix
antifield \(\psi\in\mathfrak{gl}_N[1]\) and BV differential \(Q\) with
\(Q\phi_i=0\), \(Q\psi=[\phi_1,\phi_2]\) realize the moment-map ideal
as the Koszul derived zero fibre
(\S\ref{ssec:open-mixed-bft}).  The closed shifted-cotangent extension
\[
  \mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1],
  \qquad
  \mathfrak h=\C[[z_1,z_2]]/\C\cdot 1,
\]
supplies the canonical closed sector
(\S\ref{ssec:closed-mixed-hamiltonian}).  A Hamiltonian
\(f\in\mathfrak h\) carries two Chevalley--Eilenberg coordinates on
\(\mathfrak g\): the generator-dual \(c_f\) producing the Hamiltonian
Schouten vector field \(\theta_f\), and the cotangent-dual \(u_f\)
producing the boundary observable \(O_f=J(f)\); the CE/PV cochain
dictionary on generators is
\(c_f\mapsto\theta_f,\;u_f\mapsto O_f\)
(Theorem~\ref{thm:reduced-ce-pv-central-operation}).  The continuous
dual is the Grothendieck--Matlis principal-part module
\(\mathfrak h^\vee_{\mathrm{cont}}\cong\mathcal P\)
(Theorem~\ref{thm:principal-part-coadjoint-uniqueness}).

\begin{ex}[Classical point probe, \(N=1\)]\label{exa:point-probe-N1}
  ...
  variables, and the matrix microscope is the classical evaluation
  homomorphism.
\end{ex}
...
\begin{ex}[Linear Hamiltonians and the scalar anomaly]\label{exa:linear-hamiltonians-anomaly}
  ...
  and survives open-side as the brane center of mass
  ...
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\); the first
...
\begin{ex}[A single Matlis residue]\label{exa:single-matlis-residue}
  ...
  \(\mathcal P\) without local nilpotency.  A polynomial PBW shadow
  attached to the same \((a,b)\)-bidegree appears later in the open
  one-\(\psi\) sector with a strictly different \(\mathfrak h\)-module
  structure
  (Corollary~\ref{cor:descendant-coadjoint-difference}); no
  relabelling between the residue mode here and that polynomial shadow
  exists.
\end{ex}
...
polydisk \((\widehat{\C^2}_0,dz_1\wedge dz_2)\) is an infinite-dimensional
object.  Nothing in the closed polydisk produces a finite-dimensional shadow
of \(f\) intrinsically.  A finite shadow exists exactly when the closed
polydisk is paired with an open boundary that names the two normal
...
\]
records its position inside the formal normal polydisk through a probe pair
...
for the moment-map equation.  Introduce a fresh odd matrix-valued
antifield
...
\end{equation}
The displayed equation defines \(Q\) on the new generator; every matrix
entry of the commutator equals \(Q\psi^i{}_j\) and is therefore
\(Q\)-exact, so the moment-map ideal vanishes in cohomology.  The
...
ghost-zero invariant cohomology computes the classical invariant
functions on the commuting-pair quotient.  The degree-zero Hamiltonian
trace comparison ignores higher Koszul homology; that homology enters
only through explicit invocation.  The
...
closed sector --- the Hamiltonian polynomial algebra modulo constants
\(\bar A=\C[z_1,z_2]/\C\cdot1\) --- acts on the brane through nothing
else.

\begin{lem}[Trace realisation of the formal polydisk: the matrix microscope]\label{lem:probe-trace-microscope}
...
\begin{ex}[\(N=1\): the geometric prototype]\label{ex:N1-commuting-case}
  ...
  The probe pair \((\phi_1,\phi_2)\in\C^2\) is a chosen point of the
  formal polydisk, and
  \[
    J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}=f(\phi_1,\phi_2)
    \in\C
  \]
  is ordinary scalar evaluation modulo the constant.  The matrix
  microscope reduces to the closed-string evaluation map
  \(f\mapsto f(p)\) at a chosen polydisk point \(p\), and the
  Procesi--Razmyslov input is trivial.  Every later non-commutative
  refinement of \(J\) collapses to this scalar evaluation when
  \(N=1\); the matrix structure enters with the brane stack.
\end{ex}
...
\begin{rmk}[The microscope obstruction]\label{rmk:microscope-obstruction}
  ...
  below).  The microscope does not see those homotopies: substitution
  into the probe pair encodes the symbol-level information of \(f\),
  not the cotangent-direction antifield content of the closed BV
  algebra.  The obstruction is exact and named: the unreduced
  ...
  \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\) (a six-component vector of
  cohomology classes obstructing strict all-window support-local
  current transfer to the brane line; named in
  Theorem~\ref{thm:bmk-finite-window-no-go} below).  The matrix
  microscope is the scalar shadow of \(\Phi_{\mathrm{cl}}\); the
  cotangent shadow is residual and named by the obstruction classes
  above.
\end{rmk}

\subsection{The Hamiltonian BF algebra}\label{ssec:hamiltonian-bf-algebra}

A BV theory of canonical transformations of the formal symplectic polydisk
...
\begin{ex}[The lowest non-trivial residue mode]\label{ex:residue-mode-rho10}
  ...
  \]
  The polynomial PBW shadow \(\Psi_{1,0}\), labelled identically,
  ...
  passage between the two; it is not a same-action identification.
  This worked case prevents the type collapse
  \(\Psi_{a,b}\leftrightarrow\rho_{a,b}\) flagged in
...
\begin{rmk}[Two forbidden type-collapses]\label{rmk:intro-symbol-separation}
  ...
  Lemma~\ref{lem:type-discipline} below records this as a single rule;
  the assignment \(c_f\mapsto O_f\) collapses the operation
  (Hamiltonian vector field \(\theta^I\)) onto the observable
  (Schouten function \(O_I\)), breaks the bracket compatibility
  \([\theta^I,O_J]_S=\delta^I_J\), and obstructs every
...
A single discipline rule controls the rest of the Introduction.  A
closed Hamiltonian acts on the brane through one CE coordinate, and is
measured on the brane through a different CE coordinate.  The boundary
observable is the image of the second, never the first.  This is the
type-discipline lemma; every later use of the symbols
\(c_f,u_f,\theta_f,O_f\) refers back to it.

The CE/PV comparison itself is graded into five rungs, each at its own
admissibility hypothesis.  Every later assertion of a CE/PV statement
specifies which rung it inhabits.
\[
  \begin{array}{c|l|l}
    \text{rung} & \text{statement} & \text{admissibility} \\[2pt]
    \hline\\[-9pt]
  ...
  \end{array}
\]
The unconditional content is rung 1.  Rungs~2--5 are conditional, each
on a separate admissibility datum named in
...
cochain \(\tau_{\mathrm{op}}\).  When prose below refers to ``the
CE/PV comparison'' or ``the CE/PV ladder'', the rung is identified
explicitly.

\begin{lem}[Type discipline: a Hamiltonian has two avatars]\label{lem:type-discipline}
  A Hamiltonian \(f\in\mathfrak h\) has two algebraic avatars on the
  ...
  \begin{itemize}
    ...
    \item As closed cotangent insertion:
      ...
      measured on the brane as
      \(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).
  \end{itemize}
  The assignment \(c_f\mapsto O_f\) collapses operations onto
  observables and is forbidden by the CE/PV ladder
  (Theorem~\ref{thm:universal-ce-pv-koszul-criterion},
  Theorem~\ref{thm:reduced-ce-pv-central-operation}).  Closed
  Hamiltonians act through their \(c\)-avatar; brane measurements
  record their \(u\)-avatar.  The two are dual, not equal.
\end{lem}
...
\begin{rmk}[Generator table for the type discipline]\label{rmk:type-discipline-table}
  \[
    \begin{array}{c|c|c|c}
    ...
    \end{array}
  \]
  The bracket-on-observables identity
  \[
    \theta_f(O_g)=O_{\{f,g\}}
  \]
  exhibits \(c_f\) acting on observables through its Schouten image
  \(\theta_f\) without itself being an observable: the value of the
  derivation on the observable \(O_g\) is the observable attached to
  the Hamiltonian bracket \(\{f,g\}\).
\end{rmk}
...
\subsection{The brane center of mass remembers what Hamiltonian reduction forgets}\label{subsec:scalar-anomaly}
...
\begin{ex}[Linear Hamiltonians and the brane centre of mass]\label{ex:linear-hamiltonian-com}
  ...
  at order \(\hbar\): on Weyl quantisations the commutator
  \[
  ...
  The same anomalous scalar is detected three times: on the closed
  Lie cohomology side, on the open trace algebra side, and on the
  Moyal/Weyl side.  These are the three faces of \([\bar c]\) of
  Remark~\ref{rmk:three-faces-one-class}.
\end{ex}
...
vanishes; the surviving leading term is the open-side scalar
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\), exactly the
order-\(\hbar\) image of \(\omega(z_1,z_2)=1\) on the unreduced
algebra.  The classical class \([\bar c]\) and its order-\(\hbar\)
...
action on linear generators is forced to be \(J\)
...
residue side and does not lift to the polynomial shadow
(Theorem~\ref{thm:app-defect-polarization}).

The recognition criterion is a five-level ladder
...
\(k\) lives in a single named obstruction.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{@{}p{0.05\textwidth}p{0.36\textwidth}p{0.27\textwidth}p{0.24\textwidth}@{}}
\hline
\textbf{Level} & \textbf{Comparison} & \textbf{Hypothesis} & \textbf{Status}\\
\hline
1 &
\(C^\bullet_{\mathrm{CE}}(T^*[1]\mathfrak l)\cong
\mathrm{PV}(\mathbf S\mathfrak l)\) on a finite-dim Lie algebra
\(\mathfrak l\) &
none beyond char.\ zero &
theorem
...
coordinatewise finiteness only &
theorem
...
the named five obstructions of clause~(4) of
...
completion forced by the brane microscope --- closed under the
Schouten bracket and pointwise product, with heat-kernel propagator
extension and Casimir convergence --- is the weighted-Tate completion
\(B_w\) (Theorem~\ref{thm:wt-canonical-habitat}); it satisfies levels
3 and 4, with level~5 conditional on the open-trace
...
admissibility ladder, but supplying any four does not produce a
chain morphism without the fifth.  These classes block the unreduced
...
\begin{rmk}[Quantum extension]
  ...
  normal-form criterion.  The finite normal-form
...
\((\widehat{\C^2}_0,\omega)\).  Each factor is forced.
...
Formal completion at \(L\) is forced by the matrix-probe microscope:
the brane sees only the formal holomorphic transverse geometry,
because \(\phi_1,\phi_2\) live in \(\mathfrak{gl}_N^2\) and the
substitution \(z_i\mapsto\phi_i\) is a formal substitution into a
power series.  Removing the formal completion forces the brane to see
irrelevant global geometry of the holomorphic factor and breaks the
scalar-reduced trace identification of
Lemma~\ref{lem:probe-trace-microscope}.

Three measurement principles fix the
...
The matrix microscope produces an open trace algebra at every finite
\(N\); the closed Hamiltonian algebra
\(\bar A=\C[z_1,z_2]/\C\cdot1\) is its image under the substitution
map \(J\) only after a stable trace stabilization is performed.  Four
...
(level~5 of the recognition ladder of \S\ref{ssec:two-algebras}),
...
(Theorem~\ref{thm:formal-local-universal-property}).  The boundary
observable is the image of \(u\), not of \(c\); reading the
Hamiltonian off the wrong coordinate is the type error that Witten's
source convention forbids.
...
weighted-Tate completion \(B_w\), generated by
\(\{O_f,\theta_f\}_{f\in\bar A}\) under the Schouten bracket and
...
a named cohomology class or matrix-equation obstruction:
\(\Omega^{\mathrm{rad}}_{a,b}\),
\([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\),
...
Under the matrix-microscope hypotheses --- a brane stack carrying the
...
symplectic surface at the brane point --- the geometry is forced by
the three independent locality differentials of
Proposition~\ref{prop:three-localities} to be:
...
The brane is not a representation of the Hamiltonian algebra; it is a
noncommutative point of the formal symplectic disk, and a bulk
Hamiltonian \(f\in\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) is read
at that point by the unique stable \(GL_N\)-invariant Darboux-natural
evaluation map
\begin{equation}\label{eq:setup-Jf-flagship}
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\end{equation}
the trace of \(f\) evaluated on the matrix probe pair, projected to the
stable scalar-reduced quotient. Under the matrix-microscope hypotheses
...
the open brane observables runs through a ladder with five rungs of
distinct epistemic status. (1) Finite-dimensional cotangent CE/PV
identification on every truncation
\(\mathfrak h_{\le N}=\mathfrak m^3/\mathfrak m^{N+1}\)
(Theorem~\ref{thm:phi-trunc-classical}; theorem). (2) Coordinate
...
(Theorem~\ref{thm:reduced-ce-pv-central-operation}; theorem). (3)
\(P_0\)-bracket enhancement on a chosen bracket-admissible target
\(B\subset\mathrm{PV}_{\mathrm{coord}}(A_{\partial,\mathrm{coord}})\)
(Definition~\ref{defn:bracket-kernel-admissible}; conditional on
bracket-admissibility, with the strict ambient product foreclosed by
Lemma~\ref{lem:tate-casimir-obstruction}). (4) Maurer--Cartan diagonal
kernel: the formal Casimir
\(C_{\mathfrak h}=\sum_{d\ge1}\sum_i e_{d,i}\otimes e^i_d\) converges
in the chosen completion (conditional on a regulated coefficient pair,
e.g.\ the weighted Tate envelope of
Theorem~\ref{thm:wt-cotangent-lift}). (5) Bar-cobar/Koszul Quillen
upgrade (Theorem~\ref{thm:relative-filtered-koszul-lift}; conditional
on Tate/nilpotent/weighted completion plus bounded inverse-limit
hypotheses). The eight-component vector
\(\operatorname{Ob}_T\) of \S\ref{ssec:tate-residual-resolution}
records exactly which rung is reached and which obstruction blocks the
next.

\begin{rmk}[Type discipline: \(c_f\) versus \(u_f\)]\label{rmk:type-discipline-cf-uf}
  The dictionary distinguishes two avatars of a Hamiltonian
  \(f\in\widehat{\mathfrak h}\) on the closed side. The
  Hamiltonian-dual CE coordinate \(c^f\) maps under the cotangent CE/PV
  comparison to the Schouten polyvector coordinate \(\theta^f\) on
  \(\widehat{\mathbf S}(\bar A)\); its physical role is the generator
  of an infinitesimal canonical motion, recorded as a vector field on
  the brane phase space. The shifted-cotangent-dual CE coordinate
  \(u_f\) maps to the boundary observable \(O_f=J(f)\); its physical
  role is the gauge-invariant scalar reading of \(f\) on the matrix
  probe pair, the brane measurement. The assignment \(c^f\mapsto O_f\)
  is the type error
  (Theorem~\ref{thm:reduced-ce-pv-central-operation},
  Proposition~\ref{prop:ce-source-obstruction}): a vector field is not
  an observable, and a perfect Hamiltonian Lie algebra has no closed
  length-one CE cochain in the Hamiltonian direction, so the closed
  Hamiltonian generator must enter the boundary algebra through the
  cotangent direction \(u_f\), not the Hamiltonian direction \(c^f\).
\end{rmk}

\begin{prop}[Geometry forced by the matrix microscope]\label{prop:local-model-mixed-definition}
  The geometry \eqref{eq:setup-X-L} is the unique formal local mixed
  completion carrying the matrix microscope datum
  \((\Omega_N,\operatorname{Tr})\) of the brane stack: the open
  observable family is forced to the brane substitution
  ...
  factorization-algebra shape is forced by the three independent
...
Under the matrix-microscope hypotheses of \S\ref{ssec:local-setup} ---
...
labels carry distinct module structures: the polynomial one-\(\psi\)
descendant \(\Psi_{a,b}\) of
Construction~\ref{constr:boundary-evaluation} and the Matlis residue
class \(\rho_{a,b}\) live in different
\(\widehat{\mathfrak h}\)-modules, and conflating them is the
type/module conflation
...
the polynomial PBW shadow, because the Moyal coadjoint action acts on
the residue module and does not lift to the polynomial substitute.
...
Theorem~\ref{thm:closed-bv-canonical-transformation} fixes the closed
BV field complex \eqref{eq:closed-BV-field-complex}; the next question
is which local functional generates its classical dynamics. The
shifted-cotangent Lie algebra
$\mathfrak g=\widehat{\mathfrak h}\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]$
carries a canonical degree-$(-1)$ evaluation pairing
$\langle\beta,f\rangle=\beta(f)$ between cotangent and Hamiltonian
factors. Wedging the form factors of $\mc E_{\mathrm{cl}}$ and
integrating over
$\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}$ against $dz_1\wedge
dz_2$ extends this to a degree-$(-1)$ local pairing on the closed BV
complex \eqref{eq:closed-BV-field-complex}. Under cyclicity, locality,
and linearity in the cotangent partner, exactly one cyclic local
functional, modulo $D$-exact additions, has its classical master
equation equal to the Maurer--Cartan equation of $\mathfrak g$. That
functional is the BF action.
...
moment-map operation is forced by the Dirac equal-time bracket of
Lemma~\ref{lem:dirac-probe-reduction} together with
Remark~\ref{rmk:equal-time-locality}, not postulated.
...
\begin{thm}[Matrix-microscope measurement theorem: uniqueness of \(J\)]\label{thm:Jf-coordinate-uniqueness}
  ...
  (U1)--(U5).  The categorical terminal-object refinement, including
...
\begin{proof}
  \emph{Invariant theory (U1).}  Proposition~\ref{prop:stable-trace-invariants}
  restricts \(\widetilde J(f)\) to a polynomial in cyclic words on
  \((\phi_1,\phi_2)\) modulo finite-rank trace identities, with stable
  representative an element of
  \(\mathbf S(\operatorname{Cyc}(F)_{\mathrm{nonempty}})\).

  \emph{Dirac collapse (U2).}  Restriction to the moment-map zero-fibre
  with adjacent-swap exactness (Lemma~\ref{lem:adjacent-swaps})
  collapses every cyclic word to a commutative monomial trace
  \(\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\)
  (Corollary~\ref{cor:matrix-evaluation-well-defined}); hence
  \(\widetilde J(f)\in\mathbf S(V_H)\subset A_\partial\), the symmetric
  algebra on the connected single-trace generators \(V_H\) of
  Proposition~\ref{prop:brane-ops}.

  \emph{Scalar reduction (U3).}  \(A_\partial\) is the quotient of
  \(\mathbf S(V_H)\oplus\C\cdot\overline{\operatorname{Tr}(1)}\) by
  \(\C\cdot\overline{\operatorname{Tr}(1)}\); multi-trace products of
  nonempty generators are unaffected.

  \emph{Darboux naturality and single-trace forcing (U4).}  The formal
  Darboux symplectomorphism group
  \(G_{\mathrm{Darb}}=\operatorname{Symp}_{\mathrm{form}}(\widehat{\C^2}_0)\)
  acts on \(A_\partial\) through matrix substitution
  \(\phi_i\mapsto\Phi_i(\phi_1,\phi_2)\), an algebra homomorphism with
  \(\Phi_*\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}
   =\overline{\operatorname{Tr}\,(\Phi^*f)(\phi_1,\phi_2)}\), so
  \(V_H\) and \(\mathbf S^{\ge2}(V_H)\) are both
  \(G_{\mathrm{Darb}}\)-stable.  By (U5), \(\widetilde J(z_i)\in V_H\);
  by (U4),
  \(\widetilde J(\Phi^*z_i)=\Phi_*\widetilde J(z_i)\in V_H\) for every
  \(\Phi\in G_{\mathrm{Darb}}\).  The Hamiltonian flows generated by
  \(H_k^{(1)}=-z_2^{k+1}/(k+1)\) and \(H_k^{(2)}=-z_1^{k+1}/(k+1)\)
  act by
  \[
    \Phi^*_{H_k^{(1)}}z_1=z_1-tz_2^k,\qquad
    \Phi^*_{H_k^{(2)}}z_2=z_2+tz_1^k,
  \]
  with matrix substitutions \(\phi_1\mapsto\phi_1-t\phi_2^k\),
  \(\phi_2\mapsto\phi_2+t\phi_1^k\) on the Dirac-reduced commuting
  pair.  Coefficients of \(t\) in the (U4) identity yield
  \(\widetilde J(z_i^k)=\overline{\operatorname{Tr}\phi_i^k}\) for every
  \(k\ge1\), and the composite flow
  \(\Phi^*_{H_1^{(2)}}z_2^k=(z_2+tz_1)^k=
   \sum_{a+b=k}\binom{k}{a}t^az_1^az_2^{b}\) matched against
  \(\overline{\operatorname{Tr}(\phi_2+t\phi_1)^k}=
   \sum_{a+b=k}\binom{k}{a}t^a\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\)
  yields
  \(\widetilde J(z_1^az_2^b)=\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\)
  at every bidegree.  Continuity in the
  \(\widehat{\mathfrak h}\)-topology extends to formal power series.

  \emph{Linear normalization (U5).}  By Procesi--Razmyslov on the
  commuting variety
  (Proposition~\ref{prop:stable-trace-invariants},
  Lemma~\ref{lem:adjacent-swaps}),
  \(V_H\) at bidegree \((a,b)\) is the line
  \(\C\cdot\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\); the (U5)
  values give scalar \(1\) at degree one, and (U4) gives \(1\) at
  every bidegree.  \(\widetilde J=J\).
\end{proof}

\begin{thm}[Hypothesis rigidity for the matrix-microscope measurement]\label{thm:Jf-uniqueness-rigidity}
  Only (U4) and (U5) are substantive in
  Theorem~\ref{thm:Jf-coordinate-uniqueness}: removing either admits a
  positive-dimensional family of solutions \(\widetilde J\ne J\).
  Hypotheses (U1), (U2), (U3) are codomain specifications, each
  implied by (U4)+(U5) on an enlarged ambient: on
  \(\C[\lie{gl}_N^2]/\langle[\phi_1,\phi_2]\rangle\),
...
\begin{proof}
  ...
  \[
    ...
    \text{ for }(a,b)\ne(1,1).
  \]
  For every \(\alpha\in\C\) this satisfies (U1), (U2), (U3), (U5):
  \(\overline{\operatorname{Tr}\phi_1}\,\overline{\operatorname{Tr}\phi_2}
   \in\mathbf S^{\ge2}(V_H)\) is \(GL_N\)-invariant, Dirac-reduced,
  scalar-reduced; bidegree \((1,1)\) lies outside (U5).  Under
  \(z_2\mapsto z_2+tz_1\) acting on \(z_2^2\), the source-side
  \(t^1\)-coefficient is
  \(2\,\overline{\operatorname{Tr}(\phi_1\phi_2)}
   +2\alpha\,\overline{\operatorname{Tr}\phi_1}\,
    \overline{\operatorname{Tr}\phi_2}\)
  while the matrix-side gives
  \(2\,\overline{\operatorname{Tr}(\phi_1\phi_2)}\); equality requires
  \(\alpha=0\).

  \emph{Removing (U5).}  The family
  \(\widetilde J^{\mathrm{rs}}_\lambda=\lambda\,J\),
  \(\lambda\in\C^\times\), satisfies (U1)--(U4); (U5) requires
  \(\lambda=1\).
...
The reduced bulk-to-brane evaluation
\(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) of
\S\ref{ssec:reduced-brane-coupling} matches the closed Hamiltonian
bracket on \(\widehat{\mathfrak h}\) with the brane \(P_0\)-bracket on
\(A_\partial^{\mathrm{Ham}}\) (Theorem~\ref{thm:brane-measurement-principle},
Theorem~\ref{thm:hamiltonian-current-factorization}). The unreduced
cotangent extension of this match --- a strict, all-window,
support-local cochain-level morphism of factorization algebras
\[
  \Phi_{\mathrm{cl}}\colon
  \mathrm{Obs}^{\mathrm{cl}}_{\mathrm{closed},H}\big|_L
    \longrightarrow
...
\]
realising the cotangent direction
\(\mathfrak h^\vee_{\mathrm{cont}}[1]\) at the chain level, with all
\(P_0\)-centrality homotopies and a boundary realisation of the
shifted-cotangent generators --- is the construction problem
Problem~\ref{prob:formal-factorization-center}.

This is a vector obstruction problem.  Each entry is a named
cohomology class, signed-row defect, finite-window matrix equation, or
\(\varprojlim^1\) class living in an explicit complex; joint vanishing
with chosen null-homotopies constructs the morphism, and a nonzero
entry is the precise construction defect.  The eight-component
decomposition below packages the necessary-and-sufficient obstruction
vector
...
The first certified nonzero entry is the Casimir-kernel block
\(\mathfrak o_{\mathrm{Cas}}\): the strict product/direct-sum Tate pair
\((\mathfrak h_\Pi,D_\oplus)\) carries no coefficient kernel projecting
to every finite Casimir
\(C^M_{\mathfrak h}=\sum_{d\le M}\sum_i e_{d,i}\otimes e^i_d\)
(Lemma~\ref{lem:tate-casimir-obstruction},
Theorem~\ref{thm:strict-unweighted-no-go}); the strict-unweighted
endpoint is therefore foreclosed.  The all-window analytic
no-go is Theorem~\ref{thm:bmk-finite-window-no-go}: the strict
all-window support-local Hamiltonian transfer fails on every
choice of one-pair principal-part datum, with obstruction class
\(\operatorname{Ob}^\Pi_{\mathrm{BM}}\) detected on the BMK ladder.
The supremum form of this no-go is
Corollary~\ref{cor:promatlis-retract-as-supremum}: the pro-Matlis
retract is the maximal admissible refinement, and any larger
strict refinement is blocked by a named entry of
\(\operatorname{Ob}^\Pi_{\mathrm{BM}}\).  This is the precise sense in
which the unreduced cotangent lift fails on the strict ambient product
completion and succeeds only inside a controlled refinement: the
weighted Tate completion (Theorem~\ref{thm:wt-cotangent-lift}), the
nilpotent truncation \(\mathfrak m^3/\mathfrak m^{N+1}\), or the
pro-Matlis retract.

Each component below follows the template: the desired upgrade, the
minimal input data, the obstruction class living in a precise complex,
the vanishing criterion, the first nonzero example (or the precise
construction problem inscribed if no example is yet computed), and the
physical meaning.  Theorem~\ref{thm:tate-cotangent-lift-iff} assembles
the components into the iff statement; the proved sublattice and
remaining residual are recorded in
Corollary~\ref{cor:tate-residual-sublattice}.
...
\(\mathfrak C_{\mathrm{tar}}\).  The full firewall theorem
...
frontier, the Borcherds/BKM--Igusa bridge, and the GW/DT/PT/MNOP lane
...
obstruction vector]\label{thm:tate-cotangent-lift-iff}
  ...
  \begin{itemize}
    ...
    \item \emph{Minimal input data.} The reduced bulk-to-brane
      ...
      together with chosen compatible null-homotopies for each named
      component class.
    ...
    \item \emph{Physical meaning.}  The matched-bracket open-closed
      ...
      \(C_{\mathfrak h}\) fails to converge in
      \(\mathfrak h\widehat\otimes\mathcal P\) until a regulator is
      installed.  Vanishing of \(\operatorname{Ob}_T\) names exactly
      the regulator data --- weight system, propagator extension,
      truncation, primitive shadow, parametrix expansion, protected
      anomaly cancellation, and matched-conventions descent --- under
      which the unreduced lift is constructed.
  \end{itemize}
\end{thm}
...
formal symplectic polydisk: its operator algebra is generated by cyclic
words in two \(\mathfrak{gl}_N\)-valued probe matrices, and its Weyl
quantisation produces a scalar shift \(\hbar N\) that the connected
single-trace projection cannot absorb.  Procesi--Razmyslov 1976
(\cites{procesi,razmyslov}; Proposition~\ref{prop:stable-trace-invariants})
fixes the classical content of the matrix probe operator side; Capelli
...
\begin{enumerate}[wide=0pt,leftmargin=*]
  ...
  \item \emph{Admissible Tate CE/bar-cobar.}  The cochain layer is
    ...
    Schouten bracket and pointwise product, with heat-kernel propagator
    extension and Casimir convergence --- is the weighted-Tate completion
    \(B_w\) (Theorem~\ref{thm:wt-canonical-habitat}).
  ...
  \item \emph{PBW/Rees special-fibre shadow.}  The Rees enveloping
    ...
    \((pb-qa)\).  The PBW shadow is not the coadjoint module; no
    Hamiltonian-equivariant relabelling exists.
  ...
  \item \emph{Brane-line factorisation-current comparison.}
    ...
    verdict \(H^1=\C\!\cdot\!E_{\theta_3}\ne0\) and the
    supplied-counterterm verdict \(H^1=0\)
...
finite-certificate degree-zero Hamiltonian trace comparison.  The
...
non-scalar window \(\theta_3\) splits into a bare verdict
\(H^1=\C\!\cdot\!E_{\theta_3}\ne0\) and a supplied-counterterm verdict
\(H^1=0\) with \(c=1\) unique
(Theorem~\ref{thm:app-theta-three-finite-window-H1}).

\medskip\noindent
\emph{The center of mass remembers what Hamiltonian reduction
forgets.}  On the closed side, the Hamiltonian vector field of every
constant is zero; the further quotient
\(\bar A=\C[z_1,z_2]/\C\cdot1\) removes the unit from the source.
On the brane side, the unit \(1\in\C[z_1,z_2]\) lifts to
...
\]
while \(\{z_1,z_2\}_{\bar A}=0\) modulo constants.  Quantising the same
linear generators by the Moyal star and reading the Weyl commutator,
\[
  [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N,
\]
...
\begin{prob}[Hamiltonian specialization datum for Costello perturbative BV]\label{prob:analytic-graph-realization}
  \begin{itemize}
    \item \emph{Desired upgrade:} a Costello-type closed-open BV
      ...
      sector \(\R^2\times\C^2\) of \S\ref{sec:setup}.
...
The order-\(\hbar\) coefficient \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\)
identifies the linear-generator anomaly; the all-order question is
whether the same identification persists at orders \(\hbar^3,\hbar^5,\dots\).
...
\begin{thm}[Componentwise quantum coefficient comparison]\label{thm:componentwise-quantum-coefficient-comparison}
  ...
  \begin{itemize}
    \item \emph{Components proved compatible:} weighted BV kernel
      ...
      \(A^{\mathrm{pp},w}_{\partial,\hbar}(I)\); finite-certificate
    ...
    \item \emph{Minimal input data:} weighted Tate completion
      ...
      finite-certificate enumeration cited in clause~(4).
    \item \emph{Obstruction classes outside this product:} the
      ...
      H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}})\)
      (Problem~\ref{prob:quantum-p0-operation-realization}); the
      \(\theta_3\) bare-verdict / supplied-counterterm dichotomy
      (Theorem~\ref{thm:app-theta-three-finite-window-H1}).
    \item \emph{Vanishing criterion for the four named components:}
      ...
      finite-certificate range listed in
    ...
    \item \emph{First nonzero example outside the proved components:}
      ...
      \(\theta_3\), where the bare verdict is \(H^1=\C\!\cdot\!E_{\theta_3}\ne0\).
    \item \emph{Physical meaning:} the Moyal star, the matrix Weyl
      algebra, and the brane trace measurement agree
      componentwise on the proved completion; mismatch on either of the
      four named obstruction classes is the precise location at which
...
\begin{rmk}[Reproducibility of the finite Moyal/radial verification]\label{rmk:reproducibility-certificate}
  The finite-certificate clause of
  Theorem~\ref{thm:componentwise-quantum-coefficient-comparison} is
  computer-assisted.  The harness:
  \begin{enumerate}[label=(\arabic*)]
    \item \emph{Arithmetic.}  Exact rational arithmetic over
      \(\mathbb Q\), implemented in Python~3 with the standard
      \texttt{fractions} module; no floating-point step.
    \item \emph{Moyal coefficient sweep.}
      \texttt{scripts/check\_moyal\_coefficients.py} runs the
      Procesi--Razmyslov--Capelli round-trip and the prequotient
      scalar Moyal leading expansion on the rectangular bidegree
      window \(0\le k,l,m,n\le\texttt{max\_exp}=10\), to Moyal order
      \(r\le\texttt{max\_order}=11\); the script asserts the
      coefficient identities of
      Proposition~\ref{prop:app-edge-pbw-telescoping} and
      Theorem~\ref{thm:phi-hbar-all-order} pointwise on the sweep and
      exits zero on PASS.
    \item \emph{One-\(\psi\) homology sweep.}
      \texttt{scripts/check\_one\_psi\_homology.py} verifies the
      descendant action and the residue-coadjoint formula
      \(z_1^pz_2^q\!\cdot\!\rho_{a,b}=
        -(pb-qa+p-q)\rho_{a-p+1,b-q+1}\) of
      Theorem~\ref{thm:principal-part-coadjoint-uniqueness} on the
      rectangular window \(0\le k,l\le\texttt{max\_exp}\) with
      \texttt{max\_exp=5} as the script default, exact rational;
      exits zero on PASS.
    \item \emph{Per-bidegree representation.}  The radial Stokes
      boundary
      \(B_{a,b}\colon\mathsf S_{a,b}\!\oplus\!\mathsf W_{a,b}\to
        \mathsf T_{a,b}\), the decorated weight \(C^+_{a,b}\), and the
      principal-part edge tower of
      Proposition~\ref{prop:app-edge-pbw-telescoping} are built
      explicitly as rational matrices; \(\ker B_{a,b}^*\) and
      \(\operatorname{coker}B_{a,b}\) are recovered by exact Gaussian
      elimination on \(\mathbb Q\)-arrays of \texttt{Fraction} entries.
    \item \emph{Reproduction.}  Run each script from the repository
      root.  A nonzero exit indicates either a coefficient regression
      against the stated identities or a transcription drift on the
      bidegree window above.
  \end{enumerate}
  No claim is made outside the rectangular sweep window above; the
  all-bidegree obstruction
  \(\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}\) is the
  named cohomology class blocking extension to the full bidegree
  lattice
  (Proposition~\ref{prop:app-radial-dual-potential-obstruction}).
\end{rmk}
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  ...
  \begin{itemize}
    ...
    \item \emph{First nonzero example or known failure:} the
      smallest non-scalar window \(\theta_3\) carries the bare verdict
      ...
      \(\widetilde\lambda_{02,\mathfrak b}\) that cannot be repaired
      inside the current Bianchi-exposed completion.
...
\addcontentsline{toc}{section}{Closing thesis}

A Dirac brane stack is a finite-matrix microscope for a formal
holomorphic-symplectic disk.  Its protected local measurement theorem
says that a Hamiltonian is measured by trace substitution into the
brane's matrix-valued transverse coordinate:
\[
  f\longmapsto J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)} .
\]
The shifted-cotangent closed BV theory separates canonical motion from
measurement: ghosts move, cotangent coordinates measure
(\(c_f\mapsto\theta_f,\ u_f\mapsto O_f\)).  Continuous duality turns
cotangent modes into Grothendieck--Matlis principal parts.  Scalar
reduction leaves a centre-of-mass cocycle \(\hbar N[\bar c]\).  Moyal
quantization tests, bidegree by bidegree, whether Weyl ordering
commutes with brane trace measurement.  Every stronger global,
quantum, chain-level, all-bidegree, modular, or CY-categorical claim is
a transport problem for the local Maurer--Cartan datum
\(\Theta_{\mathrm{loc}}\) into a larger target deformation complex
\(\mathfrak K_{\mathcal T}\).  The named obstruction classes
\(\hbar N[\bar c]\), \(\Omega^{\mathrm{rad}}_{a,b}\),
\([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\),
\(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\),
\(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\), and
\(\operatorname{per}_X(v)\) are the curvature components of those
transports.  Vanishing gives strict coherence; null-homotopy gives
counterterm coherence; central exactness gives projective or
gerbe-valued coherence; non-absorptive curvature forces target
replacement.  Volumes I, II, and III supply the algebraic, physical,
and Calabi--Yau categorical arenas in which these transports live;
the local theorem of this manuscript is the formal Hamiltonian
Darboux brane-probe chart of the last arrow of that chain, and never
bypasses the functorial gates between volumes.

The deepest sentence is the same in every voice the manuscript is
written in:
\[
  \boxed{\;
  \text{The brane does not represent the Hamiltonian algebra; it evaluates Hamiltonians on its matrix-valued transverse coordinate.}
  \;}
\]
Everything above is the deformation theory of making that sentence
global, quantum, and coherent.  An obstruction is the memory of
structure erased by an over-strict target; the master deformation
complex is the language in which that memory is computed.
...
and Harish-Chandra radial-parts theorem supply the matrix-microscope
side; Matlis 1958 and Hartshorne local cohomology supply the
principal-part dual; Kontsevich's deformation quantisation supplies
the all-order Moyal target.  The local Hamiltonian BF analysis is a
finite algebraic chain-level probe of the formal-polydisk corner of
that programme.  Its single measurement principle is that a closed
Hamiltonian on the formal symplectic normal polydisk is evaluated on
the brane's matrix-valued transverse coordinate; the obstructions to
extending this principle are the named classes
\(\mathfrak o_{\mathrm{Cas}}\), \(\hbar N[\bar c]\),
\(\Omega^{\mathrm{rad}}_{a,b}\),
\([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\), and the
\(\theta_3\) finite-window class.
