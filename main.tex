...
\end{titlingpage}
  ...
  \(s\colon\bar A\to\C[z_1,z_2]\) and
  \(J_N^s(f)=[\operatorname{Tr}s(f)(\phi_1,\phi_2)]\),
  \(\{J_N^s(f),J_N^s(g)\}
...
Four symbols are reserved.  The finite-\(N\) dashed generator boundary
...
\(u_f\mapsto M_{J_N^s(f)}\).  The symbol
...
\[
  \tau_{N,1}(f)=X_{f,N},\qquad
  u_f\longmapsto M_{J_N^s(f)}
\]
extends to a Hochschild-centre twisting cochain when the centrality
defects admit chosen primitives:
\[
...
is denoted \(\Phi_N^{\mathrm{gen}}\); the twisting cochain with these
primitives is \(\Phi_N^{\mathrm{Hoch}}\).
...
\label{prop:finite-N-leftover-residue}
  ...
  generator assignment \(\Phi_N^{\mathrm{gen}}\) determines a curvature
  class
  \[
    \kappa_N^{\mathrm{cent}}(\Phi_N^{\mathrm{gen}})
    \in H^2(\mathfrak C^{\mathrm{cent}}_N).
  \]
  Its three low-degree components are represented by
  \(\delta^{\mathrm{Lie}}_{x,y}\),
  \(\delta^{\mathrm{prod}}_{x,a}\), and
  \(\delta^{P_0}_{x,a}\).  The low-degree centrality datum consists of
  primitives \(H_{x,y}\), \(H^{\mathrm{prod}}_{x,a}\), and
  \(H^{P_0}_{x,a}\) whose centrality differentials kill these
  representatives, together with the
  completion compatibility required in
...
\begin{thm}[Trace-sector generator boundary value and CE/PV reduction]\label{thm:trace-sector-generator-boundary-value}
  ...
  \[
    J_N^s(f)=
      \bigl[\operatorname{Tr}\,s(f)(\phi_1,\phi_2)\bigr],
    \qquad
  ...
  \(u_f\mapsto M_{J_N^s(f)}\), by \(\Phi_N^{\mathrm{gen}}\).  Its
...
\begin{proof}
  ...
  supplies the Koszul cocycle \(X_{f,N}\), and the matrix Poisson
  calculation gives the two fixed-rank identities displayed in the
  theorem.  These constitute the arity-one Hamiltonian coefficient and
  the coordinate-dual trace boundary value.
...
\begin{equation}\label{eq:setup-Jf-flagship}
  J_N^s(f)=\bigl[\operatorname{Tr}\,s(f)(\phi_1,\phi_2)\bigr],
  \qquad
  J^{\mathrm{st}}(f)=O_f.
\end{equation}
...
\label{def:hamiltonian-cyclic-koszul-lie-model}
  ...
  carries sign \(+1\) on ordinary Hamiltonian words.  This is the
  Hamiltonian convention induced by
  \(\omega(x,y)=1\), \(\omega(y,x)=-1\), and
  \(\iota_{X_F}\omega=-dF\); the letter \(\psi\) is unpaired.
\end{defn}
...
\begin{proof}
  ...
  \[
    ...
    X_F(\psi)=0 .
  \]
  Thus the smallest coordinate calculation already fixes the signs:
  \[
    X_{[x]}(y)=[1]=\{[x],[y]\}_{\mathrm{neck}}.
  \]
  A noncommutative word calculation retains the ordering information.  For
  \(F=[x^2y]\) and \(G=[xy^2]\),
  \[
    \partial_xF=xy+yx,\quad \partial_yF=x^2,
    \qquad
    \partial_xG=y^2,\quad \partial_yG=yx+xy,
  \]
  and therefore
  \[
    \{F,G\}_{\mathrm{neck}}
  ...
  and pairing the four possible second-cut positions.  Jacobi also follows
  directly from the constant double bracket on \(T(E_{\mathrm K})\) whose
  only nonzero generator values are
  \(\{\!\{x,y\}\!\}=1\otimes1\) and
  \(\{\!\{y,x\}\!\}=-1\otimes1\).  Its double Jacobiator vanishes on
  generators and hence on the tensor algebra; multiplication followed by
  the graded cyclic quotient is exactly the displayed necklace bracket.
  The scalar word must be present:
  \(\{[x],[y]\}_{\mathrm{neck}}=[1]\).
...
\begin{proof}
  ...
  \[
    N\{F,G\}_{\mathrm{neck}}
      ...
      =\{N(F),N(G)\}.
  \]
  The same derivative identities show that commutativizing the Hamiltonian
  generator values gives
  \[
    X_F(x)\longmapsto-\partial_{z_2}N(F),
    \qquad
    X_F(y)\longmapsto\partial_{z_1}N(F).
  \]
  These are the formal-Darboux and finite-rank Hamiltonian signs fixed by
  \(\iota_{X_f}(dz_1\wedge dz_2)=-df\).  Thus the normal-ordering map,
  the matrix trace bracket, and the scalar-contact term all have the same
  bracket orientation.
...
\label{lem:gauge-paired-necklace-plain-quotient-obstruction}
  ...
  \(\langle a,\psi\rangle=\langle\psi,a\rangle=1\) to the necklace bracket.
  The map that sends
...
\begin{proof}
  ...
  \[
    X_F(x)=-\partial_yF,\qquad
    X_F(y)=\partial_xF,\qquad
    X_F(\gamma)=(-1)^{|F|}\partial_\psi F,\qquad
    X_F(\psi)=(-1)^{|F|}\partial_\gamma F .
  \]
  Cyclic derivatives are taken in the graded cyclic quotient; moving the
  cut word past the removed letter gives the Koszul sign.  Equivalently,
  the inverse pairing gives
  \[
    \{F,G\}_{\mathrm{neck}}
      =\bigl[
       \partial_xF\,\partial_yG-\partial_yF\,\partial_xG
       +(-1)^{|F|}\bigl(
         \partial_\gamma F\,\partial_\psi G
         +\partial_\psi F\,\partial_\gamma G\bigr)
      \bigr]_{\mathrm{cyc}},
  \]
  so \(\{[x],[y]\}_{\mathrm{neck}}=[1]\) while
  \(\{[\gamma],[\psi]\}_{\mathrm{neck}}=-[1]\).  With these
  ...
  \]
  on the four generators \(x,y,\gamma,\psi\).  The corresponding constant
  graded double bracket is determined by the displayed even pairing.  Its
  double Jacobiator vanishes on generators and hence on the tensor algebra;
  multiplication followed by the graded cyclic quotient gives the displayed
  necklace bracket.  Thus the necklace bracket satisfies Jacobi.

  Put \(\mu=xy-yx\).  For
  \(S=-[\gamma\mu]-[\psi\gamma\gamma]\), the same
  cyclic-derivative convention gives
  \[
    \partial_\psi S=-\gamma^2,\qquad
    \partial_yS=x\gamma-\gamma x,\qquad
    \partial_xS=\gamma y-y\gamma,\qquad
    \partial_\gamma S=-\mu-\gamma\psi-\psi\gamma .
  \]
  ...
  \(Q^2y=0\).  Moreover \(Q\mu=\gamma\mu-\mu\gamma\), whence
  \[
    Q^2\psi
  ...
  \]
  The master equation can also be read directly in cyclic words:
  \[
    \frac12\{S,S\}_{\mathrm{neck}}
      =\bigl[(\gamma y-y\gamma)(x\gamma-\gamma x)
       -(\mu+\gamma\psi+\psi\gamma)\gamma^2\bigr]_{\mathrm{cyc}}.
  \]
  The first summand is \([\mu\gamma^2]_{\mathrm{cyc}}\), and
  \[
    [\,(\gamma\psi+\psi\gamma)\gamma^2\,]_{\mathrm{cyc}}=0,
    \qquad
    [\gamma\psi\gamma^2]_{\mathrm{cyc}}
      =-[\psi\gamma^3]_{\mathrm{cyc}}.
  \]
  Hence \(\{S,S\}_{\mathrm{neck}}=0\), and
  \(Q=\{S,-\}_{\mathrm{neck}}\) is a square-zero derivation of the
  necklace bracket.
...
At finite \(N\) the polynomial trace map
\(J_N^s(f)=\operatorname{Tr}\,s(f)(\phi_1,\phi_2)\), \(f\in\bar A\),
requires a word ordering of the noncommuting
...
\[
  ...
  -\operatorname{Tr}(u\phi_2\phi_1v),
\]
so \(J_N^s(f)\) depends only on the symbol \(f\).
...
u_f\mapsto M_{J_N^s(f)})\); the notation
...
\begin{thm}[Fixed-rank boundary values and scalar-stable formal-Darboux coordinates]\label{thm:main-local}
  ...
  \(X_{f,N}\in Z^1C^\bullet_{\mathrm{Hoch}}
  (A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) with
  \begin{equation}\label{eq:schouten-explicit}
    ...
    \qquad [Q,X_{f,N}]=0.
  \end{equation}
  Their fixed-rank action retains the scalar contact:
  ...
  boundary value \(u_f\mapsto M_{J_N^s(f)}\).  The trace-identity ideal,
...
\begin{proof}
  ...
  \(1\)-cochain, and multiplication by \(J_N^s(f)\) defines a Hochschild
  ...
  \(X_{f,N}\) and \(M_{J_N^s(f)}\); a Hochschild cochain map is the
  ...
  cup product and the shifted Gerstenhaber bracket of degree \(0\) on
  \(C^\bullet_{\mathrm{Hoch}}[1]\); in unshifted notation the generator
  identities are
  \[
    [X_{f,N},X_{g,N}]_G=X_{\overline{\{s(f),s(g)\}},N},
    \qquad
    X_{f,N}(J_N^s(g))
      =J_N^s\!\left(\overline{\{s(f),s(g)\}}\right)
       +N\omega(f,g)\mathbf1 .
...
\begin{defn}[Admissible Hochschild-centre lift]\label{defn:admissible-hochschild-centre-lift}
  ...
  The finite-rank boundary datum begins with the first Taylor coefficient
  \[
    \tau_{N,1}\colon\mathfrak g\longrightarrow
    C^\bullet_{\mathrm{Hoch},\mathrm{adm}}
      (A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})[1],
    \qquad
    \tau_{N,1}(f)=X_{f,N}\quad(f\in\mathfrak h),
  \]
  together with the coordinate-dual boundary value
  \(u_f\mapsto M_{J_N^s(f)}\).  Let \(\tau_N^{\mathrm{gen}}\) denote
  the resulting arity-one element of the convolution algebra.  An
  admissible Hochschild-centre lift is a Maurer--Cartan element
  \[
    \mathcal C_{\Phi_N^{\mathrm{Hoch}}}
    =\tau_N^{\mathrm{gen}}
      +H^{\mathrm{Lie}}+H^{\mathrm{act}}+H^{\mathrm{prod}}
  ...
  \[
    \{\tau_{N,1}(x),M_a\}_{P_0}-M_{\rho_x^{P_0}(a)}
      =dH^{P_0}_{x,a}.
  \]
  For invariant centre classes, \(\rho_x=\rho_x^{P_0}=0\) and these
  equations reduce to null-centrality.  The term $H^3$ is the order-three
...
\label{prop:low-degree-centre-lift-obstruction}
  The boundary coefficient \(\tau_{N,1}\) extends to a
  ...
  \[
    \operatorname{ob}^{\leq3}_{\mathrm{cent}}(\tau_{N,1})
      :=
      \bigl[
        \delta^{\mathrm{Lie}},
        \delta^{\mathrm{act}},
  ...
  \(\rho_x,\rho_x^{P_0}\) vanish and the equations specialize to strict
  centrality; general closed currents satisfy the displayed
  homotopy-equivariance equations.
\end{prop}
...
pair is \(f\mapsto X_{f,N}\), \(u_f\mapsto M_{J_N^s(f)}\); its extension
...
\begin{proof}
  ...
  has fixed-rank boundary values \(u_f\mapsto M_{J_N^s(f)}\) and
...
\input{appendix-theorem-status-referee-checklist}
