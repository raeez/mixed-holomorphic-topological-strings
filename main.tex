...
\emph{topological} and \emph{holomorphic} each name the unique
factorization habitat compatible with one of the locality
differentials of Proposition~\ref{prop:three-localities}.  This is the
local string-field-theoretic use of the word ``string'', not a global
theory of worldsheet moduli or amplitudes.  The protected algebraic
content under Theorem~\ref{thm:bv-replacement-unitarity} is the
classical and quantum master equations, locality, and factorization
associativity; what \emph{topological} and \emph{holomorphic} add is
the constraint on how local operators vary across spacetime, forced by
the locality differentials.
...
The Hamiltonian sector at a single brane point fixes the stability
discipline used in every later large-\(N\) statement.  Coefficient
algebras: \(\bar A=\C[z_1,z_2]/\C\cdot1\) on the polynomial side and
...
named in \S\ref{ssec:two-algebras}.
...
\(O_{k,l}\).  The U(1) center class \([\bar c]\) records the scalar
invisibility/anomaly: invisible after empty-trace projection, surviving
as the Moyal/Capelli shift \(\hbar N[\bar c]\) under quantization.
...
is structural, not an identification.  The three locality differentials
of Proposition~\ref{prop:three-localities} -- support, topological,
formal holomorphic -- are independent and force the mixed
factorization-algebra shape of
\(\mathcal F^{\mathrm{mix}}_{\mathrm{Ham}}\).  Componentwise quantum
coefficient comparison
(Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}) is the
quantization measurement.  Stronger statements are obstruction problems:
factorization-centre lift, non-scalar QME, all-bidegree PBW Stokes, and
native all-window BMK transfer.
