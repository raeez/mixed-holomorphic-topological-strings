...
\end{titlingpage}
  ...
  \(\mathrm{Ran}(\C^2)\), valued in \(E_2\)-algebras \textemdash{} a curve vertex
  ...
  compact-target lifts\textemdash{}BCOV holomorphic anomaly, Gopakumar--Vafa,
  Donaldson--Thomas\textemdash{}transport \(\Theta_{\mathrm{loc}}\) into the
...
\]
Every other piece of open-side structure\textemdash{}symplectic potential,
moment map, Dirac constraint, BV differential, gauge-invariant operator
algebra\textemdash{}is forced from this substitution.
...
antifield
\(\psi\in\mathfrak{gl}_N[1]\) \textemdash{} the BV antifield \(\psi=A^*\) of the
Lagrange multiplier \(A\), independent of \(\phi_1,\phi_2\) and not a
function of them \textemdash{} and let \(Q\) be the unique derivation defined on
generators by
...
coordinate envelope
\(C^\bullet_{\mathrm{CE},\mathrm{coord}}(\mathfrak g)\)\textemdash{}the product
over finite graded-commutative words in the CE coordinates\textemdash{}and the
...
one of them.  Hamiltonian ghosts \(c^I\)\textemdash{}dual to generators
\(H_I\in\widehat{\mathfrak h}\)\textemdash{}generate the closed motion on the
polydisk; cotangent coordinates \(u_I\)\textemdash{}dual to the shifted antifield
generators in \(\mathfrak h^\vee_{\mathrm{cont}}[1]\)\textemdash{}are the
...
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\cdot I\) in $M_N(\C)$ \textemdash{} four equivalent
...
completion forced by \(J\) \textemdash{} closed under the Schouten bracket and
pointwise product, with heat-kernel propagator extension and Casimir
convergence \textemdash{} is the weighted-Tate completion \(B_w\)
...
classes \textemdash{} the strict Casimir support obstruction \(C_{\mathfrak h}\),
...
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}, item (4)) \textemdash{} are
...
\begin{rmk}[Native bulk object on $\C^2$]\label{rmk:not-a-curve-voa}
  ...
  after a restriction datum\textemdash{}a holomorphic curve
  \(C\hookrightarrow\C^2\) with transverse boundary condition, or a
  factorization-homology pushforward\textemdash{}which singles out a
...
Under the brane-probe hypotheses \textemdash{} a brane stack carrying the
...
symplectic surface at the brane point \textemdash{} the geometry is forced by
...
in the chosen completion \textemdash{} realised by the weighted Tate envelope of
...
Under the brane-probe hypotheses of \S\ref{ssec:local-setup} \textemdash{}
...
\(GL_N\)-conjugation gauge symmetry generated by that moment map \textemdash{}
...
strict cotangent extension of this matched bracket \textemdash{} the unreduced
Tate-coefficient cotangent lift \textemdash{} is the obstruction calculus of
...
\begin{proof}
  ...
  \(\overline{\operatorname{Tr}((\phi_1')^p(\phi_2')^q)}
   +\overline{\operatorname{Tr}((\phi_1'')^p(\phi_2'')^q)}\)
  \textemdash{} this is the Loday--Quillen--Tsygan primitive on
  \(H^{\mathrm{CE}}_*(\lie{gl}_\infty(A_{\bar f}))\) \textemdash{} and a
...
cotangent extension of this match \textemdash{} a strict, all-window,
...
shifted-cotangent generators \textemdash{} is the construction problem
Problem~\ref{prob:formal-factorization-center}.
...
obstruction vector]\label{thm:tate-cotangent-lift-iff}
  ...
  \begin{itemize}
    ...
    \item \emph{Physical meaning.}  The matched-bracket open-closed
      ...
      exactly the regulator data \textemdash{} weight system, propagator
      ...
      \textemdash{} under which the unreduced lift is constructed.
  \end{itemize}
\end{thm}
...
four-window identification \textemdash{} Lie cohomology, classical trace
algebra, Capelli quantum shift, Moyal star product \textemdash{} is the
...
fail at a single class \textemdash{} the Tate Casimir
...
\begin{enumerate}[wide=0pt,leftmargin=*]
  ...
  \item \emph{Admissible Tate CE/bar-cobar.}  The cochain layer is
    ...
    completion forced by \(J\) \textemdash{} closed under the Schouten bracket
    and pointwise product, with heat-kernel propagator extension and
    Casimir convergence \textemdash{} is the weighted-Tate completion \(B_w\)
    (Theorem~\ref{thm:wt-canonical-habitat}).
...
obstruction \textemdash{} algebraic central extension, Lie central extension,
trace-algebraic boundary central extension, and quantum Weyl
commutator \textemdash{} realise the same class
...
\label{thm:scalar-qme-alternatives}
  \begin{itemize}
    ...
    \item \emph{Vanishing criterion:} either replace the scalar-reduced
      ...
      central-contact cone \textemdash{} each cancellation is a source
