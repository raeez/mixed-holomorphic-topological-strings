...
\end{titlingpage}
  ...
  A one-dimensional chiral algebra is obtained only after an
  additional restriction or pushforward to a curve
  $C\hookrightarrow\C^2$ has been constructed.  Such a construction
  must specify the transverse modes, the pushed-forward bracket, the
  BV pairing, the image of the brane chart, and the anomaly
  normalisation.  The formal-Darboux theorem by itself does not turn
  the chart \(A_b\cong\C[[z_1,z_2]]\) into a Heisenberg, affine
  Kac--Moody, \(\beta\gamma\), Virasoro, or \(W\)-algebra vertex
  algebra; \S\ref{sec:examples} records these as comparison targets
  with their extra hypotheses.
...
algebra on $\C^2$.  A curve vertex operator algebra (VOA) is a
separate one-dimensional object.  It can be compared with the native
factorization algebra only after a controlled reduction supplies
explicit $z_2$-mode or principal-part data, pushed-forward bracket,
BV pairing, brane image, and anomaly matching.  Only then do the Zhu
...
vertex algebras are not alternative formal-Darboux chart algebras.
They are curve chiral algebras which may be compared with the
Hamiltonian formal disk only after a separate curve reduction and
anomaly matching.  For a factorization algebra \(\mathcal X\) on
...
The examples are consistency checks on the formal-Darboux stalk
theorem.  They do not enlarge the theorem.  The theorem proved in
\S\ref{ssec:local-theorem} has three objects:
\[
  \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p},\qquad
  A^{\mathrm{cl}}_{\partial,N}
    =C^\bullet_{\mathrm{CE}}
  ...
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)} .
\]
Its native singular product is the two-variable diagonal
local-cohomology operation of
Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  A
one-dimensional vertex algebra appears only after a separate
restriction or pushforward to a curve, together with a choice of
fields, conformal weights, invariant form, stress tensor, and anomaly
normalisation.  The formal-Darboux theorem by itself does not produce
an affine Kac--Moody algebra, a \(\beta\gamma\) system, a Virasoro
algebra, or a \(W\)-algebra.

\subsubsection{Heisenberg current on the formal disk}\label{sssec:ex-heisenberg}

For \(N=1\) the brane chart is the formal disk
\[
  A_b=\widehat{\mc O}_{\C^2,p}\cong\C[[z_1,z_2]],
  \qquad
  (\phi_1,\phi_2)=(z_1,z_2).
\]
The trace is evaluation of the formal Hamiltonian:
\[
  J(z_1^a z_2^b)=z_1^a z_2^b .
\]
The closed-to-open dictionary is
\[
  c_f\mapsto\theta_f,\qquad u_f\mapsto J(f).
\]
Classically this is the strict formal-Darboux stalk of
Theorem~\ref{thm:main-local}.  After Weyl quantisation the relation
\[
  [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar
\]
is the one-pair Heisenberg relation.  Thus the central class is
\(\hbar[\bar c]\) if the central extension is retained; it vanishes
only after projecting to the scalar-reduced Hamiltonian algebra.  The
\(N\)-brane case multiplies this scalar by \(N\).  No vertex algebra
...
An affine Kac--Moody vertex algebra is not a brane chart of the
formal-Darboux stalk.  To obtain \(V_k(\mathfrak g_0)\) one must
choose a smooth curve \(C\), a finite-dimensional simple Lie algebra
\(\mathfrak g_0\), an invariant bilinear form \(\kappa\), and the
standard one-dimensional local cocycle
\[
  [J^a_m,J^b_n]
   = f^{ab}_{\ \ c}J^c_{m+n}
     +k\,m\,\kappa^{ab}\delta_{m+n,0}.
\]
These data are the defining data of the affine current algebra
\cite{frenkel-ben-zvi}*{\S 2.4}; they are not contained in the
formal coordinate ring \(\C[[z_1,z_2]]\) or in the trace map
\(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\).

Consequently the manuscript does not identify the brane number \(N\)
with the affine level \(k\), and it does not identify the Capelli
scalar \(\hbar N[\bar c]\) with the Kac--Moody cocycle.  Such an
identification would require a curve restriction functor from the
two-dimensional holomorphic factorization algebra, a pushforward of
the bracket, and an anomaly calculation comparing the residue
cocycle with \(k\kappa\).  Those data are outside the
formal-Darboux theorem.

\subsubsection{\texorpdfstring{\(\beta\gamma\)}{beta-gamma} system}\label{sssec:ex-betagamma}

The free \(\beta\gamma\) system is a chiral algebra on a curve.  For
a vector space \(V\), its fields are
\[
  \gamma\in V,\qquad \beta\in V^\vee\otimes K_C,
  \qquad
  \beta_i(z)\gamma^j(w)\sim \frac{\delta_i^j}{z-w}.
\]
This is a one-dimensional chiral construction.  The formal-Darboux
stalk supplies instead the two formal Hamiltonian coordinates
\((z_1,z_2)\) and their Weyl commutator after quantisation.  A
comparison with the rank-one \(\beta\gamma\) system requires a
polarisation, a curve \(C\hookrightarrow\C^2\), and a proof that the
two-variable diagonal principal part pushes forward to the Cauchy
kernel on \(C\).

Under such extra data one may compare the canonical pair
\((z_1,z_2)\) with \((\beta,\gamma)\).  The standard central charge
of a rank-\(r\) \(\beta\gamma\) system is \(2r\) after a choice of
stress tensor.  It is not the Capelli scalar
\(\hbar N[\bar c]\) of the formal-Darboux trace algebra.
...
\]
It is not a chart algebra \(A_b\) of the formal-Darboux brane.  A
Virasoro field appears only after a one-dimensional chiral theory has
been supplied with a stress tensor.  For example, a Heisenberg or
affine Kac--Moody algebra gives a Virasoro field by the Sugawara
construction under its usual hypotheses
\cite{frenkel-ben-zvi}*{\S\S 2.5, 3.4}.

The formal-Darboux theorem therefore proves no equality
\[
  \hbar N[\bar c]=c .
\]
The left-hand side is the finite-\(N\) scalar central extension of
the Hamiltonian trace algebra.  The right-hand side is the central
charge of a chosen curve chiral algebra.  A comparison between them
is an anomaly-matching problem, not a consequence of the local
stalk theorem.  The Brown--Henneaux central charge belongs to the
asymptotic symmetry algebra of three-dimensional gravity and is not
obtained by setting \(N=c_{\mathrm{BH}}\).

\subsubsection{\texorpdfstring{\(W_N\)}{W\_N} via
Drinfeld--Sokolov}\label{sssec:ex-wN}

The \(W_N\)-algebra is obtained from an affine
\(\widehat{\mathfrak{sl}}_N\) algebra by Drinfeld--Sokolov reduction
\cite{frenkel-ben-zvi}*{\S 15.6}.  This construction requires a
curve chiral algebra before the reduction is applied.  It is not a
restriction of the formal coordinate ring
\(\C[[z_1,z_2]]\), and the rank \(N\) of \(W_N\) is not the brane
number unless a separate comparison fixes that identification.

Thus the formal-Darboux theorem supplies only the Hamiltonian trace
sector which a later curve-chiral construction may compare with a
chosen Heisenberg or affine subalgebra.  It does not compute the
\(W_N\) central charge, the higher-spin OPE constants, or the
Drinfeld--Sokolov BRST cohomology.  Any proposed equality between
\(\hbar N[\bar c]\) and a \(W_N\) central parameter is an additional
normalisation theorem to be proved.

\subsubsection{Toric \texorpdfstring{$\C^3$}{C\^{}3}}\label{sssec:ex-c3}

The local theorem is two-complex-dimensional.  For a toric
Calabi--Yau threefold \(X=\C^3\) with volume form
\(\Omega=dz_1\wedge dz_2\wedge dz_3\), it applies only after a
two-dimensional holomorphic symplectic normal sector has been chosen,
for instance the formal \((z_1,z_2)\)-plane with
\(dz_1\wedge dz_2\).  The third complex direction and any
\(\Omega\)-background data are extra structure.

The brane chart remains
\[
  A_b=\widehat{\mc O}_{\C^2,p}\cong\C[[z_1,z_2]],
  \qquad
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)} .
\]
The Hilbert scheme \(\Hilb^N(\C^2)\) appears only after adjoining a
framing vector \(v\) and imposing the cyclic stability condition
\(\C[\phi_1,\phi_2]\cdot v=\C^N\).  Nekrasov--Schwarz deformation and
twisted-M-theory comparisons
\cite{nekrasov-schwarz-noncommutative,costello-twistedM} are
external matched-conventions problems.  The formal-Darboux theorem
does not prove the equivariant formula
\((\varepsilon_1+\varepsilon_2)/(\varepsilon_1\varepsilon_2)\) for
the Capelli scalar.

\subsubsection{\texorpdfstring{$K3$}{K3}}\label{sssec:ex-k3}

For \(X=K3\), the holomorphic symplectic form gives a formal
Darboux chart at each point \(p\):
\[
  \widehat{\mc O}_{K3,p}\cong\C[[z_1,z_2]],
  \qquad
  \omega=dz_1\wedge dz_2 .
\]
At this point the local theorem applies literally, with the same
trace map \(J\) and the same Capelli class as in the formal
polydisk.  This is a pointwise statement.

The elliptic genus of \(K3\) is a weak Jacobi form.  Its occurrence
in the second-quantised \(K3\) story and in Borcherds products is a
global statement, not a consequence of the pointwise
formal-Darboux stalk.  A theorem identifying a protected trace of
\(\Omega_{\mathrm{central}}\)-twisted operators with
\(\mathrm{Ell}(K3;\tau,z)\) would require global descent, a trace
theory, and anomaly cancellation.  It is not proved by the local
chart calculation.

\subsubsection{\texorpdfstring{$K3\times E$}{K3 x E}}\label{sssec:ex-k3-times-e}

For \(X=K3\times E\), the formal-Darboux theorem applies to the
\(K3\) holomorphic symplectic factor at a point \(p\in K3\).  If
\(q\in E\), the coordinate on \(\widehat E_q\) is an additional
one-dimensional parameter; it is not part of the two-dimensional
Hamiltonian formal disk unless a separate reduction includes it.
Thus the local chart used by the theorem is
\[
  \widehat{\mc O}_{K3,p}\cong\C[[z_1,z_2]],
\]
not the three-dimensional completed local ring of \(K3\times E\).

The Igusa cusp form \(\Phi_{10}\) and the Borcherds product
\(\Delta_5=\Phi_{10}^{1/2}\) belong to the global modular and
Borcherds--Kac--Moody comparison problem
\cite{igusa-cusp-form,gritsenko-nikulin-paramodular}.  The local
Capelli scalar is not equal to \(\Phi_{10}\), to \(\Delta_5\), or
to a section of a Siegel modular line bundle without a separate
operator-level lift and a protected trace.  The manuscript therefore
records these as open comparison targets, not as outputs of the
formal-Darboux theorem.
...
\]
belong to distinct construction layers: holomorphic Euler
characteristic, Hodge-theoretic index, Heisenberg chart rank,
Borcherds denominator weight, and Mukai-lattice fibre rank.  They do
not form one invariant.  Any spectral sequence relating these layers
is conjectural.

\subsubsection{Generic compact CY\(_3\)}\label{sssec:ex-generic-cy3}

A generic compact Calabi--Yau threefold has a holomorphic volume form,
not a holomorphic symplectic form.  The formal-Darboux theorem of
this manuscript applies to a two-dimensional holomorphic symplectic
formal sector.  Therefore it does not apply to an arbitrary compact
Calabi--Yau threefold without additional data selecting such a sector
and proving that the mixed holomorphic-topological theory reduces to
the Hamiltonian BF model used here.

A global compact-target theorem would require descent, the quantum
master equation, anomaly cancellation, and locality for the
bulk-to-defect kernel.  Those are separate obligations.  The local
formal-Darboux theorem supplies the model for a formal
holomorphic-symplectic two-plane; it is not a compact
Calabi--Yau threefold theorem.

Thus each example records one of three statuses: proved pointwise by
the formal-Darboux stalk theorem; available only after a specified
curve-chiral construction; or open as a global matched-conventions
comparison.

\section{The \texorpdfstring{$\mathcal{W}_\infty[\lambda]$}{W-infty[lambda]} comparison problem}\label{sec:w-infinity-endpoint}\label{ssec:w-infinity-endpoint}

The family \(\mathcal W_N\) and its large-rank limits are
one-dimensional vertex-algebra objects.  They are not obtained from
the formal-Darboux stalk theorem without a curve chiral construction.
The purpose of this section is therefore a comparison problem: state
what must be supplied before the Hamiltonian trace sector of
Theorem~\ref{thm:main-local} can be compared with a
\(\mathcal W_\infty[\lambda]\) spin tower.

Let \(\Lambda_{\mathrm{adm}}\subset\{(N,k,\lambda)\}\) denote an
admissible locus on which the standard \(\mathcal W\)-algebra
constructions below are defined.  The hypotheses (H1)--(H4) concern
the \(\mathcal W\)-algebras themselves.  They do not imply a
formal-Darboux comparison until the additional conditions (C1)--(C3)
below are imposed.
...
\begin{thm}[{Conditional \(\mathcal W_\infty[\lambda]\) comparison criterion}]\label{thm:w-infty-e-infty-admissible}
  Assume (H1)--(H4).  A comparison between the formal-Darboux stalk
  of Theorem~\ref{thm:main-local} and a
  \(\mathcal W_\infty[\lambda]\) limit requires the following extra
  data.
  \begin{enumerate}
    \item[(C1)] A curve restriction or pushforward of the
      two-dimensional holomorphic factorization algebra
      \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) to a curve \(C\),
      with pushed-forward bracket, BV pairing, brane image, and
      anomaly normalisation.
    \item[(C2)] A vertex-algebra morphism from the restricted curve
      theory to the Drinfeld--Sokolov system whose cohomology is the
      chosen \(\mathcal W_N\)-algebra, compatible with the
      \(N\to\infty\) spin truncations on \(\Lambda_{\mathrm{adm}}\).
    \item[(C3)] An anomaly-matching identity identifying the local
      Capelli class \(\hbar N[\bar c]\) with the central parameter
      of the chosen \(\mathcal W\)-algebra under the morphism of
      (C2).
  \end{enumerate}
  Under (C1)--(C3), the trace sector
  \[
    c_f\longmapsto\theta_f,\qquad
    u_f\longmapsto J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}
  \]
  has a trace-compatible image in the
  \(\mathcal W_\infty[\lambda]\) spin tower on
  \(\Lambda_{\mathrm{adm}}\).  Without (C1)--(C3), no
  \(\mathcal W_\infty[\lambda]\) or \(E_\infty\) conclusion follows
  from the formal-Darboux theorem.
\end{thm}

The load-bearing local object remains the trace map \(J\).  A
\(\mathcal W_N\)-trace is meaningful only after a curve chiral
algebra and a Drinfeld--Sokolov reduction have been supplied.  The
Capelli scalar is not transported to a higher-spin central charge by
(H1)--(H4) alone; it is transported only if the anomaly identity
(C3) is proved.
...
$\Lambda_{\mathrm{adm}}$}, not proof of an \(E_\infty\) structure and
not proof of a formal-Darboux comparison.  The missing link is the
curve restriction and anomaly matching in (C1)--(C3).
...
The admissible \(\mathcal W_\infty[\lambda]\) comparison is therefore
open until (C1)--(C3) are constructed.  It is not a fibre of the
formal-Darboux stalk without those data.  The remaining frontier
problems are global comparison problems of the same kind.

\section{The frontier}\label{sec:frontier}

The formal-Darboux stalk theorem is the formal local model for the
