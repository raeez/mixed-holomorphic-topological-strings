...
\title{\texorpdfstring{\textit{Remarks on Mixed Holomorphic-Topological Strings}\\[0.5em]\large Trace Map, CE/PV Koszul Dictionary, and Brane $E_1$-Hochschild Structure at $N$ Dirac Branes on $\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}$\\[0.3em]\normalsize the formal-Darboux stalk of the Mixed Holomorphic--Topological Deligne conjecture}{Remarks on Mixed Holomorphic-Topological Strings: Trace Map, CE/PV Koszul Dictionary, and Brane E1-Hochschild Structure at N Dirac Branes on R2 top x C2 hol --- the formal-Darboux stalk of the Mixed Holomorphic-Topological Deligne conjecture}}
\date{May 2026}
...
\makeatother
\hypersetup{
  pdftitle={Remarks on mixed holomorphic-topological strings: trace map, CE/PV Koszul dictionary, and brane E1-Hochschild structure at N Dirac branes on R2 top x C2 hol --- the formal-Darboux stalk of the Mixed Holomorphic-Topological Deligne conjecture},
...
and the Trace Chart at $N$ Dirac Branes on $\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}$}

\vspace{1cm}

{\normalsize\itshape the formal-Darboux stalk of the Mixed
...
\end{titlingpage}
  ...
  holomorphic-symplectic brane point, a closed sector conjectured to
  coincide with the derived chiral \(E_2\)-centre of the boundary
  open category \(\mathcal C^{\mathrm{op}}_\partial\): the Mixed
  Holomorphic--Topological Deligne conjecture, the two-complex-dimensional
  extension of the chiral Deligne theorem proved at \(d=1\) for
  chiral algebras factorising over curves.  This manuscript proves
  the formal-Darboux stalk of that conjecture at \(N\) Dirac branes
  over the Heisenberg-core vacuum \(b\): the equivalence
  \[
  ...
  \]
  as \(E_2\)-algebras between the bulk
  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) restricted to the
  formal neighbourhood of \(b\) and the derived \(E_1\)-centre /
  \(E_1\)-Hochschild cohomology of the brane \(E_1\)-algebra
  \(A^{\mathrm{cl}}_{\partial,N}\) on the worldline \(\R_t\), with
  formal-Darboux coordinate expression the trace character
  ...
  Bershadsky--Cecotti--Ooguri--Vafa (BCOV) \cite{bcov} in the
  renormalized Batalin--Vilkovisky (BV) form of Costello and Costello--Li
  ...
  the trace map is
  \(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\).
  Existence of an admissible protected restriction for a given compact
  target is the matched-conventions theorem
  \ref{thm:matched-conventions-transfer}, an external input controlled
  across volumes.
  ...
  condition along \(\R_t\subset X\); its transverse position on the
  holomorphic-symplectic factor is the
  \(\mathfrak{gl}_N\)-valued matrix coordinate pair
  ...
  \medskip\noindent
  \emph{The trace map \(J\).}  At \(N\) Dirac branes the trace map
  \[
    J\colon\mathfrak h\to\mathcal O\bigl(\mu^{-1}_{\mathrm{der}}(0)\bigr)^{GL_N},
    \qquad
    J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
  \]
  is the gauge-invariant trace character of a Hamiltonian
  \(f\in\mathfrak h\).
  ...
  \medskip\noindent
  \emph{Closed-open coupling.}  The classical local observables of the
  BV theory form the mixed holomorphic-topological factorization algebra
  \[
    \mathcal F^{\mathrm{cl}}_{\mathrm{Ham}}(U\times B)
  ...
  fields and antifields (Definition~\ref{def:local-th-string},
  Proposition~\ref{prop:three-localities}).  Pushforward along the
  projection $\pi\colon X\to\C^2_{\mathrm{hol}}$ integrates out
  $\R^2_{\mathrm{top}}$:
  \[
  ...
  on $\R^2$ are $E_2$-algebras (Lurie--Costello--Gwilliam).  In the
  Ran-space algebraic-geometric presentation
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is the holomorphic
  factorization object on $\mathrm{Ran}(\C^2)$ valued in
  $E_2$-algebras, related to the analytic presentation above by the
  de Rham complex.  Restriction along a holomorphic
  $C\hookrightarrow\C^2$ produces a Beilinson--Drinfeld curve chiral
  algebra (a vertex algebra) with generators and relations the
  one-variable specialisation of the two-dimensional OPE
  (Remark~\ref{rmk:not-a-curve-voa}).

  The native two-dimensional holomorphic OPE of
  $\mc A^{\mathrm{cl}}_{\mathrm{bulk}}$ on $\C^2$ is computed in
  Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}: at two
  insertion points $z,w\in\C^2$, the singular part is governed by
  the Grothendieck local-cohomology principal-part module of the
  diagonal $\Delta\subset\C^2_z\times\C^2_w$ and the two-variable
  Cauchy kernel
  $K^{(2)}_\Delta(z,w)=d\zeta_1\,d\zeta_2/(\zeta_1\zeta_2)$ in formal
  diagonal coordinates $\zeta_i=z_i-w_i$.  Restricting the bulk
  factorisation algebra along a holomorphic curve
  $C\hookrightarrow\C^2$ recovers a Beilinson--Drinfeld curve vertex
  algebra with generators and relations the one-variable
  specialisation of the two-dimensional OPE
  (Remark~\ref{rmk:not-a-curve-voa},
  \S\ref{sssec:formal-two-dimensional-holomorphic-ope}).

  The brane is the $E_1$-module
  \(A^{\mathrm{cl}}_{\partial,N}=
  ...
  $f\in\mathfrak h$ has two CE coordinate avatars on $\mathfrak g$:
  ...
  identified with the Hochschild cochain $E_2$-algebra by Deligne's
  $E_1$-Hochschild theorem and matched with the formal-Darboux stalk
  of $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ by the
  Costello--Gwilliam bulk--boundary correspondence; the chain-level
  lift is conditional on vanishing of the obstruction vector
  $\operatorname{Ob}_{\mathrm{cent/QME/desc}}$
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}) with
  chosen null-homotopies, supplied by the completion constructions of
  Appendix~\ref{app:completion-blueprint}.
  ...
  $\bar A=\C[z_1,z_2]/\C\cdot 1$, has three faces: the Lie 2-cocycle
  $\omega(f,g)=[\{f,g\}]_0$ on $\bar A$
  (Lemma~\ref{lem:omega-cocycle}),
  the Poisson bracket of trace coordinates
  $\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$, and the
  Moyal commutator
  $[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\cdot I$ in $M_N(\C)$
  (Theorem~\ref{thm:app-scalar-anomaly-transgression}).  The number
  $N$ is the trace-$U(1)$ central charge of the noncommutative D-brane
  stack on $\C^2_\hbar$, equivalently the projective curvature
  $\Omega_{\mathrm{central}}|_{J(f)}$ of $\Phi_N$.  The strict
  ...
  \S\ref{ssec:master-deformation-target}.  Globalization,
  ...
  the decorated Poincar\'e--Birkhoff--Witt (PBW) Stokes operator \(B_{a,b}\)
  ...
  \(\Phi_N\).  No minimal-model classification is asserted here; an
  exact count of nonzero higher brackets is a separate theorem.

  \medskip\noindent
  The local model is a two-dimensional chiral algebra of Hamiltonian
  currents on $\C^2$ --- a holomorphic factorisation algebra over the
  holomorphic surface $\C^2$, the two-complex-dimensional analogue of
  the Heisenberg vertex algebra --- with $\mathcal P$ playing the role
  of the Kac--Moody central pairing module $\Omega_K$ on a curve $K$
  ...
  the universal formal-Darboux stalk chart of $\Hilb^N(\C^2)$ in its
  noncommutative Moyal deformation.

  \medskip\noindent
  The local theorem is the formal-Darboux stalk chart
  (Definition~\ref{def:formal-darboux-stalk-chart}) restriction of the
  derived chiral centre \(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\) (the
  chiral Hochschild cochain centre at \(d=1\); at \(d=2\) the
  conjectural Mixed HT Deligne identification of
  \S\ref{ssec:binding-convention-layer}), identified with the
  bulk-boundary datum of a 3d holomorphic--topological boundary-bulk
  system, and sourced from a Calabi--Yau category through the two-stage
  functor
...
(Theorem~\ref{thm:matched-conventions-transfer},
\S\ref{ssec:cross-volume-firewall}).

\paragraph{Geometric and algebraic conventions.}
The complex target dimension is \(d=\dim_\C X\); the manuscript
proves the formal-Darboux stalk identification
...
Compact CY\(_3\), BCOV, quintic, Ooguri--Strominger--Vafa (OSV),
Gopakumar--Vafa (GV), Maulik--Nekrasov--Okounkov--Pandharipande (MNOP),
Abel--Jacobi, cohomological Hall algebra (CoHA), Igusa, and
Borcherds--Kac--Moody (BKM) are external programmes governed by
Theorem~\ref{thm:cross-volume-firewall} and the matched-conventions
acceptance class \(\operatorname{Ob}_{\mathrm{UKD}}(\mathfrak C_{\mathrm{tar}})\).
...
the bulk.  The chiral derived centre at \(d=1\) is
\(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\simeq C^\bullet_{\mathrm{ch}}(A,A)\)
(Beilinson--Drinfeld chiral algebras factorising over curves; Vol I).
At \(d=2\) the analogous identification of the bulk with the chiral
derived centre of the boundary open category
\(\mathcal C^{\mathrm{op}}_\partial\) is the Mixed
Holomorphic--Topological Deligne conjecture; this manuscript proves
the formal-Darboux stalk identification
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\simeq
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) of that
conjecture at the brane vacuum.  Three distinct
algebraic structures appear and must not be confused:
\begin{enumerate}
\item the bulk chiral \(E_2\) factorization algebra
  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) on \(\C^2\);
\item the formal-Darboux stalk
  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\) at the brane
  vacuum, an \(E_2\)-algebra carrying the chiral structure
  restricted to the formal neighbourhood of \(b\);
\item the boundary \(E_1\)-brane algebra
  ...
  HH^\bullet(A^{\mathrm{cl}}_{\partial,N},
  A^{\mathrm{cl}}_{\partial,N})\) is an \(E_2\)-algebra by
  Deligne's \(E_1\)-Hochschild theorem.
\end{enumerate}
The chart algebra
\(A_b=\End_{\mathcal C^{\mathrm{op}}_\partial}(b)\cong\C[[z_1,z_2]]\)
is a formal coordinate ring; it has no inherent chiral algebra
structure, and the symbol \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) is
meaningful only after \(A_b\) is equipped with the chiral structure
restricted from \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\).
The local theorem of \S\ref{ssec:local-theorem} proves the
formal-Darboux stalk identification
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\simeq
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) by
Costello--Gwilliam bulk--boundary correspondence and Deligne's
\(E_1\)-Hochschild theorem; the \(E_2\) structure on both sides
matches.  The Mixed Holomorphic--Topological Deligne conjecture is
the global identification of the bulk chiral \(E_2\) algebra
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) with the chiral derived
\(E_2\)-centre of \(\mathcal C^{\mathrm{op}}_\partial\); this
manuscript proves the formal-Darboux stalk identification
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\simeq
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) of that
conjecture at one Dirac brane vacuum.  Where \(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) appears below,
\(A_b\) carries this restricted chiral structure and the symbol
denotes \(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\)
under Costello--Gwilliam.

\paragraph{Constellation-anchored objects.}
The boundary open category on the Kato--Nakayama (KN) log boundary is
...
with \(Q\psi=[\phi_1,\phi_2]\), \(Q\phi_i=0\); the brane realization is
the physical interpretation.  The Capelli scalar identification
...
section; \(\Delta_5=\Phi_{10}^{1/2}\) is the level-2 modular section
of \(\Omega_{\mathrm{central}}\) (its operator-level lift on
\(K3\times E\) is the open problem of
\S\ref{ssec:open-modular-lift}).  The CE/PV dictionary
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\) of
Lemma~\ref{lem:app-sign-ce-pv} is the Koszul resolution of the
brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) at the chosen Dirac brane
vacuum \(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) with chart algebra
\(A_{b_p}=\End_{\mathcal{C}^{\mathrm{op}}_\partial}(b_p)\)
(Theorem~\ref{thm:formal-local-universal-property}); the Mixed
Holomorphic--Topological Deligne conjecture identifies this at the
formal-Darboux stalk with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b_p\)).
...
\item BMK lane: the one-pair analytic pro-Matlis retract of
  ...
  target replacement that absorbs that obstruction.
...
\paragraph{Cross-volume firewall.}  Transport of the local theorem
to a compact-target programme (BCOV / Kodaira--Spencer; Vol III
CY-to-chiral; Igusa / BKM; GW / DT / PT / MNOP; OSV; mixed-Dunn /
six-relation; global Hamiltonian descent on a compact
holomorphic-symplectic surface) is the matched-conventions theorem
(Theorem~\ref{thm:cross-volume-firewall},
Theorem~\ref{thm:matched-conventions-transfer}) with universal
Koszul-descent acceptance class
\(\operatorname{Ob}_{\mathrm{UKD}}(\mathfrak C_{\mathrm{tar}})\); a
nonzero coordinate of \(\operatorname{Ob}_{\mathrm{UKD}}\) is the
...
A \emph{brane vacuum at a closed point} \(p\in\C^2\) is the object
\(b_p\) of the open category
\(\mathcal{C}^{\mathrm{op}}_\partial=
\Phi^{\mathrm{FA}}_d(\mathcal A)|_{\partial X^{\mathrm{KN}}}\) on the
Kato--Nakayama log boundary of
\(X=\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) whose endomorphism
algebra is the formal completion
\[
  A_{b_p}
...
\(\bigl(\widehat{\C^2}_p,\,\omega_p,\,A_{b_p},\,(z_1,z_2)\bigr)\) with
formal completion
...
\(A_{b_p}\), and formal Darboux coordinates \((z_1,z_2)\) supplied by
the formal holomorphic Darboux theorem.  The \emph{Dirac brane
formal-Darboux stalk chart at \(N\) branes over \(b_p\)} adjoins the
Chan--Paton substitution
\[
  \rho_\phi\colon A_{b_p}\to M_N(\C),\qquad
...
factor (its transverse position), and Dirac/moment-map reduction
imposes \([\phi_1,\phi_2]=0\).
For an augmented brane vacuum
\(b\in\mathcal{C}^{\mathrm{op}}_\partial\) with chart algebra
\(A_b=\End_{\mathcal{C}^{\mathrm{op}}_\partial}(b)\) other than the
formal-completion endomorphism (level-\(k\) affine Kac--Moody, free
\(\beta\gamma\), Virasoro at central charge \(c\), \(W_N\), of
\S\ref{sec:examples}), the formal-Darboux stalk chart of this
manuscript governs the Heisenberg core of \(A_b\) along the chosen
abelian subalgebra; the matched-conventions transfer
(Theorem~\ref{thm:matched-conventions-transfer}) identifies that core
with \(A_{b_p}\) for the closed point \(p\) carrying the geometric data
of the chosen vacuum.  For a factorization algebra \(\mathcal X\) on
...
A Hamiltonian \(f\in A_{b_p}\) on the formal symplectic normal polydisk
has trace character
\[
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\]
a single \(GL_N\)-invariant number on the brane stack.  The supporting
...
configuration of \(N\) points in the polydisk.  The trace character
map at the Chan--Paton brane \(\rho_\phi\) is
\[
  J\colon\mathfrak h\to A_\partial=\widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h),
  \qquad
...
\]
the gauge-invariant trace character of a Hamiltonian \(f\in A\).
...
representation moduli, and the cotangent-dual \(u_f\) mapping to the
boundary observable \(O_f=J(f)\) on the brane; the CE/PV cochain
...
the brane carries only the formal holomorphic transverse geometry,
...
identification of Lemma~\ref{lem:formal-stalk-trace}.
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  Its native OPE is the two-variable diagonal local-cohomology expansion
  of Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  A
  one-dimensional vertex operator algebra (VOA) / one-variable OPE
  statement requires a separate restriction theorem choosing a complex
  line or holomorphic defect, a transverse boundary condition or
  restriction functor, and the finite Laurent OPE expansion on that
  restricted object.
\end{defn}
...
\begin{proof}
  ...
  two-dimensional OPE is the diagonal local-cohomology theorem inserted
  next; the one-dimensional vertex/OPE structure requires the separate
  restriction data of Theorem~\ref{thm:dict-kinematic-locality}.
\end{proof}

\input{two-dimensional-holomorphic-ope}
...
side to the trace character map $u_f\mapsto J_f$.  The
...
\begin{proof}
  ...
  The map \(\Phi_N\) is defined on CE generators as Hochschild cochains.  For
  ...
  is Theorem~\ref{thm:reduced-ce-pv-central-operation}.
  Deligne's \(E_1\)-Hochschild theorem identifies
  \(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) with
  \(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) as
  \(E_2\)-algebras; the Costello--Gwilliam factorization-algebra
  bulk--boundary correspondence glues the generator data into the
  universal bulk-to-boundary chain map landing in this
  \(E_2\)-algebra.  Uniqueness of the boundary values
  \(u_f\mapsto J(f)\) is Theorem~\ref{thm:Jf-coordinate-uniqueness}.
  ...
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\) of
  Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}; the
  curvature class is the difference between the derived
  \(E_1\)-centre \(E_2\)-structure of Deligne's theorem and the
  strict \(P_0\)-factorization-centre.
\end{proof}

\begin{rmk}[Native bulk object on $\C^2$]\label{rmk:not-a-curve-voa}
  The native object is the mixed holomorphic-topological factorization
  algebra $\mathcal F^{\mathrm{cl}}_{\mathrm{Ham}}$ on
  ...
  \(\C^2_{\mathrm{hol}}\) valued in \(E_2\)-algebras --- the
  $E_2$-valuation encoding the locally constant $\R^2_{\mathrm{top}}$
  ...
  which Zhu / finite-Laurent OPE applies.  Before any such restriction,
  the native singular product is two-complex-dimensional:
  Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope} identifies
...
\begin{proof}
  ...
  retract the target replacement absorbing this leak
...
polydisk supplies the Hamiltonian deformations as conjugate matrix
...
The trace functional of a Hamiltonian \(f\in\mathfrak h\) on the brane
is
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\).

The pair \((\phi_1,\phi_2)\) realizes a
\(\lie{gl}_N\otimes\lie{gl}_N\)-valued holomorphic symplectic
Chan--Paton pair; its scalar reduction is two complex coordinates
conjugate under one holomorphic two-form.  The transverse data is
therefore a single holomorphic symplectic surface \((Y,\omega)\) at
the brane point.

The formal-stalk chart carries only the local behaviour at the brane,
never global topology of \(Y\); the relevant datum is the formal
completion of \(Y\) at the brane point. A holomorphic Darboux theorem trivialises
...
moduli. The trace character of a bulk Hamiltonian
\(f\in\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) at the
shifted-cotangent avatar \(u_f\) is
\begin{equation}\label{eq:setup-Jf-flagship}
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\end{equation}
the gauge-invariant trace of \(f\) evaluated on the Chan--Paton pair,
projected to the stable scalar-reduced quotient. Under the formal-stalk-chart hypotheses
...
the only candidate gauge-invariant scalar functionals
...
Theorem~\ref{thm:formal-local-universal-property} is the universal
property statement.  Among all admissible protected Hamiltonian
sectors at a holomorphic-symplectic brane point, the formal Darboux
model
...
\begin{rmk}[\(c_f\) versus \(u_f\)]\label{rmk:type-discipline-cf-uf}
  ...
  reading of \(f\) on the Chan--Paton pair.  The closed Hamiltonian
...
\begin{prop}[Geometry determined by the formal-stalk chart]\label{prop:local-model-mixed-definition}
  ...
  completion carrying the formal-stalk-chart datum
...
\begin{rmk}[Two avatars of a Hamiltonian: operation and evaluation]\label{rmk:intro-symbol-separation}
...
positivity belong to a parent unitary theory, not to the cohomological
algebra of protected observables.
...
together with associativity of the resulting factorization algebra.  The
native two-dimensional holomorphic OPE is the diagonal
local-cohomology expansion of
Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}; a
one-dimensional VOA/OPE algebra is present only after a separate
one-complex-dimensional reduction datum retaining the required transverse
...
differentials.  Closure of the BV theory on this configuration forces
...
(Theorem~\ref{thm:bf-action-uniqueness-qme}). These statements reduce
...
and master action \(S=\int\langle\beta,D\alpha+\tfrac12\{\alpha,\alpha\}\rangle dz_1dz_2\),
forced by Darboux + cyclic trace + linearity in the cotangent partner.
The open boundary at a stack of \(N\) Dirac branes pins the
...
\section{The derived commuting variety stack at $N$ Dirac branes}\label{sec:derived-commuting-variety}
...
\(\mu(\phi_1,\phi_2)=[\phi_1,\phi_2]\) on the transverse position pair
\((\phi_1,\phi_2)\in\lie{gl}_N\oplus\lie{gl}_N\) --- the
\(\lie{gl}_N\)-valued matrix coordinates of the \(N\)-brane stack on
the holomorphic-symplectic factor
...
A Hamiltonian \(f\) has trace evaluation
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}
\]
on the chart: the formal Darboux coordinate of the brane-side
\(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\),
identified at the formal-Darboux stalk with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b_p\)) by Costello--Gwilliam bulk--boundary correspondence and
Deligne's \(E_1\)-Hochschild theorem.  The data are the formal Darboux
polydisk at the brane point and the Chan--Paton substitution
\[
  z_1\rightsquigarrow\phi_1,\qquad z_2\rightsquigarrow\phi_2,\qquad
  \phi_i\in\mathfrak{gl}_N,
\]
sending the holomorphic coordinate functions on the polydisk to the
matrix-valued transverse positions of the brane stack.  Every other
piece of open-side structure---symplectic potential, moment map,
Dirac constraint, BV differential, gauge-invariant operator
...
\begin{proof}
  ...
  elementary circle-sum argument runs as follows.  Before quotienting
  by cyclic relabelling, send a chip configuration to the sum of the
  chip positions in
...
trace observables generating the open-string sector classical
observables factorization algebra on the brane line.  The trace
functional of a closed Hamiltonian \(f\in\mathfrak h\) on this brane
sector is the \(\mathrm{GL}_N\)-invariant
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\); its
...
\subsection{The reduced CE/PV trace map}\label{ssec:reduced-ce-pv-measurement}

The trace map at a Dirac brane vacuum is
\[
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\]
the formal Darboux coordinate expression at
\(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) of the brane-side
derived \(E_1\)-centre
\(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\simeq
HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
(Deligne's \(E_1\)-Hochschild theorem), which the Mixed
Holomorphic--Topological Deligne conjecture identifies at the
formal-Darboux stalk with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra on \(\C^2\) restricted to the formal
neighbourhood of \(b_p\); \S\ref{ssec:binding-convention-layer}).  Here
\(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) is the chosen vacuum
at \(p=0\in\C^2\)
(Definition~\ref{def:formal-darboux-stalk-chart}), with chart
algebra
\(A_{b_p}=\End_{\mathcal{C}^{\mathrm{op}}_\partial}(b_p)\cong\C[[z_1,z_2]]\)
on the brane line \(L=\R_t\times\{z_1=z_2=0\}\) of
\S\ref{sec:derived-commuting-variety}; \(J\) is the stable
scalar-reduced cyclic-trace class of the unreduced
\(J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\).  The closed BV
theory carries two CE coordinate types and the chart carries only
one of them: Hamiltonian ghosts \(c^I\)---dual to generators
\(H_I\in\widehat{\mathfrak h}\)---generate the closed motion on
the polydisk; cotangent coordinates \(u_I\)---dual to the shifted
antifield generators in
\(\mathfrak h^\vee_{\mathrm{cont}}[1]\)---are the boundary
observables.  The map \(J\) is the unique \(\C\)-linear,
\(GL_N\)-equivariant, Darboux-natural primitive single-trace map
\(\mathfrak h\to\mathcal O(\mu^{-1}_{\mathrm{der}}(0))^{GL_N}\)
satisfying scalar reduction, Dirac descent, and
\(J(z_i)=\mathrm{Tr}\,\phi_i\)
(Theorem~\ref{thm:formal-local-universal-property}).  Centrality
is not a separate condition: it is the moment-map identity
\(\{J(f),J(g)\}_\partial=J(\{f,g\}_{\mathfrak h})\) on the chart
\(A_{b_p}\).  Closed Hamiltonians act through \(c_f\); \(J\)
records \(u_f\); below, the symbols \(c_f,u_f,\theta_f,O_f\)
refer back to Lemma~\ref{lem:type-discipline}.
...
\begin{lem}[Two CE coordinates of a Hamiltonian]\label{lem:type-discipline}
  ...
  \begin{itemize}
    ...
    \item As closed cotangent insertion:
      ...
      \(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\) on the chart \(A_b\)
      after stable scalar reduction.
  \end{itemize}
  Closed Hamiltonians act through \(c_f\); the trace map \(J\) records
  ...
  \[
    \begin{array}{c|c|c|c}
      ...
      u_f & (\mathfrak h^\vee_{\mathrm{cont}}[1])^\vee
        ...
        & \text{chart trace map }J\text{ on }A_b
    \end{array}
  \]
\end{rmk}
...
At finite \(N\) the trace map \(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\)
(equivalently \(\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\) after
stable scalar reduction) requires a word ordering of the noncommuting
...
\begin{thm}[Witten centrality on the Dirac brane: the local moment-map trace identity]
  \label{thm:witten-centrality-local-form}
  In the reduced classical \(P_0\) factorization algebra
  \(A_\partial\) on the brane line, the trace map
  \(J\colon\bar A\to A_\partial\),
  \(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\), with smeared form
  \(f\mapsto B_f(a)=\int_L a(t)\,J(f)(t)\,dt\), realises every closed
  Hamiltonian \(f\in\bar A\) as a boundary observable whose ordered
  products and \(P_0\)-bracket with smeared Hamiltonian insertions
  \(B_g(b)\) are the moment-map operations
...
\begin{proof}
  ...
  bracket on the symbols, evaluated again on the Chan--Paton pair.  This
...
The trace substitution \(J\) produces the open algebra.  The Dirac
brane stack is the Dirac-reduced first-order action
\eqref{eq:open-action-derived}; its ghost-zero invariant ring is
\(H^0(\mathcal R_N^{GL_N},Q)\).  By
Lemma~\ref{lem:formal-stalk-trace} the stable connected
...
trace observables generate the open-string sector classical
observables factorization algebra
\[
  \mathcal A_N(I)
...
\begin{thm}[Brane trace-map principle: bulk Hamiltonians as moment-map currents]\label{thm:brane-measurement-principle}
...
with Theorem~\ref{thm:brane-measurement-principle} the trace map \(J\)
...
Witten's source convention forces the closed-CE-coordinate
distinction \(c\mapsto\theta\) versus \(u\mapsto O\): the
Hamiltonian-dual coordinate \(c\) carries vector-field operations,
while the cotangent-dual coordinate \(u\) carries boundary
Hamiltonians.  Grothendieck residue duality forces the
...
convention, not an identification of underlying modules.
...
the brane), with the assignment \(u\mapsto O_{k,l}\) forced by the
...
convention takes the boundary observable through the cotangent
coordinate \(u\) and the Schouten action on observables through the
Hamiltonian coordinate \(c\).  The dictionary
\(c_f\mapsto\theta_f,\ u_f\mapsto J(f)\) is the Koszul resolution of
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) at the chart
\(A_{b_p}\); the cotangent CE/PV reduced central-operation spine of
\S\ref{ssec:coordinate-free-ce-pv-synthesis} is the formal-Darboux
stalk expression of that resolution.  The Mixed
Holomorphic--Topological Deligne conjecture identifies the resolution
target with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
of \(b_p\)).
...
\subsection{The CE/PV Koszul resolution of the brane $E_1$-Hochschild}
\label{ssec:coordinate-free-ce-pv-synthesis}

The dictionary \(c_f\mapsto\theta_f,\ u_f\mapsto J(f)\) is the Koszul
resolution of the brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) at the formal-Darboux
stalk of the Dirac brane vacuum \(b_p\): closed CE
...
through the cotangent coordinate, not the Hamiltonian one.  The Mixed
Holomorphic--Topological Deligne conjecture identifies the resolution
target with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
...
The local theorem identifies the formal-Darboux stalk of the bulk
chiral $E_2$ factorization algebra at the Dirac brane vacuum $b_p$
with the brane-side $E_1$-Hochschild,
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\simeq
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\simeq
HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
(Costello--Gwilliam bulk--boundary correspondence + Deligne's
$E_1$-Hochschild theorem).  Its formal Darboux coordinate is the trace
map $J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}$
(Theorem~\ref{thm:formal-local-universal-property}); its
closed-to-open dictionary is the Koszul resolution
$c_f\mapsto\theta_f$, $u_f\mapsto J(f)$
(Theorem~\ref{thm:universal-ce-pv-koszul-criterion} below); its
projective curvature is the Capelli scalar $\hbar N[\bar c]$
(Theorem~\ref{thm:app-scalar-anomaly-transgression}).  Promotion to a
global compact-target identification requires descent, QME closure,
anomaly compatibility, and locality on the Kato--Nakayama log
boundary; the four obstruction theorems
\ref{thm:app-unreduced-cotangent-lift-obstruction},
\ref{thm:app-theta-three-finite-window-H1},
\ref{thm:app-radial-bicomplex-acyclic},
\ref{thm:bmk-finite-window-no-go}
control the conditional chain-level lift.
...
\begin{thm}[Formal local Hamiltonian BF/Dirac-brane comparison]\label{thm:main-local}
  ...
  \begin{enumerate}
    \item the Dirac brane formal-stalk chart with
...
\begin{proof}
  The Darboux structure, the Dirac brane stack, the Hamiltonian BF coefficient
...
separates canonical motion from evaluation; the CE/PV dictionary
...
componentwise whether Weyl ordering commutes with the trace map.
...
\begin{rmk}[Worked instances]\label{rmk:Jf-uniqueness-running-examples}
  ...
  \S\ref{ssec:formal-darboux-stalk-one-calculation} pin the theorem
  concretely.  Example~\ref{exa:point-evaluation-N1} is the \(N=1\) case:
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  holomorphic OPE is not an open item in this problem: its diagonal
  coefficient module, mode expansion, and current singular product are
  Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  The open
  extension problem is to lift that binary singular product, together
  with the trace map \(J\), through the unreduced cotangent sector,
  Costello heat-kernel coefficients, and the quantum master equation
  tower.  Thus one asks for a coefficient category in which:
  \begin{enumerate}
...
\subsubsection{Quantization as a brane trace-map test}\label{sssec:quantization-measurement-test}
...
the matrix entries of \((\phi_1,\phi_2)\), and the brane trace map
\(J\colon f\mapsto\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\)
extended to a quantum trace map \(\Phi_\hbar\).  The question is
...
\cite{capelli,howe-capelli}.  The three faces of the same scalar
obstruction --- Lie 2-cocycle on $\bar A$, Poisson bracket of trace
coordinates, and quantum Moyal--Weyl commutator --- realise the same
class
\(\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\)
...
\begin{proof}
  ...
  identity.  A finite exact check verifies
...
\begin{prob}[Hamiltonian specialization datum for Costello perturbative BV]\label{prob:analytic-graph-realization}
  \begin{itemize}
    ...
    \item \emph{Physical meaning:} the analytic specialization datum
      ...
      Hamiltonian trace map is simultaneously controllable.
  \end{itemize}
...
\label{thm:scalar-qme-alternatives}
  ...
  \[
    ...
    \Xi^{\mathrm{Cost},\mathrm{sc}}_M).
  \]
  A complete classification of scalar QME cancellations requires a
  separate target theorem.  Any further mechanism must be an explicitly
...
\begin{thm}[Componentwise quantum coefficient comparison]\label{thm:componentwise-quantum-coefficient-comparison}
  ...
  \begin{itemize}
    ...
    \item \emph{Physical meaning:} the Moyal star, the matrix Weyl
      algebra, and the brane trace map agrees
...
\begin{rmk}[Computer verification]\label{rmk:computer-verification}
  ...
  Procesi--Razmyslov--Capelli inverse identity and the prequotient scalar
...
the product/OPE of quantum observables are related but distinct.  The
formal holomorphic singular coefficient of the product is the
\(K^{(2)}_\Delta\)-kernel of
Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}; the finite-scale
...
The dictionary \(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\) is the
Koszul resolution of the brane-side derived \(E_1\)-Hochschild
\(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\) at the formal-Darboux
stalk of the Dirac brane vacuum \(b_p\)
(Theorem~\ref{thm:main-local}); the Mixed Holomorphic--Topological
Deligne conjecture identifies this at the formal-Darboux stalk with
the bulk \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the
chiral \(E_2\) factorization algebra restricted to the formal
neighbourhood of \(b_p\)).  Scalar reduction kills the constant
Hamiltonian on the closed side and retains a \(\mathrm{U}(1)\)
centre-of-mass anomaly on the open side: a single Lie 2-cocycle
...
survives Moyal quantisation as the order-\(\hbar\) Capelli shift.
...
\(M_N(\C)\) --- packaging the four equivalent sightings of
...
\begin{ex}[Linear Hamiltonians and the brane centre of mass]\label{ex:linear-hamiltonian-com}
  ...
  charge and the modular projective-curvature
  $\Omega_{\mathrm{central}}|_{J(f)}$ are equivalent interpretations
  of the multiplier $N$
...
quantized symbol on the Chan--Paton pair.  They commute exactly when the
...
the quantum face of the brane trace-map test, with no higher-order
...
\label{thm:capelli-equals-modular-curvature}\label{thm:capelli-omega-central-identification}
  ...
  \(\Omega_{\mathrm{central}}\) denote the canonical curvature of
  the determinant line bundle on the moduli stack of the
  closed-string sector classical observables factorization algebra
  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) at \(N\) Dirac branes
  on \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\).  Then
  \[
...
\begin{proof}
  ...
  follows.  The brane \(E_1\)-algebra
  \(A^{\mathrm{cl}}_{\partial,N}=
    C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N,
                            \mathrm{Kosz}([\phi_1,\phi_2]))\)
  on the worldline \(\R_t\) has trace coordinate \(u_f\mapsto J(f)\)
  and CE coordinate \(c_f\mapsto\theta_f\)
  by \S\ref{ssec:reduced-ce-pv-measurement};
...
on the Chan--Paton pair.  The assignment \(c_f\mapsto O_f\) is a type
error
...
The Capelli scalar
\(\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}\) is the
projective curvature of the modular line bundle evaluated at the
trace generator
(Theorem~\ref{thm:capelli-equals-modular-curvature}); it is one
component of a single curvature
\(F_{\mathcal T}(\Theta_{\mathrm{loc}})\) in the master deformation
complex \(\mathfrak K_{\mathcal T}\), whose four-curvature taxonomy
classifies the global obstructions and locates the projective cell.

\paragraph{All-bidegree Capelli through the radial-parts identity.}
The linear-generator collapse of
Theorem~\ref{thm:quantum-classical-anomaly} extends to all bidegrees
\((a,b)\) through a per-bidegree exact rational reduction: at
...
\(v_{a,b}\in\operatorname{im}B_{a,b}\) the elimination returns the
unique correction \(C^+_{a,b}\) with
\(B_{a,b}C^+_{a,b}=v_{a,b}\) and
\(\Omega^{\mathrm{rad}}_{a,b}=0\); when it does not, the
...
all classes vanish and the Capelli identification extends; outside
the listed range \(\Omega^{\mathrm{rad}}_{a,b}\) is the
counterterm-cell \((d_\Theta\tau)\) coordinate of
\(F_{\mathcal T}(\Theta_{\mathrm{loc}})\) in the four-curvature
taxonomy of Theorem~\ref{thm:four-curvature-taxonomy}.  The
Razmyslov--Procesi radical structure of the trace ideal at finite
\(N\) does not enter the radial reduction: every step is exact
rational and basis-canonical.

\input{appendix-master-deformation-complex}

\subsection{Tate-coefficient cotangent lift: the six gauge slices in body}\label{ssec:tate-residual-resolution}
...
The four-curvature taxonomy and six-coordinate classification
(Theorems~\ref{thm:four-curvature-taxonomy},
\ref{thm:six-coordinates-one-curvature}) live natively on the
inverse limit
\(\widehat{\mathcal P}_\Pi=\varprojlim_N\mathcal P_{\le N}\) of finite
Matlis windows; the Bochner--Martinelli no-go on each finite window
forces the pro-Matlis target as the categorical home of bulk-boundary
duality, constructed in the section below.
...
Definition~\ref{defn:tate-cotangent-lift-obstruction-vector} is, on
\(\widehat{\mathcal P}_\Pi\), a coherence class for the inverse-limit
...
The formal-Darboux stalk theorem at one Dirac brane vacuum, the
six-coordinate obstruction classification, and the pro-Matlis target
that supplies their categorical home are now in place.  The
identification
\(A^{\mathrm{cl}}_{\partial,N}=
C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N,\mathrm{Kosz}([\phi_1,\phi_2]))\),
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\),
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\), is tested at the brane
vacua of the central families.

\section{Examples}\label{sec:examples}\label{ssec:examples}

At \(d=1\) the chiral derived
centre \(Z^{\mathrm{der}}_{\mathrm{ch}}(A)\simeq
C^\bullet_{\mathrm{ch}}(A,A)\) of a chiral algebra factorising over a
curve is the Beilinson--Drinfeld theorem
\cite{beilinson-drinfeld-chiral}*{\S 4} (recapped in Vol I); at
\(d=2\) the
analogous identification is the Mixed Holomorphic--Topological
Deligne conjecture, whose formal-Darboux stalk at the Heisenberg-core
vacuum \(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) is the local
theorem of \S\ref{ssec:local-theorem}.  Every example below therefore
inherits the native two-dimensional diagonal OPE
\[
  \mathsf{J}_x(z)\mathsf{J}_y(w)
  \sim
  K^{(2)}_\Delta(z,w)\mathsf{J}_{[x,y]_{\mathfrak g}}(w)
\]
of Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope} before any
curve restriction, affine chart, Drinfeld--Sokolov reduction, or
large-\(N\) spin limit is imposed.  The one-variable OPEs in the
examples are restrictions or matched-conventions transfers of this
diagonal contact algebra.

Each example records: the brane vacuum
\(b\in\mathcal{C}^{\mathrm{op}}_\partial\); the chart algebra
\(A_b=\End_{\mathcal{C}^{\mathrm{op}}_\partial}(b)\) and its
reduction to the Heisenberg core
\(A_{b_p}=\widehat{\mathcal O}_{\C^2,p}\) along the chosen abelian
subalgebra (Definition~\ref{def:formal-darboux-stalk-chart}); the
trace \(J(f)\); the \(d=1\) chiral derived centre target where
applicable; status --- proved (at the Heisenberg core), conditional
(matched-conventions transfer through the chosen reduction), or open
(global compact-target identification beyond the formal-Darboux
stalk, requiring descent, QME, anomaly, and locality data on the
target geometry).

\subsubsection{Heisenberg current on the formal disk}\label{sssec:ex-heisenberg}

\(N=1\), \(b=\) the formal-disk vacuum at the brane point
\(p\in\widehat{\C^2}_0\).  The chart algebra is
\(A_b=\C[[z_1,z_2]]\) with the canonical pair
\((\phi_1,\phi_2)=(z_1,z_2)\) and
\(\overline{\operatorname{Tr}}=\mathrm{ev}_p\).  The trace
\(J(z_1^k z_2^l)=z_1^k z_2^l\) is the evaluation map; the closed
generator dictionary is \(c_f\mapsto\theta_f\),
\(u_f\mapsto J(f)\).  The Capelli scalar \(\hbar N[\bar c]\) vanishes at \(N=1\): the
trace becomes evaluation, the U(1) centre-of-mass charge is one
unit and the scalar quotient kills the central extension on the
single brane, so
\([\bar c]=0\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\) after
scalar reduction.  The order-\(\hbar\) Heisenberg commutator
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar\)
(cf.\ Example~\ref{ex:linear-hamiltonian-com}) carries no
nontrivial extension class for \(N=1\): this is the strict cell
(case~1) of Theorem~\ref{thm:four-curvature-taxonomy}.  The
formal-Darboux stalk reads
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\simeq
\mathrm{PV}(\C[[z_1,z_2]])\)
at \(N=1\) via the bulk-boundary correspondence, with
\(\C[[z_1,z_2]]\) carrying the chiral $E_2$ structure restricted from
the bulk; the polyvector reading is Costello--Gwilliam
\cite{costello-gwilliam}*{Vol.~II, \S 5} on the formal
disk.  Proved (the local theorem of~\S\ref{ssec:local-theorem}
at \(N=1\)).

\subsubsection{Affine Kac--Moody current
\texorpdfstring{\(\widehat{\mathfrak g}_k\)}{ghat\_k}}\label{sssec:ex-affine-km}

\(\mathfrak g_0\) finite-dimensional simple complex; \(b=\) the
level-\(k\) vacuum module \(V_k(\mathfrak g_0)\) of
\cite{frenkel-ben-zvi}*{\S 2.4}.  The chart algebra is
\(A_b=V_k(\mathfrak g_0)\); the Chan--Paton pair is the affine
current \(J^a(z)\) and its conjugate, with the Heisenberg core
recovered along the Cartan.  The trace
\(J(f)=\overline{\operatorname{Tr}\rho_\phi(f)}\) on the chart is
the Sugawara projection of the polynomial Hamiltonian.  The
Capelli scalar reads
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar k\cdot I\): the
brane multiplicity \(N\) is replaced by the affine level \(k\);
the central extension \([\bar c]\) is the Kac--Moody central
charge --- the projective \((\Omega_{\mathrm{central}})\)-cell of
Theorem~\ref{thm:four-curvature-taxonomy} at the
\(\widehat{\mathfrak g}_k\)-chart, with chart-multiplicity formula
\([\hbar N\bar c]_{\widehat{\mathfrak g}_k}=[\hbar k]\).  On the $d=1$
curve restriction (along a holomorphic
\(C\hookrightarrow\C^2\), Remark~\ref{rmk:not-a-curve-voa}), the
$d=1$ chiral Hochschild target is
\(C^\bullet_{\mathrm{ch},d=1}(V_k(\mathfrak g_0),V_k(\mathfrak g_0))
\simeq Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\mathfrak g_0))\) along
the Beilinson--Drinfeld chiral Deligne theorem at $d=1$
\cite{beilinson-drinfeld-chiral}; the $d=2$ formal-Darboux stalk
identifies $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}$ with
$Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})$ by
Costello--Gwilliam bulk--boundary correspondence.  Identification of the
formal-Darboux stalk is proved on the Heisenberg core
(\S\ref{ssec:local-theorem}); the Sugawara reading to the full
\(\widehat{\mathfrak g}_k\) chart is the standard
Frenkel--Ben-Zvi datum, pending a matched-conventions transfer at
the joint level/multiplicity scaling
\((k,N)\to\hbar k N[\bar c]\).

\subsubsection{\texorpdfstring{\(\beta\gamma\)}{beta-gamma} system}\label{sssec:ex-betagamma}

\(N=1\), \(b=\) the \(\beta\gamma\) vacuum on the formal disk;
the Chan--Paton pair \((\phi_1,\phi_2)\) realises the canonical
commutation \([\beta(z),\gamma(w)]=(z-w)^{-1}\) at the brane
point as the Heisenberg pair of weight \((1,0)\) and \((0,1)\)
fields after choosing the one-variable curve restriction.  Before that
restriction, the same Heisenberg pair has the native contact term
\(\hbar K^{(2)}_\Delta(z,w)\mathbf1\) of
Corollary~\ref{cor:quantum-brane-trace-two-dimensional-ope}.  The chart
algebra is the \(\beta\gamma\) vertex algebra
\(A_b=V^{\beta\gamma}\); the trace
\(J(z_1^k z_2^l)=\overline{\operatorname{Tr}}(\beta^k\gamma^l)\)
collapses to the Wick-ordered monomial.  The central charge
\(c=2\) equals \(\hbar N[\bar c]\) with \(N=1\) and the doubled
Heisenberg cocycle: the projective
\((\Omega_{\mathrm{central}})\)-cell of
Theorem~\ref{thm:four-curvature-taxonomy} at the
\(\beta\gamma\)-chart with chart-multiplicity formula
\([\hbar N\bar c]_{\beta\gamma}=[\hbar\cdot 2]=[\hbar c]\).  On the
$d=1$ curve restriction (Remark~\ref{rmk:not-a-curve-voa}), the $d=1$
chiral Hochschild target is
\(C^\bullet_{\mathrm{ch},d=1}(V^{\beta\gamma},V^{\beta\gamma})\),
the $d=1$ chiral derived centre of the free \(\beta\gamma\)
factorization algebra of
\cite{costello-gwilliam}*{Vol.~II~\S 5.4}.  Proved at
\(c=2\); the chart matches the formal-Darboux stalk by the same
Heisenberg cocycle that controls the brane centre-of-mass
(Example~\ref{ex:linear-hamiltonian-com}).

\subsubsection{Virasoro at central charge
\texorpdfstring{\(c\)}{c}}\label{sssec:ex-virasoro}

\paragraph{Chart and trace.}
\(b=\) the Virasoro vacuum module \(V_{\mathrm{Vir},c}\) at
central charge \(c\); chart algebra \(A_b=\mathrm{Vir}_c\) with
generators \(L_n\), \(n\in\Z\), and Virasoro relations
\([L_n,L_m]=(n-m)L_{n+m}+\tfrac{c}{12}(n^3-n)\delta_{n+m,0}\)
\cite{frenkel-ben-zvi}*{\S 2.5}.  The trace map reads polynomial
Hamiltonians on the Virasoro generators through the Sugawara
projection from the Heisenberg core (\S\ref{sssec:ex-heisenberg}):
on a Hamiltonian \(f\in\mathfrak h\) admitting a Sugawara
presentation
\(L_n=\tfrac12\sum_{k\in\Z}{:}a_{n-k}a_k{:}\) with
\(a_n\in\widehat{\mathfrak{gl}}_1\),
\[
  J(f)=\overline{\operatorname{Tr}\,f(L_n)}
   =\overline{\operatorname{Tr}\,
     f\!\left(\tfrac12\sum_{k\in\Z}{:}a_{n-k}a_k{:}\right)},
\]
the Wick-ordered Sugawara composite of the Heisenberg trace
generators of \S\ref{sssec:ex-heisenberg}.

\paragraph{Capelli scalar and four-curvature cell.}
The Capelli identification of \S\ref{sec:capelli-scalar},
\(\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}\), reads at
\(A_b=\mathrm{Vir}_c\) as the Virasoro central charge:
\(\Omega_{\mathrm{central}}|_{J(L_n)J(L_m)}=
\tfrac{c}{12}(n^3-n)\delta_{n+m,0}\) at the
\((L_n,L_m)\)-trace generator pair.
Theorem~\ref{thm:four-curvature-taxonomy} places the Virasoro
chart on the projective \((\Omega_{\mathrm{central}})\)-cell:
\(c\) is the projective curvature of the chiral central extension
of the Witt algebra \(\mathfrak h_W=\C[z,z^{-1}]\partial_z\) by
\(\C\cdot 1\), Lie 2-cocycle
\(\bar c(L_n,L_m)=\tfrac1{12}(n^3-n)\delta_{n+m,0}\); under the
Sugawara presentation through the Heisenberg core the chart
multiplicity is \(c=N\) (one boson per brane), the
chart-multiplicity formula of the Capelli identification.

\paragraph{Cross-volume bridge: algebraic-HT 3d-gravity.}
The $d=1$ chiral derived centre
\(C^\bullet_{\mathrm{ch},d=1}(\mathrm{Vir}_c,\mathrm{Vir}_c)\)
of the Virasoro vertex algebra at central charge \(c\) on a curve
restriction (Remark~\ref{rmk:not-a-curve-voa}); under
matched-conventions transfer to
\(\sim/\mathrm{chiral\text{-}bar\text{-}cobar\text{-}vol2/}\)
(Vol.~II; \(d=1\) algebraic 3d-gravity HT sector at
\(A=\mathrm{Vir}_c\)) the formal-Darboux stalk at the Virasoro
chart agrees with the Vol.~II 3d-gravity HT sector at
\(A=\mathrm{Vir}_c\): the Brown--Henneaux boundary asymptotic
symmetry algebra
\(\mathrm{Vir}_{c_{\mathrm{BH}}}\oplus\mathrm{Vir}_{c_{\mathrm{BH}}}\)
at central charge \(c_{\mathrm{BH}}=3\ell/2G_N\) recovers as the
double-Virasoro chart at \(N=c_{\mathrm{BH}}\).  The identification
is of HT sectors --- the algebraic-HT lane of the seventeen-site
row 13 catalogue --- not a construction of the dynamical-metric
path integral on \(\mathrm{AdS}_3\).

\paragraph{Open: operator-level BTZ lift.}
The operator-level lift of the modular line at the Virasoro chart,
identifying \(\Omega_{\mathrm{central}}|_{\mathrm{Vir}_c}\)-twisted
operators with BTZ--CFT\(_2\) Cardy data at central charge
\(c=c_{\mathrm{BH}}\), is the Virasoro analogue of the
\(K3\times E\) operator-level lift problem
(\S\ref{sssec:ex-k3-times-e}) and is open.

Identification of the formal-Darboux stalk: proved at the
Heisenberg core (\S\ref{ssec:local-theorem}); promotion to the
Virasoro chart through Sugawara: standard Frenkel--Ben-Zvi datum
\cite{frenkel-ben-zvi}*{\S\S 2.5, 3.4}; the Capelli identification
\([\hbar N\bar c]_{\mathrm{Vir}_c}=[c]\) is forced by the
projective central extension; algebraic-HT bridge to BTZ open at
the operator level.

\subsubsection{\texorpdfstring{\(W_N\)}{W\_N} via
Drinfeld--Sokolov}\label{sssec:ex-wN}

\paragraph{Chart and trace.}
\(b=\) the principal Drinfeld--Sokolov reduction of
\(\widehat{\mathfrak{sl}}_N\) at level \(k\); the chart algebra is
\(A_b=W_N(\mathfrak{sl}_N)_k\)
\cite{frenkel-ben-zvi}*{\S 15.6} with generating currents
\(W^{(s)}(z)\) of conformal weights \(s=2,3,\dots,N\): the stress
tensor \(W^{(2)}=L\) at \(s=2\) plus higher-spin currents
\(W^{(3)},\dots,W^{(N)}\) at integer weights up to \(s=N\).  The
trace map at the Drinfeld--Sokolov chart is
\(J(f)=\overline{\operatorname{Tr}\rho_\phi(f)}\) on polynomial
Hamiltonians in the \(W^{(s)}\)-currents, the iterated Sugawara
projection from the Heisenberg core (\S\ref{sssec:ex-heisenberg})
through the \(\widehat{\mathfrak{sl}}_N\) parafermion at level
\(k\).  On the $d=1$ curve restriction
(Remark~\ref{rmk:not-a-curve-voa}), the $d=1$ chiral Hochschild
target is \(C^\bullet_{\mathrm{ch},d=1}(W_N,W_N)\).

\paragraph{Capelli scalar at the \texorpdfstring{$W_N$}{W\_N}-chart.}
The Capelli identification of \S\ref{sec:capelli-scalar} reads at
the \(W_N\)-chart as the level-multiplicity scaling of the
parafermion central extension: under matched-conventions transfer
through Drinfeld--Sokolov, the projective curvature
\(\Omega_{\mathrm{central}}|_{J(W^{(s)}(z)W^{(s')}(w))}\) on the
diagonal \(s=s'\) recovers the parafermion central charge
\(c_{W_N}=N-1+(2N(N+1)-N)/(k+N)\) as the chart-multiplicity
formula \([\hbar N\bar c]_{W_N}=[c_{W_N}]\); the off-diagonal
\(s\neq s'\) blocks are higher-spin Atiyah classes
\([\bar c]^{(s,s')}\) on the Drinfeld--Sokolov chart.

\paragraph{The \texorpdfstring{$N\to\infty$}{N -> infinity} admissible endpoint.}
The conditional admissible endpoint
\(W_\infty[\lambda]\Rightarrow E_\infty\) of the four-hypothesis
ladder (\S\ref{ssec:w-infinity-endpoint}; Prochazka triangular
truncation, Creutzig--Kanade--Linshaw parafermion compatibility,
Pope--Romans--Shen/Bakas, Yamada weight-window) governs the
\(N\to\infty\) limit: under (H1)--(H4) the \(N\to\infty\) Capelli
scaling
\([\hbar N\bar c]_{W_\infty[\lambda]}=
[\hbar c_{W_\infty[\lambda]}]\) lifts to the
\(\Omega_{\mathrm{central}}\)-cell of the four-curvature taxonomy
on the \(E_\infty\)-target.

\paragraph{Cross-volume bridge.}
The \(W_N\)-chart at the Drinfeld--Sokolov reduction transports
under matched-conventions transfer to
\(\sim/\mathrm{chiral\text{-}bar\text{-}cobar/}\) (Vol.~I; \(d=1\)
chiral Koszul; bar/cobar/centre distinction at \(A=W_N\)) and to
\(\sim/\mathrm{chiral\text{-}bar\text{-}cobar\text{-}vol2/}\)
(Vol.~II; \(d=1\) algebraic 3d-gravity HT sector at the
\(W\)-algebra chart) on the asymptotic-symmetry side of higher-spin
\(\mathrm{AdS}_3\).

The \(W_N\)-chart identification is proved at the Heisenberg core
(\S\ref{ssec:local-theorem}); promotion to the chiral Hochschild
target through Drinfeld--Sokolov is the standard construction of
\cite{frenkel-ben-zvi}*{\S 15.6}; the \(N\to\infty\) admissible
\(E_\infty\) endpoint is conditional on the four hypotheses
listed.

\subsubsection{Toric \texorpdfstring{$\C^3$}{C\^{}3}}\label{sssec:ex-c3}

\paragraph{Chart, brane line, \texorpdfstring{$\Omega$}{Omega}-background.}
Target \(X=\C^3\) with the toric Calabi--Yau holomorphic volume
\(\Omega=dz_1\wedge dz_2\wedge dz_3\); \(b=\) the origin
\(0\in\C^3\). The mixed-HT formal-Darboux stalk sees only the
formal holomorphic-symplectic stalk
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\) inside the \(\C^3\)-target.
The third direction supplies the twisted-M-theory equivariant
scaling of \cite{costello-twistedM} through the brane-preserving
\(\Omega\)-background torus
\(T_\Omega=\C^*_{\varepsilon_s}\times\C^*_{\varepsilon_1}
\times\C^*_{\varepsilon_2}\) acting by normal scaling on
\(N_LX=\R_s\oplus\C_{z_1}\oplus\C_{z_2}\) with the brane line
\(t\) fixed and the deformed BV operator
\(Q_\Omega=Q+\iota_{V_\Omega}\), \(Q_\Omega^2=L_{V_\Omega}\)
(theorem-control predicate seven of the convention layer).

\paragraph{Trace and the Hilbert-scheme chart.}
The chart algebra is the matrix-Chan--Paton completion
\(A_b=M_N(\C[[z_1,z_2]])^{T_\Omega}\) on the brane-preserving
torus-fixed locus; the trace
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\) on the
cyclic framed stable branch \(\C[\phi_1,\phi_2]\cdot v=\C^N\) is
the formal-Darboux stalk chart of \(\Hilb^N(\C^2)\) in its
Nekrasov--Schwarz noncommutative Moyal deformation
\cite{nekrasov-schwarz-noncommutative,costello-twistedM}, the brane
form of the Nekrasov vertex.

\paragraph{Capelli scalar at \texorpdfstring{$\C^3$}{C^3}.}
The Capelli identification of \S\ref{sec:capelli-scalar} reads at
\(\C^3\) under matched-conventions transfer to twisted M-theory:
\(\Omega_{\mathrm{central}}|_{J(f)}\) localises on the
\(T_\Omega\)-fixed tower of \(\Hilb^N(\C^2)\) with equivariant
weights \(\varepsilon_1,\varepsilon_2\), recovering the
\(N\)-instanton chart-multiplicity scaling
\([\hbar N\bar c]_{\C^3}=\hbar N\cdot
(\varepsilon_1+\varepsilon_2)/(\varepsilon_1\varepsilon_2)\) under
the Nekrasov normalisation, the Costello--Gaiotto twisted-M-theory
match \cite{costello-twistedM}.

\paragraph{Cross-volume bridge.}
The \(\C^3\) global reading through twisted M-theory is the
matched-conventions transfer to
\(\sim/\mathrm{calabi\text{-}yau\text{-}quantum\text{-}groups}/\)
(Vol.~III; CY-to-chiral two-stage
\(\Phi_3=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_2,C}\circ
\Phi^{\mathrm{FA}}_3\)); the formal-Darboux stalk at the brane
chart agrees with the Vol.~III \(d=3\) chart on the cyclic framed
stable branch.

The formal-Darboux stalk identification is proved at the
two-complex-dimensional brane chart
(\S\ref{ssec:local-theorem}); the global \(\C^3\)-toric reading
through twisted M-theory is matched-conventions external to the
local theorem.

\subsubsection{\texorpdfstring{$K3$}{K3}}\label{sssec:ex-k3}

\paragraph{Chart and trace.}
Target \(X=K3\); at every formal holomorphic-symplectic point
\(p\in K3\) the holomorphic Darboux theorem trivialises the germ
to \((\widehat{\C^2}_0,dz_1\wedge dz_2)\), so \(K3\) contributes
only its formal symplectic polydisk to the brane chart.  The
chart algebra at \(b=p\) is
\(A_b=\widehat{\mc O}_{K3,p}\cong\C[[z_1,z_2]]\) and the trace is
the formal-Darboux
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\) of
\S\ref{ssec:local-theorem}.

\paragraph{Capelli scalar and the elliptic genus.}
The Capelli class \([\hbar N\bar c]_{K3}\) at the \(K3\)-chart
agrees pointwise with the Heisenberg core
(\S\ref{sssec:ex-heisenberg}); under global descent through the
Mukai pairing the protected scalar trace of
\(\Omega_{\mathrm{central}}\)-twisted operators recovers the
elliptic genus
\(\mathrm{Ell}(K3;\tau,z)=\sum_{n,\ell}c(n,\ell)q^ny^\ell\) of
\cite{witten-elliptic-genus} as the level-\(2\) modular section
of the modular line on \(\overline{\mathcal A}_g\) at \(K3\):
\([\mathrm{Ell}(K3;\tau,z)]=[\Omega_{\mathrm{central}}]|_{K3}\)
under the Borcherds--Kac--Moody product expansion of
\cite{gritsenko-nikulin-paramodular} on the Mukai lattice.

\paragraph{Four-curvature cell.}
The class \([\hbar N\bar c]_{K3}\) lies on the projective
\((\Omega_{\mathrm{central}})\)-cell of
Theorem~\ref{thm:four-curvature-taxonomy}: under matched
identification of \(\Omega_{\mathrm{central}}\) with the modular
line at \(K3\) the curvature class is the Atiyah class of the
\(\det^{1/2}\) of the trace representation, and the elliptic
genus is its level-\(2\) section.

\paragraph{Cross-volume bridge.}
The \(K3\)-fibre data of the \(K3\times E\) example
(\S\ref{sssec:ex-k3-times-e}) is exactly this: the
\(\kappa_{\mathrm{fiber}}=24\) elliptic-genus fibre rank of the
five-layer \(\kappa\)-stratification and the \(\Delta_5\)
denominator at \(K3\times E\) reduce to the elliptic genus of
\(K3\) on the elliptic-fibre limit \(\tau\to i\infty\) at fixed
\(z\).

The formal-Darboux stalk identification is proved pointwise on
\(K3\) (\S\ref{ssec:local-theorem}); the global identification of
the chiral Hochschild target with the elliptic genus is a
matched-conventions transfer through descent, QME, anomaly, and
locality.

\subsubsection{\texorpdfstring{$K3\times E$}{K3 x E}}\label{sssec:ex-k3-times-e}

\paragraph{Chart and trace.}
Target \(X=K3\times E\) (3 complex dimensions); brane vacuum
\(b=(p,q)\in K3\times E\).  Holomorphic Darboux on
\(K3\) trivialises the germ \(\widehat{K3}_p\) to
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\); the elliptic factor
\(\widehat E_q\) supplies one transverse complex coordinate
\(\tau\).  The formal-Darboux stalk of
\S\ref{ssec:local-theorem} applies to the two-complex-dimensional
\(\widehat{K3}_p\)-factor; \(\tau\) parametrises the family of
formal-Darboux stalks along the elliptic fibre.  The chart algebra at
\(b\) is
\(A_b=\widehat{\mc O}_{K3,p}\,\widehat\otimes\,\widehat{\mc O}_{E,q}
\cong\C[[z_1,z_2]][[\tau]]\), and the trace map of
\S\ref{ssec:reduced-ce-pv-measurement} reads a Hamiltonian
\(f\in\mathfrak h\) on the brane chart through the Chan--Paton pair
\((\phi_1,\phi_2)\) of the \(K3\) directions:
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\), with
\(\tau\)-dependence inherited from the elliptic moduli.

\paragraph{Capelli scalar and four-curvature cell.}
The Capelli identification of \S\ref{sec:capelli-scalar},
\(\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}\), places the
class \(\hbar N[\bar c]\) on the
\((\Omega_{\mathrm{central}})\)-cell of the four-curvature taxonomy
of Theorem~\ref{thm:four-curvature-taxonomy}: at \(K3\times E\) the
modular line bundle \(\Omega_{\mathrm{central}}\) on
\(\overline{\mathcal A}_2\) has level-\(2\) section the Igusa cusp
form \(\Phi_{10}\in S_{10}(\mathrm{Sp}(2,\Z))\) of
\cite{igusa-cusp-form}; the paramodular Borcherds product is
\(\Delta_5=\Phi_{10}^{1/2}\) on the paramodular cover
\cite{gritsenko-nikulin-paramodular}.  Pointwise on the brane chart
the Capelli scalar identification is the statement
\([\hbar N\bar c]=[\Phi_{10}]|_{J(f)}\) at the trace generator,
modulo the Atiyah class of \(\det^{1/2}\) of the trace
representation.

\paragraph{Pro-Matlis residue.}
The cotangent module of the \(K3\)-factor is
\(\mathfrak h^\vee_{\mathrm{cont}}=\mathcal P=\bigoplus_{a+b>0}
\C\,\rho_{a,b}\) with
\(\rho_{a,b}=z_1^{-a-1}z_2^{-b-1}dz_1dz_2\); the elliptic factor
contributes the elliptic period lattice through \(\tau\).  The
pro-Matlis target \(\widehat{\mathcal P}_\Pi\) along the \(N\)-tower
of finite Matlis windows is the categorical home of the
four-curvature taxonomy at this example
(Theorem~\ref{thm:promatlis-universal-property}); the
Bochner--Martinelli no-go on each finite window
(Theorem~\ref{thm:bmk-finite-window-no-go}) forces the inverse
limit.

\paragraph{Five-layer \texorpdfstring{$\kappa$}{kappa} stratification.}
Five distinct construction layers attach five \(\kappa\)-numbers,
none of which collapses to another:
\(\kappa_{\mathrm{cat}}=\chi(\mathcal O_{K3\times E})=0\) by
K\"unneth multiplicativity;
\(\kappa^{\mathrm{Hodge}}_{\mathrm{ch}}
=\sum_q(-1)^q h^{0,q}(K3\times E)=0\);
\(\kappa^{\mathrm{Heis}}_{\mathrm{ch}}=3\) on the
Heisenberg--Mukai chart;
\(\kappa_{\mathrm{BKM}}(\Delta_5)=c_1(0)/2=5\) of
Borcherds--Gritsenko--Nikulin; and the fibre rank
\(\kappa_{\mathrm{fiber}}=24\) of the extended Mukai lattice.  The
spectral-sequence collapse of these five layers through the
Hall--Drinfeld--Borcherds comparison sequence is conjectural per
seventeen-site row 6
(\S\ref{ssec:cross-volume-firewall}, Vol.~III matched-conventions
data).  Cross-volume bridges:
\(\sim/\mathrm{calabi\text{-}yau\text{-}quantum\text{-}groups}\)
(Vol III; CY-to-chiral two-stage
\(\Phi_d=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}\circ
\Phi^{\mathrm{FA}}_d\)) and
\(\sim/\mathrm{igusa\text{-}cusp\text{-}form}\) (Borcherds lift
\(\Phi_{10}\), denominator \(\Delta_5\)).

\paragraph{Open problems forced by this example.}
(i) The operator-level lift of the modular line on \(K3\times E\)
(\S\ref{ssec:open-modular-lift}): the candidate operator-valued
cocycle on the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\) at the \(K3\times E\)
brane vacuum (the chiral \(E_2\) factorization algebra restricted to
the formal neighbourhood of \(b\)), twisted by
\(\Omega_{\mathrm{central}}\) at level 2, whose
protected scalar trace at paramodular CM points recovers
\(\Phi_{10}\); its construction is the cross-volume firewall
acceptance class \(\mathrm{ob}_{\mathrm{Igusa/BKM}}\) of
Theorem~\ref{thm:cross-volume-firewall}.  (ii) The
Hall--Drinfeld--Borcherds comparison spectral sequence
(\S\ref{ssec:open-hall-drinfeld-borcherds}): five-layer collapse
to a single \(\kappa\)-invariant; the comparison residual is the
Hall--Drinfeld--Borcherds obstruction class.

Pointwise formal-Darboux stalk: proved
(\S\ref{ssec:local-theorem}); five-layer \(\kappa\)-stratification:
named, not collapsed; modular section \(\Delta_5\): the
Borcherds--Gritsenko--Nikulin datum; spectral-sequence collapse
and operator-level lift: the two open problems above.

\subsubsection{Generic compact CY\(_3\)}\label{sssec:ex-generic-cy3}

Target \(X=\) compact Calabi--Yau threefold of generic type;
\(b=\) any formal holomorphic-symplectic point in a chosen
two-complex-dimensional slice transverse to the holomorphic
volume. The mixed-HT formal-Darboux stalk sees only the
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\) of
\S\ref{ssec:local-theorem}. The full closed-sector
identification on \(X\) requires:
formal Darboux on the two-complex-dimensional slice; descent
across the Weiss/Cech--Roos open cover with the Cech
cone~\eqref{eq:setup-X-L} converging; QME closure of the
brane-defect propagator at every loop order with the Costello
counterterm chosen and the \(\theta_3\) graph-row obstruction
\(\mathfrak O_{\theta_3}\) of
Theorem~\ref{thm:first-third-costello-normalizations} resolved;
anomaly cancellation through the Capelli scalar
\(\hbar N[\bar c]\) on the global complex with
\([\bar c]_{\mathrm{glob}}\) vanishing; and locality of
the bulk-to-defect kernel. Joint vanishing of the
matched-conventions cross-volume obstruction
\(\mathfrak o_{\mathrm{cv}}\) of \S\ref{ssec:tate-residual-resolution}
transports the formal-Darboux stalk into a global identification on
\(X\); pointwise the identification is the local theorem of
\S\ref{ssec:local-theorem}.

Each example of \S\ref{sec:examples} records its four-curvature
cell, its Capelli scalar value, and its cross-volume bridge.  The
\(N\to\infty\) direction of the
\(W_N\to W_\infty[\lambda]/E_\infty\) family lifts the
chart-multiplicity formula
\([\hbar N\bar c]_{W_N}=[c_{W_N}]\) to a conditional admissible
endpoint on \(\Lambda_{\mathrm{adm}}\subset\{(N,k,\lambda)\}\).

\section{The \texorpdfstring{$\mathcal{W}_\infty[\lambda]/E_\infty$}{W-infty[lambda]/E-infty} admissible endpoint}\label{sec:w-infinity-endpoint}\label{ssec:w-infinity-endpoint}
...
\emph{conditional}: each of the four hypotheses is a proved
structural input in the cited literature, whose composite gives the
named limit.
...
\begin{thm}[{Conditional $\mathcal W_\infty[\lambda]/E_\infty$ admissible endpoint}]\label{thm:w-infty-e-infty-admissible}
  ...
  \[
    \lim_{N\to\infty}A^{\mathrm{cl}}_{\partial,N}\Big|_{\Lambda_{\mathrm{adm}}}
      \;\simeq\;
      \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\Big|_{b,\,\Lambda_{\mathrm{adm}}}
  \]
  as $E_\infty$-algebras over the Yamada window (the chiral $E_2$
  factorization algebra restricted to the formal neighbourhood of $b$,
  with native diagonal OPE
  Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}, evaluated
  on the admissible locus), with chart the
  CE/PV dictionary
  \[
...
computation through spin~$8$.  These checks concern the transported
\(\mathcal W_\infty[\lambda]\) one-variable spin OPE, not the native
two-dimensional diagonal OPE, which is fixed by
Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  Per the
...
\paragraph{Remaining hypotheses.}  Three inputs would remove the
conditional clauses from the admissible endpoint.
...
\begin{enumerate}
  \item[(O1)] \emph{Closed-form OPE on $\Lambda_{\mathrm{adm}}$.}
    ...
    Bakas provide the spin-$\leq 8$ check; the all-spin closed form
...
(H1)--(H4) and the inputs (O1)--(O3), is the
...
The formal-Darboux stalk theorem is the tangent vector to the global
Mixed Holomorphic--Topological Deligne identification
$Z^{\mathrm{der}}_{E_d^{\mathrm{ch}}}(\mathcal{C}^{\mathrm{op}}_\partial)
\simeq\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$.  Three open problems
follow: the chiral $E_d$ Deligne theorem at $d\geq 2$
(\S\ref{ssec:open-chiral-Ed-Deligne}); the
Hall--Drinfeld--Borcherds comparison spectral sequence on compact
non-toric CY$_3$ (\S\ref{ssec:open-hall-drinfeld-borcherds}); the
operator-level lift of the modular line on $K3\times E$
(\S\ref{ssec:open-modular-lift}).  Three frontier inputs anchor them:
...
purely chiral statement on the bulk side; the local theorem does not
prove it.
...
(\S\ref{ssec:cross-volume-firewall}, Vol.~III
...
the modular line is the bulk
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b$ (the chiral $E_2$
factorization algebra at the brane vacuum of the $K3\times E$
holomorphic-topological theory, restricted to the formal
neighbourhood of $b$), twisted by $\Omega_{\mathrm{central}}$ at
level~$2$.
...
\begin{conj}[Operator-level lift of $\Phi_{10}$]\label{conj:operator-modular-lift}
  ...
  boundary of $K3\times E$.  The bulk
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b$ at the formal-Darboux
  stalk carries a canonical lift of
...
Darboux coordinate expression of the brane-side derived
$E_1$-Hochschild $HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})$ at one
Dirac brane vacuum.  The Mixed Holomorphic--Topological Deligne
conjecture identifies this at the formal-Darboux stalk with the bulk
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}$ (the chiral $E_2$
factorization algebra on $\C^2$ restricted to the formal neighbourhood
of $b_p$) with native singular product
\[
  \mathsf{J}_x(z)\mathsf{J}_y(w)
  \sim
  K^{(2)}_\Delta(z,w)\mathsf{J}_{[x,y]_{\mathfrak g}}(w),
\]
generalising the Beilinson--Drinfeld chiral Deligne theorem
proved at $d=1$ for chiral algebras factorising over algebraic curves.  The
global identification at $d=2$, the operator-level lift of the
modular line on $K3\times E$, and the chiral $E_d$ Deligne theorem
at $d\geq 2$ are the open problems the formal-Darboux stalk has now
made well-posed.
...
and Harish-Chandra radial-parts theorem supply the formal-stalk-chart side;
...
\appendix
\input{appendix-higher-factorization-categories}
...
\input{appendix-completion-blueprint}
...
\bib{milnor-axiomatic-homology}{book}{
...
\bib{lorgat-vol1}{misc}{
   ...
   note={Manuscript, Volume I of the constellation},
   date={2026},
}
...
\bib{kato-log}{article}{
   author={Kato, Kazuya},
   title={Logarithmic structures of Fontaine--Illusie},
   conference={
      title={Algebraic analysis, geometry, and number theory (Baltimore, MD, 1988)},
   },
   book={
...
\bib{kato-nakayama-log}{article}{
   author={Kato, Kazuya},
   author={Nakayama, Chikara},
   title={Log Betti cohomology, log \'etale cohomology, and log de Rham cohomology of log schemes over $\mathbf{C}$},
