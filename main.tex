...
\end{titlingpage}
  ...
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$.  It factorises over
  the holomorphic surface in the sense of Costello--Gwilliam
  ...
  $[\phi_1,\phi_2]=0$.  The antifield
  $\psi\in\mathfrak{gl}_N[1]$ with $Q\psi=[\phi_1,\phi_2]$
  gives the Koszul resolution of the moment-map ideal, equivalently
  the derived zero fibre of
  \(\mu(\phi_1,\phi_2)=[\phi_1,\phi_2]\).  The classical
  open-string algebra on the worldline is the
  ...
  \]
  the $GL_N$-invariant function obtained by substituting the matrices
  \(\phi_i\) into \(f\) and taking the trace on the derived commuting
  variety \(\mu^{-1}_{\mathrm{der}}(0)\) at the brane vacuum
  \(b\in\C^2\).
  ...
  neighbourhood of $b$, identified with the brane-side derived
  ...
  equivalence.  Theorem~\ref{thm:main-local} is a local
  formal-Darboux statement.  Away from the stalk, the corresponding
  global assertion is the open chiral \(E_d\)-Deligne conjecture
  at \(d=2\).
  ...
  \medskip\noindent
  Concretely the closed observables are the
  Chevalley--Eilenberg chains of the local Lie algebra
  $\Omega^\bullet_c(U)\widehat\otimes\Omega^{0,\bullet}_c(B)
  \widehat\otimes\mathfrak g$ on $U\subset\R^2_{\mathrm{top}}$,
  $B\subset\C^2_{\mathrm{hol}}$
  (Definition~\ref{def:local-th-string},
  Proposition~\ref{prop:three-localities}); pushforward along
  $\pi\colon X\to\C^2_{\mathrm{hol}}$ integrates out the topological
  factor and gives
  ...
  Corollary~\ref{cor:E2-bulk-algebra}); the $E_2$-valuation arises
  from the locally constant factorization-algebra structure on
  $\R^2_{\mathrm{top}}$ (Lurie--Costello--Gwilliam).  The native
  holomorphic OPE of $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is a
  two-variable expansion on the diagonal
  $\Delta\subset\C^2_z\times\C^2_w$: the singular coefficient
  module is the Grothendieck local-cohomology principal-part module
  $H^2_\Delta(\C[[z_1,z_2,w_1,w_2]])\,d\zeta_1d\zeta_2$, with
  arity-two Cauchy kernel
  $K^{(2)}_\Delta(z,w)=d\zeta_1\,d\zeta_2/(\zeta_1\zeta_2)$ in formal
  diagonal coordinates $\zeta_i=z_i-w_i$
  (Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}).
  Restriction along a holomorphic curve $C\hookrightarrow\C^2$
  specialises this kernel to the one-variable curve Cauchy kernel
  $K^{(1)}_C(z,w)=dz/(z-w)$ and produces a chiral algebra on $C$
  whose generators and relations are the one-variable specialisation
  of the two-dimensional OPE; the specific brane chart $A_b$ at a
  chosen vacuum $b$ specialises to a known vertex algebra (Heisenberg,
  affine Kac--Moody $V_k(\mathfrak g_0)$, $\beta\gamma$, Virasoro
  $\mathrm{Vir}_c$, or $W_N(\mathfrak{sl}_N)_k$ via Drinfeld--Sokolov;
  \S\ref{sec:examples}).

  \medskip\noindent
  The closed-to-open dictionary on Chevalley--Eilenberg generators of
  $\mathfrak g$ assigns to each generator-dual coordinate $c_f$ the
  Hamiltonian Schouten derivation
  $\theta_f\in HH^1(A^{\mathrm{cl}}_{\partial,N})$ and to each
  cotangent-dual coordinate $u_f$ the brane observable
  $O_f=J(f)\in HH^0(A^{\mathrm{cl}}_{\partial,N})$
  (Theorem~\ref{thm:closed-to-open-derived-centre-map}).  Lifted to
  factorisation algebras, this is the chain map
  \[
    \Phi_N\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk},0}
    \longrightarrow Z^{\mathrm{der}}_{E_1}
       \bigl(A^{\mathrm{cl}}_{\partial,N}\bigr)
  \]
  whose existence at the chain level is conditional on vanishing of
  ...
  null-homotopies supplied by the completion constructions of
  ...
  \medskip\noindent
  Across volumes, the bulk algebra
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is sourced from a
  Calabi--Yau category $\mathcal C$ through the two-stage functor
  $\Phi_d=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}
  \circ\Phi^{\mathrm{FA}}_d$ --- the
  Calabi--Yau-to-factorization-algebra functor $\Phi^{\mathrm{FA}}_d$
  followed by Lurie--Ayala--Francis factorization-homology
  specialisation $\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}(\mathcal F)
  =\int_{\Sigma_{d-1}}\mathcal F|_C$; at $d=2$, $\mathcal C$ is the
  Calabi--Yau category of the constellation Vol.~III
  (calabi-yau-quantum-groups).  Conventions match the global theorem
  through the matched-conventions acceptance class
  $\operatorname{Ob}_{\mathrm{UKD}}$ of
  Theorem~\ref{thm:matched-conventions-transfer}: a nonzero
  coordinate of $\operatorname{Ob}_{\mathrm{UKD}}$ is the
  obstruction to transporting the local theorem to a compact-target
  programme (BCOV/Kodaira--Spencer, Vol.~III CY-to-chiral,
  Igusa/BKM, GW/DT/PT/MNOP, OSV).
...
The native bulk object is the two-complex-dimensional factorization
algebra on $\C^2$; restricting it along a holomorphic curve
$C\hookrightarrow\C^2$ produces a curve vertex operator algebra
(VOA) whose generators and relations are the one-variable
specialisation of the two-dimensional OPE, with the Zhu algebra and
the Vol.~II $\C\times\R$ chiral-topological theorems applying to the
restricted curve theory after the controlled reduction supplies
explicit $z_2$-mode or principal-part data, pushed-forward bracket,
BV pairing, brane image, and anomaly matching
...
the bulk.  For a chiral algebra $A$ on a smooth curve $C$ the $d=1$ chiral
derived centre is
...
The boundary open category on the Kato--Nakayama (KN) log boundary
$\partial X^{\mathrm{KN}}$ is
$\mathcal C^{\mathrm{op}}_\partial=
\Phi^{\mathrm{FA}}_d(\mathcal A)\big|_{\partial X^{\mathrm{KN}}}$,
the restriction of the bulk factorisation algebra to the boundary
in the sense of Costello--Gwilliam.  Factorisation-homology
...
(Lurie--Ayala--Francis).  The Dirac brane stack at $N$ branes is the
...
$\mathfrak{gl}_N\oplus\mathfrak{gl}_N$.  The Capelli scalar identification
...
$Y\supset\widehat{Y}_p$ transports the local theorem to that
ambient datum after matched-conventions descent
(\S\ref{ssec:cross-volume-firewall}).
...
polynomial PBW shadows of the open one-$\psi$ sector.
...
A \emph{local string theory} in the sense of Costello--Gwilliam
\cite{costello-gwilliam} is a classical BV theory whose local
observables form a factorization algebra equipped with a cyclic open
BV theory on a brane and a bulk-to-boundary central operation.  The
...
directions.  The word \emph{string} appears here in this local
string-field-theoretic sense, attached to the BV / factorization data
rather than to a global theory of worldsheet moduli or amplitudes.
...
\begin{rmk}[Worked instances]\label{rmk:Jf-uniqueness-running-examples}
  ...
  \S\ref{ssec:formal-darboux-stalk-one-calculation} determine the theorem
...
The chart algebra at any brane vacuum $b\in\mathcal C^{\mathrm{op}}_\partial$
is uniformly the formal coordinate ring
$A_b=\End_{\mathcal C^{\mathrm{op}}_\partial}(b)\cong\C[[z_1,z_2]]$
(Definition~\ref{def:formal-darboux-stalk-chart}); each example below
specifies (i) the Chan--Paton substitution
$\rho_\phi\colon A_b\to M_N(\C)\otimes\mathfrak g_0$ implanting a
chosen flavour Lie algebra $\mathfrak g_0$ on the brane stack, (ii)
the holomorphic curve $C\hookrightarrow\C^2$ along which the bulk
factorisation algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is restricted, and
(iii) the specific vertex algebra produced by that restriction
(Heisenberg, $V_k(\mathfrak g_0)$, $V^{\beta\gamma}$,
$\mathrm{Vir}_c$, $W_N(\mathfrak{sl}_N)_k$) with its $d=1$ chiral
derived centre target.  Status: proved at the Heisenberg core
(\S\ref{ssec:local-theorem}); conditional on matched-conventions
transfer through the chosen reduction; or open at the global
compact-target identification beyond the formal-Darboux stalk,
requiring descent, QME, anomaly, and locality data on the target
geometry.
...
Take $\mathfrak g_0$ a finite-dimensional simple complex Lie algebra,
$N=\dim\mathfrak g_0$, and brane vacuum $b=b_p$ at $p\in\C^2$ with
chart algebra $A_b=\widehat{\mathcal O}_{\C^2,b}\cong\C[[z_1,z_2]]$.
The Chan--Paton substitution
$\rho_\phi\colon A_b\to M_N(\C)$, $z_1\mapsto\phi_1$,
$z_2\mapsto\phi_2$ valued in $\mathfrak g_0\otimes\mathfrak{gl}_N$
implants $\mathfrak g_0$-flavour on the brane stack; the trace
$J(f)=\overline{\operatorname{Tr}\,\rho_\phi(f)}$ becomes the
Sugawara projection of the polynomial Hamiltonian.  Restricting
the bulk factorisation algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ along a holomorphic curve
$C\hookrightarrow\C^2$ transverse to the brane line produces the
affine Kac--Moody vertex algebra $V_k(\mathfrak g_0)$ at level $k=N$,
with affine current $J^a(z)$ in the standard
Knizhnik--Zamolodchikov form on $C$
(\cite{frenkel-ben-zvi}*{\S 2.4}).  The Capelli identification
$\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}$ specialises along
this curve restriction to the Kac--Moody central charge: the
projective $(\Omega_{\mathrm{central}})$-cell of
Theorem~\ref{thm:four-curvature-taxonomy} with chart-multiplicity
formula $[\hbar N\bar c]_{\widehat{\mathfrak g}_k}=[\hbar k]$.  On
the curve $C$ the chiral derived centre is
$Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\mathfrak g_0))\simeq
C^\bullet_{\mathrm{ch},d=1}(V_k(\mathfrak g_0),
V_k(\mathfrak g_0))$ by the $d=1$ chiral Deligne theorem
\cite{beilinson-drinfeld-chiral}*{\S 4}; the $d=2$ formal-Darboux
stalk identifies $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\simeq
Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})$ by
Costello--Gwilliam bulk--boundary correspondence, and the curve
restriction recovers the $d=1$ statement on $C$.  Identification at
the Heisenberg core is proved (\S\ref{ssec:local-theorem}); the
Sugawara reading to the full $\widehat{\mathfrak g}_k$ curve
specialisation is the standard Frenkel--Ben-Zvi datum, pending a
matched-conventions transfer at the joint level/multiplicity scaling
$(k,N)\to\hbar k N[\bar c]$.

\subsubsection{\texorpdfstring{\(\beta\gamma\)}{beta-gamma} system}\label{sssec:ex-betagamma}

Take $N=1$ and brane vacuum $b=b_p$ at $p\in\C^2$ with chart
algebra $A_b=\widehat{\mathcal O}_{\C^2,b}\cong\C[[z_1,z_2]]$.  The
Chan--Paton substitution $(\phi_1,\phi_2)=(z_1,z_2)$ is the
two-variable formal Heisenberg pair, with contact term
$\hbar K^{(2)}_\Delta(z,w)\mathbf 1$ in the native
two-dimensional OPE
(Corollary~\ref{cor:quantum-brane-trace-two-dimensional-ope}).
Restricting the bulk factorisation algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ along a holomorphic curve
$C\hookrightarrow\C^2$ on which $z_1$ becomes a weight-$1$
holomorphic field and $z_2$ a weight-$0$ partner produces the free
$\beta\gamma$ vertex algebra $V^{\beta\gamma}$ at central charge
$c=2$, with $\beta(z)=z_1|_C$, $\gamma(z)=z_2|_C$ satisfying the
standard $\beta\gamma$ OPE $\beta(z)\gamma(w)\sim 1/(z-w)$
(\cite{costello-gwilliam}*{Vol.~II, \S 5.4}); the trace
$J(z_1^k z_2^l)=\overline{\operatorname{Tr}}(\beta^k\gamma^l)$
collapses on $C$ to the Wick-ordered monomial.  The Capelli
scalar $\hbar N[\bar c]$ equals $\hbar c$ with $c=2$ on this curve
restriction: the projective $(\Omega_{\mathrm{central}})$-cell of
Theorem~\ref{thm:four-curvature-taxonomy} with chart-multiplicity
formula $[\hbar N\bar c]_{\beta\gamma}=[\hbar\cdot 2]=[\hbar c]$.
On $C$ the $d=1$ chiral derived centre is
$C^\bullet_{\mathrm{ch},d=1}(V^{\beta\gamma},V^{\beta\gamma})$, the
chiral derived centre of the free $\beta\gamma$ factorization
algebra of \cite{costello-gwilliam}*{Vol.~II~\S 5.4}.  Proved at
$c=2$ on the curve restriction; the $d=2$ formal-Darboux stalk
identification at the brane vacuum is the local theorem of
\S\ref{ssec:local-theorem} with the same Heisenberg cocycle
controlling the brane centre-of-mass
...
Brane vacuum $b=b_p$ at $p\in\C^2$ with chart algebra
$A_b=\widehat{\mathcal O}_{\C^2,b}\cong\C[[z_1,z_2]]$ and
Chan--Paton pair $(\phi_1,\phi_2)$.  Restricting the bulk
factorisation algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ along a holomorphic curve
$C\hookrightarrow\C^2$ on which the Sugawara composite of the
Heisenberg-core fields specialises to the stress tensor produces
the Virasoro vertex algebra $\mathrm{Vir}_c$ at central charge $c$,
with generators $L_n$, $n\in\Z$, and Virasoro relations
$[L_n,L_m]=(n-m)L_{n+m}+\tfrac{c}{12}(n^3-n)\delta_{n+m,0}$
\cite{frenkel-ben-zvi}*{\S 2.5}.  For a Hamiltonian
$f\in\mathfrak h$ admitting a Sugawara presentation
$L_n=\tfrac12\sum_{k\in\Z}{:}a_{n-k}a_k{:}$ with
$a_n\in\widehat{\mathfrak{gl}}_1$, the trace specialises on $C$ to
\[
  J(f)|_C=\overline{\operatorname{Tr}\,f(L_n)}
...
Brane vacuum $b=b_p$ at $p\in\C^2$ with chart algebra
$A_b=\widehat{\mathcal O}_{\C^2,b}\cong\C[[z_1,z_2]]$ and
Chan--Paton pair $(\phi_1,\phi_2)$ valued in
$\mathfrak{sl}_N\otimes\mathfrak{gl}_N$.  Restricting the bulk
factorisation algebra
$\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ along a holomorphic curve
$C\hookrightarrow\C^2$, then applying principal Drinfeld--Sokolov
reduction to the affine $\widehat{\mathfrak{sl}}_N$ current algebra
produced on $C$, gives the $W_N(\mathfrak{sl}_N)_k$ vertex algebra
...
$W^{(3)},\dots,W^{(N)}$ at integer weights up to $s=N$.  The trace
specialises on $C$ to
$J(f)|_C=\overline{\operatorname{Tr}\,\rho_\phi(f)}$ on polynomial
Hamiltonians in the $W^{(s)}$-currents, the iterated Sugawara
projection from the Heisenberg core (\S\ref{sssec:ex-heisenberg})
through the $\widehat{\mathfrak{sl}}_N$ parafermion at level $k$.
On $C$ the $d=1$ chiral derived centre is
$C^\bullet_{\mathrm{ch},d=1}(W_N,W_N)$.
