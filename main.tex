...
\end{titlingpage}
  ...
  \]
  carries a stack of \(N\) branes on the topological line \(L\); the
  holomorphic factor is read through its formal symplectic disk
  \((\widehat{\C^2}_0,dz_1\wedge dz_2)\) at the origin.

  A stack of \(N\) branes records its position inside the normal disk
  through a probe pair of \(\mathfrak{gl}_N\)-valued matrices
  \(\phi_1,\phi_2\); \(GL_N\)-conjugation has moment map
  \(-\operatorname{Tr}(\epsilon[\phi_1,\phi_2])\), so the Dirac
  first-class boundary action is
  \[
    \int_{\R_t}\operatorname{Tr}
      \bigl(\phi_1\,d\phi_2+A[\phi_1,\phi_2]\bigr),
    \qquad
    Q\psi=[\phi_1,\phi_2].
  \]

  Infinitesimal canonical transformations of the holomorphic-symplectic
  disk are Hamiltonian vector fields, and constants have zero
  Hamiltonian vector field; the closed Lie algebra of the slice is the
  Hamiltonian Lie algebra of the disk modulo constants,
  \(\mathfrak h=\C[[z_1,z_2]]/\C\cdot 1\), and a canonical-transformation
  BV theory has fields valued in its shifted cotangent extension
  \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\).
  Continuity of the disk topology forces the continuous dual to be the
  Grothendieck--Matlis principal-part module
  \(\mathcal P=\bigoplus_{a+b>0}\C\rho_{a,b}\) with
  \(\rho_{a,b}=z_1^{-a-1}z_2^{-b-1}dz_1dz_2\); polynomial trace classes
  produced by the matrix probe see only its PBW special-fibre shadow.
  ...
  finite-dimensional Lie algebra \(\mathfrak l\) the Schouten identity
  turns continuous CE cochains of \(T^*[1]\mathfrak l\) into Schouten
  polyvectors of the symmetric algebra \(\mathbf S(\mathfrak l)\);
  whether this identity, and its descendant \(P_0\)-bracket, kernel,
  and Koszul-duality structure, lift from finite-dimensional
  \(\mathfrak l\) to the infinite-dimensional disk algebra
  \(\mathfrak h\) is the question of the manuscript.

  The center-of-mass Poisson pair on the unreduced brane phase space
  is \(\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N\); after
  scalar reduction it becomes the generator \([\bar c]\) of a
  one-dimensional central extension of \(\mathfrak h\), and Moyal
  quantisation returns it as the Capelli term \(\hbar N[\bar c]\) --
  the first quantum master equation obstruction in the ordinary
...
The brane is the topological line
\[
  L=\R_t\times\{s=z_1=z_2=0\}
  \subset
  X=\R_t\times\R_s\times\C^2_{z_1,z_2}.
\]
The normal holomorphic disk is
\[
  (\widehat{\C^2}_{0},\omega),\qquad \omega=dz_1\wedge dz_2 .
\]
An ambient holomorphic symplectic surface contributes only its formal
Darboux disk at the chosen point.

The brane position in the normal disk is represented on an \(N\)-probe
stack by
\[
  z_1\mapsto\phi_1,\qquad z_2\mapsto\phi_2,\qquad
  \phi_i\in\lie{gl}_N .
\]
The matrix symplectic form and its potential are
\[
  \Omega_N=\operatorname{Tr}(d\phi_1\wedge d\phi_2),
  \qquad
  \theta_N=\operatorname{Tr}(\phi_1\,d\phi_2).
\]
Conjugation by \(GL_N\) has moment map
\[
  \mu_\epsilon=-\operatorname{Tr}(\epsilon[\phi_1,\phi_2]),
  \qquad \epsilon\in\lie{gl}_N .
\]
The first-class constraint is
\[
  [\phi_1,\phi_2]=0.
\]
With multiplier \(A\in\Omega^1(\R)\otimes\lie{gl}_N\), the boundary
action is
\begin{equation}\label{eq:open-action-derived}
S_\partial
  =\int_\R\operatorname{Tr}\bigl(\phi_1\,d\phi_2+A[\phi_1,\phi_2]\bigr).
\end{equation}
The linear Dirac normalization used to pass from the shifted open
Chern--Simons coefficients to \(\phi_1,\phi_2,A\) is fixed below in
Lemma~\ref{lem:open-action-reduction} and
Proposition~\ref{prop:open-bv-truncation}.

The BV reduction is the Koszul complex of the moment map:
\begin{equation}\label{eq:open-Q}
Q\psi=[\phi_1,\phi_2],\qquad Q\phi_1=Q\phi_2=0,
\end{equation}
where \(\psi=A^*\in\lie{gl}_N[1]\).  At a brane point
\[
\mathcal R_N
  =
...
\]
The ghost-zero open observables are
\[
  H^0(\mathcal R_N^{GL_N},Q),
\]
the invariant functions on the derived zero-fibre of the conjugation
moment map.  Lemma~\ref{lem:dirac-probe-reduction} records the
constrained-mechanics form of this statement.

\subsection{The Hamiltonian BF algebra}

The closed infinitesimal symmetries of the formal disk are Hamiltonian
vector fields.  Constants have zero vector field.  Hence
...
\[
  ...
  \pmod{\C}.
\]
Polynomial trace and Moyal computations use \(\bar A\).  Completed
CE/Koszul constructions use \(\widehat{\mathfrak h}\).  Its continuous
dual is
\[
  \mathfrak h^\vee=
  \widehat{\mathfrak h}^{\vee}_{\mathrm{cont}},
\]
with basis \(\delta_{k,l}\), \(k+l>0\).  The symbol
\(\bar A_{\mathrm{desc}}\) denotes an abstract copy of \(\bar A\) whose
nonconstant basis vectors are represented by the first descendant classes
\(\Psi_{k,l}\) in Construction~\ref{constr:boundary-evaluation}.
...
\begin{rmk}[Scalar Hamiltonian and center-of-mass term]\label{rmk:scalar-hamiltonian-center-of-mass}
  ...
  \[
    \{z_1,z_2\}=1\equiv0.
  \]
  On the unreduced brane phase space,
  \[
    \{\operatorname{Tr}(\phi_1),\operatorname{Tr}(\phi_2)\}=N.
  \]
  This is the center-of-mass symplectic pair.  Every bracket involving
  linear Hamiltonians is read after projection to the Hamiltonian quotient
  by scalar trace classes.  The same scalar appears after quantization as
  the Capelli shift of Remark~\ref{rmk:normal-ordering-obstruction}.
\end{rmk}

BCOV \cite{bcov} fixes the local language of Dolbeault polyvectors,
divergence, and cotangent BV fields.  The theorem here is the
two-complex-dimensional Hamiltonian analogue on the formal symplectic
disk.  The closed algebra is
...
coadjoint action on \(\mathfrak h^\vee[1]\).  The closed observables are
...
\]
The proved cotangent CE/PV identity
(Theorem~\ref{thm:reduced-ce-pv-central-operation}) lives on the
coordinate envelope
\(C^\bullet_{\mathrm{CE},\mathrm{coord}}(\mathfrak g)\), the product over
finite graded-commutative words in the CE coordinates; the continuous
CE cochains are the continuity-controlled subspace inside that product,
and the \(P_0\)-bracket and Maurer--Cartan kernel refinements live
exactly on bracket-admissible
\(B\subset\mathrm{PV}_{\mathrm{coord}}(A_{\partial,\mathrm{coord}})\)
(Definition~\ref{defn:bracket-kernel-admissible}).

For a finite Lie algebra \(\mathfrak l\), CE cochains on
\(T^*[1]\mathfrak l\) are Schouten polyvectors on
\(\mathbf S(\mathfrak l)\).  The generator map is
\[
  c^I\longmapsto\theta^I,\qquad u_I\longmapsto O_I.
\]
Here \(c^I\) is dual to a Hamiltonian generator, \(u_I\) is dual to the
shifted cotangent generator, \(\theta^I\) is the Schouten vector-field
coordinate, and \(O_I\) is the corresponding boundary Hamiltonian
function.  The formal disk is obtained by finite-coordinate coefficient
tests.  Bar-cobar or enveloping algebra conclusions require the
admissible Tate, nilpotent, relative, or weighted hypotheses stated
later.

The PBW Rees algebra has special fibre
...
\]
realized by Matlis principal parts \(\rho_{a,b}\).  Theorem
\ref{thm:app-matlis-rees-fourier-bridge} gives only the stated
Fourier-polarized Rees bridge between these label systems.
Proposition~\ref{prop:hamiltonian-polyvector-reduction} derives
\(\widehat{\mathfrak h}\) from divergence-closed polyvectors on the
formal disk.
...
\(c^I,u_I\) &
CE coordinates; \(c^I\) is Hamiltonian-dual, \(u_I\) is cotangent-dual.\\
\(\theta^I,O_I\) &
PV coordinates; \(c^I\mapsto\theta^I\), \(u_I\mapsto O_I\).\\
\(O_{a,b}\) &
Open trace class
\(\overline{\operatorname{Tr}(\phi_1^a\phi_2^b)}\), \(a+b>0\).\\
\(\Psi_{a,b}\) &
Primitive one-\(\psi\) PBW special-fibre class after removing the scalar
descendant.\\
\(\rho_{a,b}\) &
Principal part
\(z_1^{-a-1}z_2^{-b-1}dz_1dz_2\), a basis element of
...
\begin{rmk}[Separation convention]\label{rmk:intro-symbol-separation}
  The boundary Hamiltonian observable is the image of the shifted-cotangent
  coordinate \(u\).  The Hamiltonian CE coordinate \(c\) gives a vector-field
  operation.  The polynomial class \(\Psi_{a,b}\) and the principal part
  \(\rho_{a,b}\) have the same bidegree label and lie in different
  modules.  The CE/PV comparison gives the reduced Hamiltonian
  \(P_0\)-central-operation model.  The unreduced
  \(P_0\)-factorization-centre theorem is
  Problem~\ref{prob:formal-factorization-center}.
\end{rmk}

\subsection{The reduced CE/PV measurement}

A Hamiltonian \(f\) is measured on the brane by substitution:
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}.
\]
At finite \(N\) a word ordering is chosen.  In
\(H^0(\mathcal R_N^{GL_N},Q)\), adjacent swaps are exact:
\[
  Q\,\operatorname{Tr}(\psi vu)
    =
  \operatorname{Tr}([\phi_1,\phi_2]vu).
\]
Thus the stable connected class \(J(f)\) depends only on the Hamiltonian.
...
\end{equation}
after scalar reduction.  For monomials,
...
Taylor degree truncation does not give finite Lie quotients of
\(\mathfrak h\).  Linear Hamiltonians lower degree, projected brackets
fail Jacobi, and the diagonal cotangent Casimir misses the strict
product/direct-sum endpoint.  Finite coordinate projections are used only
as coefficient tests.  They assemble to the completed coordinate
isomorphism
\[
...
\]
A \(P_0\) comparison is asserted after choosing a bracket-admissible
completion \(B\).  A Maurer--Cartan kernel is asserted after convergence
of the diagonal tensor in the corresponding kernel-admissible tensor
product.  This is the reduced Hamiltonian central-operation model.

Witten's centrality computation is used here in its local algebraic form.
Write \(Z^{P_0}_{\mathrm{red,Ham}}(A;B)=B\) when
\(B\subset\mathrm{PV}(A)\) is the chosen bracket-admissible Poisson-CE
model.  Under this identification \(J(f)\) is the image of the coordinate
dual to the shifted cotangent generator, and the Hamiltonian ghost gives
the Hamiltonian vector-field operation on the brane.

\begin{thm}[Reduced Hamiltonian central-operation criterion, local \(P_0\) form]
  \label{thm:witten-centrality-local-form}
  Let \(A_\partial\) be a locally constant classical \(P_0\)
  factorization algebra on the brane line in the reduced algebraic model.
  Let \(F\in A_\partial(I)\) be the boundary image of a closed insertion.
  The operation defined by \(F\) lies in the \(P_0\)-central-operation
  complex precisely when its ordered products and Poisson brackets with
  open insertions are connected by homotopies natural for interval
  inclusions and disjoint factorization products.  For a scalar closed
  insertion the degree-zero shadows are
  \[
    F\cdot G\simeq (-1)^{|F||G|}G\cdot F,\qquad
    \{F,G\}_{P_0}\simeq 0.
  \]
  For Hamiltonian insertions the bracket is the moment-map operation
  \[
    \{B_f(a),B_g(b)\}=B_{\{f,g\}}(ab).
  \]
  The morphism to the unreduced derived \(P_0\)-factorization centre,
  including centrality homotopies against arbitrary open observables and
  principal-part cotangent currents, is
  Problem~\ref{prob:formal-factorization-center}.
\end{thm}

\begin{proof}
  In a locally constant factorization algebra on the line, moving a closed
  insertion through an open insertion compares the two ordered
  factorization products.  A chain homotopy between them is the Hochschild
  centrality condition of the associated \(E_1\)-algebra stalk.  The
  \(P_0\) refinement imposes the same compatibility for the classical
  Poisson bracket.

  For the Dirac brane the equal-time bracket is
  \[
    \{(\phi_1)^i{}_j(t),(\phi_2)^k{}_l(s)\}
      =\delta^i_l\delta^k_j\,\delta(t-s).
  \]
  Smearing by \(a(t)b(s)\) turns \(\delta(t-s)\) into \(ab\).  The matrix
  symplectic form contracts the Hamiltonian derivatives to \(\{f,g\}\).
  Hence
  \[
    \{B_f(a),B_g(b)\}=B_{\{f,g\}}(ab).
  \]
  Scalar closed insertions have zero Hamiltonian vector field, so their
  bracket component is null-homotopic.  The unreduced cotangent statement
  requires the missing homotopies for arbitrary open observables and
  principal-part cotangent currents.
\end{proof}
...
$\mathrm{Tr}(1)=N$ on the open side removes the
constant-Hamiltonian central extension. Its extension class is the U(1)
center anomaly: a distinguished nontrivial scalar-extension class
...
\begin{thm}[U(1) center anomaly: closed side]\label{thm:u1-center-anomaly}
  ...
  distinguished anomaly line attached to this constant-Hamiltonian
  extension.
\end{thm}
...
\begin{proof}
  ...
  same lemma shows that the scalar-extension line $\C\cdot[\bar c]$ is
  nonzero; no classification of all of
  $H^2_{\mathrm{Lie}}(\bar A,\C)$ is asserted. Splitting this extension
  is exactly the problem of lifting back to $\mathfrak h_{\mathrm{poly}}$.
\end{proof}
...
\begin{thm}[Quantum-classical equivalence of the U(1) center class]\label{thm:quantum-classical-anomaly}
  ...
  The Rees quantization of the central extension
  $\bar A_{[\bar c]}$ carries the same Lie cocycle
  $[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)$. After applying the
  finite-\(N\) trace character \(K\mapsto N I\) on the central generator,
  its order-\(\hbar\) scalar contact term is the Capelli shift \(\hbar N\).
\end{thm}
...
\begin{proof}
  ...
  For the central-extension comparison, the Rees algebra
  ...
  classical-to-quantum reduction therefore record the same scalar-extension
  class in
  ...
  of the Moyal star commutator on the central direction. Evaluating the
  central generator by the trace character \(K\mapsto NI\) converts that
  order-\(\hbar\) central term into the scalar Capelli contact \(\hbar N\).
\end{proof}

\begin{rmk}
  The class $[\bar c]$ is the scalar central class of the brane algebra:
  the U(1) center of $\lie{gl}_N$ is evaluated by the trace character,
  and the Capelli shift $\hbar N$ is the charge of this central direction.
  Scalar reduction kills the empty-trace scalar value
  \(\operatorname{Tr}(1)=N\). It does not remove the nonempty linear trace
  observables \(\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\); their
  unreduced bracket is the scalar \(N\), which becomes zero only after this
  projection.
\end{rmk}
...
\end{itemize}
The product on the closed side in this paragraph is the topological-current
avatar used for the local trace comparison.  The native holomorphic
operator algebra is the \(\C^2\)-holomorphic factorization algebra
\(\mathcal F^{\mathrm{hol}}_{\mathrm{Ham}}\) of
Definition~\ref{def:local-hamiltonian-chiral-factorization-algebra}; no
one-dimensional vertex OPE or Zhu algebra is asserted here.

\paragraph{Stable large-\(N\) admissibility target.}\label{conj:An-Koszul}
The stable large-\(N\) comparison is not the source of the local
theorem.  It is a later admissibility target, requiring its own
open-trace \(A_\infty\)-Koszul acceptance datum inside an admissible
Tate/stable category.  The local
theorem below supplies the formal Hamiltonian BF/Moyal construction and
the coordinate CE/PV central-operation comparison that such a target may
use after its own conilpotence, endpoint, and inverse-limit hypotheses
have been supplied.  The raw finite-\(N\) operator algebras are not
identified here with the strict formal Hamiltonian envelope.

The Koszul spine is stratified. Layer 0 is the operadic Lie/coComm
Koszul input. Layer 1 is the Tate CE/bar-cobar identity under an
admissible lower-central Tate-pronilpotent envelope. Layer 2 is the Tate Quillen
equivalence. Layer 3 is the coordinate CE/PV reduced \(P_0\)
central-operation theorem. Layer 4 is the PBW/Rees special-fibre shadow.
Layer 5 is the Matlis/Grothendieck cotangent module. Layer 6 is the
brane-line factorization-current comparison. Layer 7 is the stable
large-\(N\) bulk-boundary horizon, conjectural here and not used in the
local Hamiltonian BF/Moyal theorem.
...
background. If an ambient background supplies an admissible protected
Hamiltonian restriction, then the local theorem of
\S\ref{ssec:local-theorem} applies to that restricted sector.
...
\begin{defn}[Working local topological factorization sector]\label{def:local-topological-string-sector}
  ...
  specifying a cyclic open BV theory and the bulk-to-boundary
  central-operation data it carries. In a de Rham model the closed fields carry the
...
\begin{defn}[Working local holomorphic factorization sector]\label{def:local-holomorphic-string-sector}
  ...
  polydisks, the factorization products and single-open structure maps are
...
\begin{rmk}
  The active geometry is the formal local mixed space
  \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_{0,\mathrm{hol}}\).
  BCOV/Kodaira-Spencer theory supplies only the source convention
  \cite{bcov}: Dolbeault polyvectors, cotangent BV notation, and the
  divergence determined by a holomorphic volume form.  In the present
  surface model the holomorphic symplectic form
  \(dz_1\wedge dz_2\) and the volume form \(dz_1dz_2\) are local
  unimodularity data; they give the BV pairing, the divergence operator,
  and cyclic trace densities.  The resulting divergence-free vector
  fields are represented locally by Hamiltonians modulo constants.
\end{rmk}
...
\begin{rmk}
  ...
  are not used as transfer theorems here. The model below is a different formal local
...
\begin{defn}[Working classical local mixed holomorphic-topological sector]\label{def:local-th-string}
  ...
  brane it produces the Ext algebra. A full bulk-to-boundary mixed datum would
...
\begin{rmk}
  ...
  manuscript proves the componentwise quantum coefficient statement:
  weighted BV kernel convergence, the balanced scalar-contact zero
  projection on its habitat, reduced weighted Moyal principal-part
  current brackets, and a degree-zero Hamiltonian trace comparison
  controlled by the radial/Weyl normal-form criterion.  The finite
  normal-form computations close the balanced diagonal through \((6,6)\), all
  non-edge bidegrees of total degree at most \(13\), and the
  total-degree \(14\) cases
  \[
  ...
  statement is the exact obstruction problem
  \[
  ...
  Problem~\ref{prob:quantum-p0-operation-realization}.  The full
  renormalized all-graph realization still also requires the analytic
  graph-realization data of
...
worldsheet example, while BCOV/Kodaira-Spencer supplies only the
target-space source convention for Dolbeault polyvectors, divergence, and
cotangent BV notation. The Hamiltonian BF complex below is the formal
local target-space sector used here. A mixed
...
at \(L\) and taking the Darboux holomorphic normal disk
\((\widehat{\C^2}_0,dz_1\wedge dz_2)\).
The closed sector is the Hamiltonian BF formal-local sector on this mixed
space, in the Kodaira--Spencer source convention rather than in a pure
A-model or pure B-model. The open sector is the
...
Every theorem below is a theorem about this completed
\(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\) model.  Compact or
global comparisons require separate hypotheses and are not part of the
formal-local claim.

\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  The central factorization object in this paper is
  \[
    \mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}
  \]
  the formal Hamiltonian holomorphic factorization algebra on the
  ...
  \]
  The formal stalk of observables is
  \(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\); its
  identification with polyvectors is the coordinate/admissible CE/PV
  theorem below, not an unrestricted strict-product \(P_0\) assertion.
...
Proposition~\ref{prop:skyscraper-ext}; the local unimodular datum is the
residue/cyclic pairing on this two-dimensional Ext
algebra.  In the general source convention of cyclic open field theory,
the open field of an object $\mc F$ is $\RHom(\mc F,\mc F)[1]$, and the
formal Chern-Simons action
...
\cite{witten-cs} and of the AKSZ packaging \cite{aksz}.  The local proof
below is the finite Dirac/Koszul matrix calculation.  For the local
model at hand,
...
The Dirac variables used in the body are
\(\phi_1=-\chi_1\), \(\phi_2=\chi_2\), and
\(A=-A_{\mathrm{CS}}\).  In those variables the ghost-zero action is
the first-order gauged matrix model of \eqref{eq:open-action-derived},
where $\phi_1,\phi_2$ are adjoint scalar normal coordinates and $A$
imposes the moment-map equation $[\phi_1,\phi_2]=0$.
...
The CE/PV cotangent block below uses its own coordinate grading: the
odd BV cotangent generator $\lambda[1]$ has BV degree $-1$, while the
CE coordinate $u_\lambda$ dual to it is assigned PV function degree
$0$. Equivalently, the symbol ``dual'' in that block means the
...
$Q\psi=[\phi_1,\phi_2]$ is the derived Koszul zero-fibre complex for the
moment-map equation.  For \(N\ge2\) the commuting-pair equations are not
being asserted to form a regular sequence.

Open strings on a stack produce matrix algebras; closed modes appear in
the primitive stable part. In this local computation,
Procesi--Razmyslov invariant theory controls the stable trace invariant
coordinate ring.  The Loday--Quillen--Tsygan principle
\cites{loday-quillen,tsygan}; see also
\cite{loday-vallette}*{Ch.~13} for the operadic packaging, supplies the
cyclic-homology template for primitive large-\(N\) classes, not the
coordinate-ring invariant theorem itself. The moment-map equation
$[\phi_1,\phi_2]=0$ is the first-class constraint whose Koszul derived
zero-fibre complex is exactly the BV reduction
$Q\psi=[\phi_1,\phi_2]$.  Theorem~\ref{thm:main-local} verifies the
corresponding connected-trace pattern directly on the Hamiltonian trace
sector.
...
\begin{rmk}[Same-rank deletion is not the LQT projection]
...
differential. No global $HC_{*-1}$ convention is transferred into
...
Thus $u_{k,l}$ is a closed-sector CE source coordinate dual to the shifted cotangent
...
This realizes $\mathfrak h^\vee_{\mathrm{cont}}$ with the
principal-part coadjoint action of
...
$\mc P=\bigoplus\C\rho_{a,b}$.  The available passage from the PBW
bidegree label carried by \(\Psi_{k,l}\) to the principal-part label
\(\rho_{k,l}\) is the Fourier-polarized Rees
$D_\hbar$-module label bridge of
Theorem~\ref{thm:app-matlis-rees-fourier-bridge}; it is not a
same-action $\mathfrak h$-module realization and it is not the unreduced
quantum boundary observable.
...
The finite identity is algebraic: CE cochains on
\(T^*[1]\mathfrak l\) are Schouten polyvectors on
\(\mathbf S(\mathfrak l)\), with generator map \(c^I\mapsto\theta^I\)
and \(u_I\mapsto O_I\).  The boundary Hamiltonian is the image of the
shifted-cotangent CE coordinate \(u\), not the Hamiltonian CE coordinate
\(c\).  Completion at the formal disk preserves this formula
coefficientwise by finite-coordinate tests; bar-cobar/Koszul duality is
the separate admissible enhancement governed by conilpotence and
filtered convergence.  The Schouten/cotangent bracket carrying the
comparison has degree \(-1\); the reduced Hamiltonian boundary bracket
introduced below is the degree-zero Poisson bracket.
...
Theorem~\ref{thm:main-local} uses the affine $\Ext$ calculation
...
Theorem~\ref{thm:bulk-boundary}); the admissible
bar-cobar/enveloping computation;
	the coordinate-free CE/PV source-to-boundary synthesis of
Theorem~\ref{thm:coordinate-free-closed-open-synthesis}; the
componentwise quantum coefficient comparison of
Theorem~\ref{thm:componentwise-quantum-coefficient-comparison}; and the first bracket
calculation \eqref{eq:bracket-coefficient}.

The formal Weyl/Moyal calculation is the all-order algebraic
deformation-quantisation target for the constant Poisson formal disk. Its
monomial coefficients are checked by exact arithmetic in the stated
finite ranges and by the edge-strip argument in
Proposition~\ref{prop:app-edge-pbw-telescoping}.  A closed-string
Feynman-integral derivation of the same coefficients requires its own
propagator, counterterm, and QME data.

\subsection{The local theorem}\label{ssec:local-theorem}

The ghost-zero brane theory is the invariant algebra of commuting matrix
pairs with the first Koszul descendants of the moment equation.  The
closed theory is the Hamiltonian cotangent CE/PV complex with continuous
dual realized by Matlis principal parts.  The boundary map evaluates
Hamiltonians on the matrix-valued normal coordinates of the brane.  The
one-\(\psi\) trace classes give PBW special-fibre labels; principal parts
give the deformation-level cotangent module.

The native objects are
\(\mathcal F_{\mathrm{Ham}}^{\mathrm{hol}}\), its mixed
topological-current enhancement over \(\R^2_{\mathrm{top}}\), the
Weyl/Moyal brane algebra, the defect principal-part current objects, and
the Bochner--Martinelli--Koppelman current data of
Proposition~\ref{prop:finite-window-bm-native-e2-transfer}.  Curve
vertex, Zhu, and strict all-arity BM statements require the reduction or
current-transfer hypotheses stated in their own theorems.

The reduced statement is the coordinate CE/PV theorem and admissible Koszul
criterion of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}, the brane-line
factorization-current lift, the Matlis principal-part cotangent factor
after de Rham-current contraction, the scalar-reduced degree-zero
Moyal/Weyl deformation, the componentwise quantum coefficient comparison,
and the U(1) scalar anomaly.  The full formal disk is a completed
coordinate cochain theorem.  The \(P_0\) form uses a topologized
bracket-admissible realization \(B\) containing the relevant \(O_f\)'s
and \(d_\pi^{\mathrm{coord}}O_f\)'s.  The kernel form uses a
kernel-admissible convolution dg Lie algebra in which the diagonal
converges.  The bar-cobar/enveloping upgrade is an admissibility
criterion:
admissible Tate, nilpotent, relative, or weighted replacement data
promote the coordinate theorem to Koszul duality, and the residual terms
are the obstruction data listed in the criterion.
...
The proof uses the formal local model alone.  Its portable content is
not confined to one coefficient envelope: the cochain CE/PV
identification is formal-disk level, while the enveloping/Koszul upgrade
is the criterion of
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.  The unreduced
Tate-coefficient cotangent lift is isolated in
\ref{prob:formal-factorization-center} and separated into its precise
chain-level, nilpotent, weighted, associated-graded, inverse-limit, and
QME-obstruction components. The stronger analytic realization requires a
Costello renormalized BV graph construction on an elliptic parent with
brane-defect kernel and a vanishing mixed brane-defect QME obstruction
class.

The protected twisted-M-theory origin is not used as a proof of the
local theorem. Once an ambient background supplies the admissible
protected Hamiltonian restriction, the local model applies to that
protected sector and no further.
...
with $M$ the topological factor, $Y$ the holomorphic factor, all
Hamiltonian calculations performed in the formal neighbourhood of $p=0\in Y$.
...
\begin{prop}[Local mixed model]\label{prop:local-model-mixed-definition}
  The formal local Hamiltonian data define a closed mixed
  holomorphic-topological BV sector and a compatible cyclic open brane
  BV theory; the constructed bulk-to-brane operation is the degree-zero
  Hamiltonian boundary evaluation. The full Poisson
  factorization-center morphism is the residual
  Problem~\ref{prob:formal-factorization-center}.
\end{prop}
...
\begin{proof}
  ...
  $dz_1\wedge dz_2$. Write
  \[
    F_\alpha=D\alpha+\frac12\{\alpha,\alpha\},\qquad
    D_\alpha=D+\{\alpha,-\}.
  \]
  Since \(D\) is a derivation of the bracket and the bracket satisfies
  Jacobi, \(D_\alpha F_\alpha=0\). With compact support in the local
	  calculation and with the coadjoint-invariant evaluation pairing, the BV
	  coadjoint transpose is fixed by
	  \[
	    \int_{D\times B}\langle D_\alpha^\vee\beta,\xi\rangle\,dz_1dz_2
	    =
	    -(-1)^{|\beta|}
	    \int_{D\times B}\langle\beta,D_\alpha\xi\rangle\,dz_1dz_2 .
	  \]
	  Variation of \(S=\int\langle\beta,F_\alpha\rangle dz_1dz_2\) gives
	  \[
	    \frac12\{S,S\}_{\mathrm{BV}}
	      =\int_{D\times B}
	        \left\langle D_\alpha^\vee\beta,F_\alpha\right\rangle dz_1dz_2
	      =-(-1)^{|\beta|}\int_{D\times B}
	        \left\langle\beta,D_\alpha F_\alpha\right\rangle dz_1dz_2
	      =0.
	  \]
  Thus the total BF action satisfies the classical master equation without
  ...
  Poisson factorization-center morphism is Problem~\ref{prob:formal-factorization-center},
  outside the proved local theorem.
\end{proof}
...
\begin{constr}[The open field from the Ext basis]\label{constr:open-field-basis}
  ...
  $$A_{\mathrm{CS}}\in\Omega^1(\R)\otimes\lie{gl}_N,\qquad
    \chi_1,\chi_2\in\Omega^0(\R)\otimes\lie{gl}_N,$$
  and in coefficient coordinates they appear as
  $$\alpha_0^{\mathrm{coef}}
    =A_{\mathrm{CS}}+\chi_1\varepsilon_1+\chi_2\varepsilon_2.$$
  The Dirac variables used in the rest of the manuscript are the
  linearly normalized variables
  $$\phi_1=-\chi_1,\qquad \phi_2=\chi_2,\qquad A=-A_{\mathrm{CS}}.$$
  This normalization is chosen once and for all so that the symplectic
  potential is \(\operatorname{Tr}(\phi_1d\phi_2)\) and the multiplier
  term is \(A[\phi_1,\phi_2]\).
...
The ghost-zero coefficient truncation
\(\alpha_0^{\mathrm{coef}}\) of
Construction~\ref{constr:open-field-basis}, written after the
Dirac-normalization
\((A_{\mathrm{CS}},\chi_1,\chi_2)\mapsto(A,\phi_1,\phi_2)\), is the
physical sector used below. Reducing the Chern-Simons action to it
produces the first-order matrix mechanics governing the brane:
...
\begin{rmk}
  The same local open model is obtained from the affine \(\C^2\)
  point-brane residue calculation by tensoring the skyscraper object with
  $\C^N$ and passing to endomorphisms. This is the local
  two-dimensional residue pairing fixed by \(dz_1dz_2\).
\end{rmk}
...
An infinitesimal canonical transformation of the holomorphic-symplectic
disk is a Hamiltonian function modulo constants; constants act trivially
because their Hamiltonian vector fields vanish. The BF partner is the
cotangent variable that promotes this Lie algebra of canonical
transformations to a local BV field theory, and the same symplectic
normal plane governs both the closed BF sector and the open first-order
matrix mechanics.

The closed sector is a mixed field theory: a topological de Rham factor
along $\R^2$ and a holomorphic Dolbeault factor along $\C^2$, not a
tensor product of two independent closed-string state spaces:
...
Poisson bracket on the Hamiltonian entries with the tensor-product Koszul
sign,
...
For an analytic model adequate to propagators, circle-regularize one real
topological direction to obtain $\R\times S^1\times Y_{\mathrm{hol}}$ with
$Y_{\mathrm{hol}}$ holomorphic symplectic, completed at the trivial
solution. The
...
data of \ref{prob:analytic-graph-realization} fixed in this compact or
...
\subsection{Reduced Hamiltonian brane coupling}
...
degree-zero Hamiltonian component of the reduced CE/PV central-operation map;
the full Poisson factorization-center morphism is left to
...
Costello--Gwilliam central-operations theorem is used in the reduced
Hamiltonian proof.
...
\begin{ex}[Low-degree monomial verification]\label{ex:low-degree-phi}
  We verify \(\Phi_{\mathrm{coord}}\) on small monomials of the formal
  symplectic disk.
...
Problem~\ref{prob:formal-factorization-center} splits into proved
mechanisms and one residual obstruction. The Costello--Li 2012 BCOV/HCS
theorem fails as a source: it does not transfer to the present
brane-defect geometry, and that failure forces the replacement
mechanisms. Weighted Tate completion closes Casimir convergence and the
finite-scale heat-kernel propagator extension. Nilpotent truncation
closes a conformal-symplectic-quotient sector classically and exhibits
the all-order Moyal obstruction. Chain-level primitive projection
closes the cosheaf and factorization compatibilities of the
indecomposable shadow without analytic input. The weighted
Hadamard/Mittag-Leffler theorem reduces the remaining Costello-locality
compatibility of Problem~\ref{prob:weighted-rg-locality} to the mixed
brane-defect QME obstruction class. Hadamard control is not anomaly
cancellation: it supplies the coefficient-kernel and locality regulator,
while the QME branch still requires the scalar and non-scalar
obstruction classes to vanish in their own local-functional complexes.
...
\begin{rmk}[Reduced statement and residual obstruction complex]
  Theorem~\ref{thm:main-local} consists exactly of the proved Hamiltonian,
  factorization, principal-part, and anomaly statements stated above.
  ...
  extension of the degree-zero Moyal/Weyl theorem requires vanishing of the
...
\begin{rmk}[Stratified Weiss/Cech--Roos descent obstruction]\label{rmk:weiss-ran-descent-obstruction}
  A global factorization-center morphism is not obtained by choosing Darboux
  coordinates at each point and gluing the resulting stalk maps.  In the
  internal stratified construction the descent defect is the Cech cone
  \[
    ...
    \right)
  \]
  for collared stratified Weiss covers \(\mathcal U\) of \(V\), together
  with the Roos compatibility class
  \[
    \mathfrak w^{\check C/R}_{V,\Omega,N,M}
  \]
  under refinements, finite windows, interval inclusions, and admissible
  weight transports.  Thus the internal descent entry is
  \[
    ...
    \right).
  \]
  The displayed descent entry is the datum used by the local
  factorization-center morphism.
\end{rmk}

\subsection{Open-brane operators and the large $N$ limit}

The reduced BV model carries fields $\phi_1,\phi_2$ and one antifield
$\psi=A^*$ for the Lagrange multiplier $A$, with the Koszul differential
$Q\psi=[\phi_1,\phi_2]$ enforcing the moment-map equation. Local
operators are degree-zero rational \(GL_N\)-invariant functions in
characteristic zero: this is the algebraic invariant theory feeding the
trace theorem, not the unreduced Lie-CE BRST cohomology of
\(\lie{gl}_N\), whose scalar-stabilizer ghost classes from the trivially
acting scalar gauge direction sit outside the Hamiltonian trace sector.
The trace algebra therefore starts as cyclic words in two matrices and
collapses in $Q$-cohomology onto commutative monomials on the formal
symplectic plane.
...
The open side has produced primitive ghost-zero monomials
$\overline{\mathrm{Tr}(\phi_1^k\phi_2^l)}$ and primitive one-$\psi$
descendants $\Psi_{k,l}$. The closed side must match these as a Lie
algebra of local modes whose Chevalley--Eilenberg cochains carry the
residual BRST differential.
...
\subsection{Stratified CE/PV and admissible Koszul duality}

Closed local operators are continuous Chevalley--Eilenberg cochains of
$\mathfrak g$; the brane Hamiltonian sector is a free
graded-commutative algebra on $\bar A$ and $\bar A_{\mathrm{desc}}[1]$.
Comparing them as $E_1$-algebras requires the operation that exchanges
commutative cochains and associative enveloping algebras: Koszul
duality, here in its admissible form.

Three layers separate. The cochain layer is the completed cotangent
CE/PV identity of Theorem~\ref{thm:universal-ce-pv-koszul-criterion},
valid for the formal disk on finite coordinate tests; a
kernel-admissible Maurer--Cartan tensor appears only after the diagonal
converges. The enveloping layer is a bar-cobar criterion that applies
exactly when the chosen Tate or weighted coefficient category makes the
CE coalgebra conilpotent and the filtered inverse limits exact. The
strict product/direct-sum full-disk coefficient pair fails this
criterion: its obstruction tuple, whose vanishing or replacement is the
input required for the enveloping upgrade, is preserved as load-bearing
data, not discarded.
