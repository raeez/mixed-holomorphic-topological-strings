...
\end{titlingpage}
  ...
  The B-model topological string on a Calabi--Yau threefold is governed
  by Bershadsky--Cecotti--Ooguri--Vafa Kodaira--Spencer gravity
  \cite{bcov}, given a renormalized BV formulation by Costello and
  Costello--Li
  \cite{costello-renormalization,costello-li-quantum-bcov,costello-li-open-closed-bcov}.
  Localizing near a smooth holomorphic-symplectic point with transverse
  normal directions \((t,s,z_1,z_2)\) reduces the string field theory
  to its formal-local mixed holomorphic-topological part on
  \[
    X=\R_t\times\R_s\times\C^2_{z_1,z_2},
  \]
  topological in \((t,s)\) and holomorphic in \((z_1,z_2)\).  Closed
  strings live as BV Hamiltonian gravity for the formal symplectic
  polydisk \((\widehat{\C^2}_0,dz_1\wedge dz_2)\), the holomorphic
  polydisk analogue of the Kodaira--Spencer master action.  Open
  strings are matrix-model branes along the topological line: a stack
  of \(N\) Dirac branes carrying a Chan--Paton representation of the
  polynomial Hamiltonian algebra \(A=\C[[z_1,z_2]]\).
  ...
  \(\theta_f\) on functions on the moduli; the
  shifted-cotangent insertion \(u_f\) is measured on the brane as
  \(O_f=J(f)\); the brane functional \(J\) carries the cotangent
  coordinate to the boundary observable, while \(c_f\) acts on the
  observable algebra as a Schouten derivation.
  ...
  \(\mathcal T\rightsquigarrow\mathcal T'\).  The named curvature classes are: the radial cokernel
  ...
  \(\operatorname{Ob}^{\Pi}_{\mathrm{BM}}\), nonzero in every finite
  Matlis window; the Hamiltonian factorization current acts coherently
  on the pro-Matlis tower
