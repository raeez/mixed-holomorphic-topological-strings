...
\begin{thm}[Universal local mixed-HT Deligne theorem at $N$ Dirac branes]\label{thm:main-local}
  ...
  \begin{equation}\label{eq:thm-A-equivalence}
    ...
    HH^\bullet_{\mathrm{adm}}\!\bigl(
      ...
      A^{\mathrm{cl}}_{\partial,N}\bigr),
  \end{equation}
  determined on Chevalley--Eilenberg generators by
  \begin{equation}\label{eq:ce-pv-dictionary-strong}
    c_f\;\longmapsto\;\theta_f\in HH^1_{\mathrm{adm}},\qquad
    u_f\;\longmapsto\;J(f)\cdot(-)\in HH^0_{\mathrm{adm}},\qquad
    K\;\longmapsto\;\operatorname{Tr}\mathbf 1=N,
  \end{equation}
  with trace map $J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)$ and
  Schouten derivation
