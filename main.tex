...
through which the Hamiltonian \(f\in\mathfrak h\) is read on the
brane as the trace functional
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\).
...
The formal-stalk chart sees only the local behaviour at the brane,
...
\begin{lem}[Two CE coordinates of a Hamiltonian]\label{lem:type-discipline}
  ...
  \begin{itemize}
    ...
    \item As closed cotangent insertion:
      ...
      \(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\) read on the chart \(A_b\)
...
\((\phi_1,\phi_2)=(z_1,z_2)\) and \(\overline{\operatorname{Tr}}=
\mathrm{ev}_p\). The trace
\(J(z_1^k z_2^l)=z_1^k z_2^l\) is the evaluation map; the closed
generator dictionary is \(c_f\mapsto\theta_f\),
\(u_f\mapsto J(f)\). The Capelli scalar \(\hbar N[\bar c]\) vanishes
identically: \([\bar c]=0\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\)
on the cyclic-collapsed quotient at \(N=1\), and the
order-\(\hbar\) Heisenberg commutator
\([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar\) (cf.\
Example~\ref{ex:linear-hamiltonian-com}) carries no nontrivial
extension class for \(N=1\). The chiral Hochschild target is the
free polynomial reading
\(C^\bullet_{\mathrm{ch}}(\C[[z_1,z_2]],\C[[z_1,z_2]])
\simeq \mathrm{PV}(\C[[z_1,z_2]])\)
of \cite{beilinson-drinfeld-chiral}*{\S\S 3, 19} on the formal
disk. Proved (the local theorem
of~\S\ref{ssec:local-theorem} at \(N=1\)).

\subsubsection{Affine Kac--Moody current
\texorpdfstring{\(\widehat{\mathfrak g}_k\)}{ghat\_k}}\label{sssec:ex-affine-km}

\(\mathfrak g_0\) finite-dimensional simple complex; \(b=\)
the level-\(k\) vacuum module \(V_k(\mathfrak g_0)\) of
...
charge. The chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(V_k(\mathfrak g_0),V_k(\mathfrak g_0))
\simeq Z^{\mathrm{der}}_{\mathrm{ch}}(V_k(\mathfrak g_0))\)
along the chiral Deligne ladder
\cite{beilinson-drinfeld-chiral}. 
Identification of the formal-Darboux stalk is proved on the
Heisenberg core (\S\ref{ssec:local-theorem}); the Sugawara reading
to the full \(\widehat{\mathfrak g}_k\) chart is the standard
...
\(N=1\), \(b=\) the \(\beta\gamma\) vacuum on the formal disk; the
Chan--Paton pair \((\phi_1,\phi_2)\) realizes the canonical
commutation \([\beta(z),\gamma(w)]=(z-w)^{-1}\) read at the brane
...
\(c=2\) reads as \(\hbar N[\bar c]\) with \(N=1\) and the doubled
Heisenberg cocycle. The chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(V^{\beta\gamma},V^{\beta\gamma})\), the
chiral derived centre of the free \(\beta\gamma\) factorization
algebra of
\cite{costello-gwilliam}*{Vol.~II~\S 5.4}. 
Proved at \(c=2\); the chart matches the formal-Darboux stalk by
the same Heisenberg cocycle that controls the brane
centre-of-mass (Example~\ref{ex:linear-hamiltonian-com}).
...
\(c_{W_N}=N-1+(2N(N+1)-N)/(k+N)\) of
\cite{fateev-zamolodchikov-conformal} as the chart-multiplicity
...
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\) reads on
the cyclic framed stable branch
\(\C[\phi_1,\phi_2]\cdot v=\C^N\) the formal-Darboux stalk chart
of \(\Hilb^N(\C^2)\) in its Nekrasov--Schwarz noncommutative
Moyal deformation \cite{nekrasov-instanton-counting,
costello-twistedM}, the brane reading of the Nekrasov vertex.
