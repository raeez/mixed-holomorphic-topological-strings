...
\end{titlingpage}
  ...
  local model, and the present paper computes it.  The reduction is
  the holomorphic-topological BF theory on
  ...
  on \(\widehat{\C^2}_0\), \(\mathfrak h=\C[[z_1,z_2]]/\C\); and the
  universal brane probe is a single trace functional, the matrix
  microscope.

  \medskip\noindent
  \emph{The classical BV theory.}  The BV fields are
  \[
  ...
  \]
  with $(-1)$-shifted symplectic pairing supplied by integration of
  mixed-HT forms against the holomorphic volume \(dz_1\,dz_2\) and
  the residue pairing
  \(\langle f,\omega\rangle=\Res_0\,f\omega\) on \(\mathfrak g\).
  The classical action is the BF action
  \[
    S=\int_X\langle\mathcal B,
      (d+\bar\partial)\mathcal A
      +\tfrac12[\mathcal A,\mathcal A]_{\mathfrak g}\rangle,
    \qquad
  ...
  generalized Laplacian \(D=[Q,Q^{\mathrm{GF}}]\) and the heat-kernel
  propagator
  \(P_{\varepsilon,L}=\int_\varepsilon^L(Q^{\mathrm{GF}}\!\otimes\!1)
    K_t\,dt\); the Costello effective interaction at scale \(L\) is
  \(I[L]=W(P_{\varepsilon,L},I)\), satisfying the QME
  \[
    Q I[L]+\tfrac12\{I[L],I[L]\}_L+\hbar\Delta_L\,I[L]=0.
  \]
  This is the renormalized BV theory whose deformations carry the
  obstruction calculus stated below.
  ...
  Heisenberg current.  Inverse limits over Matlis windows
  ...
  \(\int_{\R_t}\operatorname{Tr}(\phi_1\dot\phi_2
    +A[\phi_1,\phi_2])\) and Koszul antifield
  \(\psi\in\mathfrak{gl}_N[1]\),
  \(Q\psi=[\phi_1,\phi_2]\).  The conjugation moment-map ideal is
  realized as the derived zero fibre; the matrix entries of
  \([\phi_1,\phi_2]\) form a non-regular sequence for \(N\ge 2\), so
  the brane is the derived commuting scheme, the framed Hilbert
  scheme of \(N\) points on \(\C^2\) in its Beilinson-quiver
  presentation.  The Moyal/Weyl deformation of the brane Poisson
  algebra is the noncommutative Hilbert scheme of
  Nekrasov--Schwarz.
  ...
  is the unique \(GL\)-equivariant Tate-cyclic-trace functional
  \(\mathfrak h\to\operatorname{HC}_0(\End\C^N)^{GL_N}\) that is
  Darboux-natural under the formal symplectomorphism group,
  block-additive under brane stacking, and linear-normalized
  \(J(z_i)=\operatorname{Tr}\phi_i\); the forcing is
  ...
  on a small interval of the worldline
  (Definition~\ref{def:E1-brane-algebra}).  The closed-to-open map
  \[
  ...
  \medskip\noindent
  \emph{Capelli scalar anomaly.}  Closed Hamiltonian reduction kills
  \(\operatorname{Tr}\,1=N\); the brane records it as the trace-$U(1)$
  centre-of-mass cocycle of \(N\) coincident D-branes on
  noncommutative \(\C^2_\hbar\).  The same class
  \(\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]\) is
  detected as a Lie 2-cocycle, the unreduced bracket
  ...
  (Theorem~\ref{thm:obstruction-twist-dichotomy}).  The local model
  is formal: the minimal model of the filtered \(L_\infty\) algebra
  $\mathfrak K_{\mathrm{loc}}$ controlling deformations of $\Phi_N$
  has exactly four nonzero higher brackets, the named curvature
  classes
  \begin{itemize}
    \item the radial Weyl cokernel
      $\Omega^{\mathrm{rad}}_{a,b}\in\operatorname{coker}B_{a,b}$
      controlling all-bidegree Weyl/trace compatibility
    ...
    \item the reduced Costello QME class
      ...
      in local-functional cohomology, with cubic finite window
      $\theta_3$ exhibiting
    ...
    \item the Bochner--Martinelli class
      $\operatorname{Ob}^\Pi_{\mathrm{BM}}\in
       \varprojlim^1_N\operatorname{HH}^\bullet(\mathcal P_{\le N})$,
      nonzero in every finite Matlis window; the Hamiltonian
      factorization current acts coherently on the derived pro-Matlis
      tower
  ...
  of Beilinson--Drinfeld with $\mathcal P$ in the role of $\Omega_K$
  and $\mathfrak h$ in the role of regular functions; the
  formal-symplectic local fixture of the Costello--Yagi twisted
  M-theory family \cite{costello-twistedM}; and the universal local
  probe of $\Hilb^N(\C^2)$ in its noncommutative Moyal deformation.
  It is the local model in which the BCOV holomorphic anomaly
  equations and the Gopakumar--Vafa / Donaldson--Thomas
  correspondences become tractable in a future global theorem.
...
\]
(Theorem~\ref{thm:reduced-ce-pv-central-operation}).  Continuous duality on the formal-disk topology
...
gerbe-valued coherence; non-absorptive curvature forces an
enlargement of the target along a substitution
