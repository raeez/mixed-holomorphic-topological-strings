...
Target \(X=K3\times E\); the brane chart at any formal point
\((p,q)\in K3\times E\) sees the formal-Darboux stalk in the
\(K3\)-direction (Theorem \S\ref{ssec:local-theorem}), with the
\(E\)-direction supplying the elliptic-fibre data of the
modular line. Five distinct construction layers attach five
\(\kappa\)-numbers to this target, none of which collapses to
another:
...
\(\kappa_{\mathrm{fiber}}=24\). The Igusa cusp form
\(\Phi_{10}\in S_{10}(\mathrm{Sp}(2,\Z))\) of
\cite{igusa-cusp-form} is the level-2 modular section of the
modular line bundle on \(\overline{\mathcal A}_2\), and
\(\Delta_5=\Phi_{10}^{1/2}\) is the Borcherds product on the
paramodular cover \cite{gritsenko-nikulin-paramodular}. The
spectral-sequence collapse of these five layers through the
Hall--Drinfeld--Borcherds spectral sequence is conjectural;
cross-volume bridge to \(\sim/\mathrm{calabi\text{-}yau\text{-}quantum\text{-}groups}\)
(Vol III; CY-to-chiral two-stage \(\Phi_d=
\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}\circ\Phi^{\mathrm{FA}}_d\))
and \(\sim/\mathrm{igusa\text{-}cusp\text{-}form}\) (Borcherds lift
\(\Phi_{10}\), denominator
\(\Delta_5\)). Pointwise formal-Darboux stalk
proved (\S\ref{ssec:local-theorem}); the five-layer
\(\kappa\)-stratification is named, not collapsed; the modular
section \(\Delta_5\) is the Borcherds--Gritsenko--Nikulin
datum; spectral-sequence collapse and operator-level lift are
open.
