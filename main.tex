...
\end{titlingpage}
  ...
  The native holomorphic OPE is the formal diagonal expansion computed
  in Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope}.  In
  coordinates \(\zeta_i=z_i-w_i\) the singular kernel is
  \(K^{(2)}_\Delta=d\zeta_1d\zeta_2/(\zeta_1\zeta_2)\), and
  \[
    \mathsf{J}_x(z)\mathsf{J}_y(w)
    \sim
    K^{(2)}_\Delta(z,w)\mathsf{J}_{[x,y]_{\mathfrak g}}(w).
  \]
  Thus the two-dimensional OPE is computed before any curve
  restriction; the curve vertex algebra is the one-variable
  specialization after choosing the restriction datum.
