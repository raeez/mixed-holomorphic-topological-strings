...
\begin{exa}[A single Matlis residue]\label{exa:single-matlis-residue}
  ...
  \]
  pairs with \(f\in\mathfrak h\) by
  \(\langle\rho_{a,b},f\rangle=\operatorname{Res}_0(\rho_{a,b}f)\) and
  detects the Taylor coefficient of \(z_1^az_2^b\) in \(f\).  Linear
  Hamiltonians act on \(\mathcal P\) via the residue-coadjoint formula
  \(z_1^pz_2^q\cdot\rho_{a,b}=-(pb-qa+p-q)\rho_{a-p+1,b-q+1}\), never
  locally nilpotently.  The polynomial PBW class
  \(\widetilde\Psi_{a,b}\) carries the same label and a strictly
