...
\addcontentsline{toc}{subsection}{Notation for the local theorem}
...
holomorphic-symplectic brane point, a closed sector that the Mixed
Holomorphic--Topological Deligne problem identifies with the chiral
Hochschild cohomology of the boundary algebra; this manuscript
computes the formal-Darboux coordinate expression of the trace sector
of that identification at \(N\) Dirac branes.  The B-model
topological string has a Costello holomorphic--topological
local sector on
...
\begin{rmk}[Quantum extension]
  ...
  classical cyclic boundary equations close; the exact computation on
  these windows produces neither a decorated target solution nor a
  signed obstruction row.
...
\paragraph{The factors of the local geometry.}
...
\begin{prop}[Bochner--Martinelli current data and finite-window obstruction for $E_2$ transfer]\label{prop:finite-window-bm-native-e2-transfer}
  ...
  every nontrivial finite Matlis window therefore carries the nonzero
  Bochner--Martinelli class \(\operatorname{ob}_{\mathfrak h,N}\).  A one-pair derived analytic quotient
  does give a
...
product carries the Casimir obstruction of
...
The chart of \S\ref{ssec:formal-darboux-stalk-one-calculation}, the
locality structure of Proposition~\ref{prop:three-localities}, and the
closed--open factorization triple assemble into one statement: the
trace-sector boundary value of the closed Hamiltonian BF observables
at \(N\) Dirac branes, in the formal Darboux coordinates of the chart.
...
The Hamiltonian BF action determines the classical local operators at
a formal insertion point: the continuous Chevalley--Eilenberg cochains
of \(\mathfrak g\).
...
\[
  ...
  =\hbar N\mathbf 1,
\]
the Capelli projective anomaly.
...
The trace map \(J\) is now characterised: its degree-zero
specialisation is the moment map, and six Darboux-natural conditions
...
\subsection{First-order Poisson identification}
...
\subsection{Quantization and the brane trace map}\label{sssec:quantization-trace-map}
...
\subsection{Formal Weyl/Moyal algebra, all orders}
...
\subsection{Third-order Costello graph class}
...
$\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$
(Remark~\ref{rmk:consistency-probes}(2)).  A determinant-line
...
\]
the brane centre-of-mass charge.  The closed side quotients the
constant line; the open side retains it with value \(N\).  The
asymmetry is a single Lie
cohomology class \([\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)\),
...
The cocycle has three realisations: the Lie 2-cocycle on
\(\bar A\), the Poisson bracket of trace coordinates, and the projective
Moyal--Weyl commutator
\[
  [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_{\mathrm{proj}}
    =\hbar N\cdot\mathbf 1 ;
\]
Theorem~\ref{thm:app-scalar-anomaly-transgression} identifies the four
equivalent representatives (the abstract
central extension and the explicit Lie central extension reduce
to one realisation on \(\bar A\)).
...
\begin{proof}
  ...
  $\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}
  =\operatorname{Tr}(I\cdot I)=N$: the closed cocycle and the open
  trace bracket represent the same class.
\end{proof}
...
\begin{rmk}[One class, three realisations]\label{rmk:three-faces-one-class}
...
% MOVED: closed-sector operator algebra (prop:grav-ops) -> section 2; strict-product remark -> obstruction calculus
...
\input{section-obstruction-calculus}

\input{tate-T4-bv-vanishing}

\input{tate-P1-hadamard-mittag-leffler}

\subsection{Graph formalism and specialization datum}

The Moyal coefficients \(\hbar P\), \(\hbar^3 P^3/24\), \(\hbar^5 P^5/1920\),
\(\hbar^7 P^7/322560\) of the all-order Weyl/Moyal expansion
(Theorem~\ref{thm:phi-hbar-all-order}) form the algebraic
...
on the Chan--Paton pair; Theorem~\ref{thm:reduced-ce-pv-central-operation}
separates the two columns.  The full
stratified comparison is \S\ref{ssec:stratified-koszul}.
...
(Lemma~\ref{lem:tate-casimir-obstruction},
Theorem~\ref{thm:strict-unweighted-obstruction}); the strict-unweighted
endpoint carries this obstruction.  Theorem~\ref{thm:bmk-finite-window-obstruction}
...
\input{section-pro-matlis-envelope}

\input{tate-P1b-bmk-promatlis-transfer}
...
The Hamiltonian coadjoint action moves classes out of the finite
Matlis window \(\mathcal P_{\le N}\); this is the finite-rank Matlis
duality obstruction on the
...
problem recorded by each component is the choice of pro-Matlis-coherent
...
\subsection{The single brane: \texorpdfstring{$N=1$}{N=1}}\label{sssec:ex-heisenberg}
...
\subsection{Rank two: the first finite trace relation}\label{sssec:ex-N2-finite-window}
...
\subsection{The brane stack: \texorpdfstring{$N\ge 2$}{N >= 2}}\label{sssec:ex-N-brane}
...
% STRUCTURAL-STUB: platonic chapter 10 — the W_infinity[lambda]/E_infinity
% admissible endpoint. A conditional theorem under four named hypotheses
% (Prochazka triangular truncation; Creutzig–Kanade–Linshaw parafermion
% compatibility; Pope–Romans–Shen/Bakas; Yamada weight-window), per the
% seventeen-site catalogue, site 14. A previous unconditional draft was
% deleted (commit 3ed5e29) as comparison-problem disclaimers in disguise;
% the chapter enters only as genuine conditional mathematics sourced from
% the four primary references. Until then the endpoint identification is
% named in the frontier (sec:compact-comparison-obstructions).
...
restriction.  The global mixed-HT identification at \(d=2\),
controlled by \(\mathfrak{Ob}_{\mathrm{Del}}^{\mathrm{glob}}\), the
operator-level lift of the modular line on \(K3\times E\)
(\S\ref{ssec:open-modular-lift}), and the chiral \(E_d\) Deligne
theorem at \(d\geq2\) (\S\ref{ssec:open-chiral-Ed-Deligne}) are the
open problems the formal-Darboux stalk has made well-posed.
