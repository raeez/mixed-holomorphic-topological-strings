...
\cite{frenkel-ben-zvi}*{\S 15.6}. The trace
\(J(f)=\overline{\operatorname{Tr}\rho_\phi(f)}\) on the
Drinfeld--Sokolov chart reads the formal-Darboux stalk through the
\(W_N\)-currents of conformal weights
\((2,3,\dots,N)\). The chiral Hochschild target is
\(C^\bullet_{\mathrm{ch}}(W_N,W_N)\). The conditional admissible
endpoint
\(W_\infty[\lambda]\Rightarrow E_\infty\) of the four-hypothesis
ladder (Prochazka triangular truncation,
Creutzig--Kanade--Linshaw parafermion compatibility,
Pope--Romans--Shen/Bakas, Yamada weight-window;
\S\ref{ssec:w-infinity-endpoint}) governs the
\(N\to\infty\) limit. The \(W_N\)-chart
identification is proved at the Heisenberg core
(\S\ref{ssec:local-theorem}); promotion to the chiral
Hochschild target through Drinfeld--Sokolov is the standard
construction of \cite{frenkel-ben-zvi}*{\S 15.6}; the
\(N\to\infty\) admissible \(E_\infty\) endpoint is conditional on
the four hypotheses listed.
