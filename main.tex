...
\end{titlingpage}
  ...
  module
  \[
    \mathcal P=\operatorname{Ann}_{\Res}(\C\cdot1)\subset H^2_{\mathfrak m}(A)\otimes\omega_A,
    \quad
  ...
  \]
  the holomorphic-surface two-variable refinement of the Heisenberg
  current on the formal disk.  Inverse limits over Matlis windows
  ...
  Matlis principal-part module \(\mathcal P\).  Under a curve
  restriction \(C\hookrightarrow\C^2\), \(\mathcal P\) restricts to the
  Kac--Moody central-pairing module \(\Omega_K\) and \(\mathfrak h\) to
  the regular functions on the curve.  The same formal local sector appears in Costello's
  ...
  Calabi--Yau category of the holomorphic-symplectic
...
A curve vertex operator algebra (VOA) is a
...
The boundary open category on the Kato--Nakayama log boundary
$\partial X^{\mathrm{KN}}$ of the brane log structure on $X$
(equivalently the Costello--Gwilliam brane-link boundary $\partial X$
of the Dirac brane defect $L\subset X$, henceforth written $\partial X$)
is
...
Global open--closed comparison (OCA) on \(\partial X\)
&
conditional
&
...
Theorem~\ref{thm:typed-global-mixed-ht-deligne}.
  ...
  \[
    P=\mathfrak h^\vee_{\mathrm{cont}}=
      \operatorname{Ann}_{\Res}(\C\cdot1)\subset H^2_{\mathfrak m}(R)\,dz_1dz_2,
    \qquad
...
\begin{prob}[Costello first/third graph-normalization obstruction datum]\label{prob:first-third-graph-normalizations}
  ...
  deformation-quantization convention. The analytic problem is reduced
...
Z^{\mathrm{der}}_{E_2^{\mathrm{HT}}}\!(\mathcal{C}^{\mathrm{op}}_\partial)\)
of Theorem~\ref{thm:main-global-conditional}.  By the
