...
\title{\texorpdfstring{\textit{The Trace-Sector Formal-Darboux Stalk in Mixed Holomorphic-Topological Strings}\\[0.5em]\large Stable Trace Map, CE/PV Koszul Dictionary, and Brane $E_1$-Hochschild Boundary Value at $N$ Dirac Branes on $\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}$\\[0.3em]\normalsize the trace-sector formal-Darboux stalk of the Mixed Holomorphic--Topological Deligne problem}{The Trace-Sector Formal-Darboux Stalk in Mixed Holomorphic-Topological Strings: Stable Trace Map, CE/PV Koszul Dictionary, and Brane E1-Hochschild Boundary Value at N Dirac Branes on R2 top x C2 hol --- the trace-sector formal-Darboux stalk of the Mixed Holomorphic-Topological Deligne problem}}
\date{May 2026}
...
\makeatother
\hypersetup{
  pdftitle={The Trace-Sector Formal-Darboux Stalk in Mixed Holomorphic-Topological Strings: stable trace map, CE/PV Koszul dictionary, and brane E1-Hochschild boundary value at N Dirac branes on R2 top x C2 hol --- the trace-sector formal-Darboux stalk of the Mixed Holomorphic-Topological Deligne problem},
...
{\HUGE\scshape The Trace-Sector Formal-Darboux Stalk in\\Mixed Holomorphic-Topological Strings}
...
{\normalsize\itshape the trace-sector formal-Darboux stalk of the Mixed
...
\end{titlingpage}
  ...
  Calabi--Yau target.  In the Costello--Li BV renormalisation
  \cite{costello-renormalization}\cite{costello-li-quantum-bcov}\cite{costello-li-open-closed-bcov},
  its local Costello holomorphic--topological sector
  \cite{costello-twistedM} lives on
  \(X=\R^2_{t,s}\times\widehat{\C^2}_{0,\mathrm{hol}}\): the theory is
  topological in \((t,s)\) and holomorphic in \((z_1,z_2)\).  A Dirac
  brane at \(p\in\C^2\) means the formal skyscraper boundary condition
  supported on \(L=\R_t\times\{0_s\}\times\{p\}\); a stack of \(N\)
  Dirac branes is this boundary condition with Chan--Paton space
  \(\C^N\), equivalently the local transverse derived chart
  \([\mu^{-1}_{\mathrm{der}}(0)/\mathrm{GL}_N]\) with
  \(\mu(\phi_1,\phi_2)=[\phi_1,\phi_2]\)
  (Definition~\ref{defn:dirac-brane-stack-boundaries}).  Near this
  holomorphic-symplectic formal boundary condition the closed fields
  form Hamiltonian BF theory.  The closed-string fields
  ...
  \]
	  The classical observables form the holomorphic factorization algebra
	  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) on the holomorphic
	  surface in the sense of Costello--Gwilliam
	  \cite{costello-gwilliam}.
	  For \(p\in\C^2\), the formal-Darboux stalk functor
	  \(\operatorname{FDSt}_p\) is restriction of a mixed-HT
	  factorization algebra to
	  \(\R^2_{\mathrm{top}}\times\widehat{\C^2}_p\), expressed in a
	  formal Darboux coordinate system; it is equivariant under the formal
	  symplectomorphism group.  Thus
	  \[
	    \operatorname{FDSt}_p(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}})
	    =
	    C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
	  \]
	  for the Hamiltonian BF bulk.

  \medskip\noindent
  The open boundary is a stack of \(N\) Dirac branes on the worldline
  \(\R_t\subset X\).  Its two holomorphic transverse coordinates become
  Chan--Paton matrices
  ...
  \]
  with Koszul differential $Q\psi=[\phi_1,\phi_2]$.  A polynomial closed
  Hamiltonian \(f\in\bar A=\C[z_1,z_2]/\C\cdot1\) acts on this brane
  sector through the trace map
  \[
    J\colon\bar A\longrightarrow
  ...
  \(b\in\C^2\).  For \(f\in\mathfrak h\), \(J(f)\) denotes the
  continuous extension to the completed formal brane algebra.

  \medskip\noindent
  Two boundary objects occur, and they are not identified in the local
  theorem.  The local open--closed comparison uses the
  Costello--Gwilliam brane-link boundary
  \[
    \partial_{\mathrm{link}}X=\partial(X\setminus\nu(L)),
  \]
  the boundary of a tubular neighbourhood of the Dirac defect.  In the
  local model it is \(L\times S^4_\epsilon\).  A Kato--Nakayama
  boundary \(\partial X^{\mathrm{KN}}\) is attached instead to a chosen
  log compactification \((\overline X,D)\).  Any comparison between
  \(\partial_{\mathrm{link}}X\) and \(\partial X^{\mathrm{KN}}\) is part
  of the global matched-conventions datum, not part of the
  formal-Darboux trace-sector theorem.

  \medskip\noindent
  The mixed holomorphic--topological Deligne problem asks whether the
  physical bulk
  \(\mathcal A^{\mathrm{cl}/\hbar}_{\mathrm{bulk}}\) and the Morita
  mixed-HT chiral \(E_2\)-centre
  \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
  (\mathcal C^{\mathrm{op}}_\partial)\) of the primitive boundary open
  factorisation category \(\mathcal C^{\mathrm{op}}_\partial\) on the
  Costello--Gwilliam brane-link boundary
  \(\partial_{\mathrm{link}}X\) are related by a global open--closed
  quasi-isomorphism.  This paper proves the formal-Darboux trace-sector
  stalk of that problem at the Dirac brane vacuum.  Before
  any Hochschild-centre lift, the finite-\(N\) datum is the generator
  assignment
  \[
    ...
    J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)} .
  \]
  After scalar reduction, degreewise stable trace passage,
  Roos/strict-limit comparison, and the admissible completion
  \(\mathcal B^{\mathrm{adm}}\), this assignment becomes the
  Chevalley--Eilenberg/polyvector coordinate isomorphism
  \[
    C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
    \xrightarrow{\ \sim\ }
  ...
  \]
  A Hochschild cochain map to the whole finite-\(N\) centre, a
  mixed-HT \(E_2\) open--closed quasi-isomorphism, and a global Deligne
  equivalence require the additional all-arity, descent, quantum master
  equation, anomaly, locality, and cone-contraction data recorded below.

  \medskip\noindent
  \emph{What is new.}  The formal-local theorem has three proved
  outputs.
  \begin{enumerate}[label=\textup{(\arabic*)}]
    \item The trace map \(J(f)=\overline{\operatorname{Tr}\,
      f(\phi_1,\phi_2)}\) is the unique stable formal-Darboux trace
      coordinate compatible with Procesi--Razmyslov trace generators,
      Koszul descent for \([\phi_1,\phi_2]\), and change of Darboux
      chart.
    \item The assignment \(c_f\mapsto\theta_f,\;u_f\mapsto J(f)\) is
      a completed Chevalley--Eilenberg/polyvector coordinate
      isomorphism of dg commutative algebras, with restricted
      \(P_0\)-enhancement carried by the bracket-admissible structure of
      \(\mathcal B^{\mathrm{adm}}\).
    \item The finite-\(N\) Capelli term \(\hbar N[\bar c]\) is the
      projective anomaly at the trace generator;
      it is the determinant-line curvature when a determinant or
      modular line with connection pulls its Atiyah class back to
      \([\bar c]\).
  \end{enumerate}

  \medskip\noindent
  \emph{Scope.}  The native object is the two-complex-dimensional
  holomorphic \(E_2\) factorisation algebra on \(\widehat{\C^2}_0\).
  The curve vertex algebra is its restriction along a holomorphic curve
  with pushed-forward bracket and BV pairing; the compact Calabi--Yau
  threefold BCOV comparison and the Hall--Drinfeld--Borcherds/Igusa
  operator lift are its images under the matched-conventions functors of
  Theorem~\ref{thm:cross-target-restriction} and
  Conjectures~\ref{conj:hall-drinfeld-borcherds-ss}
  and~\ref{conj:operator-modular-lift}, each carrying its own comparison
  datum.  The finite-\(N\) trace-relation ideal and stack stabilizer
  classes are the kernel and cokernel of the finite-\(N\) restriction of
  Theorem~\ref{thm:six-coordinates-one-curvature}, recorded in
  \S\ref{ssec:finite-N-leftovers}.

  \medskip\noindent
  \emph{What is not proved here.}
  A proof of the full global mixed-HT Deligne theorem requires the
  following six data, with notation as in
  Definitions~\ref{defn:global-deligne-source-complexes}
  and~\ref{defn:global-mixed-ht-deligne-obstruction-datum} and
  Theorem~\ref{thm:typed-global-mixed-ht-deligne}.
  \begin{enumerate}[label=\textup{(D\arabic*)}]
    \item Construct the Morita chiral \(E_2\)-centre
      \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
      (\mathcal C^{\mathrm{op}}_\partial)\) as an
      \(\mathrm{SC}^{\mathrm{HT}}_{(1,2)}\)-algebra and solve the
      Maurer--Cartan lift in
      \[
        \mathfrak{Def}^{\mathrm{HT}}_{\mathrm{Mor}\mbox{-}E_2}
        (\mathcal C^{\mathrm{op}}_\partial,b_p),
      \]
      extending the binary diagonal kernel \(K^{(2)}_\Delta\) to all
      chiral \(E_2\) arities and killing
      \(\operatorname{Ob}^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}\).
    \item Construct the typed open--closed morphism
      \[
        \mathrm{OCA}_N\colon
      ...
      \]
      on the Costello--Gwilliam brane-link boundary, with formal
      Darboux fibre equal to Theorem~\ref{thm:main-local}, and give a
      Weiss/Ran descent datum killing
      \(\operatorname{Ob}^{\mathrm{desc}}_{\partial}\).
    \item Choose \(Q^{\mathrm{GF}}\), heat kernels \(K_t\), finite-scale
      propagators \(P_{\varepsilon,L}\), BV Laplacians \(\Delta_L\),
      and local counterterms \(I[L]\) satisfying the Costello
      scale-\(L\) quantum master equation
      \[
        QI[L]+\tfrac12\{I[L],I[L]\}_L+\hbar\Delta_LI[L]=0
      \]
      in the admissible completion, thereby killing
      \(\operatorname{Ob}^{\mathrm{QME}}_{\mathrm{all\mbox{-}order}}\).
    \item Give a null-homotopy of the anomaly class in
      \[
        H^2\!\left(\mathrm{Conv}_{\mathrm{SC}}\!\bigl(
        \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}},
        Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
        (\mathcal C^{\mathrm{op}}_\partial)\bigr)\right),
      \]
      with trace-sector scalar equal to the Capelli class
      \(\hbar N[\bar c]\); a determinant or modular curvature
      interpretation requires the corresponding line and connection.
    \item Give the factorization-locality homotopy for the
      disjoint-union extension of \(\mathrm{OCA}_N\), equivalently kill
      \(\operatorname{Ob}^{\mathrm{loc}}\) in the Costello--Gwilliam
      \v{C}ech--Weiss--Ran locality complex on
      \(\partial_{\mathrm{link}}X\).
    \item Construct a contracting homotopy for
      \(\operatorname{Cone}(\mathrm{OCA}_N)\) in
      \(\mathcal B^{\mathrm{adm}}\).  An equivalence of mixed-HT
      \(\mathrm{SC}\)-algebras further requires the operadic
      homotopy-inverse datum.
  \end{enumerate}
  The formal-Darboux theorem supplies the trace-sector fibre entering
  \textup{(D2)} and the Capelli scalar appearing in \textup{(D4)}.  It
  does not construct the all-arity centre \textup{(D1)}, global descent
  datum \textup{(D2)}, all-order quantum master equation \textup{(D3)},
  anomaly null-homotopy \textup{(D4)}, locality homotopy
  \textup{(D5)}, or cone contraction \textup{(D6)}.

  \begin{tcolorbox}[
    colback=white,
    colframe=black,
    title={Hypotheses for Theorem~\ref{thm:intro-main-formal-darboux}},
    fonttitle=\bfseries]
  \begin{enumerate}[label=\textup{(A\arabic*)}]
	    \item The local source is the formal Hamiltonian BF Lie algebra
	      \(\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]\)
	      obtained from
	      \(\operatorname{FDSt}_{p}(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}})\)
	      of Definition~\ref{defn:formal-darboux-stalk-functor}
	      at the Dirac brane vacuum \(b_p\) over \(p\) on
	      \(\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\); no compact target
	      or curve vertex algebra is part of the source.
    \item The target is the Morita mixed-HT chiral \(E_2\)-centre
      \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
      (\mathcal C^{\mathrm{op}}_\partial)\) in the admissible completion
      \(\mathcal B^{\mathrm{adm}}\).  Its formal-Darboux fibre is the
      scalar-reduced stable trace sector obtained by \(U_{\mathrm{sc}}\)
      and \(\operatorname{StTr}\); the finite-\(N\) trace-relation ideal
      and Chevalley--Eilenberg stabilizer cohomology are the kernel and
      cokernel of the finite-\(N\) restriction.
    \item The \(P_0\)-enhancement uses the bracket-admissible subspace
      and the kernel-admissible weighted completion of
      Proposition~\ref{prop:verified-badm-weighted-model}, in which the
      diagonal Casimir converges.  Without that topology the displayed
      Casimir is only a formal finite-window family.
    \item A Hochschild cochain map \(\Phi_N^{\mathrm{Hoch}}\) exists only
      after one supplies the centre-lift Maurer--Cartan element
      \(\Theta_{\mathrm{OCA}}+\tau\) and centrality primitives
      \(h_\Omega\).  The finite-\(N\) generator assignment by itself is
      not a Hochschild cochain map.
    \item A global identification on a holomorphic-symplectic target is
      not asserted by the formal-Darboux theorem.  The global criterion
      requires nonresonant equivariant parameters
      \((\epsilon_1,\epsilon_2)\), the admissible nuclear completion
      \(\mathcal B^{\mathrm{adm}}\), a Hamiltonian \(T_\Omega\)-action
      whose fixed locus is the Dirac support, and the residual
      \v{C}ech period \(\operatorname{per}_X(v)\).
  \end{enumerate}
  \end{tcolorbox}
  ...
  \begin{thm}[Main theorem, introductory form]\label{thm:intro-main-formal-darboux}
    ...
    \]
    and let \(\widehat{\mathfrak g}_{\mathrm{cent}}\) be the Capelli
    projective central extension with cocycle
    \(\omega(f,g)=[\{f,g\}]_0\).  Put
    \[
      A^{\mathrm{cl}}_{\partial,N}=
      C^\bullet_{\mathrm{CE}}\bigl(\mathfrak{gl}_N,\,
        \mathrm{Sym}(\mathfrak{gl}_N\oplus\mathfrak{gl}_N
    ...
    \]
    the quotient-stack Koszul cdga of the derived commuting stack at
    \(N\) Dirac branes, and
    \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}\!
    \bigl(\mathcal C^{\mathrm{op}}_\partial\bigr)\) its Morita mixed-HT
    chiral \(E_2\)-centre.  Let
    \[
      A_{\partial,H}^{\mathrm{st}}
      \cong
      \widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h)
    \]
    denote the stable scalar-reduced trace algebra obtained from
    \(A^{\mathrm{cl}}_{\partial,N}\) by the functors
    \(U_{\mathrm{sc}}\), \(\operatorname{StTr}\), the specified
    Roos/strict-limit convention, and the admissible completion
    \(\mathcal B^{\mathrm{adm}}\) of
    Definitions~\ref{defn:typed-local-source-target-dictionary},
    \ref{defn:trace-sector-reduction},
    and~\ref{defn:admissible-host-category}, with weighted Hamiltonian
    model Proposition~\ref{prop:verified-badm-weighted-model} when a
    diagonal BV kernel is used.

    The finite-\(N\) boundary value is the generator assignment
    \[
      \Phi_N^{\mathrm{gen}}\colon\quad
      c_f\longmapsto\theta_f,\qquad
      u_f\longmapsto J(f)\cdot(-),\qquad
      c_K\longmapsto\gamma_{\mathbf 1},\qquad
      J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)} ,
    \]
    not a Hochschild cochain map by itself.  A map
    \(\Phi_N^{\mathrm{Hoch}}\) exists only after a centre-lift
    Maurer--Cartan element \(\Theta_{\mathrm{OCA}}+\tau\) and the
    residual weight-zero primitives of
    Theorem~\ref{thm:omega-localization-collapse} have been supplied.
    Its projective scalar curvature is then the Capelli class
    \(\hbar N[\bar c]\).

    After the scalar reduction, stable trace passage, Roos/strict-limit
    comparison, and admissible completion named above, the
    formal-Darboux restriction is the dg-commutative coordinate
    isomorphism
    \[
      ...
      HH^\bullet_{\mathrm{adm},\mathrm{HKR}}
        (A_{\partial,H}^{\mathrm{st}},
         A_{\partial,H}^{\mathrm{st}}) .
    \]
    Under the bracket-admissibility estimates in
    \(\mathcal B^{\mathrm{adm}}\) and the localized centre lift
    \(\tau\), the isomorphism carries the restricted
    \(P_0\)-bracket on trace generators.  In the projective deformation
    the central generator \(K\) acts on the rank-\(N\) trace line by
    \(N\), so the trace generator has Capelli scalar
    \(\hbar N[\bar c]\).  This scalar is a projective anomaly; it is a
    determinant-line or modular curvature only after the corresponding
    line and connection have been supplied.

    At the formal-Darboux stalk this is the chiral \(E_2\)-Deligne
    theorem at \(d=2\) for the Hamiltonian BF model
    (Theorem~\ref{app:thm:FCR-at-stalk}), and it globalises
    unconditionally to every Stein holomorphic-symplectic surface
    (Theorem~\ref{app:thm:stein-global-deligne}) -- in particular
    \(\C^2\), smooth affine symplectic surfaces, and \(T^*\Sigma\) -- by
    canonicity and Cartan \v{C}ech--Dolbeault descent.  On compact
    targets the residual is the Hamiltonian period
    \(\operatorname{per}_X(v)\in H^1(X,\C_X)\)
    (Theorem~\ref{thm:main-global-deligne-criterion}); the only open
    case is the formality of the chiral \(E_2\) operad at \(d=2\) for a
    general factorization algebra
    (Conjecture~\ref{conj:chiral-Ed-deligne}).  The curve vertex algebra
    and compact Calabi--Yau comparisons are its matched-conventions
    images.
  \end{thm}

  \medskip\noindent
  \phantomsection\label{tab:intro-theorem-status}
  The status of the load-bearing statements is:
  \begin{center}
  \footnotesize
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.20\textwidth}
    ...
    \textbf{Evidence in this manuscript} &
    \textbf{Boundary or missing datum} \\
    Trace coordinate \(J(f)\) & proved &
    Proposition~\ref{prop:J-three-input-uniqueness} and
    Theorem~\ref{thm:Jf-coordinate-uniqueness}, with
    Proposition~\ref{prop:stable-trace-invariants} and
    Lemma~\ref{lem:koszul-adjacent-transpositions}. &
    Stable range and Koszul descent are part of the statement; formal
    \(f\in\mathfrak h\) requires the chosen completion. \\
    CE/PV trace-sector coordinate map & proved after reductions &
    Theorem~\ref{thm:intro-main-formal-darboux} and
    Theorem~\ref{thm:main-local}, using
    Definitions~\ref{defn:typed-local-source-target-dictionary},
    \ref{defn:trace-sector-reduction}, and
    \ref{defn:admissible-host-category}. &
    Scalar reduction, degreewise stable trace passage, and
    \(\mathcal B^{\mathrm{adm}}\) are structural hypotheses; this is
    not the whole finite-\(N\) Hochschild centre. \\
    Hochschild cochain map
    \(\Phi_N^{\mathrm{Hoch}}\) & conditional construction &
    Definition~\ref{defn:admissible-hochschild-centre-lift} and
    Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}. &
    Requires a Hochschild-centre Maurer--Cartan lift and centrality
    primitives; before that \(\Phi_N^{\mathrm{gen}}\) is only the
    generator assignment. \\
    Capelli class \(\hbar N[\bar c]\) & proved projective anomaly &
    Theorem~\ref{thm:capelli-equals-modular-curvature}. &
    It is determinant or modular curvature only after a line with
    connection pulls its Atiyah class back to \([\bar c]\). \\
    Global mixed-HT Deligne quasi-isomorphism & criterion /
    obstruction problem &
    Definition~\ref{defn:global-mixed-ht-deligne-obstruction-datum}
    and Theorem~\ref{thm:main-global-deligne-criterion}. &
    Requires all-arity chiral \(E_2\) operations, descent, all-order
    quantum master equation counterterms, anomaly/locality
    null-homotopies, and an open--closed cone contraction. \\
    Curve vertex-algebra reduction & conditional comparison &
    Native operation is the two-complex-dimensional diagonal
    local-cohomology product. &
    A vertex algebra appears only after a curve restriction with
    pushed-forward bracket, BV pairing, brane image, and anomaly
    matching. \\
    Compact Calabi--Yau / Hall / Igusa operator material & conjectural /
    separate &
    The restriction theorem is
    Theorem~\ref{thm:cross-target-restriction}; the compact statements
    are Conjectures~\ref{conj:hall-drinfeld-borcherds-ss} and
    \ref{conj:operator-modular-lift}. &
    These require matched compact-target conventions and do not follow
    from the formal-Darboux stalk. \\
    Whole fixed-\(N\) Hochschild centre & non-claim &
    \S\ref{ssec:finite-N-leftovers}. &
    Finite-\(N\) trace relations and stabilizer cohomology remain as
    kernel and cokernel data. \\
  \end{tabular}
  \end{center}

  \medskip\noindent
  \emph{Proof dependencies.}  The proof of
  Theorem~\ref{thm:intro-main-formal-darboux} uses the following
  directed proof dependencies.
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item The finite cotangent CE/PV identity is
      Theorem~\ref{thm:coordinate-free-cotangent-ce-pv}, and its
      formal coordinate extension is
      Theorem~\ref{thm:reduced-ce-pv-central-operation}.
    \item Stable trace and Koszul descent are
      Proposition~\ref{prop:stable-trace-invariants},
      Lemma~\ref{lem:koszul-adjacent-transpositions},
      Proposition~\ref{prop:J-three-input-uniqueness}, and
      Theorem~\ref{thm:Jf-coordinate-uniqueness}.
    \item Admissible topology and residue duality are fixed by
      Definition~\ref{defn:admissible-host-category},
      Theorem~\ref{thm:principal-part-coadjoint-uniqueness}, and
      Theorem~\ref{thm:wt-omega-kernel-admissibility}.
    \item Trace-sector selection is
      Definition~\ref{defn:trace-sector-reduction}.
    \item The projective anomaly is
      Theorem~\ref{thm:capelli-equals-modular-curvature}.
    \item The Hochschild lift boundary is
      Definition~\ref{defn:admissible-hochschild-centre-lift} together
      with Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}.
  \end{enumerate}
  These statements imply Theorem~\ref{thm:main-local}; a global
  mixed-HT Deligne equivalence would require the additional obstruction
  classes named in Theorem~\ref{thm:main-global-deligne-criterion}.
  ...
  Chevalley--Eilenberg differential of $\mathfrak g$; the classical
  master equation $\{S,S\}=0$ holds because the BF interaction
  $I=\tfrac12\int_X\langle\mathcal B,
  [\mathcal A,\mathcal A]_{\mathfrak h}\rangle$ encodes the Lie
  bracket on $\mathfrak h$.

  A Costello-renormalized quantization of this mixed brane-defect sector
  is additional data.  One must choose a gauge-fixing operator
  \(Q^{\mathrm{GF}}\), heat kernel \(K_t\), finite-scale propagator
  \(P_{\varepsilon,L}\), BV Laplacian \(\Delta_L\), and a coefficient
  completion in which the Tate Casimir converges
  (Lemma~\ref{lem:tate-casimir-obstruction}).  Only after these data are
  fixed does the scale-\(L\) quantum master equation
  \[
    QI[L]+\tfrac12\{I[L],I[L]\}_L+\hbar\Delta_L\,I[L]=0
  \]
  have a defined local-functional meaning.  Its obstruction is the
  filtered tower of scalar-projection classes, reduced non-scalar
  \(H^1\)-classes, and \(\varprojlim^1H^0\) primitive-compatibility
  classes of Definition~\ref{defn:wt-nonscalar-qme-tower} and
  Theorem~\ref{thm:wt-nonscalar-counterterm-criterion}.  The Hochschild
  lift \(\Phi_N^{\mathrm{Hoch}}\) uses this QME datum only as a
  criterion; the formal CE/PV trace-coordinate theorem below is
  classical after the stated scalar reduction and admissible completion.
  ...
  \[
    ...
    \mathcal P
      &=\operatorname{Ann}_{\Res}(\C\cdot1)
        \subset H^2_{\mathfrak m}(A)\otimes\omega_A,\\
    \mathcal P
      &=\bigoplus_{a+b>0}\C\,\rho_{a,b},\qquad
        \omega_A=dz_1\wedge dz_2,\\
    \rho_{a,b}
      &=z_1^{-a-1}z_2^{-b-1}\,dz_1\,dz_2 .
    \end{aligned}
  \]
  ...
  vector $v\in\C^N$ with $\C[\phi_1,\phi_2]\cdot v=\C^N$.
  The Nekrasov--Schwarz noncommutative Hilbert scheme is obtained
  from the framed ADHM/noncommutative deformation; its central
  parameter is carried by the framing or by a projective central
  extension, not by the finite-dimensional equation
  $[\phi_1,\phi_2]=\hbar I_N$ in $\mathfrak{gl}_N$
  ...
  $J(z_i)=\overline{\operatorname{Tr}\phi_i}$, these data determine
  \(J\) monomial-by-monomial
  (Proposition~\ref{prop:J-three-input-uniqueness};
  Theorem~\ref{thm:Jf-coordinate-uniqueness});
  among admissible protected Hamiltonian sectors at a
  holomorphic-symplectic brane point, $J$ is the formal-Darboux
  normal-form datum
  ...
  as curve-chiral comparison objects with these extra hypotheses.

  \medskip\noindent
  The closed-to-open generator boundary value on
  Chevalley--Eilenberg generators of \(\mathfrak g\) assigns to each
  generator-dual coordinate \(c_f\) the Hamiltonian Schouten derivation
  \(\theta_f\in C^1(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
  and to each cotangent-dual coordinate \(u_f\) the brane observable
  \(O_f=J(f)\in C^0(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
  (Theorem~\ref{thm:closed-to-open-derived-centre-map}).  A full
  Hochschild-centre lift would be a chain map
  \[
  ...
  \]
  and exists at the chain level only after the vanishing of
  the obstruction vector
  $\operatorname{Ob}_{\mathrm{cent}/\mathrm{QME}/\mathrm{desc}}$
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}) and the
  corresponding centrality, quantum master equation, and descent
  primitives in Appendix~\ref{app:completion-formal-local-theorem}.
  ...
  projective Moyal commutator of the Weyl-ordered trace representation,
  \[
    [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_{\mathrm{proj}}
      =\hbar N\cdot\mathbf 1
  \]
  (Theorem~\ref{thm:app-scalar-anomaly-transgression}).  Here
  \([\, ,\,]_{\mathrm{proj}}\) denotes the central component of the
  Weyl-ordered commutator in the Capelli central extension
  \(\mathfrak h_{\hbar,N}\).  The number
  $N$ is the trace-$U(1)$ central charge of the noncommutative D-brane
  stack on $\C^2_\hbar$.  A determinant-line curvature interpretation of
  this class requires the determinant-line datum of
  Theorem~\ref{thm:capelli-omega-central-identification}.  The strict
  scalar-reduced Hamiltonian action lifts to a projective action
  ...
  \((J,\phi_1,\phi_2,\psi,c_f\mapsto\theta_f,u_f\mapsto J(f))\).
  Let \(d_{\mathrm{QME}}^{L}=Q+\{I[L],-\}_{L}+\hbar\Delta_L\) denote
  Costello's scale-\(L\) quantum master differential on local
  functionals; its associated-graded classical part is
  \(d_{\mathrm{cl}}=Q+\{I_0,-\}\).
  Globalization, quantization, all-bidegree Weyl/Moyal compatibility,
  and chain-level centre maps place this datum in different native
  deformation complexes.  A single curvature equation is defined only
  after a common deformation complex and comparison morphisms have been
  constructed (Theorem~\ref{thm:obstruction-twist-dichotomy}).  All-bidegree
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\) for all \((a,b)\) under
  the Harish-Chandra/Levasseur--Stafford radial-image hypothesis of
  Statement~\ref{stmt:app-radial-parts-hypotheses}; the obstruction to a
  chain-level \(P_0\)-factorization-centre lift of
  \(\Phi_N^{\mathrm{gen}}\) by the factorization-centre vector
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\)
  (Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}); the
  reduced non-scalar Costello QME by the scalar-projection lift classes
  \(o_{\sigma,w}^{(r)}\), followed by the class
  \([\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]\in
  H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}}^{L})\) and its
  finite-window \(\varprojlim^1H^0\)-primitive class
  (Definition~\ref{defn:wt-nonscalar-qme-tower};
  Theorem~\ref{thm:wt-nonscalar-counterterm-criterion}).  Its order-three
  CE-source row \(\theta_3\) gives the prototype
  \(H^1_{\mathrm{bare}}=\C\!\cdot\!\mathsf E_{\theta_3,M}\).  After a
  scalar-zero local counterterm \(C_{\theta_3,M}\) with
  \(d_M C_{\theta_3,M}=-\mathsf E_{\theta_3,M}\) is adjoined to the
  finite-window complex, the coefficient \(c=1\) kills the row in any
  finite window \(M\ni\theta_3\)
  (Theorem~\ref{thm:app-theta-three-finite-window-H1}); and the
  attempted strict all-window Bochner--Martinelli current transfer
  meets \(\operatorname{Ob}^\Pi_{\mathrm{BM}}\), nonzero on every finite
  Matlis window \(\mathcal P_{\le N}\)
  (Theorem~\ref{thm:bmk-finite-window-obstruction}), with coherent action
  on the pro-Matlis substituted module
  \(\widehat{\mathcal P}_\Pi=\varprojlim_N\mathcal P_{\le N}\)
  (Theorem~\ref{thm:promatlis-universal-property}).  These four
  classes are the obstructions identified for the centre, quantum,
  radial, and current-transfer comparisons considered here; each
  comparison carries its null-homotopy in its native complex.
  Minimal-model classification and an exact
count of nonzero higher brackets are independent classification theorems.
  ...
  the regular functions on the curve.  Costello's twisted M-theory
  family \cite{costello-twistedM} motivates the source conventions;
  no identification with that theory is used in the proof.  On the cyclic
  framed stable branch it gives the trace chart on
  \(\Hilb^N(\C^2)\); the Moyal deformation is the corresponding
  framed noncommutative deformation, not an ordinary finite-rank
  scalar commutator equation.

  \medskip\noindent
  The global formulation has a source category
  \(\mathcal C\) and the composite functor
  \(\Phi_d=\mathrm{Sp}^{\mathrm{ch}}_{\Sigma_{d-1},C}
  \circ\Phi^{\mathrm{FA}}_d\).  The proof below uses only the local
  formal stalk: the local holomorphic \(E_2\) factorization algebra
  \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\) on the formal
  holomorphic surface.  Compatibility with any global theorem is
  measured by the matched-conventions obstruction class
  $\operatorname{Ob}_{\mathrm{comparison}}$
  of Theorem~\ref{thm:matched-conventions-transfer}: a nonzero
  coordinate of $\operatorname{Ob}_{\mathrm{comparison}}$ is the obstruction
  to using the local Hamiltonian BF theorem inside a compact target
  carrying a BCOV/Kodaira--Spencer propagator, a
  Calabi--Yau-to-chiral specialization, an Igusa/Borcherds--Kac--Moody
  normalization, a Gromov--Witten/Donaldson--Thomas/Pandharipande--Thomas
  virtual trace, or an Ooguri--Strominger--Vafa attractor comparison.
...
trace theorem first gives the trace-sector generator boundary value
\[
  C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
  \dashrightarrow
...
\]
and proves the stable scalar-reduced CE/PV equivalence after the
...
comparison theorems governed by Theorem~\ref{thm:cross-target-restriction}
and the matched-conventions
obstruction class \(\operatorname{Ob}_{\mathrm{comparison}}(\mathfrak C_{\mathrm{tar}})\).
...
one-dimensional chiral object.  It can be compared with the native
factorization algebra only after a controlled reduction gives
...
symbol \(\operatorname{Tr}\) is the graded cyclic quotient, its
Hochschild shadow is obtained by forgetting the \(S^1\)-equivariance
in the cyclic complex, and the Connes \(B\)-operator is the connecting
operator in the SBI sequence.  The negative-cyclic class is the
Calabi--Yau pairing on the target category.  Drinfeld centre \(Z(\mathcal C)\), derived centre
...
\(Z^{\mathrm{der}}_{E_2^{\mathrm{ch}}}
(\mathcal C^{\mathrm{op}}_\partial)\), the chiral derived
\(E_2\)-centre of the boundary open category.  The formal-Darboux trace
theorem gives the trace-sector generator boundary value and, after
...
derived-centre comparison is measured by
\(\mathfrak{Ob}_{\mathrm{Del}}^{\mathrm{glob}}\) and the typed
criterion of Theorem~\ref{thm:typed-global-mixed-ht-deligne}.  Three distinct
...
trace-sector Hochschild generator assignment
\[
  c_f\mapsto\theta_f,\qquad
  u_f\mapsto J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)} .
\]
for polynomial \(f\), with formal \(f\) interpreted after completion.
After the stable scalar-reduced trace quotient and admissible
completion this is the CE/PV coordinate isomorphism.  At fixed finite
...
global open--closed morphism from the bulk chiral \(E_2\) algebra
...
Hochschild target of this trace-sector map.

\paragraph{Cross-volume objects anchored to the present theorem.}
The global boundary category lives on the Kato--Nakayama log boundary
\(\partial X^{\mathrm{KN}}\).  The local calculation uses the
Costello--Gwilliam brane-link boundary \(\partial_{\mathrm{link}}X\)
of the Dirac brane defect \(L\subset X\).  A comparison between these
...
moment-map ideal of conjugation on
	...
	\(\hbar N[\bar c]\) is the projective Lie anomaly at the trace generator
	(Theorem~\ref{thm:capelli-omega-central-identification}).  The
compact Igusa comparison identifies the modular line bundle on
\(\overline{\mathcal A}_g\) with the central projective-curvature
line after matched conventions; the Igusa cusp form \(\Phi_{10}\) is
the level-2 scalar section, and
...
Lemma~\ref{lem:app-sign-ce-pv} is the trace-sector Koszul assignment from
the bulk formal stalk to the brane-side \(E_1\)-Hochschild cochain
complex
\(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
at the chosen Dirac brane vacuum
\(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\), with formal coordinate
algebra
\(A^{\mathrm{coord}}_{b_p}=\widehat{\mathcal O}_{\C^2,p}\)
(Theorem~\ref{thm:formal-local-universal-property}).  The full
Mixed Holomorphic--Topological Deligne identification is the
derived-centre problem controlled below.
...
\item BMK comparison: the one-pair analytic pro-Matlis retract of
  ...
  target replacement in which that obstruction is represented.
...
compact object to which it is compared.  According to the case, this
...
\[
  c_f\longmapsto\theta_f,\qquad u_f\longmapsto J(f)
\]
...
The strict ambient product does not carry this theorem: the
degree-lowering Hamiltonian bracket has a Jacobi obstruction
(Lemma~\ref{lem:gauge-paired-necklace-plain-quotient-obstruction},
Theorem~\ref{thm:gauge-brst-cyclic-obstruction}), and the diagonal
Casimir becomes a Maurer--Cartan kernel only after the weighted Tate
completion of Theorem~\ref{thm:wt-canonical-envelope}.
...
curvature class \(\hbar N[\bar c]\) of
Theorem~\ref{thm:capelli-equals-modular-curvature}.

The global open--closed comparison on the Costello--Gwilliam
brane-link boundary \(\partial_{\mathrm{link}}X\) requires the
vanishing and chosen
null-homotopies of the chiral \(E_2\)-Deligne, descent, and all-order QME
obstruction classes, together with a contraction of
...
The strict-product obstructions of row two force the admissible
...
convergent diagonal kernel.  The centre-lift, descent, quantum master
equation, anomaly, locality, and cone-contraction primitives remain the
...
For fixed \(N\), the brane stack is represented by
\[
  \mathcal R_N=
...
\[
  ...
  \right).
\]
Proposition~\ref{prop:stable-trace-sector-type} says
\(I^{\mathrm{tr}}_{N,d}=0\) in the stable range \(N\ge d\).  Outside
that range it is not zero; for \(N=2\), degree \(3\) contains the
...
\[
  ...
  C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N,\mathcal R_N)
\]
records stack stabilizer cohomology.  Reductive exactness
(Lemma~\ref{lem:reductive-invariants-koszul-exactness}) identifies the
length-zero invariant Koszul sector with invariant cohomology; it does
not kill the stabilizer classes of
\([\mu^{-1}_{\mathrm{der}}(0)/GL_N]\).  These classes are outside
\(\operatorname{TrSec}^{\mathrm{st}}_{\mathrm{adm}}\).

Third, the generator assignment
\[
  \Phi_N^{\mathrm{gen}}\colon
  c_f\longmapsto\theta_f,\qquad
  u_f\longmapsto J(f)
\]
becomes a Hochschild-centre map only after the centrality defects vanish
with chosen primitives:
\[
  d_{\mathrm{cent}}H_{x,y}=\delta^{\mathrm{Lie}}_{x,y},\qquad
  d_{\mathrm{cent}}H^{\mathrm{prod}}_{x,a}
    =\delta^{\mathrm{prod}}_{x,a},\qquad
  d_{\mathrm{cent}}H^{P_0}_{x,a}=\delta^{P_0}_{x,a}.
\]
...
Theorem~\ref{thm:completion-centrality-homotopies}.  Until these
homotopies are supplied, \(\Phi_N^{\mathrm{gen}}\) is a trace-sector
generator assignment, not the whole finite-\(N\) Hochschild centre.

The deformation-quantized scalar is likewise projective.  The finite
matrix equation \([\phi_1,\phi_2]=\hbar I_N\) is impossible for
\(\hbar\ne0\), since its trace would give \(0=\hbar N\).  The surviving
...
holomorphic-symplectic brane point, a trace-sector boundary value in
the brane Hochschild complex.  A Hochschild cochain map requires the
centrality Maurer--Cartan lift.  This manuscript computes the
formal-Darboux coordinate expression of the scalar-stable trace sector
at \(N\) Dirac branes.
...
\(A^{\mathrm{coord}}_{b_p}\), and formal Darboux coordinates \((z_1,z_2)\) from
...
the Koszul derived zero fibre of the commutator moment map.
...
Hamiltonian formal disk only after a curve-reduction datum and
...
\]
	the completed gauge-invariant trace character; on polynomial
	Hamiltonians it is the ordinary trace on the finite-\(N\) brane algebra.
...
underived commuting representation functor, and the cotangent-dual
\(u_f\) mapping to the boundary observable \(O_f=J(f)\) on the brane;
the CE/PV cochain
...
\begin{ex}[Single-point evaluation, \(N=1\)]\label{exa:point-evaluation-N1}
  ...
  \((\phi_1,\phi_2)\in\C^2\).  This is the underived support of the
...
\label{exa:two-branes-generic}
  ...
  Cayley--Hamilton identity for the \(2\times2\) matrix \(X\) gives
  \[
    2\operatorname{Tr}(X^3)
    -3\operatorname{Tr}(X)\operatorname{Tr}(X^2)
    +\operatorname{Tr}(X)^3=0.
  \]
  Thus \(J(z_1^3)\) is a finite-rank trace function, not a free stable
  ...
  \(\hbar N[\bar c]\).  The full finite-window computation is recorded
...
\begin{ex}[Linear Hamiltonians and the scalar anomaly]\label{exa:linear-hamiltonians-anomaly}
  ...
  representation,
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_{\mathrm{proj}}
    =\hbar N\cdot\mathbf 1\); the first
...
\(\mathfrak h_{\leq N}\), so the plain projected bracket
...
\S\ref{sec:capelli-scalar} records the finite-\(N\) scalar of
Example~\ref{exa:linear-hamiltonians-anomaly} as the projective Lie
anomaly \(\hbar N[\bar c]\).  These choices give the
completed CE/PV trace-sector map.  They do not turn the finite-\(N\)
  brane algebra into the whole derived centre; the strict-product and
  unreduced \(P_0\)-centrality extensions remain the obstruction problem.
\end{rmk}

\subsection{Mixed holomorphic-topological strings}

In the local theory, a mixed-HT string-field-theoretic sector is a
classical BV theory whose local observables form a factorization
algebra in the Costello--Gwilliam sense
\cite{costello-gwilliam}, together with a cyclic open BV theory on a
...
\begin{rmk}[Canonical completion]
  ...
  BCOV/Kodaira--Spencer \cite{bcov} gives the source conventions:
...
\begin{rmk}[Quantum extension]
  ...
  \]
  The balanced \((7,7)\) is the first undecided case in the exact
  kernel-correction window.
...
\begin{defn}[Local Hamiltonian holomorphic factorization algebra]\label{def:local-hamiltonian-chiral-factorization-algebra}
  ...
  The open quantum brane model is the Weyl/Moyal algebra of the
  ...
  bar-cobar models.  The support-local defect/cotangent model on an
  ...
  statement requires a restriction theorem choosing a complex
...
\begin{proof}
  ...
  next; the one-dimensional vertex/OPE structure requires the
  restriction data of Theorem~\ref{thm:dict-kinematic-locality}.
\end{proof}

\input{two-dimensional-holomorphic-ope}

\subsubsection{The closed--open factorization triple: \texorpdfstring{\(E_2\)}{E2} bulk, \texorpdfstring{\(E_1\)}{E1} brane, trace-sector action}\label{sssec:mixed-ht-factorization-triple}
...
point.  A lift to the Koszul derived zero fibre is a choice of homotopy
on the moment-map equation.
...
\begin{lem}[Hamiltonian vector field on the representation stack]\label{lem:hamiltonian-vector-field-on-Rep}
  ...
  characters is independent of that choice and is
  \[
    \theta_f^0\bigl(J_g\bigr)
     =J_{\{f,g\}_{\bar A}}
...
\begin{proof}
  ...
  functor of \(A\).  Since \(\delta_f(g)=\{f,g\}_{\bar A}\), cyclicity
  of trace gives
  \(\theta_f^0(J_g)=J_{\{f,g\}_{\bar A}}\).
  ...
  lifts to this Koszul cdga after choosing a null-homotopy for its
  action on \(\mu\).  The displayed equation is precisely the condition
  \([Q,\widetilde\theta_f]\psi=0\).  Adding a \(Q\)-closed vertical term
  to \(h_f\) changes the lift, but not the induced derivation of the
  underived quotient or the displayed trace action on \(H^0\).
\end{proof}

\begin{thm}[Trace-sector Hochschild action and CE/PV reduction]\label{thm:closed-to-open-derived-centre-map}
  There is a generator-level trace-sector Hochschild action on the
  Chevalley--Eilenberg generators of the formal bulk stalk,
  \[
    \Phi_N^{\mathrm{gen}}\colon
    \operatorname{Gen}_{\mathrm{CE}}\!
    \bigl(C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)\bigr)
     \dashrightarrow C^\bullet_{\mathrm{Hoch}}
     (A^{\mathrm{cl}}_{\partial,N},
                       A^{\mathrm{cl}}_{\partial,N}),
  \]
  while Deligne's \(E_1\)-Hochschild theorem identifies the derived
  \(E_1\)-centre of the brane algebra with the Hochschild cochain model
  \[
  ...
  \[
    c_f\longmapsto\theta_f\in C^1(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N}),
    \quad
    u_f\longmapsto O_f=J_f\cdot(\,\cdot\,)\in C^0(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})
    \quad(f\in\bar A),
  \]
  where \(J_f=J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\)
  acts by multiplication and \(\theta_f\) is a chosen Koszul lift of the
  Hamiltonian vector field of
  Lemma~\ref{lem:hamiltonian-vector-field-on-Rep}.  For formal
  \(f\in\mathfrak h\), the assignment is defined in the completed
  brane trace algebra.  The displayed target is the Hochschild
  target of Deligne's \(E_1\)-theorem; it is not an assertion that the
  bulk is equivalent to the whole derived centre, and it is not by
  itself a cochain map from Chevalley--Eilenberg cochains to ordinary
  Hochschild cochains.  The proved dg identity is the
  Chevalley--Eilenberg/polyvector identity of
  Theorem~\ref{thm:reduced-ce-pv-central-operation} on
  \(c^{a,b},u_{a,b}\), with Schouten differential
  \(d_\pi=[\pi,-]_S\).  If compatible Koszul lifts
  \((\widetilde\theta_f,h_f)\) and Hochschild equivariance homotopies are
  fixed, the assignment extends to the trace Hochschild subcomplex
  generated by \(J(f)\) and \(\theta_f\).  The full chain-level lift to the strict
  ...
  the dg coordinate identity in the reduced Hamiltonian \(P_0\) model;
  its Hochschild image is a generator action.  A Hochschild-cochain map
  \(\Phi_N^{\mathrm{Hoch}}\), and hence the chain-level \(P_0\)-centre map
  \(\Phi^{\mathrm{cl}}\), exists only
  after compatible null-homotopies of
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\) have been chosen.
\end{thm}
...
\begin{proof}
  ...
  The assignment is defined on CE generators as Hochschild cochains.  For
  \(f\in\bar A\), the Hamiltonian-dual coordinate \(c_f\) is sent
  ...
  \(\psi\), and a Koszul lift is part of the chosen datum);
  the cotangent-dual coordinate \(u_f\) is sent to the Hochschild
  \(0\)-cochain \(J_f\cdot(\,\cdot\,)\in
  C^0(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\),
  multiplication by the trace character
  \(J_f=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\).  For
  \(f\in\mathfrak h\) the same formula has meaning only in the
  completed formal brane algebra.  The action identities in Hochschild
  cochains are
  \[
  ...
  They express the Hamiltonian action on open observables.  The
  Chevalley--Eilenberg differential itself is matched in the reduced
  Hamiltonian \(P_0\) model:
  \[
    d_\pi\theta^K=-\tfrac12 C^K_{IJ}\theta^I\theta^J,\qquad
    d_\pi O_K=C^L_{KJ}O_L\theta^J,
  \]
  which is Theorem~\ref{thm:reduced-ce-pv-central-operation}.  Thus
  \(c\mapsto\theta\) and \(u\mapsto O\) are a dg substitution in
  \((\mathrm{PV},d_\pi)\), while their Hochschild realisation is an
  action datum.  It becomes a Hochschild cochain map only after the
  Koszul lifts satisfy \([Q,\theta_f]=0\) and the centre-lift
  homotopies have been supplied.  Continuous extension across the
  graded-commutative coordinate algebra is
  Theorem~\ref{thm:reduced-ce-pv-central-operation}.
  Deligne's \(E_1\)-Hochschild theorem identifies the target
  \(Z^{\mathrm{der}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\) with the
  Hochschild cochain \(E_2\)-algebra
  \(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\),
  whose cohomology is
  \(HH^\bullet(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\).
  The displayed generator formulas define the Hochschild trace action.
  The
  Costello--Gwilliam factorization-algebra lift is the strict
  centre-map problem governed by
  \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\).  Uniqueness of the boundary values
  \(u_f\mapsto J(f)\) is Theorem~\ref{thm:Jf-coordinate-uniqueness}.

  A Hochschild-cochain model for \(\Phi_N^{\mathrm{Hoch}}\) in every
  cochain degree is precisely the additional lift \(\Phi^{\mathrm{cl}}\)
  to the strict \(P_0\)-factorization-centre.  Its existence is governed
  by the obstruction \(\operatorname{Ob}_{\mathrm{cent/QME/desc}}\) of
  Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}; the
  curvature class is the difference between the derived \(E_1\)-centre
  \(E_2\)-structure of Deligne's theorem and the strict
...
\begin{prop}[Bochner--Martinelli current data and finite-window obstruction for $E_2$ transfer]\label{prop:finite-window-bm-native-e2-transfer}
  ...
  finite-window homotopy requires an additional construction.  It agrees,
...
\begin{rmk}[Hamiltonian period class and global descent]
  ...
  obstruction in
	  ...
	  matched-conventions hypothesis for globalization.  Descent, QME,
	  anomaly cancellation, locality, and the OCA comparison-cone
...
\(\R\subset T^*\R\) and the B-brane \(\mc O_p\) at the formal
  ...
  in the AKSZ form \cite{aksz}, gives the formal action
...
\(f\in\mathfrak h\) it is the completed formal trace.
...
representation functor by Hamiltonian vector fields; the
\(c_f\mapsto\theta_f\) coordinate records its infinitesimal motion on
the moduli. The trace character of a bulk Hamiltonian
\(f\in\bar A\) at the
shifted-cotangent coordinate \(u_f\) is
\begin{equation}\label{eq:setup-Jf-flagship}
  J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)},
\end{equation}
the gauge-invariant trace of \(f\) evaluated on the Chan--Paton pair,
projected to the stable scalar-reduced quotient; for
\(f\in\widehat{\mathfrak h}=\C[[z_1,z_2]]/\C\cdot1\) this is read in the
completed formal brane algebra. Under the formal-stalk-chart hypotheses
...
\(GL_N\)-invariance, formal Darboux naturality, and stable scalar
  reduction, three independent structures force \(J\). First,
...
the only possible gauge-invariant scalar functionals
...
analytic strict transfer and selecting the pro-Matlis retract as the
replacement target
(Corollary~\ref{cor:promatlis-retract-compatible-replacement}).
...
twist class \(\operatorname{Ob}^{\mathrm{univ}}_{p,X}\) to be
represented and killed.
...
\begin{rmk}[\(c_f\) versus \(u_f\)]\label{rmk:type-discipline-cf-uf}
  ...
  \(u_f\) is the CE coordinate whose CE/PV image \(O_f\) is itself an
  observable.
\end{rmk}
...
\begin{prop}[Geometry determined by the formal-stalk chart]\label{prop:local-model-mixed-definition}
  ...
  \((\Omega_N,\operatorname{Tr})\) of the brane stack.  The open
	  polynomial observable family equals
	  \(J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}\), with completed
	  extension for \(f\in\mathfrak h\), by the
...
\begin{rmk}[Two CE coordinates of a Hamiltonian: operation and evaluation]\label{rmk:intro-symbol-separation}
...
one-dimensional VOA/OPE algebra is present only after a
...
theory \cite{bcov} fixes this language.  The closed BV Lie algebra is
...
Both factors are forced: the Hamiltonian factor by
...
\begin{rmk}[Casimir/Tate obstruction to a strict cotangent kernel]\label{rmk:closed-bv-casimir-obstruction}
  ...
  cotangent-kernel construction must clear this obstruction.
\end{rmk}
...
\(\mathfrak{gl}_N\)-equivariant trace Hochschild subalgebra is
...
infinite-dimensional.  The finite-rank reduction used here is the open
boundary that names the two normal coordinates: a stack of
...
A polynomial Hamiltonian \(f\) has trace evaluation
\[
  J(f)=\overline{\operatorname{Tr}f(\phi_1,\phi_2)}
\]
on this chart; a formal \(f\in\mathfrak h\) has the same expression only
in the completed brane algebra.  This is the formal Darboux coordinate
of the trace-sector generator boundary value from the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) to the brane-side
\(E_1\)-Hochschild cochain complex
\(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\).
The full derived-centre identification is a separate open--closed
Deligne problem; in this manuscript it is represented only by the
obstruction criteria and hypotheses named above.  The trace calculation
...
Koszul sector; by
Lemma~\ref{lem:reductive-invariants-koszul-exactness} it agrees with
the \(GL_N\)-invariants in the Koszul cohomology.  It does not compute
the positive CE-length stabilizer classes of the quotient stack.
...
\]
so the difference of the two adjacent orderings is the BRST image of an
antifield insertion.  Cyclic invariance and this Koszul homotopy reduce every cyclic
...
\(\bar A=\C[z_1,z_2]/\C\cdot 1\) acts on the Hamiltonian trace sector
through \(J\).
...
\begin{lem}[Trace realisation of the formal polydisk]\label{lem:formal-stalk-trace}
  ...
  polynomial in trace observables, while the primitive Hamiltonian
  sector \(\bar A=\C[z_1,z_2]/\C\cdot 1\) acts through \(J\).
\end{lem}
...
open trace cohomology generates the stable open trace algebra.
...
\begin{ex}[\(N=1\): the geometric prototype]\label{ex:N1-commuting-case}
  ...
  \]
	  ...
	  Formal \(f\) are evaluated after completion at the chosen point.  \(J\) reduces
...
\begin{lem}[Cyclic framing branch and the framed noncommutative deformation]\label{lem:cyclic-framing-hilbert-branch}\label{rmk:cyclic-framing-hilbert-branch}
  ...
  derived commuting stack does not become underived on this locus:
  ...
  finite-dimensional fibre \([\phi_1,\phi_2]=\hbar I_N\), since
...
\begin{proof}
  ...
  \([\phi_1,\phi_2]=\hbar I_N\) has no solution for \(\hbar\ne0\).
...
requires an open-trace \(A_\infty\)-Koszul homotopy datum
(level~5 of \S\ref{ssec:two-algebras}),
which is an additional hypothesis beyond the stable hypotheses.
...
generated by cyclic trace words.  In the connected sector the Koszul
differential
...
Connected-trace projection: the Loday--Quillen--Tsygan cyclic-homology calculation
\cite{loday-quillen}\cite{tsygan}\cite{loday-vallette} gives the primitive
...
Hamiltonian envelope requires the open-trace
...
has an \(HC_{*-1}\)-normalization precisely after the displayed
finite-window and Milnor conditions have been imposed.
...
\begin{defn}[Stable scalar-reduced trace-sector passage]\label{def:stable-trace-sector-passage}
  ...
  primitive/indecomposable projection for block diagonal sum, and the
  ...
  The construction is a functor from the truncation poset \(W\) to
  graded vector spaces; it is not a statement about the full finite-\(N\)
  quotient-stack cohomology.
\end{defn}
...
\begin{prop}[Type and finite-rank kernel of the stable trace sector]\label{prop:stable-trace-sector-type}
  ...
  is zero in word degree \(d\) for \(N\geq d\).  If an LQT
...
\begin{proof}
  ...
  cohomology class as
  \(\operatorname{Tr}(\phi_1^k\phi_2^l)\).  Reductive exactness
  (Lemma~\ref{lem:reductive-invariants-koszul-exactness}) is the only
  passage from invariant Koszul cochains to invariant Koszul
  cohomology.
  ...
  The finite-rank kernel statement is the second fundamental theorem
  of Procesi--Razmyslov in the same degree convention.  The final
...
\label{lem:reductive-invariants-koszul-exactness}
  ...
  \]
  is an isomorphism.  The same statement holds for every finite
  augmentation-window quotient
  ...
  are not part of the scalar-reduced stable trace sector.
\end{lem}
...
\begin{proof}
  ...
  Reynolds projector splits every short exact sequence
  \(0\to B^p\to Z^p\to H^p\to0\) and
  \(0\to Z^p\to C^p\to B^{p+1}\to0\).  Taking invariants therefore
  commutes with cohomology on every finite augmentation quotient and
  on every fixed Koszul-weight summand.

  The full polynomial complex requires no separate limiting theorem:
  the same exactness applies directly to the rational cochain groups
  \(C^p=\mc R_N^p\), their \(G_N\)-stable cycles, and their
  \(G_N\)-stable boundaries.  The finite quotients and homogeneous
  Koszul-weight summands are included because the trace-sector construction is always
  read in such windows before passage to the stable inverse system.
  This proves the displayed comparison for the length-zero Koszul
  complex.  No assertion is made about killing positive
  Chevalley--Eilenberg length in
...
algebra, after quantum Hamiltonian reduction by \(\lie{sl}_N\), the
normal-ordered moment relation reads
\[
...
\begin{proof}
  ...
  facts. First, $\C[\lie{gl}_N^2]^{GL_N}$ is generated by trace functions of
  ...
  cyclic words.  When these traces are read in the derived commuting
  stack, Lemma~\ref{lem:reductive-invariants-koszul-exactness} is the
  only interchange used between invariant functions and Koszul
...
\label{thm:normal-ordering-cyclic-word-homotopy}
  ...
  of the full cyclic open complex requires additional data.
\end{thm}
...
\begin{proof}
  ...
  derivation of \(\wedge\), and \(Q\) is a derivation of the necklace
	  ...
	  the product in the symmetric algebra.  If two currents have disjoint
...
\begin{defn}[Three large-\(N\) operations]\label{defn:three-large-N-operations}
  ...
  unprojected \(N\to\infty\) algebra and the permuted composites are
  different objects.
\end{defn}
...
\begin{rmk}[Open-trace $A_\infty$-Koszul homotopy datum]\label{rmk:open-trace-koszul-admissibility}
  ...
  Statement~\ref{stmt:app-radial-parts-hypotheses} are imposed.  The decoupled scalar class
...
\(\mathfrak h\) only after completion; its
...
\[
  J_{\leq W}(f)
  =
  \overline{\operatorname{Tr}\,\tilde f(\phi_1,\phi_2)},\qquad
  f\in \mathfrak m/\mathfrak m^{W+1},
\]
is independent of the ordered noncommutative lift \(\tilde f\) in the
...
\[
  J(f)=\operatorname{Tr}f(\phi_1,\phi_2),\qquad
  f\in\mathfrak h .
\]
...
transpositions makes it well defined on the derived stack
  ...
  identity is isolated in Remark~\ref{rmk:consistency-probes}.

\begin{prop}[Three-input uniqueness of the trace coordinate]
\label{prop:J-three-input-uniqueness}
  ...
  exactly one continuous \(\C\)-linear map
  \[
  ...
  \begin{enumerate}[label=\textup{(\arabic*)}]
    \item \emph{Stable trace invariance.}  In total word degree \(d\),
      \(\widetilde J(f)\) is represented in ranks \(N\geq d\) by a
      \(GL_N\)-invariant primitive single-trace class; the empty trace
      \(\operatorname{Tr}_N(1)\) is removed by the scalar quotient.  With a
      Chevalley--Eilenberg cochain window \(L\), the same assertion is
    ...
    \item \emph{Darboux naturality and normalization.}  The map is
      ...
      substitution on the trace sector, and
      \[
...
\begin{proof}
  ...
  invariant trace class is a polynomial in cyclic-word traces, and no
  finite-rank trace identity occurs in that window
  ...
  \]
  applied to the normalized value of \(z_1\), forces
...
the formal Darboux coordinate expression at
  \(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) of the trace-sector
generator assignment whose \(u_f\)-component is multiplication by
\(J(f)\) inside the brane-side \(E_1\)-Hochschild complex
  \(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\).
  It is not, before the centre-lift Maurer--Cartan datum and residual
centrality primitives are supplied, a Hochschild cochain map.  Deligne's
\(E_1\)-Hochschild theorem gives the \(E_2\)-structure on the target; see
\nameref{ssec:local-theorem-conventions}.  Here
\(b_p\in\mathcal{C}^{\mathrm{op}}_\partial\) is the chosen vacuum
at \(p=0\in\C^2\)
(Definition~\ref{def:formal-darboux-stalk-chart}), with chart
algebra
\(A^{\mathrm{coord}}_{b_p}=\widehat{\mathcal O}_{\C^2,p}\cong\C[[z_1,z_2]]\)
...
\(J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\).  For \(f\in\mathfrak h\),
the same symbol denotes the continuous extension to the completed
brane algebra.  The closed BV
...
\(\{J(f),J(g)\}_\partial=J(\{f,g\}_{\mathfrak h})\) on the trace
sector of \(A^{\mathrm{cl}}_{\partial,N}\).  Closed Hamiltonians act
through \(c_f\); \(J\)
...
\begin{lem}[Two CE coordinates of a Hamiltonian]\label{lem:type-discipline}
  ...
  closed shifted-cotangent BV side.  The CE/PV map has degree \(0\);
  \(c_f\) and \(\theta_f\) are odd, while \(u_f\) and \(O_f\) are even.
  The cotangent / Schouten bracket has degree \(-1\).
  \begin{itemize}
    ...
    \item As closed cotangent insertion:
      ...
      \(J(f)=\mathrm{Tr}\,f(\phi_1,\phi_2)\) through the coordinate
      algebra \(A_b^{\mathrm{coord}}\)
...
\begin{rmk}[Generator table]\label{rmk:type-discipline-table}
  \[
    \begin{array}{c|c|c|c}
      ...
      \hline\\[-9pt]
      c_f\ \text{(odd)} & \mathfrak h^\vee[-1]
        & \theta_f\in\mathrm{Vect}(\widehat{\mathbf S}(\bar A))
        & \text{generator of canonical motion}\\
      u_f\ \text{(even)} & (\mathfrak h^\vee_{\mathrm{cont}}[1])^\vee
        & O_f=J(f)\in\widehat{\mathbf S}(\bar A)
        & \text{coordinate trace map }J\text{ through }A_b^{\mathrm{coord}}
    \end{array}
  \]
  The nonzero generator bracket is
  \([\theta_f,O_g]_S=\theta_f(O_g)=O_{\{f,g\}}\), equivalently
  \(d_{\mathrm{CE}}u_g(c_f)=u_{\{f,g\}}\).
\end{rmk}
...
\(H^0(\mathcal R_N^{GL_N},Q)\): adjacent transpositions are exact via
...
The plain finite-rank reduction of \(\widehat{\mathfrak h}\) is its
...
\(H^0(\mathcal R_N^{GL_N},Q)\).  By
Lemma~\ref{lem:reductive-invariants-koszul-exactness} this is the
invariant Koszul cohomology in CE length zero; by
...
(Theorem~\ref{thm:wt-canonical-envelope}); it satisfies levels 3
and 4, with level~5 under the open-trace
\(A_\infty\)-Koszul homotopy datum.
...
stack of \(N\) topological branes on \(\R\times p\), gives the
physical motivation for a Hamiltonian formal sector at the symplectic
polydisk \(\widehat X_p\).  Once an ambient background gives an
...
ambient product is an admissibility hypothesis.
...
\begin{defn}[Bracket-admissible CE/PV realizations]\label{defn:bracket-kernel-admissible}
  A coordinate realization \(B\subset
...
\begin{proof}
  ...
  \(\mathfrak m^3\) forces that radical to be Lie-stable under the
...
Koszul model for the trace-sector generator assignment inside the chiral
Hochschild complex at the brane vacuum; the admissible criterion for its
bracket-admissible and kernel-admissible enhancements is
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.  Theorem~\ref{thm:main-local}
identifies this trace sector after scalar reduction and completion,
not the whole finite-\(N\) Hochschild complex.
...
Lie algebra \(\mathfrak l\), CE cochains on
\(\mathfrak l\ltimes\mathfrak l^\vee[1]\) are Schouten polyvectors on
\(\mathfrak l^\vee\), with \(c^\xi\mapsto\theta^\xi\) and
\(u_x\mapsto O_x\).  The formal-Darboux statement is obtained only
after replacing \(\mathfrak l\) by finite Hamiltonian coordinate
windows, replacing the ordinary dual by the continuous Matlis dual,
...
the trace summand in
\(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
at the coordinate chart \(A^{\mathrm{coord}}_{b_p}\); the cotangent CE/PV reduced
central-operation construction of
\S\ref{ssec:coordinate-free-ce-pv-synthesis} is the formal-Darboux
stalk expression of that trace-sector map.  The Mixed
Holomorphic--Topological Deligne conjecture is the further
derived-centre identification with the bulk
...
resolution of the trace summand in the brane-side derived
\(E_1\)-Hochschild complex
\(C^\bullet_{\mathrm{Hoch}}(A^{\mathrm{cl}}_{\partial,N},A^{\mathrm{cl}}_{\partial,N})\)
at the formal-Darboux stalk of the Dirac brane vacuum \(b_p\): closed CE
...
Holomorphic--Topological Deligne conjecture identifies this trace
sector with the bulk in the presence of the derived-centre, descent,
and QME data.  The
...
\begin{defn}[Reduced Hamiltonian \(P_0\) central-operation complex]\label{defn:reduced-hamiltonian-p0-center}
  ...
  An HKR or Hochschild-center identification is a continuous
  Hochschild--Kostant--Rosenberg theorem compatible with the chosen
  completion and interval functoriality.
\end{defn}

\begin{thm}[Finite-dimensional model calculation: cotangent CE/PV identity]
  ...
  \(\mathfrak l^\vee\) in the direction \(\xi\).  The parities are
  \[
    |u_x|=|O_x|=0,\qquad |c^\xi|=|\theta^\xi|=1,
  \]
  and the cotangent / Schouten bracket has degree \(-1\), normalized by
  \[
...
\begin{proof}
  ...
  Lemma~\ref{lem:app-sign-ce-pv}: \(c^i,\theta^i\) are odd,
...
\begin{thm}[Formal-polydisk source-to-boundary CE/PV coordinate reconstruction]
  ...
  whose connected/indecomposable quotient is \(\mathfrak h\), the
	  coordinate finite-coefficient construction gives
  \[
    C^\bullet_{\mathrm{CE},\mathrm{coord}}
  ...
  completion makes the coordinate contraction bracket continuous, the
	  dashed generator map extends to the completed bracketed comparison
  ...
  \(\Phi_{\mathfrak l}\), four changes occur.  First,
  \(\mathfrak l\) is replaced by the inverse system of finite
  Hamiltonian coordinate windows of \(\mathfrak h\).  Second,
  \(\mathfrak l^\vee\) is replaced by the continuous Matlis dual
...
\begin{proof}
  ...
  is coefficientwise through finite coefficient projections.  For each
...
Theorem~\ref{thm:main-local} is the formal-Darboux answer to the
mixed-HT Deligne problem at the Dirac brane vacuum \(b_p\).  The
closed local operator algebra
\[
  C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
\]
acts on the boundary algebra by Hamiltonian Schouten derivations and
trace functions:
\[
  c_f\mapsto\theta_f,\qquad u_f\mapsto J(f)\cdot(-),\qquad
  J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\quad(f\in\bar A).
\]
For \(f\in\mathfrak h\), \(J(f)\) is evaluated in the completed formal
brane algebra.  After scalar reduction, passage to the stable trace
...
classes, and the unreduced \(P_0\)-centrality homotopies measure the
part of the Hochschild centre not seen by the trace-sector map.  The
finite-\(N\) scalar anomaly is the projective Lie class
\(\hbar N[\bar c]\)
(Theorem~\ref{thm:capelli-equals-modular-curvature}).  The global
...
the Casimir converges (Theorem~\ref{thm:wt-canonical-envelope}).
      ...
      hypotheses.
	    \item If an admissible Tate, nilpotent, relative, or weighted
	      ...
	      duality, a filtered twisting cochain, strong convergence, and exact
		      ...
		      extends only to the admissible bar-cobar/enveloping statement
	      \[
...
\begin{thm}[Formal-Darboux trace-sector coordinate theorem at \(N\) Dirac branes]\label{thm:main-local}
  ...
  $\mathfrak g=\mathfrak h\ltimes P[1]$ carries, at a stack of $N$
  Dirac branes, the Capelli-projective central extension whose central
  cocycle is supported on the Hamiltonian summand:
  ...
  \(\bar A=\C[z_1,z_2]/\C\cdot 1\) to the projective Lie class
  $\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]$
  (Theorem~\ref{thm:capelli-equals-modular-curvature}).  The
  corresponding Chevalley--Eilenberg generator is the degree-one dual
  central coordinate \(c_K\); the evaluation \(K\mapsto
  \operatorname{Tr}\mathbf 1=N\) is a central character imposed only
  after passage to the projective current/enveloping algebra.  Let
  \[
    A^{\mathrm{cl}}_{\partial,N}=
    C^\bullet_{\mathrm{CE}}\bigl(\mathfrak{gl}_N,\,
       \mathrm{Sym}(\mathfrak{gl}_N\oplus\mathfrak{gl}_N
  ...
  \]
	  ...
	  admissible filtered nuclear Tate / pro-Matlis /
	  Capelli-projective category of
	  Definition~\ref{defn:admissible-host-category}, with the formal
	  completion theorem proved in
	  Appendix~\ref{app:completion-formal-local-theorem}.
  The base admissible structure consists of the first five conditions
  below.  The last two entries are additional centre and quantum data.
  They are not consequences of the formal-Darboux trace-sector theorem:
  the \(\Omega\)-localization of
  Theorem~\ref{thm:omega-localization-collapse} contracts nonzero
  weights and identifies the scalar Capelli projection only after the
  completed comparison complex and centre-lift curvature have been
  supplied.
  \begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item the diagonal local-cohomology kernel
      ...
      kernel-admissible;
    \item the Hochschild-centrality convolution Lie algebra is completed;
      a centre lift, when chosen, is a Maurer--Cartan element with
      components $\{h_{\alpha,a},h_{\alpha,a,b},\dots\}$;
    \item quantum and unreduced cotangent extensions use
      scalar-contact and QME counterterm towers $\{C_{n,M}\}$ satisfying
      Milnor/Roos compatibility $\pi_{M,N}C_{n,M}=C_{n,N}$.
  \end{enumerate}
  Then the continuous local-observable algebra at \(b\) of the formal
  current stalk of the bulk mixed-HT Hamiltonian BF factorization algebra
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}=\pi_*
  C^{\mathrm{CE}}_\bullet(\Omega^\bullet_c(\R^2)
  \widehat\otimes\Omega^{0,\bullet}_c(\C^2)
  \widehat\otimes\widehat{\mathfrak g}_{\mathrm{cent}})$
  is identified by exact continuous Tate duality with
  \(C^\bullet_{\mathrm{CE},\mathrm{cont}}(
  \widehat{\mathfrak g}_{\mathrm{cent}})\).  The trace sector is the
  finite-window/stable image functor of
  Definition~\ref{defn:trace-sector-reduction}.  At finite \(N\), before
  a centre-lift Maurer--Cartan element and its centrality primitives are
  chosen, the closed-to-open boundary value is the generator assignment
  ...
  \end{equation}
  determined on Chevalley--Eilenberg generators by the two compatible
  target components
  \begin{equation}\label{eq:ce-pv-dictionary-strong}
    \bigl(c_f,u_f\bigr)\;\longmapsto\;\Bigl(
       \theta_f,\;J(f)\cdot(-)\Bigr),
    \qquad
    c_K\longmapsto\gamma_{\mathbf 1}
    \text{ in }
    C^\bullet_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1].
  \end{equation}
  Here \(\theta_f\) is a chosen Koszul lift,
  \(J(f)\cdot(-)\) is a Hochschild \(0\)-cochain,
  \(c_K\) is the Chevalley--Eilenberg dual of the central Lie generator,
  \(\gamma_{\mathbf 1}\) is the scalar-contact ghost for the omitted
  constant Hamiltonian line, and the projective central character later
  evaluates \(K\) by \(K\mapsto\operatorname{Tr}\mathbf 1=N\).  The
  trace map is \(J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\) for
  \(f\in\bar A\), with formal extension in the completed brane algebra.
  The
  Schouten derivation satisfies
  ...
  \end{equation}
  The admissible Hochschild-centre lift of
  Definition~\ref{defn:admissible-hochschild-centre-lift} is additional
  data.  Its obstruction components are the low-degree centrality vector
  of Proposition~\ref{prop:low-degree-centre-lift-obstruction}, the
  residual non-scalar weight-zero classes of
  Lemma~\ref{lem:nonscalar-weight-zero-vanishing}, and the higher
  filtration/QME counterterm tower.  Proposition~\ref{prop:residual-vanishes-bf}
  proves scalarity only for the displayed generator trace-sector rows;
  Theorem~\ref{thm:omega-localization-collapse} kills nonzero weights
  but does not supply the residual weight-zero primitives.  When the
  centre-lift Maurer--Cartan element and these primitives are supplied,
  the generator assignment promotes to a Hochschild cochain morphism
  \(\Phi_N^{\mathrm{Hoch}}\).  After applying the scalar-reduction
  functor, passing to the stable
  trace algebra
  \(A_{\partial,H}^{\mathrm{st}}\cong
  \widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h)\), and keeping the
  admissible \(P_0\)-bracket, \(\Phi_{\mathrm{coord}}^{\mathrm{st}}\) is
  the continuous
  Chevalley--Eilenberg/polyvector chosen-coordinate isomorphism
  \[
    C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
    \;\simeq\;
    \bigl(\mathrm{PV}_{\mathrm{adm}}\!
    ...
    HH^\bullet_{\mathrm{adm,HKR}}\!
      \bigl(A_{\partial,H}^{\mathrm{st}},
            A_{\partial,H}^{\mathrm{st}}\bigr).
  \]
  The last isomorphism is the completed Hochschild--Kostant--Rosenberg
  identification for the stable scalar-reduced trace algebra; it is not
  an assertion about the finite-\(N\) Hochschild complex.
  The finite-\(N\) failure of
  equation~\eqref{eq:finite-N-generator-assignment} to be a whole Hochschild-centre
  computation is measured by the trace-identity ideal, the stack-level
  Chevalley--Eilenberg stabilizer classes, and the unreduced
  \(P_0\)-centrality vector of
  Theorem~\ref{thm:app-unreduced-cotangent-lift-obstruction}; the stable
  scalar-reduced trace algebra quotients to the sector where the CE/PV
  coordinate isomorphism is proved.
  ...
  quotient \(U_{\mathrm{sc}}\) and then applies the stable trace-sector
  functor, the supplied Roos-limit or strict-limit comparison, and the
  classical \(\hbar=0\) Hochschild--Kostant--Rosenberg / CE/PV shadow.
  It recovers the scalar-reduced trace-sector map
  \[
  ...
  \]
  and this map is an equivalence exactly after replacement of the
  finite-\(N\) algebra by the stable scalar-reduced trace algebra above.
  The central generator, pro-Matlis tower, equivariance homotopy
  hierarchy, and QME counterterm tower are not removed by
  \(U_{\mathrm{sc}}\) itself; their removal or replacement is part of the
  additional data entering \(\operatorname{Red}_{\mathrm{cl,tr}}\).
\end{thm}
...
\begin{proof}
  ...
  holomorphic in \(\C^2_{\mathrm{hol}}\).
	  ...
	  current stalk with
	  \(C^{\mathrm{CE}}_\bullet(\widehat{\mathfrak g}_{\mathrm{cent}})\).
  ...
  the derived commuting stack for \(f\in\bar A\), and for
  \(f\in\mathfrak h\) after completion at the formal Darboux point.  In the stable trace range the
  ...
  \emph{(4) Hamiltonian action preserves the derived constraint.}
  The derivation \(\theta_f\) of
  \eqref{eq:schouten-explicit} preserves the moment-map ideal and
  extends to \(\psi\) with \([Q,\theta_f]=0\) once the Koszul homotopy
  \(h_f\) has been chosen.  It therefore defines a Hochschild
  \(1\)-cochain, and multiplication by \(J(f)\) defines a Hochschild
  ...
  Theorem~\ref{thm:omega-localization-collapse} contracts only nonzero
  weights after the completed comparison complex is fixed; it does not
  by itself supply the residual weight-zero centrality primitives.  When
  those primitives are supplied, the action is the Hochschild cochain map
  \(\Phi_N^{\mathrm{Hoch}}\).
  ...
  The second term is the pullback of the central extension
  \(0\to\C\cdot1\to R\to R/\C\cdot1\to0\), represented by the central
  generator \(K\) in \(\widehat{\mathfrak g}_{\mathrm{cent}}\).

  \emph{(6) CE/PV comparison.}
  The finite-dimensional shifted-cotangent identity
  \(C^\bullet_{\mathrm{CE}}(\mathfrak l\ltimes\mathfrak l^\vee[1])
  \cong \mathrm{PV}(\mathbf S(\mathfrak l))\) extends coefficientwise
  to the formal disk under the admissible continuity estimates of
  \(\mathcal B^{\mathrm{adm}}\).  It gives a chosen-coordinate
  isomorphism on the
  stable scalar-reduced trace algebra
  \(A_{\partial,H}^{\mathrm{st}}\cong
  \widehat{\mathbf S}_{\mathrm{Tate}}(\mathfrak h)\).  The same
  generator formula gives a map to the finite-\(N\) Hochschild complex,
  but the Procesi--Razmyslov ideal and stack BRST classes prevent the
  finite-\(N\) target from being identified with the stable symmetric
  algebra.
  ...
  carry the Deligne \(E_2\)-structure.  Under the admissible
  Hochschild-centre lift, the induced map \(\Phi_N^{\mathrm{Hoch}}\) respects the
  Gerstenhaber operations on the completed trace sector, since
  \[
    [\theta_f,\theta_g]=\theta_{\{f,g\}},\qquad
    \theta_f(J(g))=J(\{f,g\}),\qquad
    [J(f),J(g)]_{\mathrm{proj}}=\hbar N[\{f,g\}]_0 .
  \]
  The diagonal local-cohomology class
  \(d\zeta_1d\zeta_2/(\zeta_1\zeta_2)\) gives the arity-two
  holomorphic singular product \eqref{eq:ope-linear}, and the
  central cocycle gives \eqref{eq:ope-projective}.

  The scalar quotient \(U_{\mathrm{sc}}\) removes only the declared
  central scalar line.  The passage to the scalar-reduced trace-sector
  CE/PV comparison uses the composite
  \(\operatorname{Red}_{\mathrm{cl,tr}}\), hence also the stable trace
  transition maps, Roos-limit convention, admissible HKR comparison, and
  classical \(\hbar=0\) projection.  A full finite-\(N\)
  derived-centre equivalence is precisely the additional centrality and
  descent problem measured by the obstruction vector named in the
  theorem.
\end{proof}

\begin{prop}[Diagonal local-cohomology kernel operad on $\C^2$]\label{prop:diagonal-cohomology-operad}
  Let
  \[
  ...
  is a formal coloured dg kernel operad in nuclear Tate vector spaces;
  on the Hamiltonian BF model it acts on
...
\begin{proof}
  ...
  arities $n\geq 4$ inherit the kernel associativity from the
  factorization-algebra composition law
  ...
  The all-arity chiral \(E_2\)-Deligne action on the derived centre is
  a separate Hochschild-centre lift problem.
\end{proof}
...
\begin{defn}[Admissible Hochschild-centre lift]\label{defn:admissible-hochschild-centre-lift}
  ...
  \[
    \{\Phi_N^{\mathrm{gen}}(x),M_a\}_{P_0}-M_{\rho_x^{P_0}(a)}
      =dH^{P_0}_{x,a}.
  \]
  Null-centrality is the special case \(\rho_x=\rho_x^{P_0}=0\), hence
  applies only to invariant centre classes.  The term $H^3$ is the order-three
  Jacobiator/associator
  $dH^3_{x,a,b}=\mathrm{Jac}\bigl(H^{\mathrm{prod}},H^{P_0}\bigr)$,
  and higher $H^n$ the Stasheff coherences.  The Maurer--Cartan
...
\label{defn:low-degree-hochschild-centrality-defects}
  ...
  \(\Phi=\Phi_N^{\mathrm{gen}}\) be the generator assignment of
  Definition~\ref{defn:admissible-hochschild-centre-lift}.  For
  homogeneous closed generators \(x,y\) and homogeneous open observable
  \(a\), with \(O_y\) denoting the degree-zero boundary observable
  attached to \(y\) when \(y\) is a \(u\)-coordinate, define the four
  primary centrality defects
  \[
    \begin{aligned}
      \delta^{\mathrm{Lie}}_{x,y}
        &:=[\Phi(x),\Phi(y)]_G-\Phi([x,y]),\\
      \delta^{\mathrm{act}}_{x|y}
        &:=[\Phi(x),M_{O_y}]_G-M_{\rho_x(O_y)},\\
      \delta^{\mathrm{prod}}_{x,a}
        &:=[\Phi(x),M_a]_G-M_{\rho_x(a)},\\
      \delta^{P_0}_{x,a}
        &:=\{\Phi(x),M_a\}_{P_0}-M_{\rho_x^{P_0}(a)}.
    \end{aligned}
  \]
  ...
  \(\delta^3_{x,y,a}\).  These five rows are the filtration
...
\label{prop:low-degree-centre-lift-obstruction}
  ...
  \[
    \begin{aligned}
      ...
      d_{\mathrm{cent}}H_{x|y}&=\delta^{\mathrm{act}}_{x|y},\\
      d_{\mathrm{cent}}H^{\mathrm{prod}}_{x,a}
        &=\delta^{\mathrm{prod}}_{x,a},\\
  ...
  \[
    \operatorname{ob}^{\leq3}_{\mathrm{cent}}(\Phi_N^{\mathrm{gen}})
      :=
      \bigl[
        \delta^{\mathrm{Lie}},
        \delta^{\mathrm{act}},
      ...
      \right)
  \]
  vanishes.  This obstruction class is zero for a strict centre class
  only after the open actions \(\rho_x,\rho_x^{P_0}\) vanish; otherwise
  the required datum is equivariance, not ordinary centrality.
\end{prop}
...
\begin{proof}
  ...
  change of lift by cochains \(H_{x,y},H_{x|y},
...
\begin{rmk}[Low-degree consequences]\label{rmk:consistency-probes}
  ...
  \begin{enumerate}[label=\textup{(\arabic*)}]
    ...
    \item \emph{Hochschild equivariance.}
      $[\Phi_N^{\mathrm{gen}}(x),M_a]_G-M_{\rho_x(a)}
      =dH^{\mathrm{prod}}_{x,a}$ and
      $\{\Phi_N^{\mathrm{gen}}(x),M_a\}_{P_0}
      -M_{\rho_x^{P_0}(a)}=dH^{P_0}_{x,a}$
    ...
    \item \emph{QME recursion.}  After the scalar-contact projection has
      been lifted to a filtered chain map, the order-\(\hbar^n\),
...
\begin{thm}[Hamiltonian trace generators in the admissible trace sector]\label{thm:protected-summand-exhausts}
  Inside $\mathcal B^{\mathrm{adm}}$,
  \[
    C^\bullet_{\mathrm{Hoch,tr,adm}}\!\bigl(A^{\mathrm{cl}}_{\partial,N},
                    A^{\mathrm{cl}}_{\partial,N}\bigr)
    \;=\;
    \bigl\langle\,J(f),\,\theta_f
       :f\in\mathfrak h\,\bigr\rangle_{\smile,\,\{-,-\}},
  \]
	  where the left-hand side is
	  \(\operatorname{TrSec}^{\mathrm{st}}_{\mathrm{adm}}\) of
	  Definition~\ref{defn:trace-sector-reduction}, whenever the
	  admissible Hochschild-centre Maurer--Cartan lift has been chosen.
	  Equivalently, it is the trace Hochschild subcomplex generated
	  under cup product and Gerstenhaber bracket by the trace coordinates
	  \(J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)\) for polynomial
	  \(f\), its completed formal extension, and their Schouten derivations
	  \(\theta_f\).  The trace-\(\mathrm U(1)\) central character belongs
  to the projective current algebra; its Chevalley--Eilenberg dual
  \(c_K\) maps to the scalar-contact ghost \(\gamma_{\mathbf 1}\), and
  \(K\) evaluates as \(\operatorname{Tr}\mathbf 1=N\) only after the
  projective central extension has been chosen.  The
  scalar-reduced shadow at $K=0$ gives the identity for
  the scalar-reduced trace subalgebra.  The theorem makes no assertion
  that this subalgebra is the whole finite-\(N\) Hochschild centre.
\end{thm}

\begin{proof}
  The finite-window CE/PV calculation
  (Theorem~\ref{thm:reduced-ce-pv-central-operation};
  Theorem~\ref{thm:universal-filtered-cobar-recognition}) identifies the
  admissible Hamiltonian trace summand with the corresponding polyvector
  summand in
  $PV^\bullet_{\mathrm{adm}}(\mathrm{Sym}\,\mathfrak h)^{GL_N}$.
  Its interpretation as a Hochschild-centre summand uses the admissible
  Hochschild-centre lift of
  Definition~\ref{defn:admissible-hochschild-centre-lift}; it is not a
  consequence of associative bar-cobar duality alone.
  Procesi--Razmyslov stable trace
  ...
  The corresponding trace polyvector summand is
  generated as a graded-commutative algebra over this stable function ring
  by the Schouten derivatives $\theta_f$ along the symplectic
  structure on $\widehat\C^2$.  The projective Capelli completion is a
  separate central extension adjoining $K$ with the cocycle
  ...
  $J(f)$ and every $\theta_f$, and is closed under cup product
...
\begin{prop}[Forgetful diagram for compact, boundary, and formal data]\label{prop:monotone-strength-tower}
  ...
  corresponding forgetful morphism:
	  the Capelli projective Lie anomaly $\hbar N[\bar c]$; the pro-Matlis
...
\begin{proof}
  ...
  $c_f\mapsto\theta_f$, $u_f\mapsto O_f$.  The last arrow to
  $T_{\mathrm{red}}$ quotients constants, killing $K$ and the
  pro-Matlis tail.  These are exactly the components omitted by the
  displayed arrows.
\end{proof}
...
pro-Matlis coherence.  The trace-$U(1)$ scalar anomaly
\(\hbar N[\bar c]\) is represented by the projective Capelli central extension
$\widehat{\mathfrak g}_{\mathrm{cent}}$ and reappears in
$\mathcal B^{\mathrm{adm}}$ as the central charge at the trace
generator.  A modular-line curvature interpretation requires the
determinant-line datum and connection.
...
charge \(\hbar N[\bar c]\) is the projective Lie anomaly at the trace
generator.  It becomes determinant-line curvature only after a
...
\begin{thm}[Global open--closed obstruction criterion on the Costello--Gwilliam brane-link boundary]\label{thm:main-global-deligne-criterion}
  ...
  \(\partial X\) be its Costello--Gwilliam brane-link boundary
  \cite{costello-gwilliam}*{Vol.~II}: the link sphere bundle over \(L\),
  ...
  Let a pre-open--closed comparison datum
  \(\mathrm{preOCA}_N\) of Definition~\ref{defn:oca-comparison} be
  given in the admissible category \(\mathcal B^{\mathrm{adm}}\), with
  source the bulk mixed-HT factorization algebra
  ...
  Theorem~\ref{thm:main-local}.  This supplies only the stalkwise
  trace-sector input; it supplies none of the global null-homotopies or
  cone contraction below.

  The global comparison is the following obstruction criterion, not a
  consequence of the formal-Darboux theorem alone.  A Hamiltonian
  \(T_\Omega\)-action with fixed locus the Dirac support and nonresonant
  \((\epsilon_1,\epsilon_2)\) supplies the weight decomposition used by
  Theorem~\ref{thm:omega-localization-collapse}; it does not construct
  the all-arity centre lift, descent datum, all-order quantum master
  equation, anomaly/locality null-homotopies, or comparison-cone
  contraction.  Let
  \[
    \operatorname{Ob}^{\mathrm{global},5}
      :=
  ...
  \[
    ...
    \{C_{w,r}\},\qquad h_{\mathrm{anom}},\qquad h_{\mathrm{loc}}
  \]
  satisfying respectively the Maurer--Cartan lifting equation, the
  \v{C}ech--Weiss descent equation on \(\mathfrak U\), the scale-\(L\)
  Costello quantum master equation, the operadic anomaly equation, and
  the factorization-locality equation, then \(\mathrm{preOCA}_N\) is
  promoted to an open--closed morphism
  \[
    \mathrm{OCA}_N\colon
    \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}
    \longrightarrow
    ...
    (\mathcal C^{\mathrm{op}}_\partial)
  \]
  of \(\mathrm{SC}^{\mathrm{HT}}_{(1,2)}\)-algebras.  The QME primitive
  \(\{C_{w,r}\}\) is meaningful only after the gauge fixing, finite-scale
  BV propagator, BV Laplacian \(\Delta_L\), local interaction \(I[L]\),
  convergent coefficient Casimir, and admissible counterterm complex
  have been fixed.
  ...
  \[
    ...
    =
    \operatorname{id}_{\operatorname{Cone}(\mathrm{OCA}_N)}
  \]
  in the sense of Definition~\ref{defn:oca-comparison-cone}, or supplies
  the filtered associated-graded cone data of
  Theorem~\ref{thm:filtered-oca-cone-criterion}, then
  \(\mathrm{OCA}_N\) is a global open--closed quasi-isomorphism in
  \(\mathcal B^{\mathrm{adm}}\).  An equivalence of mixed-HT
  \(\mathrm{SC}\)-algebras requires the corresponding operadic
  ...
  The five native obstruction components are:
  \begin{itemize}
    \item
      $\operatorname{Ob}^{\mathrm{ch}\,E_2}_{\mathrm{Deligne}}$ is
	      ...
	      to a Morita chiral \(E_2\)-centre action on
	      \(Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
    ...
    \item
	      $\operatorname{Ob}^{\mathrm{desc}}_{\partial}=
	       [\delta_{\check C}]\in
	       \check H^2_{\mathrm{Weiss}}\bigl(
	       \mathfrak U;H^{-1}(\mathcal E^\bullet_{\mathrm{desc}})\bigr)$
	      is the triple-overlap Čech obstruction class in the
	      Čech--Weiss descent complex of
    ...
    \item
	      ...
	      differential is
	      \(d_{\mathrm{QME}}^{L}=Q+\{I[L],-\}_{L}+\hbar\Delta_L\), with
	      ...
	      coefficient Casimir of the chosen Costello local-functional model
		      ...
		      Lemma~\ref{lem:tate-casimir-obstruction}).  Thus this component is
		      a criterion unless the counterterm tower, propagator, and
		      completion data are constructed.  The counterterm primitive
		      \(\{C_{w,r}\}\) is an input solving this tower, not a
	    ...
	    \item
	      ...
	      projection is the Capelli class \(\hbar N[\bar c]\); a
	      determinant-line or modular-line interpretation requires the
	      corresponding line and connection.
	    \item
	      \(\operatorname{Ob}^{\mathrm{loc}}\in
	      H^1(\partial X,
	      \underline{\mathrm{Hom}}^0_{\mathcal B^{\mathrm{adm}}}(
	      \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}},
	      Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}
	      (\mathcal C^{\mathrm{op}}_\partial)))\) is the
...
\begin{proof}
  ...
  five displayed primitives are exactly the null-homotopies which solve
  the native chiral-operadic, descent, QME, anomaly, and locality
  obstruction equations; hence they promote the pre-comparison datum to
  an \(\mathrm{SC}^{\mathrm{HT}}_{(1,2)}\)-morphism.  The equation for
  \(\kappa_{\operatorname{Cone}}\), or the filtered cone criterion, is
  precisely the acyclicity condition on the comparison cone, and
  therefore gives a quasi-isomorphism in \(\mathcal B^{\mathrm{adm}}\).
  The formal-Darboux theorem enters only through the prescribed
  trace-sector fibre.  It constructs neither the chiral \(E_2\)
  centrality homotopies, nor the Weiss descent homotopy, nor the
  finite-scale QME counterterms, nor the anomaly/locality
  null-homotopies, nor the cone contraction.
\end{proof}

\begin{rmk}[Compact modular-line addendum]
The operator-level lift of the modular line bundle on the compact target
...
compact-target complex.  Its scalar curvature section is the Igusa cusp
form \(\Phi_{10}\), with Borcherds square-root branch
\(\Delta_5=\Phi_{10}^{1/2}\).
\end{rmk}
...
\begin{rmk}[Formal representatives and the global obstruction]
  ...
  the all-arity formal tree-kernel current operad of
  ...
  values whose extension, gluing, renormalisation, anomaly
	  trivialisation, locality homotopy, and comparison-cone acyclicity are
	  measured by \(\operatorname{Ob}^{\mathrm{global},5}\) and
...
\begin{thm}[Uniqueness of the stable formal trace coordinate \(J\)]\label{thm:Jf-coordinate-uniqueness}
  ...
  \(H^0(\mathcal R_N^{GL_N},Q)\) is the invariant degree-zero Koszul
  cohomology justified by
  ...
  For \(f\in\mathfrak h_{\leq W}\), choose any ordinary-word lift
  ...
  Among continuous linear maps, for the \(\mathfrak m\)-adic topology on
  \(\mathfrak h\) and the inverse-limit topology on \(A_\partial\),
  \[
    \widetilde J\colon\mathfrak h\longrightarrow A_\partial
  \]
  whose truncations \(\widetilde J_{\leq W}\colon
  \mathfrak h_{\leq W}\to A_{\partial,\leq W}^{\mathrm{tr,st}}\) satisfy:
  \begin{enumerate}[label=(U\arabic*)]
    \item (\emph{Polynomial \(GL_N\)-invariance, stable})
      \(\widetilde J(f)\) is a stable \(GL_N\)-invariant trace
      polynomial in \((\phi_1,\phi_2)\) in each total word degree
      \(d\), represented only for ranks \(N\geq d\), in the sense of
  ...
  that the whole Hochschild centre has been identified.
\end{thm}

\begin{proof}
  The argument is degreewise.  Fix \(W\) and work in
  \(\mathfrak h_{\leq W}\to A_{\partial,\leq W}^{\mathrm{tr,st}}\);
  ...
  \(\mathfrak h_{\leq W+1}\to\mathfrak h_{\leq W}\).  Continuity in the
...
\begin{proof}
  ...
  algebra is a bar-cobar hypothesis. The moment-map specialisation follows from
...
\begin{prob}[PBW-source \(P_0\)-centre action criterion and CE comparison datum]\label{prob:pbw-p0-center-action}
  ...
  requires additional quantized boundary data: in the source
  ...
  comparison is the extra datum required for a cochain-level map from
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  tower.  Thus the coefficient category must satisfy:
  \begin{enumerate}
...
on \(\mathfrak h_{\leq N}\) requires a Moyal-flat truncation
...
\(\mathfrak o_{\mathrm{QME}}\): Hadamard control gives the
...
\ref{defn:regulator-admissible-sector}.  Hadamard control gives the
...
\hbar\Delta I_{\mathrm{prot}}]\).  The vanishing condition for
...
The full compact-comparison theorem
...
theorems for compact Calabi--Yau threefold BCOV, the compact Calabi--Yau-to-chiral comparison,
...
obstruction vector]\label{thm:tate-cotangent-lift-iff}
  ...
  \begin{itemize}
    ...
    \item \emph{Vanishing criterion.}
      \(\Phi_{\mathrm{cl}}\) exists precisely when
      \(\operatorname{Ob}_T\) is represented and killed by chosen
      compatible null-homotopies.  The all-order one-antifield quantum
      extension additionally requires a null-homotopy of
      \(\mathfrak o_{\mathrm{QME}}\).  The compact or cross-target
      version additionally requires a null-homotopy of
      \(\mathfrak o_{\mathrm{cv}}\).
...
\begin{proof}
  ...
  \(\mathfrak o_{\mathrm{QME}}=0\) gives the all-order one-antifield
...
Feynman-integral proof has Costello graph weights as its input.

\subsubsection{Quantization and the brane trace map}\label{sssec:quantization-measurement-test}
...
d_{\mathrm{QME}}^L)\) class exists only after the scalar projection has
been lifted and the finite-window tower has been made compatible
...
\begin{cor}[Renormalized stable connected trace map]\label{cor:renormalized-stable-connected-map}
  ...
  block-restriction inverse-limit element in the unrenormalized normal-ordered trace
...
\begin{thm}[Reduced Moyal commutator: radial hypothesis and obstruction criterion]\label{thm:finite-n-reduced-moyal}
  ...
  \(\hbar N\) has already been recorded in the \(J_N\)-basis and then
...
\begin{proof}
  ...
  Hamiltonian reduction, and has the generator image normalization
	  recorded in Statement~\ref{stmt:app-radial-parts-hypotheses}.  The
	  ...
	  Lemma~\ref{lem:app-radial-nonlinear-shear-obstruction} explains why
		  ...
		  pure-power radial images give only the special cases. Therefore the
...
\(\hbar^7 P^7/322560\) of the previous subsection form the algebraic
...
\begin{lem}[Tate Casimir obstruction for the full Hamiltonian sector]\label{lem:tate-casimir-obstruction}
  ...
  Remark~\ref{rmk:linear-heat-versus-bv-kernel} gives a BV propagator,
...
\label{thm:scalar-qme-alternatives}
  \begin{itemize}
    ...
    \item \emph{First nonzero example:} on the linear pair
      ...
      commutator gives the Capelli term \(\hbar N\cdot\mathbf 1\)
      at order \(\hbar^1\).
  ...
  \[
    \hbar N[\bar c]\,\gamma_{\mathbf 1}
    \in H^2_{\mathrm{Lie}}(\bar A;\C\gamma_{\mathbf 1})[1][[\hbar]] .
  \]
...
trace representation; the all-order question is whether the same
central class persists at orders $\hbar^3,\hbar^5,\dots$.
...
\begin{thm}[Degree-zero Moyal target and decorated radial obstruction]\label{thm:phi-hbar-all-order}
  ...
  \begin{enumerate}
    ...
    \item The ordinary Weyl-trace image clause is equivalent, after the
      ...
      Proposition~\ref{prop:app-free-trace-diagram-K44},
      Proposition~\ref{prop:app-free-residual-vanishings}, and the
      exact-arithmetic normal-form calculation prove finite interior
...
\begin{thm}[Componentwise quantum coefficient comparison]\label{thm:componentwise-quantum-coefficient-comparison}
  ...
  \begin{itemize}
    ...
    \item \emph{First nonzero example outside the proved components:}
      ...
      linear generators by the projective scalar
      \(\hbar N\cdot\mathbf 1\); the
...
\begin{rmk}[Exact finite-window verification]\label{rmk:computer-verification}
  ...
  verified by exact rational arithmetic.  The
  ...
  Theorem~\ref{thm:phi-hbar-all-order}.  The same finite-window
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  ...
  obstruction triple
  \((o_{\sigma,w}^{(r)},[\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}],
  ...
  \[
    \mathfrak Q^\bullet_{w,\partial,\hbar}(I)
      ...
      d_{\mathrm{QME},w}^{L}=Q+\{I^w[L],-\}_{\hbar,L}
        +\hbar\Delta_L
      \right),
  \]
  ...
  \[
    \operatorname{Ob}_{w,\partial,\hbar}
      =
      d_{\mathrm{QME}}^{L}\kappa_{\hbar,w,I}
  ...
  \[
    \operatorname{Curv}(\kappa_{\hbar,w,I}+C_{\hbar,w})
      =
      d_{\mathrm{QME}}^{L}(\kappa_{\hbar,w,I}+C_{\hbar,w})
...
\begin{proof}
  ...
  current completion.  The target vector \((0,0,-1)^T\) forces a
...
\begin{proof}
  ...
  $\hbar P(f,g)$ and $\frac{\hbar^3}{24}P^3(f,g)$. The four primary cases
	  ...
	  \(\hbar^{11}\) give exact-arithmetic verification of this
...
coefficient requires a separate trivalent boundary contraction
...
\begin{thm}[Costello first/third normalization recognition criterion]\label{thm:first-third-costello-normalizations}
  ...
  \begin{enumerate}
    \item the connected one-propagator brane-defect ribbon graph carries
	      coefficient $\hbar(kn-lm)z_1^{k+m-1}z_2^{l+n-1}$ for monomial arguments
      $f=z_1^kz_2^l$, $g=z_1^mz_2^n$;
...
\begin{rmk}
  ...
  degree-zero rational invariant calculation used above.  The full
...
trace-sector CE/PV coordinate map at the formal-Darboux stalk of the
Dirac brane vacuum \(b_p\) (Theorem~\ref{thm:main-local}).  After the
stable scalar-reduced trace quotient and the admissible completion it
is the CE/PV isomorphism.  At fixed finite \(N\) it is a
Hochschild generator assignment; a cochain map requires the
centrality homotopies and is not the whole derived
\(E_1\)-centre.  Scalar reduction kills the constant Hamiltonian on
the closed side and retains a \(\mathrm{U}(1)\) centre-of-mass anomaly
on the open side: a single Lie \(2\)-cocycle
\([\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)\) classified in the
section below.

\section{The Capelli scalar}\label{sec:capelli-scalar}

The Capelli scalar \(\hbar N[\bar c]\) is the projective Lie anomaly of
the trace representation, realised by the Capelli central extension
$0\to\C\cdot K\to\widehat{\mathfrak g}_{\mathrm{cent}}\to\mathfrak g\to0$
of \eqref{eq:cent-extension}; the defining bracket value is
$\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$
(Remark~\ref{rmk:consistency-probes}, check~(2)).  A determinant-line
curvature interpretation is a further theorem after the determinant
line and its connection are constructed.
...
\begin{ex}[Linear Hamiltonians and the brane centre of mass]\label{ex:linear-hamiltonian-com}
  ...
  projective scalar at order \(\hbar\):
  \[
    [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_{\mathrm{proj}}
      =\hbar N\cdot\mathbf 1 .
  \]
...
\(\hbar N\cdot\mathbf 1\), exactly the order-\(\hbar\) image of
\(\omega(z_1,z_2)=1\) on the unreduced algebra.  The classical class \([\bar c]\) and its order-\(\hbar\)
...
\begin{thm}[Moyal survival of $\bar c$: the order-$\hbar$ Capelli shift]\label{thm:quantum-classical-anomaly}
  ...
  modulo the quantum moment-map ideal.  This is a matrix Weyl-algebra
  congruence in the projective Hamiltonian reduction, not an equality
  in a finite-dimensional matrix representation.  On the linear trace generators
...
\begin{thm}[Capelli scalar and determinant-line condition]
\label{thm:capelli-equals-modular-curvature}\label{thm:capelli-omega-central-identification}
  Let \(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\), with the
  completed formal meaning for \(f\in\mathfrak h\), be the trace
  ...
  \[
    \kappa_{\mathrm{Cap}}(J)=\hbar N[\bar c]
    \quad\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]],
  \]
  where \([\bar c]\) is the Lie 2-cocycle of
  Lemma~\ref{lem:omega-cocycle}.  If a determinant line
  \(\det^{1/2}_{J}\) and a connection \(\nabla_J\) are constructed for
  this projective trace representation, and if their Atiyah class pulls
  back to \([\bar c]\), then the curvature of \(\nabla_J\) restricts to
  the same class \(\hbar N[\bar c]\).  The determinant-line construction
  supplies the line and connection; the local theorem supplies the Lie
  cocycle.
\end{thm}
...
\begin{proof}
  ...
  representatives of \([\bar c]\) coincide under the canonical scalar
  projection, with the multiplier \(\hbar N\) coming from the open
  Chan--Paton trace pairing.  The brane \(E_1\)-algebra
  \(A^{\mathrm{cl}}_{\partial,N}=
  ...
  under this identification the projective Capelli normal-ordered shift
  ...
  \(K\mapsto N\).  The linear Hamiltonian generators \(z_1,z_2\) act on
  the trace representation with classical Capelli identity
  \cite{capelli}\cite{howe-capelli}; its order-\(\hbar\) deformation is
  the central extension with cocycle \(\hbar N\omega\), where
  \(\omega(z_1,z_2)=1\) by
  Corollary~\ref{cor:app-sign-universal-dual-normalization}.  Hence the
  Lie cohomology class is \(\hbar N[\bar c]\).  A determinant-line
  curvature formula is a statement about the determinant line and
  connection named in the theorem.
\end{proof}
...
\begin{rmk}[Determinant line and scheme dependence]\label{rmk:determinant-line-scheme}
  ...
  \(\det_J^{1/2}\), a connection on it, and a pullback of its Atiyah
  class to \(H^2_{\mathrm{Lie}}(\bar A,\C)\) are additional data.  When
  those data exist and the pullback is \([\bar c]\), the Capelli
  shift \(\hbar N\,I\) is the curvature value at the linear trace
  generators.  The numerical
...
The Capelli scalar \(\hbar N[\bar c]\) is the projective Lie anomaly
evaluated at the trace generator
(Theorem~\ref{thm:capelli-equals-modular-curvature}).  It lives in
...
listed range and without those hypotheses,
\(\Omega^{\mathrm{rad}}_{a,b}\) is the radial obstruction class in its
native decorated PBW--Stokes bicomplex.  A counterterm interpretation requires a target
deformation complex and comparison map.  The
...
\(\widehat{\mathcal P}_\Pi=\varprojlim_N\mathcal P_{\le N}\) records the
...
The obstruction classes of \S\ref{ssec:tate-residual-resolution} use
the pro-Matlis target
\[
  \widehat{\mathcal P}_\Pi
...
pro-Matlis envelope used for the \(N\)-tower coefficient comparison.
...
pro-Matlis replacement theorem
(Theorem~\ref{thm:promatlis-universal-property}) selects
\(\widehat{\mathcal P}_\Pi\) as the completed receiver for the fixed
finite-window moment maps.  The full \(\mathfrak h\)-coadjoint action
...
\]
The first object is the stable trace shadow of the second, not its
definition.  The duality with the bulk CE complex lives in the
pro-Matlis target.  At each finite Hamiltonian window the truncated
cotangent CE/PV coordinate identification holds at finite presentation
...
of the finite-window dualities.  The discarded strict all-window
...
scalar \(\hbar N[\bar c]\) sit at the trace
...
determine the local obstruction complex.  The trace-sector datum
\(A^{\mathrm{cl}}_{\partial,N}=
C^\bullet_{\mathrm{CE}}(\mathfrak{gl}_N,\mathrm{Kosz}([\phi_1,\phi_2]))\),
\(J(f)=\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}\), with formal
completion understood,
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\), is evaluated at the brane
...
\input{typed-boundary-open-closed}

\section{Examples}\label{sec:examples}\label{ssec:examples}

At each closed point \(p\in\C^2\), the formal-Darboux theorem identifies
only the trace-sector map
  \[
    C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g)
  \dashrightarrow_{\Phi_N^{\mathrm{gen}}}
  C^\bullet_{\mathrm{Hoch}}\bigl(A^{\mathrm{cl}}_{\partial,N},
                  A^{\mathrm{cl}}_{\partial,N}\bigr),\qquad
  c_f\mapsto\theta_f,\quad
  u_f\mapsto
  \overline{\operatorname{Tr}\,f(\phi_1,\phi_2)} .
\]
After stable scalar reduction and admissible completion this is the
CE/PV coordinate isomorphism.  The examples below compute \(J\) at the
smallest brane multiplicities and show where finite-rank trace
relations first break the stable trace identification.  Global realisations on
Stein or compact geometries are governed by the descent and QME
obstruction classes of Theorem~\ref{thm:main-global-deligne-criterion}.
Vertex-algebra
...
$c_f\mapsto\theta_f$, $u_f\mapsto J(f)$.  The formal Weyl algebra of
one pair gives
$[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar$, the one-pair
Heisenberg relation; the Capelli scalar
$\hbar N[\bar c]\big|_{N=1}=\hbar[\bar c]$ is retained by the
projective central extension; the ordinary scalar commutator is lost
only in the unprojectivized scalar-reduced shadow.
...
\[
  ...
  QX=QY=0,\qquad Q\psi=[X,Y].
\]
The invariant degree-zero Koszul cohomology
\[
  H^0(\mc R_2^{GL_2},Q)
  =
  H^0(\mc R_2,Q)^{GL_2}
...
\[
  J_2(z_1)=\overline{\operatorname{Tr}X},\quad
  J_2(z_2)=\overline{\operatorname{Tr}Y},\quad
  J_2(z_1^2)=\overline{\operatorname{Tr}X^2},\quad
  J_2(z_1z_2)=\overline{\operatorname{Tr}XY},\quad
  J_2(z_2^2)=\overline{\operatorname{Tr}Y^2}.
\]
...
\[
  ...
  +\operatorname{Tr}(X)^3=0.
\]
Thus the rank-two connected primitive class represented by
\(\operatorname{Tr}(X^3)\) is killed after quotienting decomposable
products.  The finite-rank class \(J_2(z_1^3)\) exists, but it is not a
free stable trace generator.  This is the finite-\(N\) leftover
excluded from Theorem~\ref{thm:Jf-coordinate-uniqueness} by the
condition \(N\geq d\).
...
\[
  [\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_{\mathrm{proj}}
  =
  2\hbar\cdot\mathbf 1,
\]
the \(N=2\) instance of the Capelli class
\(\hbar N[\bar c]\).  It is a projective scalar at the trace
generator, not a determinant-line curvature until the determinant-line
datum of Theorem~\ref{thm:capelli-equals-modular-curvature} is supplied.
...
Procesi--Razmyslov forcing makes $J$ the unique
$GL_N$-equivariant primitive single-trace Darboux-natural map.  The
...
$\hbar N[\bar c]\in H^2_{\mathrm{Lie}}(\bar A,\C)[[\hbar]]$
(Theorem~\ref{thm:capelli-equals-modular-curvature}) is the projective
...
\[
  Z^{\mathrm{der},\mathrm{Mor}}_{E_2^{\mathrm{HT}}}\!
    (\mathcal{C}^{\mathrm{op}}_\partial).
\]
The existence of the global comparison is the obstruction problem of
Theorem~\ref{thm:main-global-deligne-criterion}.  By the
formal-Darboux restriction theorem
(Theorem~\ref{thm:cross-target-restriction}), the chiral \(E_d\) Deligne
conjecture at \(d\geq 2\), the Hall--Drinfeld--Borcherds filtered
comparison complex on compact non-toric Calabi--Yau threefold, the operator-level
lift of the modular line on \(K3\times E\), and the
\(W_\infty[\lambda]/E_\infty\) admissible endpoint identification are
distinct theorems.  The three remaining conjectures have the following
mathematical form:
the chiral \(E_2\)-Deligne conjecture contains
...
gives the arity-two \(d=2\) formal-coordinate operation as the
diagonal kernel
...
\begin{conj}[Hall--Drinfeld--Borcherds filtered comparison complex on
  ...
  \]
	  whose associated graded contains the scalar shadows defined by the
	  chosen compact-target datum.  When \(X\) carries a specified
	  \(K3\)-Mukai component, as \(K3\times E\) does, these shadows are
	  \[
  ...
  These shadows become entries of a spectral-sequence page only after
  the filtered complex, bidegrees, differential, and abutment have been
  constructed.
\end{conj}
On \(X=K3\times E\) the scalar shadows predicted by the matched
...
\begin{conj}[Operator-level lift of $\Phi_{10}$]\label{conj:operator-modular-lift}
  ...
  trace on \(\mathcal C^{\mathrm{op}}_\partial\).  The level-\(2\)
  projective modular class
  \(\Omega_{\mathrm{central}}\bigl|_{\overline{\mathcal A}_2}\) lifts to an
...
are inverse scalar shadows.  The protected trace of the modular curvature
cocycle gives \(\Phi_{10}\); the unnormalised protected scalar BPS
partition function, when the compact operator object exists, gives
\(\Phi_{10}^{-1}\), while the OP-normalised scalar is
\(-4096\,\Phi_{10}^{-1}\).  Thus an identity for
\(Z^{K3\times E}_{\mathrm{BPS}}\) has operator meaning only after its
normalisation convention and operator-valued cocycle have been fixed.
...
controlled by Theorem~\ref{thm:main-local}; the formal-Darboux theorem
does not construct the compact-target operator complex.  The
formal-Darboux restriction theorem
(Theorem~\ref{thm:cross-target-restriction})
...
Darboux coordinate of the trace-sector generator boundary value at one
Dirac brane vacuum; for \(f\in\mathfrak h\) it is read in the completed
brane algebra.  On the stable scalar-reduced trace algebra the
...
comparison asks for the full derived-centre quasi-isomorphism with the
bulk \(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\), whose
...
restriction.  The global mixed-HT comparison at \(d=2\) is controlled by
\(\mathfrak{Ob}_{\mathrm{Del}}^{\mathrm{glob}}\).  The operator-level
lift of the modular line on \(K3\times E\) and the chiral \(E_d\)
Deligne theorem at \(d\geq2\) are separate comparison and operadic
problems.
...
and Harish-Chandra radial-parts theorem give the formal-stalk-chart side;
Matlis 1958 and Hartshorne local cohomology give the principal-part
dual; Kontsevich's deformation quantisation gives the all-order
Moyal target.
...
\input{appendix-completion-formal-local-theorem}
