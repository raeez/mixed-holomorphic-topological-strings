...
\end{titlingpage}
  ...
  \medskip\noindent
  The classical BV pair is the field $\mathcal A$ valued in
  $\Omega^\bullet(\R^2)\widehat\otimes\Omega^{0,\bullet}(\C^2)
  \widehat\otimes\mathfrak h$ and antifield $\mathcal B$ valued in
  $\Omega^\bullet(\R^2)\widehat\otimes\Omega^{0,\bullet}(\C^2)
  \widehat\otimes\mathfrak h^\vee_{\mathrm{cont}}[1]$, paired by the
  $(-1)$-shifted symplectic form
  $\int_X\langle\mathcal B,\mathcal A\rangle$ obtained from the
  residue pairing on
  $\mathfrak h\otimes\mathfrak h^\vee_{\mathrm{cont}}$ and integration
  of mixed-HT forms against $dz_1\,dz_2$.  The BV differential is
  $Q=d+\bar\partial+d_{\mathrm{CE}}$, with $d_{\mathrm{CE}}$ the
  Chevalley--Eilenberg differential of $\mathfrak g$; the master
  equation $\{S,S\}=0$ holds because the BF interaction
  $I=\tfrac12\int_X\langle\mathcal B,
  [\mathcal A,\mathcal A]_{\mathfrak h}\rangle$ encodes the Lie
  bracket on $\mathfrak h$.  Renormalisation proceeds in the
  heat-kernel scheme of Costello--Gwilliam: a gauge-fixing operator
  $Q^{\mathrm{GF}}=d^*+\bar\partial^*$ for chosen flat and Hermitian
  metrics gives the Laplacian $D=[Q,Q^{\mathrm{GF}}]$, heat kernel
  $K_t=e^{-tD}$, and propagator
  $P_{\varepsilon,L}=\int_\varepsilon^L
  \tfrac12(Q^{\mathrm{GF}}\!\otimes\!1+1\!\otimes\!Q^{\mathrm{GF}})\,
  K_t\,dt$.  At RG scale $L$ the effective interaction is the
  Costello transform $I[L]=W(P_{\varepsilon,L},I)$, and the quantum
  master equation
  \[
    QI[L]+\tfrac12\{I[L],I[L]\}_L+\hbar\Delta_L\,I[L]=0
  \]
  is the consistency condition for renormalised quantisation.
  Costello deformation cohomology of $I[L]$ is the obstruction
  theory governing the closed-to-open coupling $\Phi_N$
  (Theorem~\ref{thm:closed-to-open-derived-centre-map}); its
  failure in the reduced non-scalar sector is the class
  ...
  \medskip\noindent
  The moment-map equation $[\phi_1,\phi_2]=0$ defines a singular
  variety on $\mathfrak{gl}_N\oplus\mathfrak{gl}_N$, since the
  entries of $[\phi_1,\phi_2]$ are not a regular sequence for
  $N\ge 2$.  The Koszul resolution
  $\C[\phi_1,\phi_2,\psi]$ with $Q\psi=[\phi_1,\phi_2]$ produces a
  derived enhancement: the ghost-zero brane moduli is the derived
  commuting-pair stack $[\mu^{-1}_{\mathrm{der}}(0)/GL_N]$.  The
  classical Hilbert scheme $\Hilb^N(\C^2)$ is recovered on the
  cyclic framed stable branch obtained by adjoining a framing
  vector $v\in\C^N$ with $\C[\phi_1,\phi_2]\cdot v=\C^N$; the
  Moyal--Weyl deformation on that branch produces the
  Nekrasov--Schwarz noncommutative Hilbert scheme
  \cite{nekrasov-schwarz-noncommutative}.

  \medskip\noindent
  Uniqueness of $J$.  Procesi--Razmyslov \cites{procesi,razmyslov}
  identifies the polynomial $GL_N$-invariants on
  $\mathfrak{gl}_N\oplus\mathfrak{gl}_N$ with the algebra generated
  by mixed traces of words in $\phi_1,\phi_2$;
  Loday--Quillen--Tsygan \cite{loday-quillen-tsygan} identifies the
  primitive part with the cyclic-word trace classes.  Combined with
  Dirac descent through the moment-map ideal, formal-Darboux
  naturality, and the linear normalisation
  $J(z_i)=\overline{\operatorname{Tr}\phi_i}$, $J$ is forced
  monomial-by-monomial (Theorem~\ref{thm:Jf-coordinate-uniqueness});
  among admissible protected Hamiltonian sectors at a
  holomorphic-symplectic brane point, $J$ is the terminal datum
...
\begin{proof}
  ...
  and the reverse forces \(c_{0,k}=1\), hence every \(c_{a,b}=1\).
