...
% --- 2026-05-17 phantom-label sweep: pre-existing undefined references ---
\phantomsection\label{ssec:open-operator-modular-lift}% % frontier subsection: pre-existing reference target
...
\end{titlingpage}
  ...
  Bershadsky--Cecotti--Ooguri--Vafa \cite{bcov} is the topological
  B-twist of the $\mathcal N=(2,2)$ worldsheet sigma model on a
  Calabi--Yau target; its Costello--Li BV renormalisation
  \cites{costello-renormalization}{costello-li-quantum-bcov}{costello-li-open-closed-bcov}
  admits a further holomorphic-topological twist localising it on
  $X=\R^2_{t,s}\times\widehat{\C^2}_{0,\mathrm{hol}}$ in the Costello
  twisted-supergravity sense \cite{costello-twistedM} --- topological
  in the worldvolume coordinates $(t,s)$ and holomorphic in the
  transverse coordinates $(z_1,z_2)$ --- with the closed-string
  sector becoming the holomorphic-topological Hamiltonian BF theory
  below.  The closed-string fields
  ...
  brane-link boundary $\partial X$ of the Dirac brane defect
  $L=\R_t\times\{b\}\subset X$ --- the two-complex-dimensional extension
  of the $d=1$ chiral Deligne theorem for chiral algebras on algebraic
  curves \cite{beilinson-drinfeld-chiral}*{\S 4}.  This manuscript proves
  ...
  reduced non-scalar obstruction is the class
  ...
  (Theorem~\ref{thm:bmk-finite-window-obstruction}), with coherent action
  ...
  \(\Phi_N\).  Minimal-model classification and an exact count of
  nonzero higher brackets are separate theorems.
  ...
  Calabi--Yau category $\mathcal C$ of the holomorphic-symplectic
  target.  Conventions match a global theorem through the
  matched-conventions acceptance class $\operatorname{Ob}_{\mathrm{UKD}}$
  of Theorem~\ref{thm:matched-conventions-transfer}: a nonzero
  coordinate of $\operatorname{Ob}_{\mathrm{UKD}}$ is the obstruction
  to transporting the local theorem to a compact-target programme
  (BCOV/Kodaira--Spencer, CY-to-chiral, Igusa/BKM, GW/DT/PT/MNOP, OSV).
...
\S\ref{ssec:cross-target-firewall}).
...
Theorem~\ref{thm:cross-target-firewall} and the matched-conventions
...
$g_s$ belongs to the ambient genus expansion.  The local theorem uses
$\hbar$.  Compatibility with the BCOV holomorphic-anomaly equation is
...
the negative-cyclic class is the Calabi--Yau pairing on the target
category.  Drinfeld centre \(Z(\mathcal C)\), derived centre
...
bulk is the derived chiral centre.  For a chiral algebra $A$ on a
...
is a formal coordinate ring.  The symbol
\(C^\bullet_{\mathrm{ch}}(A_b,A_b)\) is meaningful after \(A_b\) is
equipped with the chiral structure restricted from
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\).
...
The boundary open category on the Costello--Gwilliam brane-link
boundary $\partial X$ of the Dirac brane defect $L\subset X$ is
$\mathcal C^{\mathrm{op}}_\partial=
\Phi^{\mathrm{FA}}_d(\mathcal A)\big|_{\partial X}$,
the bulk--boundary restriction of the bulk factorisation algebra to the
link sphere bundle over $L$ in the sense of Costello--Gwilliam
\cite{costello-gwilliam}*{Vol.~II}.  Factorisation-homology
...
\item BMK lane: the one-pair analytic pro-Matlis retract of
  ...
  replacement for strict all-window support-local current transfer;
  the obstruction on every fixed finite Matlis window is
  \(\operatorname{Ob}^\Pi_{\mathrm{BM}}\)
  (Theorem~\ref{thm:bmk-finite-window-obstruction}), and the pro-tower
...
\paragraph{Convention firewall.}  Transport of the local theorem
to a compact-target programme (BCOV / Kodaira--Spencer; CY-to-chiral;
Igusa / BKM; GW / DT / PT / MNOP; OSV; mixed-Dunn / six-relation;
global Hamiltonian descent on a compact holomorphic-symplectic surface)
is the matched-conventions theorem
...
\subsection*{Reader's compass: what is proved, what fails strictly, what is conditional}
\label{ssec:proof-boundary-table}
\addcontentsline{toc}{subsection}{Reader's compass: what is proved, what fails strictly, what is conditional}

The local theorem of this manuscript replaces strict-product
realisations of the open--closed dictionary with the admissible
filtered nuclear Tate / pro-Matlis / Capelli-projective derived-centre
category \(\mathcal B^{\mathrm{adm}}\).  The table below records the
boundary between the proved formal-Darboux stalk theorem, the strict
obstructions it must traverse, and the conditional global identification
on the Costello--Gwilliam brane-link boundary \(\partial X\).

\begin{center}
\small
\begin{tabular}{@{}p{0.30\textwidth}p{0.18\textwidth}p{0.18\textwidth}p{0.24\textwidth}@{}}
\hline
\textbf{Claim} & \textbf{Status} & \textbf{Required data} & \textbf{Reference}\\
\hline
Coordinate CE/PV dictionary
\(c_f\mapsto\theta_f\), \(u_f\mapsto J(f)\)
&
proved
&
finite coordinate window
&
Theorem~\ref{thm:reduced-ce-pv-central-operation}\\
Strict \(P_0\)-bracket lift of CE/PV in the ambient product
&
fails: degree-lowering bracket misses Jacobi
&
---
&
Lemma~\ref{lem:gauge-paired-necklace-naive-quotient-obstruction},
Theorem~\ref{thm:gauge-brst-cyclic-obstruction}\\
Diagonal Casimir and Maurer--Cartan kernel
&
divergent strict; convergent in \(\mathcal B^{\mathrm{adm}}\)
&
weighted-Tate completion
&
Theorem~\ref{thm:wt-canonical-habitat}\\
Admissible \(P_0\)-bracket lift on \(B\subset\mathrm{PV}_{\mathrm{coord}}\)
&
proved in \(\mathcal B^{\mathrm{adm}}\)
&
bracket-admissible \(B\)
&
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}\\
Bar--cobar / open \(A_\infty\)-Koszul upgrade
&
conditional on acceptance datum, \(\varprojlim^1=0\)
&
filtered cobar acceptance
&
Lemma~\ref{lem:filtered-cobar-qiso-criterion}\\
Local formal-Darboux stalk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\simeq
Z^{\mathrm{der,adm}}_{E_1}(A^{\mathrm{cl}}_{\partial,N})\)
&
proved in \(\mathcal B^{\mathrm{adm}}\)
&
full admissible category
&
Theorem~\ref{thm:main-local}\\
Capelli scalar \(=\) projective curvature
...
Global OCA identification on \(\partial X\)
&
conditional
&
\(\operatorname{Ob}^{\mathrm{global}}=0\)
(chiral \(E_2\)-Deligne; descent; all-loop QME)
&
Theorem~\ref{thm:main-global-conditional}\\
Operator lift of the modular line on \(K3\times E\);
Hall--Drinfeld--Borcherds spectral sequence on compact non-toric CY\(_3\)
&
open frontier
&
separate chain-level construction;
\(\operatorname{Ob}_{\mathrm{UKD}}=0\)
&
\S\ref{sec:frontier}\\
\hline
\end{tabular}
...
The strict-product obstructions of row two are not pathologies to be
dismissed but the structural reason the admissible completion is the
theorem.  Remark~\ref{rmk:strict-failure-admissible-spine} crystallises
the three obstruction mechanisms (degree-lowering bracket; divergent
Casimir; \(\varprojlim^1\) defect); the local theorem absorbs all three
simultaneously inside \(\mathcal B^{\mathrm{adm}}\).
...
holomorphic-symplectic brane point, a closed sector that coincides with
the chiral Hochschild cohomology of the boundary algebra; this
...
\(\mathcal{C}^{\mathrm{op}}_\partial=
\Phi^{\mathrm{FA}}_d(\mathcal A)|_{\partial X}\) on the
Costello--Gwilliam brane-link boundary \(\partial X\) of the Dirac brane
defect \(L_p=\R_t\times\{p\}\subset X=\R^2_{\mathrm{top}}\times\C^2_{\mathrm{hol}}\)
(link sphere bundle over the worldline) whose endomorphism algebra is
...
\label{exa:two-branes-generic}
  ...
  \(J(z_1z_2)=a_1b_1+a_2b_2\) are the two-brane fingerprints of the
  ...
  \(\mathfrak C_2=\{(\phi_1,\phi_2)\in\mathfrak{gl}_2^2
    :[\phi_1,\phi_2]=0\}\) by Procesi--Razmyslov
  \cites{procesi}{razmyslov}; the dictionary
  \(u_{p,q}\mapsto J(z_1^pz_2^q)\) identifies the boundary observables
  with the single-trace generators.  Outside the generic stratum the
  commuting variety has the well-known non-reduced strata at coincident
  eigenvalue pairs; the trace map descends to a single-valued
...
\begin{rmk}[Conceptual spine: strict realization fails; the admissible completion is the theorem]
\label{rmk:strict-failure-admissible-spine}
...
\(z_i\in H_1\) does not preserve any finite Taylor truncation
\(\mathfrak h_{\leq N}\), so the naive projected bracket
...
Theorem~\ref{thm:main-local}) resolves these three obstructions
\emph{simultaneously} by replacing the strict ambient product with the
admissible filtered nuclear Tate / pro-Matlis / Capelli-projective
derived-centre category \(\mathcal B^{\mathrm{adm}}\): the
weighted-Tate completion of
Theorem~\ref{thm:wt-canonical-habitat} makes the Casimir converge,
the pro-Matlis completion of
Theorem~\ref{thm:principal-part-coadjoint-uniqueness} stores the
principal-part module without a degree-lowering pathology, and the
Capelli-projective central extension
\(\widehat{\mathfrak g}_{\mathrm{cent}}\) of
\S\ref{sec:capelli-scalar} absorbs the finite-\(N\) scalar of
Example~\ref{exa:linear-hamiltonians-anomaly} as the curvature
\(\Omega_{\mathrm{central}}|_{J(f)}\) of the modular line.  Inside
\(\mathcal B^{\mathrm{adm}}\) the formal-Darboux stalk identification
is unconditional; the strict-product version is not the theorem, and
the admissible completion is not a technical convenience but the
content of the dictionary at \(N\) Dirac branes.
\end{rmk}
...
constraining how local operators vary across the chosen spacetime
  ...
  global theories of worldsheet moduli or amplitudes require additional
  target data.
...
\label{def:local-topological-string-sector}
  ...
  \mathrm{Obs}^{\mathrm{cl}}(V)\) is the operator product on disjoint
...
topological direction supplies the propagator and the bulk-to-brane
...
holomorphic-symplectic normal disk: two complex coordinates supply
...
\subsubsection{The closed--open factorization triple: \texorpdfstring{\(E_2\)}{E2} bulk, \texorpdfstring{\(E_1\)}{E1} brane, derived-centre map}\label{sssec:mixed-ht-factorization-spine}
...
\begin{cor}[$E_2$-bulk algebra]\label{cor:E2-bulk-algebra}
  ...
  \[
    \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}(B)
      =\mathcal F^{\mathrm{cl}}_{\mathrm{Ham}}(D^2\times B)
  \]
  is an \(E_2\)-algebra under the Lurie--Costello--Gwilliam
...
\begin{rmk}[Native bulk object on $\C^2$]\label{rmk:native-bulk-object-c2}
...
\begin{prop}[Bochner--Martinelli current data and finite-window obstruction for $E_2$ transfer]\label{prop:finite-window-bm-native-e2-transfer}
  ...
  the coordinate window; the full Hamiltonian Lie bracket requires
  ...
  A finite-window homotopy \(H_{\chi,N}\) on the full
  compact-support/current complex additionally requires the identity
  \[
  ...
  Consequently strict finite support-local current quotient retaining
  ...
  obstructed.  The projected finite-window \(L_\infty\) escape repair
  is obstructed as well: the forced return block already has
  \[
  ...
  coefficient comparison induced by \(p_N\) and \(i_N\).  A
  finite-window homotopy would be an additional construction.  It agrees,
...
\begin{proof}
  ...
  current complex.  It would make every closed top current whose analytic
...
\begin{rmk}[Cross-target anchor: HT-import licensing]
  ...
  holomorphic-topological theory.  It is the named hypothesis
  $\mathsf{Hyp}_{\mathrm{HT,glob,DR}}$ carrying the four-step license
  (descent $+$ QME $+$ anomaly $+$ locality) required to pass from the
  formal stalk model to a compact global HT theory on $X$.
\end{rmk}
...
linear normalization on \(z_1,z_2\) forces the scalar correction to
vanish at every monomial \(z_1^a z_2^b\): the naturality lemma gives
...
follows by linearity and \(\mathfrak m\)-adic continuity.
...
Theorem~\ref{thm:bmk-finite-window-obstruction} excluding one-pair
...
\begin{rmk}[\(c_f\) versus \(u_f\)]\label{rmk:type-discipline-cf-uf}
  ...
  on observables, and perfection of the Hamiltonian Lie algebra kills
  closed length-one CE cochains in the Hamiltonian direction, so
...
\begin{prop}[Locality forced by the three differentials]
  ...
  centrality homotopies require additional data.  The trace
...
\begin{thm}[BV replacement for unitarity in the protected sector]
  ...
  evolution operator are additional structures supplied by a parent
  theory. If the sector is obtained as \(Q\)-cohomology of a unitary
...
\begin{proof}
  ...
  physical theory supplies a Hilbert space, a positive form, and a
...
constructed in this section is the closed-side input of
Theorem~\ref{thm:main-local}; its Capelli-projective central extension
$\widehat{\mathfrak g}_{\mathrm{cent}}$ enters as the bulk
Chevalley--Eilenberg target.
...
\begin{thm}[Defect momentum is the principal-part module]\label{thm:hbf-cotangent-uniqueness-summary}
  ...
  Equivalently, this is the unique continuous total-degree-zero
  \(\mathfrak h\)-module structure on a continuous dual of
  \(\widehat{\mathfrak h}\) compatible with the residue pairing on the
  formal polydisk.
\end{thm}
...
Lemma~\ref{lem:gauge-paired-necklace-naive-quotient-obstruction}. The
...
\begin{rmk}[Casimir/Tate obstruction to a strict cotangent kernel]\label{rmk:closed-bv-casimir-obstruction}
  ...
  kernel requires repair to a kernel-admissible model through the
...
\begin{rmk}[Named obstruction to a renormalized BF theory]\label{rmk:bf-renormalization-obstruction}
  ...
  Before (a) and (b) are supplied, \eqref{eq:bf-action} remains a
...
The Dirac brane stack constructed here is the open $E_1$-algebra
$A^{\mathrm{cl}}_{\partial,N}$ of Theorem~\ref{thm:main-local}; its
$\mathfrak{gl}_N$-equivariant centre exhaustion is
Theorem~\ref{thm:protected-summand-exhausts}.

\subsection{The Dirac brane stack}\label{ssec:dirac-brane-stack}

The primitive open object is the open category
\(\mathcal C^{\mathrm{op}}_\partial\) on the Costello--Gwilliam
brane-link boundary \(\partial X\); a chosen vacuum \(b\) gives the
chart \(A_b=\End(b)\), and at the stack of \(N\) Dirac branes
\(A_b\) is the matrix endomorphism algebra of the corresponding
boundary state.  The manuscript works in the chart determined by this
primitive open object.
...
is the derived zero fibre of the moment map.  Its
...
\paragraph{\texorpdfstring{Word-ordering independence of \(J\).}{Word-ordering independence of J}}
\(J(f)\) depends only on the symbol \(f\).  Adjacent letters in any trace word commute up to a
\(Q\)-exact correction,
\[
...
open trace cohomology generates the open sector as a trace algebra.
...
\begin{lem}[Cyclic framing branch is the Hilbert scheme;
Nekrasov--Schwarz noncommutative deformation]\label{lem:cyclic-framing-hilbert-branch}\label{rmk:cyclic-framing-hilbert-branch}
  Let
  \[
    X^{\mathrm{cyc}}_N
  ...
  points on \(\C^2\) \cite{nakajima-hilbert-lectures}.  The derived
  fibre of \([\mu^{-1}_{\mathrm{der}}(0)/GL_N]\) over this locus is
  underived: the Koszul antifield \(\psi\in\mathfrak{gl}_N[1]\) of the
  Dirac brane stack is contractible there.  Turning on the
  Fayet--Iliopoulos parameter \(\zeta=\hbar\cdot I\) deforms the
  moment-map fibre
  \(\mu^{-1}(0)\rightsquigarrow\mu^{-1}(\hbar\cdot I)\) and gives the
  Nekrasov--Schwarz noncommutative Hilbert scheme
  \cite{nekrasov-schwarz-noncommutative}
  \[
    \Hilb^N(\C^2)_\hbar
     =\bigl[\mu^{-1}(\hbar\cdot I)\cap X^{\mathrm{cyc}}_N\bigr]/GL_N;
  \]
  equivalently, the Moyal--Weyl deformation of the brane Poisson
  algebra \(\C[\phi_1,\phi_2]\) replaces \([\phi_1,\phi_2]=0\) by
  \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar N\cdot I\)
  (Theorem~\ref{thm:quantum-classical-anomaly}), so the Capelli
  scalar \(\hbar N[\bar c]\) of
  Example~\ref{exa:linear-hamiltonians-anomaly} is exactly the
  \(U(1)\) centre-of-mass charge of the deformation.
\end{lem}
...
\begin{proof}
  ...
  \(g\) fixes a spanning set of \(\C^N\), forcing \(g=I\)
  ...
  On \(X^{\mathrm{cyc}}_N\) the moment-map differential
  \(d\mu\colon\mathfrak{gl}_N^{\oplus 2}\to\mathfrak{gl}_N\),
  \((\delta\phi_1,\delta\phi_2)\mapsto
  [\delta\phi_1,\phi_2]+[\phi_1,\delta\phi_2]\), is surjective: its
  cokernel is the cyclic stabiliser of \((\phi_1,\phi_2)\), which is
  trivial by freeness.  The Koszul resolution of
  Definition~\ref{def:formal-darboux-stalk-chart}, with antifield
  \(\psi\in\mathfrak{gl}_N[1]\) and differential
  \(Q\psi=[\phi_1,\phi_2]\), has zero homology in positive degree on
  this locus by surjectivity of \(d\mu\), so the derived fibre is
  underived there.

  Fayet--Iliopoulos deformation.  Replacing the moment-map equation
  \([\phi_1,\phi_2]=0\) by \([\phi_1,\phi_2]=\hbar\cdot I\) requires
  trace-anomaly cancellation: taking the trace,
  \(\operatorname{Tr}[\phi_1,\phi_2]=0\) classically, but
  \(\operatorname{Tr}(\hbar\cdot I)=\hbar N\), so the deformation
  exists only after centre-of-mass twist by the \(U(1)\) charge
  \(\hbar N\).  This is Nekrasov--Schwarz~\cite{nekrasov-schwarz-noncommutative}*{Theorem~1}.

  Moyal--Weyl side.  Theorem~\ref{thm:quantum-classical-anomaly}
  computes the Moyal commutator of Weyl-symmetric quantisations
  $\Phi_\hbar(z_1)=\phi_1$, $\Phi_\hbar(z_2)=\phi_2$ on the
  Chan--Paton pair: \([\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star
  =\hbar N\cdot I\).  This is exactly the FI deformation of the
  moment map.  The class
  \([\bar c]=[\omega(z_1,z_2)\,c_1\wedge c_2]\in H^2_{\mathrm{Lie}}
  (\bar A,\C)[[\hbar]]\) of Theorem~\ref{thm:u1-center-anomaly} is the
  pull-back of the Atiyah class of the trace representation
  \(\bar A\to\mathfrak{gl}_N\) (Theorem~\ref{thm:capelli-equals-modular-curvature});
  multiplied by \(\hbar N\), it is the centre-of-mass \(U(1)\) charge
  of the deformed Hilbert scheme.
\end{proof}

\(J\) is exact at the level of the scalar trace algebra.  The full
\(P_0\)-factorization centre requires additional cotangent centrality
...
which is a conditional input beyond the stable hypotheses.
...
single-cycle class; same-rank deletion has a nonzero commutator
...
Identifying the raw finite-\(N\) operator algebras with the strict
formal Hamiltonian envelope requires the open-trace
...
The acceptance datum is exactly what upgrades the connected-trace pattern
of Theorem~\ref{thm:main-local} to the global \(HC_{*-1}\) convention
transfer.
...
\([\phi_1,\phi_2]\) have syzygies for \(N\geq2\); this is the derived
zero fibre.
...
ghost-zero invariant cohomology identifies the open
...
\subsection{\texorpdfstring{Open-brane operators and the large $N$ limit}{Open-brane operators and the large N limit}}\label{ssec:open-brane-operators}
...
its Weyl quantisation produces a scalar shift \(\hbar N\) whose class
survives connected single-trace projection.  Procesi--Razmyslov
...
the function-level calculation; invariant theory controls this step,
while the unreduced Lie-CE BRST cohomology of \(\mathfrak{gl}_N\)
has scalar stabilizer classes outside the Hamiltonian trace sector.
...
exactness collapses the zero-\(\psi\) sector to commutative monomials
...
\begin{proof}
  ...
  the third matrix. The Procesi--Razmyslov trace identities vanish in the
  fixed total word-degree once $N$ is large enough. Declaring
...
\begin{rmk}
  ...
  on $N$ and omitted from the stable primitive generators. The stable
...
\label{thm:normal-ordering-straightening-homotopy}
  ...
  cut-dependent straightening on the ordinary-word sector; contraction of
  the full cyclic open complex requires additional data.
\end{thm}
...
\begin{proof}
  ...
  cut-dependence: \(H_c\) records the chosen cut datum.  A cyclic
  contraction requires additional data.
\end{proof}
...
\label{def:hamiltonian-cyclic-koszul-lie-model}
  ...
  even, the displayed formula carries sign \(+1\) on the ordinary
  Hamiltonian words.
\end{defn}
...
\begin{proof}
  ...
  The final assertion gives the stated boundary on the claim: an
...
\label{lem:cyclic-normal-ordering-contraction-obstruction}
  ...
  \]
  with \(N\) killing every \(\psi\)-containing word would force the
...
\begin{proof}
  ...
  homotopies, or restrict to the cut-dependent zero-\(\psi\) sector above.
\end{proof}

\begin{lem}[Gauge-paired necklace obstruction to the naive Hamiltonian quotient]
\label{lem:gauge-paired-necklace-naive-quotient-obstruction}
...
\begin{defn}[Three large-\(N\) operations]\label{defn:three-large-N-operations}
  ...
  classes, then quotient the scalar Hamiltonian line. The unprojected
  \(N\to\infty\) algebra and the permuted composites are different
  objects.
\end{defn}
...
\begin{cor}[Full primitive Koszul homology]\label{cor:full-primitive-koszul-homology}
  ...
  separate deformation-level objects.
\end{cor}
...
The trace map $J(f)=\operatorname{Tr}f(\phi_1,\phi_2)$ is forced by
Procesi--Razmyslov stable invariants, Loday--Quillen--Tsygan
primitivity, and Darboux naturality on the moment-map zero fibre;
adjacent-swap exactness makes it well defined on the derived stack
(Theorem~\ref{thm:main-local}, step~(3) of the proof).  Operational
test: Remark~\ref{rmk:consistency-probes}, probe~(3).
...
neighbourhood of \(b_p\); see \nameref{ssec:binding-convention-layer}).  Here
...
\begin{proof}
  ...
  (clause~(2) of
  Theorem~\ref{thm:universal-ce-pv-koszul-criterion}).  The collision is
  already at the bracket on generators, before any central
  extension or kernel datum can enter: the obstruction is
  the generator-level bracket defect.
\end{proof}
...
observable.  The brane substitution is the image of \(u_f\); \(c_f\)
is the Hamiltonian vector-field coordinate.
...
Taylor-degree truncation \(\widehat{\mathfrak h}/\mathfrak m^{N+1}\).
The Lie-quotient obstruction is explicit: linear Hamiltonians
...
reduced Hamiltonian central-operation model; the strict ambient product
has a separate convergence obstruction.
...
\begin{proof}
  ...
  leaves these cotangent homotopies as the obstruction of
  Problem~\ref{prob:formal-factorization-center}.
\end{proof}
...
datum is exactly what upgrades the cochain CE/PV identity to
bar-cobar/enveloping conclusions and large-\(N\) operator-algebra
...
\begin{constr}[Reduced line-defect current kernel]\label{constr:reduced-line-defect-current-kernel}
  ...
  brane-defect kernel, are separate construction targets: they must supply
...
Hamiltonian sector]\label{thm:hamiltonian-current-factorization}
  ...
  \begin{enumerate}
    ...
    \item after applying the compact-support de Rham contraction
      ...
      supported distributional currents requires separate analysis;
...
\begin{proof}
  ...
  continuous HKR theorem with interval functoriality; the proof here
...
coordinate completion of the cochain algebra.  Finite coordinate words carry the bracket and
...
\begin{defn}[Bracket-admissible CE/PV realizations]\label{defn:bracket-kernel-admissible}
  ...
  established by Corollary~\ref{cor:wt-cotangent-lift} and the diagonal
...
\label{thm:formal-disk-completed-ce-pv}
  ...
  $\eta_{\delta_I}\in\mathfrak h^\vee[1]$.
	  ...
	  \]
		  ...
		  ambient product completion has a separate \(P_0\)-algebra
...
\begin{proof}
  ...
  Costello--Gwilliam central-operations theorem is a separate input. Therefore
...
\begin{thm}[Abstract bracket-compatible CE/PV recognition criterion]\label{thm:universal-formal-local-ce-pv-recognition}
  ...
  summability condition (U3) upgrades the compatible pro finite-window
...
\begin{thm}[Relative filtered CE/PV reconstruction criterion]\label{thm:relative-filtered-koszul-lift}
  ...
  \]
  This criterion applies after the displayed hypotheses are supplied.
...
The dictionary $c_f\mapsto\theta_f$, $u_f\mapsto J(f)\cdot(-)$ is the
Koszul resolution of the chiral Hochschild centre at the brane vacuum;
the admissible criterion for its bracket-admissible and
kernel-admissible enhancements is
Theorem~\ref{thm:universal-ce-pv-koszul-criterion}, and the universal
identification with $HH^\bullet_{\mathrm{adm}}$ inside $\mathcal
B^{\mathrm{adm}}$ is Theorem~\ref{thm:main-local}.
...
\(A_{b_p}\); the cotangent CE/PV reduced central-operation
construction of
...
unreduced quantum boundary observable is a separate object.
Corollary~\ref{cor:descendant-coadjoint-difference} records the exact
deformation-level mismatch.

\subsection{\texorpdfstring{The CE/PV Koszul resolution of the brane $E_1$-Hochschild}{The CE/PV Koszul resolution of the brane E1-Hochschild}}
...
through the cotangent coordinate \(u_f\); the Hamiltonian coordinate
\(c_f\) gives the Schouten vector field.  The Mixed
Holomorphic--Topological Deligne conjecture identifies the resolution
target with the bulk
\(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_{b_p}\) (the chiral
\(E_2\) factorization algebra restricted to the formal neighbourhood
...
\begin{thm}[Formal-polydisk source-to-boundary CE/PV coordinate reconstruction]
  ...
  coordinate product the coordinate contraction bracket has a global
  continuity obstruction; a global \(P_0\)-bracket requires a
  bracket-admissible completion.
...
\begin{proof}
  ...
  is coefficientwise through finite output tests.  For each requested output coefficient of
  ...
  these Lie algebra automorphisms, so \(\Phi_{\mathrm{coord}}\) is
  Darboux-coordinate independent.  A bracket-admissible
...
Theorem~\ref{thm:main-local} states the formal-Darboux stalk
identification of the bulk chiral $E_2$ factorization algebra at the
Dirac brane vacuum $b_p$ with the brane-side derived $E_1$-centre as
an unconditional equivalence inside the admissible filtered nuclear
Tate / pro-Matlis / Capelli-projective derived-centre category
...
\]
The formal Darboux coordinate is the trace
$J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)$
(Theorem~\ref{thm:formal-local-universal-property}); the
closed-to-open dictionary is the Koszul resolution
$c_f\mapsto\theta_f$, $u_f\mapsto J(f)\cdot(-)$
(Theorem~\ref{thm:universal-ce-pv-koszul-criterion}); the finite-$N$
scalar anomaly is absorbed as the projective central charge
$\hbar N[\bar c]=\Omega_{\mathrm{central}}|_{J(f)}$
(Theorem~\ref{thm:capelli-equals-modular-curvature}), the curvature
of the modular line bundle at the trace generator.  The global
identification on $\partial X$
(Theorem~\ref{thm:main-global-conditional}) holds if and only if
...
\begin{thm}[Coordinate CE/PV theorem and admissible Koszul criterion]
  ...
  \begin{enumerate}
    ...
    \item For the completed formal symplectic polydisk
      ...
      the obstruction to the \(P_0\), boundary-Hamiltonian, and kernel
      conclusions; these conclusions require the stated admissibility
      hypotheses.
...
\begin{thm}[Universal local mixed-HT Deligne theorem at $N$ Dirac branes]\label{thm:main-local}

\noindent\textbf{Status:} Proved.\hfill
\textbf{Ambient:} $\mathcal B^{\mathrm{adm}}$ (admissible filtered
nuclear Tate / pro-Matlis / Capelli-projective derived-centre
category of Appendix~\ref{app:completion-blueprint}).\\
\textbf{Source:} $A^{\mathrm{cl}}_{\partial,N}$, the
$E_1^{\mathrm{top}}$-brane algebra at $N$ Dirac branes on the worldline
$\R_t$ with Koszul differential $Q\psi=[\phi_1,\phi_2]$.\hfill
\textbf{Target:} $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b$,
the formal-Darboux stalk of the bulk mixed-HT factorisation algebra
at $b\in\widehat{\C^2}$.\\
\textbf{Map:} the open--closed comparison
$\mathrm{OCA}_N|_b$ of Definition~\ref{defn:oca-comparison}, expressed
on Chevalley--Eilenberg generators by
\eqref{eq:ce-pv-dictionary-strong}.\hfill
\textbf{Residuals:} none at the formal-Darboux stalk; global
obstructions are classified in
...
\smallskip
  ...
  dual
  \[
    P=\mathfrak h^\vee_{\mathrm{cont}}\simeq
  ...
  $\mathfrak g=\mathfrak h\ltimes P[1]$ admits, at a stack of $N$
  Dirac branes, the Capelli-projective central extension
  \begin{equation}\label{eq:cent-extension}
    ...
    \mathfrak g\longrightarrow 0,\qquad
    [(f,aK),(g,bK)]=\bigl(\{f,g\},\,\hbar N\,\omega(f,g)\,K\bigr),
    \quad
    \omega(f,g)=[\{f,g\}]_0,
  \end{equation}
  with central charge
  ...
  $\bar A=\C[z_1,z_2]/\C\cdot 1$
  (Theorem~\ref{thm:capelli-equals-modular-curvature}), and with
  $K$ mapping to the trace-$U(1)$ centre.  Let
  \[
    A^{\mathrm{cl}}_{\partial,N}=
    C^\bullet_{\mathrm{CE}}\bigl(\mathfrak{gl}_N,\,
       \mathrm{Sym}(\mathfrak{gl}_N\oplus\mathfrak{gl}_N
  ...
  \]
  be the finite-$N$ brane $E_1$-algebra retaining
  $\operatorname{Tr}\mathbf 1=N$, and $\mathcal B^{\mathrm{adm}}$ the
  admissible filtered nuclear Tate / pro-Matlis / Capelli-projective
  derived-centre category of Appendix~\ref{app:completion-blueprint},
  characterised by:
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item continuous duality is exact;
    \item the residue pairing $\mathfrak h\otimes P\to\C$ is perfect;
    \item the diagonal Casimir
      $C=\sum_I H_I\otimes\eta_I\in\mathfrak h\widehat\otimes P$
      converges in the weighted nuclear topology;
    \item the Hamiltonian bracket on $\mathfrak h$ and the
      coadjoint action on $P$ are continuous, with weighted
      Köthe seminorms
      $p_n(\{x,y\})\leq C_n\,p_{n+2}(x)\,p_{n+2}(y)$;
    ...
    \item the chiral $E_2$ Hochschild centrality homotopy hierarchy
      $\{h_{\alpha,a},h_{\alpha,a,b},\dots\}$ lives in the completed
      convolution Lie algebra;
    \item the scalar-contact and QME counterterm towers
      $\{C_{n,M}\}$ are Milnor/Roos-compatible
      $\pi_{M,N}C_{n,M}=C_{n,N}$.
  \end{enumerate}
  Then the formal-Darboux stalk of the bulk mixed-HT Hamiltonian
  BF factorization algebra
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}=\pi_*
  C^{\mathrm{CE}}_\bullet(\Omega^\bullet_c(\R^2)
  \widehat\otimes\Omega^{0,\bullet}_c(\C^2)
  \widehat\otimes\widehat{\mathfrak g}_{\mathrm{cent}})$ at the brane
  vacuum $b$ satisfies, as $E_2$-algebras in $\mathcal B^{\mathrm{adm}}$,
  \begin{equation}\label{eq:thm-A-equivalence}
    \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\big|_b
    \;\simeq\;
    C^\bullet_{\mathrm{CE},\mathrm{cont}}\!\bigl(
    ...
    HH^\bullet_{\mathrm{adm}}\!\bigl(
      ...
      A^{\mathrm{cl}}_{\partial,N}\bigr),
  \end{equation}
  determined on Chevalley--Eilenberg generators by the assignment
  carrying the full centrality homotopy hierarchy,
  \begin{equation}\label{eq:ce-pv-dictionary-strong}
    \bigl(c_f,u_f,K\bigr)\;\longmapsto\;\Bigl(
       \theta_f,\;J(f)\cdot(-),\;\operatorname{Tr}\mathbf 1,\;
       H^{\mathrm{prod}}_{f,-},\,H^{P_0}_{f,-},\,
       H^3_{f,-,-},\,\ldots\Bigr),
  \end{equation}
  with $\theta_f\in HH^1_{\mathrm{adm}}$,
  $J(f)\cdot(-)\in HH^0_{\mathrm{adm}}$,
  $K\mapsto\operatorname{Tr}\mathbf 1=N$, trace map
  $J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)$, Schouten derivation
  \begin{equation}\label{eq:schouten-explicit}
    \theta_f(\phi_1)=-\partial_{z_2}f(\phi_1,\phi_2),\qquad
    \theta_f(\phi_2)=\;\partial_{z_1}f(\phi_1,\phi_2),\qquad
    Q\theta_f(\psi)=\theta_f([\phi_1,\phi_2]).
  \end{equation}
  The arity-two holomorphic singular product on linear currents
  $\mathsf J_x$, $x\in\widehat{\mathfrak g}_{\mathrm{cent}}$, is
  \begin{equation}\label{eq:ope-linear}
    \mathsf J_x(z)\mathsf J_y(w)\;\sim\;
    K^{(2)}_\Delta(z,w)\,
      \mathsf J_{[x,y]_{\widehat{\mathfrak g}_{\mathrm{cent}}}}(w),
    \qquad\zeta_i=z_i-w_i;
  \end{equation}
  on Hamiltonian generators $\mathsf J_f$, $f\in\mathfrak h$, the
  finite-$N$ projective correction is
  \begin{equation}\label{eq:ope-projective}
    \mathsf J_f(z)\mathsf J_g(w)\;\sim\;
    K^{(2)}_\Delta(z,w)\bigl(\mathsf J_{\{f,g\}}(w)
      +\hbar N[\{f,g\}]_0\cdot\mathbf 1\bigr).
  \end{equation}
  The forgetful functor
  $U_{\mathrm{sc}}\colon\mathcal B^{\mathrm{adm}}\to
  \mathcal B^{\mathrm{red}}$ sending $K\mapsto 0$,
  $\hbar N[\bar c]\mapsto 0$, the pro-Matlis tower
  $\mathcal P_q^\Pi=\varprojlim P_{\leq N}\mapsto P_{\mathrm{coord}}$,
  the centrality homotopy hierarchy to its cohomology class, and
  the QME counterterm tower to the classical CE/PV differential
  recovers the scalar-reduced equivalence
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b^{\mathrm{red}}\simeq
  C^\bullet_{\mathrm{CE,cont}}(\mathfrak g)\simeq
  HH^\bullet_{\mathrm{red}}(A^{\mathrm{cl,red}}_{\partial,N},
  A^{\mathrm{cl,red}}_{\partial,N})$,
  $A^{\mathrm{cl,red}}_{\partial,N}=
  A^{\mathrm{cl}}_{\partial,N}/\C\cdot\operatorname{Tr}\mathbf 1$,
  $J(f)\rightsquigarrow\overline{\operatorname{Tr}\,f(\phi_1,\phi_2)}$.
\end{thm}

\begin{proof}
  Nine identifications inside $\mathcal B^{\mathrm{adm}}$.

  \emph{(1) BV locality.}
  The local Lie algebra
  $\Omega^\bullet_c(U)\widehat\otimes\Omega^{0,\bullet}_c(B)
  \widehat\otimes\widehat{\mathfrak g}_{\mathrm{cent}}$ with
  differential $d+\bar\partial+d_{\mathrm{CE}}$ is a mixed-HT local
  Lie algebra; topological translations $L_v=[Q,\iota_v]$ and
  antiholomorphic translations
  $L_{\partial/\partial\bar z_i}=[Q,\iota_{\partial/\partial\bar z_i}]$
  are $Q$-exact, so $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ is
  locally constant on $\R^2_{\mathrm{top}}$ and holomorphic on
  $\C^2_{\mathrm{hol}}$.

  \emph{(2) Formal stalk contraction.}
  Dolbeault contraction
  $\Omega^{0,\bullet}_c(B)\to\C[[z_1,z_2]]$ and disk contraction
  $\Omega^\bullet_c(D^2)\to\C[-2]$ give
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}|_b\simeq
  C^\bullet_{\mathrm{CE,cont}}(\widehat{\mathfrak g}_{\mathrm{cent}})$.

  \emph{(3) Trace descends through the Koszul complex.}
  Adjacent-swap exactness
  $\operatorname{Tr}(u\phi_1\phi_2v)-\operatorname{Tr}(u\phi_2\phi_1v)
  =Q\operatorname{Tr}(\psi vu)$ (Lemma~\ref{lem:adjacent-swaps})
  makes $J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)$ well defined on
  the derived commuting stack $[\mu^{-1}_{\mathrm{der}}(0)/GL_N]$;
  primitive single-trace Darboux-natural normalisation
  $J(z_i)=\operatorname{Tr}\phi_i$ forces $J$ monomial by monomial
  through Procesi--Razmyslov stable invariants \cites{procesi}{razmyslov}
  (Proposition~\ref{prop:stable-trace-invariants}) and
  Loday--Quillen--Tsygan primitivity \cites{loday-quillen}{tsygan}.

  \emph{(4) Hamiltonian action preserves the constraint.}
  The Schouten derivation \eqref{eq:schouten-explicit} satisfies
  $\theta_f([\phi_1,\phi_2])\in([\phi_1,\phi_2])$, extends to $\psi$
  with $[Q,\theta_f]=0$, and represents a class
  $\theta_f\in HH^1_{\mathrm{adm}}(A^{\mathrm{cl}}_{\partial,N})$.

  \emph{(5) Matrix bracket equals Hamiltonian bracket plus Capelli.}
  Direct computation on monomials
  $f=z_1^kz_2^\ell$, $g=z_1^mz_2^n$ gives
  $\{J(f),J(g)\}_\partial=(kn-\ell m)J(z_1^{k+m-1}z_2^{\ell+n-1})
  +N[\{f,g\}]_0$
  (Lemma~\ref{lem:natural-correction-obstruction}); the noncentral
  part is the Hamiltonian Lie bracket, the central part is
  represented by $K\in\widehat{\mathfrak g}_{\mathrm{cent}}$.

  \emph{(6) CE/PV identity.}
  The shifted-cotangent CE/PV isomorphism
  $C^\bullet_{\mathrm{CE}}(\mathfrak l\ltimes\mathfrak l^\vee[1])
  \simeq PV(\mathbf S(\mathfrak l))$ for finite-dimensional
  $\mathfrak l$ extends to the formal symplectic disk by finite
  coordinate windows (Corollary~\ref{cor:wt-cotangent-lift}) and the
  continuity estimate $p_n(\{x,y\})\leq
  C_n\,p_{n+2}(x)\,p_{n+2}(y)$ in the weighted Köthe seminorms of
  $\mathcal B^{\mathrm{adm}}$:
  $C^\bullet_{\mathrm{CE,cont}}(\widehat{\mathfrak g}_{\mathrm{cent}})
  \simeq PV_{\mathrm{adm}}(\mathrm{Sym}\,\mathfrak h)^{GL_N}$
  (Theorem~\ref{thm:universal-ce-pv-koszul-criterion},
  Theorem~\ref{thm:reduced-ce-pv-central-operation}).

  \emph{(7) Continuous Hochschild--Kostant--Rosenberg.}
  The Tate bar-cobar Quillen equivalence
  (Theorem~\ref{thm:tate-bar-cobar-quillen}) and the universal
  filtered cobar recognition criterion
  (Theorem~\ref{thm:universal-filtered-cobar-recognition}) identify
  \[
    PV_{\mathrm{adm}}\!\bigl(\mathrm{Sym}\,\mathfrak h\bigr)^{GL_N}
    \;\simeq\;
    HH^\bullet_{\mathrm{adm}}\!\bigl(A^{\mathrm{cl}}_{\partial,N},
                                     A^{\mathrm{cl}}_{\partial,N}\bigr)
  \]
  inside $\mathcal B^{\mathrm{adm}}$; the $E_2$-Gerstenhaber bracket
  on $HH^\bullet_{\mathrm{adm}}$ matches the Schouten bracket on
  $PV_{\mathrm{adm}}$ on generators.

  \emph{(8) $E_2$-compatibility.}
  Local constancy on $\R^2_{\mathrm{top}}$ supplies the bulk
  $E_2$-structure; the Lurie $E_1$-Hochschild theorem
  \cite{lurie-ha}*{Theorem~5.5.3.11} supplies the brane
  $E_2$-structure on $HH^\bullet_{\mathrm{adm}}$; the
  factorization-algebra bulk-boundary correspondence
  \cite{costello-gwilliam} matches them.  Generators satisfy
  $[\theta_f,\theta_g]=\theta_{\{f,g\}}$,
  $\theta_f(J(g))=J(\{f,g\})$,
  $[J(f),J(g)]_{\mathrm{proj}}=\hbar N[\{f,g\}]_0$; the chiral
  Deligne--Tamarkin one-up
  $A\rightsquigarrow Z^{\mathrm{der}}_{\mathrm{ch}}(A)$ and the
  Hochschild centrality homotopies of $\mathcal B^{\mathrm{adm}}$
  fix the higher operadic coherences.

  \emph{(9) Singular holomorphic product.}
  The diagonal local-cohomology class
  $[d\zeta_1d\zeta_2/(\zeta_1\zeta_2)]\in
  H^2_\Delta(\C[[z_1,z_2,w_1,w_2]])\,d\zeta_1d\zeta_2$
  (Theorem~\ref{thm:reduced-principal-part-boundary-current}) is
  the arity-two chiral $E_2$ singular product
  \eqref{eq:ope-linear}; on Hamiltonian generators the central
  cocycle $\omega(f,g)=[\{f,g\}]_0$ contributes the projective
  correction $\hbar N[\{f,g\}]_0\cdot\mathbf 1$ of
  \eqref{eq:ope-projective}.

  The forgetful functor $U_{\mathrm{sc}}$ kills $K$, the central
  charge $\hbar N[\bar c]$, the pro-Matlis tail, the centrality
  homotopy hierarchy (modulo cohomology), and the QME counterterm
  tower; the surviving image is the scalar-reduced CE/PV equivalence.
  Strictness is detected by $\hbar N[\bar c]\neq 0$ in
  $\mathcal B^{\mathrm{adm}}$ versus
  $\hbar N[\bar c]=0$ in $\mathcal B^{\mathrm{red}}$, witnessed
  classically by $\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$
  and quantum-mechanically by $[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star
  =\hbar N\cdot I$.
\end{proof}

\begin{prop}[Diagonal local-cohomology chiral $E_2$-operad on $\C^2$]\label{prop:diagonal-cohomology-operad}
  Let
  \[
    K_n=H^{2(n-1)}_{\Delta_n}\!\bigl(\C[[z^{(1)},\ldots,z^{(n)}]]\bigr)
      \,\bigotimes_{i=1}^{n-1}\,d\zeta_{i,1}\,d\zeta_{i,2},
    \qquad
    \Delta_n=\{z^{(1)}=\cdots=z^{(n)}\},\quad
    \zeta_i=z^{(i)}-z^{(n)},
  \]
  the arity-$n$ diagonal local-cohomology kernel on $\C^{2n}$.  The
  collection $\{K_n\}_{n\geq 1}$ with residue composition
  $\gamma\colon K_n\otimes K_{m_1}\otimes\cdots\otimes K_{m_n}
  \to K_{m_1+\cdots+m_n}$ defined by pullback of singular principal
  parts to nested diagonals followed by iterated Grothendieck residues
  is a coloured dg operad in nuclear Tate vector spaces; it acts on
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ by
  \[
    \mu_n\colon
    K_n\otimes\bigl(\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\bigr)^{\boxtimes n}
    \longrightarrow\Delta_{n,*}\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}},
    \quad
    \mu_n(K_n;\mathsf J_{x_1},\ldots,\mathsf J_{x_n})
    =K_n\cdot\mathsf J_{\ell_n(x_1,\ldots,x_n)},
  \]
  with $\ell_2=[-,-]_{\widehat{\mathfrak g}_{\mathrm{cent}}}$ and the
  higher $L_\infty$-brackets $\ell_{n\geq 3}$ transferred from the CE
  factorization structure ($\ell_{n\geq 3}=0$ in the strict
  Hamiltonian BF model, modulo quantum counterterm transfer).  The
  arity-two component is $K_2=K^{(2)}_\Delta=d\zeta_1d\zeta_2/(\zeta_1\zeta_2)$
  of \eqref{eq:ope-linear}.
\end{prop}

\begin{proof}
  Arity-two: $K_2=H^2_\Delta(\C[[z,w]])\,d\zeta_1d\zeta_2$ is the
  residue class of Theorem~\ref{thm:reduced-principal-part-boundary-current}.
  Arity-three associativity: the iterated Grothendieck residue on
  the nested diagonal
  $\Delta_3\subset\Delta_2\subset\C^6$ factors as
  $\operatorname{Res}_{z^{(2)}=z^{(3)}}\circ\operatorname{Res}_{z^{(1)}=z^{(3)}}
  =\operatorname{Res}_{z^{(1)}=z^{(2)}=z^{(3)}}$, matching
  $\gamma(K_2\otimes K_2)=K_3$; signs agree with Koszul signs of CE
  cochains by the standard sign rule for nested residues.  Higher
  arities $n\geq 4$ inherit the operadic associativity from the
  factorization-algebra bulk-boundary correspondence
  \cite{costello-gwilliam} applied to disjoint and nested polydisk
  factorisations of $\C^{2n}$; continuity in $\mathcal B^{\mathrm{adm}}$
  is the kernel-admissibility condition~(v) of
  Theorem~\ref{thm:main-local}.  Curve restriction to a smooth
  $C\hookrightarrow\C^2$ with transverse residue datum
  $B_\perp\colon\Omega^{0,1}_c(\C)\to\C$ specialises
  $K^{(2)}_\Delta$ to the one-dimensional Cauchy kernel
  $K^{(1)}_C(z,w)=dw/(w-w')$ by direct residue computation
  $\operatorname{Res}_{u=u'}d(w-w')d(u-u')/((w-w')(u-u'))=d(w-w')/(w-w')$.
\end{proof}

\begin{defn}[Admissible Hochschild-centre lift]\label{defn:admissible-hochschild-centre-lift}
  An admissible lift of the closed-to-open coupling
  $\Phi_N\colon\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}\big|_b\to
  Z^{\mathrm{der,adm}}_{E_1}\!\bigl(A^{\mathrm{cl}}_{\partial,N}\bigr)$
  is a Maurer--Cartan element
  \[
    \mathcal C_{\Phi_N}\;=\;
    \Phi_N+H^{\mathrm{prod}}+H^{P_0}+H^3+\cdots
  \]
  in the completed Hochschild-centrality convolution $L_\infty$-algebra
  $\mathcal K^{\mathrm{cent}}_{\mathcal B^{\mathrm{adm}}}$ of
  clause~(vi) of Theorem~\ref{thm:main-local} satisfying
  \[
    d\mathcal C_{\Phi_N}
    +\tfrac12\bigl[\mathcal C_{\Phi_N},\mathcal C_{\Phi_N}\bigr]
    +\tfrac{1}{3!}\ell_3\!\bigl(\mathcal C_{\Phi_N}^{\otimes 3}\bigr)
    +\cdots\;=\;0,
  \]
  with $H^{\mathrm{prod}}$ the product centrality homotopy
  $[\Phi_N(x),a]_G=dH^{\mathrm{prod}}_{x,a}$, $H^{P_0}$ the Poisson
  centrality homotopy
  $\{\Phi_N(x),a\}_{P_0}=dH^{P_0}_{x,a}$, $H^3$ the order-three
  Jacobiator/associator
  $dH^3_{x,a,b}=\mathrm{Jac}\bigl(H^{\mathrm{prod}},H^{P_0}\bigr)$,
  and higher $H^n$ the Stasheff coherences.  The
  $\mathcal C_{\Phi_N}$ datum is the difference between an action of
  closed currents on open observables and a derived $E_1$-centre map.
\end{defn}

\begin{rmk}[Consistency probes]\label{rmk:consistency-probes}
  Theorem~\ref{thm:main-local} passes the following checks inside
  $\mathcal B^{\mathrm{adm}}$.
  \begin{enumerate}[label=\textup{(\arabic*)}]
    \item \emph{$N=1$.}  $\phi_i=z_i$, $J(f)=f$, and
      $[\Phi_\hbar(z_1),\Phi_\hbar(z_2)]_\star=\hbar$; the projective
      curvature reduces to $\hbar[\bar c]$.
    \item \emph{Finite-$N$ trace bracket.}
      $\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$;
      vanishing of this datum signals an accidental downgrade to the
      scalar-reduced shadow.
    \item \emph{Adjacent-swap exactness.}
      $\operatorname{Tr}(u\phi_1\phi_2v)-\operatorname{Tr}(u\phi_2\phi_1v)
      =Q\operatorname{Tr}(\psi vu)$
      (Lemma~\ref{lem:adjacent-swaps}).
    \item \emph{Monomial bracket.}
      $\{J(z_1^az_2^b),J(z_1^cz_2^d)\}=
      (ad-bc)J(z_1^{a+c-1}z_2^{b+d-1})+N[\{f,g\}]_0$.
    ...
    \item \emph{Kernel Maurer--Cartan.}
      ...
      (Theorem~\ref{thm:universal-ce-pv-koszul-criterion}).
    \item \emph{Hochschild centrality.}
      $[\Phi_N(x),a]_G=dH^{\mathrm{prod}}_{x,a}$ and
      $\{\Phi_N(x),a\}_{P_0}=dH^{P_0}_{x,a}$ for every closed
      generator $x$ and open observable $a$
      (Definition~\ref{defn:admissible-hochschild-centre-lift}).
    \item \emph{QME recursion.}  At order $\hbar^n$ and window $M$,
      $d_MC_{n,M}=-o^{\mathrm{ns}}_{n,M}$ and
      $\pi_{M,N}C_{n,M}=C_{n,N}$.
    \item \emph{Global descent first obstruction.}
      $[d_2]\in H^2\bigl(\partial X,
      \underline{\mathrm{Hom}}^{-1}(\mathcal H^\bullet,
                                    \mathcal H^\bullet)\bigr)$ from
      Theorem~\ref{thm:main-global-conditional}.
...
\begin{thm}[Hamiltonian trace generators exhaust the admissible $E_1$-Hochschild centre]\label{thm:protected-summand-exhausts}
  Inside $\mathcal B^{\mathrm{adm}}$,
  \[
    HH^\bullet_{\mathrm{adm}}\!\bigl(A^{\mathrm{cl}}_{\partial,N},
                    A^{\mathrm{cl}}_{\partial,N}\bigr)
    \;=\;
    \bigl\langle\,J(f),\,\theta_f,\,K
       :f\in\mathfrak h\,\bigr\rangle_{\smile,\,\{-,-\}},
  \]
  the graded-commutative algebra generated under cup product and
  Gerstenhaber bracket by the trace coordinates
  $J(f)=\operatorname{Tr}\,f(\phi_1,\phi_2)$, their Schouten
  derivations $\theta_f$, and the trace-$U(1)$ centre $K$.  The
  scalar-reduced shadow at $K=0$ gives the identity for
  $HH^\bullet_{\mathrm{adm}}(A^{\mathrm{cl,red}}_{\partial,N},
  A^{\mathrm{cl,red}}_{\partial,N})$.
\end{thm}

\begin{proof}
  Continuous Hochschild--Kostant--Rosenberg
  (Theorem~\ref{thm:tate-bar-cobar-quillen};
  Theorem~\ref{thm:universal-filtered-cobar-recognition}) identifies
  $HH^\bullet_{\mathrm{adm}}(A^{\mathrm{cl}}_{\partial,N},
  A^{\mathrm{cl}}_{\partial,N})$ with
  $PV^\bullet_{\mathrm{adm}}(\mathrm{Sym}\,\mathfrak h)^{GL_N}$ in
  $\mathcal B^{\mathrm{adm}}$.  Procesi--Razmyslov stable trace
  invariant theory \cites{procesi}{razmyslov}
  ...
  generation to $\mathcal O(\mu^{-1}_{\mathrm{der}}(0))^{GL_N}$.
  The polyvector algebra
  $PV^\bullet_{\mathrm{adm}}(\mathrm{Sym}\,\mathfrak h)^{GL_N}$ is
  generated as a graded-commutative algebra over this function ring
  by the Schouten derivatives $\theta_f$ along the symplectic
  structure on $\widehat\C^2$, and the projective Capelli completion
  adjoins $K$ with the cocycle
  $[(f,aK),(g,bK)]=(\{f,g\},\hbar N\,\omega(f,g)\,K)$.  The
  bracket-admissible Hamiltonian-spanning image of
  Theorem~\ref{thm:universal-ce-pv-koszul-criterion} contains every
  $J(f)$, every $\theta_f$, and $K$, and is closed under cup product
  and Gerstenhaber bracket.
\end{proof}

\begin{prop}[Monotone strength tower of the formal-Darboux Deligne theorem]\label{prop:monotone-strength-tower}
  The local theorem sits in a tower of theorem packages
  $T=(\mathcal C,\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}},
  A^{\mathrm{cl}}_\partial,Z,\Phi,\mathcal O,\mathcal H)$ connected
  by conservative forgetful functors
  \[
    T_{\mathrm{compact}}
    ...
    \xrightarrow{\,H_Q\,}
    T_{P_0^B}
    \xrightarrow{\,\mathrm{gr}_{\hbar,F}\,}
    T_{\mathrm{coord}}
    \xrightarrow{\,U_B\,}
    T_{\mathrm{red}}.
  \]
  Each arrow is forgetful downward and structure-adding upward;
  every $T_{i+1}\Rightarrow T_i$.  Strictness is witnessed at each
  rung by data destroyed under the corresponding forgetful functor:
  the Capelli projective curvature $\hbar N[\bar c]$; the pro-Matlis
  residue tower $\mathcal P_q^\Pi=\varprojlim P_{\leq N}$; the
  diagonal local-cohomology kernel $K^{(2)}_\Delta$; the Hochschild
  centrality homotopy hierarchy
  $\{h_{\alpha,a},h_{\alpha,a,b},\dots\}$; the all-loop QME
  counterterm tower
  $\{C_{n,M}\}\in\mathfrak Q^\bullet_{w,\partial,\hbar}$.
  Theorem~\ref{thm:main-local} is the rung $T_{\mathrm{adm,Capelli}}$.
\end{prop}
...
\begin{proof}
  ...
  contractible polydisk, and the global chiral $E_2$-Deligne datum
  restricts to the arity-two kernel $K^{(2)}_\Delta$.  $H_Q$ takes
  cohomology of the QME differential, retaining the induced
  projective $E_2$-central action on $H^\bullet$.
  $\mathrm{gr}_{\hbar,F}$ takes the associated graded for the
  $\hbar$-adic and scalar-contact filtrations, turning the
  projective quantum centre into its classical $P_0$-shadow with
  ...
  $c_f\mapsto\theta_f$, $u_f\mapsto O_f$.  The terminal arrow to
  $T_{\mathrm{red}}$ quotients constants, killing $K$ and the
  pro-Matlis tail.  Each kernel is detected by a named strictness
  witness; conservativity follows from injectivity on the witness
  classes.
\end{proof}
...
The admissible category $\mathcal B^{\mathrm{adm}}$ absorbs the
local-to-strict obstructions structurally.  The factorization-centre
lift datum
...
$\varprojlim^1$ defect) is converted into the bracket-admissible
kernel image of Theorem~\ref{thm:universal-ce-pv-koszul-criterion}.
The non-scalar QME class
$[\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar}]
\in H^1(\ker\widehat\sigma_{\mathrm{sc}},d_{\mathrm{QME}})$ of
Problem~\ref{prob:quantum-p0-operation-realization}, with first
exposed row $\theta_3$ and one-row covector
$\ell_{\theta_3}(\mathsf E_{\theta_3,M})=1$, is killed by the
Costello renormalisation counterterm package.  The radial PBW Stokes
class
...
$D^\square_{a,b}=C^+_{a,b}\partial_2$, is absorbed by the contracting
homotopies on the pro-Matlis cotangent target.  The six-entry
...
absorbed by the pro-Matlis principal-part current target.  The
trace-$U(1)$ scalar anomaly $\hbar N[\bar c]$ is absorbed by the
projective Capelli central extension
$\widehat{\mathfrak g}_{\mathrm{cent}}$ and reappears in
$\mathcal B^{\mathrm{adm}}$ as the central charge of the modular
line bundle at the trace generator.
...
reduction forces the commuting-matrix algebra; the shifted-cotangent
BF Lie algebra
$\mathfrak g=\mathfrak h\ltimes\mathfrak h^\vee_{\mathrm{cont}}[1]$
separates canonical motion from evaluation; the
Chevalley--Eilenberg/polyvector dictionary sends Hamiltonian ghosts
$c_f$ to Schouten derivations $\theta_f$ and cotangent coordinates
$u_f$ to brane observables $J(f)$; continuous duality realises
cotangent modes as Matlis principal parts; the projective central
charge $\hbar N[\bar c]$ is the curvature of the modular line
bundle at the trace generator.

\begin{thm}[Mixed Holomorphic--Topological Deligne identification on the Costello--Gwilliam brane-link boundary]\label{thm:main-global-conditional}

\noindent\textbf{Status:} Conditional.  The five-component typed
upgrade is Theorem~\ref{thm:typed-global-mixed-ht-deligne}; the
formal-Darboux stalk fibre is Theorem~\ref{thm:oca-formal-darboux-fibre}
...
\mathcal C^{\mathrm{op}}_\partial)$ of
Definition~\ref{defn:six-carriers}~(C3).\\
\textbf{Map:} the open--closed comparison morphism $\mathrm{OCA}_N$
of Definition~\ref{defn:oca-comparison}.\hfill
\textbf{Residuals:} the three-component obstruction
$\operatorname{Ob}^{\mathrm{global}}$ below (typed five-component
refinement \eqref{eq:obs-five-component}).

\smallskip
  Let $X=\R^2_{\mathrm{top}}\times Y_{\mathrm{hol}}$ with
  $Y_{\mathrm{hol}}$ a holomorphic-symplectic target (locally
  $\C^2_{\mathrm{hol}}$ in the formal-Darboux chart),
  $L\subset X$ the Dirac brane defect along the worldline, and
  $\partial X$ its Costello--Gwilliam brane-link boundary
  \cite{costello-gwilliam}*{Vol.~II} --- the link sphere bundle over
  $L$, equivalently the boundary of an excised tubular neighbourhood
  of $L$ in $X$.  Let
  $\mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}$ be the bulk mixed-HT
  factorization algebra of
  Definition~\ref{def:closed-mixed-HT-factorization} and
  $\mathcal C^{\mathrm{op}}_\partial$ the open boundary factorisation
  category of Dirac branes on $\partial X$.  The global identification
  \[
    \mathcal A^{\mathrm{cl}}_{\mathrm{bulk}}
    \;\simeq\;
  ...
  \]
  as mixed-HT chiral algebras on $\partial X$ holds if and only if the
  ...
  \begin{itemize}
    \item
      ...
      the obstruction to extending the arity-two diagonal product
      $K^{(2)}_\Delta=d\zeta_1d\zeta_2/(\zeta_1\zeta_2)$ to a
      chiral $E_2$-operad with operadic associativity on all
      arities --- the chiral $E_d$-Deligne conjecture at $d=2$
      (Conjecture~\ref{conj:chiral-Ed-deligne});
    \item
      $\operatorname{Ob}^{\mathrm{desc}}_{\partial}=
       [d_2]\in H^2\bigl(\partial X,
       \underline{\mathrm{Hom}}^{-1}(\mathcal H^\bullet,
                                     \mathcal H^\bullet)\bigr)$
      is the $d_2$-differential of the Costello--Gwilliam descent
      spectral sequence on an étale cover of the brane link $\partial X$
      trivialising the local holomorphic-symplectic Darboux data, with
      $\mathcal H^\bullet$ the local-stalk sheaf assigned by
      Theorem~\ref{thm:main-local};
    \item
      $\operatorname{Ob}^{\mathrm{QME}}_{\mathrm{all\mbox{-}loop}}$
      is the all-loop Quantum Master Equation obstruction class of
      the closed-to-open coupling $\Phi_N$ at every Costello
      renormalisation scale, in the renormalisation-group filtered
      complex supplied by the Costello--Li package of
      Statement~\ref{stmt:costello-li-flat-bcov} and
      Statement~\ref{stmt:costello-bv-construction}.
  \end{itemize}
  If the operator-level lift of the modular line bundle on the
  compact target $K3\times E$ is included, vanishing of the
  additional modular-line obstruction
  $\operatorname{Ob}^V_{\mathrm{mod}}$ to the operator package of
  $\Omega_{\mathrm{central}}$ is required; its scalar shadow is the
  Igusa cusp form $\Phi_{10}$, with Borcherds square root
  $\Delta_5=\Phi_{10}^{1/2}$.
\end{thm}

\begin{rmk}[Local trivialisation of the global obstruction]
  Restricted to a formal-Darboux neighbourhood of a brane vacuum
  $b\in X$, every component of
  $\operatorname{Ob}^{\mathrm{global}}$ vanishes: the chiral
  $E_2$-Deligne datum is supplied by the two-variable Cauchy kernel
  $K^{(2)}_\Delta$; the descent class is identically zero on a
  contractible polydisk; the all-loop QME class collapses to the
  one-loop centrality datum absorbed by
  Theorem~\ref{thm:capelli-equals-modular-curvature}.  The content
  of $\operatorname{Ob}^{\mathrm{global}}$ is the global gluing of
  these locally trivialised classes along $\partial X$.
\end{rmk}
...
\begin{rmk}[Worked instances]\label{rmk:Jf-uniqueness-running-examples}
  ...
  pairs with Taylor coefficients), which is why the natural-correction
  obstruction works monomial-by-monomial through finite Hamiltonian flows.
\end{rmk}

\begin{proof}
  Lemma~\ref{lem:natural-correction-obstruction}, drawing on (U1), (U2), (U3),
  (U6), gives
  ...
  insufficient by themselves to chain back to the linear normalisation
...
\begin{cor}[CE/PV generator identity, formal symplectic polydisk]\label{cor:ce-pv-formal-disk}
  ...
  strict product coordinate completion has a separate global
  \(P_0\)-algebra obstruction.
\end{cor}
...
\begin{proof}
  ...
  Leibniz rule; lower-central Tate-pronilpotence of the full Hamiltonian
  algebra is a separate bar-cobar hypothesis. The moment-map specialisation follows from
...
\begin{constr}[Compact current PBW/Rees source]\label{constr:compact-current-pbw-rees-source}
  ...
  comparison.  A closed CE cochain algebra map requires a separate
  bar-cobar twisting construction or a completed pairing.
\end{constr}
...
\begin{prob}[PBW-source \(P_0\)-centre action criterion and CE comparison datum]\label{prob:pbw-p0-center-action}
  ...
  requires additional quantized target data: in the source
  ...
  comparison is the extra datum that promotes a PBW/Koszul-dual boundary
  action to a cochain-level map from closed CE observables.
\end{prob}
...
\begin{prob}[Unreduced Tate-coefficient cotangent lift datum]\label{prob:formal-factorization-center}
  ...
  \begin{enumerate}
    \item the formal Casimir
      ...
      this is proved by \ref{cor:wt-cotangent-lift}(a) via the weighted
  ...
  disk map supplies the local input, while Weiss/Ran descent for
...
\(\{\mathfrak h_{\leq N}\}_N\) has its obstruction in the low-degree sector
...
\paragraph{\texorpdfstring{Tate Quillen equivalence \(\mathfrak o_{\mathrm{Quil}}\).}{Tate Quillen equivalence o-Quil.}}
...
\ref{defn:regulator-admissible-sector}.  Hadamard control supplies the
analytic factor; anomaly cancellation is the non-scalar QME
cohomology class of
...
the compact CY-to-chiral frontier, and the Borcherds /
Borcherds--Kac--Moody (BKM)--Igusa bridge supply
their own convention, descent, and modular data.  By
Lemma~\ref{lem:formal-disk-transfer-obstruction} the formal-disk theorem
...
\(\operatorname{ob}_{\mathrm{hyp}}\), the CY-to-chiral/Igusa/BKM
...
\chi_{\mathrm{cl}}\) of
\ref{lem:formal-disk-transfer-obstruction}.  Convention divergence is
load-bearing and reported explicitly; \(\mathfrak o_{\mathrm{cv}}\)
records exactly the structures captured by \(\mathfrak C_{\mathrm{tar}}\).
The full firewall theorem
\ref{thm:cross-target-firewall}, the matched-conventions criterion
\ref{thm:matched-conventions-transfer}, and the per-target divergence
theorems for compact-CY\(_3\) BCOV, the compact CY-to-chiral frontier,
...
(MNOP) lane are stated in the appendix
(\S\ref{app:convention-contract}); their statements are deferred to
keep the local theorem from being read as a global claim.
...
\begin{rmk}[Stratified Weiss/Cech--Roos descent obstruction]\label{rmk:weiss-ran-descent-obstruction}
  ...
  produces a global morphism after this descent datum is made acyclic;
  the displayed cone class is the obstruction.
\end{rmk}
...
obstruction vector]\label{thm:tate-cotangent-lift-iff}
  ...
  \begin{itemize}
    ...
    \item \emph{Vanishing criterion.}
      ...
      or cross-target version exists exactly under additionally
...
\begin{proof}
  ...
  matched-conventions datum supplies the global or cross-target
...
\begin{enumerate}[wide=0pt,leftmargin=*]
  ...
  \item \emph{Tate Quillen equivalence (Beilinson--Drinfeld 2004,
    ...
    The lower-central Tate-pronilpotence hypothesis is an extra input
    on the strict full polydisk; the weighted
    completion or nilpotent truncation supplies it.  The strict
...
with the \(\mathfrak h\)-module structure supplied separately. The degreewise polynomial subalgebra
...
\begin{thm}[Classical bulk-to-boundary Hamiltonian map]\label{thm:bulk-boundary}
  ...
  full Poisson factorization-center morphism is a separate construction.
\end{thm}
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption[Notation diagram for the reduced open-line order-hbar raw commutator graph Gamma1]{Notation diagram for the reduced open-line order-\(\hbar\) raw
...
Proposition~\ref{prop:first-order-bracket}. An independent
Feynman-integral proof would require Costello graph data.
...
\cites{capelli}{howe-capelli}.  The three faces of the same scalar
...
closed-open BV construction must recover. Construction of that BV theory is
separate.  The classical degree-zero Hamiltonian bracket is the
...
Moyal Lie algebra by $\C[[\hbar]]$; the associative algebra quotient is a
...
\begin{rmk}
  ...
  $\hbar^{-1}\operatorname{ad}\widehat\mu_\epsilon$ is a derivation. The
...
\begin{proof}
  ...
  Capelli source context is \cites{capelli}{howe-capelli}.  The exact
...
\begin{cor}[Renormalized stable connected trace map]\label{cor:renormalized-stable-connected-map}
  ...
  stable class determined by the leading primitive trace.  The literal
  block-restriction inverse-limit element in the naive normal-ordered trace
  basis over plain \(\C[[\hbar]]\) is a different object.
\end{cor}
...
\begin{thm}[Reduced Moyal commutator: radial input and obstruction criterion]\label{thm:finite-n-reduced-moyal}
  ...
  Statement~\ref{stmt:app-radial-external-input} is the quantum shear
  primitive:
  for every \(a,b,N\),
  \[
    E_{a,b,N}
...
\begin{proof}
  ...
  Hamiltonian reduction, and has the generator image normalization
	  ...
	  Lemma~\ref{lem:app-radial-nonlinear-shear-obstruction} explains why
		  ...
		  pure-power radial images supply only the special cases. Therefore the
  ...
  falsification tests; the all-\(N\) source theorem remains separate.
  ...
  functorial all-interior splitting remains the missing construction.
\end{proof}
...
\begin{rmk}
  ...
  calculation as its input.  The brane
...
\begin{lem}[Finite-window obstruction for the full Hamiltonian sector]\label{lem:finite-window-hamiltonian-obstruction}
  ...
  the following finite-window obstructions to a naive finite-dimensional Lie
...
\begin{lem}[Tate Casimir obstruction for the full Hamiltonian sector]\label{lem:tate-casimir-obstruction}
  ...
  Consequently the full Hamiltonian sector lacks a strict BV pairing
  in this topology; a Costello coefficient kernel here requires a
  different coefficient category. The
  operator-level homotopy of
  Remark~\ref{rmk:linear-heat-versus-bv-kernel} supplies a BV propagator,
  BV Laplacian, and renormalized graph weights only after a coefficient
  category in which the Casimir converges has been chosen.
\end{lem}
...
\begin{prob}[Hamiltonian specialization datum for Costello perturbative BV]\label{prob:analytic-graph-realization}
  ...
  \begin{enumerate}
    ...
    \item choose the induced finite-scale gauge fixing, heat-kernel propagator,
      ...
      cancellation supplies a source theorem and a model; transfer requires
...
\begin{rmk}[Parabolic functoriality of the local model]\label{rmk:parabolic-functoriality}
  ...
  \[
    ...
    \{\varphi\in\mathrm{Symp}_{\mathrm{form}}(\C^2,0):
      \varphi^*(z_1)\in (z_1)\}.
  \]
	  Any cross-target transfer therefore supplies a parabolic-equivariant
...
\begin{thm}[Degree-zero Moyal target and decorated radial obstruction]\label{thm:phi-hbar-all-order}
  ...
  Naive normal-ordered block-restriction limits are different objects.

  The following data are proved under the stated finite-window and radial
  obstruction hypotheses.
  \begin{enumerate}
...
\begin{proof}
  ...
  \(\Omega^{\mathrm{rad}}_{a,b}=0\), equivalently
	  \(D^\square_{a,b}=C^+_{a,b}\partial_2\), with the signed-row obstruction
...
\begin{thm}[Componentwise quantum coefficient comparison]\label{thm:componentwise-quantum-coefficient-comparison}
  ...
  \begin{itemize}
    ...
    \item \emph{Vanishing criterion for the four named components:}
      the displayed equalities of items (1)--(4) hold on the
      scalar-contact balanced completion and on the
  ...
  centre morphism, and all-interior radial theorem are separate constructions
...
\begin{proof}
  ...
  action; transport from polynomial descendants is a separate problem.
...
\begin{prob}[Non-scalar quantum \(P_0\)-operation obstruction complex]\label{prob:quantum-p0-operation-realization}
  ...
  \]
  Thus the coefficient-current zero row supplies evidence only for the
...
\begin{prop}[Finite-window criterion for the first \(\theta_3\) row]\label{prop:theta-three-finite-window-criterion}
  ...
  \(C_{\theta_3}\) supplied apart from the differential
  \(d_MC_{\theta_3,M}=-\mathsf E_{\theta_3,M}\), scalar-zero value, and
  tower compatibility is merely notation; a counterterm consists of all
  three data.
  ...
  supplying a CE ancestor for \(\zeta_{M,\nu_3}\).  This is an exact
...
\begin{proof}
  ...
  the equation.  If the candidate is named while this differential entry is
  ...
  counterterm requires the additional column displayed below.
  ...
  is a separate row datum.  It changes the residual
  ...
  construction and all-graph QME solution require additional data.
\end{proof}
...
\begin{prob}[Tate-coefficient lift of the descendant cochain isomorphism]\label{prob:tate-coefficient-descendant-lift}
  ...
  Corollary~\ref{cor:wt-cotangent-lift}(a)--(d); the all-order quantum
...
\begin{proof}
  ...
  coefficient formula. The external finite-\(N\) radial-parts input remains
...
\begin{rmk}
  ...
  the reduced open-line Weyl model. The Costello closed-open graph theorem
  still requires an elliptic parent or dimensional
...
component extracted from this product. In this convention the order in $\hbar$
  ...
  \begin{tikzpicture}[scale=1.3]
    ...
    \node[font=\footnotesize, anchor=north, align=center] at (0,-2.05)
      {orientations $a=1,2$:\ contributions $-3(k)_2lm(n)_2$ and
        $+3k(l)_2(m)_2n$};
  \end{tikzpicture}
	  \caption[Third-order graphs Gamma3a and Gamma3b for order-hbar^3 boundary contractions]{The third-order graphs $\Gamma_{3a}$ (top) and $\Gamma_{3b}$
...
\begin{thm}[Costello first/third normalization recognition criterion]\label{thm:first-third-costello-normalizations}
  ...
  at order $\hbar^3$ every connected local term outside the
...
\begin{proof}
  ...
  Item (i): one edge, trivial edge automorphism group, weight $\hbar P/2$ for the
...
\begin{rmk}
  ...
  data of Problem~\ref{prob:analytic-graph-realization} upgrades the formal
  Weyl/Moyal normalization test above to a Costello graph statement.
\end{rmk}
...
\begin{prob}[Costello first/third graph-normalization obstruction datum]\label{prob:first-third-graph-normalizations}
  ...
  Costello specialization really lands on the Weyl/Moyal product, thereby
  fixing the deformation-quantization convention. The analytic problem is reduced
    ...
    \item identify the connected three-propagator brane-defect ribbon graphs
      whose tensor contraction is $P^3(f,g)$;
	    ...
	    \item compute the graph weight and automorphism factor after the
	      ...
	      $\frac1{24}$;
	    \item check that connected large-$N$ extraction preserves the factor of
	      \(N\), sign, and scalar moment-map normalization;
...
\begin{rmk}
  ...
  The full Lie-algebra Chevalley--Eilenberg BRST cohomology of
  \(\lie{gl}_N\) is a separate stack-level calculation.  A stack-level
...
The Capelli central charge $\hbar N[\bar c]$ is the projective curvature
$\Omega_{\mathrm{central}}|_{J(f)}$ of the modular line bundle at the
trace generator, realised by the Capelli central extension
$0\to\C\cdot K\to\widehat{\mathfrak g}_{\mathrm{cent}}\to\mathfrak g\to0$
of \eqref{eq:cent-extension}; the operational test is
$\{\operatorname{Tr}\phi_1,\operatorname{Tr}\phi_2\}=N$
(Remark~\ref{rmk:consistency-probes}, probe~(2)).
...
\begin{lem}[Closed--open mismatch lemma]\label{lem:omega-cocycle}
  ...
  exact on the unreduced algebra and obstructed after the constant-quotient.
  Equivalently:
  \begin{itemize}
...
\begin{proof}
  ...
  line.  Suppose some $\bar\eta\colon\bar A\to\C$ primitive of $[\bar c]$ existed;
...
the quantum face of the brane trace-map test. Linear trace generators have
only the order-\(\hbar\) correction.
...
\begin{thm}[Moyal survival of $\bar c$: the order-$\hbar$ Capelli shift]\label{thm:quantum-classical-anomaly}
  ...
  Theorem~\ref{thm:u1-center-anomaly-open} times \(\hbar\); higher-order
  Moyal corrections vanish.  Equivalently, the Rees quantisation
...
\begin{proof}
  ...
  trace-relation ideal at finite $N$ is separate from this calculation.
...
support-local Hamiltonian transfer has an obstruction on every choice of
one-pair principal-part datum, with obstruction class
...
completion and succeeds inside a controlled refinement: the weighted Tate
...
vector]\label{defn:tate-cotangent-lift-obstruction-vector}
  ...
  Costello--Li 2012 BCOV/HCS theorem belongs to a different hypothesis
...
D_{\oplus}\) of \ref{thm:strict-unweighted-obstruction}.  The construction
...
Matlis windows; the Bochner--Martinelli obstruction on each finite window
...
of the finite-window dualities.  Strict all-window support-local current
transfer on \(\widehat{\mathcal P}_\Pi\) is governed by
\(\operatorname{Ob}^\Pi_{\mathrm{BM}}\); this obstruction blocks the
strict transfer outside the pro-Matlis sector
(Theorem~\ref{thm:bmk-finite-window-obstruction}).
...
\]
of the Hadamard / Mittag-Leffler residual, measuring the obstruction to
...
vacua of the central families after the typed-discipline master
theorem of the next section organises the six holographic carriers,
names the open--closed comparison morphism, and pins each obstruction
class to one cell of the four-curvature taxonomy.

\input{typed-boundary-holographic}
...
the concrete-global-realizations catalogue of
Corollary~\ref{cor:concrete-global-realizations} extends this to ten
unconditional Stein holomorphic-symplectic targets
\((\C^2,\,D^2,\,T^*\C,\,T^*\mathbb H,\,B^4_r,\,\widehat{\C^2}_b,\,
   \C\times\C^*,\,\C^2\setminus\{0\},\,\C^2/\Gamma,\,\text{toric CY}_2)\)
plus four compact-containing realisations
\((T^*E,\,T^*\Sigma_{g\geq 1},\,\widetilde{\C^2/\Gamma}_{\mathrm{ADE}},\,
   \mathrm{Hilb}^N(\C^2))\) governed by the compact-frontier theorem,
with central-charge / period-class absorption into
\(\widehat{\mathfrak g}_{\mathrm{cent}}\) for the non-contractible
items.  In each case the trace map \(J\) realises the local theorem
on the chosen target.  Vertex-algebra avatars (Heisenberg,
\(\widehat{\mathfrak g}_k\), \(\beta\gamma\), \(\mathrm{Vir}_c\),
\(W_N\)) arise as curve restrictions of the bulk under the controlled
reduction of Remark~\ref{rmk:native-bulk-object-c2}, classified by the
class \(\mathrm L\) / class \(\mathrm M\) typed-boundary partition of
Remark~\ref{rmk:class-L-vs-class-M}.
