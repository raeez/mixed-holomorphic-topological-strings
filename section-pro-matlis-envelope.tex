...
Position observables on the brane are polynomial; the conjugate
momenta are residues.  The same local-cohomology object also gives the
...
and Theorem~\ref{thm:formal-two-dimensional-holomorphic-ope} uses
\(\rho^\Delta_{0,0}\) as the Cauchy kernel.
...
obstruction.  Fourier--Rees conjugation gives the only universal
...
\label{thm:app-defect-polarization}
  ...
  one-\(\psi\) shadow has the same obstruction after inverting
...
\label{def:app-matlis-principal-parts}
  ...
  locally finite topological vector spaces.  The space \(\mc P\) is a Hamiltonian-coadjoint module;
  the \(R\)-submodule structure of \(H^2_{\mathfrak m}(R)\,dz_1dz_2\) is
