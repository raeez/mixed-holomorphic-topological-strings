...
The formal-Darboux calculation first produces six native obstruction
classes
...
the weight-graded projections of one projective curvature only after a
comparison-unifying datum has been supplied: a Morita centre lift, a
...
complete target.  With such data fixed, the common target is the mixed
...
its finite-Matlis window, radial bidegree, \(\hbar\)-order,
...
determinant-line curvature interpretation requires an additional
determinant line and connection, and the
...
arity-one component is the supplied pre-comparison
\(\mathrm{preOCA}_N\),
...
compatible with the \(L_\infty\)-brackets.  Then
...
coordinates of a single curvature.
\end{prop}
...
\(\mathfrak Q_{w,\partial,\hbar}\) maps to the positive-\(\hbar\)-order
part, \(\mathfrak J_{\mathrm{fact}}\) maps to the \(P_0\)-cotangent
...
Theorem~\ref{thm:capelli-projective-anomaly}.  The curvature
...
\begin{thm}[Projected projective curvature criterion]\label{thm:six-coordinates-one-curvature}
Assume the comparison-unifying datum of
...
\(\hbar N[\bar c]\cdot\mathbf 1\) and no residual non-scalar
weight-zero component exactly on the vanishing locus of this residual
class, after compatible weight-zero primitives have been chosen.
\end{thm}
