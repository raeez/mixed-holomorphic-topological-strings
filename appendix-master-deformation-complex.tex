...
For the formal-Darboux Hamiltonian BF first Taylor datum
\(\tau_{N,1}^{\mathrm{tr}}\), assume the chosen Koszul lifts and, for a
nonzero linear-Poisson Hochschild realization, the admissible
twisted-formality and Poincar\'e--Birkhoff--Witt datum.  The product,
action, and \(P_0\) centrality
defects of
Definition~\ref{defn:low-degree-hochschild-centrality-defects} have zero
non-scalar projection on the scalar-reduced trace-sector first Taylor
coefficient.  The remaining low-degree weight-zero scalar projection is
the raw Rees/Weyl Capelli class
\(\hbar_{\mathrm{W}}\chi_N(K)[\bar c]
=\hbar_{\mathrm{W}}N[\bar c]\).  This verifies the
residual criterion of
Lemma~\ref{lem:nonscalar-weight-zero-vanishing} for the displayed
generator trace-sector rows.  The all-arity Morita centre lift and the
all-order Costello QME tower are the separate extension data governed by
their obstruction complexes.
\end{prop}
...
hypothesis.  On these first Taylor components the non-scalar weight-zero
defects of
Definition~\ref{defn:low-degree-hochschild-centrality-defects} are
strict identities.  The unshifted Gerstenhaber bracket of degree
\(-1\) sends a derivation and a multiplication cochain to multiplication
by the derivative,
\([X_x,M_a]_G=M_{X_x(a)}\), so with the realized
action \(\rho_x(a)=X_x(a)\)
\[
  \delta^{\mathrm{prod}}_{x,a}=[X_x,M_a]_G-M_{X_x(a)}=0,
  \qquad
  \delta^{\mathrm{act}}_{x|y}
  =[X_x,M_{O_y}]_G-M_{O_{\{x,y\}_{\mathfrak h}}}=0,
\]
the latter by the strict identity
\(X_x(O_y)=O_{\{x,y\}_{\mathfrak h}}\) of
...
normalized classical Lie identity is
\([X_x,X_y]_G=X_{\{x,y\}_{\mathfrak h}}\).  In the raw Rees/Weyl
...
\(\bar cK\).  Hence the non-scalar primitives are
...
displayed first-Taylor trace-sector rows have no non-scalar weight-zero
