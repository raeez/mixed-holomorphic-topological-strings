...
The matrix microscope on the formal symplectic plane $\C^2$ probes the
Hamiltonian Lie algebra
\[
  \mathfrak h=\bigoplus_{d\geq1}H_d/\mathbb C\cdot1,\qquad
  H_d=\mathrm{Span}\bigl\{z_1^az_2^b:a+b=d\bigr\},
\]
with Hamiltonian bracket $\{H_p,H_q\}\subset H_{p+q-2}$ and BV inverse
pairing the formal Casimir
\(
...
realizes the propagator as
$P_{\epsilon,L}=P_{\epsilon,L}^{\mathrm{base}}\widehat\otimes
C_{\mathfrak h\oplus\mathfrak h^\vee[1]}$, in which the bulk piece is
the standard heat-kernel inverse and the coefficient piece is exactly
$C_{\mathfrak h}$ extended to the cotangent direction.  The propagator
exists in $\mathcal E\widehat\otimes\mathcal E$ if and only if
$C_{\mathfrak h}$ exists as a coefficient kernel.
...
$C_{\mathfrak h}$ has nonzero $H_d^\vee$-component in every degree.
Hence
\(
  C_{\mathfrak h}\notin\mathfrak h_\Pi\widehat\otimes D_\oplus.
\)
By Lemma~\ref{lem:tate-casimir-obstruction} this obstruction is
categorical: no rescaling of the bases inside the strict pair moves
$C_{\mathfrak h}$ from $\prod_d(H_d\otimes H_d^\vee)$ into the strict
sum-with-product
$\bigl(\prod_dH_d\bigr)\widehat\otimes\bigl(\bigoplus_dH_d^\vee\bigr)$;
Theorem~\ref{thm:strict-unweighted-no-go} below sharpens this to the
finite-window form. The cure must change the coefficient \emph{category}.

The brane microscope acts on the boundary observable algebra
$\mathrm{PV}_{\mathrm{coord}}(A_{\partial,\mathrm{coord}})$ by the
Hamiltonian-and-cotangent generators $\{O_f,\theta_f\}_{f\in\bar A}$, the
Schouten differential $d_\pi$, the Schouten bracket $[-,-]_{\mathrm{Sch}}$,
and the pointwise product. The minimal coefficient subspace closed under
these three operations and large enough to host the Casimir is the
weighted Tate envelope $B_w$ of
Theorem~\ref{thm:wt-canonical-habitat} below: the weighted-Tate topology
is the local energy/RG topology of the brane microscope.

\subsection*{Strict Tate pair: failure of the Casimir kernel}

\begin{rmk}[Strict Tate Casimir obstruction]\label{rmk:diagnosis-unweighted-casimir}
  The product/direct-sum Tate pair has Hamiltonian Lie algebra and
  continuous dual
  ...
  \mathfrak h^\vee_{\mathrm{cont}}$, which is
  Lemma~\ref{lem:tate-casimir-obstruction}. The cure changes the
  coefficient \emph{category}: the second factor must itself be a
  product, with the change of dualization inserted by an intermediate
  weight system that is itself a Mittag-Leffler inverse-limit object.
\end{rmk}
...
\begin{rmk}[Costello's coefficient requirements]\label{rmk:costello-coefficient-requirements}
  ...
  i.e. the Casimir $C_{\mathfrak h}$ together with its dual coevaluation.
  Hence the full kernel exists in $\mathcal E\widehat\otimes\mathcal E$
  exactly when $C_{\mathfrak h}$ exists as a coefficient kernel.
...
\subsection*{Degree weights and the canonical habitat \(B_w\)}

The brane microscope chooses the coefficient category. A degree weight
on the Hamiltonian Lie algebra is bracket-tame, windowwise
coadjoint-continuous, and Mittag-Leffler-coevaluating; the resulting
weighted Tate envelope is closed under exactly the three operations
$d_\pi$, $[-,-]_{\mathrm{Sch}}$, and $\cdot$ that the brane microscope
acts by, and the Casimir converges inside it. This is
Theorem~\ref{thm:wt-canonical-habitat}: $B_w$ is the minimal RG-stable
analytic completion forced by the matrix probe.
...
\begin{defn}[The weighted-Tate envelope \(B_w\)]\label{defn:wt-Bw-envelope}
  ...
  \[
    B_w
      :=\overline{\bigl\langle
          \{\mathcal O_f,\theta_f\}_{f\in\bar A}
  ...
  the matrix-microscope generators
  $\{\mathcal O_f,\theta_f\}_{f\in\bar A}$, closed under the Schouten
...
\begin{thm}[Canonical weighted-Tate habitat]\label{thm:wt-canonical-habitat}
  Let $w$ be a degree weight as in
  Definition~\ref{defn:wt-degree-weight} and let $B_w$ be the envelope
  of Definition~\ref{defn:wt-Bw-envelope}. The brane microscope on the
  formal symplectic plane $\C^2$ selects $B_w$ as the minimal RG-stable
  coefficient habitat: every condition imposed by Costello's heat-kernel
  BV calculus and by the matrix-probe operations is satisfied internally
  to $B_w$.
  \begin{enumerate}[label=(\alph*)]
    \item\label{wt:canon:closure}
      \emph{Closure under the brane-microscope operations.} $B_w$ is
    ...
    \item\label{wt:canon:casimir}
      ...
      \]
      converges to an element of the weighted continuous tensor product
      $\mathfrak h_w\widehat\otimes
      \mathfrak h^{\vee,w}_{\mathrm{cont}}\subset B_w\otimes B_w$, and
      realizes Costello requirement~\ref{costello:CC1}: the BV
      coefficient inverse pairing on the cotangent extension.
    \item\label{wt:canon:propagator}
      ...
      $[Q,P^w_{\epsilon,L}]=K^w_\epsilon-K^w_L$ and Costello regularity
      \ref{costello:CC2}: the family is continuous in $(\epsilon,L)$ in
      the weighted Tate topology, and the limit
      $L\to\infty,\epsilon\to0^+$ exists as a distributional kernel after
      the standard bulk counterterm subtraction.
    \item\label{wt:canon:rg}
      ...
      corresponding spacetime graph weight, with constants supplied by
  ...
  Conversely, $B_w$ is minimal: any complete filtered subspace
  $B'\subset\mathrm{PV}_{\mathrm{coord}}(A_{\partial,\mathrm{coord}})$
  satisfying \ref{wt:canon:closure}, containing
  $\{\mathcal O_f,\theta_f\}_{f\in\bar A}$, and supporting the kernel
  $C_{\mathfrak h,w}$ of \ref{wt:canon:casimir}, contains $B_w$. The
  weighted-Tate topology on $B_w$ is the local energy/RG topology of the
  matrix microscope: it is the unique coefficient topology in which both
  the brane-defect heat-kernel propagator and the Schouten algebra
  generated by $\{\mathcal O_f,\theta_f\}_{f\in\bar A}$ remain inside the
  same complete category.
\end{thm}

\begin{proof}
  \ref{wt:canon:closure}: the Schouten bracket of two polynomials in
  $\mathcal O_f$ and the Schouten differential acting on
  $\theta_f$ produce, in coordinates,
  \[
  ...
  (cf.~main.tex). The bracket-tame estimate \ref{w:W1} bounds, on each
  finite Tate window, the resulting polynomial-degree shift by a finite
  constant; the windowwise coadjoint-continuity \ref{w:W2} bounds the
  cotangent leg. Multiplication is closed by definition of the
  envelope. The closure $\overline{(\cdot)}$ in
  Definition~\ref{defn:wt-Bw-envelope} adjoins all $d_\pi$-,
  $[-,-]_{\mathrm{Sch}}$-, and product-completions; that closure is
  itself stable under the same operations because each operation is
  continuous in the weighted Tate topology by \ref{w:W1}--\ref{w:W2}.
  ...
  $\{\mathcal O_f,\theta_f\}$ contains all $d_\pi$-, bracket-, and
  product-iterates of these generators, hence the algebraic envelope.
  Supporting $C_{\mathfrak h,w}$ as a continuous element of
  $B'\otimes B'$ forces $B'$ to be complete in the weighted Tate
  topology, hence to contain the closure $B_w$.
\end{proof}

\begin{rmk}[Brane microscope as RG topology]\label{rmk:wt-brane-microscope-rg}
  The matrix microscope reads the brane line through the Schouten
  algebra generated by
  $\{\mathcal O_f,\theta_f\}_{f\in\bar A}$, with $d_\pi$ and the Schouten
  bracket implementing infinitesimal gauge variation. Costello's
  finite-scale heat-kernel propagator interlaces this Schouten algebra
  with the bulk operator $\int_\epsilon^L Q^{\mathrm{GF}}e^{-tD}dt$,
  contracting against the coefficient pairing
  $\langle\mathfrak h,\mathfrak h^\vee_{\mathrm{cont}}\rangle$. Both
  structures must close inside the same coefficient category. The
  bracket-tame condition \ref{w:W1} is precisely that closure for the
  Schouten algebra; the Mittag-Leffler coevaluation \ref{w:W3} is
  precisely that closure for the heat-kernel propagator;
  Theorem~\ref{thm:wt-canonical-habitat} is the simultaneous statement.
  The weighted-Tate topology is therefore not a workaround chosen for
  analytic convenience: it is the local energy/RG topology of the brane
  microscope, the minimal coefficient category in which $\{O_f,
  \theta_f\}$, $d_\pi$, $[-,-]_{\mathrm{Sch}}$, and $P_{\epsilon,L}$ all
  live.
\end{rmk}

The remaining propositions of this subsection prove the analytic content
of the items \ref{wt:canon:casimir}--\ref{wt:canon:rg}: convergence of
the weighted Casimir, extension of the Costello heat-kernel propagator
to the weighted complex, and graphwise stability of the RG flow. The
brane-defect QME counterterms and locality compatibility are isolated as
the content of Problem~\ref{prob:weighted-rg-locality}; the proven
analytic statement is the canonical-habitat theorem above, and the open
problem is the all-order vanishing of the brane-defect anomaly inside
$B_w$.
...
\begin{proof}
  ...
  The preceding comparison is an equality in the one-dimensional scalar
...
\begin{cor}[Equivariant denominators do not decide the Costello QME]\label{cor:wt-omega-denominators-qme-boundary}
  ...
  Once equivariant brane-defect Costello data are supplied, in the
  sense of
...
\begin{thm}[Weighted RG flow, graphwise compatibility]\label{thm:wt-rg-compatibility}
  ...
  obstruction class require the locality input of
  Problem~\ref{prob:weighted-rg-locality}.
\end{thm}
...
\begin{prob}[Costello-locality compatibility for the weighted RG flow]\label{prob:weighted-rg-locality}
  \emph{Construction problem.}  Construct a compatible counterterm
  ...
  \emph{Available inputs.}  Estimate \ref{w:W1} bounds vertex weights;
  \ref{w:W2} bounds coadjoint legs graph by graph;
  Theorem~\ref{thm:hadamard-mittag-leffler} controls wave-front and
  finite-window locality estimates.  The construction problem is the
  exact further datum that turns Hadamard control into anomaly
  cancellation.
\end{prob}
...
The transport groupoid identifies any two admissible weights on the
finite-window Mittag--Leffler sector.  Classes detecting the quotient
\(D_{\Pi,w}/D_{\oplus}\) are outside this sector, and a \(q\to1^+\)
endpoint to the strict product/direct-sum Tate pair is excluded by
Theorem~\ref{thm:strict-unweighted-no-go}.
...
\label{defn:wt-nonscalar-qme-tower}
  ...
  differential are the transported finite-window structures supplied by
	  the weighted RG criterion; they are not naive projected Hamiltonian brackets.
  The inverse system
...
\label{prop:wt-raw-kappa-cochain-obstruction}
  ...
  \]
  Thus the regular-density graph layer supplies a proved
...
\label{prop:wt-first-regular-density-residual-criterion}
  ...
  If the criterion does not hold, the regular-density branch is not
...
\begin{proof}
  ...
  Without this criterion, the only invariant operation supplied by the
...
\label{prop:wt-regular-density-curvature-criterion}
  ...
  is supplied for this evaluated curvature.  The closed-exchange
  solution is
  \[
    W_{n_0,M}
      =
...
\begin{proof}
  ...
  is proved as a distributional graph layer, but it is not a
...
\label{thm:wt-wavefront-distributional-primitive-cone}
  ...
  complex; it is not an element of the original Costello counterterm
  complex \(\mathcal Q^\bullet_{w,M}(I)\), and the construction does
  not identify a singular current with a smooth density.  A primitive
...
\subsection*{Local regulator boundary}
...
\begin{rmk}[Local-disk regulator datum]\label{rmk:wt-no-transfer-vol3}
  ...
  bulk $\R^2\times\C^2$.  This regulator datum addresses the analytic
  obstruction named in
