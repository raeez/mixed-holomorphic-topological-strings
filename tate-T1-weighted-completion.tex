...
\]
where \(P_{\epsilon,L}^{\mathrm{base}}\) is the kernel of
\[
  \int_\epsilon^L Q^{\mathrm{GF}}e^{-tD}\,dt
\]
on
\(\Omega^\bullet(\R^2)\widehat\otimes
\Omega^{0,\bullet}(\C^2)\), and the
...
\subsection*{Strict Tate pair: Casimir-kernel obstruction}
...
\begin{rmk}[Costello's coefficient requirements]\label{rmk:costello-coefficient-requirements}
  ...
  \[
    \int_\epsilon^L Q^{\mathrm{GF}}e^{-tD}\,dt,
  \]
  continuous by Remark~\ref{rmk:linear-heat-versus-bv-kernel}. The
  coefficient piece is the BV inverse pairing on
  ...
  \begin{enumerate}[label=(C\arabic*)]
    ...
    \item\label{costello:CC2} finite-scale regularity in the cutoff
      parameters $\epsilon$ and $L$, in the sense that the heat-kernel
      propagator $P_{\epsilon,L}$ is a continuous family in the
      coefficient topology for \(0<\epsilon\leq L<\infty\); if the
      endpoint \(L\to\infty,\epsilon\to0^+\) or counterterm subtraction
      is invoked, the renormalized endpoint and the counterterm tower
      are additional Costello data, not consequences of coefficient
      admissibility;
...
\subsection*{Degree weights and the fixed-weight completion \(B_w\)}
...
\begin{defn}[Weighted Tate coefficient pair]\label{defn:wt-coefficient-pair}
  ...
  \prod_{d\leq w_0}H_d^{(w)})$, and define the \emph{weighted BV
  coefficient partner}
  ...
  $(\mathfrak h_w,D^{\mathrm{BV}}_w)$.  The second factor is the
  product coefficient module used as the BV receiver; it replaces the
  strict continuous dual only for the coevaluation kernel in this
  declared coefficient category.
\end{defn}

\begin{rmk}[Topological content]\label{rmk:wt-topology-content}
  Both $\mathfrak h_w$ and \(D^{\mathrm{BV}}_w\) are
  ...
  replaces the direct-sum strict dual in the coevaluation slot by a
  symmetric Mittag-Leffler product coefficient module.  This is an
  additional coefficient category for the BV kernel; the ordinary
  continuous dual of $\mathfrak h_w$ remains the strict Tate dual.
\end{rmk}
...
\begin{defn}[The weighted-Tate envelope \(B_w\)]\label{defn:wt-Bw-envelope}
  ...
  $\mathfrak h_w\widehat\otimes D^{\mathrm{BV}}_w$.
...
\begin{thm}[Initial weighted-Tate envelope for a fixed weight]\label{thm:wt-canonical-envelope}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item\label{wt:canon:casimir}
      ...
      $\mathfrak h_w\widehat\otimes
      D^{\mathrm{BV}}_w$. Under the
      $P_0$-algebra map $e_{d,i}\mapsto O_{e_{d,i}}$,
      $e^i_d\mapsto\theta^{e^i_d}$ of
      Theorem~\ref{thm:reduced-ce-pv-central-operation}, the first
      factor lies in \(B_w\) and the second factor is read in the
      declared BV receiver \(D_{B_w}=D^{\mathrm{BV}}_w\).  Thus the
      kernel lies in
      \(B_w\widehat\otimes D_{B_w}\), not in
      \(B_w\widehat\otimes B_w\) as a strict continuous tensor unless
      an additional identification of \(D_{B_w}\) with \(B_w\) has been
      supplied.  It realizes Costello requirement~\ref{costello:CC1} as
      the BV inverse pairing on the cotangent extension.
    \item\label{wt:canon:propagator}
      \emph{Finite-scale heat-kernel propagator extension.} The Costello heat-kernel
      propagator
      \[
        P^w_{\epsilon,L}
      ...
      $[Q,P^w_{\epsilon,L}]=K^w_\epsilon-K^w_L$ and finite-scale
      Costello regularity~\ref{costello:CC2}.  The coefficient piece
      $C_{\mathfrak h,w}$ is counterterm-free; it is the fixed
      Mittag-Leffler kernel of \ref{wt:canon:casimir}.  The
      renormalized endpoint and the mixed brane-defect QME counterterm
      tower are the separate data of
      Problem~\ref{prob:weighted-rg-locality}.
    ...
    \item\label{wt:canon:phi}
      ...
      $\mathfrak g_w=\mathfrak h_w\ltimes D^{\mathrm{BV}}_w[1]$
  ...
  equipped with the fixed declared receiver
  \(D_{B'}=D^{\mathrm{BV}}_w\),
  satisfying \ref{wt:canon:closure}, containing
  $\{O_f,\theta_f\}_{f\in\bar A}$, and containing
  \(C_{\mathfrak h,w}\in B'\widehat\otimes D_{B'}\) as a continuous
  element of the weighted Tate topology, contains \(B_w\).
\end{thm}
...
\begin{proof}
  ...
  The initial generating set is
  \(S=\{O_f,\theta_f:f\in\bar A\}\).  Definition~\ref{defn:wt-Bw-envelope}
  takes the smallest weighted-closed subspace containing \(S\) and
  closed under the three operations \(d_\pi\), the Schouten bracket, and
  pointwise product.  Hence, if \(x,y\in B_w\) are limits of finite
  expressions \(x_n,y_n\) in those operations, continuity gives
  \(x_ny_n\to xy\), \(d_\pi x_n\to d_\pi x\), and
  \([x_n,y_n]_{\mathrm{Sch}}\to[x,y]_{\mathrm{Sch}}\); the limits again
  lie in the same weighted closure.  The estimates
  \ref{w:W1}--\ref{w:W2} are precisely the continuity input for
  \(d_\pi\) and the bracket, while multiplication is continuous in the
  completed symmetric topology used in the envelope.
  ...
  Containing
  \(C_{\mathfrak h,w}\in B'\widehat\otimes D_{B'}\) as a continuous
  element in the fixed declared kernel pair requires \(B'\) to be
  complete in the weighted Tate topology, hence to contain the closure
  \(B_w\).
\end{proof}
...
\begin{defn}[Category of admissible weighted-Tate completions]\label{defn:wt-Adm-category}
  ...
  The category $\mathsf{Adm}_w$ has objects: pairs
  \((B',D_{B'})\), where
  \(D_{B'}=D^{\mathrm{BV}}_w\) is the fixed declared BV receiver and
  \(B'\subset\mathrm{PV}_{\mathrm{coord}}(A_{\partial,\mathrm{coord}})\)
  is a complete filtered subspace satisfying
  \begin{enumerate}[label=(A\arabic*)]
    ...
    \item\label{adm:casimir} \emph{(Casimir convergence)} the weighted
      ...
      \(B'\widehat\otimes D_{B'}\) in the weighted Tate topology;
  ...
  $[-,-]_{\mathrm{Sch}}$, the pointwise product, the weighted Casimir in
  the declared kernel pair, and the heat-kernel propagator.
\end{defn}
...
\begin{rmk}\label{rmk:wt-canonical-as-initiality}
  ...
  intrinsic to $(\mathfrak h_w,D^{\mathrm{BV}}_w,
  P^w_{\epsilon,L})$.
\end{rmk}
...
\begin{prop}[Casimir convergence]\label{prop:wt-casimir-convergence}
  ...
  weighted category.  More explicitly, let
  \(C_{\mathfrak h,w}^{\leq M}\) be the truncation of
  \eqref{eq:wt-casimir-weighted} to \(d\leq M\), and let \(p_M\) be the
  \(M\)-th finite-window projection of the completed tensor product.
  Then, for \(M'\geq M\),
  \[
    p_M(C_{\mathfrak h,w}^{\leq M'}-C_{\mathfrak h,w}^{\leq M})=0.
  \]
  If the finite window is given the projective tensor norm induced by
  the \(\ell^1\)-norms in the weighted bases
  \(\{w(d)e_{d,i}\}_{d\leq M}\) and
  \(\{w(d)^{-1}e^i_d\}_{d\leq M}\), then
  \[
    \bigl\|p_M(C_{\mathfrak h,w})\bigr\|_{\pi,M}
      \leq
    \sum_{d=1}^{M}\dim H_d
      =
    \frac{M(M+3)}{2}.
  \]
  These two finite-window estimates are the asserted convergence
  statement; no summability in the strict direct-sum dual is claimed.
\end{prop}
...
\begin{proof}
  ...
  sum by the terms in degrees $w_0+1,\ldots,w_0'$, and the projection
  back to the \(w_0\)-window removes precisely those added terms.  Thus
  for the partial sums \(C_{\mathfrak h,w}^{\leq M}\) one has
  \(p_M(C_{\mathfrak h,w}^{\leq M'}-C_{\mathfrak h,w}^{\leq M})=0\) for
  every \(M'\geq M\).  This is the Cauchy criterion in the strict
  Mittag--Leffler topology of
  \(\mathfrak h_w\widehat\otimes
  D^{\mathrm{BV}}_w\), so the inverse limit exists
  by Definition~\ref{defn:wt-coefficient-pair} and
  \ref{w:W3} of Definition~\ref{defn:wt-degree-weight}.

  The stated norm estimate is finite-window bookkeeping.  In degree
  \(d\), the homogeneous polynomials in two variables have
  \(\dim H_d=d+1\).  In the weighted \(\ell^1\)-basis, each simple tensor
  \((w(d)e_{d,i})\otimes(w(d)^{-1}e^i_d)\) has projective norm \(1\);
  the weights cancel between the two factors.  Hence
  \[
    \bigl\|p_M(C_{\mathfrak h,w})\bigr\|_{\pi,M}
      \leq
    \sum_{d=1}^{M}(d+1)
  ...
  D^{\mathrm{BV}}_w$, supported in finitely many
  Tate windows, gives
  \[
    (\phi\otimes 1)(C_{\mathfrak h,w})
      =
      \sum_{d\in \operatorname{supp}\phi}\sum_i
      \phi\!\bigl(w(d)e_{d,i}\bigr)\,w(d)^{-1}e^i_d,
  \]
  a finite sum which is the element of
  \(D^{\mathrm{BV}}_w\) corresponding to evaluation
  against \(\phi\). Hence $C_{\mathfrak h,w}$
...
\begin{prop}[Finite-scale heat-kernel propagator extension to the weighted pair]\label{prop:wt-propagator-extends}
  Let $w$ be a degree weight as in
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \item finite-scale Costello regularity \ref{costello:CC2}: the family is
      continuous in $(\epsilon,L)$ in the weighted Tate topology;
    \item the coefficient direction introduces no new spacetime singular
      support: every finite-scale wavefront and support statement for
      \(P_{\epsilon,L}^{\mathrm{base}}\) is tensored with the fixed
      Mittag--Leffler coefficient kernel \(C_{\mathfrak h,w}\).  The
      endpoint \(L\to\infty,\epsilon\to0^+\), locality, and QME
      counterterm compatibility for the mixed brane-defect theory remain
      the content of Problem~\ref{prob:weighted-rg-locality}.
  \end{enumerate}
\end{prop}
...
\begin{proof}
  ...
  constructed in \cite{costello-renormalization}*{Ch.~5}. The finite-scale
  BV homotopy identity is the following commutator calculation.  With
  ...
  $\mathfrak h_w[1]\oplus D^{\mathrm{BV}}_w[2]$.
  Therefore
  ...
  Item~(3) is a tensor-product support statement.  The coefficient
  kernel \(C_{\mathfrak h,w}\) is independent of the base variables and
  belongs to the fixed weighted coefficient tensor product.  Tensoring
  the base finite-scale kernel with it cannot create a new base
  wavefront direction or a new collision stratum.  This proves
  finite-scale kernel regularity and absence of new coefficient
  wavefront directions.  The endpoint distribution, brane-defect
  locality, and QME counterterm obstruction classes are separate
...
\begin{rmk}[Brane defect]\label{rmk:wt-brane-defect}
  ...
  $$\mathcal E_w^{\partial}=\Omega^\bullet(\R)\widehat\otimes\bigl(
    \mathfrak h_w[1]\oplus D^{\mathrm{BV}}_w[2]\bigr).$$
...
\begin{cor}[Equivariant Costello QME obstruction vector]\label{cor:wt-omega-denominators-qme-boundary}
  ...
  brane-defect QME counterterm tower exactly on the common zero locus of
  this vector, and only there.
\end{cor}
...
\begin{proof}
  ...
  to carry a QME counterterm tower.
\end{proof}
...
\begin{proof}
  ...
  \mathfrak h_w\widehat\otimes D^{\mathrm{BV}}_w\to\C$
...
\begin{proof}
  ...
  graphwise well-definedness to a renormalized QME counterterm tower is
  precisely the residual analytic problem.
\end{proof}

\subsection*{Cotangent-lift inside \(B_w\)}
...
\begin{cor}[Cotangent-lift inside the weighted coefficient category]\label{cor:wt-cotangent-lift}
  ...
  lift $\Phi_{\hbar_{\mathrm{QME}},\mathrm{loc}}^w$ is
...
\begin{prop}[Weighted QME lift under obstruction vanishing]\label{prop:wt-qme-lift-obstruction-vanishing}
  ...
  CE/PV central-operation map $\Phi_{\hbar_{\mathrm{QME}}}^{(0)}$ governed by
  Theorem~\ref{thm:phi-hbar-all-order} has a weighted-continuous
  cotangent extension
  $$\Phi_{\hbar_{\mathrm{QME}},\mathrm{loc}}^{w}\colon
...
\begin{proof}
  ...
  Problem~\ref{prob:weighted-rg-locality}.  With that assumption
  imposed, the formal $\hbar_{\mathrm{QME}}$-adic construction of
...
\begin{prob}[Costello-locality compatibility for the weighted RG flow]\label{prob:weighted-rg-locality}
  ...
  \begin{enumerate}[label=(L\arabic*)]
    \item each $C_{n,M}$ is a local functional in
      $\mathcal O_{\mathrm{loc}}^{\hbar_{\mathrm{QME}}^n}(\mathcal E_w^{\leq M})$ of
      bounded vertex polynomial degree;
    \item the truncations are compatible:
      $\pi_{M,N}C_{n,M}=C_{n,N}$ for $M\geq N$;
    \item the recursion cancels the QME curvature order by order.
  \end{enumerate}
...
\begin{cor}[Descendant cochain comparison under weighted QME vanishing]\label{cor:wt-descendant-qme-vanishing}
  ...
  $$\Phi_{\hbar_{\mathrm{QME}},\mathrm{loc}}^{(0,1)}\colon
    C^\bullet_{\mathrm{CE},\mathrm{cont}}\bigl(
    \mathfrak h_{w,\hbar_{\mathrm{QME}}}\ltimes D^{\mathrm{BV}}_{w,\hbar_{\mathrm{QME}}}[1]
    \bigr)^{(0)+(1)}[[\hbar_{\mathrm{QME}}]]
    \xrightarrow[\mathrm{QME\ obstruction}=0]{\sim}
    Z^{P_{0,\hbar_{\mathrm{QME}}}}_{\mathrm{fact}}\bigl(\mathrm{Obs}^\hbar_{\mathrm{open}}
...
\begin{rmk}[Sector restriction]\label{rmk:wt-sector-restriction}
  ...
  ambient polynomial Hamiltonian dual to the weighted BV coefficient partner
  $D^{\mathrm{BV}}_w=\prod_d w(d)^{-1}H_d^\vee$.
  The strict polynomial Hamiltonian dual
  $\mathfrak h^\vee_{\mathrm{cont}}=\bigoplus_d H_d^\vee$ is a
  \emph{dense subspace} of $D^{\mathrm{BV}}_w$
...
\begin{defn}[Regulator-admissible finite-window sector]\label{defn:regulator-admissible-sector}
  ...
  \begin{enumerate}[label=(A\arabic*)]
    \item The transition maps in the coefficient and local-functional
      ...
      complexes.
    \item At each fixed \(\hbar_{\mathrm{QME}}\)-order and for each fixed Costello
...
\label{defn:wt-exponential-window-tower}
  ...
  \[
    (\mathfrak h^0_{\leq M},D^0_{\leq M})
...
\begin{stmt}[Weighted RG transport criterion]\label{stmt:weighted-rg-criterion}
  ...
  \begin{enumerate}[label=(G\arabic*)]
    ...
    \item At every fixed \(\hbar_{\mathrm{QME}}\)-order and for every fixed Costello
...
\begin{thm}[Diagonal transport of finite-degree local observables]
\label{thm:wt-finite-degree-observables}
  ...
  \(d\leq M\).  Then \(R^M_{q,q'}\) is the diagonal transport between
...
\label{defn:wt-nonscalar-qme-tower}
  ...
  \begin{aligned}
    A_{\partial,\hbar_{\mathrm{QME}}}^{\mathrm{pp},w,M}(I)
      =
      \widehat{\mathbf S}_{\C[[\hbar_{\mathrm{QME}}]]}
      \bigl(\Omega^\bullet_c(I)\otimes\prod_{d\leq M}w(d)H_d[[\hbar_{\mathrm{QME}}]]\bigr)
      \widehat\otimes
      \widehat{\mathbf S}_{\C[[\hbar_{\mathrm{QME}}]]}
      \bigl((\mathcal D'_c(I)\widehat\otimes
        D_{w,\leq M}[[\hbar_{\mathrm{QME}}]])[1]\bigr).
  \end{aligned}
  \]
  ...
  \]
  where
  \(d_M=Q+\{I^w_{0,M},-\}_{\hbar_{\mathrm{QME}},M}\).  The bracket, interaction, and
  ...
  \(\pi_{M,N}\) for \(M\geq N\), together with the \(\hbar_{\mathrm{QME}}\)-adic
...
\label{thm:wt-nonscalar-counterterm-criterion}
  ...
  counterterm system has been chosen through order \(\hbar_{\mathrm{QME}}^{n-1}\), and
  let
  \[
  ...
  \[
    \mathfrak o^{\mathrm{ns}}_{n,M}
      =
      [\hbar_{\mathrm{QME}}^n]\left(
        \operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}},M}
        +d_MC_{<n,M}
        +\frac12\{C_{<n,M},C_{<n,M}\}_{\hbar_{\mathrm{QME}},M}
      \right),
  \]
  ...
  \(\hbar_{\mathrm{QME}}\)-adic compatible counterterm
  \(C_{\hbar_{\mathrm{QME}},w}=\sum_{n\geq1}\hbar_{\mathrm{QME}}^n C_{n,w}\) in the admissible
...
\begin{proof}
  The lower-order counterterms solve the QME modulo \(\hbar_{\mathrm{QME}}^n\).  Applying
  ...
  Suppose compatible counterterms \(C_{n,M}\) exist.  The equation
  \(d_MC_{n,M}=-\mathfrak o^{\mathrm{ns}}_{n,M}\) makes
  \([\mathfrak o^{\mathrm{ns}}_{n,M}]=0\) in every
  \(H^1(\mathcal Q^\bullet_{w,M}(I))\).  The compatibility equations
  \(\pi_{M,N}C_{n,M}=C_{n,N}\) make every primitive-difference class
  \([\pi_{M,N}C_{n,M}-C_{n,N}]\) equal to zero.  Hence the class in the
  Roos cokernel computing
  \(\varprojlim^1_MH^0(\mathcal Q^\bullet_{w,M}(I))\) is zero.
  ...
  commute with truncation.  On the cofinal successor tower set
  \(A_M=H^0(\mathcal Q^\bullet_{w,M}(I))\) and
  \[
    \Delta_Q\colon\prod_M A_M\longrightarrow\prod_M A_M,\qquad
    \Delta_Q((a_M)_M)
      =\bigl(a_M-\pi_{M+1,M}a_{M+1}\bigr)_M .
  \]
  The classes
  \(\delta_M=[\pi_{M+1,M}c_{n,M+1}-c_{n,M}]\) form the Roos
  \(1\)-cocycle \((\delta_M)_M\), and its cokernel class is
  \(\lambda_n\).  If \(\lambda_n=0\), choose degree-zero cocycles
  \(a_M\) and representatives of degree-zero boundaries so that
  \(\delta_M=a_M-\pi_{M+1,M}a_{M+1}\) in \(A_M\).  Replacing
  \(c_{n,M}\) by \(c_{n,M}+a_M\), and then by boundary representatives
  in \(\mathcal Q^0_{w,M}(I)\), gives primitives \(C_{n,M}\) satisfying
  \(\pi_{M+1,M}C_{n,M+1}=C_{n,M}\).  Compatibility for arbitrary
  \(M\geq N\) follows by iteration, and the inverse-limit element is
  \(C_{n,w}\).

  The assertion about the \(H^0\)-Mittag--Leffler hypothesis is the
  following calculation.  For a Mittag--Leffler inverse system
  \((A_M,\pi_{M+1,M})\) of vector spaces, replace each \(A_M\) by the
  stable image of \(A_{M'}\to A_M\) for \(M'\gg M\); the resulting
  cofinal subsystem has surjective transition maps.  Given
  \(b=(b_M)_M\in\prod_M A_M\), choose \(a_1\) arbitrarily and then lift
  \(a_M-b_M\) to \(a_{M+1}\) under the surjective map
  \(A_{M+1}\to A_M\).  Then \(\Delta_Q(a)=b\), so
  \(\operatorname{coker}\Delta_Q=\varprojlim^1 A_M=0\).
  Weight invariance follows from
...
\label{defn:wt-regular-density-closed-exchange-complex}
  ...
  filtered \(\C[[\hbar_{\mathrm{QME}}]]\)-span, inside
...
\label{prop:wt-raw-kappa-cochain-obstruction}
  Let
  \(\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}\) be the finite-window
  ...
  \(\mathfrak Q^\bullet_{w,\partial,\hbar_{\mathrm{QME}}}(I)\).  If one tries to take the
  regular-density CE source itself as the closed-exchange complex and
  \(\Xi_M=\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}\), then the exact
  obstruction to an ordinary cochain map into
  \(\mathcal Q^\bullet_{w,M}(I)\) is the pair
  \[
    \widehat\sigma_{\mathrm{sc},M}
      \kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I},
    \qquad
    d_{\mathbf K}\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}
      =
      d_M\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}
      -
      \kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}d_{\mathrm{CE}}
  \]
  ...
  is required to be a \(P_{0,\hbar_{\mathrm{QME}}}\)-central-operation kernel, the
...
\begin{proof}
  ...
  \(\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}\) has degree zero, the
  ...
  \(P_{0,\hbar_{\mathrm{QME}}}\)-central-operation kernel is a Maurer--Cartan element in
...
\label{thm:wt-closed-exchange-counterterm-criterion}
  ...
  \[
    \Delta_X((a_M)_M)
      =
      \bigl(a_M-\rho^X_{M+1,M}a_{M+1}\bigr)_M,
  \]
...
\begin{proof}
  ...
  \(([u_M])_M\) by the negative of the Roos coboundary \(\Delta_X\).  Thus
  the displayed class \(\mu^{\mathrm{cl}}_n(\alpha)\) is independent of the
  choices.  In
  ...
  their cohomology classes by the negative of the Roos coboundary
  \(\Delta_Q\).  Hence the
  ...
  For either \(H^0\)-tower, write the Roos differential as
  \(\Delta((a_M)_M)=(a_M-\rho_{M+1,M}a_{M+1})_M\).  The
  Mittag--Leffler condition lets one replace the tower by the cofinal
  tower of stable images, whose transition maps are surjective.  The
  recursive lifting of \(a_M-b_M\) to \(a_{M+1}\) solves
  \(\Delta(a)=b\) for every \(b\in\prod_MH^0_M\).  Hence
  \(\operatorname{coker}\Delta=\varprojlim^1H^0_M=0\).  Therefore an
  \(H^0\)-Mittag--Leffler hypothesis on the closed-exchange tower kills
  the representative obstruction \(\mu^{\mathrm{cl}}_n\), and the
  corresponding hypothesis on the non-scalar counterterm tower kills
...
\label{prop:wt-Xi-reg-first-residual-quotient-obstruction}
  ...
  \(n_0\) be the first \(\hbar_{\mathrm{QME}}\)-order at which the lower-order choices
  ...
  \[
    ...
    H^1\bigl(\mathcal C^{\bullet,\mathrm{reg}}_{w,M}(I)\bigr)
  \]
  are the quotient image of the closed-exchange obstruction
  \[
...
\begin{proof}
  ...
  \(\eta^{\mathrm{reg}}_{n_0}\) implies nonvanishing of
  \(\beta^{\mathrm{cl},\mathrm{reg}}_{n_0}\).
...
\label{prop:wt-first-regular-density-residual-criterion}
  ...
  \[
    \mathfrak o^{\mathrm{ns}}_{n_0,M}
      =
      [\hbar_{\mathrm{QME}}^{n_0}]\left(
        \operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}},M}
        +d_MC_{<n_0,M}
        +\frac12\{C_{<n_0,M},C_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M}
      \right)
      \in Z^1\bigl(\mathcal Q^\bullet_{w,M}(I)\bigr),
  \]
  ...
  \begin{enumerate}[label=(R\arabic*)]
    ...
    \item The equality with the curvature coefficient above holds in the
      ...
      (\mathcal E_w^M|_I;
      A_{\partial,\hbar_{\mathrm{QME}}}^{\mathrm{pp},w,M}(I))\) before cohomology and
  ...
  \(\operatorname{Ob}^{\mathrm{red}}_{w,\partial,\hbar_{\mathrm{QME}},M}\), the lower-order
...
\label{prop:wt-regular-density-curvature-criterion}
  ...
  subcomplex.  Let
  \[
    \operatorname{Ob}^{\mathrm{red,reg}}_{w,\partial,\hbar_{\mathrm{QME}},M}
      =
    \operatorname{ev}_{\mathcal G_M}
      \operatorname{Curv}_{\mathbf K}
      \bigl(\kappa^{\mathrm{graph,reg},M}_{\hbar_{\mathrm{QME}},w,I}\bigr)
      \in
    \mathcal Q^1_{w,M}(I)
  \]
  ...
  \[
    \mathfrak o^{\mathrm{ns,reg}}_{n_0,M}
      ...
      [\hbar_{\mathrm{QME}}^{n_0}]\left(
        ...
        +\frac12
          \{C^{\mathrm{reg}}_{<n_0,M},
            C^{\mathrm{reg}}_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M}
      \right).
  \]
  Then
  \[
  ...
  coefficient of \(\hbar_{\mathrm{QME}}^{n_0}\), there is a finite indexing set
  ...
  \[
    ...
    \sum_{\alpha\in\mathcal A^{\mathrm{reg}}_{n_0,M}}
      ...
      +[\hbar_{\mathrm{QME}}^{n_0}]d_MC^{\mathrm{reg}}_{<n_0,M}
      +\frac12[\hbar_{\mathrm{QME}}^{n_0}]
        \{C^{\mathrm{reg}}_{<n_0,M},
          C^{\mathrm{reg}}_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M},
  \]
  ...
  \[
    ...
    \operatorname{Ob}^{\mathrm{red,reg}}_{w,\partial,\hbar_{\mathrm{QME}},M}
      +
    \operatorname{Ob}^{\mathrm{red},\neg\mathrm{rd}}_{w,\partial,\hbar_{\mathrm{QME}},M},
    \qquad
    C_{<n_0,M}
      =
    C^{\mathrm{reg}}_{<n_0,M}
  ...
  \begin{aligned}
    \eta^{\mathrm{reg}}_{n_0,M}
      =
      \Bigl[
      q_M[\hbar_{\mathrm{QME}}^{n_0}]\Bigl(
        &\operatorname{Ob}^{\mathrm{red},\neg\mathrm{rd}}_{w,\partial,\hbar_{\mathrm{QME}},M}
        +d_MC^{\neg\mathrm{rd}}_{<n_0,M} \\
        &+\{C^{\mathrm{reg}}_{<n_0,M},
             C^{\neg\mathrm{rd}}_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M}
        +\frac12
          \{C^{\neg\mathrm{rd}}_{<n_0,M},
             C^{\neg\mathrm{rd}}_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M}
      \Bigr)\Bigr]
  ...
  distributional current counterterm required by such a label, is the exact
...
\begin{proof}
  ...
  BV-bracket terms appearing in the finite graph differential, so the
  ...
  \(\hbar_{\mathrm{QME}}^{n_0}\)-coefficient lies in
  \(\mathcal X^{\bullet,\mathrm{reg}}_{w,M}(I)\).
...
\label{thm:wt-wavefront-singular-current-eta-obstruction}
  ...
  quotient class obstructs a choice-free regular-density representative of
  ...
  \[
    \mathfrak o^{\mathrm{ns,WF}}_{n_0,M}
      =
      [\hbar_{\mathrm{QME}}^{n_0}]\left(
        \operatorname{Ob}^{\mathrm{red,WF}}_{w,\partial,\hbar_{\mathrm{QME}},M}
        +d_MC^{\mathrm{WF}}_{<n_0,M}
        +\frac12
          \{C^{\mathrm{WF}}_{<n_0,M},
            C^{\mathrm{WF}}_{<n_0,M}\}_{\hbar_{\mathrm{QME}},M}
      \right)
      \in Z^1\bigl(\mathcal Q^\bullet_{w,M}(I)\bigr),
  \]
...
\label{thm:wt-wavefront-distributional-primitive-cone}
  ...
  \[
    ...
    \mathcal Q^\bullet_{w,M}(I)
      \widehat{\oplus}
    \C[[\hbar_{\mathrm{QME}}]]\cdot p^{\mathrm{WF}}_{n_0,M},
    \qquad |p^{\mathrm{WF}}_{n_0,M}|=0,
  \]
  completed in the same \(\hbar_{\mathrm{QME}}\)-adic, scalar-contact, and
  finite-window filtrations as \(\mathcal Q^\bullet_{w,M}(I)\).  The new
  generator is placed in \(\hbar_{\mathrm{QME}}\)-order \(n_0\), in the scalar-contact
...
\begin{proof}
  ...
  wavefront cone, \(\hbar_{\mathrm{QME}}\)-order, scalar-contact order, and finite
...
\begin{proof}
  ...
  \(C^{w_0}_{\mathfrak h}\) for all \(w_0\) gives a nonzero
  ...
  Suppose a \(q\to1^+\) endpoint existed in the strict pair.  Compatibility
  with finite-window restriction identifies its finite-window projection
  with the limit of the weighted finite-window kernels.  After the
  weighted bases are identified with the unweighted basis, every such
  finite-window kernel is exactly \(C^{w_0}_{\mathfrak h}\), independent
  of \(q\).  Such an endpoint therefore gives a tensor \(K\) with the
  obstructed projections above.
...
\label{thm:wt-ce-pv-qtoone-stabilization}
  ...
  \((\mathfrak h^0_{\leq M},D^0_{\leq M})\).  Then
  \[
