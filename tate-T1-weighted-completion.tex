...
Theorem~\ref{thm:strict-unweighted-no-go} sharpens this to the
...
\begin{rmk}[Strict Tate continuous duality]\label{rmk:diagnosis-unweighted-casimir}
  ...
  not in $\bigoplus_d H_d^\vee=D_\oplus$.
\end{rmk}
...
\begin{rmk}[Costello's coefficient requirements]\label{rmk:costello-coefficient-requirements}
  ...
  not a formal consequence of any coefficient enlargement; it is the
  residual locality/QME problem of
  Problem~\ref{prob:weighted-rg-locality}.
\end{rmk}

\subsection*{Degree weights and the canonical completion \(B_w\)}
...
\begin{proof}
  ...
  the finite-window restriction, and a uniform inequality without it
  is false.
...
\begin{rmk}[Choice of exponential weight]\label{rmk:wt-canonical-weight}
  ...
  module, not a symplectic automorphism of the formal polydisk.  Thus the
...
\begin{thm}[Canonical weighted-Tate habitat]\label{thm:wt-canonical-habitat}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item\label{wt:canon:propagator}
      ...
      $C_{\mathfrak h,w}$ requires no counterterm; it is the fixed
...
\begin{defn}[Category of admissible weighted-Tate completions]\label{defn:wt-Adm-category}
  ...
  \begin{enumerate}[label=(A\arabic*)]
    ...
    \item\label{adm:rg} \emph{(graphwise RG boundedness)} for every
      ...
      supplied by \ref{w:W1} and \ref{w:W2}.
  \end{enumerate}
...
\begin{proof}
  By Theorem~\ref{thm:wt-canonical-habitat}, $B_w\subset B'$ for every
...
\begin{proof}
  ...
  The residue convention fixes the formal residue density and inserts no
  smooth normal Euler factor; the Euler convention pushes along the
...
\begin{cor}[Equivariant Costello QME obstruction vector]\label{cor:wt-omega-denominators-qme-boundary}
  Theorem~\ref{thm:wt-omega-kernel-admissibility} supplies the
  coefficient Casimir and the localized normal contracting kernel in
  the weighted finite-window category, the input needed to form an
  equivariant Costello kernel complex.  Once equivariant brane-defect
  Costello data are supplied, in the sense of
...
\begin{cor}[Cotangent-lift inside the weighted coefficient category]\label{thm:wt-cotangent-lift}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item the central-operation cochain $\Phi$ of
      ...
      $$\Phi^w\colon C^\bullet_{\mathrm{CE},\mathrm{cont}}(\mathfrak g_w)
        ...
        (\mathfrak h_w)\bigr).$$
  \end{enumerate}
  Conditional on Problem~\ref{prob:weighted-rg-locality}, the all-order
  lift $\Phi_{\hbar,\mathrm{cond}}^w$ is
  Proposition~\ref{prop:wt-qme-lift-obstruction-vanishing}.
\end{cor}

\begin{proof}
  This is Theorem~\ref{thm:wt-canonical-habitat} parts
  ...
  of Definition~\ref{defn:bracket-kernel-admissible}; it is not the
  strict continuous duality statement of
...
\begin{proof}
  ...
  and to the principal-part current extension; on their own they do not
  decide compatibility with
...
\begin{proof}
  ...
  therefore give a tensor \(K\) with the obstructed projections above.
...
\begin{proof}
  ...
  Theorem~\ref{thm:strict-unweighted-no-go}; the obstruction is
...
\begin{ex}[Super-exponential bracket obstruction]\label{ex:bad-weight-not-regulator}
  ...
  \ref{w:W1} is violated.  The Poisson bracket is not bracket-tame in
  that coefficient topology, and the graphwise constants in
  Proposition~\ref{prop:wt-rg-tame} need not be finite uniformly in the
  degrees used by the interaction.  Theorem~\ref{thm:wt-regulator-independence-admissible}
  does not compare such a topology with the admissible weighted theory.
\end{ex}
