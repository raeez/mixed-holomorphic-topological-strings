...
\emph{Closure-layer organizing question.}  In which coefficient
category does the formal Casimir
$C_{\mathfrak h}=\sum_d\sum_i e_{d,i}\otimes e_d^i$ converge as a BV
inverse pairing for the unreduced cotangent lift datum of
Problem~\ref{prob:formal-factorization-center}~item~(1)?

\emph{Named obstruction class.}  In the strict Tate pair
$(\mathfrak h,\mathfrak h^\vee_{\mathrm{cont}})
=(\prod_{d\geq1}H_d,\bigoplus_{d\geq1}H_d^\vee)$, every element of the
second factor has support in finitely many degrees, while
$C_{\mathfrak h}$ has nonzero $H_d^\vee$-component in every $d$.  The
obstruction is
\[
  \mathfrak o_{\mathrm{Cas}}=\bigl(C^M_{\mathfrak h}\bigr)_{M\geq1}
  \in Q_{\mathrm{Cas}}
  =\frac{\varprojlim_M(H_{\leq M}\otimes H_{\leq M}^\vee)}
        {\operatorname{im}\bigl(\mathfrak h\widehat\otimes
        \mathfrak h^\vee_{\mathrm{cont}}\bigr)},
\]
nonzero by Lemma~\ref{lem:tate-casimir-obstruction} and strengthened
by Theorem~\ref{thm:strict-unweighted-no-go}: no tensor in
$\mathfrak h_\Pi\widehat\otimes D_\oplus$ has the finite Casimirs
$C^{w_0}_{\mathfrak h}$ as its $w_0$-projections.  The strict pair
fails Costello requirement~\ref{costello:CC1}.

\emph{Unique survivor.}  Theorem~\ref{thm:wt-cotangent-lift}: a
weighted Tate envelope
$(\mathfrak h_w,\mathfrak h^{\vee,w}_{\mathrm{cont}})$, with degree
weight $w$ satisfying the bracket-tame, windowwise
coadjoint-continuity, and Mittag--Leffler coevaluation conditions of
Definition~\ref{defn:wt-degree-weight}, in which (a) the Casimir
converges; (b) the Costello heat-kernel propagator extends; (c) every
fixed Costello graph stays in the weighted category; (d) the cochain
CE/PV map extends continuously.

\emph{Construction that closes $\mathfrak o_{\mathrm{Cas}}$.}
Replace the direct-sum dual $\bigoplus_dH_d^\vee$ by the symmetric
Mittag--Leffler product
$\mathfrak h^{\vee,w}_{\mathrm{cont}}=\varprojlim_{w_0}\prod_{d\leq
w_0}w(d)^{-1}H_d^\vee$ of
Definition~\ref{defn:wt-coefficient-pair}; the diagonal-rescaled
Casimir
$C_{\mathfrak h,w}=\sum_d\sum_i(w(d)e_{d,i})\otimes(w(d)^{-1}e^i_d)$
is then a strict Mittag--Leffler kernel
(Proposition~\ref{prop:wt-casimir-convergence}).

\emph{Residual.}  The mixed brane-defect QME obstruction
$\operatorname{Ob}^{\mathrm{QME}}_{\Omega,w}$ of
Corollary~\ref{cor:wt-omega-denominators-qme-boundary} and the
locality compatibility class of
Problem~\ref{prob:weighted-rg-locality}: the closure layer leaves a
coefficient-kernel theorem, not a Costello QME vanishing.

\subsection*{Strict Tate pair}

The strict product/direct-sum Tate pair
\(\mathfrak h=\prod_dH_d\),
\(\mathfrak h^\vee_{\mathrm{cont}}=\bigoplus_dH_d^\vee\) carries the
formal Casimir on the wrong side of the duality: the second tensor
factor lies in \(\prod_dH_d^\vee\), not in \(\bigoplus_dH_d^\vee\).
The weighted Mittag--Leffler completion below replaces the direct-sum
dual with a symmetric product coefficient module.
...
\begin{rmk}[Costello's coefficient requirements]\label{rmk:costello-coefficient-requirements}
  ...
  exists as a continuous operator. The whole kernel exists as
